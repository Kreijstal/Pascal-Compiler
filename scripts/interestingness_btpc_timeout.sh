...
# - Gwinn GPC must compile the same source, and the resulting binary
...
ASM_FILE="$TMP_DIR/gpc_out.s"
BIN_GPC="$TMP_DIR/gpc_out.bin"
...
set +e
printf "%s" "$STDIN_CONTENT" | timeout 1s "$BIN_GPC" >/dev/null 2>&1
status=$?
set -e
