...
static bool write_single_node(FILE *f, const ast_t *node) {
  if (!write_u32(f, (uint32_t)node->typ))
    return false;
  if (!write_i32(f, (int32_t)node->line))
    return false;
  if (!write_i32(f, (int32_t)node->col))
    return false;
  if (!write_i32(f, (int32_t)node->index))
    return false;
  ...
  if (node->sym != NULL && node->sym->name != NULL) {
    ...
    if (!write_u32(f, len))
      return false;
    if (fwrite(node->sym->name, 1, len, f) != len)
      return false;
  } else {
    if (!write_u8(f, 0))
      return false;
  ...
  if (!write_u8(f, node->child != NULL ? 1 : 0))
    return false;
  if (!write_u8(f, node->next != NULL ? 1 : 0))
    return false;
...
bool ast_cache_save(const char *cache_path, const ast_t *root,
  ...
  /* Header */
  if (fwrite(MAGIC, 1, MAGIC_LEN, f) != MAGIC_LEN)
    goto fail;
  if (!write_u32(f, AST_CACHE_VERSION))
    goto fail;

  /* Preprocessed source */
  uint32_t pp_len = (uint32_t)preprocessed_len;
  if (!write_u32(f, pp_len))
    goto fail;
  if (preprocessed_len > 0) {
    if (fwrite(preprocessed_buf, 1, preprocessed_len, f) != preprocessed_len)
      goto fail;
  }

  /* AST */
  if (!write_node(f, root))
    goto fail;
...
static ast_t *read_single_node_fields(FILE *f) {
  ...
  uint8_t has_sym;
  if (!read_u8(f, &has_sym)) {
    free(node);
    return NULL;
  }
  if (has_sym) {
    uint32_t sym_len;
    if (!read_u32(f, &sym_len)) {
      free(node);
      return NULL;
    }
...
static ast_t *read_node(FILE *f) {
  ...
  for (;;) {
    ...
    if (!read_u8(f, &has_child)) {
      free_ast(node);
      goto fail;
    }
    if (!read_u8(f, &has_next)) {
      free_ast(node);
      goto fail;
    }
