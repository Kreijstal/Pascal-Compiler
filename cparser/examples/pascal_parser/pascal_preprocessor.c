...
typedef struct {
  char *name;
  char *value;
  bool is_macro; // true if explicitly defined with := (should expand), false if
                 // just defined
} DefineEntry;
...
struct PascalPreprocessor {
  ...
  /* For declared() support - points to current output buffer during
   * preprocessing */
...
static const char *try_expand_macro(PascalPreprocessor *pp, const char *input,
...
static bool ensure_capacity(void **buffer, size_t element_size,
                            size_t *capacity, size_t needed);
static bool emit_line_directive(StringBuilder *sb, int line,
                                const char *filename);
...
static bool push_conditional(ConditionalStack *stack, bool parent_active,
                             bool condition, const char *filename, int line);
...
static bool handle_directive(PascalPreprocessor *pp, const char *filename,
                             const char *input, size_t length, size_t *index,
                             bool paren_style, ConditionalStack *conditions,
                             StringBuilder *output, int depth, int current_line,
                             char **error_message);
static bool preprocess_buffer_internal(PascalPreprocessor *pp,
                                       ...
                                       char **error_message);
static bool read_file_contents(const char *filename, char **buffer,
                               size_t *length, char **error_message);
static bool define_symbol(PascalPreprocessor *pp, const char *symbol);
static bool define_symbol_const(PascalPreprocessor *pp, const char *name,
                                const char *value);
...
static bool find_const_integer_in_source(PascalPreprocessor *pp,
                                         const char *symbol,
                                         int64_t *out_value);
static void trim(char **begin, char **end);
static char *duplicate_range(const char *start, const char *end);
static void extract_directory(const char *filename, char *buffer,
                              size_t buffer_size);
static bool resolve_include_path(const PascalPreprocessor *pp,
                                 const char *current_file,
                                 const char *directive_path,
                                 char **result_path);
static bool parse_identifier(const char *start, const char *end,
                             char **out_identifier);
static void uppercase(char *str);
static bool evaluate_if_directive(PascalPreprocessor *pp,
                                  const char *expression, bool *result,
                                  char **error_message);

// Expression parser functions returning int64_t
static bool parse_expression(const char **cursor, int64_t *value,
                             PascalPreprocessor *pp, char **error_message);
static bool parse_simple_expression(const char **cursor, int64_t *value,
                                    PascalPreprocessor *pp,
                                    char **error_message);
static bool parse_term(const char **cursor, int64_t *value,
                       PascalPreprocessor *pp, char **error_message);
static bool parse_factor(const char **cursor, int64_t *value,
                         PascalPreprocessor *pp, char **error_message);
static bool is_keyword_operator(const char *cursor, const char *keyword,
                                size_t keyword_len);
static const char *get_symbol_value(const PascalPreprocessor *pp,
                                    const char *symbol);
static const char *get_macro_value(const PascalPreprocessor *pp,
                                   const char *symbol);
...
bool pascal_preprocessor_add_include_path(PascalPreprocessor *pp,
  ...
  if (pp->include_path_count >= pp->include_path_capacity) {
    size_t new_capacity =
        pp->include_path_capacity == 0 ? 8 : pp->include_path_capacity * 2;
    char **new_paths =
        realloc(pp->include_paths, new_capacity * sizeof(char *));
    if (!new_paths)
      return false;
...
static void record_included_file(PascalPreprocessor *pp, const char *path) {
  ...
  if (pp->included_file_count >= pp->included_file_capacity) {
    size_t new_cap =
        pp->included_file_capacity == 0 ? 16 : pp->included_file_capacity * 2;
...
char *pascal_preprocess_buffer(PascalPreprocessor *pp, const char *filename,
  ...
  if (!preprocess_buffer_internal(pp, filename, input, length, &conditions,
  ...
  if (conditions.size != 0) {
    ...
    if (msg) {
      int offset =
          snprintf(msg, total_len, "unterminated conditional in '%s':\n",
                   filename ? filename : "<buffer>");
      for (size_t i = 0; i < conditions.size; ++i) {
        ConditionalFrame *frame = &conditions.items[i];
        offset += snprintf(
            msg + offset, total_len - offset, "  Open conditional at %s:%d\n",
      ...
      *error_message = msg;
      ...
      set_error(error_message, "unterminated conditional in '%s'",
                filename ? filename : "<buffer>");
    }
...
char *pascal_preprocess_file(PascalPreprocessor *pp, const char *filename,
  ...
  char *result = pascal_preprocess_buffer(pp, filename, buffer, length,
                                          out_length, error_message);
  free(buffer);
  return result;
}
...
static int should_skip_line(const char *fname, const char *line_start) {
  ...
  if (len >= 11 && strcmp(fname + len - 11, "systemh.inc") == 0) {
    if (strncmp(line_start, "function get_frame:pointer;", 27) == 0)
      return 1;
    if (strncmp(line_start, "Function Get_pc_addr : CodePointer;", 35) == 0)
      return 1;
  }

  if (len >= 10 && strcmp(fname + len - 10, "x86_64.inc") == 0) {
    if (strncmp(line_start,
                "function get_frame:pointer;assembler;nostackframe;", 50) == 0)
      return 2;
  }
...
static bool preprocess_buffer_internal(PascalPreprocessor *pp,
  ...
  if (depth > MAX_INCLUDE_DEPTH) {
    return set_error(error_message,
                     "maximum include depth exceeded while processing '%s'",
                     filename ? filename : "<buffer>");
  }
  ...
  bool need_line_directive =
      true; /* Need to emit line directive for first content */
  ...
  for (size_t i = 0; i < length; ++i) {
    ...
    if ((i == 0 || input[i - 1] == '\n') && !pp->flatten_only &&
      ...
      if (skip_block_mode) {
        ...
        size_t j = i;
        while (j < length && isspace((unsigned char)input[j]) &&
               input[j] != '\n')
          j++;
    ...
    bool in_comment = (brace_comment_depth > 0) || (paren_comment_depth > 0) ||
                      in_line_comment;

    if (!in_string && !in_comment) {
      if (c == '{' && i + 1 < length && input[i + 1] == '$') {
        if (!handle_directive(pp, filename, input, length, &i, false,
                              conditions, output, depth, current_line,
                              error_message)) {
          return false;
        }
        continue;
      ...
      if (c == '(' && i + 2 < length && input[i + 1] == '*' &&
          input[i + 2] == '$') {
        if (!handle_directive(pp, filename, input, length, &i, true, conditions,
                              output, depth, current_line, error_message)) {
          return false;
        }
        continue;
      }
      if (c == '{') {
        ...
        if (!has_newline && !has_nested && close < length &&
            input[close] == '}') {
                ...
                if (p == end) {
                  ...
                  if (next < length && input[next] == '(') {
                    if (pp->flatten_only || current_branch_active(conditions)) {
                      ...
                      if (need_line_directive && filename != NULL &&
                          depth > 0) {
                        if (i == 0 || input[i - 1] == '\n' ||
                            last_emitted_line == 0) {
                      ...
                      if (should_emit_directive) {
                        if (!emit_line_directive(output, current_line,
                                                 filename)) {
    ...
    if (pp->flatten_only || current_branch_active(conditions)) {
      /* Emit line directive at start of included file, or when there's a gap in
       * line numbers */
      bool should_emit_directive = false;
      ...
      if (!pp->flatten_only && !in_comment && !in_string && pp->macro_enabled) {
        ...
        const char *macro_value =
            try_expand_macro(pp, input, length, i, &identifier_len);

        if (macro_value) {
          // Output the macro value instead of the identifier (single-pass
          // expansion)
      ...
      /* Content is being skipped - mark that we need a line directive when we
       * resume */
      need_line_directive = true;
...
static bool handle_directive(PascalPreprocessor *pp, const char *filename,
  ...
  while (end < length) {
    ...
    if (paren_style && end + 1 < length && input[end] == '*' &&
        input[end + 1] == ')') {
      closed = true;
      break;
    }
  ...
  if (!closed) {
    return set_error(error_message, "unterminated directive in '%s'",
                     filename ? filename : "<buffer>");
  }
  ...
  if (!parse_identifier(content_begin, content_end, &keyword)) {
    free(content);
    return set_error(error_message, "malformed compiler directive in '%s'",
                     filename ? filename : "<buffer>");
  }
  uppercase(keyword);
  ...
  if (strcmp(keyword, "I") == 0) {
    ...
    // If the first character is + or -, treat it as a switch directive (e.g.
    // {$I-,Q-}) This takes precedence over treating it as a filename.
    if (*cursor == '+' || *cursor == '-') {
  ...
  if (is_io_check_directive) {
    ...
    bool is_include =
        (strcmp(keyword, "I") == 0 || strcmp(keyword, "INCLUDE") == 0);
    if (is_include) {
      handled = true;
      ...
      if (!path_token || path_token[0] == '\0') {
        bool err = set_error(error_message, "empty include directive in '%s'",
                             filename ? filename : "<buffer>");
        free(keyword);
        free(content);
        free(path_token);
      ...
      // Handle special FPC include macros like %DATE%, %TIME%, %FPCVERSION%,
      // etc.
      ...
      if (!resolve_include_path(pp, filename, path_token, &resolved_path)) {
        bool err =
            set_error(error_message, "unable to resolve include '%s' in '%s'",
                      path_token, filename ? filename : "<buffer>");
        free(keyword);
        free(content);
        free(path_token);
        return err;
      ...
      if (!read_file_contents(resolved_path, &include_buffer, &include_length,
                              error_message)) {
        bool retried = false;
        if (strchr(path_token, '.') == NULL) {
          ...
          if (with_inc) {
            ...
            if (resolve_include_path(pp, filename, with_inc, &resolved2)) {
              ...
              retried = read_file_contents(resolved2, &include_buffer,
                                           &include_length, error_message);
              free(resolved2);
            }
            free(with_inc);
          }
      ...
      bool ok = preprocess_buffer_internal(pp, resolved_path, include_buffer,
                                           include_length, conditions, output,
                                           depth + 1, error_message);
      ...
      *index = directive_end;
      if (skip_trailing_newline && *index + 1 < length &&
          input[*index + 1] == '\n')
        (*index)++;
      else if (skip_trailing_newline && *index + 1 < length &&
               input[*index + 1] == '\r') {
        (*index)++;
    ...
    *index = directive_end;
    ...
    if (!branch_active) {
      ...
      // Handle special FPC include macros like %DATE%, %TIME%, %FPCVERSION%,
      // etc. These return string literals instead of including files.
      if (path_token[0] == '%' && path_token[strlen(path_token) - 1] == '%') {
        char special_value[64] = "";
        bool is_special = false;
      ...
      if (!read_file_contents(resolved_path, &include_buffer, &include_length,
                              error_message)) {
        // Fallback: if directive used bare identifier without extension, try
        // appending .inc
        bool retried = false;
        if (strchr(path_token, '.') == NULL) {
          size_t base_len = strlen(path_token);
    ...
    if (branch_active) {
      ...
      if (def_begin[0] == '\0') {
        ...
        free(keyword);
        free(content);
        return set_error(error_message, "malformed define directive in '%s'",
                         filename ? filename : "<buffer>");
      }
    ...
    if (branch_active) {
      ...
      if (!parse_identifier(rest, content_end, &symbol) || symbol[0] == '\0') {
        ...
        free(keyword);
        free(content);
        return set_error(error_message, "malformed undef directive in '%s'",
                         filename ? filename : "<buffer>");
      }
    ...
    if (!parse_identifier(rest, content_end, &symbol) || symbol[0] == '\0') {
      ...
      free(keyword);
      free(content);
      return set_error(error_message, "malformed conditional directive in '%s'",
                       filename ? filename : "<buffer>");
    }
    ...
    if (!push_conditional(conditions, parent_active, defined, filename,
                          current_line)) {
      free(symbol);
      free(keyword);
      free(content);
    ...
    if (!push_conditional(conditions, parent_active, false, filename,
                          current_line)) {
      free(keyword);
      free(content);
      return set_error(error_message, "out of memory");
    ...
    if (!evaluate_if_directive(pp, rest, &cond_value, error_message)) {
      ...
      if (error_message && *error_message) {
        ...
        int len = snprintf(NULL, 0, "unsupported {$IF} expression: %s",
                           specific_error);
        if (len > 0) {
          wrapped_error = malloc(len + 1);
          if (wrapped_error) {
            snprintf(wrapped_error, len + 1, "unsupported {$IF} expression: %s",
                     specific_error);
            free(specific_error);
        ...
        return set_error(error_message, "unsupported {$IF} expression in '%s'",
                         filename ? filename : "<buffer>");
      }
      return false;
    }
    bool parent_active = current_branch_active(conditions);
    if (!push_conditional(conditions, parent_active, cond_value, filename,
                          current_line)) {
      free(keyword);
      free(content);
      return set_error(error_message, "out of memory");
  ...
  } else if (strcmp(keyword, "ELSEIFDEF") == 0 ||
             strcmp(keyword, "ELSEIFNDEF") == 0) {
    handled = true;
    ConditionalFrame *frame = peek_conditional(conditions);
    if (!frame) {
      free(keyword);
    ...
    if (strcmp(keyword, "MODE") == 0 || strcmp(keyword, "MODESWITCH") == 0 ||
        strcmp(keyword, "CALLING") == 0 || strcmp(keyword, "CODEPAGE") == 0 ||
        ...
        strcmp(keyword, "HINTS") == 0 || strcmp(keyword, "WARNINGS") == 0 ||
        strcmp(keyword, "NOTES") == 0 || strcmp(keyword, "WARN") == 0 ||
        strcmp(keyword, "FATAL") == 0 || strcmp(keyword, "ERROR") == 0 ||
        strcmp(keyword, "MESSAGE") == 0 || strcmp(keyword, "STOP") == 0) {
  ...
  if (branch_active && handled && strcmp(keyword, "INCLUDE") != 0 &&
      strcmp(keyword, "I") != 0) {
    ...
    if (!already_preserved) {
      // To avoid token merging when directives appear in-line, insert a single
      // space
      if (!string_builder_append_char(output, ' ')) {
        free(keyword);
        free(content);
...
static bool read_file_contents(const char *filename, char **buffer,
  ...
  if (size < 0) {
    fclose(file);
    return set_error(error_message, "failed to determine size of '%s'",
                     filename);
  }
...
static bool string_builder_append_char(StringBuilder *sb, char c) {
  if (!ensure_capacity((void **)&sb->data, sizeof(char), &sb->capacity,
                       sb->length + 2)) {
    return false;
  }
  sb->data[sb->length++] = c;
...
/* Emit a #line directive to track source file positions across include
 * boundaries. Format: {#line <line_number> "<filename>"} The parser will
 * recognize this and reset its line counter accordingly. */
static bool emit_line_directive(StringBuilder *sb, int line,
                                const char *filename) {
  if (filename == NULL) {
    return true;
...
static bool push_conditional(ConditionalStack *stack, bool parent_active,
                             bool condition, const char *filename, int line) {
  if (!ensure_capacity((void **)&stack->items, sizeof(ConditionalFrame),
...
static const char *try_expand_macro(PascalPreprocessor *pp, const char *input,
  ...
  *out_identifier_len = 0;
  ...
  // If the previous character is alphanumeric or underscore, we're not at the
  // start
  if (pos > 0) {
  ...
  // FPC macros expand just the macro name, e.g., UT in UT.ARG_MAX expands to
  // UnixType.ARG_MAX
  size_t start = pos;
  size_t end = pos;
  while (end < length &&
         (isalnum((unsigned char)input[end]) || input[end] == '_')) {
    ++end;
  }
...
static char *expand_macro_value_once(PascalPreprocessor *pp,
                                     const char *value) {
  if (!pp || !value) {
    return NULL;
  }
  ...
  for (size_t i = 0; i < length; ++i) {
    ...
    if (!in_string) {
      size_t identifier_len = 0;
      const char *macro_value =
          try_expand_macro(pp, value, length, i, &identifier_len);
      if (macro_value) {
...
static bool define_symbol(PascalPreprocessor *pp, const char *symbol) {
  ...
  bool is_macro =
      false; // Track if this is an explicit macro (with := assignment)
  ...
  if (!ensure_capacity((void **)&pp->defines, sizeof(DefineEntry),
                       &pp->define_capacity, pp->define_count + 1)) {
    free(name_part);
    free(value_part);
    return false;
  }
...
/* Define a symbol with a value but is_macro = false, so it's available for
 * {$if} evaluation via get_symbol_value() but NOT for text replacement via
 * get_macro_value(). */
static bool define_symbol_const(PascalPreprocessor *pp, const char *name,
                                const char *value) {
  if (!pp || !name || !value)
...
static bool undefine_symbol(PascalPreprocessor *pp, const char *symbol) {
  ...
  for (size_t i = 0; i < pp->define_count; ++i) {
    if (strcmp(pp->defines[i].name, upper) == 0) {
      ...
      // Zero out the moved-from slot to be safe, though count decrement handles
      // it
...
static bool symbol_is_defined(const PascalPreprocessor *pp,
                              const char *symbol) {
  if (!pp || !symbol) {
    return false;
  }
 ...
 * Handles: integers, identifier references (recursive), +, -, *, div, mod,
 * parentheses. This is a mini expression evaluator for const initializer
 * expressions found in source. */
static bool eval_const_expr(PascalPreprocessor *pp, const char **cursor,
                            const char *end, int64_t *value, int depth);
...
static bool resolve_builtin_type_size(PascalPreprocessor *pp,
                                      const char *type_name_input,
                                      int64_t *size) {
  ...
  if (strcmp(type_name, "NATIVEINT") == 0 ||
      strcmp(type_name, "NATIVEUINT") == 0 ||
      ...
      strcmp(type_name, "TCONSTPTRUINT") == 0 ||
      strcmp(type_name, "TCONSTPTRINT") == 0 ||
    ...
    *size = cpu64 ? 8 : 4;
    found = true;
             ...
             strcmp(type_name, "TSYSPARAM") == 0 ||
             strcmp(type_name, "ALUUINT") == 0 ||
             strcmp(type_name, "ALUSINT") == 0 ||
             strcmp(type_name, "VALSINT") == 0 ||
             strcmp(type_name, "VALUINT") == 0 ||
             strcmp(type_name, "AINTMAX") == 0 ||
             strcmp(type_name, "TBITSBASE") == 0 ||
             strcmp(type_name, "TUNSIGNEDINTTYPE") == 0 ||
             strcmp(type_name, "TSIGNEDINTTYPE") == 0 ||
             strcmp(type_name, "HANDLE") == 0 ||
             strcmp(type_name, "THANDLE") == 0 ||
             strcmp(type_name, "TLARGEINTEGER") == 0 ||
    ...
    *size = 8;
    found = true;
             ...
             strcmp(type_name, "CARDINAL") == 0 ||
             strcmp(type_name, "DWORD") == 0 ||
             strcmp(type_name, "UINT32") == 0 ||
             strcmp(type_name, "LONGWORD") == 0 ||
             strcmp(type_name, "LONGBOOL") == 0) {
    *size = 4;
    found = true;
  } else if (strcmp(type_name, "SMALLINT") == 0 ||
    ...
    *size = 2;
    found = true;
  } else if (strcmp(type_name, "SHORTINT") == 0 ||
             strcmp(type_name, "INT8") == 0 || strcmp(type_name, "BYTE") == 0 ||
             strcmp(type_name, "UINT8") == 0 ||
             strcmp(type_name, "CHAR") == 0 ||
             strcmp(type_name, "BOOLEAN") == 0 ||
             strcmp(type_name, "ANSICHAR") == 0 ||
             strcmp(type_name, "BYTEBOOL") == 0) {
    *size = 1;
    found = true;
  } else if (strcmp(type_name, "VALREAL") == 0 ||
             strcmp(type_name, "EXTENDED") == 0 ||
             strcmp(type_name, "LONGDOUBLE") == 0 ||
             strcmp(type_name, "BESTREALREC") == 0 ||
             strcmp(type_name, "TEXTENDED80REC") == 0 ||
             strcmp(type_name, "BESTREAL") == 0) {
    *size = 10;
    found = true;
    ...
    *size = 4;
    found = true;
  } else if (strcmp(type_name, "COMP") == 0 ||
             strcmp(type_name, "CURRENCY") == 0) {
    *size = 8;
    found = true;
  } else if (strcmp(type_name, "TIME_T") == 0 ||
             strcmp(type_name, "CLONG") == 0 ||
             strcmp(type_name, "CULONG") == 0 ||
             strcmp(type_name, "CLONGLONG") == 0 ||
             strcmp(type_name, "CULONGLONG") == 0 ||
             strcmp(type_name, "CSIZE_T") == 0 ||
             strcmp(type_name, "CSSIZE_T") == 0 ||
             strcmp(type_name, "COFF_T") == 0 ||
             strcmp(type_name, "OFF_T") == 0) {
    *size = 8;
    found = true;
  } else if (strcmp(type_name, "CINT") == 0 ||
             strcmp(type_name, "CUINT") == 0 ||
             strcmp(type_name, "CINT32") == 0 ||
             strcmp(type_name, "CUINT32") == 0) {
    *size = 4;
    found = true;
  } else if (strcmp(type_name, "CSHORT") == 0 ||
             strcmp(type_name, "CUSHORT") == 0 ||
             strcmp(type_name, "CINT16") == 0 ||
             strcmp(type_name, "CUINT16") == 0) {
    *size = 2;
    found = true;
  } else if (strcmp(type_name, "CCHAR") == 0 ||
             strcmp(type_name, "CSCHAR") == 0 ||
             strcmp(type_name, "CUCHAR") == 0 ||
             strcmp(type_name, "CINT8") == 0 ||
             strcmp(type_name, "CUINT8") == 0) {
    *size = 1;
    found = true;
  } else if (strcmp(type_name, "CODEPTRUINT") == 0 ||
             strcmp(type_name, "CODEPTRINT") == 0 ||
...
static bool compute_packed_record_size_from_source(PascalPreprocessor *pp,
                                                   const char *type_name,
                                                   int64_t *size) {
  if (!pp || !pp->current_output || pp->current_output_len == 0)
    return false;
  ...
  while (pos + name_len < src_end) {
    ...
    if (!match ||
        (pos > src && (isalnum((unsigned char)pos[-1]) || pos[-1] == '_')) ||
        (pos + name_len < src_end &&
         (isalnum((unsigned char)pos[name_len]) || pos[name_len] == '_'))) {
      pos++;
      continue;
    }
    ...
    while (p < src_end && isspace((unsigned char)*p))
      p++;
    if (p >= src_end || *p != '=') {
      pos++;
      continue;
    ...
    while (p < src_end) {
      ...
      colon++; /* skip ':' */
      while (colon < src_end && isspace((unsigned char)*colon))
        colon++;

      /* Parse field type name */
      const char *ftype_start = colon;
      while (colon < src_end &&
             (isalnum((unsigned char)*colon) || *colon == '_'))
        colon++;
      if (colon == ftype_start) {
        success = false;
        break;
      }
      ...
      if (!resolve_builtin_type_size(pp, field_type, &field_size)) {
        ...
        /* For type aliases like "StringDataHeaderHashType = uint16", try const
         * lookup */
        bool resolved = false;
        ...
        if (pp->current_output) {
          ...
          while (s + ft_len < src_end) {
            ...
            for (size_t i = 0; i < ft_len; i++) {
              if (ascii_tolower(s[i]) != ascii_tolower(field_type[i])) {
                m = false;
                break;
              }
            }
            if (m &&
                ...
                (s + ft_len >= src_end ||
                 (!isalnum((unsigned char)s[ft_len]) && s[ft_len] != '_'))) {
              const char *after = s + ft_len;
              while (after < src_end && isspace((unsigned char)*after))
                after++;
              if (after < src_end && *after == '=' &&
                ...
                if (after > tstart) {
                  ...
                  if (resolved_type) {
                    ...
                    if (resolve_builtin_type_size(pp, resolved_type,
                                                  &field_size)) {
                      resolved = true;
                    }
                    free(resolved_type);
...
static bool eval_const_factor(PascalPreprocessor *pp, const char **cursor,
                              const char *end, int64_t *value, int depth) {
  if (depth > 32)
    return false;
  ...
  if (*cursor + 3 <= end && ascii_strncasecmp(*cursor, "NOT", 3) == 0 &&
    ...
    *cursor += 3;
    int64_t inner;
    if (!eval_const_factor(pp, cursor, end, &inner, depth + 1))
      return false;
    *value = !inner;
    return true;
  }
  ...
  if (**cursor == '(') {
    ++(*cursor);
    if (!eval_const_expr(pp, cursor, end, value, depth + 1))
      return false;
    skip_spaces_in_range(cursor, end);
  ...
  if (isalpha((unsigned char)**cursor) || **cursor == '_') {
    ...
    if (ascii_strncasecmp(id, "TRUE", 5) == 0 && id_len == 4) {
      *value = 1;
      free(id);
      return true;
    }
    if (ascii_strncasecmp(id, "FALSE", 6) == 0 && id_len == 5) {
      *value = 0;
      free(id);
      return true;
    }
...
static bool eval_const_term(PascalPreprocessor *pp, const char **cursor,
                            const char *end, int64_t *value, int depth) {
  if (!eval_const_factor(pp, cursor, end, value, depth))
    return false;
  for (;;) {
    skip_spaces_in_range(cursor, end);
    if (*cursor >= end)
    ...
    if (**cursor == '*') {
      ++(*cursor);
      int64_t right;
      if (!eval_const_factor(pp, cursor, end, &right, depth))
        return false;
      *value *= right;
    } else if (*cursor + 3 <= end &&
               ascii_strncasecmp(*cursor, "DIV", 3) == 0 &&
      ...
      *value /= right;
               ...
               ascii_strncasecmp(*cursor, "MOD", 3) == 0 &&
               (*cursor + 3 >= end || !isalnum((unsigned char)(*cursor)[3]))) {
      *cursor += 3;
      int64_t right;
      if (!eval_const_factor(pp, cursor, end, &right, depth))
      ...
      *value %= right;
               ...
               ascii_strncasecmp(*cursor, "SHL", 3) == 0 &&
               (*cursor + 3 >= end || !isalnum((unsigned char)(*cursor)[3]))) {
      *cursor += 3;
      int64_t right;
      if (!eval_const_factor(pp, cursor, end, &right, depth))
...
static bool eval_const_additive(PascalPreprocessor *pp, const char **cursor,
                                const char *end, int64_t *value, int depth) {
  if (!eval_const_term(pp, cursor, end, value, depth))
    return false;
  for (;;) {
    skip_spaces_in_range(cursor, end);
    if (*cursor >= end)
    ...
    if (**cursor == '+') {
      ++(*cursor);
      int64_t right;
      if (!eval_const_term(pp, cursor, end, &right, depth))
        return false;
      *value += right;
    } else if (**cursor == '-') {
      ++(*cursor);
 ...
 */
static bool eval_const_comparison(PascalPreprocessor *pp, const char **cursor,
                                  const char *end, int64_t *value, int depth) {
  if (!eval_const_additive(pp, cursor, end, value, depth))
    return false;
  for (;;) {
    skip_spaces_in_range(cursor, end);
    if (*cursor >= end)
    ...
    if (*cursor + 1 < end && **cursor == '<' && (*cursor)[1] == '=') {
      *cursor += 2;
      int64_t right;
      if (!eval_const_additive(pp, cursor, end, &right, depth))
        return false;
...
static bool eval_const_expr(PascalPreprocessor *pp, const char **cursor,
                            const char *end, int64_t *value, int depth) {
  if (!eval_const_comparison(pp, cursor, end, value, depth))
    return false;
  for (;;) {
    skip_spaces_in_range(cursor, end);
    if (*cursor >= end)
    ...
    if (*cursor + 3 <= end && ascii_strncasecmp(*cursor, "AND", 3) == 0 &&
      ...
      *cursor += 3;
      int64_t right;
      if (!eval_const_comparison(pp, cursor, end, &right, depth))
        return false;
      *value = (*value && right) ? 1 : 0;
      ...
      *value = (*value || right) ? 1 : 0;
               ...
               ascii_strncasecmp(*cursor, "NOT", 3) == 0 &&
               (*cursor + 3 >= end || !isalnum((unsigned char)(*cursor)[3]))) {
...
static bool find_const_integer_in_source(PascalPreprocessor *pp,
  ...
  while (pos < src_end) {
    ...
    while (found + sym_len <= src_end) {
      ...
      if (match) {
        /* Check that it's a whole word: char before must not be alnum/_, char
         * after must not be alnum/_ */
        bool word_start =
            (found == src ||
             (!isalnum((unsigned char)found[-1]) && found[-1] != '_'));
        bool word_end = (found + sym_len >= src_end ||
                         (!isalnum((unsigned char)found[sym_len]) &&
                          found[sym_len] != '_'));
...
static bool symbol_is_declared_in_source(const char *source, size_t source_len,
                                         const char *symbol) {
  ...
  static const char *decl_keywords[] = {"var",      "const",     "type",
  ...
  while (pos < end) {
    ...
    if (found_keyword) {
      ...
      /* Now check identifiers after this keyword until we hit another keyword
       * or section */
      while (pos < end) {
        /* Skip whitespace */
        while (pos < end && isspace((unsigned char)*pos)) {
        ...
        const char *section_keywords[] = {
            "var",       "const", "type", "begin",   "end", "implementation",
            "interface", "uses",  "unit", "program", NULL};
        ...
        if (isalpha((unsigned char)*pos) || *pos == '_') {
          ...
          if (ident_len == sym_len &&
              ascii_strncasecmp(ident_start, symbol, sym_len) == 0) {
            return true;
          }
...
static bool resolve_include_path(const PascalPreprocessor *pp,
  ...
  if (pp) {
    for (size_t i = 0; i < pp->include_path_count; ++i) {
      ...
      snprintf(combined, total_len, "%s/%s", pp->include_paths[i],
               directive_path);
   ...
   * We return the relative path anyway so the caller can report a meaningful
   * error. The caller (read_file_contents) will fail to open this path and
   * report the error. */
...
static bool evaluate_if_directive(PascalPreprocessor *pp,
  ...
  if (*cursor != '\0') {
    ...
    return set_error(error_message,
                     "unexpected characters after expression in: %s", preview);
  }
  *result = (value != 0);
  return true;
}
  ...
  while (**cursor) {
    ...
    if (c == '(') {
      ...
      if (is_keyword_operator(*cursor, "DIV", 3) ||
          ...
          is_keyword_operator(*cursor, "XOR", 3))
        break;
    }
    (*cursor)++;
  }
...
static bool parse_simple_expression(const char **cursor, int64_t *value,
  ...
  while (1) {
    ...
    if (**cursor == '+') {
      ...
      op = OP_SUB;
      ++(*cursor);
    } else if (ascii_strncasecmp(*cursor, "OR", 2) == 0 &&
               !isalnum((unsigned char)(*cursor)[2]) && (*cursor)[2] != '_') {
      op = OP_OR;
      *cursor += 2;
    } else if (ascii_strncasecmp(*cursor, "XOR", 3) == 0 &&
               !isalnum((unsigned char)(*cursor)[3]) && (*cursor)[3] != '_') {
      op = OP_XOR;
      *cursor += 3;
    }
...
static bool parse_term(const char **cursor, int64_t *value,
  ...
  while (1) {
    ...
    if (**cursor == '*') {
      ...
      op = OP_DIV; // Treat / as integer division for preprocessor? Or error?
                   // Pascal / is float.
      // For preprocessor, usually integer math. Let's assume integer div.
      ++(*cursor);
    } else if (ascii_strncasecmp(*cursor, "DIV", 3) == 0 &&
               !isalnum((unsigned char)(*cursor)[3]) && (*cursor)[3] != '_') {
      op = OP_INTDIV;
      *cursor += 3;
    } else if (ascii_strncasecmp(*cursor, "MOD", 3) == 0 &&
               !isalnum((unsigned char)(*cursor)[3]) && (*cursor)[3] != '_') {
      op = OP_MOD;
      *cursor += 3;
    } else if (ascii_strncasecmp(*cursor, "SHL", 3) == 0 &&
               !isalnum((unsigned char)(*cursor)[3]) && (*cursor)[3] != '_') {
      op = OP_SHL;
      *cursor += 3;
    } else if (ascii_strncasecmp(*cursor, "SHR", 3) == 0 &&
               !isalnum((unsigned char)(*cursor)[3]) && (*cursor)[3] != '_') {
      op = OP_SHR;
      *cursor += 3;
    } else if (ascii_strncasecmp(*cursor, "AND", 3) == 0 &&
               !isalnum((unsigned char)(*cursor)[3]) && (*cursor)[3] != '_') {
      op = OP_AND;
      *cursor += 3;
    }
    ...
    case OP_INTDIV:
      if (rhs == 0)
        return set_error(error_message, "division by zero");
      *value /= rhs;
      break;
    case OP_MOD:
      if (rhs == 0)
...
static bool parse_factor(const char **cursor, int64_t *value,
  ...
  // Unary operators
  if (ascii_strncasecmp(*cursor, "NOT", 3) == 0 &&
      !isalnum((unsigned char)(*cursor)[3]) && (*cursor)[3] != '_') {
    *cursor += 3;
    int64_t inner = 0;
    if (!parse_factor(cursor, &inner, pp, error_message))
      return false;
    *value = !inner;
    return true;
  }
  ...
  if (ascii_strncasecmp(*cursor, "SIZEOF", 6) == 0) {
    *cursor += 6;
    ...
    if (**cursor != '(')
      return set_error(error_message, "expected '(' after SIZEOF");
    ++(*cursor);
    while (**cursor && isspace((unsigned char)**cursor))
      ++(*cursor);
    ...
    const char *start = *cursor;
    if (!isalpha((unsigned char)**cursor) && **cursor != '_')
      return set_error(error_message, "expected type identifier");
    ...
    if (strcmp(type_name, "NATIVEINT") == 0 ||
        strcmp(type_name, "NATIVEUINT") == 0 ||
        strcmp(type_name, "SIZEINT") == 0 ||
        strcmp(type_name, "SIZEUINT") == 0 ||
        strcmp(type_name, "PTRINT") == 0 || strcmp(type_name, "PTRUINT") == 0 ||
        strcmp(type_name, "INTPTR") == 0 || strcmp(type_name, "UINTPTR") == 0 ||
        strcmp(type_name, "TCONSTPTRUINT") == 0 ||
        strcmp(type_name, "TCONSTPTRINT") == 0 ||
    ...
    if (!found &&
        (strcmp(type_name, "INT64") == 0 || strcmp(type_name, "QWORD") == 0 ||
         strcmp(type_name, "UINT64") == 0 ||
         strcmp(type_name, "TSYSPARAM") == 0 || strcmp(type_name, "V") == 0 ||
         strcmp(type_name, "ALUUINT") == 0 ||
         strcmp(type_name, "ALUSINT") == 0 ||
         strcmp(type_name, "VALSINT") == 0 ||
         strcmp(type_name, "VALUINT") == 0 ||
         ...
         strcmp(type_name, "AINTMAX") == 0 ||
         strcmp(type_name, "ASIZEINT") == 0 ||
         strcmp(type_name, "TBITSBASE") == 0 ||
         strcmp(type_name, "TUNSIGNEDINTTYPE") == 0 ||
         strcmp(type_name, "TSIGNEDINTTYPE") == 0 ||
         strcmp(type_name, "INTPTR") == 0 ||
         strcmp(type_name, "UINTPTR") == 0 ||
         strcmp(type_name, "HANDLE") == 0 ||
         strcmp(type_name, "THANDLE") == 0 ||
         strcmp(type_name, "TLARGEINTEGER") == 0 ||
         strcmp(type_name, "TTHREADID") == 0)) {
    ...
    // Floating point types
             ...
             strcmp(type_name, "TEXTENDED80REC") == 0 ||
             strcmp(type_name, "BESTREAL") == 0) {
      // FPC on x86_64 maps ValReal/Extended/bestreal to the 80-bit extended
      // type (10 bytes) bestrealrec is TExtended80Rec on x86_64 with extended
      // float support
      size = 10;
      found = true;
    } else if (strcmp(type_name, "DOUBLE") == 0 ||
  ...
  // HIGH / LOW functions
  if (ascii_strncasecmp(*cursor, "HIGH", 4) == 0 ||
      ascii_strncasecmp(*cursor, "LOW", 3) == 0) {
    ...
    *cursor += (is_high ? 4 : 3);
    ...
    if (**cursor != '(')
      return set_error(error_message, "expected '(' after HIGH/LOW");
    ++(*cursor);
    while (**cursor && isspace((unsigned char)**cursor))
      ++(*cursor);
    ...
    if (strcmp(type_name, "VALSINT") == 0 || strcmp(type_name, "INT64") == 0) {
      ...
      found = true;
    } else if (strcmp(type_name, "VALUINT") == 0 ||
               strcmp(type_name, "QWORD") == 0 ||
               strcmp(type_name, "UINT64") == 0) {
      ...
      found = true;
    } else if (strcmp(type_name, "WORD") == 0 ||
               strcmp(type_name, "UINT16") == 0) {
    ...
    if (!found) {
      bool err = set_error(error_message, "unsupported type '%s' for HIGH/LOW",
                           type_name);
      free(type_name);
      return err;
    }
  ...
  if (ascii_strncasecmp(*cursor, "DEFINED", 7) == 0 ||
      ascii_strncasecmp(*cursor, "DECLARED", 8) == 0) {
    ...
    *cursor += (is_declared ? 8 : 7);
    ...
    const char *start = *cursor;
    if (!isalpha((unsigned char)**cursor) && **cursor != '_')
      return set_error(error_message, "expected identifier");
    while (**cursor && (isalnum((unsigned char)**cursor) || **cursor == '_' ||
                        **cursor == '.'))
      ++(*cursor);
    ...
    if (is_declared) {
      ...
      if (symbol_is_defined(pp, sym)) {
        ...
        *value = symbol_is_declared_in_source(pp->current_output,
                                              pp->current_output_len, sym)
                     ? 1
                     : 0;
      } else {
        *value = 0;
    ...
    if (val_str) {
      ...
      if (*end == '\0') {
        *value = parsed_val;
        ...
        // Or maybe 0?
        // For now, if it has a value but not number, let's say 0 unless it's
        // "TRUE"
        if (ascii_strncasecmp(val_str, "TRUE", 4) == 0)
          *value = 1;
        else if (ascii_strncasecmp(val_str, "FALSE", 5) == 0)
          *value = 0;
        else
          *value = parsed_val; // Best effort
      }
    } else {
      ...
      if (find_const_integer_in_source(pp, sym, &const_val)) {
        *value = const_val;
                 ...
                 symbol_is_declared_in_source(pp->current_output,
                                              pp->current_output_len, sym)) {
        ...
        *value = 0;
        ...
        bool err = set_error(error_message, "undefined macro '%s'", sym);
        free(sym);
        return err;
      }
    }
