...
const char *pascal_reserved_keywords[] = {
    ...
    "repeat", "until", "case", "of", "var", "const", "type", "and", "or", "not",
    "xor", "div", "mod", "in", "nil", "true", "false", "array", "record", "set",
    "packed",
    ...
    "function", "procedure", "program", "unit", "uses", "interface",
    "implementation", "label", "goto",
    // Unit initialization/finalization keywords
    "initialization", "finalization", NULL};

static const char *expression_identifier_keywords[] = {
    "program", "unit", "record", "array", "set", "packed", "object", "class",
    "integer", "real", "boolean", "char", "string", "byte", "word", "longint",
    "procedure",
...
static ParseResult match_keyword_fn(input_t *in, void *args,
  ...
  if (in->start + len > in->length ||
      strncasecmp(in->buffer + in->start, keyword, len) != 0) {
    return make_failure(in, strdup("Expected keyword"));
  }
...
combinator_t *create_keyword_parser(const char *keyword_str, tag_t tag) {
  combinator_t *comb = new_combinator();
  keyword_parser_args *args =
      (keyword_parser_args *)safe_malloc(sizeof(keyword_parser_args));
