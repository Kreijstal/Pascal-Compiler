...
static const char *skip_utf8_bom(const char *cursor, const char *end) {
  if ((size_t)(end - cursor) >= 3 && (unsigned char)cursor[0] == 0xEF &&
...
static bool buffer_starts_with_keyword(const char *buffer, size_t length,
                                       const char *keyword) {
...
static void print_profile_report(const char *filename, double file_read_seconds,
                                 double preprocess_seconds,
                                 double parser_init_seconds,
                                 double parse_seconds, parser_stats_t stats) {
  double total = file_read_seconds + preprocess_seconds + parser_init_seconds +
                 parse_seconds;
  double hit_pct =
      stats.parse_calls
          ? (100.0 * (double)stats.memo_hits / (double)stats.parse_calls)
          : 0.0;
  double miss_pct =
      stats.parse_calls
          ? (100.0 * (double)stats.memo_misses / (double)stats.parse_calls)
          : 0.0;
  double copy_pct = stats.ast_nodes_created
                        ? (100.0 * (double)stats.ast_nodes_copied /
                           (double)stats.ast_nodes_created)
                        : 0.0;
  double success_pct =
      stats.parse_calls
          ? (100.0 * (double)stats.parse_successes / (double)stats.parse_calls)
          : 0.0;
...
static int compare_comb_entries(const void *a, const void *b) {
  ...
  if (aval > bval)
    return -1;
  if (sa->failure_with_consumption < sb->failure_with_consumption)
    return 1;
  if (sa->failure_with_consumption > sb->failure_with_consumption)
    return -1;
  if (sa->failures < sb->failures)
    return 1;
  if (sa->failures > sb->failures)
    return -1;
  return 0;
...
static void print_comb_profile(size_t top_n) {
  ...
  printf("%-4s %-40s %-12s %8s %8s %8s %10s %10s %12s\n", "#", "Parser", "Type",
         "Calls", "Fails", "Fail>0", "AvgFail", "MaxFail", "TotalFail");
  for (size_t i = 0; i < top_n; ++i) {
    const parser_comb_stat_t *st = entries[i].stat;
    double avg_fail = st->failure_with_consumption
                          ? (double)st->total_failure_consumed /
                                (double)st->failure_with_consumption
                          : 0.0;
    const char *name = st->name ? st->name : "<unnamed>";
    printf("%-4zu %-40.40s %-12s %8zu %8zu %8zu %10.1f %10zu %12zu\n", i + 1,
           name, parser_type_to_string(st->type), st->calls, st->failures,
           st->failure_with_consumption, avg_fail, st->max_failure_consumed,
...
int main(int argc, char *argv[]) {
  ...
  if (filename == NULL) {
    fprintf(stderr,
            "Usage: %s [--print-ast] [--parse-procedure] [--profile] "
            "[--profile-combinators[=N]] [--memo-mode=<full|failures|off>] "
            "<filename>\n",
            argv[0]);
    return 1;
  }
  ...
  char *preprocessed_content =
      pascal_preprocess_buffer(preprocessor, filename, file_content, bytes_read,
  ...
  bool parse_as_unit = !parse_procedure &&
                       buffer_starts_with_keyword(preprocessed_content,
                                                  preprocessed_length, "unit");
  if (parse_procedure) {
  ...
  if (result.is_success) {
    ...
    if (in->start < in->length) {
      ...
      int trailing_line = 1;
      int trailing_col = 1;
      parser_calculate_line_col(in, trailing_index, &trailing_line,
                                &trailing_col);

      fprintf(stderr,
              "Error: Parser did not consume entire input. Trailing input "
              "begins at line %d, column %d.\n",
              trailing_line, trailing_col);

      char *context = parser_format_context(in, trailing_line, trailing_col,
                                            trailing_index);
      if (context != NULL) {
      ...
      if (profile && stats_ready) {
        print_profile_report(filename, file_read_seconds, preprocess_seconds,
                             parser_init_seconds, parse_seconds, stats);
      }
      if (profile_combinators) {
