...
static combinator_t *create_nested_method_directives(void) {
  ...
  if (cached_create_nested_method_directives != NULL)
    return cached_create_nested_method_directives;

  combinator_t *directive =
      multi(new_combinator(), PASCAL_T_NONE, token(keyword_ci("override")),
...
static bool read_identifier_slice(const char *buffer, int length, int pos,
                                  int *out_start, size_t *out_len) {
  if (pos >= length) {
    return false;
  }
...
static bool slice_equals_keyword_ci(const char *start, size_t len,
                                    const char *keyword) {
...
static ParseResult fail_with_message(const char *message, input_t *in,
                                     InputState *state, char *parser_name) {
  restore_input_state(in, state);
  return make_failure_static(in, message);
}

static ParseResult class_member_dispatch_fn(input_t *in, void *args,
                                            char *parser_name) {
  ...
  if (pos >= length) {
    return make_failure_static(in,
                               "Unexpected end of input inside class member");
  }
  ...
  if (slice_equals_keyword_ci(keyword_start, keyword_len, "constructor") &&
      dispatch->constructor_parser != NULL) {
    return parse(in, dispatch->constructor_parser);
  }
  if (slice_equals_keyword_ci(keyword_start, keyword_len, "destructor") &&
      dispatch->destructor_parser != NULL) {
    return parse(in, dispatch->destructor_parser);
  }
  if (slice_equals_keyword_ci(keyword_start, keyword_len, "procedure") &&
      dispatch->procedure_parser != NULL) {
    return parse(in, dispatch->procedure_parser);
  }
  if (slice_equals_keyword_ci(keyword_start, keyword_len, "function") &&
      dispatch->function_parser != NULL) {
    return parse(in, dispatch->function_parser);
  }
  if (slice_equals_keyword_ci(keyword_start, keyword_len, "operator") &&
      dispatch->operator_parser != NULL) {
    return parse(in, dispatch->operator_parser);
  }
  if (slice_equals_keyword_ci(keyword_start, keyword_len, "property") &&
      dispatch->property_parser != NULL) {
...
static ParseResult range_type_fn(input_t *in, void *args, char *parser_name) {
  ...
  if (!expr_result.is_success) {
    ...
    return fail_with_message("Expected range expression", in, &state,
                             parser_name);
  }

  ast_t *expr_ast = expr_result.value.ast;
  if (expr_ast == NULL || expr_ast->typ != PASCAL_T_RANGE ||
      expr_ast->child == NULL || expr_ast->child->next == NULL) {
    free_ast(expr_ast);
    return fail_with_message("Invalid range expression", in, &state,
                             parser_name);
  }
...
static ParseResult array_type_fn(input_t *in, void *args, char *parser_name) {
  ...
  if (has_indices) {
    // Parse ranges/indices (simplified - just accept any identifiers/ranges for
    // now)
    ...
    if (indices_res.is_success) {
      ...
      return fail_with_message("Expected array indices", in, &state,
                               parser_name);
    }
    free_combinator(index_list);
  ...
  if (!of_res.is_success) {
    ...
    return fail_with_message("Expected 'OF' after array indices", in, &state,
                             parser_name);
  }
  free_ast(of_res.value.ast);
  free_combinator(of_keyword);

  // Parse element type - support full type specs (record, set, pointer,
  // identifier with optional [size], etc.)
  ...
  if (cached_rich_element_type == NULL) {
    ...
    cached_rich_element_type = multi(
        ...
        file_type(PASCAL_T_FILE_TYPE), pointer_type(PASCAL_T_POINTER_TYPE),
  ...
  if (!el_res.is_success) {
    ...
    free_ast(indices_ast);
    return fail_with_message("Expected element type after 'OF'", in, &state,
                             parser_name);
  }
...
static ast_t *map_const_type_keyword(ast_t *ast) {
  /* keyword_ci returns ast_nil on success, so we need to create a proper
   * identifier node */
...
// simple identifiers and constructed types (like TFoo<Integer> or specialize
// TFoo<Integer>)
static combinator_t *create_type_ref_parser(void) {
  static combinator_t *cached_type_ref = NULL;
  if (cached_type_ref != NULL)
    return cached_type_ref;

  combinator_t **type_arg_ref =
  ...
  *type_arg_ref = new_combinator();
  ...
  cached_type_ref =
      multi(new_combinator(), PASCAL_T_NONE, constructed_type,
            array_type(PASCAL_T_ARRAY_TYPE), record_type(PASCAL_T_RECORD_TYPE),
...
static combinator_t *create_method_type_param_list(void) {
  ...
  if (cached_create_method_type_param_list != NULL)
...
static combinator_t *create_method_return_type_parser(void) {
  ...
  if (cached_create_method_return_type_parser != NULL)
...
static combinator_t *create_property_decl_parser(void) {
  ...
  if (cached_create_property_decl_parser != NULL)
  ...
  cached_create_property_decl_parser = seq(
      ...
      optional(token(create_keyword_parser(
          "class", PASCAL_T_IDENTIFIER))), // Support class properties
...
static combinator_t *create_record_field_attribute_parser(void) {
  ...
  if (cached_create_record_field_attribute_parser != NULL)
...
combinator_t *class_type(tag_t tag) {
  ...
  combinator_t *constructor_decl =
      seq(new_combinator(), PASCAL_T_CONSTRUCTOR_DECL,
          optional(token(create_keyword_parser(
              "class", PASCAL_T_IDENTIFIER))), // Support class constructor
  ...
  combinator_t *destructor_decl =
      seq(new_combinator(), PASCAL_T_DESTRUCTOR_DECL,
          optional(token(create_keyword_parser(
              "class", PASCAL_T_IDENTIFIER))), // Support class destructor
  ...
  //   procedure IFoo.Bar = Baz;
  //
  // The owner part must stop before ".Method". Reusing create_type_ref_parser()
  // is too greedy (it can consume dotted identifiers including the member
  // name), so parse a focused owner: identifier with optional generic args.
  combinator_t **delegation_type_arg_ref =
      (combinator_t **)safe_malloc(sizeof(combinator_t *));
  *delegation_type_arg_ref = new_combinator();
  ...
  // [override];
  combinator_t *class_operator_decl = seq(
  ...
  class_member_dispatch_args_t *class_dispatch =
      (class_member_dispatch_args_t *)safe_malloc(
          sizeof(class_member_dispatch_args_t));
  ...
  // Access sections: private, public, protected, published, strict private,
  // strict protected Handle strict modifiers first (two-word) then single-word
  // modifiers
  ...
  combinator_t *nested_parent_class =
  ...
  combinator_t *nested_type_spec = multi(
      ...
      enumerated_type(
          PASCAL_T_ENUMERATED_TYPE),       // Support nested enumeration types
      pointer_type(PASCAL_T_POINTER_TYPE), // Support nested pointer types
      array_type(PASCAL_T_ARRAY_TYPE), set_type(PASCAL_T_SET),
      range_type(PASCAL_T_RANGE_TYPE), file_type(PASCAL_T_FILE_TYPE),
  ...
  combinator_t *nested_regular_type_decl =
  ...
  combinator_t **class_const_expr_parser =
      (combinator_t **)safe_malloc(sizeof(combinator_t *));
  *class_const_expr_parser = new_combinator();
  ...
  // A field may carry a trailing `static;` directive, e.g.
  //   FFoo: array[...] of T; static;
  // which marks the (instance-section) field as a class-wide static field.
...
combinator_t *interface_type(tag_t tag) {
  ...
  // Optional parent interface specification: (ParentInterface) or
  // (ParentInterface<T>) Can also support multiple parent interfaces if needed
  combinator_t *parent_interface =
      optional(between(token(match("(")), token(match(")")),
                       sep_by(create_type_ref_parser(), token(match(",")))));
...
static ParseResult type_name_fn(input_t *in, void *args, char *parser_name) {
  ...
  const char *type_keywords[] = {
      ...
      "string",     "shortstring",   "ansistring", "unicodestring",
      "widestring", "rawbytestring", "byte",       "word",
      "longint",    "pointer",       NULL};
...
static combinator_t *create_record_field_type_spec(void) {
  ...
  if (cached_create_record_field_type_spec != NULL)
    return cached_create_record_field_type_spec;

  cached_create_record_field_type_spec = multi(
      new_combinator(), PASCAL_T_TYPE_SPEC, array_type(PASCAL_T_ARRAY_TYPE),
      ...
      procedure_type(
          PASCAL_T_PROCEDURE_TYPE), /* Allow procedure/function fields with
                                       calling conventions */
      function_type(PASCAL_T_FUNCTION_TYPE), type_name(PASCAL_T_IDENTIFIER),
      token(pascal_identifier_with_subscript(PASCAL_T_IDENTIFIER)),
      token(pascal_qualified_identifier(PASCAL_T_IDENTIFIER)),
...
static combinator_t *create_record_method_directives(void) {
  ...
  if (cached_create_record_method_directives != NULL)
    return cached_create_record_method_directives;

  combinator_t *record_method_directive_keyword = multi(
...
// Method directives for class methods - includes virtual, override,
// reintroduce, etc.
static combinator_t *create_class_method_directives(void) {
  static combinator_t *cached_create_class_method_directives = NULL;
  if (cached_create_class_method_directives != NULL)
    return cached_create_class_method_directives;

  combinator_t *class_method_directive_keyword =
      multi(new_combinator(), PASCAL_T_NONE,
...
static ParseResult variant_tag_fn(input_t *in, void *args, char *parser_name) {
  ...
  save_input_state(in, &start_state);
  ...
  if (ident_res.is_success) {
    ...
    InputState colon_state;
    save_input_state(in, &colon_state);
    ...
    if (colon_res.is_success) {
      ...
      if (!type_res.is_success) {
        ...
        return fail_with_message("Expected variant tag type", in, &start_state,
                                 parser_name);
      }
      name_ast = ident_ast;
...
static combinator_t *
create_variant_branch_parser(combinator_t **record_item_ref) {
  combinator_t *label_atom = multi(
      new_combinator(), PASCAL_T_CASE_LABEL, token(integer(PASCAL_T_INTEGER)),
  ...
  combinator_t *branch_fields =
      between(token(match("(")), token(match(")")), optional(inner_items));
...
static ParseResult variant_part_fn(input_t *in, void *args, char *parser_name) {
  ...
  if (!tag_res.is_success) {
    ...
    restore_input_state(in, &start_state);
    return fail_with_message("Expected variant selector", in, &start_state,
                             parser_name);
  }
  ...
  if (!of_res.is_success) {
    ...
    free_ast(tag_ast);
    restore_input_state(in, &start_state);
    return fail_with_message("Expected 'of' in variant part", in, &start_state,
                             parser_name);
  }
  free_ast(of_res.value.ast);

  combinator_t *branch_list =
      sep_end_by(lazy(&pargs->branch_parser), token(match(";")));
  ...
  if (!branches_res.is_success) {
    ...
    free_ast(tag_ast);
    restore_input_state(in, &start_state);
    return fail_with_message("Expected variant branch", in, &start_state,
                             parser_name);
  }
  ...
  if (branches_ast == ast_nil) {
    free_ast(tag_ast);
    restore_input_state(in, &start_state);
    return fail_with_message("Variant part requires at least one branch", in,
                             &start_state, parser_name);
  }
...
static ParseResult record_type_fn(input_t *in, void *args, char *parser_name) {
  ...
  if (is_helper) {
    combinator_t *helper_body =
        optional(until(token(keyword_ci("end")), PASCAL_T_NONE));
    ...
    if (!end_res.is_success) {
      ...
      return fail_with_message("Expected 'end' after record helper body", in,
                               &state, parser_name);
    }
  ...
  combinator_t *field_name_list =
      sep_by(token(cident(PASCAL_T_IDENTIFIER)), token(match(",")));
  combinator_t *field_type = create_record_field_type_spec();

  // Hint directives for record fields: deprecated, platform, library,
  // experimental FPC allows these after the field type without requiring a
  // semicolon
  combinator_t *field_hint_directive = optional(seq(
      new_combinator(), PASCAL_T_NONE,
  ...
  *record_item_ref = new_combinator();
  ...
  if (!is_helper) {
    combinator_t *record_items =
        sep_end_by(lazy(record_item_ref), token(match(";")));
    ParseResult items_res = parse(in, record_items);
    if (items_res.is_success) {
      fields_ast =
          (items_res.value.ast == ast_nil) ? NULL : items_res.value.ast;
    } else {
  ...
  if (!is_helper) {
    ...
    combinator_t *record_method_directives_for_operator =
        create_record_method_directives();
    combinator_t *record_operator_decl = seq(
        new_combinator(), PASCAL_T_METHOD_DECL,
        optional(token(create_keyword_parser("class", PASCAL_T_IDENTIFIER))),
    ...
    if (methods_res.is_success) {
      methods_ast =
          (methods_res.value.ast == ast_nil) ? NULL : methods_res.value.ast;
    } else {
  ...
  combinator_t *adv_field_decl =
      seq(new_combinator(), PASCAL_T_FIELD_DECL, field_attribute_adv,
          sep_by(token(cident(PASCAL_T_IDENTIFIER)), token(match(","))),
  ...
  combinator_t *pre_semi_directive =
      multi(new_combinator(), PASCAL_T_NONE,
            token(create_keyword_parser("static", PASCAL_T_IDENTIFIER)),
            token(create_keyword_parser("overload", PASCAL_T_IDENTIFIER)),
  ...
  // method directives)
  combinator_t *record_method_directives_for_operator_member =
      create_record_method_directives();
  combinator_t *adv_operator_decl = seq(
      new_combinator(), PASCAL_T_METHOD_DECL,
      optional(token(create_keyword_parser("class", PASCAL_T_IDENTIFIER))),
      token(keyword_ci("operator")), token(operator_name(PASCAL_T_IDENTIFIER)),
  ...
  combinator_t *adv_nested_type_spec = multi(
      ...
      object_type(PASCAL_T_OBJECT_TYPE), record_type(PASCAL_T_RECORD_TYPE),
      enumerated_type(PASCAL_T_ENUMERATED_TYPE),
      pointer_type(PASCAL_T_POINTER_TYPE), array_type(PASCAL_T_ARRAY_TYPE),
      set_type(PASCAL_T_SET), file_type(PASCAL_T_FILE_TYPE),
      procedure_type(PASCAL_T_PROCEDURE_TYPE),
  ...
  combinator_t *adv_nested_regular_type_decl =
      seq(new_combinator(), PASCAL_T_TYPE_DECL,
          optional(token(keyword_ci("generic"))),
          token(cident(PASCAL_T_IDENTIFIER)), token(match("=")),
  ...
  combinator_t **adv_const_expr_parser =
      (combinator_t **)safe_malloc(sizeof(combinator_t *));
  *adv_const_expr_parser = new_combinator();
  ...
  // TObject;)
  combinator_t *adv_class_var_decl =
      seq(new_combinator(), PASCAL_T_FIELD_DECL, token(keyword_ci("class")),
  ...
  if (getenv("KGPC_DEBUG_RECORD") != NULL) {
    ...
    int preview_len = 60;
    if (in->length - in->start < preview_len)
      preview_len = in->length - in->start;
  ...
  if (getenv("KGPC_DEBUG_RECORD") != NULL) {
    ...
    if (adv_res.is_success) {
      ...
      if (in->length - in->start < after_preview)
        after_preview = in->length - in->start;
      fprintf(stderr,
              "[RECORD] after adv_members, pos=%d line=%d next='%.*s'\n",
  ...
  /* Allow trailing field/variant declarations after advanced members (FPC
   * allows interleaving). */
  ...
  if (parse_tail_items) {
    combinator_t *tail_field_name_list =
        sep_by(token(cident(PASCAL_T_IDENTIFIER)), token(match(",")));
    combinator_t *tail_field_type = create_record_field_type_spec();
    combinator_t *tail_field_hint_directive = optional(seq(
        new_combinator(), PASCAL_T_NONE,
        multi(new_combinator(), PASCAL_T_NONE, token(keyword_ci("deprecated")),
              token(keyword_ci("platform")), token(keyword_ci("library")),
              token(keyword_ci("experimental")), (combinator_t *)NULL),
    ...
    *tail_record_item_ref = new_combinator();
    ...
    combinator_t *tail_variant_branch =
        create_variant_branch_parser(tail_record_item_ref);

    variant_part_args *tail_variant_args =
        safe_malloc(sizeof(variant_part_args));
    ...
    if (tail_items_res.is_success) {
      tail_fields_ast = (tail_items_res.value.ast == ast_nil)
                            ? NULL
                            : tail_items_res.value.ast;
    } else {
  ...
  if (!end_res.is_success) {
    ...
    if (fields_ast != NULL)
      free_ast(fields_ast);
    free_combinator(end_keyword);
    return fail_with_message("Expected 'end' after record fields", in, &state,
                             parser_name);
  }
  free_ast(end_res.value.ast);
  free_combinator(end_keyword);
  ...
  } else if (!align_res.is_success) {
    discard_failure(align_res);
  }
  free_combinator(align_clause);

  // Do not consume beyond 'end' or optional align; higher-level parsers will
  // handle any following tokens
...
// Object type parser: OBJECT field1: type1; [procedure/function declarations]
// ... END Legacy Pascal object type - similar to record but with methods
static ParseResult object_type_fn(input_t *in, void *args, char *parser_name) {
  ...
  combinator_t *method_directive = seq(
      new_combinator(), PASCAL_T_METHOD_DIRECTIVE,
      multi(new_combinator(), PASCAL_T_NONE,
            token(create_keyword_parser("virtual", PASCAL_T_IDENTIFIER)),
            token(create_keyword_parser("override", PASCAL_T_IDENTIFIER)),
  ...
  combinator_t *nested_type_spec = multi(
      new_combinator(), PASCAL_T_TYPE_SPEC, object_type(PASCAL_T_OBJECT_TYPE),
  ...
  *object_const_expr_parser = new_combinator();
  ...
  if (members_res.is_success) {
    fields_ast =
        (members_res.value.ast == ast_nil) ? NULL : members_res.value.ast;
  } else {
  ...
  if (!end_res.is_success) {
    ...
    if (fields_ast != NULL)
      free_ast(fields_ast);
    free_combinator(end_keyword);
    return fail_with_message("Expected 'end' after object members", in, &state,
                             parser_name);
  }
  free_ast(end_res.value.ast);
  free_combinator(end_keyword);
...
combinator_t *pointer_type(tag_t tag) {
  combinator_t *pointed_type = multi(
      new_combinator(), PASCAL_T_NONE,
      type_name(
          PASCAL_T_IDENTIFIER), // built-in Pascal types (integer, string, ...)
...
static ParseResult enumerated_type_fn(input_t *in, void *args,
                                      char *parser_name) {
  prim_args *pargs = (prim_args *)args;
  InputState state;
  save_input_state(in, &state);
  ...
  if (!open_res.is_success) {
    ...
    return fail_with_message("Expected '(' for enumerated type", in, &state,
                             parser_name);
  }
  free_ast(open_res.value.ast);
  free_combinator(open_paren);

  // Parse enumerated values: identifier [:= expression], identifier [:=
  // expression], ... FPC supports explicit ordinal values like (ms_on := 1,
  // ms_off := 2)
  ...
  while (1) {
    ...
    if (!ident_res.is_success) {
      discard_failure(ident_res);
      if (values_ast == NULL) {
        return fail_with_message("Expected enumerated values", in, &state,
                                 parser_name);
      }
      break;
    }
    ...
    if (assign_res.is_success) {
      ...
      if (val_res.is_success) {
        ...
        free_ast(values_ast);
        return fail_with_message("Expected expression after '=' in enum value",
  ...
  if (!close_res.is_success) {
    ...
    return fail_with_message("Expected ')' after enumerated values", in, &state,
                             parser_name);
  }
  free_ast(close_res.value.ast);
  free_combinator(close_paren);
...
static ParseResult set_type_fn(input_t *in, void *args, char *parser_name) {
  ...
  if (!of_result.is_success) {
    ...
    free_combinator(of_keyword);
    return fail_with_message("Expected 'of' after 'set'", in, &state,
                             parser_name);
  }
  free_combinator(of_keyword);
  ...
  // Use pascal_qualified_identifier to support nested types like
  // TRecord.TNestedType
  combinator_t *element_type =
      multi(new_combinator(), PASCAL_T_NONE,
            enumerated_type(PASCAL_T_ENUMERATED_TYPE),
...
static ParseResult file_type_fn(input_t *in, void *args, char *parser_name) {
  ...
  if (of_result.is_success) {
    ...
    combinator_t *element_type = multi(
        new_combinator(), PASCAL_T_TYPE_SPEC, array_type(PASCAL_T_ARRAY_TYPE),
        set_type(PASCAL_T_SET), pointer_type(PASCAL_T_POINTER_TYPE),
        enumerated_type(PASCAL_T_ENUMERATED_TYPE),
        range_type(PASCAL_T_RANGE_TYPE), record_type(PASCAL_T_RECORD_TYPE),
    ...
    if (!element_result.is_success) {
      discard_failure(element_result);
      return fail_with_message("Expected file element type after 'of'", in,
                               &state, parser_name);
    }
...
static ParseResult subroutine_type_fn(input_t *in, void *args,
                                      char *parser_name) {
  ...
  if (!keyword_res.is_success) {
    discard_failure(keyword_res);
    return fail_with_message(pargs->is_function ? "Expected 'function'"
                                                : "Expected 'procedure'",
                             in, &state, parser_name);
  }
  ...
  if (pargs->is_function) {
    ...
    if (!colon_res.is_success) {
      if (params_ast != NULL)
        free_ast(params_ast);
      discard_failure(colon_res);
      return fail_with_message("Expected ':' for function return type", in,
                               &state, parser_name);
    }
    free_ast(colon_res.value.ast);
    ...
    if (!type_res.is_success) {
      if (params_ast != NULL)
        free_ast(params_ast);
      discard_failure(type_res);
      return fail_with_message("Expected return type specification", in, &state,
                               parser_name);
    }
  ...
  // 4. Optionally consume calling convention/directive keywords (e.g., stdcall,
  // cdecl) that can appear after procedure/function type declarations in
  // Delphi/FPC. We ignore their semantics here and only consume the tokens to
  // avoid syntax errors.
  {
    ...
    // enclosing type declaration to consume. Only consume the ';' if a
    // directive keyword follows immediately after.
    InputState s0;
    save_input_state(in, &s0);
    ...
    if (sc.is_success) {
      // Check if a directive follows; if not, roll back and keep ';' for outer
      // parser
      InputState s1;
      save_input_state(in, &s1);
...
static ParseResult reference_to_type_fn(input_t *in, void *args,
                                        char *parser_name) {
  prim_args *pargs = (prim_args *)args;
  InputState state;
  save_input_state(in, &state);
  ...
  if (!to_res.is_success) {
    ...
    return fail_with_message("Expected 'to' after 'reference'", in, &state,
                             parser_name);
  }
  ...
  if (!subroutine_res.is_success) {
    ...
    return fail_with_message(
        "Expected procedure or function type after 'reference to'", in, &state,
        parser_name);
  }
...
// Parser function for identifier with optional [size] subscript (e.g.,
// string[20])
static ParseResult pascal_identifier_with_subscript_fn(input_t *in, void *args,
                                                       char *parser_name) {
  prim_args *pargs = (prim_args *)args;
  tag_t tag = (pargs != NULL) ? pargs->tag : PASCAL_T_IDENTIFIER;
  ...
  // Parse the base identifier (supports unit-qualified names like
  // baseunix.stat)
