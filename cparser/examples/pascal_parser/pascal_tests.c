...
static void track_parser_for_cleanup(combinator_t *parser,
                                     combinator_t **anchor) {
  if (parser == NULL) {
    if (anchor != NULL) {
      free(anchor);
  ...
  if (parser_cleanup_count == parser_cleanup_capacity) {
    size_t new_capacity =
        parser_cleanup_capacity ? parser_cleanup_capacity * 2 : 16;
    parser_cleanup_entry_t *new_items = realloc(
        parser_cleanup_entries, new_capacity * sizeof(parser_cleanup_entry_t));
    if (new_items == NULL) {
  ...
  parser_cleanup_entries[parser_cleanup_count++] =
      (parser_cleanup_entry_t){.parser = parser, .anchor = anchor};
}
...
static ast_t *find_first_node_of_type(ast_t *node, tag_t target) {
  ...
  for (ast_t *current = node; current != NULL && current != ast_nil;
...
static bool section_has_routine_named(ast_t *section, tag_t routine_type,
                                      const char *name) {
  if (section == NULL || name == NULL) {
    return false;
  }

  for (ast_t *current = section->child; current != NULL;
       current = current->next) {
...
static char *preprocess_pascal_source(const char *path, const char *kind,
...
static char *load_pascal_snippet(const char *filename) {
  ...
  if (read != (size_t)size) {
    fprintf(stderr,
            "Failed to read snippet '%s' (expected %ld bytes, got %zu)\n",
            filename, size, read);
    goto cleanup;
  }
...
static char *preprocess_pascal_source(const char *path, const char *kind,
  ...
  char *preprocessed = pascal_preprocess_buffer(
      pp, path, source, strlen(source), NULL, &preprocess_error);
...
static uint16_t read_utf16_code_unit(const uint8_t *data, size_t index,
                                     bool little_endian) {
...
static char *convert_utf16_to_utf8(const uint8_t *data, size_t byte_len,
                                   bool little_endian) {
  if (data == NULL) {
    return NULL;
  }
    ...
    if (w1 >= 0xD800 && w1 <= 0xDBFF) {
      if (i + 1 < code_units) {
        ...
        if (w2 >= 0xDC00 && w2 <= 0xDFFF) {
          codepoint = 0x10000 + ((((uint32_t)w1 - 0xD800) << 10) |
                                 ((uint32_t)w2 - 0xDC00));
...
static char *load_pascal_file(const char *filename) {
  ...
  if (!path) {
    fprintf(stderr,
            "Out of memory while constructing pascal file path for %s\n",
            filename);
    goto cleanup;
  }
  ...
  if (read != (size_t)size) {
    fprintf(stderr,
            "Failed to read Pascal file '%s' (expected %ld bytes, got %zu)\n",
            filename, size, read);
    goto cleanup;
  }
  ...
  if (result && size >= 2) {
    ...
    if ((b0 == 0xFF && b1 == 0xFE) || (b0 == 0xFE && b1 == 0xFF)) {
      ...
      char *converted = convert_utf16_to_utf8((const uint8_t *)(result + 2),
                                              (size_t)size - 2, little_endian);
      if (!converted) {
        free(result);
        result = NULL;
        goto cleanup;
...
/*
 * Regression test for GitHub issue: stray identifiers between declaration
 * sections and main block should cause a parse error, not be silently ignored.
 *
 * Bug case:
 *   program colors;
 *   const cyan=#27+'[36m';
...
void test_pascal_stray_identifier_before_begin(void) {
  ...
  const char *source = "program colors;\n"
                       "const cyan=#27+'[36m';\n"
                       "clear=#27+'[0m';\n"
  ...
  TEST_ASSERT(!res.is_success);
  TEST_MSG("Parser should reject stray identifiers between const section and "
           "begin block");
...
void test_pascal_preprocessor_conditionals(void) {
  ...
  size_t output_len = 0;
  char *result = pascal_preprocess_buffer(pp, "<memory>", source, source_len,
                                          &output_len, &error_message);
  ...
  char *result2 = pascal_preprocess_buffer(
      pp, "<memory>", source2, strlen(source2), &output_len2, &error_message2);
...
void test_pascal_preprocessor_comment_mixing(void) {
  ...
  char *result = pascal_preprocess_buffer(
      pp, "<memory>", source, strlen(source), &output_len, &error_message);
...
void test_pascal_preprocessor_numeric_comparisons(void) {
  ...
  char *error1 = NULL;
  char *result1 = pascal_preprocess_buffer(pp, "<memory>", source1,
                                           strlen(source1), NULL, &error1);
  ...
  char *error2 = NULL;
  char *result2 = pascal_preprocess_buffer(pp, "<memory>", source2,
                                           strlen(source2), NULL, &error2);
  ...
  char *error3 = NULL;
  char *result3 = pascal_preprocess_buffer(pp, "<memory>", source3,
                                           strlen(source3), NULL, &error3);
  ...
  char *error4 = NULL;
  char *result4 = pascal_preprocess_buffer(pp, "<memory>", source4,
                                           strlen(source4), NULL, &error4);
...
void test_pascal_function_call(void) {
  ...
  if (res.value.ast->child &&
      res.value.ast->child->typ == PASCAL_T_IDENTIFIER) {
    // Use the child identifier for built-in functions
    actual_name_node = res.value.ast->child;
  }

  TEST_ASSERT(actual_name_node->sym && actual_name_node->sym->name &&
...
static void assert_char_code_literal_success(combinator_t *p,
                                             const char *literal) {
  input_t *input = new_input();
...
static void assert_char_code_literal_failure(combinator_t *p,
                                             const char *literal) {
  input_t *input = new_input();
  input->buffer = strdup(literal);
  input->length = (int)strlen(literal);
...
void test_pascal_relational_operators(void) {
  const char *expressions[] = {"5 = 5", "5 <> 3", "3 < 5",
                               "5 > 3", "3 <= 5", "5 >= 3"};
  pascal_tag_t expected_tags[] = {PASCAL_T_EQ, PASCAL_T_NE, PASCAL_T_LT,
                                  PASCAL_T_GT, PASCAL_T_LE, PASCAL_T_GE};
...
void test_pascal_boolean_operators(void) {
  const char *expressions[] = {"true and false", "true or false", "not true",
                               "true xor false"};
  pascal_tag_t expected_tags[] = {PASCAL_T_AND, PASCAL_T_OR, PASCAL_T_NOT,
                                  PASCAL_T_XOR};
...
void test_pascal_bitwise_operators(void) {
  ...
  pascal_tag_t expected_tags[] = {PASCAL_T_SHL, PASCAL_T_SHR, PASCAL_T_ROL,
                                  PASCAL_T_ROR};
...
// Test field width format spec precedence: x:Width-2 should parse as
// x:(Width-2)
...
void test_pascal_typed_file_type(void) {
  combinator_t *p = get_program_parser();
  const char *src = "program FileTest;\n"
                    "type\n"
  ...
  TEST_ASSERT(elem_type->typ == PASCAL_T_TYPE_SPEC ||
              elem_type->typ == PASCAL_T_IDENTIFIER);
...
// Regression: routine-local repeated const sections with typed record/array
// consts
...
void test_type_subroutine_with_stdcall(void) {
  ...
  input_t *input = new_input();
  const char *src = "program p;\n"
                    "type TWNDPROC = function (Wnd: HWND; Msg: DWORD; wParam: "
                    "ptruint; lParam: ptrint): integer; stdcall;\n"
                    "begin\nend.";
  input->buffer = strdup(src);
  input->length = (int)strlen(src);
  ParseResult res = parse(input, p);
...
void test_pascal_is_operator(void) {
  ...
  TEST_ASSERT(actual_left_node->sym && actual_left_node->sym->name &&
  ...
  TEST_ASSERT(actual_right_node->sym && actual_right_node->sym->name &&
...
void test_pascal_as_operator_with_field_access(void) {
  ...
  TEST_ASSERT(field->sym && field->sym->name &&
              strcmp(field->sym->name, "Button") == 0);
...
void test_pascal_assignment_statement(void) {
  combinator_t *p =
      create_program_parser(); // Use program parser for terminated statements
...
void test_pascal_expression_statement(void) {
  ...
  TEST_CHECK(actual_name_node->sym && actual_name_node->sym->name &&
...
void test_pascal_unterminated_asm_block(void) {
  ...
  TEST_ASSERT(!res.is_success);
  // Could be "Expected ';'" from program parser or "Unterminated ASM block"
  // from ASM parser
...
void test_pascal_procedure_with_params(void) {
  ...
  input_t *input = new_input();
  input->buffer =
      strdup("procedure MyProcedure(x: integer; y: string); begin end");
  input->length = strlen(input->buffer);
...
void test_pascal_function_with_trailing_semicolon(void) {
  combinator_t *p = create_procedure_parser();
  input_t *input = new_input();
  input->buffer = strdup("function Multiply(x, y: integer): integer; begin; "
                         "Multiply := x * y; end;");
  input->length = strlen(input->buffer);
...
void test_pascal_function_multiple_params(void) {
  ...
  input_t *input = new_input();
  input->buffer = strdup(
      "function Calculate(a: real; b: real; c: integer): real; begin end");
  input->length = strlen(input->buffer);
...
void test_pascal_method_implementation(void) {
  ...
  // This test is expected to fail because method implementations are not fully
  // supported.
  TEST_ASSERT(res.is_success);
  ...
  if (res.is_success) {
    ...
    // The first child of the METHOD_IMPL node should be the
...
void test_pascal_with_statement_multiple_contexts(void) {
  ...
  if (res.is_success) {
    ...
    TEST_ASSERT(second_context->next == NULL ||
                second_context->next == ast_nil);
...
void test_pascal_with_statement_invalid_syntax(void) {
  ...
  leading_comma_input->buffer =
      strdup("program p; begin with , Inner do begin end; end.");
  ...
  trailing_comma_input->buffer =
      strdup("program p; begin with Outer, do begin end; end.");
  trailing_comma_input->length = strlen(trailing_comma_input->buffer);
  ParseResult trailing_comma_result =
      parse(trailing_comma_input, program_parser);
...
void test_pascal_exit_statement(void) {
  ...
  // This test is expected to fail because 'exit' statements are not
  // implemented.
  TEST_ASSERT(res.is_success);
...
void test_pascal_forward_declared_function(void) {
  ...
  // This test is expected to fail because unit parsing is not fully
  // implemented.
  TEST_ASSERT(res.is_success);
...
void test_pascal_simple_case_statement(void) {
  ...
  input_t *input = new_input();
  input->buffer =
      strdup("case x of 1: y := 2; 3: y := 4 end"); // Full case statement
  input->length = strlen(input->buffer);
...
void test_pascal_case_statement_with_ranges(void) {
  ...
  if (!res.is_success) {
    printf("Range test parse error: %s at line %d, col %d\n",
           res.value.error->message, res.value.error->line,
           res.value.error->col);
    free_error(res.value.error);
    free(input->buffer);
    free_input(input);
...
void test_pascal_case_statement_multiple_labels(void) {
  ...
  input_t *input = new_input();
  input->buffer =
      strdup("case n of 1, 3, 5: writeln(); 2, 4, 6: writeln() end");
  input->length = strlen(input->buffer);
...
void test_pascal_case_expression_labels(void) {
  ...
  input_t *input = new_input();
  input->buffer =
      strdup("case x of -1: writeln(); +5: writeln(); (10): writeln() end");
  input->length = strlen(input->buffer);
...
// Test case labels that use compile-time builtins like Low/High (should
// succeed)
...
void test_pascal_pointer_dereference(void) {
  ...
  if (res.is_success) {
    ...
    TEST_ASSERT(target->sym && target->sym->name &&
                strcmp(target->sym->name, "ptr") == 0);
    free_ast(res.value.ast);
  } else {
    free_error(res.value.error);
  }
...
void test_pascal_set_operations_program(void) {
  ...
  if (res.is_success) {
    ...
    ast_t *union_node =
        find_first_node_of_type(res.value.ast, PASCAL_T_SET_UNION);
...
void test_pascal_dialog_program(void) {
  ...
  if (!res.is_success) {
    fprintf(stderr, "Failed to parse dialog program: %s\n",
            res.value.error && res.value.error->message
                ? res.value.error->message
                : "<no message>");
...
void test_pascal_record_member_access_program(void) {
  ...
  if (res.is_success) {
    ast_t *member_node =
        find_first_node_of_type(res.value.ast, PASCAL_T_MEMBER_ACCESS);
...
void test_pascal_record_member_access_complete_program(void) {
  ...
  input_t *input = new_input();
  char *program =
      load_pascal_snippet("record_member_access_complete_program.pas");
  TEST_ASSERT(program != NULL);
  if (!program) {
    free_input(input);
    return;
...
void test_pascal_unitless_program(void) {
  ...
  if (res.is_success) {
    ...
    for (ast_t *current = program_decl->child; current != NULL;
         current = current->next) {
    ...
    if (set_literal) {
      ast_t *char_code =
          find_first_node_of_type(set_literal, PASCAL_T_CHAR_CODE);
...
static void assert_pascal_unit_parses_snippet(const char *snippet_name) {
  ...
  if (result.is_success && input->start < input->length) {
    TEST_MSG("Parser left trailing input: '%.*s'", 40,
             input->buffer + input->start);
    success = false;
  }
...
void test_pascal_advanced_record_private_type_var_section(void) {
  assert_pascal_unit_parses_snippet(
      "advanced_record_private_type_var_section.pas");
}

void test_pascal_advanced_record_private_type_object_directives(void) {
  assert_pascal_unit_parses_snippet(
      "advanced_record_private_type_object_directives.pas");
}
...
void test_pascal_unit_include_routines(void) {
  ...
  if (res.is_success) {
    ...
    TEST_ASSERT(section_has_routine_named(interface_sec, PASCAL_T_FUNCTION_DECL,
                                          "Supports"));
    TEST_ASSERT(section_has_routine_named(interface_sec, PASCAL_T_FUNCTION_DECL,
                                          "StringToGUID"));
    TEST_ASSERT(section_has_routine_named(
        interface_sec, PASCAL_T_PROCEDURE_DECL, "InitExceptions"));

    TEST_ASSERT(section_has_routine_named(implementation_sec,
                                          PASCAL_T_FUNCTION_DECL, "Supports"));
    TEST_ASSERT(section_has_routine_named(
        implementation_sec, PASCAL_T_FUNCTION_DECL, "StringToGUID"));
    TEST_ASSERT(section_has_routine_named(
        implementation_sec, PASCAL_T_PROCEDURE_DECL, "InitExceptions"));
...
void test_pascal_unit_advanced_record_public_type(void) {
  ...
  if (res.is_success) {
    ...
    TEST_ASSERT(section_has_routine_named(interface_sec, PASCAL_T_FUNCTION_DECL,
                                          "AfterRecord"));
    TEST_ASSERT(section_has_routine_named(
        implementation_sec, PASCAL_T_FUNCTION_DECL, "AfterRecord"));
...
void test_pascal_unit_type_helper_inheritance(void) {
  ...
  if (res.is_success) {
    ...
    TEST_ASSERT(section_has_routine_named(interface_sec, PASCAL_T_FUNCTION_DECL,
                                          "AfterHelper"));
    TEST_ASSERT(section_has_routine_named(
        implementation_sec, PASCAL_T_FUNCTION_DECL, "AfterHelper"));
...
void test_pascal_interspersed_decls_in_procedure(void) {
  combinator_t *p = get_program_parser();
  input_t *input = new_input();
  char *program = load_pascal_file(
      "../../../../tests/test_cases/interspersed_decls_in_procedure.p");
  TEST_ASSERT(program != NULL);
  if (!program) {
    free_input(input);
    return;
...
void test_pascal_delphi_operator_overload_names(void) {
  ...
  const char *source =
      ...
      "begin\n"
      "end;\n"
      "class operator TVector.Multiply(const V: TVector; S: Integer): "
      "TVector;\n"
      "begin\n"
      "end;\n"
      "begin\n"
...
void test_pascal_method_with_var_section(void) {
  ...
  const char *source = "program Test;\n"
                       "type\n"
                       "  TTest = record\n"
                       "    X: Integer;\n"
                       "  end;\n"
...
void test_pascal_program_forward_declaration(void) {
  ...
  const char *source = "program TestForward;\n"
                       "\n"
  ...
  if (res.is_success) {
    ...
    // Count the procedure declarations (should be 3: forward decl + First +
    // Second impl)
    int proc_count = 0;
...
void test_pascal_program_external_declaration(void) {
  ...
  const char *source = "program TestExternal;\n"
                       "\n"
                       "uses ctypes;\n"
                       "\n"
  ...
  if (res.is_success) {
    // Verify the AST has the external function declaration separate from main
    // block
    ast_t *ast = res.value.ast;
    TEST_ASSERT(ast != NULL);
    TEST_ASSERT(ast->typ == PASCAL_T_PROGRAM_DECL);
...
void test_pascal_unit_forward_declaration(void) {
  ...
  const char *source = "unit TestUnit;\n"
                       "\n"
                       "interface\n"
                       "\n"
                       "implementation\n"
  ...
  if (res.is_success) {
    // Verify the AST has the implementation section with three procedure
    // declarations
    ast_t *ast = res.value.ast;
    TEST_ASSERT(ast != NULL);
    TEST_ASSERT(ast->typ == PASCAL_T_UNIT_DECL);
...
// Test for FPC system.pp compatibility - documents parser bugs with comments
// containing keywords This test currently FAILS due to:
...
// Test for Delphi-style form unit with class, method implementation, private
// vars/consts
...
void test_specialize_multi_arg(void) {
  ...
  const char *src =
      ...
      "type\n"
      "  TFoo = record\n"
      "    generic class procedure A<T>(const X: T; var D: Array of Byte); "
      "static;\n"
      "  end;\n"
      "implementation\n"
...
// Regression: function declaration with dynamic-array alias and
...
void test_function_dynarray_alias_ppchar_return(void) {
  ...
  const char *src = "unit U;\n"
                    ...
                    "function ArrayStringToPPchar(const values: "
                    "TAnsiStringArray; StartIndex: Integer): PPAnsiChar;\n"
                    "implementation\n"
                    "end.";
  input->buffer = strdup(src);
  input->length = (int)strlen(src);
...
TEST_LIST = {
    ...
    {"test_pascal_stray_identifier_before_begin",
     test_pascal_stray_identifier_before_begin},
    {"test_pascal_preprocessor_conditionals",
     test_pascal_preprocessor_conditionals},
    {"test_pascal_preprocessor_comment_mixing",
     test_pascal_preprocessor_comment_mixing},
    {"test_pascal_preprocessor_numeric_comparisons",
     test_pascal_preprocessor_numeric_comparisons},
    {"test_pascal_function_call", test_pascal_function_call},
    {"test_pascal_function_call_with_escaped_quote",
     test_pascal_function_call_with_escaped_quote},
    {"test_pascal_call_chaining_proc_cast",
     test_pascal_call_chaining_proc_cast},
    {"test_pascal_inherited_expression", test_pascal_inherited_expression},
    {"test_pascal_inherited_expression_no_method",
     test_pascal_inherited_expression_no_method},
    {"test_pascal_specialize_typecast_expression",
     test_pascal_specialize_typecast_expression},
    {"test_pascal_string_literal", test_pascal_string_literal},
    {"test_pascal_function_call_no_args", test_pascal_function_call_no_args},
    {"test_pascal_function_call_with_args",
     test_pascal_function_call_with_args},
    ...
    {"test_pascal_comprehensive_expression",
     test_pascal_comprehensive_expression},
    ...
    {"test_pascal_as_operator_with_field_access",
     test_pascal_as_operator_with_field_access},
    // Statement tests
    ...
    {"test_pascal_array_access_with_deref",
     test_pascal_array_access_with_deref},
    // New failing tests for missing features
    {"test_pascal_enumerated_type_declaration",
     test_pascal_enumerated_type_declaration},
    {"test_pascal_simple_const_declaration",
     test_pascal_simple_const_declaration},
    {"test_pascal_complex_const_declaration",
     test_pascal_complex_const_declaration},
    ...
    {"test_pascal_pointer_operations_program",
     test_pascal_pointer_operations_program},
    {"test_pascal_record_member_access_program",
     test_pascal_record_member_access_program},
    {"test_pascal_record_member_access_complete_program",
     test_pascal_record_member_access_complete_program},
    ...
    {"test_pascal_set_of_enum_typed_constant",
     test_pascal_set_of_enum_typed_constant},
    {"test_pascal_generic_type_declaration",
     test_pascal_generic_type_declaration},
    {"test_pascal_generic_record_type_declaration",
     test_pascal_generic_record_type_declaration},
    {"test_pascal_generic_class_tfpg_declaration",
     test_pascal_generic_class_tfpg_declaration},
    {"test_pascal_generic_class_public_type_specialize",
     test_pascal_generic_class_public_type_specialize},
    {"test_pascal_advanced_record_private_type_var_section",
     test_pascal_advanced_record_private_type_var_section},
    {"test_pascal_advanced_record_private_type_object_directives",
     test_pascal_advanced_record_private_type_object_directives},
    {"test_pascal_specialize_alias", test_pascal_specialize_alias},
    {"test_pascal_class_function_modifier",
     test_pascal_class_function_modifier},
    {"test_pascal_class_operator_overload",
     test_pascal_class_operator_overload},
    ...
    {"test_finalization_initialization_exports",
     test_finalization_initialization_exports},
    ...
    {"test_generic_interface_method_delegation",
     test_generic_interface_method_delegation},
    ...
    {"test_pascal_unit_advanced_record_public_type",
     test_pascal_unit_advanced_record_public_type},
    {"test_pascal_unit_advanced_record_nested_type_array",
     test_pascal_unit_advanced_record_nested_type_array},
    {"test_pascal_unit_type_helper_inheritance",
     test_pascal_unit_type_helper_inheritance},
    {"test_pascal_unit_bitpacked_variant_record",
     test_pascal_unit_bitpacked_variant_record},
    ...
    {"test_whitespace_around_ifdef_condition",
     test_whitespace_around_ifdef_condition},
    {"test_deprecated_on_const", test_deprecated_on_const},
    {"test_variant_record_field_attributes",
     test_variant_record_field_attributes},
    {"test_pascal_interspersed_decls_in_procedure",
     test_pascal_interspersed_decls_in_procedure},
    {"test_pascal_fpc_operator_overload_symbols",
     test_pascal_fpc_operator_overload_symbols},
    {"test_pascal_delphi_operator_overload_names",
     test_pascal_delphi_operator_overload_names},
    {"test_pascal_method_with_var_section",
     test_pascal_method_with_var_section},
    {"test_pascal_operator_with_var_section",
     test_pascal_operator_with_var_section},
    {"test_pascal_constructor_destructor_with_var_section",
     test_pascal_constructor_destructor_with_var_section},
    {"test_pascal_program_forward_declaration",
     test_pascal_program_forward_declaration},
    {"test_pascal_program_external_declaration",
     test_pascal_program_external_declaration},
    {"test_pascal_unit_forward_declaration",
     test_pascal_unit_forward_declaration},
    ...
    {"test_program_routine_local_typed_consts",
     test_program_routine_local_typed_consts},
    {"test_program_case_with_begin_end_branch",
     test_program_case_with_begin_end_branch},
    {"test_program_routine_directive_before_body",
     test_program_routine_directive_before_body},
    ...
    {"test_generic_class_procedure_in_record",
     test_generic_class_procedure_in_record},
    ...
    {"test_function_dynarray_alias_ppchar_return",
     test_function_dynarray_alias_ppchar_return},
    {NULL, NULL}};
