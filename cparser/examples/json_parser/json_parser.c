...
static ParseResult number_fn(input_t *in, void *args, char *parser_name) {
  ...
  if (!isdigit(c) && c != '-') {
    ...
    return make_failure_v2(in, parser_name, strdup("Expected a number."), NULL);
  }
  if (c == '-') {
    c = read1(in);
    if (!isdigit(c)) {
      ...
      return make_failure_v2(in, parser_name,
                             strdup("Expected a digit after minus."), NULL);
    }
  }
  // consume digits
  while (in->start < in->length && isdigit(in->buffer[in->start]))
    in->start++;
  // check for .
  if (in->start < in->length && in->buffer[in->start] == '.') {
    in->start++; // consume .
    if (in->start >= in->length || !isdigit(in->buffer[in->start])) {
      restore_input_state(in, &state);
      return make_failure_v2(in, parser_name,
                             strdup("Invalid fractional part."), NULL);
    }
    while (in->start < in->length && isdigit(in->buffer[in->start]))
      in->start++;
  }
  ...
  if (in->start < in->length &&
      (in->buffer[in->start] == 'e' || in->buffer[in->start] == 'E')) {
    in->start++; // consume e/E
    if (in->start < in->length &&
        (in->buffer[in->start] == '+' || in->buffer[in->start] == '-'))
      in->start++; // consume +/-
    if (in->start >= in->length || !isdigit(in->buffer[in->start])) {
      restore_input_state(in, &state);
      return make_failure_v2(in, parser_name, strdup("Invalid exponent part."),
                             NULL);
    }
    while (in->start < in->length && isdigit(in->buffer[in->start]))
      in->start++;
  ...
  if (len == 0 || (len == 1 && in->buffer[start_pos] == '-')) {
    restore_input_state(in, &state);
    return make_failure_v2(in, parser_name, strdup("Invalid number."), NULL);
  }
...
combinator_t *json_parser() {
  ...
  *p_json_value = new_combinator();
  ...
  // Recursive definitions for array and object, using new lazy proxies each
  // time
  combinator_t *kv_pair =
      seq(new_combinator(), JSON_T_ASSIGN, json_string(JSON_T_STRING),
          expect(match(":"), "Expected ':'"), lazy(p_json_value), NULL);
  combinator_t *j_array = seq(new_combinator(), JSON_T_SEQ, match("["),
                              sep_by(lazy(p_json_value), match(",")),
                              expect(match("]"), "Expected ']'"), NULL);
  combinator_t *j_object =
      seq(new_combinator(), JSON_T_SEQ, match("{"), sep_by(kv_pair, match(",")),
          expect(match("}"), "Expected '}'"), NULL);

  // The main json_value parser is a `multi` choice between all possible types.
  multi(*p_json_value, JSON_T_NONE, j_string, j_number, j_null, j_bool, j_array,
        j_object, NULL);
