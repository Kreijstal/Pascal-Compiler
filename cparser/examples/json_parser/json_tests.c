...
static void run_json_fail_test(const char *text, const char *name) {
  ...
  // We need to check that EITHER the parser fails, OR it succeeds but doesn't
...
void check_true(ast_t *ast) {
  TEST_ASSERT(ast->typ == JSON_T_INT && strcmp(ast->sym->name, "1") == 0);
}
void check_false(ast_t *ast) {
  TEST_ASSERT(ast->typ == JSON_T_INT && strcmp(ast->sym->name, "0") == 0);
}
void check_string(ast_t *ast) {
  TEST_ASSERT(ast->typ == JSON_T_STRING &&
              strcmp(ast->sym->name, "hello world") == 0);
}
void check_int(ast_t *ast) {
  TEST_ASSERT(ast->typ == JSON_T_INT && strcmp(ast->sym->name, "123") == 0);
}
void check_float(ast_t *ast) {
  TEST_ASSERT(ast->typ == JSON_T_INT && strcmp(ast->sym->name, "-123.45") == 0);
}
void check_sci(ast_t *ast) {
  TEST_ASSERT(ast->typ == JSON_T_INT &&
              strcmp(ast->sym->name, "6.022e23") == 0);
}
void check_empty_array(ast_t *ast) {
  TEST_ASSERT(ast->typ == JSON_T_SEQ && ast->child == ast_nil);
}
void check_empty_object(ast_t *ast) {
  TEST_ASSERT(ast->typ == JSON_T_SEQ && ast->child == ast_nil);
}
...
void check_simple_array(ast_t *ast) {
  ...
  TEST_ASSERT(child2->typ == JSON_T_STRING &&
              strcmp(child2->sym->name, "two") == 0);
...
void check_simple_object(ast_t *ast) {
  ...
  TEST_ASSERT(kv1->child->typ == JSON_T_STRING &&
              strcmp(kv1->child->sym->name, "key") == 0);
  TEST_ASSERT(kv1->child->next->typ == JSON_T_STRING &&
              strcmp(kv1->child->next->sym->name, "value") == 0);
  ast_t *kv2 = kv1->next;
  TEST_ASSERT(kv2->typ == JSON_T_ASSIGN);
  TEST_ASSERT(kv2->child->typ == JSON_T_STRING &&
              strcmp(kv2->child->sym->name, "n") == 0);
  TEST_ASSERT(kv2->child->next->typ == JSON_T_INT &&
              strcmp(kv2->child->next->sym->name, "123") == 0);
}
...
TEST_LIST = {{"json_successes", test_json_successes},
