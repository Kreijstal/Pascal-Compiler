...
void init_calculator_parser(combinator_t **p) {
  combinator_t *factor =
      expect(multi(new_combinator(), CALC_T_NONE,
  ...
  expr_insert(*p, 0, CALC_T_ADD, EXPR_INFIX, ASSOC_LEFT,
              expect(token(match("+")), "Expected '+' operator"));
  expr_altern(*p, 0, CALC_T_SUB,
              expect(token(match("-")), "Expected '-' operator"));
  expr_insert(*p, 1, CALC_T_MUL, EXPR_INFIX, ASSOC_LEFT,
              expect(token(match("*")), "Expected '*' operator"));
  expr_altern(*p, 1, CALC_T_DIV,
              expect(token(match("/")), "Expected '/' operator"));
  expr_insert(*p, 2, CALC_T_NEG, EXPR_PREFIX, ASSOC_NONE,
              expect(token(match("-")), "Expected '-' for negation"));
}
