...
struct input_t {
  ...
  int line;
  int col;
  int source_line; // Original source line (survives backtracking, updated by
                   // #line directives)
  ...
  char *source_filename;    // Current source filename from {#line N "file"}
                            // directive
  memo_table_t *memo;
};
...
typedef struct ParseError {
  ...
  const char *format_arg; // Optional: if non-NULL, message is a format template
                          // with one %s arg
} ParseError;
...
typedef struct type_dispatch_args {
  ...
  combinator_t
      *class_of_parser; /* For "class of <typename>" class reference type */
  combinator_t *record_parser;
  combinator_t *object_parser; /* For legacy "object" type (similar to record
                                  with methods) */
  ...
  combinator_t *identifier_parser;
  combinator_t
      *distinct_type_parser; /* For "type <typename>" distinct type syntax */
  combinator_t *distinct_type_range_parser; /* For "type <range>" distinct type
                                               from subrange */
  combinator_t *procedure_parser; /* For traditional procedure type (without
                                     "reference to") */
  combinator_t *function_parser;  /* For traditional function type (without
                                     "reference to") */
