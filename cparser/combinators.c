...
static error_position_t error_best_position(ParseError *err) {
  ...
  error_position_t best = {
      .index = err->index, .line = err->line, .col = err->col};
  if (err->cause != NULL) {
    error_position_t nested = error_best_position(err->cause);
    if (nested.index > best.index ||
        (nested.index == best.index &&
         (nested.line > best.line ||
...
static bool is_committed_failure(const ParseResult *res) {
  return res != NULL && !res->is_success && res->value.error != NULL &&
         res->value.error->committed;
}
...
static ParseResult expect_fn(input_t *in, void *args, char *parser_name) {
  ...
  if (res.value.error && res.value.error->unexpected) {
    if (asprintf(&final_message, "%s but found '%s'", eargs->msg,
                 res.value.error->unexpected) < 0) {
...
static ast_t *append_remaining(input_t *in, sep_by_args *sargs, ast_t *tail) {
  while (1) {
    ...
    // Check if we consumed any input - prevents infinite loop on epsilon
    // matches
...
static ParseResult sep_by_common(input_t *in, sep_by_args *sargs,
                                 bool require_first) {
...
static ParseResult sep_end_by_fn(input_t *in, void *args, char *parser_name) {
  ...
  InputState final_sep_state;
  save_input_state(in, &final_sep_state);
...
static ParseResult chainl1_fn(input_t *in, void *args, char *parser_name) {
  ...
  while (1) {
    ...
    if (!right_res.is_success) {
      ...
      return wrap_failure(in,
                          strdup("Expected operand after operator in chainl1"),
                          parser_name, right_res);
    }
...
static ParseResult many_fn(input_t *in, void *args, char *parser_name) {
  ...
  while (1) {
    ...
    // Check if the parser consumed any input - prevents infinite loop on
    // epsilon matches
    if (in->start == state.start) {
  ...
  while (seq != NULL) {
    ...
    if (!res.is_success) {
      ...
      if (res.value.error->message == NULL) {
        return wrap_failure_with_ast(in, "Failed to parse sequence.", res,
                                     head);
      }
...
static ParseResult multi_fn(input_t *in, void *args, char *parser_name) {
  ...
  if (!has_best) {
    return make_failure_static(in,
                               "multi parser had no valid failure to report");
  }
...
static ParseResult left_fn(input_t *in, void *args, char *parser_name) {
  ...
  if (!r2.is_success) {
    restore_input_state(in, &state);
    return wrap_failure_with_ast(in, "left combinator failed on second parser",
                                 r2, r1.value.ast);
  }
...
combinator_t *seq(combinator_t *ret, tag_t typ, combinator_t *c1, ...) {
  ...
  int count = 1;
  size_t total_name_len =
      strlen("sequence of ") + (c1->name ? strlen(c1->name) : 0);
...
combinator_t *flatMap(combinator_t *p, flatMap_func func) {
  ...
  args->func = func;
  combinator_t *comb = new_combinator();
  int ret = asprintf(&comb->name, "flatMap over %s",
                     p->name ? p->name : "unnamed_parser");
  (void)ret;
  comb->type = COMB_FLATMAP;
  comb->fn = flatMap_fn;
  comb->args = args;
  return comb;
}
...
combinator_t *left(combinator_t *p1, combinator_t *p2) {
  ...
  args->p2 = p2;
  combinator_t *comb = new_combinator();
  int ret = asprintf(&comb->name, "left of %s and %s",
                     p1->name ? p1->name : "unnamed_parser",
                     p2->name ? p2->name : "unnamed_parser");
  (void)ret;
  comb->type = COMB_LEFT;
  comb->fn = left_fn;
  comb->args = (void *)args;
...
combinator_t *right(combinator_t *p1, combinator_t *p2) {
  ...
  args->p2 = p2;
  combinator_t *comb = new_combinator();
  int ret = asprintf(&comb->name, "right of %s and %s",
                     p1->name ? p1->name : "unnamed_parser",
                     p2->name ? p2->name : "unnamed_parser");
  (void)ret;
  comb->type = COMB_RIGHT;
  comb->fn = right_fn;
  comb->args = (void *)args;
  return comb;
}
...
combinator_t *gseq(combinator_t *ret, tag_t typ, combinator_t *c1, ...) {
  ...
  int count = 1;
  size_t total_name_len =
      strlen("gseq of ") + (c1->name ? strlen(c1->name) : 0);
...
combinator_t *between(combinator_t *open, combinator_t *close,
                      combinator_t *p) {
  ...
  args->p = p;
  combinator_t *comb = new_combinator();
  int ret = asprintf(&comb->name, "between %s and %s",
                     open->name ? open->name : "unnamed_parser",
                     close->name ? close->name : "unnamed_parser");
  (void)ret;
...
combinator_t *errmap(combinator_t *p, err_map_func func) {
  ...
  args->func = func;
  combinator_t *comb = new_combinator();
  int ret = asprintf(&comb->name, "errmap over %s",
                     p->name ? p->name : "unnamed_parser");
  (void)ret;
...
combinator_t *commit(combinator_t *p) {
  combinator_t *comb = new_combinator();
  int ret =
      asprintf(&comb->name, "commit(%s)", p->name ? p->name : "unnamed_parser");
  (void)ret;
...
combinator_t *map(combinator_t *p, map_func func) {
  ...
  args->func = func;
  combinator_t *comb = new_combinator();
  int ret = asprintf(&comb->name, "map over %s",
                     p->name ? p->name : "unnamed_parser");
  (void)ret;
  comb->type = COMB_MAP;
  comb->fn = map_fn;
...
combinator_t *sep_by(combinator_t *p, combinator_t *sep) {
  ...
  args->sep = sep;
  combinator_t *comb = new_combinator();
  int ret = asprintf(&comb->name, "%s separated by %s",
                     p->name ? p->name : "unnamed_parser",
                     sep->name ? sep->name : "unnamed_parser");
  (void)ret;
...
combinator_t *sep_by1(combinator_t *p, combinator_t *sep) {
  ...
  args->sep = sep;
  combinator_t *comb = new_combinator();
  int ret = asprintf(&comb->name, "%s separated by1 %s",
                     p->name ? p->name : "unnamed_parser",
                     sep->name ? sep->name : "unnamed_parser");
  (void)ret;
...
combinator_t *sep_end_by(combinator_t *p, combinator_t *sep) {
  sep_end_by_args *args =
      (sep_end_by_args *)safe_malloc(sizeof(sep_end_by_args));
  args->p = p;
  args->sep = sep;
  combinator_t *comb = new_combinator();
  int ret = asprintf(&comb->name, "%s separated and ended by %s",
                     p->name ? p->name : "unnamed_parser",
                     sep->name ? sep->name : "unnamed_parser");
  (void)ret;
...
combinator_t *chainl1(combinator_t *p, combinator_t *op) {
  ...
  args->op = op;
  combinator_t *comb = new_combinator();
  int ret = asprintf(&comb->name, "chainl1 of %s with %s",
                     p->name ? p->name : "unnamed_parser",
                     op->name ? op->name : "unnamed_parser");
  (void)ret;
...
combinator_t *optional(combinator_t *p) {
  ...
  args->p = p;
  combinator_t *comb = new_combinator();
  int ret = asprintf(&comb->name, "optional %s",
                     p->name ? p->name : "unnamed_parser");
  (void)ret;
