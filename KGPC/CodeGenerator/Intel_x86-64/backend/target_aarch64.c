...
static void aa_emit(BeEmitter *em, BeOp op, BeWidth w, const BeOperand *dst,
  ...
  case BE_CMP:
    if (a->kind == OPK_MEM_FRAME || b->kind == OPK_MEM_FRAME) {
      /* AArch64 has no compare-with-memory: load each frame operand into its
       * own scratch register, then compare register-to-{register,immediate}.
       * The load-then-compare sequence is the neutral counterpart to x86's
       * single cmp <mem>,<reg>. */
      Register_t *scr_a = NULL, *scr_b = NULL;
      BeOperand sreg_a, sreg_b;
      const BeOperand *na = a, *nb = b;
      if (a->kind == OPK_MEM_FRAME) {
        const char *base = (a->u.mem_frame.base == BE_BASE_SP) ? "sp" : "x29";
        scr_a = get_free_reg(get_reg_stack(), &em->list);
        assert(scr_a != NULL && "no scratch for aarch64 cmp-with-frame");
        ...
        em->list = be_add_inst_du_w(em->list, vregp(em), defs, 1, NULL, 0, tmpl,
                                    use32);
        sreg_a = (BeOperand){OPK_VREG, w, {.vreg = scr_a}};
        na = &sreg_a;
      }
      if (b->kind == OPK_MEM_FRAME) {
        const char *base = (b->u.mem_frame.base == BE_BASE_SP) ? "sp" : "x29";
        scr_b = get_free_reg(get_reg_stack(), &em->list);
        assert(scr_b != NULL && "no scratch for aarch64 cmp-with-frame");
        snprintf(tmpl, sizeof(tmpl), "\tldr\t%%0, [%s, #%lld]\n", base,
                 b->u.mem_frame.disp);
        defs[0] = scr_b;
        use32[0] = w32;
        em->list = be_add_inst_du_w(em->list, vregp(em), defs, 1, NULL, 0, tmpl,
                                    use32);
        sreg_b = (BeOperand){OPK_VREG, w, {.vreg = scr_b}};
        nb = &sreg_b;
      }
      aa_emit(em, BE_CMP, w, NULL, na, nb); /* reg/reg or reg/imm compare */
      if (scr_b != NULL)
        free_reg(get_reg_stack(), scr_b);
      if (scr_a != NULL)
        free_reg(get_reg_stack(), scr_a);
      break;
    }
