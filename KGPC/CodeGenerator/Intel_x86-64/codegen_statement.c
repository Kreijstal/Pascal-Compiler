                                    ...
                                    if (ctor_iface_count > 0) {
                                        ...
                                        for (int ii = 0; ii < class_record->num_interfaces; ii++) {
                                            ...
                                            snprintf(buffer, sizeof(buffer), "\tleaq\t%s_INTF_%s_VTABLE(%%rip), %s\n",
                                                class_type_id, iname, ivtbl_reg->bit_64);
                                            inst_list = add_inst(inst_list, buffer);
...
// removed assert on proc_name
    ...
    else if (call_hash_type == HASHTYPE_PROCEDURE)
    {
            ...
            stmt->stmt_data.procedure_call_data.vmt_index >= 0)
        {
            ...
            static int intf_dispatch_label_counter = 0;
            int label_id = intf_dispatch_label_counter++;
            ...
            inst_list = add_inst(inst_list, buffer);
            snprintf(buffer, sizeof(buffer), "\tjz\t.Lintf_direct_%d\n", label_id);
            inst_list = add_inst(inst_list, buffer);
