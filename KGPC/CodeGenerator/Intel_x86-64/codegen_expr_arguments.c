...
/* Static inline helpers duplicated from codegen_expression.c (established
 * codebase pattern) */
...
static int codegen_var_decl_contains_id(const Tree_t *decl,
                                        const char *var_id) {
...
static const Tree_t *codegen_find_var_decl_in_list(ListNode_t *decls,
                                                   const char *var_id) {
...
static const Tree_t *codegen_find_var_decl_for_symbol(
    CodeGenContext *ctx, const HashNode_t *sym_node, const char *var_id) {
...
static const Tree_t *codegen_find_var_decl_for_unit(CodeGenContext *ctx,
                                                    ...
                                                    const char *var_id) {
  if (ctx == NULL || ctx->comp_ctx == NULL || source_unit_index <= 0 ||
      var_id == NULL)
    return NULL;
...
codegen_unwrap_typecast_call_expr(struct Expression *expr, SymTab_t *symtab) {
  ...
  if (FindSymbol(&type_node, symtab, id) == 0 || type_node == NULL ||
...
static const char *
codegen_outer_owner_class_from_full(const char *owner_class,
                                    const char *owner_class_full, char *buffer,
                                    size_t size) {
...
static StackNode_t *codegen_find_nonlocal_lexical(CodeGenContext *ctx,
  ...
  if (FindSymbol(&sym_node, ctx->symtab, var_id) != 0 && sym_node != NULL) {
    ...
    if (sym_node->is_typed_const && sym_node->source_unit_index > 0) {
      char *qualified =
          codegen_make_unit_qualified_key(sym_node->source_unit_index, var_id);
      if (qualified != NULL) {
...
static ListNode_t *codegen_try_emit_nonlocal_global(ListNode_t *inst_list,
  ...
  if (effective_node != NULL &&
      (effective_node->hash_type == HASHTYPE_VAR ||
       effective_node->hash_type == HASHTYPE_ARRAY) &&
      (effective_node->source_unit_index > 0 ||
       effective_node->defined_in_unit)) {
    decl = codegen_find_var_decl_for_symbol(ctx, effective_node, var_id);
  }
  if (decl == NULL && ctx->symtab->current_unit_index > 0)
    decl = codegen_find_var_decl_for_unit(ctx, ctx->symtab->current_unit_index,
                                          var_id);

  const char *global_symbol =
      codegen_global_access_symbol_for_decl(decl, var_id);
...
static ListNode_t *codegen_try_emit_nonlocal_class_var(ListNode_t *inst_list,
  ...
  if (ctx->current_subprogram_owner_class_full != NULL) {
    const char *outer = codegen_outer_owner_class_from_full(
        ...
        ctx->current_subprogram_owner_class_full, outer_class_buf,
        sizeof(outer_class_buf));
  ...
  for (int i = 0; i < class_count; ++i) {
    struct RecordType *class_record =
        semcheck_lookup_record_type(ctx->symtab, class_labels[i]);
...
codegen_lookup_named_record_type(CodeGenContext *ctx, const char *type_name) {
...
static int
codegen_call_requires_class_method_vmt_self(const struct Expression *call_expr,
                                            CodeGenContext *ctx) {
  ...
  if (owner_class_name == NULL || method_name == NULL) {
    HashNode_t *resolved =
        call_expr->expr_data.function_call_data.resolved_func;
    if (resolved != NULL) {
  ...
  if (owner_class_name != NULL && method_name != NULL) {
    ...
    if (from_cparser_is_method_nonstatic_class_method(owner_class_name,
                                                      method_name) &&
  ...
  if (call_expr->expr_data.function_call_data.is_class_method_call &&
    ...
    owner_record = codegen_lookup_named_record_type(
        ctx, call_expr->expr_data.function_call_data.self_class_name);
    if (owner_record != NULL && record_type_is_class(owner_record))
      return 1;
  }
...
static int codegen_current_param_is_ansistring(const struct Expression *expr,
  ...
  for (ListNode_t *cur = ctx->current_subprogram_args; cur != NULL;
       cur = cur->next) {
    Tree_t *decl = (Tree_t *)cur->cur;
...
static int codegen_self_param_is_class(Tree_t *formal_arg_decl,
                                       CodeGenContext *ctx) {
...
static void arginfo_register_spill_handler(Register_t *reg,
                                           StackNode_t *spill_slot,
                                           void *context) {
...
static void arginfo_assign_register(ArgInfo *info, Register_t *reg,
                                    struct Expression *expr) {
  if (info == NULL)
    return;
  info->reg = reg;
...
static int formal_decl_expects_string(Tree_t *decl, SymTab_t *symtab) {
  ...
  if (decl->tree_data.var_decl_data.type_id != NULL) {
    ...
    if (symtab != NULL) {
      HashNode_t *type_node = NULL;
      if (FindSymbol(&type_node, symtab, type_id) != 0 && type_node != NULL &&
...
static int builtin_arg_expects_string(const char *procedure_name,
                                      int arg_index) {
  (void)procedure_name;
  (void)arg_index;
...
static int mangled_call_expects_char(const struct Expression *call_expr,
                                     int arg_index) {
  if (call_expr == NULL || call_expr->type != EXPR_FUNCTION_CALL)
    return 0;
...
static int codegen_expr_is_wide_string_value(const struct Expression *expr) {
  ...
  if (expr->type == EXPR_ADDOP &&
    ...
    return codegen_expr_is_wide_string_value(
...
static int codegen_param_real_storage_size(Tree_t *decl, SymTab_t *symtab) {
  ...
  if (decl->type == TREE_VAR_DECL) {
    ...
    if (decl->tree_data.var_decl_data.cached_kgpc_type != NULL) {
      ...
      if (kgpc_type_is_extended(decl->tree_data.var_decl_data.cached_kgpc_type))
        return 16;
      long long size =
          kgpc_type_sizeof(decl->tree_data.var_decl_data.cached_kgpc_type);
      if (size > 0) {
        return (int)size;
      }
    }
  ...
  if (decl->type == TREE_VAR_DECL &&
      decl->tree_data.var_decl_data.type_id != NULL && symtab != NULL) {
    HashNode_t *type_node = NULL;
...
static int codegen_expr_real_storage_size(const struct Expression *expr,
                                          CodeGenContext *ctx) {
  if (expr == NULL)
    return 8;
...
static int codegen_formal_shortstring_size(Tree_t *decl, SymTab_t *symtab) {
  ...
  if (decl->type == TREE_ARR_DECL) {
    struct Array *arr = &decl->tree_data.arr_decl_data;
    if (arr->is_shortstring && arr->e_range >= arr->s_range &&
        arr->e_range >= 0) {
  ...
  if (decl->type == TREE_VAR_DECL) {
    ...
    if (cached != NULL) {
      ...
      if (cached_alias != NULL && cached_alias->is_shortstring &&
          cached_alias->array_end >= cached_alias->array_start &&
          cached_alias->array_end >= 0) {
...
static inline RegisterId_t codegen_arg_reg_id_num(int num) {
  ...
  static const RegisterId_t sysv_regs[] = {REG_RDI, REG_RSI, REG_RDX,
                                           REG_RCX, REG_R8,  REG_R9};
  const RegisterId_t *regs = (g_current_codegen_abi == KGPC_TARGET_ABI_WINDOWS)
                                 ? windows_regs
                                 : sysv_regs;
...
static int codegen_formal_is_dynamic_array(Tree_t *formal, SymTab_t *symtab) {
  ...
  if (symtab != NULL && formal->tree_data.var_decl_data.type_id != NULL) {
    HashNode_t *type_node = NULL;
    if (FindSymbol(&type_node, symtab,
                   formal->tree_data.var_decl_data.type_id) != 0 &&
        type_node != NULL && type_node->type != NULL &&
...
static int
codegen_expr_is_open_array_descriptor_arg(const struct Expression *expr,
                                          CodeGenContext *ctx) {
  if (expr == NULL)
    return 0;
...
ListNode_t *codegen_get_nonlocal(ListNode_t *inst_list, char *var_id,
  ...
  HashNode_t *sym_node = NULL;
  StackNode_t *var =
      codegen_find_nonlocal_lexical(ctx, var_id, &scope_depth, &sym_node);

  if (var == NULL) {
    ListNode_t *resolved = codegen_try_emit_nonlocal_global(
        inst_list, var_id, ctx, sym_node, offset);
    if (resolved != NULL)
      return resolved;

    resolved =
        codegen_try_emit_nonlocal_class_var(inst_list, var_id, ctx, offset);
    if (resolved != NULL)
      return resolved;
    ...
    if (ctx != NULL && ctx->current_subprogram_owner_class != NULL &&
      ...
      if (self_slot != NULL) {
        ...
        if (class_record == NULL &&
          ...
          class_record = semcheck_lookup_record_type(
        ...
        if (class_record != NULL) {
            ...
            *offset = 0;
            if (codegen_record_has_class_var_named(class_record, var_id) ||
              ...
              inst_list = codegen_emit_classvar_base_address_named(
                  inst_list, current_non_local_reg64(), class_record,
                  field_offset);
            } else {
              /* Load Self pointer, then add field offset. */
              {
                ...
                BeEmitter em = codegen_beemitter(inst_list, ctx);
                BeOperand dst = {OPK_PHYS, BE_W64, {.phys = current_non_local_reg64()}};
                BeOperand src = {OPK_MEM_FRAME, BE_W64,
                                 {.mem_frame = {BE_BASE_FP, -(long long)(self_slot->offset)}}};
          ...
          for (ListNode_t *mn = class_record->methods; mn != NULL;
               mn = mn->next) {
            if (mn->cur == NULL)
              continue;
            struct MethodInfo *mi = (struct MethodInfo *)mn->cur;
            if (mi->name != NULL &&
                pascal_identifier_equals(mi->name, var_id)) {
          ...
          if (method_label == NULL) {
            ...
            while (method_label == NULL && search_record != NULL &&
              ...
              search_record = semcheck_lookup_record_type(
                  ctx->symtab, search_record->parent_class_name);
              if (search_record == NULL)
                break;
              for (ListNode_t *mn = search_record->methods; mn != NULL;
                   mn = mn->next) {
                if (mn->cur == NULL)
                  continue;
                struct MethodInfo *mi = (struct MethodInfo *)mn->cur;
    ...
    if (ctx != NULL && ctx->symtab != NULL && ctx->with_depth > 0) {
      for (int i = ctx->with_depth; i > 0; --i) {
        ...
        if (with_record == NULL || with_expr == NULL)
          continue;
        if (resolve_record_field(ctx->symtab, with_record, var_id, &field_desc,
                                 &field_offset, 0, 1) != 0 ||
            field_desc == NULL)
          continue;
        ...
        if (with_record->type_id != NULL &&
          ...
          if (addr_reg == NULL)
            return inst_list;
          inst_list = codegen_emit_classvar_base_address_named(
              inst_list, addr_reg->bit_64, with_record, field_offset);
        } else {
          inst_list =
              codegen_address_for_expr(with_expr, inst_list, ctx, &addr_reg);
          if (addr_reg == NULL)
            return inst_list;

          if (record_type_is_class(with_record)) {
            /* Integrated: same-register 64-bit pointer deref through the
             * vtable; the base is a tracked vreg USE (was a baked name,
    ...
    codegen_report_error(
        ctx, "ERROR: Unresolved non-local symbol %s reached codegen fallback.",
        var_id);
...
ListNode_t *codegen_pass_arguments(
  ...
  /* CRITICAL VALIDATION: Ensure formal_args is either NULL or properly
   * structured. This catches any remaining cases of corrupted list pointers. */
  if (formal_args != NULL) {
    /* Basic sanity check: formal_args should have a valid list type.
     * This catches cases where formal_args contains garbage data. */
    if (formal_args->type != LIST_TREE &&
        formal_args->type != LIST_UNSPECIFIED) {
      codegen_report_error(ctx,
                           "FATAL: Internal compiler error - corrupted "
                           "formal_args list (invalid type %d). "
                           "This may indicate a bug in the semantic checker or "
                           "memory corruption.",
                           formal_args->type);
      return inst_list;
    }
  ...
  enum { ARG_CLASS_INT = 0, ARG_CLASS_SSE = 1 };
  ...
  if (proc_type != NULL && proc_type->kind == TYPE_KIND_PROCEDURE &&
    ...
    if (def->type == TREE_SUBPROGRAM || def->type == TREE_SUBPROGRAM_PROC ||
        def->type == TREE_SUBPROGRAM_FUNC) {
  ...
  while (args != NULL) {
    ...
    if (arg_expr == NULL) {
      ...
      codegen_report_error(ctx,
                           "ERROR: NULL argument expression in call to %s at "
                           "argument position %d",
                           proc_name, arg_num);
      if (arg_infos != NULL)
        free(arg_infos);
    ...
    if (formal_args != NULL && !(skip_formal_for_self && arg_num == 0)) {
      /* CRITICAL VALIDATION: Before dereferencing formal_args, verify it's not
       * corrupted. On Cygwin/MSYS, corrupted list nodes can cause segfaults
       * when accessing ->cur. We check the list type to detect garbage values
       * early. */
      if (formal_args->type != LIST_TREE &&
          formal_args->type != LIST_UNSPECIFIED) {
        ...
        codegen_report_error(
            ctx,
            "FATAL: Internal compiler error - corrupted formal_args list node "
            "(type=%d) at argument %d for procedure %s. "
            "This indicates memory corruption or an improperly initialized "
            "list.",
    ...
    /* Also detect Self parameter when the argument expression IS the Self
     * variable */
    ...
    if (is_self_param && codegen_self_param_is_class(formal_arg_decl, ctx))
      is_var_param = 0;
    int is_array_param =
        (formal_arg_decl != NULL && formal_arg_decl->type == TREE_ARR_DECL);
    ...
    if (formal_arg_decl != NULL) {
      ...
      formal_is_char_set =
          formal_decl_is_char_set(formal_arg_decl, ctx->symtab);
      formal_is_dynarray =
          codegen_formal_is_dynamic_array(formal_arg_decl, ctx->symtab);
    }
    ...
    /* Also check if we're passing a static array argument (even if not declared
     * as var param) */
    int is_array_arg = (arg_expr != NULL && arg_expr->is_array_expr &&
                        !arg_expr->array_is_dynamic);
    ...
    int expected_type =
        codegen_param_expected_type(formal_arg_decl, ctx->symtab);
    ...
    if (formal_arg_decl != NULL && formal_arg_decl->type == TREE_VAR_DECL &&
        formal_arg_decl->tree_data.var_decl_data.is_untyped_param &&
        arg_expr != NULL && arg_expr->type != EXPR_POINTER_DEREF) {
      KgpcType *arg_type = expr_get_kgpc_type(arg_expr);
      int arg_is_pointer =
          (expr_has_type_tag(arg_expr, POINTER_TYPE) ||
    ...
    if ((procedure_name != NULL &&
         strcmp(procedure_name, "kgpc_trunc_currency") == 0) ||
        (call_mangled != NULL &&
         strcmp(call_mangled, "kgpc_trunc_currency") == 0))
    ...
    /* For type helper Self parameters passed by value, infer type from
     * expression. This ensures floating-point Self parameters use xmm
     * registers. */
    ...
    if (expected_type == UNKNOWN_TYPE && arg_expr != NULL &&
        arg_expr->type == EXPR_VAR_ID && arg_expr->expr_data.id != NULL &&
        ctx != NULL && ctx->symtab != NULL && !formal_is_open_array &&
        !is_array_param) {
    ...
    /* Self for type helpers over real types is passed by value (not by
     * reference). */
    if (is_self_param && expected_type == REAL_TYPE)
      is_var_param = 0;
    /* If the actual argument is REAL, pass it via SSE even when formal metadata
     * is missing. */
    if (arg_expr != NULL &&
        !((procedure_name != NULL &&
           strcmp(procedure_name, "kgpc_trunc_currency") == 0) ||
     ...
     * When formal parameter metadata is missing, infer by ref semantics from
     * the argument/expected type to avoid passing the file record by value. */
    if (!is_var_param) {
    ...
    if (is_array_arg && arg_expr != NULL && arg_expr->type == EXPR_STRING &&
        expected_type == POINTER_TYPE && !is_array_param &&
        !formal_is_open_array && !formal_is_dynarray) {
    ...
    int is_pointer_like =
        (is_var_param || is_array_param || is_array_arg || formal_is_dynarray);
    if (is_self_param && expected_type == REAL_TYPE)
      is_pointer_like = 0;
    ...
    if (expected_type == REAL_TYPE) {
      if ((procedure_name != NULL &&
           strncmp(procedure_name, "kgpc_", 5) == 0) ||
    ...
    if (arg_infos != NULL && expected_type == REAL_TYPE) {
      ...
      if (arg_expr != NULL) {
        int arg_type = expr_get_type_tag(arg_expr);
        expr_is_real =
            (arg_type == REAL_TYPE) || arg_expr->type == EXPR_RNUM ||
            (arg_expr->type == EXPR_TYPECAST &&
             arg_expr->expr_data.typecast_data.target_type == REAL_TYPE);
      }

      if (!force_runtime_real_qword &&
          arg_infos[arg_num].expected_real_size == 0) {
        if (is_self_param && expr_is_real) {
          ...
          arg_infos[arg_num].expected_real_size = 8;
                 ...
                 arg_infos[arg_num].expected_real_size == 8 && is_self_param &&
                 expr_is_real &&
    ...
    if (formal_is_open_array && arg_expr != NULL) {
      ...
      if (arg_is_open_array) {
        ...
        if (arg_expr->type == EXPR_VAR_ID && ctx->symtab != NULL) {
          ...
          if (FindSymbol(&arg_symbol, ctx->symtab, arg_expr->expr_data.id) !=
                  0 &&
        ...
        if (use_address && codegen_expr_is_addressable(arg_expr))
          inst_list =
              codegen_address_for_expr(arg_expr, inst_list, ctx, &value_reg);
        else
          inst_list =
              codegen_expr_with_result(arg_expr, inst_list, ctx, &value_reg);
        ...
        StackNode_t *desc_slot = codegen_alloc_temp_bytes(
            "openarr_desc", 2 * CODEGEN_POINTER_SIZE_BYTES);
        if (desc_slot == NULL) {
          codegen_report_error(ctx, "ERROR: Failed to allocate descriptor "
                                    "storage for open array argument.");
          return inst_list;
        }
        ...
        inst_list =
            codegen_address_for_expr(arg_expr, inst_list, ctx, &data_addr_reg);
        if (codegen_had_error(ctx) || data_addr_reg == NULL)
          return inst_list;

        Register_t *desc_addr_reg =
        ...
        {
          /* Integrated: 64-bit data-pointer store through the vtable; value
           * and base are tracked vreg USES (were baked names, invisible to
           * liveness). */
          BeEmitter em = codegen_beemitter(inst_list, ctx);
          BeOperand dst = {OPK_MEM_BD, BE_W64, {.mem_bd = {desc_addr_reg, 0}}};
          BeOperand a = {OPK_VREG, BE_W64, {.vreg = data_addr_reg}};
          kgpc_backend_target()->emit(&em, BE_STORE, BE_W64, &dst, &a, NULL);
          inst_list = em.list;
        }
        free_reg(get_reg_stack(), data_addr_reg);
        ...
        {
          /* Integrated: 64-bit element-count immediate store through the
           * vtable; the descriptor base is a tracked vreg USE (was a baked
           * name, invisible to liveness). */
          BeEmitter em = codegen_beemitter(inst_list, ctx);
          BeOperand dst = {OPK_MEM_BD, BE_W64, {.mem_bd = {desc_addr_reg, 8}}};
          BeOperand a = {OPK_IMM, BE_W64, {.imm = element_count}};
          kgpc_backend_target()->emit(&em, BE_STORE, BE_W64, &dst, &a, NULL);
          inst_list = em.list;
        }
        ...
        if (arg_spill != NULL && arg_infos != NULL) {
          {
            ...
            BeOperand dst = {OPK_MEM_FRAME, BE_W64,
                             {.mem_frame = {BE_BASE_FP, -(long long)(arg_spill->offset)}}};
            BeOperand a = {OPK_PHYS, BE_W64, {.phys = desc_addr_reg->bit_64}};
            kgpc_backend_target()->emit(&em, BE_STORE, BE_W64, &dst, &a, NULL);
            inst_list = em.list;
          }
    ...
    if (!arg_handled) {
      if (formal_is_open_array && arg_expr != NULL &&
        ...
        escape_string(escaped_str, str_data ? str_data : "",
                      sizeof(escaped_str));
        ...
        StackNode_t *desc_slot = codegen_alloc_temp_bytes(
            "str_arr_desc", 2 * CODEGEN_POINTER_SIZE_BYTES);
        if (desc_slot == NULL) {
          codegen_report_error(ctx, "ERROR: Unable to allocate descriptor for "
                                    "string literal to open array.");
          return inst_list;
        }
        ...
        /* Load string address */
        snprintf(buffer, sizeof(buffer), "\tleaq\t%s(%%rip), %s\n", label,
                 data_addr_reg->bit_64);
        inst_list = add_inst(inst_list, buffer);

        /* Store string pointer at descriptor[0].
         * Integrated: 64-bit store through the vtable; value and base are
         * tracked vreg USES (were baked names, invisible to liveness). */
        {
          BeEmitter em = codegen_beemitter(inst_list, ctx);
          BeOperand dst = {OPK_MEM_BD, BE_W64, {.mem_bd = {desc_addr_reg, 0}}};
          BeOperand a = {OPK_VREG, BE_W64, {.vreg = data_addr_reg}};
         ...
         * Integrated: immediate store through the vtable; the descriptor base
         * is a tracked vreg USE (was a baked name, invisible to liveness). */
        {
          BeEmitter em = codegen_beemitter(inst_list, ctx);
          BeOperand dst = {OPK_MEM_BD, BE_W64, {.mem_bd = {desc_addr_reg, 8}}};
          BeOperand a = {OPK_IMM, BE_W64, {.imm = str_len}};
          kgpc_backend_target()->emit(&em, BE_STORE, BE_W64, &dst, &a, NULL);
          inst_list = em.list;
        }
      ...
      /* Handle ShortString parameter (by value): materialize a ShortString
       * buffer matching the formal size and pass its address. */
      else if (expected_type == SHORTSTRING_TYPE && arg_expr != NULL &&
               !is_var_param) {
        int shortstr_size =
            codegen_formal_shortstring_size(formal_arg_decl, ctx->symtab);
        if (shortstr_size < 2)
          shortstr_size = 2;

        StackNode_t *shortstr_buf =
            codegen_alloc_temp_bytes("shortstr_arg", shortstr_size);
        ...
        if (codegen_expr_is_shortstring_value_ctx(arg_expr, ctx) &&
          ...
          if (codegen_expr_is_addressable(arg_expr)) {
            inst_list =
                codegen_address_for_expr(arg_expr, inst_list, ctx, &src_reg);
          } else {
            inst_list =
                codegen_expr_with_result(arg_expr, inst_list, ctx, &src_reg);
          }
          ...
          if (!codegen_get_char_array_length(arg_expr, ctx, &array_len) ||
              array_len <= 0) {
            free_reg(get_reg_stack(), buf_addr_reg);
            if (arg_infos != NULL)
              free(arg_infos);
          ...
          Register_t *src_reg = NULL;
          if (!codegen_expr_is_addressable(arg_expr)) {
            codegen_report_error(ctx, "ERROR: Unsupported expression type for "
          ...
          inst_list = add_inst(inst_list, "\tmovl\t$0, %eax\n");
          inst_list = codegen_call_with_shadow_space(
              inst_list, "kgpc_char_array_to_shortstring");
          free_arg_regs();
          free_reg(get_reg_stack(), src_reg);
        } else {
          Register_t *value_reg = NULL;
          ...
          if (expr_has_type_tag(arg_expr, CHAR_TYPE)) {
            ...
            StackNode_t *buf_save = add_l_t("shortstr_buf_save");
            {
              ...
              BeEmitter em = codegen_beemitter(inst_list, ctx);
              BeOperand dst = {OPK_MEM_FRAME, BE_W64,
                               {.mem_frame = {BE_BASE_FP, -(long long)(buf_save->offset)}}};
              BeOperand a = {OPK_PHYS, BE_W64, {.phys = buf_addr_reg->bit_64}};
              kgpc_backend_target()->emit(&em, BE_STORE, BE_W64, &dst, &a, NULL);
              inst_list = em.list;
            }

            const char *char_arg_reg32 =
        ...
        if (arg_expr != NULL && arg_expr->type == EXPR_NIL) {
          ...
          inst_list = codegen_materialize_array_literal(arg_expr, inst_list,
                                                        ctx, &addr_reg);
        } else {
          ...
          if (!codegen_expr_is_addressable(address_expr) &&
              address_expr != NULL &&
              address_expr->type == EXPR_FUNCTION_CALL && ctx != NULL &&
              ctx->symtab != NULL) {
          ...
          if (!codegen_expr_is_addressable(address_expr)) {
            ...
            codegen_report_error(
                ...
                "(expr_type=%d%s%s) in call to %s arg %d.",
                arg_expr != NULL ? arg_expr->type : -1,
                call_id != NULL ? " call_id=" : "",
                call_id != NULL ? call_id : "",
                procedure_name ? procedure_name : "(unknown)", arg_num);
            return inst_list;
          }
          inst_list =
              codegen_address_for_expr(address_expr, inst_list, ctx, &addr_reg);
           ...
           */
          if (addr_reg != NULL && call_expr != NULL &&
              call_expr->expr_data.function_call_data.builtin_call_lowering ==
                  BUILTIN_CALL_ASSIGNED &&
              address_expr != NULL && address_expr->type == EXPR_ARRAY_ACCESS &&
          ...
          if (addr_reg != NULL && formal_arg_decl != NULL &&
              formal_arg_decl->type == TREE_VAR_DECL &&
              formal_arg_decl->tree_data.var_decl_data.is_untyped_param &&
          ...
          /* BUGFIX: For TRUE var parameters of class types, we pass the ADDRESS
           * of the variable itself, not the value it contains. This allows the
           * callee to update the variable (e.g., FreeAndNil).
           *
           * However, for class methods, Self (first parameter) needs to be
           * dereferenced to pass the instance pointer, even though it's
           * technically a var parameter internally. */

          struct RecordType *arg_record = codegen_expr_record_type(
              arg_expr, ctx != NULL ? ctx->symtab : NULL);
          if (!forward_open_array_data && addr_reg != NULL &&
              arg_expr != NULL && arg_expr->type != EXPR_AS &&
              arg_record != NULL && record_type_is_class(arg_record)) {
            /* Check if the argument expression is itself a var parameter
             * variable. If so, codegen_address_for_expr already loaded the
             * instance pointer via movq, so we should NOT dereference again. */
            int arg_is_var_param = 0;
            if (arg_expr->type == EXPR_VAR_ID && ctx->symtab != NULL) {
              HashNode_t *arg_symbol = NULL;
              if (FindSymbol(&arg_symbol, ctx->symtab,
                             arg_expr->expr_data.id) != 0 &&
                  arg_symbol != NULL && arg_symbol->is_var_parameter) {
                arg_is_var_param = 1;
              }
            }
             ...
             *
             * For non-var class parameters (entered via
             * is_array_param/is_array_arg), we need to dereference to get the
             * instance pointer. For var parameters of class type, we pass the
             * address as-is so the callee can modify the variable. */
            ...
            if (should_dereference) {
              ...
              BeOperand dst = {OPK_VREG, BE_W64, {.vreg = addr_reg}};
              BeOperand src = {OPK_MEM_BD, BE_W64, {.mem_bd = {addr_reg, 0}}};
              kgpc_backend_target()->emit(&em, BE_LOAD, BE_W64, &dst, &src,
                                          NULL);
              inst_list = em.list;
            }
          }
        ...
        /* When passing a ShortString argument to a formal AnsiString
         * (STRING_TYPE) parameter, convert via kgpc_shortstring_to_string.
         * Without this, the raw ShortString buffer address (length-byte + data)
         * is passed and the callee interprets the length byte as the first
         * character of an AnsiString. */
        int formal_wants_ansistring = 0;
        if (expected_type == STRING_TYPE)
          formal_wants_ansistring = 1;
        else if (expected_type == UNKNOWN_TYPE && formal_arg_decl != NULL &&
                 formal_arg_decl->type == TREE_VAR_DECL &&
                 formal_arg_decl->tree_data.var_decl_data.type == STRING_TYPE)
          formal_wants_ansistring = 1;
        /* For calls through procedural type variables (function pointers),
         * expected_type may remain UNKNOWN_TYPE because
         * codegen_param_expected_type returns UNKNOWN when the formal has no
         * shortstring marker.  Detect the AnsiString-family case from the
         * formal's type_id. */
        ...
        if (is_array_arg && !is_var_param && !is_array_param &&
           ...
           * so the spill is treated as a by-value string, not as a by-ref
           * address. */
          if (arg_infos != NULL)
            arg_infos[arg_num].is_pointer_like = 0;
        }

        /* ARCHITECTURAL FIX: Spill address to stack to prevent clobbering by
         * nested calls */
        StackNode_t *arg_spill = add_l_t("arg_eval");
        if (arg_spill != NULL && arg_infos != NULL) {
          {
        ...
        if (!codegen_expr_is_addressable(arg_expr)) {
          ...
          StackNode_t *temp_slot =
              codegen_alloc_temp_bytes("dynarray_arg", descriptor_size);
          if (temp_slot == NULL) {
            codegen_report_error(ctx, "ERROR: Unable to allocate temporary "
                                      "storage for dynamic array argument.");
            return inst_list;
          }
          ...
          if (addr_reg == NULL) {
            free_reg(get_reg_stack(), value_reg);
            codegen_report_error(ctx, "ERROR: Unable to allocate register for "
                                      "dynamic array argument address.");
            return inst_list;
          }
          ...
          inst_list = codegen_vect_reg(inst_list, 0);
          inst_list = codegen_call_with_shadow_space(
              inst_list, "kgpc_dynarray_assign_from_temp");
          free_arg_regs();
          free_reg(get_reg_stack(), value_reg);
        ...
        StackNode_t *temp_slot =
            codegen_alloc_temp_bytes("tconstexprint_arg", 10);
        Register_t *value_reg = NULL;
        Register_t *addr_reg = NULL;
        int use_signed_assign =
            expr_is_signed_kgpctype(arg_expr) &&
            !(arg_expr->type == EXPR_INUM && arg_expr->expr_data.i_num >= 0);
        const char *assign_target = use_signed_assign
        ...
        if (arg_spill != NULL && arg_infos != NULL) {
          ...
          arg_infos[arg_num].is_pointer_like = 1;
                 ...
                 call_expr->expr_data.function_call_data
                         .builtin_call_lowering == BUILTIN_CALL_ASSIGNED) {
        ...
        }
        {
          ...
          BeOperand src = {OPK_MEM_BD, BE_W64, {.mem_bd = {src_reg, 0}}};
          kgpc_backend_target()->emit(&em, BE_LOAD, BE_W64, &dst, &src, NULL);
          inst_list = em.list;
        }
        ...
        if (arg_spill != NULL && arg_infos != NULL) {
          {
            ...
            BeOperand dst = {OPK_MEM_FRAME, BE_W64,
                             {.mem_frame = {BE_BASE_FP, -(long long)(arg_spill->offset)}}};
            BeOperand a = {OPK_PHYS, BE_W64, {.phys = src_reg->bit_64}};
            kgpc_backend_target()->emit(&em, BE_STORE, BE_W64, &dst, &a, NULL);
            inst_list = em.list;
          }
          free_reg(get_reg_stack(), src_reg);
        ...
        if (temp_slot == NULL) {
          codegen_report_error(ctx, "ERROR: Failed to allocate temporary "
                                    "storage for method-pointer argument.");
          return inst_list;
        }
        ...
        if (arg_spill != NULL && arg_infos != NULL) {
          {
            ...
            BeOperand dst = {OPK_MEM_FRAME, BE_W64,
                             {.mem_frame = {BE_BASE_FP, -(long long)(arg_spill->offset)}}};
            BeOperand a = {OPK_PHYS, BE_W64, {.phys = result_reg->bit_64}};
            kgpc_backend_target()->emit(&em, BE_STORE, BE_W64, &dst, &a, NULL);
            inst_list = em.list;
          }
        ...
        if (codegen_get_record_size(ctx, arg_expr, &record_size) != 0 ||
            record_size < 0) {
          codegen_report_error(
              ctx, "ERROR: Unable to determine record size for argument.");
          return inst_list;
        }
        if (record_size == 0)
          record_size = 1;
        ...
        if (record_size > INT_MAX) {
          codegen_report_error(
              ctx, "ERROR: Record argument size exceeds supported limits.");
          return inst_list;
        }
        ...
        if (record_size == 16 && arg_expr->type == EXPR_VAR_ID &&
            codegen_expr_is_addressable(arg_expr) && ctx != NULL &&
            ctx->symtab != NULL) {
          ListNode_t *all_idents =
              FindAllIdents(ctx->symtab, arg_expr->expr_data.id);
          for (ListNode_t *id_node = all_idents; id_node != NULL;
               id_node = id_node->next) {
            HashNode_t *cand = (HashNode_t *)id_node->cur;
            if (cand == NULL)
              continue;
            struct RecordType *cand_rec =
                codegen_get_record_type_from_node(cand);
        ...
        if (is_iface_guid_arg) {
          ...
          if (src_reg == NULL) {
            codegen_report_error(ctx, "ERROR: Failed to allocate register for "
                                      "interface GUID argument.");
            return inst_list;
          }
          char guid_buf[512];
          ...
          if (arg_expr->type == EXPR_FUNCTION_CALL &&
            ...
            codegen_report_error(
                ctx, "ERROR: Unsupported record argument expression.");
            return inst_list;
          } else {
            Register_t *value_reg = NULL;
            inst_list =
        ...
        if (result_reg == NULL) {
          codegen_report_error(ctx, "ERROR: Unable to allocate register for "
                                    "record argument pointer.");
          return inst_list;
        }

        /* For external C functions (cdecl), small structs (<=8 bytes) are
         * passed by VALUE, but Pascal passes them by reference (pointer). We
         * automatically dereference the pointer here so Pascal code doesn't
         * need to change.
         *
         ...
         *
         * CRITICAL FIX: Check the KgpcType's procedure definition for
         * cname_flag instead of checking if the procedure name contains "cdecl"
         * or "external". The procedure name is just "inet_ntoa", not
         * "inet_ntoa_cdecl_external".
         */
        if (is_external_c_function && record_size <= 8) {
           ...
           * Integrated: same-register load through the vtable; the base is a
           * tracked vreg USE and the destination a tracked DEF (were baked
           * names, invisible to liveness).  Narrow widths go through emit_ext
           * as zero-extending loads (movz mnemonics unchanged). */
          {
            BeEmitter em = codegen_beemitter(inst_list, ctx);
            if (record_size == 1) {
              BeOperand dst = {OPK_VREG, BE_W32, {.vreg = result_reg}};
              BeOperand src = {OPK_MEM_BD, BE_W8, {.mem_bd = {result_reg, 0}}};
              kgpc_backend_target()->emit_ext(&em, &dst, &src, BE_W8, BE_W32,
                                              0);
            } else if (record_size == 2) {
              BeOperand dst = {OPK_VREG, BE_W32, {.vreg = result_reg}};
              BeOperand src = {OPK_MEM_BD, BE_W16, {.mem_bd = {result_reg, 0}}};
              kgpc_backend_target()->emit_ext(&em, &dst, &src, BE_W16, BE_W32,
                                              0);
            } else if (record_size <= 4) {
              BeOperand dst = {OPK_VREG, BE_W32, {.vreg = result_reg}};
              BeOperand src = {OPK_MEM_BD, BE_W32, {.mem_bd = {result_reg, 0}}};
              kgpc_backend_target()->emit(&em, BE_LOAD, BE_W32, &dst, &src,
                                          NULL);
            } else /* record_size <= 8 */
            {
              BeOperand dst = {OPK_VREG, BE_W64, {.vreg = result_reg}};
              BeOperand src = {OPK_MEM_BD, BE_W64, {.mem_bd = {result_reg, 0}}};
              kgpc_backend_target()->emit(&em, BE_LOAD, BE_W64, &dst, &src,
                                          NULL);
            }
            inst_list = em.list;
        ...
        if (formal_arg_decl != NULL && ctx != NULL && ctx->symtab != NULL)
          formal_kgpc = resolve_type_from_vardecl(formal_arg_decl, ctx->symtab,
                                                  &owns_formal_kgpc);
      ...
      pass_value_arg:
        ...
        if (expected_type == REAL_TYPE && arg_infos != NULL &&
          ...
          if (expected_type == REAL_TYPE && expr_returns_sret(arg_expr) &&
            ...
            free_arg_regs();
            {
               ...
               * (byte-identical). */
              BeEmitter em = codegen_beemitter(inst_list, ctx);
              BeOperand dst = {OPK_VREG, BE_W64, {.vreg = top_reg}};
              BeOperand src = {OPK_PHYS, BE_W64, {.phys = "%rax"}};
              kgpc_backend_target()->emit(&em, BE_MOV, BE_W64, &dst, &src, NULL);
              inst_list = em.list;
            }
          ...
          /* Promote char arguments to strings when the formal parameter expects
           * string, unless the semantic checker already rewrote the call to a
           * runtime overload that accepts a char natively (e.g.
           * kgpc_string_pos_ca). Also handle explicit char-to-string typecasts
           * like AnsiString(c) where build_expr_tree strips the typecast,
           * leaving a raw char value. */
          ...
          if (!arg_is_char_value && arg_expr != NULL &&
              ...
              expr_has_type_tag(arg_expr->expr_data.typecast_data.expr,
                                CHAR_TYPE)) {
            arg_is_char_value = 1;
          ...
          if (((formal_expects_string && !formal_expects_wide_string) &&
               arg_is_char_value &&
          ...
          if (formal_expects_wide_string && arg_is_char_value &&
          ...
          if (formal_expects_string && !formal_expects_wide_string &&
              ...
              !codegen_current_param_is_ansistring(arg_expr, ctx)) {
            inst_list =
                codegen_promote_shortstring_reg(inst_list, ctx, top_reg);
          }

          if (formal_expects_string && !formal_expects_wide_string &&
              arg_expr != NULL && codegen_expr_is_wide_string_value(arg_expr)) {
            const char *arg_reg64 =
                codegen_target_is_windows() ? "%rcx" : "%rdi";
            {
              /* Integrated: emit through the target-neutral backend vtable (byte-identical). */
          ...
          if (arg_num == 0 &&
            ...
            BeEmitter em = codegen_beemitter(inst_list, ctx);
            BeOperand dst = {OPK_VREG, BE_W64, {.vreg = top_reg}};
            BeOperand src = {OPK_MEM_BD, BE_W64, {.mem_bd = {top_reg, 0}}};
            kgpc_backend_target()->emit(&em, BE_LOAD, BE_W64, &dst, &src, NULL);
            inst_list = em.list;
          }
          ...
          if (arg_spill != NULL && arg_infos != NULL) {
            ...
            if (is_real_arg && is_xmm) {
            ...
            } else {
              {
                ...
                BeOperand dst = {OPK_MEM_FRAME, BE_W64,
                                 {.mem_frame = {BE_BASE_FP, -(long long)(arg_spill->offset)}}};
                BeOperand a = {OPK_PHYS, BE_W64, {.phys = top_reg->bit_64}};
                kgpc_backend_target()->emit(&em, BE_STORE, BE_W64, &dst, &a, NULL);
                inst_list = em.list;
              }
    ...
    if (formal_args != NULL && !(skip_formal_for_self && arg_num == 0)) {
      ...
      /* CRITICAL VALIDATION: After advancing formal_args, check if the new node
       * is valid. On some platforms, corrupted list nodes may have garbage in
       * their 'next' pointer. We validate the next node before the next
       * iteration to prevent segfaults. */
      if (formal_args != NULL && formal_args->type != LIST_TREE &&
          formal_args->type != LIST_UNSPECIFIED) {
        const char *proc_name = procedure_name ? procedure_name : "(unknown)";
        codegen_report_error(
            ctx,
            "FATAL: Internal compiler error - corrupted formal_args->next "
            "(type=%d) at argument %d for procedure %s. "
            "This indicates the formal arguments list is not properly "
            "NULL-terminated or contains corrupted nodes.",
            formal_args->type, arg_num, proc_name);
        if (arg_infos != NULL)
          free(arg_infos);
        return inst_list;
  ...
  if (arg_infos != NULL) {
    for (int i = 0; i < arg_num; ++i) {
      int actual_type = (arg_infos[i].expr != NULL)
                            ? expr_get_type_tag(arg_infos[i].expr)
                            : UNKNOWN_TYPE;
      int actual_is_real =
  ...
  if (stack_slot_count > 0 || codegen_target_is_windows()) {
     ...
     * Stack arguments start immediately after the Windows shadow space (if
     * any), i.e. at offset 32(%rsp) for the 5th argument. */
    int padding =
        codegen_expr_align_to(stack_bytes, REQUIRED_OFFSET) - stack_bytes;
  ...
  for (int i = arg_num - 1; i >= 0; --i) {
    int expected_type =
        (arg_infos != NULL) ? arg_infos[i].expected_type : UNKNOWN_TYPE;
    int expected_real_size =
        (arg_infos != NULL) ? arg_infos[i].expected_real_size : 0;
    ...
    int actual_is_s32 =
        (actual_type == INT_TYPE || actual_type == LONGINT_TYPE);
    int expected_is_wider_int =
        (expected_type == LONGINT_TYPE || expected_type == INT64_TYPE ||
         expected_type == QWORD_TYPE);
    int needs_int_to_long =
        (actual_is_s32 && expected_is_wider_int && !is_ptr_like);
    ...
    struct Expression *source_expr =
        arg_infos != NULL ? arg_infos[i].expr : NULL;
    if (stored_reg != NULL) {
      ...
      const char *proc_name = procedure_name ? procedure_name : "(unknown)";
      fprintf(stderr,
              "ERROR: Missing evaluated value for argument %d in call to %s "
              "(%s).\n",
              i, proc_name, describe_expression_kind(source_expr));
      exit(1);
    }
  }
  ...
  if (g_current_codegen_abi == KGPC_TARGET_ABI_WINDOWS && is_varargs_function &&
      arg_infos != NULL) {
    for (int i = 0; i < arg_num; ++i) {
      if (arg_infos[i].assigned_class == ARG_CLASS_SSE &&
          !arg_infos[i].pass_via_stack && arg_infos[i].assigned_index >= 0) {
