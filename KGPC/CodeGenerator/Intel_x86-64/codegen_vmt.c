...
static int codegen_method_uses_sret(CodeGenContext *ctx, SymTab_t *symtab,
  ...
  if (method_sym == NULL && fallback_owner != NULL) {
    snprintf(lookup_name, sizeof(lookup_name), "%s__%s", fallback_owner,
             method_name);
    FindSymbol(&method_sym, symtab, lookup_name);
  }
  ...
  if (kgpc_type_is_record(ret_type)) {
    ...
    if (ret_rec != NULL &&
        codegen_sizeof_type_reference(ctx, RECORD_TYPE, NULL, ret_rec,
                                      &ret_size) == 0 &&
        ret_size > 8)
      return 1;
  }
...
static int codegen_emit_interface_guid(CodeGenContext *ctx,
                                       const char *iface_name, uint32_t d1,
  ...
  snprintf(guid_dedup_buf, sizeof(guid_dedup_buf), "__kgpc_guid_%s",
           iface_name);
...
static int codegen_class_overrides_inherited_interface_method(
  ...
  if (symtab == NULL || record == NULL || parent_record == NULL ||
      iface_name == NULL)
    return 0;
  ...
  for (ListNode_t *method_cur = iface_record->method_templates;
       method_cur != NULL; method_cur = method_cur->next) {
...
static void codegen_emit_class_vmt(CodeGenContext *ctx, SymTab_t *symtab,
   ...
   *   tinterfaceentry: IIDRef(^pguid,8) VTable(8)
   * IOffset(8)|IOffsetAsCodePtr(8)+IIDStrRef(8)+IType(4)+pad(4) For each
   * interface, emit standalone GUID constant + pointer indirection:
  ...
  const char **effective_iface_names =
      (const char **)record_info->interface_names;
  ...
  const struct RecordType *parent_record =
      codegen_record_parent(record_info, symtab);
  for (const struct RecordType *iface_parent = parent_record;
    ...
    if (effective_iface_append_failed)
      break;
    if (iface_parent->num_interfaces <= 0 ||
        iface_parent->interface_names == NULL)
      continue;
    for (int iidx = 0; iidx < iface_parent->num_interfaces; iidx++) {
      ...
      if (iface_name == NULL ||
        ...
        continue;
      if (!codegen_class_overrides_inherited_interface_method(
              symtab, record_info, class_label, parent_record, iface_name))
        continue;
      ...
      if (!free_effective_iface_names) {
        ...
        if (effective_iface_count > 0) {
          copied =
              (const char **)malloc(sizeof(char *) * effective_iface_count);
          if (copied == NULL) {
      ...
      const char **grown =
          (const char **)realloc((void *)effective_iface_names,
  ...
  if (effective_iface_count > 0) {
    ...
    for (int iidx = 0; iidx < effective_iface_count; iidx++) {
      ...
      codegen_emit_interface_guid(ctx, iface_name, d1, d2, d3, d4, NULL,
                                  emitted_classes);
    }
    ...
    if (record_info->parent_class_name != NULL) {
      ...
      if (FindSymbol(&parent_cls_node, symtab,
        ...
        if (parent_cls_rec == NULL && parent_cls_node->type != NULL &&
          ...
          parent_cls_rec =
              parent_cls_node->type->info.points_to->info.record_info;
      }
    ...
    for (int iidx = 0; iidx < effective_iface_count; iidx++) {
      ...
      fprintf(ctx->output_file,
              "\t# Entry for %s (%d bytes = tinterfaceentry)\n", iface_name,
              KGPC_INTF_ENTRY_SIZE);
      /* KGPC_INTF_ENTRY_IIDREF_OFFSET +0: IIDRef (^pguid) — pointer to the
       * pguid indirection cell */
      fprintf(ctx->output_file, "\t.quad\t__kgpc_guidref_%s\n", iface_name);
      /* KGPC_INTF_ENTRY_VTABLE_OFFSET +8: VTable — pointer to interface vtable
       * for this class */
      fprintf(ctx->output_file, "\t.quad\t%s_INTF_%s_VTABLE\n", class_label,
              iface_name);
      /* KGPC_INTF_ENTRY_IOFFSET_OFFSET +16: IOffset (sizeuint) — byte offset
       * from object start to interface slot */
      fprintf(ctx->output_file, "\t.quad\t%lld\n", ioffset);
      /* KGPC_INTF_ENTRY_IIDSTRREF_OFFSET +24: IIDStrRef (^pshortstring) — NULL
       * for now */
      fprintf(ctx->output_file, "\t.quad\t0\n");
      /* KGPC_INTF_ENTRY_ITYPE_OFFSET +32: IType (tinterfaceentrytype enum, 4
       * bytes) = etStandard = 0 */
    ...
    for (int iidx = 0; iidx < effective_iface_count; iidx++) {
      ...
      if (iface_name == NULL)
        continue;
      long long ioffset_for_this_iface =
          base_instance_size + vtbl_iface_slot_idx * 8;
      vtbl_iface_slot_idx++;
      ...
      while (vtbl_iface_method != NULL) {
        struct MethodTemplate *vtbl_imethod =
            (struct MethodTemplate *)vtbl_iface_method->cur;
        if (vtbl_imethod != NULL && vtbl_imethod->name != NULL) {
          ...
          if (vtbl_resolved_id != NULL) {
            ...
            /* VMT_VINSTANCESIZE2_OFFSET (slot 1): -vInstanceSize; sum with slot
             * 0 == 0 iff VMT */
            ...
            fprintf(ctx->output_file,
                    "\t.quad\t__kgpc_abstract_method_error\t# %s (not "
                    "implemented)\n",
                    vtbl_imethod->name);
          }
        }
   ...
   * e.g. math.pp declares "EInvalidArgument = class(ematherror)" with
   * lowercase, but EMathError's VMT is emitted with its declaration-case label
   * "EMathError"). */
  ...
  if (parent_vmt_label != NULL) {
    ...
    if (FindSymbol(&parent_node, symtab, parent_vmt_label) != 0 &&
      ...
      if (parent_rec != NULL && parent_rec->is_class &&
          parent_rec->type_id != NULL)
  ...
  char name_label[256];
  snprintf(name_label, sizeof(name_label), "__kgpc_typeinfo_name_%s",
           class_label);
   ...
   * when the class has interfaces.  For classes without interfaces, compute
   * now. */
  ...
  if (record_info->method_templates != NULL && record_info->methods != NULL &&
    ...
    int *method_claimed = (method_count > 0)
                              ? (int *)calloc((size_t)method_count, sizeof(int))
                              : NULL;
    ...
    for (ListNode_t *tmpl_node = record_info->method_templates;
      ...
      if (tmpl_proc_type != NULL &&
          tmpl_proc_type->kind == TYPE_KIND_PROCEDURE) {
        template_param_types = semcheck_param_types_from_params(
            tmpl_proc_type->info.proc_info.params, tmpl->is_static ? 0 : 1,
      ...
      for (ListNode_t *m = matches; m != NULL; m = m->next) {
        ...
        if (template_param_types != NULL) {
          ...
          struct TypeRef **cand_types = semcheck_param_types_from_params(
              cand->type->info.proc_info.params, tmpl->is_static ? 0 : 1,
              &cand_count);
          if (type_ref_array_equal_ci(template_param_types,
                                      template_param_types_count, cand_types,
                                      cand_count))
            sig_match = 1;
      ...
      if (resolved_id == NULL)
        resolved_id = unclaimed_strong_id;
      if (resolved_id == NULL)
        resolved_id = any_strong_id;
      if (resolved_id == NULL)
        resolved_id = unclaimed_any_id;
      if (resolved_id == NULL)
        resolved_id = any_any_id;

      KGPC_COMPILER_HARD_ASSERT(
          resolved_id != NULL || matching_defined_candidate_count == 0,
          "generic VMT specialization '%s.%s' had a same-signature "
          "implementation but no resolved id",
      ...
      if (claimed_resolved_count >= claimed_resolved_capacity) {
        int new_cap = (claimed_resolved_capacity == 0)
                          ? 4
                          : (claimed_resolved_capacity * 2);
      ...
      for (ListNode_t *method_node = record_info->methods; method_node != NULL;
           method_node = method_node->next, ++method_idx) {
        ...
        if (template_param_types != NULL && method->param_types != NULL &&
            ...
            type_ref_array_equal_ci(
                template_param_types, template_param_types_count,
  ...
  for (struct RecordType *cur_record = record_info; cur_record != NULL;) {
    for (ListNode_t *method_node = cur_record->methods; method_node != NULL;
  ...
  for (int slot = VMT_FIRST_VMETHOD_SLOT; slot <= max_vmt_index; slot++) {
    ...
    struct MethodInfo *resolved_method =
        NULL; /* most-specific with resolved in available set */
    struct MethodInfo *any_resolved =
        NULL; /* most-specific with any non-NULL resolved */
    for (struct RecordType *cur_record = record_info; cur_record != NULL;) {
      for (ListNode_t *method_node = cur_record->methods; method_node != NULL;
           method_node = method_node->next) {
        ...
        if (resolved_method == NULL && candidate->resolved_mangled_id != NULL &&
    ...
    if (slot_label != NULL) {
      ...
      if (FindSymbol(&sym, symtab, full_mangled) != 0 && sym != NULL &&
          ...
          sym->type->info.proc_info.definition->tree_data.subprogram_data
                  .statement_list != NULL)
        has_impl = 1;
      if (has_impl)
        fprintf(ctx->output_file, "\t.quad\t%s\n", full_mangled);
   ...
   * for the interface method names that forward to the implementing class
   * methods. This enables interface method calls (e.g., FStream.Read(...)) to
   * link when emitted as direct calls to the interface method mangled name.
   *
    ...
    for (int iidx = 0; iidx < record_info->num_interfaces; iidx++) {
      ...
      while (iface_method != NULL) {
        ...
        if (imethod != NULL && imethod->name != NULL) {
           ...
           */
          char iface_base[512];
          snprintf(iface_base, sizeof(iface_base), "%s__%s", iface_name,
                   imethod->name);
          ...
          if (iface_func != NULL && iface_func->mangled_id != NULL) {
            ...
            char stub_dedup[640];
            snprintf(stub_dedup, sizeof(stub_dedup), "__kgpc_abstub_%s",
            ...
            if (emitted_class_set_contains(emitted_classes, stub_dedup)) {
              ...
              if (final_target != NULL) {
                fprintf(ctx->output_file,
                        ...
                        imethod->name, final_target);
                codegen_emit_global_jump_stub(ctx, iface_func->mangled_id,
                                              final_target);
              }
            }
          }
  ...
  if (record_type_is_class(record_info) || record_has_class_vars(record_info) ||
      record_has_class_method_templates(record_info) ||
      record_has_method_decls(record_info)) {
    int include_all_fields = (!record_has_class_vars(record_info) &&
                              (record_has_class_method_templates(record_info) ||
                               record_has_method_decls(record_info)));
    long long class_var_size = codegen_class_var_storage_size(
        symtab, record_info, include_all_fields ? 1 : 0);
    if (class_var_size <= 0)
      class_var_size = 8;
    ...
    if (record_info->is_interface &&
        codegen_resolve_record_guid(symtab, record_info, &iface_d1, &iface_d2,
                                    &iface_d3, iface_d4)) {
      fprintf(ctx->output_file, "\t.long\t0x%08lX\n", (unsigned long)iface_d1);
      fprintf(ctx->output_file, "\t.short\t0x%04X\n", (unsigned int)iface_d2);
      fprintf(ctx->output_file, "\t.short\t0x%04X\n", (unsigned int)iface_d3);
      fprintf(ctx->output_file,
              "\t.byte\t0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X, "
              "0x%02X, 0x%02X\n",
              iface_d4[0], iface_d4[1], iface_d4[2], iface_d4[3], iface_d4[4],
              iface_d4[5], iface_d4[6], iface_d4[7]);
    } else {
...
static void codegen_emit_record_classvar_storage(
    CodeGenContext *ctx, SymTab_t *symtab, struct RecordType *record_info,
    const char *class_label, EmittedClassSet *emitted_classes) {
  if (record_info == NULL || record_type_is_class(record_info) ||
      class_label == NULL)
    return;
  ...
  long long class_var_size = codegen_class_var_storage_size(
      symtab, record_info, has_class_vars ? 0 : 1);
  if (class_var_size <= 0)
    class_var_size = 8;
...
static int
codegen_should_emit_plain_record_typeinfo(const struct RecordType *record_info,
...
static void codegen_emit_plain_record_typeinfo(
    CodeGenContext *ctx, const struct RecordType *record_info,
  ...
  if (best_node->source_unit_index > 0) {
    ...
    if (unit_name != NULL) {
      size_t qualified_len =
          strlen(unit_name) + 1 + strlen(record->parent_class_name) + 1;
      char *qualified_id = (char *)malloc(qualified_len);
      if (qualified_id != NULL) {
        snprintf(qualified_id, qualified_len, "%s.%s", unit_name,
        ...
        if (FindSymbol(&qualified, symtab, qualified_id) != 0 &&
          ...
          if (qualified_record == NULL && qualified->type != NULL &&
            ...
            qualified_record =
                qualified->type->info.points_to->info.record_info;
          free(qualified_id);
...
static const char *
codegen_find_interface_delegate_target_name(const struct RecordType *record,
                                            const char *iface_name,
                                            const char *method_name) {
  ...
  for (ListNode_t *cur = record->method_templates; cur != NULL;
    ...
    if (!tmpl->is_interface_delegation ||
        tmpl->delegated_interface_name == NULL || tmpl->name == NULL ||
...
const char *codegen_find_class_method_impl_id(SymTab_t *symtab,
  ...
  while (cur_record != NULL || cur_label != NULL) {
    ...
    char base_name[512];
    snprintf(base_name, sizeof(base_name), "%s__%s", owner_label,
             lookup_method_name);
...
static void
codegen_canonicalize_record_for_emission(SymTab_t *symtab,
                                         const char **class_label,
                                         struct RecordType **record_info) {
...
static void codegen_vmt_from_type_list(CodeGenContext *ctx, SymTab_t *symtab,
  ...
  while (cur != NULL) {
    ...
    if (type_tree != NULL && type_tree->type == TREE_TYPE_DECL) {
      ...
      codegen_canonicalize_record_for_emission(symtab, &class_label,
                                               &record_info);
      ...
      codegen_emit_class_vmt(ctx, symtab, record_info, class_label,
                             emitted_classes);
      codegen_emit_record_classvar_storage(ctx, symtab, record_info,
                                           class_label, emitted_classes);
      if (record_info != NULL && !record_type_is_class(record_info) &&
          class_label != NULL && class_label[0] != '\0') {
        codegen_emit_plain_record_typeinfo(ctx, record_info, class_label,
                                           emitted_classes);
      }
    }
    cur = cur->next;
...
static void
codegen_emit_vmts_from_hash_table(CodeGenContext *ctx, SymTab_t *symtab,
                                  HashTable_t *table,
                                  EmittedClassSet *emitted_classes) {
  if (table == NULL)
    return;
...
/* Helper: emit GUID data for all interfaces with GUIDs in a type declaration
 * list. Used to emit GUIDs from loaded units whose interfaces are not in the
 * local scope. */
static void
codegen_emit_guids_from_type_list(CodeGenContext *ctx, ListNode_t *type_decls,
                                  EmittedClassSet *emitted_classes) {
  ListNode_t *cur = type_decls;
  while (cur != NULL) {
    Tree_t *type_tree = (Tree_t *)cur->cur;
    if (type_tree != NULL && type_tree->type == TREE_TYPE_DECL) {
      ...
      if (record_info != NULL && record_info->is_interface) {
        ...
        uint8_t guid_d4[8] = {0};
        codegen_resolve_record_guid(ctx->symtab, record_info, &guid_d1,
                                    &guid_d2, &guid_d3, guid_d4);
        ...
        codegen_emit_interface_guid(ctx, iface_name, guid_d1, guid_d2, guid_d3,
                                    guid_d4, " (from unit)", emitted_classes);
      }
    }
    cur = cur->next;
  }
...
static void codegen_assert_interface_impl_resolved(const char *iface_name,
  ...
  fprintf(stderr,
          ...
          class_label != NULL ? class_label : "<class>", iface_symbol);
 ...
 */
static void
codegen_vmt_aliases_from_type_list(CodeGenContext *ctx, ListNode_t *type_decls,
                                   EmittedClassSet *emitted_classes) {
  ListNode_t *cur = type_decls;
  while (cur != NULL) {
    Tree_t *type_tree = (Tree_t *)cur->cur;
    if (type_tree != NULL && type_tree->type == TREE_TYPE_DECL &&
      ...
      const char *target_name =
          type_tree->tree_data.type_decl_data.info.alias.target_type_id;
      if (alias_name != NULL && target_name != NULL) {
        int target_emitted =
            emitted_class_set_contains(emitted_classes, target_name);
        int alias_already_owned =
            emitted_class_set_contains(emitted_classes, alias_name);
 ...
 * Iterates type declarations from loaded units (via comp_ctx) and the program
 * tree. Uses a single EmittedClassSet to avoid duplicate emissions. */
void codegen_vmt(CodeGenContext *ctx, SymTab_t *symtab, Tree_t *tree,
  ...
  if (comp_ctx != NULL) {
    for (int i = 0; i < comp_ctx->loaded_unit_count; ++i) {
      ...
      if (unit != NULL && unit->type == TREE_UNIT) {
        codegen_vmt_from_type_list(
            ctx, symtab, unit->tree_data.unit_data.interface_type_decls,
            &emitted_classes);
        codegen_vmt_from_type_list(
            ctx, symtab, unit->tree_data.unit_data.implementation_type_decls,
            &emitted_classes);
      }
    }
  }
  ...
  if (tree->type == TREE_PROGRAM_TYPE)
    ...
    codegen_vmt_from_type_list(ctx, symtab,
                               tree->tree_data.unit_data.interface_type_decls,
                               &emitted_classes);
    codegen_vmt_from_type_list(
        ctx, symtab, tree->tree_data.unit_data.implementation_type_decls,
        &emitted_classes);
  }
  ...
  if (ctx->is_whole_program) {
    for (ScopeNode *scope = symtab->current_scope; scope != NULL;
         scope = scope->parent) {
      codegen_emit_vmts_from_hash_table(ctx, symtab, scope->table,
                                        &emitted_classes);
    }
    ...
    for (int i = 0; i < SYMTAB_MAX_UNITS; i++) {
      ...
      if (unit_scope == NULL)
        continue;
      codegen_emit_vmts_from_hash_table(ctx, symtab, unit_scope->table,
                                        &emitted_classes);
    }
  } else {
   ...
   * implementing the interface.  This pass ensures all GUID symbols are
   * defined. */
  for (ScopeNode *scope = symtab->current_scope; scope != NULL;
       scope = scope->parent) {
      ...
      while (node != NULL) {
        ...
        if (hash_node != NULL && hash_node->hash_type == HASHTYPE_TYPE &&
          ...
          if (record_info != NULL && record_info->is_interface) {
            ...
            uint16_t guid_d2 = 0, guid_d3 = 0;
            uint8_t guid_d4[8] = {0};
            codegen_resolve_record_guid(symtab, record_info, &guid_d1, &guid_d2,
                                        &guid_d3, guid_d4);
            const char *iface_name = record_info->type_id;
            if (iface_name == NULL)
        ...
        if (hash_node != NULL && hash_node->hash_type == HASHTYPE_TYPE &&
          ...
          if (record_info != NULL && record_info->is_interface) {
            ...
            if (iface_name == NULL)
              iface_name = hash_node->id;
            if (iface_name != NULL)
              codegen_emit_interface_guid(
                  ctx, iface_name, guid_d1, guid_d2, guid_d3, guid_d4,
  ...
  if (comp_ctx != NULL) {
    for (int i = 0; i < comp_ctx->loaded_unit_count; ++i) {
      Tree_t *unit = comp_ctx->loaded_units[i].unit_tree;
      if (unit != NULL && unit->type == TREE_UNIT) {
        codegen_emit_guids_from_type_list(
            ctx, unit->tree_data.unit_data.interface_type_decls,
            &emitted_classes);
        codegen_emit_guids_from_type_list(
            ctx, unit->tree_data.unit_data.implementation_type_decls,
            &emitted_classes);
      }
    }
  }
  ...
  if (tree->type == TREE_PROGRAM_TYPE)
    codegen_emit_guids_from_type_list(
        ctx, tree->tree_data.program_data.type_declaration, &emitted_classes);
  ...
  if (comp_ctx != NULL) {
    for (int i = 0; i < comp_ctx->loaded_unit_count; ++i) {
      Tree_t *unit = comp_ctx->loaded_units[i].unit_tree;
      if (unit != NULL && unit->type == TREE_UNIT) {
        codegen_vmt_aliases_from_type_list(
            ctx, unit->tree_data.unit_data.interface_type_decls,
            &emitted_classes);
        codegen_vmt_aliases_from_type_list(
            ctx, unit->tree_data.unit_data.implementation_type_decls,
