 ...
 */
static inline struct RecordField *
codegen_lookup_field_in_members(ListNode_t *members, const char *field_name,
                                int skip_hidden) {
  if (field_name == NULL)
    return NULL;
  ...
  for (ListNode_t *cur = members; cur != NULL; cur = cur->next) {
    if (cur->type == LIST_RECORD_FIELD && cur->cur != NULL) {
      ...
      for (ListNode_t *branch_node = variant->branches; branch_node != NULL;
        ...
        struct RecordField *field = codegen_lookup_field_in_members(
            branch->members, field_name, skip_hidden);
        if (field != NULL)
          return field;
      }
    }
 ...
 */
static inline struct RecordField *
codegen_lookup_field_in_record(const struct RecordType *record,
                               const char *field_name, int skip_hidden) {
  if (record == NULL || field_name == NULL)
    return NULL;
  return codegen_lookup_field_in_members(record->fields, field_name,
                                         skip_hidden);
}
...
codegen_find_property_in_record(const struct RecordType *record,
