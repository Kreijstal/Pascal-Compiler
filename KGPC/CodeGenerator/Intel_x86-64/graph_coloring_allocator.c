...
InterferenceGraph_t *create_interference_graph(int num_regs) {
  InterferenceGraph_t *graph =
      (InterferenceGraph_t *)malloc(sizeof(InterferenceGraph_t));
  if (graph == NULL)
    return NULL;
...
ListNode_t *allocate_registers_graph_coloring(InterferenceGraph_t *graph) {
  ...
  /* Stack for simplification order (LIFO for coloring) - use ListBuilder for
   * O(1) append */
  ...
  /* Active set (nodes not yet processed) - use ListBuilder for O(1) initial
   * build */
  ...
  while (active != NULL) {
    ...
    if (low_degree != NULL) {
      ...
      if (spill_candidate != NULL) {
        ...
        list_builder_append(&spilled_builder, spill_candidate,
                            LIST_UNSPECIFIED);
 ...
 * build_interference_edges(). Identical to allocate_registers_graph_coloring()
 * except step 1 is skipped. */
ListNode_t *
...
void free_interference_graph(InterferenceGraph_t *graph) {
  ...
  /* NOTE: We do NOT free the LiveRange_t objects themselves because they are
   * owned by the register stack (reg_stack->active_live_ranges), not by the
   * interference graph. The graph only references them. The LiveRange_t objects
   * will be freed when the register stack is cleaned up.
   *
   * We only need to free the list structure and the neighbor lists within each
   * LiveRange.
   */
  ...
  while (cur != NULL) {
    ...
    if (lr != NULL && lr->neighbors != NULL) {
      DestroyList(lr->neighbors);
      lr->neighbors = NULL; /* Prevent double-free if same LiveRange is in
                               multiple graphs */
    }
    cur = cur->next;
  }
