...
static int codegen_proc_def_has_matching_impl(SymTab_t *symtab,
                                              const Tree_t *proc_def) {
...
const char *codegen_resolve_function_call_target(CodeGenContext *ctx,
  ...
  if ((call_target == NULL || call_target[0] == '\0' ||
       call_target_needs_resolution) &&
      ctx->symtab != NULL && expr->expr_data.function_call_data.id != NULL &&
    ...
    size_t prefix_len = (last_sep != NULL)
                            ? (size_t)(last_sep - stale_target + 1)
                            : strlen(stale_target);
    ListNode_t *candidates =
        FindAllIdents(ctx->symtab, expr->expr_data.function_call_data.id);
    for (ListNode_t *node = candidates; node != NULL; node = node->next) {
      HashNode_t *cand = (HashNode_t *)node->cur;
  ...
  if ((call_target == NULL || call_target[0] == '\0') && ctx->symtab != NULL &&
      expr->expr_data.function_call_data.id != NULL) {
    HashNode_t *sym = NULL;
    if (FindSymbol(&sym, ctx->symtab, expr->expr_data.function_call_data.id) !=
            0 &&
        sym != NULL) {
  ...
  if ((call_target == NULL || call_target[0] == '\0') &&
    ...
    if (!is_external && expr->expr_data.function_call_data.id != NULL) {
      char *computed_mangled =
          MangleFunctionName(expr->expr_data.function_call_data.id,
                             expr->expr_data.function_call_data.call_kgpc_type
                                 ->info.proc_info.params,
                             ctx->symtab);
...
int codegen_resolve_virtual_vmt_index(CodeGenContext *ctx,
  ...
  KGPC_COMPILER_HARD_ASSERT(ctx != NULL && ctx->symtab != NULL,
                            "virtual VMT index resolution requires a codegen "
                            "context and symbol table");
  KGPC_COMPILER_HARD_ASSERT(
      owner_class_name != NULL && owner_class_name[0] != '\0',
  ...
  KGPC_COMPILER_HARD_ASSERT(strstr(method_name, "__") == NULL,
                            "virtual VMT index resolution received mangled "
                            "method name '%s'; pass cached_method_name instead",
                            method_name);

  struct RecordType *record =
      semcheck_lookup_record_type(ctx->symtab, owner_class_name);
  KGPC_COMPILER_HARD_ASSERT(
      record != NULL,
  ...
  if (call_type != NULL && call_type->kind == TYPE_KIND_PROCEDURE) {
    ...
    call_param_types = semcheck_param_types_from_params(
        params, skip_self, &call_param_types_count);
  }
...
KgpcType *codegen_resolve_function_call_type(CodeGenContext *ctx,
  ...
  resolved_target =
      codegen_resolve_function_call_target(ctx, expr, &owned_resolved_target);
  if (ctx != NULL && ctx->symtab != NULL && resolved_target != NULL &&
      (expr->expr_data.function_call_data.mangled_id == NULL ||
       strcmp(resolved_target, expr->expr_data.function_call_data.mangled_id) !=
           0) &&
...
int codegen_tag_from_kgpc(const KgpcType *type) {
  ...
  if (kgpc_type_is_array(type) && type->type_alias != NULL &&
...
void codegen_collect_callable_export_names(ListNode_t *sub_list) {
  while (sub_list != NULL) {
    ...
    if (export_name != NULL &&
        !codegen_set_contains_ci(&g_codegen_callable_exports, export_name))
