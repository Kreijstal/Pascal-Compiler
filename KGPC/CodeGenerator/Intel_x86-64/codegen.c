...
int evaluate_real_const_expr(SymTab_t *symtab, struct Expression *expr,
                             double *out_value);
int expression_contains_real_literal_impl(SymTab_t *symtab,
...
static void
codegen_register_record_field_enum_literals(SymTab_t *symtab,
                                            struct RecordType *record);
static void codegen_register_type_enum_literals(ListNode_t *type_decls,
                                                SymTab_t *symtab);
 ...
 * labels. Declared non-static so codegen_symbol_resolution.c can access them
 * via extern. */
...
static char *codegen_make_program_var_label(CodeGenContext *ctx,
                                            const char *name);
static int codegen_typed_const_name_collides_ctx(CodeGenContext *ctx,
                                                 const char *bare_id);
static void codegen_emit_bss_or_comm(FILE *out, const char *sym,
                                     const char *label, int size, int alignment,
                                     int defined_in_unit);
...
int codegen_parse_guid_literal(const char *guid, uint32_t *d1, uint16_t *d2,
  ...
  unsigned int td4[8];
  int matched = sscanf(p, "%8x-%4x-%4x-%2x%2x-%2x%2x%2x%2x%2x%2x", &td1, &td2,
                       &td3, &td4[0], &td4[1], &td4[2], &td4[3], &td4[4],
                       &td4[5], &td4[6], &td4[7]);
  if (matched != 11)
    return 0;
...
int codegen_template_matches_methodinfo(const struct MethodTemplate *tmpl,
  ...
  if (tmpl == NULL || method == NULL || tmpl->name == NULL ||
      method->name == NULL)
    return 0;
...
struct RecordType *codegen_lookup_record_type_by_name(SymTab_t *symtab,
  ...
  if (record != NULL) {
    ...
    uint16_t guid_d3 = 0;
    uint8_t guid_d4[8] = {0};
    if (codegen_resolve_record_guid(symtab, record, &guid_d1, &guid_d2,
                                    &guid_d3, guid_d4))
      return record;
  }
  ...
  for (int i = 1; i <= n_units && i < SYMTAB_MAX_UNITS; ++i) {
    ...
    if (node != NULL && node->hash_type == HASHTYPE_TYPE) {
      KGPC_COMPILER_HARD_ASSERT(
          0, "suffix-only record type lookup for '%s' reached codegen",
          type_name);
    }
  }
...
int emitted_class_set_add(EmittedClassSet *set, const char *label) {
  ...
  if (set->count == set->capacity) {
    ...
    const char **new_labels = (const char **)realloc(
        set->labels, sizeof(const char *) * (size_t)new_capacity);
    if (new_labels == NULL)
      return 1;
...
int codegen_type_decl_suppressed(const Tree_t *decl) {
  return (decl != NULL && decl->type == TREE_TYPE_DECL &&
...
struct RecordType *codegen_record_from_type_decl_ex(Tree_t *decl,
  ...
  if (kgpc != NULL) {
    ...
    if (kgpc->kind == TYPE_KIND_POINTER && kgpc->info.points_to != NULL &&
  ...
  if (decl->tree_data.type_decl_data.kind == TYPE_DECL_RECORD) {
    ...
    if (fallback != NULL && fallback->is_class && fallback->fields == NULL &&
        fallback->parent_fields_merged && symtab != NULL &&
      ...
      if (FindSymbol(&canon_node, symtab, decl->tree_data.type_decl_data.id) &&
        ...
        else if (ct->kind == TYPE_KIND_POINTER && ct->info.points_to != NULL &&
...
static int record_type_get_field_offset(SymTab_t *symtab,
                                        struct RecordType *record,
  ...
  long long offset = 0;
  if (resolve_record_field(symtab, record, field_name, &field_desc, &offset, 0,
                           1) != 0 ||
      field_desc == NULL)
    return -1;
...
static const char *
codegen_class_constructor_target(SymTab_t *symtab, const char *owner,
                                 const struct MethodTemplate *tmpl,
                                 Tree_t **definition_out) {
  assert(symtab != NULL);
  assert(owner != NULL);
  assert(tmpl != NULL);
...
static void codegen_mark_class_constructors_used(ListNode_t *type_decls,
  ...
  for (ListNode_t *node = type_decls; node != NULL; node = node->next) {
    ...
    if (record == NULL || owner == NULL)
      continue;
    for (ListNode_t *mnode = record->method_templates; mnode != NULL;
         mnode = mnode->next) {
      ...
      const char *target =
          codegen_class_constructor_target(symtab, owner, tmpl, &definition);
...
int codegen_record_field_alignment(const struct RecordField *field,
  ...
  if (field != NULL) {
    if (field->type == EXTENDED_TYPE)
      return 16;
    if (field->type_id != NULL &&
        pascal_identifier_equals(field->type_id, "Extended"))
      return 16;
  }
  return (field_size > 8)
             ? 16
             : ((field_size >= 8) ? 8 : ((field_size >= 4) ? 4 : 1));
}
 ...
 *
 * This function extracts the class name from the mangled method name
 * (ClassName__MethodName), looks up the class type in the symbol table, and
 * registers each class var field with the stack manager using add_static_var.
 */
void codegen_add_class_vars_for_method(const char *owner_class,
  ...
  if (record_info == NULL) {
     ...
     * records) */
    if (class_node->type != NULL &&
        class_node->type->kind == TYPE_KIND_POINTER) {
  ...
  int is_static_check =
      from_cparser_is_method_static(class_name, method_name_arg);
  int is_nonstatic_class_method = from_cparser_is_method_nonstatic_class_method(
      class_name, method_name_arg);
  if (!is_static_check && !is_nonstatic_class_method && !has_class_vars) {
    free(class_name);
    return;
  }

  /* Use the original class name from the type definition to match the CLASSVAR
   * label. The mangled name may have different casing, but the CLASSVAR label
   * uses the original type_id. */
  ...
  while (field_node != NULL) {
    ...
    if (field != NULL && field->name != NULL && field->name[0] != '\0') {
      ...
      if (field_static_label != NULL) {
        int written;
        if (current_offset == 0)
          written = snprintf(field_static_label, field_label_len, "%s",
                             classvar_label);
        else
          written = snprintf(field_static_label, field_label_len, "%s+%lld",
                             classvar_label, current_offset);

        /* Verify buffer was large enough */
        assert(written >= 0 && (size_t)written < field_label_len);

        /* Register class vars under a class-qualified key to avoid cross-class
         * collisions. */
...
int codegen_real_param_storage_size(Tree_t *arg_decl,
  ...
  if (arg_decl != NULL && arg_decl->type == TREE_VAR_DECL) {
    ...
    if (arg_decl->tree_data.var_decl_data.type_id != NULL &&
        pascal_identifier_equals(arg_decl->tree_data.var_decl_data.type_id,
                                 "Extended"))
      return 16;
    struct TypeAlias *alias =
        arg_decl->tree_data.var_decl_data.inline_type_alias;
  ...
  if (resolved_type_node != NULL && resolved_type_node->type != NULL) {
    ...
    if (kgpc_type_is_real(resolved_type_node->type)) {
      long long real_size =
          kgpc_type_real_storage_size(resolved_type_node->type);
      if (real_size > 8)
        return 16;
      if (real_size > 0)
...
static inline int get_var_storage_size(HashNode_t *node) {
  ...
  if (node_alias != NULL) {
    ...
    if (node_alias->is_shortstring) {
      ...
      if (node_alias->array_end >= node_alias->array_start &&
          node_alias->array_end >= 0)
  ...
  if (node->type != NULL) {
    ...
    } else if (node->type->kind == TYPE_KIND_RECORD ||
               node->type->kind == TYPE_KIND_ARRAY) {
      /* For classes, allocate only pointer size since instances are
       * heap-allocated */
  ...
  if (alias != NULL) {
    ...
    if (alias->is_set) {
      ...
      KGPC_COMPILER_HARD_ASSERT(set_size > 0 && set_size <= INT_MAX,
                                ...
                                alias->alias_name != NULL ? alias->alias_name
                                                          : "(anonymous set)");
      return (int)set_size;
    }
    if (alias->is_shortstring) {
...
void codegen_register_local_types(ListNode_t *type_decls, SymTab_t *symtab) {
  ...
  for (ListNode_t *cur = type_decls; cur != NULL; cur = cur->next) {
    ...
    if (kgpc != NULL) {
      ...
      if (decl->tree_data.type_decl_data.info.record != NULL) {
        codegen_register_record_field_enum_literals(
            symtab, decl->tree_data.type_decl_data.info.record);
      }
      if (created_kgpc)
  ...
  /* Second pass: register aliases and resolve pointer targets now that records
   * exist. */
  for (ListNode_t *cur = type_decls; cur != NULL; cur = cur->next) {
    if (cur->type != LIST_TREE || cur->cur == NULL)
      continue;
    ...
    int created_kgpc = 0;
    if (kgpc == NULL) {
      kgpc = create_kgpc_type_from_type_alias(
          alias, symtab, decl->tree_data.type_decl_data.defined_in_unit);
      created_kgpc = 1;
    }
...
codegen_register_record_field_enum_literals(SymTab_t *symtab,
  ...
  for (ListNode_t *field_node = record->fields; field_node != NULL;
       field_node = field_node->next) {
    if (field_node->type == LIST_RECORD_FIELD && field_node->cur != NULL) {
      struct RecordField *field = (struct RecordField *)field_node->cur;
      if (field->enum_literals != NULL) {
        ...
        int ordinal = 0;
        for (ListNode_t *lit = field->enum_literals; lit != NULL;
             lit = lit->next, ++ordinal) {
      ...
      for (ListNode_t *branch_node = variant->branches; branch_node != NULL;
        ...
        if (branch_node->type == LIST_VARIANT_BRANCH &&
            branch_node->cur != NULL) {
          struct VariantBranch *branch =
              (struct VariantBranch *)branch_node->cur;
...
static int codegen_eval_const_expr(struct Expression *expr, SymTab_t *symtab,
                                   long long *out_value);
static int codegen_eval_string_const_expr(struct Expression *expr,
                                          SymTab_t *symtab, char **out_value);

static HashNode_t *codegen_find_const_eval_symbol(SymTab_t *symtab,
                                                  const char *id) {
  ...
  *out_value = NULL;
  ...
  case EXPR_VAR_ID: {
    if (symtab == NULL || expr->expr_data.id == NULL)
      return 1;
    HashNode_t *node =
        codegen_find_const_eval_symbol(symtab, expr->expr_data.id);
    if (node == NULL)
      return 1;
  ...
  char *left = NULL;
  char *right = NULL;
  if (codegen_eval_string_const_expr(expr->expr_data.addop_data.left_expr,
                                     symtab, &left) != 0)
    return 1;
  if (codegen_eval_string_const_expr(expr->expr_data.addop_data.right_term,
                                     symtab, &right) != 0) {
    free(left);
    return 1;
  }
...
void codegen_register_const_decls(ListNode_t *const_decls, SymTab_t *symtab) {
  ...
  for (ListNode_t *cur = const_decls; cur != NULL; cur = cur->next) {
    ...
    if (expression_is_set_const_expr(symtab, value) &&
      ...
      PushSetConstOntoScope(symtab, (char *)id, set_bytes, (int)set_size,
                            set_type);
      if (set_type != NULL)
        destroy_kgpc_type(set_type);
      continue;
    ...
    if (codegen_eval_const_expr(value, symtab, &const_value)) {
      ...
      if ((value != NULL && value->type == EXPR_CHAR_CODE) ||
        ...
        if (char_type != NULL) {
          if (PushConstOntoScope_Typed(symtab, (char *)id, const_value,
                                       char_type) == 0) {
            HashNode_t *const_node = NULL;
            if (FindSymbol(&const_node, symtab, id) != 0 &&
                const_node != NULL && const_node->const_string_value == NULL) {
              char char_string[2];
      ...
      int string_eval_ok =
          (codegen_eval_string_const_expr(value, symtab, &string_value) == 0);
      if (string_eval_ok && string_value != NULL) {
...
static void codegen_register_inline_var_enum_literals(Tree_t *decl,
                                                      SymTab_t *symtab) {
...
static KgpcType *
codegen_prefer_promoted_shortstring_type(KgpcType *decl_type,
                                         const HashNode_t *var_node) {
  if (decl_type != NULL && var_node != NULL && var_node->type != NULL &&
      kgpc_type_is_string(decl_type) && !kgpc_type_is_shortstring(decl_type) &&
...
void codegen_register_decl_list(CodeGenContext *ctx, ListNode_t *decls,
  ...
  for (ListNode_t *cur = decls; cur != NULL; cur = cur->next) {
    ...
    for (ListNode_t *id_node = ids; id_node != NULL; id_node = id_node->next) {
      ...
      if (is_array_decl)
        PushArrayOntoScope_Typed(symtab, (char *)id_node->cur,
                                 effective_decl_type);
      else
        PushVarOntoScope_Typed(symtab, (char *)id_node->cur,
                               effective_decl_type);
       ...
       * same-named typed-consts across units. */
      {
        ...
        if (registered != NULL) {
          ...
          if (registered->is_typed_const && registered->defined_in_unit &&
              registered->source_unit_index > 0 &&
              registered->mangled_id == NULL &&
              codegen_typed_const_name_collides_ctx(
                  ctx, (const char *)id_node->cur)) {
            registered->mangled_id = codegen_make_unit_qualified_key(
                registered->source_unit_index, (const char *)id_node->cur);
          }
        }
      }
      ...
      if (is_param) {
        ...
        if (param_node != NULL) {
          ...
          param_node->is_var_parameter =
              (is_var_param || is_untyped_param) ? 1 : 0;
        }
      }
    }
...
static void codegen_static_link_spilled(Register_t *reg,
...
void codegen_push_pending_ctor_temp(CodeGenContext *ctx, int rbp_offset) {
  ...
  if (ctx->pending_ctor_temp_count == ctx->pending_ctor_temp_capacity) {
    int new_capacity = (ctx->pending_ctor_temp_capacity == 0)
                           ? 4
...
ListNode_t *codegen_flush_pending_ctor_temps(CodeGenContext *ctx,
  ...
  for (int i = 0; i < ctx->pending_ctor_temp_count; ++i) {
    ...
    inst_list =
        codegen_call_with_shadow_space(inst_list, "kgpc_release_class_temp");
  }
...
void codegen_track_managed_dynarray_temp(CodeGenContext *ctx, int rbp_offset) {
  ...
  if (ctx->managed_dynarray_temp_count == ctx->managed_dynarray_temp_capacity) {
    int new_capacity = (ctx->managed_dynarray_temp_capacity == 0)
                           ? 4
...
Register_t *codegen_acquire_static_link(CodeGenContext *ctx,
  ...
  if (scope == NULL) {
    codegen_report_error(
        ctx,
        "ERROR: Unable to determine current scope for static link traversal.");
    return NULL;
  }
  ...
  if (offsets == NULL) {
    codegen_report_error(
        ctx, "ERROR: Failed to allocate static link traversal metadata.");
    return NULL;
  }
  ...
  for (int i = 0; i < levels_to_traverse; ++i) {
    if (current_scope == NULL) {
      codegen_report_error(
          ctx, "ERROR: Static link chain shorter than requested depth.");
      free(offsets);
      return NULL;
    }

    if (!codegen_find_static_link_offset(current_scope, &offsets[i])) {
      codegen_report_error(ctx, "ERROR: Static link slot missing at depth %d.",
                           i);
      free(offsets);
      return NULL;
    }
 ...
 * ------------------------------------------------------------------------- */
static void asm_debug_comment(FILE *out, const char *tag, int indent,
                              const char *fmt, ...) {
...
static const char *hash_type_to_string(enum HashType type) {
  ...
  case HASHTYPE_BUILTIN_PROCEDURE:
    return "builtin-proc";
  case HASHTYPE_TYPE:
    return "type";
  default:
...
static void summarize_string_literal(const char *src, char *dest,
                                     size_t dest_size) {
...
static void codegen_emit_semantic_scope_comments(FILE *out,
  ...
  for (int i = 0; i < TABLE_SIZE; ++i) {
    ...
    while (entry != NULL) {
      ...
      if (node->mangled_id != NULL && node->mangled_id[0] != '\0')
        snprintf(mangled_buf, sizeof(mangled_buf), ", mangled=%s",
                 node->mangled_id);
      ...
      if (node->hash_type == HASHTYPE_CONST) {
        if (node->const_string_value != NULL) {
          char snippet[48];
          summarize_string_literal(node->const_string_value, snippet,
                                   sizeof(snippet));
      ...
      asm_debug_comment(out, "semcheck", indent + 1, "%s kind=%s type=%s%s%s%s",
                        (node->id != NULL) ? node->id : "<unnamed>",
                        hash_type_to_string(node->hash_type),
                        hashnode_type_to_string(node), mangled_buf, const_buf,
                        link_buf);
    }
  }
...
static void codegen_emit_semantic_debug_block(CodeGenContext *ctx) {
  if (!asm_debug_flag() || ctx == NULL || ctx->symtab == NULL ||
      ctx->output_file == NULL)
    return;

  FILE *out = ctx->output_file;
  asm_debug_comment(out, "semcheck", 0, "--- symbol table snapshot ---");
  codegen_emit_semantic_scope_comments(out, ctx->symtab->builtin_scope->table,
                                       "builtins", 0);
...
static void codegen_emit_function_debug_comments(const char *func_name,
                                                 CodeGenContext *ctx) {
  if (!asm_debug_flag() || ctx == NULL || ctx->output_file == NULL ||
      func_name == NULL)
    return;
  ...
  if (scope != NULL) {
    ...
    asm_debug_comment(ctx->output_file, "codegen", 1,
                      "stack locals=%dB temps=%dB args=%dB total=%dB", locals,
                      temps, args, total);
  } else {
    asm_debug_comment(ctx->output_file, "codegen", 1,
  ...
  asm_debug_comment(ctx->output_file, "codegen", 1, "static-link=%s",
                    needs_link ? "required" : "not-required");
}
...
static char *codegen_make_program_var_label(CodeGenContext *ctx,
  ...
  char buffer[256];
  snprintf(buffer, sizeof(buffer), "__kgpc_program_var_%s_%d", sanitized,
           ++ctx->global_data_counter);
 ...
 * codegen passes. */
static int codegen_decl_list_has_typed_const_named(ListNode_t *decls,
                                                   const char *bare_id) {
  for (ListNode_t *cur = decls; cur != NULL; cur = cur->next) {
    Tree_t *decl = (Tree_t *)cur->cur;
    if (decl == NULL)
...
static void codegen_emit_bss_or_comm(FILE *out, const char *sym,
                                     const char *label, int size, int alignment,
                                     int defined_in_unit) {
  int need_alias = (label != NULL && sym != NULL && defined_in_unit &&
                    strcmp(sym, label) != 0);
...
void codegen_common_typeinfo_label_for_type_id(SymTab_t *symtab,
  ...
  HashNode_t *type_node = NULL;
  if (symtab != NULL && FindSymbol(&type_node, symtab, type_id) != 0 &&
...

    {
      ...
      if (record != NULL) {
         ...
         * type_id is the full qualified name (e.g.
         * "tcgprocinfo.ttempinfo_flags_entry") which matches how the label is
         * emitted.  Fall back to the caller's type_id if record->type_id is not
         * set. */
        const char *label_name =
            (record->type_id != NULL) ? record->type_id : type_id;
...
static void codegen_emit_enum_typeinfo_for_alias(CodeGenContext *ctx,
                                                 const char *type_name,
                                                 struct TypeAlias *alias) {
  if (ctx == NULL || ctx->output_file == NULL || type_name == NULL ||
      alias == NULL)
    return;
  if (!alias->is_enum || alias->enum_literals == NULL)
    return;
  ...
  codegen_sanitize_identifier_for_label(type_name, type_label,
                                        sizeof(type_label));

  size_t typeinfo_len =
      strlen(type_label) + strlen("__kgpc_enum_typeinfo_") + 1;
   ...
   *   NameList: packed shortstrings for each literal, followed by unit/type
   * owner This keeps KGPC-emitted enum RTTI aligned with the TypInfo offsets
...
static void codegen_emit_enum_typeinfo_from_table(
    CodeGenContext *ctx, HashTable_t *table, int emit_unit_types,
    const char **emitted_labels, int *emitted_count, int *emitted_any) {
  ...
  for (int i = 0; i < TABLE_SIZE; ++i) {
    ...
    while (entry != NULL) {
      ...
      const char *type_name =
          (alias->alias_name != NULL) ? alias->alias_name : node->id;
      ...
      for (int idx = 0; idx < *emitted_count; ++idx) {
        if (emitted_labels[idx] != NULL &&
            strcmp(emitted_labels[idx], label) == 0) {
          already_emitted = 1;
          break;
        }
      }
...
static void codegen_emit_enum_typeinfo(CodeGenContext *ctx, SymTab_t *symtab,
                                       int emit_unit_types) {
  ...
  if (symtab->builtin_scope->table != NULL)
    codegen_emit_enum_typeinfo_from_table(ctx, symtab->builtin_scope->table,
                                          emit_unit_types, emitted_labels,
                                          &emitted_count, &emitted_any);

  ScopeNode *scope = symtab->current_scope;
  while (scope != NULL) {
    HashTable_t *table = scope->table;
    codegen_emit_enum_typeinfo_from_table(ctx, table, emit_unit_types,
                                          emitted_labels, &emitted_count,
                                          &emitted_any);
    scope = scope->parent;
  }
...
ListNode_t *codegen_emit_interface_vtable_slot_init(
  ...
  if (ctx == NULL || class_record == NULL || class_type_id == NULL ||
      instance_reg == NULL || class_record->num_interfaces <= 0)
    return inst_list;

  long long base_size = 0;
  codegen_sizeof_record_type(ctx, (struct RecordType *)class_record,
                             &base_size);
...
ListNode_t *codegen_emit_interface_dispatch(
  ...
  if (ctx == NULL || self_reg == NULL || label_prefix == NULL ||
      target_slot_label == NULL)
    return inst_list;
  ...
  /* VMT_VINSTANCESIZE2_OFFSET (slot 1): -vInstanceSize; sum with slot 0 == 0
   * iff VMT */
 ...
 * If nostackframe is set, only emits the label without prologue (push %rbp /
 * mov %rsp, %rbp). If cname_override is set and differs from func_name, emits
 * an additional .globl + label alias. */
void codegen_function_header_ex_alias(char *func_name, CodeGenContext *ctx,
                                      int nostackframe,
                                      const char *cname_override) {
  codegen_function_header_ex_alias_vis(func_name, ctx, nostackframe,
                                       cname_override, 0);
}
...
void codegen_function_header_ex(char *func_name, CodeGenContext *ctx,
                                int nostackframe) {
...
void codegen(Tree_t *tree, const char *input_file_name, CodeGenContext *ctx,
  ...
  if (ctx->target_abi != KGPC_TARGET_ABI_SYSTEM_V &&
      ctx->target_abi != KGPC_TARGET_ABI_WINDOWS)
    ctx->target_abi = current_target_abi();

  g_current_codegen_abi = ctx->target_abi;
  g_stack_home_space_bytes = (ctx->target_abi == KGPC_TARGET_ABI_WINDOWS)
                                 ? KGPC_WINDOWS_SHADOW_SPACE_BYTES
                                 : 0;
      ...
      for (int i = 0; i < comp_ctx->loaded_unit_count; ++i) {
        ...
        if (unit != NULL && unit->type == TREE_UNIT)
          codegen_apply_collision_prefixes(
              unit->tree_data.unit_data.subprograms, symtab, &collision_map);
      }
    }
  ...
  if (comp_ctx != NULL) {
    for (int i = 0; i < comp_ctx->loaded_unit_count; ++i) {
      Tree_t *unit = comp_ctx->loaded_units[i].unit_tree;
      if (unit != NULL && unit->type == TREE_UNIT)
        codegen_collect_callable_export_names(
  ...
  if (comp_ctx != NULL) {
    for (int i = 0; i < comp_ctx->loaded_unit_count; ++i) {
      ...
      if (unit == NULL || unit->type != TREE_UNIT)
        continue;
      codegen_mark_class_constructors_used(
  ...
  codegen_mark_class_constructors_used(
      tree->tree_data.program_data.type_declaration, symtab);
  ...
  if (comp_ctx != NULL) {
    for (int i = 0; i < comp_ctx->loaded_unit_count; ++i) {
      Tree_t *unit = comp_ctx->loaded_units[i].unit_tree;
      if (unit != NULL && unit->type == TREE_UNIT)
        codegen_collect_available_subprogram_labels(
            unit->tree_data.unit_data.subprograms);
    }
  }
  codegen_collect_available_subprogram_labels(
  ...
  if (g_codegen_available_subprograms != NULL) {
    ...
    g_codegen_available_subprograms = NULL;
    codegen_set_destroy(&g_available_subprograms_set);
    memset(&g_available_subprograms_set, 0,
           sizeof(g_available_subprograms_set));
    g_available_subprograms_tail = NULL;
  }
...
void codegen_unit(Tree_t *tree, const char *input_file_name,
                  CodeGenContext *ctx, SymTab_t *symtab) {
  assert(tree != NULL);
  ...
  codegen_mark_class_constructors_used(
      tree->tree_data.unit_data.interface_type_decls, symtab);
  codegen_mark_class_constructors_used(
      tree->tree_data.unit_data.implementation_type_decls, symtab);
  codegen_rodata(ctx, symtab);
  codegen_emit_enum_typeinfo(ctx, symtab, 1);
  codegen_collect_available_subprogram_labels(
      tree->tree_data.unit_data.subprograms);
  ...
  if (tree->tree_data.unit_data.initialization != NULL) {
    ...
    snprintf(init_label, sizeof(init_label), "_UNIT_%s_INIT",
    ...
    push_stackscope();
    codegen_function_locals(tree->tree_data.unit_data.interface_var_decls, ctx,
                            symtab);
    codegen_function_locals(tree->tree_data.unit_data.implementation_var_decls,
                            ctx, symtab);
    codegen_emit_const_decl_equivs_from_list(
        ctx, tree->tree_data.unit_data.interface_const_decls);
    codegen_emit_const_decl_equivs_from_list(
        ctx, tree->tree_data.unit_data.implementation_const_decls);
    {
    ...
    ctx->next_vreg_id = 0;
    inst_list = codegen_stmt(tree->tree_data.unit_data.initialization,
  ...
  if (tree->tree_data.unit_data.finalization != NULL) {
    ...
    snprintf(final_label, sizeof(final_label), "_UNIT_%s_FINAL",
             unit_id ? unit_id : "UNKNOWN");
    ...
    ListNode_t *inst_list = NULL;
    ctx->next_vreg_id = 0;
    inst_list = codegen_stmt(tree->tree_data.unit_data.finalization, inst_list,
                             ctx, symtab);
...
static int codegen_const_symbol_emitted(ListNode_t *emitted_symbols,
                                        const char *id) {
...
static void codegen_emit_integer_const_equivs_from_table(
    CodeGenContext *ctx, HashTable_t *table, ListNode_t **emitted_symbols) {
  assert(ctx != NULL);
  ...
  for (int bucket = 0; bucket < TABLE_SIZE; ++bucket) {
    ...
    while (bucket_node != NULL) {
      ...
      if (symbol != NULL && symbol->hash_type == HASHTYPE_CONST &&
          symbol->is_constant && symbol->const_string_value == NULL &&
          symbol->const_set_value == NULL && symbol->id != NULL &&
          codegen_should_emit_const_equiv_symbol(ctx, table, symbol,
                                                 symbol->id) &&
...
static void codegen_emit_integer_const_equivs(CodeGenContext *ctx,
                                              SymTab_t *symtab) {
  assert(ctx != NULL);
  assert(symtab != NULL);
  ...
  codegen_emit_integer_const_equivs_from_table(
      ctx, symtab->builtin_scope->table, &emitted_symbols);
...
void codegen_emit_local_const_equivs(CodeGenContext *ctx, SymTab_t *symtab) {
  ...
  ListNode_t *emitted_symbols = NULL;
  codegen_emit_integer_const_equivs_from_table(
      ctx, symtab->current_scope->table, &emitted_symbols);
...
static int codegen_eval_const_expr(struct Expression *expr, SymTab_t *symtab,
  ...
  case EXPR_TYPECAST:
    if (expr->expr_data.typecast_data.expr != NULL)
      return codegen_eval_const_expr(expr->expr_data.typecast_data.expr, symtab,
                                     out_value);
    return 0;
  case EXPR_ADDOP: {
    long long left, right;
    if (codegen_eval_const_expr(expr->expr_data.addop_data.left_expr, symtab,
                                &left) &&
        codegen_eval_const_expr(expr->expr_data.addop_data.right_term, symtab,
                                &right)) {
  ...
  case EXPR_MULOP: {
    long long left, right;
    if (codegen_eval_const_expr(expr->expr_data.mulop_data.left_term, symtab,
                                &left) &&
        codegen_eval_const_expr(expr->expr_data.mulop_data.right_factor, symtab,
                                &right)) {
...
void codegen_emit_const_decl_equivs_from_list(CodeGenContext *ctx,
  ...
  for (ListNode_t *cur = const_decls; cur != NULL; cur = cur->next) {
    ...
    if (ctx->symtab != NULL) {
      HashNode_t *node = NULL;
      if (FindSymbol(&node, ctx->symtab, (char *)id) != 0 && node != NULL &&
          (node->hash_type == HASHTYPE_CONST || node->is_typed_const)) {
        int type_tag = codegen_tag_from_kgpc(node->type);
        if (!is_ordinal_type(type_tag))
          continue;
    ...
    long long const_value = 0;
    if (codegen_eval_const_expr(value, ctx != NULL ? ctx->symtab : NULL,
                                &const_value))
      fprintf(ctx->output_file, ".equ %s, %lld\n", id, const_value);
  }
}
...
void codegen_register_owner_unit_scope(CodeGenContext *ctx, SymTab_t *symtab,
                                       int source_unit_index) {
  if (ctx == NULL || symtab == NULL || ctx->comp_ctx == NULL ||
      source_unit_index <= 0)
    return;

  LoadedUnit *loaded_unit =
      compilation_context_find_unit(ctx->comp_ctx, source_unit_index);
  ...
  codegen_register_type_enum_literals(
      unit->tree_data.unit_data.interface_type_decls, symtab);
  codegen_register_type_enum_literals(
      unit->tree_data.unit_data.implementation_type_decls, symtab);
  codegen_register_decl_list(ctx, unit->tree_data.unit_data.interface_var_decls,
                             symtab, 0);
  codegen_register_decl_list(
      ctx, unit->tree_data.unit_data.implementation_var_decls, symtab, 0);
  codegen_register_const_decls(unit->tree_data.unit_data.interface_const_decls,
                               symtab);
  codegen_register_const_decls(
      unit->tree_data.unit_data.implementation_const_decls, symtab);
  if (unit_scope != NULL)
...
void codegen_main(char *prgm_name, CodeGenContext *ctx) {
  ...
  call_space = codegen_target_is_windows() ? g_stack_home_space_bytes
                                           : KGPC_WINDOWS_SHADOW_SPACE_BYTES;
  if (call_space > 0) {
  ...
  while (inst_list != NULL) {
    ...
    if (text != NULL) {
      ...
      while ((cursor = strchr(cursor, '-')) != NULL) {
        ...
        if (endptr != NULL && strncmp(endptr, "(%rbp)", 6) == 0 && offset > 0 &&
            offset <= INT_MAX && (int)offset > max_offset) {
...
static int codegen_compute_stack_space_from_insts(ListNode_t *inst_list,
                                                  CodeGenContext *ctx) {
...
void codegen_stack_space_for_inst_list(ListNode_t *inst_list,
  ...
  if (aligned_space != 0) {
    ...
    /* Zero-initialize the allocated stack space to ensure local variables start
     * with zero values. This is critical for code that assumes uninitialized
     * variables are zero (like linked lists). We use rep stosq for efficient
     * zero-filling.
     *
     ...
     * r10 and r11 are caller-saved scratch registers safe to use on both
     * platforms.
     */
    ...
    if (codegen_target_is_windows()) {
       ...
       * Note: rdi is non-volatile (callee-saved) on Windows x64 ABI, so we must
       * preserve it. */
      fprintf(ctx->output_file,
...
char *codegen_program(Tree_t *prgm, CodeGenContext *ctx, SymTab_t *symtab,
  ...
  if (comp_ctx != NULL) {
    ...
    for (int i = 0; i < comp_ctx->loaded_unit_count; ++i) {
      ...
      codegen_function_locals(unit->tree_data.unit_data.interface_var_decls,
                              ctx, symtab);
      codegen_function_locals(
          unit->tree_data.unit_data.implementation_var_decls, ctx, symtab);
      codegen_emit_const_decl_equivs_from_list(
          ctx, unit->tree_data.unit_data.interface_const_decls);
      codegen_emit_const_decl_equivs_from_list(
          ctx, unit->tree_data.unit_data.implementation_const_decls);
    }
    symtab->current_unit_index = saved_unit_index;
    symtab->current_scope = saved_scope;
  }
   ...
   */
  {
     ...
     * but no cname_override.  Copy the alias so the impl header emits both
     * labels. */
    for (ListNode_t *fwd = all_subs_head; fwd != NULL; fwd = fwd->next) {
      if (fwd->type != LIST_TREE || fwd->cur == NULL)
        continue;
      Tree_t *fwd_sub = (Tree_t *)fwd->cur;
      if (fwd_sub->type != TREE_SUBPROGRAM)
        continue;
      const char *alias = fwd_sub->tree_data.subprogram_data.cname_override;
      if (alias == NULL)
        continue;
      if (fwd_sub->tree_data.subprogram_data.statement_list != NULL)
        continue; /* has body — not a forward decl */
      /* Only propagate FPC_/KGPC_ internal aliases */
      if (strncmp(alias, "FPC_", 4) != 0 && strncmp(alias, "KGPC_", 5) != 0)
        continue;
      ...
      for (ListNode_t *impl = all_subs_head; impl != NULL; impl = impl->next) {
        if (impl->type != LIST_TREE || impl->cur == NULL)
          continue;
        Tree_t *impl_sub = (Tree_t *)impl->cur;
        if (impl_sub == fwd_sub || impl_sub->type != TREE_SUBPROGRAM)
          continue;
        if (impl_sub->tree_data.subprogram_data.statement_list == NULL)
          continue;
        if (impl_sub->tree_data.subprogram_data.cname_override != NULL)
          continue; /* already has alias */
        const char *impl_id = impl_sub->tree_data.subprogram_data.id;
        if (impl_id == NULL)
          continue;
        int matched = (strcasecmp(fwd_id, impl_id) == 0);
        if (!matched &&
            impl_sub->tree_data.subprogram_data.cname_override != NULL &&
            strcasecmp(impl_sub->tree_data.subprogram_data.cname_override,
                       alias) == 0)
          matched = 1;
        if (matched) {
          impl_sub->tree_data.subprogram_data.cname_override =
              strdup(fwd_sub->tree_data.subprogram_data.cname_override);
          break;
        }
      }
    }
  ...
  if (comp_ctx != NULL) {
    ...
    for (int i = 0; i < comp_ctx->loaded_unit_count; ++i) {
      ...
      if (unit_scope != NULL)
        symtab->current_scope = unit_scope;
      inst_list = codegen_var_initializers(
          unit->tree_data.unit_data.interface_var_decls, inst_list, ctx,
          symtab);
      inst_list = codegen_var_initializers(
          unit->tree_data.unit_data.implementation_var_decls, inst_list, ctx,
          symtab);
    }
    symtab->current_unit_index = saved_unit_index;
    symtab->current_scope = saved_scope;
  ...
  inst_list =
      codegen_var_initializers(data->var_declaration, inst_list, ctx, symtab);
      ...
      if (init_stmt->type == STMT_COMPOUND_STATEMENT) {
        ...
        while (stnode != NULL) {
          if (stnode->type == LIST_STMT && stnode->cur != NULL)
            inst_list = codegen_stmt((struct Statement *)stnode->cur, inst_list,
                                     ctx, symtab);
          stnode = stnode->next;
        }
      } else {
  ...
  inst_list = codegen_emit_managed_global_cleanup(
      inst_list, data->var_declaration, ctx, symtab);
...
void codegen_function_locals(ListNode_t *local_decl, CodeGenContext *ctx,
  ...
  while (cur != NULL) {
    ...
    if (tree->type == TREE_VAR_DECL) {
      ...
      KgpcType *decl_type =
          resolve_type_from_vardecl(tree, symtab, &decl_type_owned);
      ...
      while (id_list != NULL) {
        ...
        param_type =
            codegen_prefer_promoted_shortstring_type(param_type, var_info);
        KGPC_COMPILER_HARD_ASSERT(
            param_type != NULL,
        ...
        if (param_type != NULL && kgpc_type_is_array(param_type)) {
          ...
          int dim_info_result = kgpc_type_get_array_dimension_info(
              param_type, symtab, &array_info);

          /* If dimension info lookup succeeded, use its values; otherwise fall
           * back to simpler methods */
          int element_size;
          int array_start;
          ...
          if (dim_info_result == 0 && array_info.dim_count > 0 &&
            ...
            /* Use computed values from kgpc_type_get_array_dimension_info which
             * handles multi-dimensional arrays and constant-based bounds
             * correctly */
            ...
            if (element_size <= 0 && param_type != NULL &&
                ...
                param_type->info.array_info.element_type->kind ==
                    TYPE_KIND_RECORD) {
              long long record_size = 0;
              if (codegen_sizeof_record_type(
                      ctx,
                      param_type->info.array_info.element_type->info
                          .record_info,
                      &record_size) == 0 &&
                  record_size > 0 && record_size <= INT_MAX) {
          ...
          if (is_open_array) {
            ...
            if (is_program_scope) {
              static_label =
                  codegen_make_program_var_label(ctx, (char *)id_list->cur);
              if (ctx->output_file != NULL && static_label != NULL) {
                int descriptor_bytes =
                    codegen_dynamic_array_descriptor_bytes(element_size);
                int alignment = descriptor_bytes >= 8 ? 8 : DOUBLEWORD;
                fprintf(ctx->output_file, "\t.comm\t%s,%d,%d\n", static_label,
                        descriptor_bytes, alignment);
              }
            }
            {
              char *storage_key = codegen_var_storage_key(
                  ctx, symtab, tree, (const char *)id_list->cur);
              add_dynamic_array(
                  storage_key != NULL ? storage_key : (char *)id_list->cur,
        ...
        } else if (var_info != NULL && var_info->type != NULL &&
                   kgpc_type_is_array(var_info->type)) {
          KgpcType *array_type = var_info->type;
          KgpcType *element_type = array_type->info.array_info.element_type;
          long long element_size_ll =
              kgpc_type_get_array_element_size(array_type);
          ...
          if (is_open_array) {
            ...
            if (is_program_scope) {
              static_label =
                  codegen_make_program_var_label(ctx, (char *)id_list->cur);
              if (ctx->output_file != NULL && static_label != NULL) {
                int descriptor_bytes = codegen_dynamic_array_descriptor_bytes(
                    (int)element_size_ll);
                int alignment = descriptor_bytes >= 8 ? 8 : DOUBLEWORD;
                fprintf(ctx->output_file, "\t.comm\t%s,%d,%d\n", static_label,
                        descriptor_bytes, alignment);
          ...
          if (symtab != NULL) {
            if (FindSymbol(&var_info, symtab, id_list->cur) != 0 &&
                var_info != NULL)
              size_node = var_info;
          }
          ...
          if (decl_storage_size > 0) {
            ...
            if (size > 0) {
              ...
              if (node_is_class_type(size_node)) {
                CODEGEN_DEBUG("DEBUG ALLOC: Detected class type for '%s', "
                              "allocating 8 bytes\n",
                              (char *)id_list->cur);
                alloc_size =
                    8; /* Classes are heap-allocated; variable holds pointer */
              } else {
                CODEGEN_DEBUG("DEBUG ALLOC: Detected record type for '%s', "
                              "allocating full size\n",
                              (char *)id_list->cur);
          ...
          if (is_program_scope) {
            const char *absolute_target =
                tree->tree_data.var_decl_data.absolute_target;
            const char *absolute_base =
                tree->tree_data.var_decl_data.absolute_base_id;
            const char *absolute_field =
                tree->tree_data.var_decl_data.absolute_field_id;
            if (absolute_target != NULL && id_list != NULL &&
                id_list->next == NULL) {
              if (absolute_base != NULL && absolute_field != NULL) {
                ...
                if (base_var != NULL && field_name != NULL) {
                  ...
                  if (field_offset >= 0) {
                    if (add_absolute_var_alias_with_offset(
                            (char *)id_list->cur, (char *)base_var,
                            field_offset, alloc_size) == 0) {
                ...
                fprintf(stderr,
                        "Warning: absolute variable alias to record field '%s' "
                        "failed to resolve.\n",
                        absolute_target);
              } else if (absolute_base != NULL &&
                         (add_absolute_var_alias((char *)id_list->cur,
                                                 absolute_base) == 0 ||
                ...
                fprintf(stderr,
                        "Warning: failed to resolve absolute variable alias "
                        "target '%s'.\n",
                        absolute_target);
              }
            }
            ...
            if (ctx->output_file != NULL && static_label != NULL) {
              if (is_external_var) {
                /* External variable: don't allocate storage, just reference the
                 * symbol */
                /* No .comm directive needed - the symbol is defined elsewhere
                 */
              } else {
                int alignment =
                    alloc_size >= 16 ? 16 : (alloc_size >= 8 ? 8 : DOUBLEWORD);
                /* Only emit a bare-name alias when there is no explicit cname
                 * override */
                int defined_for_alias =
                    (cname_override == NULL)
                        ? (tree->tree_data.var_decl_data.defined_in_unit &&
                           !(tree->tree_data.var_decl_data.is_typed_const &&
                             codegen_typed_const_name_collides_ctx(
                ...
                if (cname_override == NULL &&
                    tree->tree_data.var_decl_data.is_typed_const &&
                    codegen_try_emit_typed_const_record_static_alias(
                        ctx, symtab, tree, (const char *)id_list->cur,
                        static_label, alloc_size, defined_for_alias) == 0) {
                  statically_emitted = 1;
                }
                if (!statically_emitted) {
                  codegen_emit_bss_or_comm(
                      ctx->output_file, (const char *)id_list->cur,
                      static_label, alloc_size, alignment, defined_for_alias);
                }
              }
            }
            char *storage_key = codegen_var_storage_key(
                ctx, symtab, tree, (const char *)id_list->cur);
            add_static_var(storage_key != NULL ? storage_key
                                               : (char *)id_list->cur,
            ...
            if (tree->tree_data.var_decl_data.has_static_storage &&
              ...
              if (!tree->tree_data.var_decl_data.static_storage_emitted) {
                if (ctx->output_file != NULL) {
                  int alignment = alloc_size >= 16
                                      ? 16
                                      : (alloc_size >= 8 ? 8 : DOUBLEWORD);
                  fprintf(ctx->output_file, "\t.comm\t%s,%d,%d\n", static_label,
      ...
      if (element_size <= 0 &&
          codegen_sizeof_type_reference(ctx, arr->type, arr->type_id,
                                        record_desc, &computed_size) == 0 &&
          computed_size > 0 && computed_size <= INT_MAX) {
        element_size = (int)computed_size;
      } else if (record_desc != NULL &&
                 codegen_sizeof_record_type(ctx, record_desc, &computed_size) ==
                     0 &&
                 computed_size > 0 && computed_size <= INT_MAX) {
        element_size = (int)computed_size;
      }
      ...
      if (is_dynamic) {
        ...
        if (id_list != NULL && symtab != NULL) {
          ...
          if (FindSymbol(&var_node, symtab, var_name) && var_node != NULL &&
              var_node->type != NULL) {
              ...
              if (FindSymbol(&type_node, symtab, range_str) != 0 &&
                ...
                if (range_alias != NULL) {
                  if (range_alias->is_enum &&
                    ...
                    dim_size =
                        (long long)ListLength(range_alias->enum_literals);
                ...
                if (dotdot != NULL) {
                  ...
                  if (left_len > 0 && left_len < sizeof(left_buf)) {
                    ...
                    if (right_len > 0 && right_len < sizeof(right_buf)) {
                      ...
                      int lo_ok = 0, hi_ok = 0;
                      HashNode_t *ln = NULL;
                      if (FindSymbol(&ln, symtab, lp) != 0 && ln != NULL &&
                      ...
                      HashNode_t *rn = NULL;
                      if (FindSymbol(&rn, symtab, rp) != 0 && rn != NULL &&
...
static ListNode_t *codegen_emit_dynarray_cleanup_for_type(
    ListNode_t *inst_list, CodeGenContext *ctx, KgpcType *type,
    const char *base_label, int base_rbp_offset, long long extra_offset,
    int depth);
static ListNode_t *codegen_emit_dynarray_cleanup_record_fields(
    ListNode_t *inst_list, CodeGenContext *ctx, struct RecordType *record,
    const char *base_label, int base_rbp_offset, long long base_within_record,
    int depth);
...
static ListNode_t *codegen_emit_finalize_call(ListNode_t *inst_list,
  ...
  inst_list = add_inst(inst_list, "\tmovl\t$0, %eax\n");
  inst_list =
      codegen_call_with_shadow_space(inst_list, "kgpc_dynarray_finalize_local");
  return inst_list;
}
...
static ListNode_t *codegen_emit_dynarray_cleanup_record_fields(
  ...
  for (ListNode_t *cur = record->fields; cur != NULL; cur = cur->next) {
    ...
    if (field->is_array && field->array_is_open) {
      ...
      long long sz = 0;
      if (codegen_sizeof_type(ctx, field->type, field->type_id, NULL, &sz,
                              depth + 1) != 0)
        sz = 0;
      field_size = sz;
      if (sz >= 8)
    ...
    if (field->is_array && field->array_is_open) {
      ...
      if (!element_has_dynarray) {
        inst_list = codegen_emit_finalize_call(inst_list, ctx, base_label,
                                               base_rbp_offset, offset);
      }
    } else if (field->nested_record != NULL &&
               codegen_record_contains_dynarray(field->nested_record)) {
      inst_list = codegen_emit_dynarray_cleanup_record_fields(
          inst_list, ctx, field->nested_record, base_label, base_rbp_offset,
          offset, depth + 1);
      ...
      if (count > 0) {
        ...
        if (element_has_dynarray && element_size > 0) {
          ...
          for (long long i = 0; i < emit_count; ++i) {
            ...
            if (field->array_element_record != NULL) {
              inst_list = codegen_emit_dynarray_cleanup_record_fields(
                  inst_list, ctx, field->array_element_record, base_label,
                  base_rbp_offset, elem_offset, depth + 1);
            } else if (field->array_element_kgpc_type != NULL) {
              inst_list = codegen_emit_dynarray_cleanup_for_type(
                  inst_list, ctx, field->array_element_kgpc_type, base_label,
                  base_rbp_offset, elem_offset, depth + 1);
            }
          }
        }
...
static ListNode_t *codegen_emit_dynarray_cleanup_for_type(
  ...
  if (kgpc_type_is_dynamic_array(type)) {
    ...
    if (element_type != NULL && codegen_type_contains_dynarray(element_type))
      return inst_list;
    return codegen_emit_finalize_call(inst_list, ctx, base_label,
                                      base_rbp_offset, extra_offset);
  }
  if (type->kind == TYPE_KIND_RECORD) {
    struct RecordType *rec = type->info.record_info;
    if (rec != NULL && codegen_record_contains_dynarray(rec)) {
      return codegen_emit_dynarray_cleanup_record_fields(
          inst_list, ctx, rec, base_label, base_rbp_offset, extra_offset,
          depth + 1);
    }
    return inst_list;
  }
  if (type->kind == TYPE_KIND_ARRAY) {
    ...
    for (long long i = 0; i < emit_count; ++i) {
      inst_list = codegen_emit_dynarray_cleanup_for_type(
          inst_list, ctx, element_type, base_label, base_rbp_offset,
...
static ListNode_t *codegen_emit_dynarray_cleanup_for_var(ListNode_t *inst_list,
  ...
  if (slot == NULL && symtab != NULL) {
    char *storage_key =
        codegen_var_storage_key(ctx, symtab, var_decl, var_name);
    if (storage_key != NULL) {
  ...
  return codegen_emit_dynarray_cleanup_for_type(inst_list, ctx, type, NULL,
                                                base_rbp_offset, 0, 0);
}
...
static ListNode_t *codegen_emit_dynarray_cleanup_for_global_var(
  ...
  const char *asm_label =
      (slot->static_label != NULL) ? slot->static_label : var_name;

  return codegen_emit_dynarray_cleanup_for_type(inst_list, ctx, type, asm_label,
                                                0, 0, 0);
}
  ...
  for (ListNode_t *cur = declarations; cur != NULL; cur = cur->next) {
    ...
    if (tree->type == TREE_VAR_DECL) {
      ...
      for (ListNode_t *id_node = tree->tree_data.var_decl_data.ids;
        ...
        if (id_node->cur == NULL)
          continue;
        inst_list = codegen_emit_dynarray_cleanup_for_var(
            inst_list, ctx, symtab, tree, (const char *)id_node->cur);
      }
    } else if (tree->type == TREE_ARR_DECL) {
