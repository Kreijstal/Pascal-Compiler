...
long long expr_get_array_element_size(const struct Expression *expr, CodeGenContext *ctx)
{
    ...
    if (expr->type == EXPR_POINTER_DEREF)
    {
        ...
        {
            struct RecordField *field = codegen_expr_lookup_record_field((struct Expression *)lookup_expr, ctx);
            if (field != NULL)
            {
                {
...
struct RecordField *codegen_expr_lookup_record_field(struct Expression *record_access_expr,
    CodeGenContext *ctx)
{
...
Register_t *codegen_try_get_reg(ListNode_t **inst_list, CodeGenContext *ctx, const char *usage)
{
