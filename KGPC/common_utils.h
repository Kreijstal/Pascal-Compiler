 ...
 * @return 1 if all pointers are non-NULL, 0 otherwise
 */
static inline int ensure_non_null_3(const void *a, const void *b,
                                    const void *c) {
 ...
 * @return 1 if all pointers are non-NULL, 0 otherwise
 */
static inline int ensure_non_null_4(const void *a, const void *b, const void *c,
                                    const void *d) {
 ...
 */
#define SAFE_FREE_WITH(ptr, destructor)                                        \
  do {                                                                         \
    if ((ptr) != NULL) {                                                       \
      destructor(ptr);                                                         \
      (ptr) = NULL;                                                            \
