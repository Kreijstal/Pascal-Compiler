 ...
 * auto-clears the pointer if it currently refers to the context being destroyed,
 * ensuring passes that call compilation_context_get_active() cannot reach stale
                                            ...
                                            const char *const *files, int count)
{
    for (int i = 0; i < count; ++i)
    {
        if (ctx->include_file_count == ctx->include_file_capacity)
        {
            ...
            if (new_arr == NULL)
            {
                fprintf(stderr, "ERROR: Out of memory in compilation_context_add_include_files\n");
                return false;
            }
