...
static void print_usage(const char *prog_name)
{
    ...
    fprintf(out, "    --dump-ir-after=cfg      Dump control-flow graph to stderr after each function\n");
    fprintf(out, "    --dump-ir-after=liveness Dump live-in/live-out sets to stderr after each function\n");
    fprintf(out, "    --dump-ir-after=vregs    Dump virtual register ID assignments to stderr after each function\n");
    ...
    while (count > 0)
    {
        ...
        else if (strncmp(arg, "--dump-ir-after=", 16) == 0)
        {
            ...
            else if (strcmp(phase, "liveness") == 0)
            {
                ...
                fprintf(stderr, "IR dump after liveness enabled (output to stderr).\n\n");
            }
            else if (strcmp(phase, "vregs") == 0)
            {
                set_dump_ir_vregs_flag();
                fprintf(stderr, "IR dump after vregs enabled (output to stderr).\n\n");
            }
            else
            {
                fprintf(stderr, "WARNING: Unknown --dump-ir-after phase '%s' (supported: def-use, cfg, liveness, vregs)\n", phase);
            }
        }
