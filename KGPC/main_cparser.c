...
static char
    g_codegen_cache_obj_path[4096]; /* path to the cached .o used/created */
static char g_codegen_cache_asm_path[4096]; /* path to the cache .s (unit
                                               functions only) */
...
/* Ensure program-defined subprograms are emitted even if reachability misses
 * them */
    ...
    if (cur->type == LIST_TREE && cur->cur != NULL) {
      ...
      if (sub->type == TREE_SUBPROGRAM) {
        ...
        if (sub->tree_data.subprogram_data.subprograms != NULL) {
          ...
          wrapper.tree_data.program_data.subprograms =
              sub->tree_data.subprogram_data.subprograms;
...
static void print_usage(const char *prog_name) {
  ...
  fprintf(out, "    -non-local            Enable non-local variable chasing "
               "(experimental)\n");
  fprintf(
      out,
  ...
  fprintf(out, "    --dump-ir-after=cfg      Dump control-flow graph to stderr "
               "after each function\n");
  fprintf(out, "    --dump-ir-after=liveness Dump live-in/live-out sets to "
               "stderr after each function\n");
  fprintf(out, "    --time-passes         Print timing information for major "
               "compiler stages\n");
  fprintf(out, "    --asm-debug           Annotate emitted assembly with "
               "semantic/codegen info\n");
  fprintf(out,
          "    --disable-dce         Emit unused subprograms (debugging)\n");
  fprintf(out, "    -I<path>              Add include path for preprocessor\n");
  fprintf(out,
          "    -Fu<path>             Add unit search path (FPC compatible)\n");
  fprintf(out,
          "    --no-vendor-units     Disable built-in KGPC vendor units\n");
  fprintf(out, "    --no-stdlib           Disable KGPC stdlib; load minimal "
               "prelude instead\n");
  fprintf(out, "    --pp-cache-dir=<dir>  Cache parsed unit ASTs to <dir> for "
               "faster re-compilation\n");
  fprintf(out, "    -D<symbol>[=<value>]  Define preprocessor symbol\n");
  fprintf(out,
          "    -Us                   Compile System unit (FPC compatible)\n");
  fprintf(
      out,
      "    -Sg                   Enable goto statements (FPC compatible)\n");
}
...
static ssize_t get_executable_path(char *buffer, size_t size,
                                   const char *argv0) {
...
static char *resolve_stdlib_path(const char *argv0) {
  ...
  if (source_root != NULL) {
    ...
    int written = snprintf(candidate, sizeof(candidate),
                           "%s/KGPC/Units/system.p", source_root);
  ...
  if (len > 0) {
    ...
    if (dir != NULL) {
      ...
      for (size_t i = 0;
           i < sizeof(relative_candidates) / sizeof(relative_candidates[0]);
           ++i) {
        char candidate[PATH_MAX];
        int written = snprintf(candidate, sizeof(candidate), "%s/%s", dir,
                               relative_candidates[i]);
        if (written > 0 && written < (int)sizeof(candidate) &&
            access(candidate, R_OK) == 0) {
          char *dup = strdup(candidate);
          if (dup != NULL)
...
static char *resolve_prelude_path(const char *argv0) {
  ...
  if (source_root != NULL) {
    char candidate[PATH_MAX];
    int written = snprintf(candidate, sizeof(candidate),
                           "%s/KGPC/Units/prelude.p", source_root);
    if (written > 0 && written < (int)sizeof(candidate) &&
        access(candidate, R_OK) == 0) {
      char *dup = strdup(candidate);
      if (dup != NULL)
...
static char *build_default_ast_cache_dir(const char *argv0) {
  ...
  *last_sep = '\0';
  ...
  snprintf(signature_input, sizeof(signature_input), "%s|%lld|%lld.%09ld",
           compiler_path, (long long)st.st_size,
           (long long)compiler_mtime.tv_sec, (long)compiler_mtime.tv_nsec);
  uint64_t sig = fnv1a64_bytes((const unsigned char *)signature_input,
                               strlen(signature_input));

  size_t needed =
      strlen(compiler_path) + 1 + strlen("kgpc_ast_cache_") + 16 + 1;
  char *dir = (char *)malloc(needed);
  if (dir == NULL)
    return NULL;

  snprintf(dir, needed, "%s/kgpc_ast_cache_%016llx", compiler_path,
...
static SetFlagsResult set_flags(char **optional_args, int count) {
  ...
  while (count > 0) {
    ...
    if (strcmp(arg, "-non-local") == 0) {
      ...
      fprintf(
          stderr,
          "WARNING: Non-local is still in development and is very buggy!\n\n");
      set_nonlocal_flag();
      ...
      set_o2_flag();
               ...
               strcmp(arg, "--parse-only") == 0) {
      fprintf(stderr, "Parse-only mode enabled.\n\n");
      set_parse_only_flag();
    } else if (strcmp(arg, "--emit-link-args") == 0) {
      g_emit_link_args = 1;
    } else if (strcmp(arg, "--target-windows") == 0 ||
               strcmp(arg, "-target-windows") == 0 ||
               strcmp(arg, "--windows-abi") == 0) {
      fprintf(stderr, "Target ABI: Windows x64\n\n");
      fflush(stderr);
      set_target_windows_flag();
    } else if (strcmp(arg, "--target-sysv") == 0 ||
               strcmp(arg, "-target-sysv") == 0 ||
               strcmp(arg, "--sysv-abi") == 0) {
      ...
      if (strcasecmp(value, "windows") == 0 ||
          strcasecmp(value, "win64") == 0) {
        fprintf(stderr, "Target ABI: Windows x64\n\n");
        fflush(stderr);
        set_target_windows_flag();
      } else if (strcasecmp(value, "sysv") == 0 ||
                 strcasecmp(value, "systemv") == 0 ||
    ...
    } else if (strcmp(arg, "--asm-debug") == 0 ||
               strcmp(arg, "--asm-debug-comments") == 0) {
      fprintf(stderr, "Assembly debug comments enabled.\n\n");
      set_asm_debug_flag();
    } else if (strcmp(arg, "--disable-dce") == 0 ||
               strcmp(arg, "--no-dce") == 0) {
      fprintf(stderr,
              "Dead-code elimination disabled (emit unused subprograms).\n\n");
      ...
      if (strcmp(phase, "def-use") == 0) {
        set_dump_ir_flag();
        fprintf(stderr,
                "IR dump after def-use enabled (output to stderr).\n\n");
      } else if (strcmp(phase, "cfg") == 0) {
        set_dump_ir_cfg_flag();
        fprintf(stderr, "IR dump after cfg enabled (output to stderr).\n\n");
      } else if (strcmp(phase, "liveness") == 0) {
        set_dump_ir_liveness_flag();
        fprintf(stderr,
                "IR dump after liveness enabled (output to stderr).\n\n");
      } else {
        fprintf(stderr,
                "WARNING: Unknown --dump-ir-after phase '%s' (supported: "
    ...
    } else if (strcmp(arg, "--no-stdlib") == 0 ||
               strcmp(arg, "--no-prelude") == 0) {
      g_skip_stdlib = true;
...
static bool parse_pascal_file(const char *path, Tree_t **out_tree,
                              bool convert_to_tree) {
  Tree_t *tree = NULL;
...
static bool unit_set_add(UnitSet *set, char *name) {
  ...
  if (set->count == set->capacity) {
    ...
    char **new_names =
        (char **)realloc(set->names, new_capacity * sizeof(char *));
    if (new_names == NULL) {
      free(name);
      return false;
    }
        ...
        while (arg != NULL) {
          if (arg->type == LIST_TREE && arg->cur != NULL) {
            ...
            if (arg_tree->type == TREE_VAR_DECL) {
              ...
              if (unit_index > 0 &&
                  arg_tree->tree_data.var_decl_data.source_unit_index == 0)
                arg_tree->tree_data.var_decl_data.source_unit_index =
                    unit_index;
            } else if (arg_tree->type == TREE_ARR_DECL) {
              arg_tree->tree_data.arr_decl_data.defined_in_unit = 1;
              if (unit_index > 0 &&
                  arg_tree->tree_data.arr_decl_data.source_unit_index == 0)
                arg_tree->tree_data.arr_decl_data.source_unit_index =
                    unit_index;
            }
          }
          arg = arg->next;
  ...
  while (node != NULL) {
    if (node->type == LIST_TREE && node->cur != NULL) {
      ...
      if (decl->type == TREE_TYPE_DECL) {
        ...
        if (unit_index > 0 &&
            decl->tree_data.type_decl_data.source_unit_index == 0)
        ...
        if (unit_index > 0 &&
          ...
          decl->tree_data.type_decl_data.info.record->source_unit_index =
              unit_index;
      }
    }
    node = node->next;
  ...
  while (node != NULL) {
    if (node->type == LIST_TREE && node->cur != NULL) {
      ...
      if (decl->type == TREE_CONST_DECL) {
        ...
        if (unit_index > 0 &&
            decl->tree_data.const_decl_data.source_unit_index == 0)
  ...
  while (node != NULL) {
    if (node->type == LIST_TREE && node->cur != NULL) {
      ...
      if (decl->type == TREE_VAR_DECL) {
        ...
        if (unit_index > 0 &&
            decl->tree_data.var_decl_data.source_unit_index == 0)
        ...
        if (unit_index > 0 &&
            decl->tree_data.arr_decl_data.source_unit_index == 0)
...
static ListNode_t *merge_unit_type_decls_before_locals(ListNode_t *head,
                                                       ListNode_t *unit_types) {
  if (unit_types == NULL)
    return head;
  if (head == NULL)
    return unit_types;
...
/* Detach and return only public interface subprogram declarations from a unit
 * subprogram list. Non-public implementation subprogram nodes remain in *list
 * for the caller to dispose with the unit tree. */
...
static void build_combined_program_view(CompilationContext *comp_ctx) {
  ...
  for (int i = 0; i < comp_ctx->loaded_unit_count; ++i) {
    ...
    *type_list = merge_unit_type_decls_before_locals(
        *type_list, unit_tree->tree_data.unit_data.interface_type_decls);
    ...
    if (is_program) {
      ...
      *type_list = merge_unit_type_decls_before_locals(
          *type_list, unit_tree->tree_data.unit_data.implementation_type_decls);
      ...
      while (dbg != NULL) {
        if (dbg->type == LIST_TREE) {
          ...
          if (decl != NULL && decl->type == TREE_CONST_DECL &&
            ...
            fprintf(stderr,
                    "[KGPC] merging interface const '%s' from unit '%s'\n",
    ...
    *const_list = ConcatList(
        *const_list, unit_tree->tree_data.unit_data.interface_const_decls);
    unit_tree->tree_data.unit_data.interface_const_decls = NULL;

    if (is_program) {
      *const_list =
          ConcatList(*const_list,
    ...
    *var_list = ConcatList(*var_list,
    ...
    if (is_program) {
      *var_list = ConcatList(
          *var_list, unit_tree->tree_data.unit_data.implementation_var_decls);
    ...
    if (is_program) {
      *sub_list =
          ConcatList(*sub_list, unit_tree->tree_data.unit_data.subprograms);
...
static int get_subprogram_unit_index(const Tree_t *t) {
  if (t == NULL || t->type != TREE_SUBPROGRAM)
    return 0;
...
static void unbuild_combined_program_view(CompilationContext *comp_ctx) {
  ...
  for (int i = 0; i < comp_ctx->loaded_unit_count; ++i) {
    ...
    ListNode_t *unit_types =
        extract_unit_nodes(type_list, unit_idx, get_type_decl_unit_index);
    unit_tree->tree_data.unit_data.interface_type_decls = unit_types;

    /* Extract const declarations */
    ListNode_t *unit_consts =
        extract_unit_nodes(const_list, unit_idx, get_const_decl_unit_index);
    unit_tree->tree_data.unit_data.interface_const_decls = unit_consts;

    /* Extract var declarations */
    ListNode_t *unit_vars =
        extract_unit_nodes(var_list, unit_idx, get_var_decl_unit_index);
     ...
     * extracted only the *public* subprograms via
     * extract_public_unit_subprograms, leaving the non-public ones still
     * attached to unit_tree->subprograms. Concatenate the round-tripped public
     * subs back onto that residual list — overwriting it would orphan the
     * private impl-only subprograms (and their nested type_decls + records). */
    ListNode_t *unit_subs =
        extract_unit_nodes(sub_list, unit_idx, get_subprogram_unit_index);
    unit_tree->tree_data.unit_data.subprograms =
...
static void load_unit(CompilationContext *comp_ctx, const char *unit_name,
  ...
  if (preloaded != NULL) {
    ...
    (void)semcheck_register_source_buffer(preloaded->path, preloaded->pp_source,
                                          preloaded->pp_length);
  ...
  if (unit_tree == NULL) {
    fprintf(stderr,
            "ERROR: Failed to convert unit '%s' into legacy parse tree.\n",
            unit_name);
    exit(1);
  }
  ...
  /* Record dependency edges: this unit depends on each unit it uses */
  {
    ...
    ListNode_t *dep;
    for (dep = unit_tree->tree_data.unit_data.interface_uses; dep != NULL;
         dep = dep->next) {
      if (dep->type == LIST_STRING && dep->cur != NULL)
        unit_registry_add_iface_dep(this_idx,
                                    unit_registry_add((const char *)dep->cur));
    }
    for (dep = unit_tree->tree_data.unit_data.implementation_uses; dep != NULL;
         dep = dep->next) {
      if (dep->type == LIST_STRING && dep->cur != NULL)
        unit_registry_add_dep(this_idx,
                              unit_registry_add((const char *)dep->cur));
    }
   ...
   * These functions just set flags on AST nodes, they don't touch the symbol
   * table. */
  {
    ...
    mark_unit_type_decls(unit_tree->tree_data.unit_data.interface_type_decls, 1,
                         unit_idx);
    mark_unit_const_decls(unit_tree->tree_data.unit_data.interface_const_decls,
                          1, unit_idx);
    mark_unit_var_decls(unit_tree->tree_data.unit_data.interface_var_decls, 1,
                        unit_idx);
    if (program->type == TREE_PROGRAM_TYPE) {
      mark_unit_type_decls(
          unit_tree->tree_data.unit_data.implementation_type_decls, 0,
          unit_idx);
      mark_unit_const_decls(
          unit_tree->tree_data.unit_data.implementation_const_decls, 0,
          unit_idx);
      mark_unit_var_decls(
          unit_tree->tree_data.unit_data.implementation_var_decls, 0, unit_idx);
...
static int compile_single_program(const char *input_file,
                                  const char *output_file, Tree_t *prelude_tree,
                                  char *prelude_path, bool use_stdlib,
                                  bool convert_to_tree, arena_t *arena,
                                  char **argv);
...
static int batch_mode_main(int argc, char **argv) {
  int max_parallel =
      g_batch_max_parallel > 0 ? g_batch_max_parallel : get_nproc();
  ...
  while (fgets(line, (int)sizeof(line), stdin) != NULL) {
    ...
    if (pid == 0) {
      ...
      int result = compile_single_program(batch_input, batch_output,
                                          prelude_tree, prelude_path,
                                          use_stdlib, true, child_arena, argv);
...
static int compile_single_program(const char *input_file,
  ...
  emit_profile_stage("parse user source",
                     current_time_seconds() - user_profile_start);
  ...
  if (preprocessed_source != NULL && preprocessed_length > 0) {
    ...
    if (saved_preprocessed_source != NULL) {
      memcpy(saved_preprocessed_source, preprocessed_source,
             preprocessed_length);
  ...
  if (user_tree->type == TREE_UNIT) {
    fprintf(stderr,
            "[batch] Batch mode does not support unit compilation (%s)\n",
            input_file);
    destroy_tree(user_tree);
  ...
  if (prelude_tree != NULL) {
    ...
    if (prelude_types != NULL) {
      ...
      user_tree->tree_data.program_data.type_declaration = ConcatList(
          prelude_types, user_tree->tree_data.program_data.type_declaration);
    ...
    if (prelude_consts != NULL) {
      ...
      user_tree->tree_data.program_data.const_declaration = ConcatList(
          prelude_consts, user_tree->tree_data.program_data.const_declaration);
    ...
    if (prelude_vars != NULL) {
      ...
      user_tree->tree_data.program_data.var_declaration = ConcatList(
          prelude_vars, user_tree->tree_data.program_data.var_declaration);
      clear_prelude_var_decls(prelude_tree);
    }

    {
      int sys_idx = unit_registry_add("System");
      semcheck_predeclare_program_into_unit_scope(early_symtab, user_tree,
                                                  sys_idx);
    }

    load_prelude_uses(&g_comp_ctx, prelude_tree, &visited_units);
    emit_profile_stage("program: merge prelude and prelude uses",
                       current_time_seconds() - prelude_merge_start);
  }
  if (!use_stdlib) {
  ...
  if (pascal_frontend_is_objfpc_mode()) {
    ...
    emit_profile_stage("program: auto-load ObjPas",
                       current_time_seconds() - objpas_start);
  }
  ...
  double combined_view_start =
      profile_pipeline_flag() ? current_time_seconds() : 0.0;
  build_combined_program_view(&g_comp_ctx);
  emit_profile_stage("program: build combined view from loaded units",
                     current_time_seconds() - combined_view_start);
  compilation_context_set_active(&g_comp_ctx);

  double merge_user_start =
      profile_pipeline_flag() ? current_time_seconds() : 0.0;
  user_tree->tree_data.program_data.type_declaration = ConcatList(
      user_tree->tree_data.program_data.type_declaration, user_types);
  user_tree->tree_data.program_data.const_declaration = ConcatList(
      user_tree->tree_data.program_data.const_declaration, user_consts);
  user_tree->tree_data.program_data.var_declaration =
      ConcatList(user_tree->tree_data.program_data.var_declaration, user_vars);
  emit_profile_stage("program: merge user declarations",
                     current_time_seconds() - merge_user_start);
  double generic_alias_start =
      profile_pipeline_flag() ? current_time_seconds() : 0.0;
  resolve_pending_generic_aliases(user_tree);
  emit_profile_stage("program: resolve generic aliases",
                     current_time_seconds() - generic_alias_start);
  from_cparser_resolve_deferred_arrays(user_tree);
  double generic_method_start =
      profile_pipeline_flag() ? current_time_seconds() : 0.0;
  append_generic_method_clones(user_tree);
  emit_profile_stage("program: append generic method clones",
                     current_time_seconds() - generic_method_start);
  double generic_sub_start =
      profile_pipeline_flag() ? current_time_seconds() : 0.0;
  resolve_pending_generic_subprograms(user_tree);
  emit_profile_stage("program: resolve generic subprograms",
                     current_time_seconds() - generic_sub_start);
  ...
  int sem_result = 0;
  double sem_profile_start =
      profile_pipeline_flag() ? current_time_seconds() : 0.0;
  SymTab_t *symtab =
      start_semcheck_with_symtab(early_symtab, user_tree, &sem_result);
  emit_profile_stage("program: semantic analysis",
                     current_time_seconds() - sem_profile_start);
  ...
  if (sem_result <= 0) {
    ...
    double mark_used_start =
        profile_pipeline_flag() ? current_time_seconds() : 0.0;
    mark_used_functions(user_tree, symtab);
    emit_profile_stage("program: mark used functions (pass 1)",
                       current_time_seconds() - mark_used_start);
    double mark_program_subs_start =
        profile_pipeline_flag() ? current_time_seconds() : 0.0;
    mark_program_subs_used(user_tree);
    emit_profile_stage("program: mark program subprograms",
                       current_time_seconds() - mark_program_subs_start);
    double mark_used_second_start =
        profile_pipeline_flag() ? current_time_seconds() : 0.0;
    mark_used_functions(user_tree, symtab);
    emit_profile_stage("program: mark used functions (pass 2)",
                       current_time_seconds() - mark_used_second_start);

    double unbuild_start =
        profile_pipeline_flag() ? current_time_seconds() : 0.0;
    unbuild_combined_program_view(&g_comp_ctx);
    emit_profile_stage("program: unbuild combined view for codegen",
                       current_time_seconds() - unbuild_start);

    double codegen_profile_start =
        profile_pipeline_flag() ? current_time_seconds() : 0.0;
    codegen(user_tree, input_file, &ctx, symtab, &g_comp_ctx);
    emit_profile_stage("program: code generation",
                       current_time_seconds() - codegen_profile_start);
    ...
    if (codegen_failed && !codegen_cache_miss_flag()) {
      ...
      if (codegen_failed)
        fprintf(stderr, "WARNING: codegen had errors; keeping output for cache "
                        "(--gc-sections will discard broken sections).\n");
      /* On cache miss, assemble the unit functions .s into the cache .o */
      if (!codegen_failed)
        codegen_cache_populate(g_codegen_cache_asm_path);
      emit_link_args();
    }
    /* Clear transient cache-mode flags so they don't leak into later
     * compilations. */
  ...
  emit_profile_stage("total pipeline",
                     current_time_seconds() - pipeline_total_start);
  return exit_code;
}
...
static void codegen_cache_compute_key(const char *input_file, char *key_buf,
                                      size_t key_buf_size) {
   ...
   * reuse object code generated by a previous compiler built in the same
   * second. */
  {
    char exe_path[PATH_MAX];
...
static int codegen_cache_patch_broken_sections(const char *cache_asm_file,
  ...
  /* A section starts at ".section\t.text.NAME" and ends at the next section
   * directive */
  ...
  for (long i = 0; i < fsize;) {
    ...
    if (strncmp(content + i, "\t.section\t.text.", 16) == 0) {
      ...
      size_t name_len = comma ? (size_t)(comma - name_start)
                              : (size_t)(eol - (long)(name_start - content));
      if (name_len >= 256)
        name_len = 255;
  ...
  for (long i = 0; i < fsize;) {
    ...
    for (int s = 0; s < num_sections; s++) {
      if (cur_line >= sections[s].start_line &&
          cur_line <= sections[s].end_line && sections[s].broken) {
        in_broken = 1;
        broken_idx = s;
        break;
      }
    ...
    if (in_broken && cur_line == sections[broken_idx].start_line) {
      ...
      fprintf(f,
              ...
              "%s:\n"
              "\tud2\n",
              sections[broken_idx].name, sections[broken_idx].name,
...
/* After a cache-miss compile, assemble the unit-functions-only .s into the
 * cache .o. The cache_asm_file contains only unit subprogram .text.* sections
 * (written by codegen via ctx->cache_output).  No main, no vars, no
 * init/finalization. If assembly fails due to broken inline asm, those sections
 * are replaced with ud2 stubs and assembly is retried.  --gc-sections removes
 * them at link time. */
static void codegen_cache_populate(const char *cache_asm_file) {
  ...
  char cmd[8500];
  snprintf(cmd, sizeof(cmd), "as '%s' -o '%s' 2>'%s'", cache_asm_file,
           tmp_obj_file, err_file);
  int rc = system(cmd);
  ...
  if (rc != 0) {
    ...
    if (codegen_cache_patch_broken_sections(cache_asm_file, err_buf)) {
      ...
      snprintf(cmd, sizeof(cmd), "as '%s' -o '%s' 2>/dev/null", cache_asm_file,
               tmp_obj_file);
      rc = system(cmd);
    }
    ...
    if (rc != 0) {
      fprintf(
          stderr,
          "WARNING: codegen cache: as failed (rc=%d), cache not populated\n",
          rc);
      remove(tmp_obj_file);
...
static void emit_link_args(void) {
  ...
  if (g_codegen_cache_hit && g_codegen_cache_obj_path[0] != '\0') {
    used += (size_t)snprintf(buffer + used, sizeof(buffer) - used,
                             " %s -Wl,--gc-sections", g_codegen_cache_obj_path);
...
int main(int argc, char **argv) {
  ...
  /* Process ALL arguments through set_flags first, then extract positional
   * args. This allows flags like --no-stdlib to appear anywhere in the command
   * line. */
  ...
  if (prelude_path == NULL) {
    fprintf(stderr,
            "Error: Unable to locate %s. Set KGPC_STDLIB or run from the "
            "project root.\n",
            use_stdlib ? "system.p" : "prelude.p");
    clear_dump_ast_path();
    unit_search_paths_destroy(&g_unit_paths);
  ...
  bool track_time = time_passes_flag();
  {
    ...
    double stdlib_profile_start =
        profile_pipeline_flag() ? current_time_seconds() : 0.0;
    bool parsed_stdlib =
        parse_pascal_file(prelude_path, &prelude_tree, convert_to_tree);
    if (track_time) {
      g_time_parse_stdlib += current_time_seconds() - stdlib_start;
      ++g_count_parse_stdlib;
    }
    emit_profile_stage(use_stdlib ? "parse stdlib/prelude" : "parse prelude",
                       current_time_seconds() - stdlib_profile_start);
    if (!parsed_stdlib) {
      destroy_tree(prelude_tree);
      free(prelude_path);
      clear_dump_ast_path();
  ...
  if (parse_only) {
    ...
    fprintf(
        stderr,
        "Parse-only mode: skipping semantic analysis and code generation.\n");
  ...
  if (user_tree->type == TREE_UNIT) {
    ...
    if (prelude_tree != NULL) {
      /* For units, we still need to merge prelude for built-in functions and
       * types */
      ...
      /* Skip merging prelude types/consts/vars when compiling the System unit
       * itself, since System defines its own core types and constants. */
      int is_system_unit = pascal_identifier_equals(
          user_tree->tree_data.unit_data.unit_id, "System");

      /* Merge prelude types into unit for FPC compatibility (SizeInt,
       * PAnsiChar, etc.). For System itself, keep prelude types unmarked so
       * real System declarations take precedence while still providing missing
       * core aliases (e.g. Currency). */
      ListNode_t *prelude_types = get_prelude_type_decls(prelude_tree);
      if (prelude_types != NULL) {
        if (!is_system_unit)
          mark_unit_type_decls(prelude_types, 1, unit_registry_add("System"));
        user_tree->tree_data.unit_data.interface_type_decls = ConcatList(
            prelude_types, user_tree->tree_data.unit_data.interface_type_decls);
        clear_prelude_type_decls(prelude_tree);
      }

      /* Merge prelude constants into unit for FPC compatibility (fmClosed,
       * fmInput, etc.) */
      ListNode_t *prelude_consts = get_prelude_const_decls(prelude_tree);
      if (prelude_consts != NULL && !is_system_unit) {
        mark_unit_const_decls(
            prelude_consts, 1,
            unit_registry_add("System")); /* Mark as exported from unit */
        user_tree->tree_data.unit_data.interface_const_decls =
            ConcatList(prelude_consts,
                       user_tree->tree_data.unit_data.interface_const_decls);
        clear_prelude_const_decls(prelude_tree);
      }
      ...
      if (prelude_vars != NULL && !is_system_unit) {
        mark_unit_var_decls(
            prelude_vars, 1,
            unit_registry_add("System")); /* Mark as exported from unit */
        user_tree->tree_data.unit_data.interface_var_decls = ConcatList(
            prelude_vars, user_tree->tree_data.unit_data.interface_var_decls);
        clear_prelude_var_decls(prelude_tree);
      }
    }
    ...
    if (!use_stdlib && !pascal_identifier_equals(
                           user_tree->tree_data.unit_data.unit_id, "System")) {
      double system_load_start =
          profile_pipeline_flag() ? current_time_seconds() : 0.0;
    ...
    if (pascal_frontend_is_objfpc_mode()) {
      ...
      emit_profile_stage("unit compile: auto-load ObjPas",
                         current_time_seconds() - objpas_start);
    }
    ...
    {
      int target_idx =
          unit_registry_add(user_tree->tree_data.unit_data.unit_id);
      ListNode_t *dep;
      for (dep = user_tree->tree_data.unit_data.interface_uses; dep != NULL;
           dep = dep->next) {
        if (dep->type == LIST_STRING && dep->cur != NULL)
          unit_registry_add_iface_dep(
              target_idx, unit_registry_add((const char *)dep->cur));
      }
      for (dep = user_tree->tree_data.unit_data.implementation_uses;
           dep != NULL; dep = dep->next) {
        if (dep->type == LIST_STRING && dep->cur != NULL)
          unit_registry_add_dep(target_idx,
                                unit_registry_add((const char *)dep->cur));
      }
      unit_registry_add_iface_dep(target_idx, unit_registry_add("System"));
    }

    emit_profile_stage("unit compile: total imports",
                       current_time_seconds() - unit_import_start);
    unit_set_destroy(&visited_units);
    ...
    emit_profile_stage("unit compile: semantic analysis",
                       current_time_seconds() - sem_profile_start);
    ...
    emit_profile_stage("unit compile: code generation",
                       current_time_seconds() - codegen_profile_start);
  ...
  if (prelude_tree != NULL) {
    ...
    /* Merge prelude types into program for FPC compatibility (SizeInt,
     * PAnsiChar, etc.) */
    ListNode_t *prelude_types = get_prelude_type_decls(prelude_tree);
    if (prelude_types != NULL) {
      mark_unit_type_decls(prelude_types, 1, unit_registry_add("System"));
  ...
  if (sem_result <= 0) {
    ...
    emit_profile_stage("program: mark program subprograms",
                       current_time_seconds() - mark_program_subs_start);
    /* Run mark_used again to discover functions called by newly-marked
     * subprograms (e.g., inherited methods in specialized generics) */
    double mark_used_second_start =
        profile_pipeline_flag() ? current_time_seconds() : 0.0;
    mark_used_functions(user_tree, symtab);
    emit_profile_stage("program: mark used functions (pass 2)",
