...
static bool ast_cache_is_fresh(const char *cache_path,
...
static int detect_shortstring_in_buffer(const char *buffer, size_t length,
                                        bool *value_out) {
...
static char *resolve_include_for_h_detection(const char *source_path,
                                             const char *inc_name) {
...
static int detect_shortstring_default(const char *buffer, size_t length,
  ...
  while (pos < end) {
    if (*pos == '{' && (pos + 1) < end && pos[1] == '$') {
      ...
      if (dir_len >= 2 && (dir_start[0] == 'i' || dir_start[0] == 'I') &&
        ...
        if (fname_start < dir_end && *fname_start != '+' &&
            *fname_start != '-') {
          ...
          if (fname_len > 0 && fname_len < 256) {
            ...
            char *inc_path =
                resolve_include_for_h_detection(source_path, inc_name);
            if (inc_path != NULL) {
...
bool pascal_frontend_is_objfpc_mode(void) { return g_objfpc_mode_detected; }
...
void pascal_frontend_set_objfpc_mode(void) { g_objfpc_mode_detected = true; }

bool pascal_frontend_default_shortstring(void) { return g_default_shortstring; }
...
void pascal_frontend_set_ast_cache_dir(const char *dir) {
  ...
  if (g_ast_cache_dir != NULL && kgpc_mkdir(g_ast_cache_dir, 0775) != 0) {
    ...
    if (stat(g_ast_cache_dir, &st) != 0 || !S_ISDIR(st.st_mode)) {
      fprintf(stderr,
              "Warning: AST cache directory '%s' is not usable; disabling AST "
              "cache.\n",
              g_ast_cache_dir);
      free(g_ast_cache_dir);
...
static char *read_file(const char *path, size_t *out_len) {
...

  {
    ...
    if (conflict_line > 0) {
      fprintf(stderr, "ERROR: Merge conflict marker found in %s at line %d\n",
              path, conflict_line);
      free(buffer);
      return NULL;
    }
...
static void set_preprocessed_context(const char *buffer, size_t length,
                                     const char *path) {
  ...
  if (length >= 3 && ubytes[0] == 0xEF && ubytes[1] == 0xBB &&
      ubytes[2] == 0xBF) {
    buffer += 3;
    length -= 3;
  }
...
static const char *skip_utf8_bom(const char *cursor, const char *end) {
  if (end - cursor >= 3 && (unsigned char)cursor[0] == 0xEF &&
...
static const char *skip_whitespace_and_comments(const char *cursor,
                                                const char *end) {
  while (cursor < end) {
...
static ParseError *create_preprocessor_error(const char *path,
                                             const char *detail) {
  ...
  /* parser_name is a borrowed/static reference per free_error contract — do not
   * strdup. */
...
static void report_preprocessor_error(ParseError **error_out, const char *path,
                                      const char *detail) {
...
static char *compute_ast_cache_path(const char *source_path) {
  ...
  uint64_t hash =
      fnv1a64_bytes((const unsigned char *)source_path, strlen(source_path));
  ...
  if (cache_path == NULL)
    return NULL;
  snprintf(cache_path, total, "%s/%016llx.ast_cache", g_ast_cache_dir,
...
static bool path_is_compiler_prelude(const char *path) {
  ...
  return strstr(path, "KGPC/Units/") != NULL ||
         strstr(path, "KGPC\\Units\\") != NULL;
}
...
bool pascal_parse_source(const char *path, bool convert_to_tree,
                         Tree_t **out_tree, ParseError **error_out) {
  if (error_out != NULL)
    *error_out = NULL;
  ...
  /* --- AST cache: try loading a cached binary AST to skip
  ...
  if (cache_units_only && g_ast_cache_reads_enabled) {
    ...
    if (cache_path != NULL) {
      ...
      if (ast_cache_is_fresh(cache_path, path) &&
          ast_cache_load(cache_path, &cached_ast, &cached_pp_buf,
                         &cached_pp_len)) {
        free(cache_path);
        /* Set up preprocessed source context (needed for semcheck error
         * reporting) */
         ...
         * in from_cparser.c sees the correct flag. */
        {
          ...
          if (raw_buf != NULL) {
            detect_shortstring_default(raw_buf, raw_len, &g_default_shortstring,
                                       path);
            free(raw_buf);
          }
        }
  ...
  if (preprocessor == NULL) {
    report_preprocessor_error(error_out, path,
                              "unable to initialise preprocessor");
    free(buffer);
    return false;
  }
  ...
  for (int i = 0; i < g_user_include_path_count; ++i) {
    if (g_user_include_paths[i] != NULL) {
      pascal_preprocessor_add_include_path(preprocessor,
                                           g_user_include_paths[i]);
    }
  }
  ...
  const char *default_symbols[] = {
      ...
      "FPC_RELEASE := 2", "FPC_STACKALIGNMENT := 16",
      // FPC feature flags
      "FPC_HAS_FEATURE_SUPPORT", "FPC_HAS_FEATURE_DYNARRAYS",
      "FPC_HAS_FEATURE_ANSISTRINGS", "FPC_HAS_FEATURE_WIDESTRINGS",
      "FPC_HAS_FEATURE_UNICODESTRINGS", "FPC_HAS_UNICODESTRING",
      "FPC_HAS_FEATURE_CLASSES", "FPC_HAS_FEATURE_OBJECTS",
      "FPC_HAS_FEATURE_EXCEPTIONS", "FPC_HAS_FEATURE_RTTI",
      "FPC_HAS_FEATURE_HEAP", "FPC_HAS_FEATURE_TEXTIO",
      "FPC_HAS_FEATURE_FILEIO", "FPC_HAS_FEATURE_CONSOLEIO",
      "FPC_HAS_FEATURE_RANDOM", "FPC_HAS_FEATURE_VARIANTS",
      "FPC_HAS_FEATURE_INITFINAL", "FPC_HAS_FEATURE_EXITCODE",
      "FPC_HAS_FEATURE_RESOURCES", "FPC_HAS_FEATURE_COMMANDARGS",
      "FPC_HAS_FEATURE_PROCESSES", "FPC_HAS_FEATURE_THREADING",
      "FPC_HAS_FEATURE_DYNLIBS", "FPC_HAS_FEATURE_OBJECTIVEC1",
      ...
      "LEGACYHEAP"};
  for (size_t i = 0; i < sizeof(default_symbols) / sizeof(default_symbols[0]);
       ++i) {
  ...
  for (size_t i = 0; i < sizeof(const_symbols) / sizeof(const_symbols[0]);
       ++i) {
    if (!pascal_preprocessor_define_const(preprocessor, const_symbols[i].name,
                                          const_symbols[i].value)) {
      char detail[128];
      snprintf(detail, sizeof(detail), "unable to define constant '%s'",
               const_symbols[i].name);
      report_preprocessor_error(error_out, path, detail);
      pascal_preprocessor_free(preprocessor);
      free(buffer);
      return false;
...
/* On x86-64 architecture, define CPUX86_64 for FPC compatibility */
  ...
  if (!pascal_preprocessor_define(preprocessor, "CPUX86_64")) {
    report_preprocessor_error(error_out, path,
                              "unable to define CPUX86_64 symbol");
    pascal_preprocessor_free(preprocessor);
    free(buffer);
    return false;
  }
  ...
  if (!pascal_preprocessor_define(preprocessor, "CPUINT64")) {
    report_preprocessor_error(error_out, path,
                              "unable to define CPUINT64 symbol");
    pascal_preprocessor_free(preprocessor);
    free(buffer);
    return false;
  }
  ...
  if (!pascal_preprocessor_define(preprocessor, "x86_64")) {
    report_preprocessor_error(error_out, path,
                              "unable to define x86_64 symbol");
    pascal_preprocessor_free(preprocessor);
    free(buffer);
    return false;
  }
  /* Define first_mm_imreg for FPC x86_64 paramgr.pas (CPU register constant
   * from cpubase.pas) */
  if (!pascal_preprocessor_define_const(preprocessor, "first_mm_imreg", "32")) {
    report_preprocessor_error(error_out, path,
                              "unable to define first_mm_imreg symbol");
    pascal_preprocessor_free(preprocessor);
    free(buffer);
    return false;
  }
  /* Define optimizer switch constants for {$IF x in
   * supported_optimizerswitches} expressions. cs_opt_use_load_modify_store is
   * at ordinal 25 in the toptimizerswitch enum. supported_optimizerswitches for
   * x86_64 includes this (via genericlevel3optimizerswitches). We represent
   * sets as bitmasks: bit N = ordinal N is in set.
   */
  if (!pascal_preprocessor_define_const(preprocessor,
                                        "cs_opt_use_load_modify_store", "25")) {
    report_preprocessor_error(error_out, path,
                              "unable to define cs_opt_use_load_modify_store");
    pascal_preprocessor_free(preprocessor);
    free(buffer);
    return false;
  }
  /* Bitmask with at least bit 25 set for supported_optimizerswitches */
  if (!pascal_preprocessor_define_const(
          preprocessor, "supported_optimizerswitches", "33554432")) {
    report_preprocessor_error(error_out, path,
                              "unable to define supported_optimizerswitches");
    pascal_preprocessor_free(preprocessor);
    free(buffer);
    return false;
  }
  ...
  if (!pascal_preprocessor_define(preprocessor, "ENDIAN_LITTLE")) {
    report_preprocessor_error(error_out, path,
                              "unable to define ENDIAN_LITTLE symbol");
    pascal_preprocessor_free(preprocessor);
    free(buffer);
    return false;
  }
  if (!pascal_preprocessor_define(preprocessor, "FPC_LITTLE_ENDIAN")) {
    report_preprocessor_error(error_out, path,
                              "unable to define FPC_LITTLE_ENDIAN symbol");
    pascal_preprocessor_free(preprocessor);
    free(buffer);
    return false;
  }
  ...
  if (target_windows_flag()) {
    if (!pascal_preprocessor_define(preprocessor, "MSWINDOWS")) {
      report_preprocessor_error(error_out, path,
                                "unable to define MSWINDOWS symbol");
      pascal_preprocessor_free(preprocessor);
      free(buffer);
      return false;
    }
  ...
  char *preprocessed_buffer =
      pascal_preprocess_buffer(preprocessor, path, buffer, length,
                               &preprocessed_length, &preprocess_error);
  /* Record included files in the compilation context for cache key. */
  {
    const char *const *inc_files = NULL;
    size_t inc_count =
        pascal_preprocessor_get_included_files(preprocessor, &inc_files);
    if (inc_count > 0) {
  ...
  if (kgpc_getenv("KGPC_DUMP_PP") != NULL && path != NULL &&
      strstr(path, "system.pp") != NULL) {
    FILE *dump = fopen("/tmp/system_pp_preprocessed.txt", "w");
    if (dump != NULL) {
      fwrite(preprocessed_buffer, 1, preprocessed_length, dump);
      fclose(dump);
      fprintf(stderr,
              "[PP] Dumped preprocessed system.pp to "
              "/tmp/system_pp_preprocessed.txt (%zu bytes)\n",
              preprocessed_length);
    }
  }
  ...
  bool is_unit = buffer_starts_with_unit(buffer, length);
  combinator_t *parser =
      is_unit ? get_or_create_unit_parser() : get_or_create_program_parser();
  ...
  if (kgpc_getenv("KGPC_DEBUG_TFPG_AST") != NULL && result.is_success &&
  ...
  if (!result.is_success) {
    ...
    if (convert_to_tree) {
      ...
      if (tree == NULL) {
        fprintf(stderr,
                "Error: Failed to convert AST for '%s' to legacy parse tree.\n",
                path);
      } else {
        success = true;
      }
  ...
  malloc_trim(0);
#endif
  /* Don't reset file_to_parse to NULL - it's needed for semantic error
   * reporting */
  /* file_to_parse = NULL; */
...
void pascal_print_parse_error(const char *path, const ParseError *err) {
  ...
  const char *display_path =
      (err->source_filename != NULL) ? err->source_filename : path;
