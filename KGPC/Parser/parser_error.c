...
static void sanitize_token_text(const char *yytext, int yyleng, char *buffer,
                                size_t buffer_size) {
  if (buffer_size == 0)
    return;
...
static void extract_token_sections(const char *message, char *unexpected,
                                   size_t unexpected_size, char *expected,
  ...
  if (unexpected_start != NULL) {
    ...
    size_t len = unexpected_end != NULL
                     ? (size_t)(unexpected_end - unexpected_start)
  ...
  if (expected_start != NULL) {
    ...
    while (*cursor != '\0') {
      ...
      size_t segment_len =
          next != NULL ? (size_t)(next - cursor) : strlen(cursor);
...
static void print_context_line(const char *file_path, int error_line,
                               int caret_column) {
  if (file_path == NULL)
    return;
...
void parser_error_report(const char *message, const char *yytext, int yyleng) {
  ...
  fprintf(stderr, "Syntax error in %s at line %d, column %d\n", file_path,
          line_num, caret_column);

  char unexpected[128];
  char expected[256];
  extract_token_sections(message, unexpected, sizeof(unexpected), expected,
                         sizeof(expected));
...
/* Helper to check if a line is a {#line N "file"} directive and extract the
 * line number. Returns the new line number if it's a directive, or -1 if not.
 */
/* Parse {#line N "filename"} directive and extract line number and optional
 * filename */
static int parse_line_directive_with_file(const char *line, size_t len,
                                          char *filename_out,
...
int print_source_context_from_buffer(const char *buffer, size_t length,
  ...
  char current_file[512] =
      ""; /* Track current source file from {#line} directives */
  ...
  while (current_line < start_line && idx < length) {
    ...
    int directive_line = parse_line_directive_with_file(
...
int print_source_context_at_offset(const char *buffer, size_t length,
  ...
  /* Find the line number and file at source_offset by scanning backwards for
   * #line directives */
  char current_file[512] = "";
  int current_line_at_offset =
      1; /* Default line number if no directive found */
  ...
  int actual_error_line =
      (error_line > 0) ? error_line : current_line_at_offset;
   ...
   * Example: if error_line=9 and we crossed 2 newlines, we're at start of line
   * 8 (not line 7), because: line 9 -> cross \n -> line 8 -> cross \n -> at
   * start of line 8 The line 531 skip puts us at start of the line AFTER the
   * newline we landed on. */
  ...
  while (line_num <= end_line && idx < length) {
    ...
    char directive_file[512] = "";
    int directive_line =
        parse_line_directive_with_file(buffer + line_start_pos, line_len,
