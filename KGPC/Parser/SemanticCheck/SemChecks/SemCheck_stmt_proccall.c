...
int semcheck_proccall(SymTab_t *symtab, struct Statement *stmt,
  ...
  if (args_given != NULL) {
    ...
    const char *replacement =
        (result_var != NULL && result_var[0] != '\0') ? result_var : "Result";
    for (ListNode_t *arg_cur = args_given; arg_cur != NULL;
      ...
      int is_result_name = (cur_sub_id != NULL &&
                            pascal_identifier_equals(arg_id, cur_sub_id)) ||
                           (result_var != NULL &&
                            pascal_identifier_equals(arg_id, result_var)) ||
                           (method_name != NULL &&
                            pascal_identifier_equals(arg_id, method_name));
      if (!is_result_name)
        continue;
      ...
      if (!pascal_identifier_equals(arg_id, replacement)) {
        ...
        if (symtab != NULL && symtab->current_scope != NULL &&
          ...
          if (orig_check != NULL && (orig_check->hash_type == HASHTYPE_VAR ||
        ...
        if (symtab != NULL && symtab->current_scope != NULL &&
            symtab->current_scope->table != NULL) {
          local_check =
              FindIdentInTable(symtab->current_scope->table, replacement);
          if (local_check != NULL &&
  ...
  if (stmt->stmt_data.procedure_call_data.is_method_call_placeholder &&
    ...
    if (first_arg != NULL && first_arg->type == EXPR_VAR_ID &&
      ...
      int with_status = semcheck_with_try_resolve(
          first_arg->expr_data.id, symtab, &with_expr, stmt->line_num);
    ...
    if (first_arg != NULL && first_arg->type == EXPR_VAR_ID &&
      ...
      int type_found =
          (FindSymbol(&type_node, symtab, first_arg->expr_data.id) != 0 &&
      ...
      if (!type_found) {
        ...
        if (dollar != NULL && dollar > first_arg->expr_data.id) {
          char *gen_base = strndup(first_arg->expr_data.id,
                                   (size_t)(dollar - first_arg->expr_data.id));
          if (gen_base != NULL) {
            type_node = NULL;
      ...
      if (type_found) {
        struct RecordType *record_info =
            semcheck_stmt_get_record_type_from_node(type_node);
        if (record_info != NULL && record_info->type_id != NULL &&
          ...
          /* Check if method_name is actually a field of procedure type on the
           * class. E.g., tmodule.finish_module(hp) where finish_module is a
           * class var of procedural type, not a method. Since the receiver is a
           * type name, only class vars (or static fields) are valid here. */
          {
            ...
            while (walk_rec != NULL && !is_classvar_proc) {
              ...
              walk_rec = (parent != NULL)
                             ? semcheck_lookup_record_type(symtab, parent)
                             : NULL;
            }
            if (is_classvar_proc) {
               ...
               */
              struct Expression *type_expr =
                  mk_varid(stmt->line_num, strdup(first_arg->expr_data.id));
              struct Expression *field_access = mk_recordaccess(
                  stmt->line_num, type_expr, strdup(method_name));
              ...
              return_val += semcheck_stmt_expr_tag(
                  &field_tag, symtab, field_access, max_scope_lev, NO_MUTATE);
          ...
          int is_nonstatic_class_method =
              (!is_static_method && from_cparser_is_method_class_method(
                                        record_info->type_id, method_name));
          ...
          if (new_proc_id != NULL) {
            ...
            if (is_nonstatic_class_method) {
              ...
              /* Check if this is a constructor call (e.g.,
               * TMyItem.Create(...)). If so, mark it for the codegen so it
...
skip_type_receiver_rewrite:
   ...
   * When the parser sees Unit.Procedure(args), it creates a procedure call with
   * id "__Procedure" and passes Unit as the first argument (as if it were a
   * method call). We need to detect this pattern and transform it back to a
   * direct procedure call.
   *
   * Pattern: proc_id starts with "__", first arg is a VAR_ID that names a known
   * unit (preferred) or is unresolved in the symbol table (secondary
   * heuristic), and the procedure name (without "__" prefix) exists in symbol
   * table.
   */
  if (stmt->stmt_data.procedure_call_data.is_method_call_placeholder &&
      args_given != NULL) {
    ...
    if (first_arg != NULL && first_arg->type == EXPR_VAR_ID &&
      ...
      /* Prefer explicit unit-name recognition; keep unresolved-name secondary
       * path for parser shapes where unit qualifiers are not injected into
       * symbol tables. */
      if (!is_unit_qualifier &&
        ...
        if (FindSymbol(&self_node, symtab, "Self") != 0 && self_node != NULL) {
          struct RecordType *self_record =
              semcheck_stmt_get_record_type_from_node(self_node);
          if (self_record != NULL &&
              semcheck_find_class_field_including_hidden(
                  symtab, self_record, potential_unit_name, NULL) != NULL) {
        ...
        if (!looks_like_self_member && with_context_count > 0) {
          ...
          int ws = semcheck_with_try_resolve(potential_unit_name, symtab,
                                             &with_check, stmt->line_num);
      ...
      if (is_unit_qualifier) {
        ...
        if (stmt->stmt_data.procedure_call_data.placeholder_method_name !=
            NULL) {
          real_proc_name = strdup(
              stmt->stmt_data.procedure_call_data.placeholder_method_name);
        } else if (proc_id != NULL && proc_id[0] == '_' && proc_id[1] == '_' &&
                   proc_id[2] != '\0') {
        ...
        if (real_proc_name == NULL) {
          ...
          if (pascal_identifier_equals(potential_unit_name, "System") &&
            ...
            stmt->stmt_data.procedure_call_data.call_qualifier =
                unit_qualifier_copy;
            ...
            if (proc_candidates != NULL || force_strip_system_qualifier) {
              ...
              if (stmt->stmt_data.procedure_call_data.placeholder_method_name !=
                  NULL) {
                free(stmt->stmt_data.procedure_call_data
              ...
              /* Continue with normal procedure call handling using the
               * transformed call */
            } else {
              /* Procedure not found - allow System.Exit without a symbol table
               * entry. */
              if (pascal_identifier_equals(real_proc_name, "Exit")) {
                ...
                stmt->stmt_data.procedure_call_data.is_method_call_placeholder =
                    0;
                if (stmt->stmt_data.procedure_call_data.call_qualifier !=
                    NULL) {
                  free(stmt->stmt_data.procedure_call_data.call_qualifier);
                  stmt->stmt_data.procedure_call_data.call_qualifier = NULL;
                }
                ...
                /* Procedure not found - free real_proc_name and fall through to
                 * report error */
                free(real_proc_name);
              }
            }
  ...
  if (proc_id != NULL && pascal_identifier_equals(proc_id, "Exit") &&
      !stmt->stmt_data.procedure_call_data.is_method_call_placeholder) {
    int arg_count = 0;
    ...
    if (arg_count > 1) {
      semcheck_error_with_context_at(
          ...
          "Error on line %d, Exit() expects at most one argument.\n\n",
          stmt->line_num);
      return 1;
    }
    ...
    if (arg_count == 1) {
      ...
      if (exit_expr != NULL)
        return_val += semcheck_stmt_expr_tag(&expr_type, symtab, exit_expr,
                                             max_scope_lev, 0);
    }
  ...
  /* INTERNPROC: Transform TypedFile Rewrite/Reset calls by injecting element
   * size. Must happen before overload resolution so the 2-arg variant is
   * selected. */
  if (semcheck_internproc_typedfile_rewrite_reset(symtab, stmt))
    args_given = stmt->stmt_data.procedure_call_data.expr_args;

  /* If no explicit receiver was provided, but Self is in scope and defines this
   * method, prepend Self so unqualified method calls resolve correctly. */
  if (!was_unit_qualified && proc_id != NULL &&
    ...
    if (kgpc_getenv("KGPC_DEBUG_ASSIGN") != NULL &&
      ...
      fprintf(stderr,
              "[ASSIGN-TRACE] Self_found=%d owner=%s scope_kind=%d "
              "proc_owner=%s proc_hash=%d\n",
              dbg_found != 0, dbg_owner ? dbg_owner : "<null>",
              symtab->current_scope ? symtab->current_scope->num_deps : -1,
              (dbg_proc && dbg_proc->owner_class) ? dbg_proc->owner_class
                                                  : "<null>",
    ...
    if (FindSymbol(&self_node, symtab, "Self") != 0 && self_node != NULL) {
      ...
      if (self_record == NULL) {
        ...
        struct RecordType *helper_record =
            semcheck_lookup_type_helper(symtab, self_type_tag, self_type_name);
    ...
    if (self_record != NULL) {
      ...
      struct RecordType *lookup_record =
          (owner_record != NULL) ? owner_record : self_record;
      HashNode_t *method_node =
          semcheck_find_class_method(symtab, lookup_record, proc_id, NULL);
      ...
      if (method_node == NULL && self_record->type_id != NULL) {
        if (cur_owner != NULL &&
          ...
          if (owner_record != NULL) {
            method_node =
                semcheck_find_class_method(symtab, owner_record, proc_id, NULL);
            if (method_node != NULL)
      ...
      if (method_node != NULL &&
         ...
         * convert_method_implementation in
         * from_cparser_statements_and_programs.c). In that case the WITH target
         * IS Self, so the regular Self-prepend path below resolves correctly
         * with proper overload-aware metadata.  Only override when there is an
        ...
        if (with_context_count > 0 && !innermost_is_synthetic_self) {
          struct Expression *with_expr = NULL;
          int wm = semcheck_with_try_resolve_method(proc_id, symtab, &with_expr,
                                                    stmt->line_num);
          if (wm == 0 && with_expr != NULL) {
            ...
            struct Expression *field_access =
                mk_recordaccess(stmt->line_num, with_expr, strdup(proc_id));
            if (field_access != NULL) {
              ...
              int field_tag = UNKNOWN_TYPE;
              return return_val +
                     semcheck_stmt_expr_tag(&field_tag, symtab, field_access,
        ...
        const char *receiver_class_name = (method_node->owner_class != NULL)
                                              ? method_node->owner_class
                                              : self_record->type_id;
        int method_is_static =
            (receiver_class_name != NULL && bare_method_name != NULL)
                ? from_cparser_is_method_static(receiver_class_name,
                                                bare_method_name)
                : 0;
        int method_is_class = (receiver_class_name != NULL &&
                               bare_method_name != NULL && !method_is_static)
                                  ? from_cparser_is_method_class_method(
                                        receiver_class_name, bare_method_name)
                                  : 0;
        if (!method_is_static) {
          struct Expression *receiver_expr =
              mk_varid(stmt->line_num, strdup("Self"));
          ListNode_t *receiver_arg =
              (receiver_expr != NULL) ? CreateListNode(receiver_expr, LIST_EXPR)
                                      : NULL;
        ...
        /* Update proc_id to the resolved method's id (e.g. TBase__Bump, not
         * TDerived__Bump when the method is inherited from a parent class). */
        ...
        /* Check if this is a virtual/abstract method call that needs VMT
         * dispatch. Only for instance methods (not class/static methods), since
         * class methods use a different VMT dispatch convention (single
         * indirection). */
        ...
        if (self_record->type_id != NULL && bare_method_name != NULL &&
            from_cparser_is_method_virtual_with_types(
                ...
                NULL, 0) &&
            !from_cparser_is_method_static(self_record->type_id,
                                           bare_method_name)) {
        ...
        if (self_record != NULL && self_record->is_interface &&
          ...
          int idx = 0;
          for (ListNode_t *mt = self_record->method_templates; mt != NULL;
               mt = mt->next, idx++) {
        ...
        if (self_record->type_id != NULL && bare_method_name != NULL) {
          ...
          while (check_class != NULL) {
            if (from_cparser_is_method_nonstatic_class_method(
                    check_class, bare_method_name)) {
              stmt->stmt_data.procedure_call_data.is_class_method_call = 1;
              break;
            }
            const char *parent =
        ...
        /* Check if proc_id is already a resolved method call (has owner_class
         * in symbol table) */
        HashNode_t *proc_check_node = NULL;
        int is_already_method = 0;
        if (FindSymbol(&proc_check_node, symtab, proc_id) != 0 &&
            proc_check_node != NULL && proc_check_node->owner_class != NULL)
          is_already_method = 1;
        if (!is_already_method) {
          /* Check if proc_id is a procedural-type field or property of Self's
           * class. This handles patterns like FCallBack(Self,a,b,c) and
           * OnQueryInterface(x,y,z) where FCallBack is a field of type
           * TThunkCallBack (procedural type) and OnQueryInterface is a property
           * reading from a procedural-type field. */
          const char *field_name = proc_id;
          int is_proc_field = 0;

          /* Use semcheck_lookup_record_type to get a safe, validated RecordType
           * from the symbol table, since self_record obtained from temp call
           * contexts can have corrupt data */
          struct RecordType *safe_record =
              semcheck_lookup_record_type(symtab, self_record->type_id);
          if (safe_record != NULL) {
            for (ListNode_t *f = safe_record->fields; f != NULL; f = f->next) {
              ...
              if (strcasecmp(rf->name, field_name) == 0) {
                if (rf->proc_type != NULL) {
                  ...
                  /* proc_type is resolved during
                   * semcheck_qualify_nested_types_for_record so this path
                   * handles non-nested procedural types only. */
                  HashNode_t *type_node = NULL;
            ...
            if (!is_proc_field) {
              /* Check properties — if a property's read_accessor is a
               * procedural-type field */
              struct ClassProperty *prop = semcheck_find_class_property(
                  symtab, safe_record, proc_id, NULL);
              if (prop != NULL && prop->read_accessor != NULL) {
                for (ListNode_t *f2 = safe_record->fields; f2 != NULL;
                     f2 = f2->next) {
          ...
          if (is_proc_field) {
             ...
             * Build a record access expression Self.field and set it as
             * procedural_var_expr */
            struct Expression *self_expr =
                mk_varid(stmt->line_num, strdup("Self"));
            struct Expression *field_access =
                mk_recordaccess(stmt->line_num, self_expr, strdup(field_name));
  ...
  return_val +=
      try_resolve_builtin_procedure(symtab, stmt, "Halt", semcheck_builtin_halt,
                                    max_scope_lev, &handled_builtin);
  if (handled_builtin)
    return return_val;

  handled_builtin = 0;
  return_val += try_resolve_builtin_procedure(symtab, stmt, "Error",
                                              semcheck_builtin_error,
                                              max_scope_lev, &handled_builtin);
  if (handled_builtin)
    return return_val;
  ...
  return_val += try_resolve_builtin_procedure(symtab, stmt, "SetTextCodePage",
                                              semcheck_builtin_settextcodepage,
                                              max_scope_lev, &handled_builtin);
  if (handled_builtin)
    return return_val;
  ...
  handled_builtin = 0;
  return_val +=
      try_resolve_builtin_procedure(symtab, stmt, "Move", semcheck_builtin_move,
                                    max_scope_lev, &handled_builtin);
  if (handled_builtin)
    return return_val;
  ...
  handled_builtin = 0;
  return_val +=
      try_resolve_builtin_procedure(symtab, stmt, "Val", semcheck_builtin_val,
                                    max_scope_lev, &handled_builtin);
  if (handled_builtin)
    return return_val;
  ...
  handled_builtin = 0;
  return_val +=
      try_resolve_builtin_procedure(symtab, stmt, "Inc", semcheck_builtin_inc,
                                    max_scope_lev, &handled_builtin);
  if (handled_builtin)
    return return_val;
  ...
  handled_builtin = 0;
  return_val +=
      try_resolve_builtin_procedure(symtab, stmt, "Dec", semcheck_builtin_dec,
                                    max_scope_lev, &handled_builtin);
  if (handled_builtin)
    return return_val;
  ...
  handled_builtin = 0;
  return_val +=
      try_resolve_builtin_procedure(symtab, stmt, "New", semcheck_builtin_new,
                                    max_scope_lev, &handled_builtin);
  if (handled_builtin)
    return return_val;
  ...
  return_val += try_resolve_builtin_procedure(symtab, stmt, "Assert",
                                              semcheck_builtin_assert,
                                              max_scope_lev, &handled_builtin);
  if (handled_builtin)
    return return_val;

  /* Handle procedural fields on records (advanced records) similarly to
   * function calls */
  if (proc_id != NULL && args_given != NULL) {
    struct Expression *receiver_expr = (struct Expression *)args_given->cur;
    if (receiver_expr != NULL &&
        receiver_expr->type == EXPR_RECORD_CONSTRUCTOR &&
    ...
    if (receiver_expr != NULL) {
      ...
      semcheck_stmt_expr_tag(&recv_type, symtab, receiver_expr, max_scope_lev,
                             NO_MUTATE);
      ...
      /* Also try resolved_kgpc_type directly for record types when record_type
       * is NULL */
      ...
      if (recv_record != NULL) {
        ...
        int field_found =
            resolve_record_field(symtab, recv_record, field_lookup, &field_desc,
        ...
        if (field_found == 0 && field_desc != NULL) {
          ...
          if (field_desc->type_id != NULL) {
             ...
             */
            if (!is_proc_field && recv_record != NULL &&
                recv_record->type_id != NULL) {
              char qualified[512];
              snprintf(qualified, sizeof(qualified), "%s.%s",
                       recv_record->type_id, field_desc->type_id);
              type_node = NULL;
          ...
          if (is_proc_field) {
            ...
            if (proc_type != NULL) {
              ...
              if (ListLength(formal_params) != ListLength(remaining_args)) {
                semcheck_error_with_context_at(
                    stmt->line_num, stmt->col_num, stmt->source_index,
                    "Error on line %d, call to procedural field %s: expected "
                    "%d arguments, got %d\n",
                    stmt->line_num, proc_id, ListLength(formal_params),
                    ListLength(remaining_args));
                destroy_expr(proc_expr);
                /* proc_type already released by destroy_expr via
                 * resolved_kgpc_type */
                return ++return_val;
              }
              ...
              while (formal != NULL && actual != NULL) {
                ...
                struct Expression *actual_expr =
                    (struct Expression *)actual->cur;
                int formal_type = resolve_param_type(formal_decl, symtab);
                int actual_type = UNKNOWN_TYPE;
                semcheck_stmt_expr_tag(&actual_type, symtab, actual_expr,
                                       max_scope_lev, NO_MUTATE);
                if (formal_type != UNKNOWN_TYPE &&
                    actual_type != UNKNOWN_TYPE && formal_type != actual_type) {
   ...
   * where first arg is the instance. */
  if (stmt->stmt_data.procedure_call_data.is_method_call_placeholder &&
      args_given != NULL) {
    struct Expression *first_arg = (struct Expression *)args_given->cur;
    int first_arg_type_tag;
    semcheck_stmt_expr_tag(&first_arg_type_tag, symtab, first_arg,
                           max_scope_lev, NO_MUTATE);
    ...
    if (owner_type != NULL) {
      if (owner_type->kind == TYPE_KIND_RECORD) {
        ...
        KgpcType *pointee =
            kgpc_type_resolve_pointer_pointee(owner_type, symtab);
        if (pointee != NULL && pointee->kind == TYPE_KIND_RECORD) {
          ...
          KgpcType *pointee2 =
              kgpc_type_resolve_pointer_pointee(pointee, symtab);
    ...
    /* Do not rely on legacy record_type metadata; prefer resolved KgpcType
     * only. */
    ...
    if (record_info != NULL && record_info->type_id != NULL) {
      ...
      if (method_node == NULL && !record_info->is_type_helper) {
        struct RecordType *helper_record =
            semcheck_lookup_type_helper_for_record_member(symtab, record_info,
                                                          method_name);
        if (helper_record != NULL) {
          actual_method_owner = NULL;
          method_node = semcheck_find_class_method(
              symtab, helper_record, method_name, &actual_method_owner);
          if (method_node != NULL)
      ...
      if (!is_static && actual_method_owner != NULL &&
        ...
        is_static = from_cparser_is_method_static(actual_method_owner->type_id,
                                                  method_name);
      }
      int is_nonstatic_class_method =
          (!is_static && from_cparser_is_method_class_method(
                             record_info->type_id, method_name));
      if (!is_nonstatic_class_method && !is_static &&
          actual_method_owner != NULL && actual_method_owner->type_id != NULL &&
          method_name != NULL) {
        is_nonstatic_class_method = from_cparser_is_method_class_method(
            actual_method_owner->type_id, method_name);
      }

      if (method_node != NULL) {
        /* Keep class-prefixed id for static/class calls (e.g.
         * ClassName.Create), but canonicalize instance calls to the resolved
         * owner id. Use the actual method owner's class name so inherited
         * methods resolve to the defining class (e.g. TObject__Free, not
         * TChild__Free). */
        if (static_method_receiver || is_nonstatic_class_method) {
          const char *owner_id = (actual_method_owner != NULL &&
                                  actual_method_owner->type_id != NULL)
                                     ? actual_method_owner->type_id
                                     : record_info->type_id;
        ...
        if (args_given != NULL && args_given->cur != NULL) {
          struct Expression *receiver_expr =
              (struct Expression *)args_given->cur;
          if (receiver_expr != NULL && receiver_expr->type == EXPR_VAR_ID &&
              receiver_expr->expr_data.id != NULL) {
            HashNode_t *receiver_node = NULL;
            if (FindSymbol(&receiver_node, symtab,
                           receiver_expr->expr_data.id) != 0 &&
                receiver_node != NULL &&
        ...
        if (!is_static && !is_nonstatic_class_method &&
            !stmt->stmt_data.procedure_call_data
                 .is_tp_new_dispose_helper_call &&
            !receiver_is_type_ident) {
          semcheck_stmt_set_receiver_virtual_dispatch(
              stmt, record_info, method_name, method_node->type);
        }

        {
          ...
          if (constructor_owner != NULL &&
              semcheck_stmt_method_is_declared_constructor(
                  symtab, constructor_owner, method_name)) {
            stmt->stmt_data.procedure_call_data.is_constructor_call = 1;
        ...
        if (is_nonstatic_class_method && receiver_is_type_ident) {
          ...
          /* For static methods, remove the first argument (the instance/type
           * identifier) */
        ...
        if (resolve_record_field(symtab, record_info, method_name, &proc_field,
                                 &proc_field_offset, stmt->line_num,
                                 1 /* silent */) == 0 &&
            proc_field != NULL) {
          KgpcType *proc_type = NULL;
          ...
          if (proc_type != NULL) {
            /* Remove first_arg from args (it becomes part of the field access
             * expr) */
            ListNode_t *old_head = args_given;
            ...
            /* Build record access expression for the procedural field */
            struct Expression *proc_expr =
                mk_recordaccess(stmt->line_num, first_arg, strdup(method_name));
            proc_expr->expr_data.record_access_data.field_offset =
                (int)proc_field_offset;
            kgpc_type_retain(proc_type);
            proc_expr->resolved_kgpc_type = proc_type;
            ...
            for (ListNode_t *arg_cur = args_given; arg_cur != NULL;
              ...
              if (arg != NULL)
                semcheck_stmt_expr_tag(NULL, symtab, arg, max_scope_lev,
                                       NO_MUTATE);
            }
  ...
  int proc_is_method_placeholder =
      stmt->stmt_data.procedure_call_data.is_method_call_placeholder;
  ...
  if (!proc_is_method && proc_id != NULL &&
      FindSymbol(&proc_method_node, symtab, proc_id) != 0 &&
      proc_method_node != NULL && proc_method_node->owner_class != NULL) {
    proc_is_method = 1;
  ...
  if (proc_is_method && args_given != NULL && !static_arg_already_removed) {
    ...
    if (proc_is_method_placeholder) {
      ...
      if (args_given != NULL && args_given->cur != NULL) {
        ...
        if (first_arg->type == EXPR_VAR_ID) {
          ...
          if (FindSymbol(&var_node, symtab, first_arg->expr_data.id) != 0 &&
              ...
              var_node->type->kind == TYPE_KIND_RECORD) {
        ...
        if (record_type == NULL && first_arg->pointer_subtype_id != NULL) {
          ...
          if (FindSymbol(&subtype_node, symtab,
                         first_arg->pointer_subtype_id) != 0 &&
              subtype_node != NULL) {
        ...
        if (record_type == NULL || record_type->type_id == NULL) {
          ...
          semcheck_stmt_expr_tag(&helper_tag, symtab, first_arg, max_scope_lev,
                                 NO_MUTATE);
          /* Resolve arg_type AFTER semcheck_stmt_expr_tag — the latter may
             free and replace types on the expression, invalidating earlier
             pointers. */
          int arg_type_owned = 0;
          KgpcType *arg_type = semcheck_resolve_expression_kgpc_type(
              symtab, first_arg, max_scope_lev, NO_MUTATE, &arg_type_owned);
          ...
          struct RecordType *helper_record =
              semcheck_lookup_type_helper_for_member(symtab, helper_tag,
            ...
            if (FindSymbol(&var_node, symtab, first_arg->expr_data.id) != 0 &&
              ...
              if (var_helper_name != NULL)
                helper_record = semcheck_lookup_type_helper_for_member(
                    symtab, UNKNOWN_TYPE, var_helper_name, method_name);
            }
          }
          if (helper_record != NULL)
        ...
        if (class_name == NULL && record_type != NULL &&
            record_type->type_id != NULL) {
          if (kgpc_getenv("KGPC_DEBUG_TYPE_HELPER") != NULL) {
            fprintf(stderr,
                    "[SemCheck] method placeholder: resolved helper record %s "
                    "for %s\n",
                    record_type->type_id,
      ...
      /* Case 2: ClassName__MethodName - class_name already set from symbol
       * table lookup */
    ...
    if (class_name != NULL && method_name != NULL) {
      ...
      if (FindSymbol(&class_node, symtab, class_name) != 0 &&
          class_node != NULL) {
        struct RecordType *class_record =
            semcheck_stmt_get_record_type_from_node(class_node);
        if (class_record != NULL)
          resolved_method = semcheck_find_class_method(symtab, class_record,
                                                       method_name, NULL);
      }
...

      {
        const char *overload_check_id =
            (resolved_method != NULL && resolved_method->id != NULL)
                ? resolved_method->id
                : proc_id;
        semcheck_stmt_try_set_method_mangled_id(
            symtab, stmt, overload_check_id,
            resolved_method != NULL ? resolved_method->mangled_id : NULL);
      }
      if (!static_method_receiver && resolved_method != NULL &&
          resolved_method->id != NULL) {
      ...
      if (args_given != NULL && args_given->cur != NULL) {
        ...
        if (receiver_expr != NULL && receiver_expr->type == EXPR_VAR_ID &&
          ...
          if (pascal_identifier_equals(receiver_expr->expr_data.id, "Self"))
            receiver_is_self = 1;
          HashNode_t *receiver_node = NULL;
          if (FindSymbol(&receiver_node, symtab, receiver_expr->expr_data.id) !=
                  0 &&
              receiver_node != NULL &&
              receiver_node->hash_type == HASHTYPE_TYPE)
            receiver_is_type_ident = 1;
  ...
  int proc_is_method_for_type_res =
      stmt->stmt_data.procedure_call_data.is_method_call_placeholder;
  if (!proc_is_method_for_type_res && proc_id != NULL &&
      FindSymbol(&type_res_method_node, symtab, proc_id) != 0 &&
      type_res_method_node != NULL &&
  ...
  if (proc_is_method_for_type_res && args_given != NULL &&
    ...
    struct Expression *first_arg = (struct Expression *)args_given->cur;
    if (first_arg != NULL) {
      /* IMPORTANT: Call semcheck_expr_main FIRST to resolve the expression
       * type. This may modify/replace first_arg->resolved_kgpc_type. Only AFTER
       * this call should we get the KgpcType, otherwise we risk getting a
       * pointer that gets freed when semcheck_expr_main updates the type.
       * (e.g., for 'as' expressions which destroy and replace
       * resolved_kgpc_type)
       */
      int helper_tag = UNKNOWN_TYPE;
      semcheck_stmt_expr_tag(&helper_tag, symtab, first_arg, max_scope_lev,
      ...
      /* Now it's safe to get the KgpcType since semcheck_expr_main has
       * finalized it */
      int arg_type_owned = 0;
      KgpcType *arg_type = semcheck_resolve_expression_kgpc_type(
          symtab, first_arg, INT_MAX, NO_MUTATE, &arg_type_owned);
      ...
      if (arg_type != NULL) {
        ...
        if (obj_record_type == NULL) {
          ...
          struct RecordType *helper_record =
              semcheck_lookup_type_helper_for_member(
                  symtab, helper_tag, helper_name, method_name_part);
          if (helper_record != NULL)
        ...
        if (obj_record_type != NULL) {
          if (!obj_record_type->is_type_helper &&
            ...
            struct RecordType *helper_record =
                semcheck_lookup_type_helper_for_record_member(
                    symtab, obj_record_type, method_name_part);
            if (helper_record != NULL &&
                semcheck_find_class_method(symtab, helper_record,
                                           method_name_part,
                                           &helper_owner) != NULL) {
              obj_record_type =
                  (helper_owner != NULL) ? helper_owner : helper_record;
            }
          }

          /* Found the object with a record type. Now find the class name for
           * this type. Use the type_id stored directly on the RecordType, which
           * is the canonical type name where methods are registered. This
           * avoids issues with type aliases (e.g., IInterface = IUnknown) where
           * walking the symbol table might find the alias name instead of the
           * original type name. */
          ...
          if (correct_class_name != NULL) {
            ...
            while (current_record != NULL && current_class_name != NULL) {
              ...
              sprintf(mangled_name, "%s__%s", current_class_name,
                      method_name_part);

              /* Check if this mangled name exists in the symbol table */
              HashNode_t *proc_node = NULL;
              if (FindSymbol(&proc_node, symtab, mangled_name) != 0 &&
                  proc_node != NULL) {
                /* Found it! Update the procedure ID */
                free(proc_id);
                proc_id = mangled_name;
                stmt->stmt_data.procedure_call_data.id = proc_id;
                /* Don't set mangled_id here - let the normal mangling process
                 * handle it */
                method_found = 1;
                break;
              }
                ...
                if (FindSymbol(&parent_node, symtab, parent_name) != 0 &&
                  ...
                  if (parent_node->type->kind == TYPE_KIND_RECORD) {
                    current_record = parent_node->type->info.record_info;
                             ...
                             parent_node->type->info.points_to->kind ==
                                 TYPE_KIND_RECORD) {
                    current_record =
                        parent_node->type->info.points_to->info.record_info;
                  } else {
                    current_record = NULL;
                  }
                  current_class_name = parent_name;
                } else {
                  /* Parent not found in symbol table (shouldn't happen for
                   * valid code) */
                  current_record = NULL;
                }
              } else {
            ...
            if (!method_found) {
               ...
               * check produces a clear error message (or the virtual method
               * resolves later).
               */
              ...
              char *mangled_name =
                  (char *)malloc(class_len + 2 + method_len + 1);
              if (mangled_name != NULL) {
                sprintf(mangled_name, "%s__%s", correct_class_name,
                        method_name_part);
                free(proc_id);
                proc_id = mangled_name;
                stmt->stmt_data.procedure_call_data.id = proc_id;
...
skip_method_placeholder_resolution:
  ...
  if (stmt->stmt_data.procedure_call_data.mangled_id != NULL) {
    ...
    mangled_name =
        MangleFunctionNameFromCallSite(proc_id, args_given, symtab, INT_MAX);
  }
  assert(mangled_name != NULL);

  /* Check for procedural type typecast-call pattern: TypeName(source)(args)
   * The parser creates a call to TypeName with (source, args...) as flat
   * arguments. If TypeName is a procedural type, the first arg is the typecast
   * source and remaining args are the actual call arguments. Transform into
   * indirect call. */
  {
    HashNode_t *type_check = NULL;
    if (FindSymbol(&type_check, symtab, proc_id) && type_check != NULL &&
        type_check->hash_type == HASHTYPE_TYPE && type_check->type != NULL &&
        type_check->type->kind == TYPE_KIND_PROCEDURE && args_given != NULL) {
      /* Count expected parameters from the procedural type */
      int expected_params = 0;
      if (type_check->type->info.proc_info.params != NULL) {
        for (ListNode_t *p = type_check->type->info.proc_info.params; p != NULL;
             p = p->next)
          expected_params++;
      }
      ...
      /* If we have exactly expected_params + 1 arguments, the first is the
       * typecast source */
      if (actual_args == expected_params + 1) {
        struct Expression *typecast_source =
            (struct Expression *)args_given->cur;

        /* Create a typecast expression wrapping the source */
        struct Expression *typecast_expr =
            (struct Expression *)calloc(1, sizeof(struct Expression));
        ...
        return_val += semcheck_stmt_expr_tag(
            &typecast_tag, symtab, typecast_expr, max_scope_lev, NO_MUTATE);
        ...
        return return_val + semcheck_call_with_proc_var(
                                symtab, stmt, type_check, max_scope_lev);
      }
    }
  }
  ...
  if (overload_candidates != NULL) {
    ...
    while (cur != NULL) {
      ...
      if (candidate->mangled_id != NULL &&
         ...
         * (e.g., for recursion), we may find the same mangled name multiple
         * times. Just keep the first match - they're functionally equivalent.
         */
  ...
  if (resolved_proc == NULL && proc_id != NULL &&
      FindSymbol(&parent_lookup_node, symtab, proc_id) != 0 &&
      parent_lookup_node != NULL && parent_lookup_node->owner_class != NULL) {
    {
      ...
      if (class_name != NULL && method_name != NULL) {
        if (kgpc_getenv("KGPC_DEBUG_INHERITED") != NULL) {
          fprintf(
              stderr,
              "[KGPC] Trying to resolve inherited call: class=%s method=%s\n",
        ...
        if (FindSymbol(&class_node, symtab, class_name) != 0 &&
          ...
          while (parent_class_name != NULL && resolved_proc == NULL) {
            ...
            char *parent_method_name = (char *)malloc(
                strlen(parent_class_name) + 2 + strlen(method_name) + 1);
            if (parent_method_name != NULL) {
              snprintf(parent_method_name,
                       strlen(parent_class_name) + 2 + strlen(method_name) + 1,
                       "%s__%s", parent_class_name, method_name);

              /* Use the same name mangling function that's used for regular
               * method calls */
              char *parent_mangled_name = MangleFunctionNameFromCallSite(
                  parent_method_name, args_given, symtab, INT_MAX);
              if (parent_mangled_name != NULL) {
                /* Look for the parent method using the base name, then check
                 * mangled names */
                ListNode_t *parent_candidates =
                    FindAllIdents(symtab, parent_method_name);
                ...
                if (parent_candidates != NULL) {
                  ...
                  while (cur != NULL) {
                    HashNode_t *candidate = (HashNode_t *)cur->cur;
                    if (candidate->mangled_id != NULL &&
                        strcmp(candidate->mangled_id, parent_mangled_name) ==
                            0) {
                      /* Found the method in parent class - use it */
                      resolved_proc = candidate;
                      match_count = 1;

                      /* Use the resolved declaration id, not a mangled
                       * signature string. */
            ...
            if (parent_class_name != NULL) {
              HashNode_t *parent_class_node =
                  semcheck_find_preferred_type_node(symtab, parent_class_name);
              if (parent_class_node != NULL) {
                record_info =
                    semcheck_stmt_get_record_type_from_node(parent_class_node);
                if (record_info == NULL)
                  break;
  ...
  if (resolved_proc == NULL && overload_candidates == NULL && proc_id != NULL) {
    ...
    if (dunder != NULL && dunder > proc_id) {
      ...
      if (class_name != NULL && method_name[0] != '\0') {
        HashNode_t *class_node =
            semcheck_find_preferred_type_node(symtab, class_name);
        if (class_node == NULL)
        ...
        const char *parent_class_name =
            (record_info != NULL) ? record_info->parent_class_name : NULL;
        while (parent_class_name != NULL && resolved_proc == NULL) {
          ...
          if (parent_candidates != NULL) {
            for (ListNode_t *pc = parent_candidates; pc != NULL;
                 pc = pc->next) {
          ...
          HashNode_t *pnode =
              semcheck_find_preferred_type_node(symtab, parent_class_name);
          struct RecordType *prec =
              (pnode != NULL) ? semcheck_stmt_get_record_type_from_node(pnode)
                              : NULL;
  ...
  if (match_count == 0 && overload_candidates != NULL && !force_best_match) {
    HashNode_t *wildcard_proc = semcheck_find_untyped_mangled_match(
        overload_candidates, proc_id, mangled_name);
  ...
  if (!static_arg_already_removed && overload_candidates != NULL &&
    ...
    if (!receiver_is_type_ident && !receiver_is_self) {
      ...
      for (ListNode_t *cur = overload_candidates; cur != NULL;
           cur = cur->next) {
        HashNode_t *candidate = (HashNode_t *)cur->cur;
      ...
      if (missing_class_self != NULL) {
        struct Expression *class_expr =
            mk_varid(stmt->line_num, strdup(missing_class_self));
        ListNode_t *class_arg =
            (class_expr != NULL) ? CreateListNode(class_expr, LIST_EXPR) : NULL;
        if (class_arg != NULL) {
  ...
  if (match_count == 0 && proc_id != NULL && with_context_count > 0 &&
    ...
    if (FindSymbol(&global_proc, symtab, proc_id) && global_proc != NULL &&
      ...
      if (global_proc->type != NULL &&
          kgpc_type_is_procedure(global_proc->type))
        global_param_count =
            ListLength(global_proc->type->info.proc_info.params);
      int call_arg_count =
    ...
    struct Expression *with_expr = NULL;
    int wm = has_global_proc ? 1
                             : semcheck_with_try_resolve_method(
                                   proc_id, symtab, &with_expr, stmt->line_num);
  ...
  if (match_count == 0 && overload_candidates != NULL) {
    ...
    int overload_status = semcheck_resolve_overload(
        &best_candidate, &num_best_matches, overload_candidates, args_given,
        symtab, &call_stub, max_scope_lev, 0);
  ...
  if (match_count == 1 && overload_candidates != NULL &&
      overload_candidates->next != NULL) {
    HashNode_t *best_candidate = NULL;
    int num_best_matches = 0;
    struct Expression call_stub;
    memset(&call_stub, 0, sizeof(call_stub));
    ...
    if (overload_status == 0 && best_candidate != NULL &&
        num_best_matches == 1 && best_candidate != resolved_proc) {
       ...
       * different arity to replace it causes false "not enough arguments"
       * errors when allow_implicit_leading_self adjusts arity in the resolver.
       */
      int best_total = 0;
      if (best_candidate->type != NULL &&
          best_candidate->type->kind == TYPE_KIND_PROCEDURE)
        best_total =
            ListLength(kgpc_type_get_procedure_params(best_candidate->type));
      int resolved_total = 0;
      if (resolved_proc->type != NULL &&
          resolved_proc->type->kind == TYPE_KIND_PROCEDURE)
        resolved_total =
            ListLength(kgpc_type_get_procedure_params(resolved_proc->type));
      int given_count = ListLength(args_given);
  ...
  if (match_count == 1) {
    ...
    if (resolved_proc != NULL && overload_candidates != NULL) {
      for (ListNode_t *cand_node = overload_candidates; cand_node != NULL;
           cand_node = cand_node->next) {
    ...
    if (kgpc_getenv("KGPC_DEBUG_ASSIGN") != NULL &&
      ...
      fprintf(stderr, "[ASSIGN-RESOLVED] mangled=%s match_count=%d\n",
              resolved_proc->mangled_id ? resolved_proc->mangled_id : "<null>",
              match_count);
    if (stmt->stmt_data.procedure_call_data.mangled_id != NULL) {
      free(stmt->stmt_data.procedure_call_data.mangled_id);
      stmt->stmt_data.procedure_call_data.mangled_id = NULL;
    ...
    if (resolved_proc->mangled_id != NULL)
      stmt->stmt_data.procedure_call_data.mangled_id =
          strdup(resolved_proc->mangled_id);
    else if (resolved_proc->type != NULL &&
             resolved_proc->type->kind == TYPE_KIND_PROCEDURE) {
      ListNode_t *formal_params =
          kgpc_type_get_procedure_params(resolved_proc->type);
    ...
    if (stmt->stmt_data.procedure_call_data.mangled_id == NULL &&
      ...
      /* Ensure direct calls have a concrete target name even without external
       * alias */
      stmt->stmt_data.procedure_call_data.mangled_id =
          strdup(resolved_proc->id);
    }
    if (resolved_proc->type != NULL &&
        resolved_proc->type->kind == TYPE_KIND_PROCEDURE) {
    ...
    stmt->stmt_data.procedure_call_data.call_hash_type =
        resolved_proc->hash_type;
    semcheck_stmt_set_call_kgpc_type(
        stmt, resolved_proc->type,
        stmt->stmt_data.procedure_call_data.is_call_info_valid == 1);
    semcheck_stmt_set_call_owner_info(stmt, resolved_proc->owner_class,
                                      resolved_proc->method_name);
    stmt->stmt_data.procedure_call_data.is_call_info_valid = 1;
    semcheck_mark_call_requires_static_link(resolved_proc);

    /* Centralized virtual dispatch resolution — catches abstract virtual
     * methods that weren't detected by the early Self-injection check. Only
     * applies to methods without a body (abstract) and not class/static methods
     * (which use single-indirection VMT dispatch that codegen doesn't support
     * yet). */
    int resolved_param_count = -1;
    if (resolved_proc->type != NULL &&
        resolved_proc->type->kind == TYPE_KIND_PROCEDURE) {
      resolved_param_count =
          ListLength(resolved_proc->type->info.proc_info.params);
    ...
    /* Check for procedure variables (HASHTYPE_VAR) or procedure constants
     * (HASHTYPE_CONST) with a procedural type. This allows calling procedure
     * variables and typed constants that hold procedure addresses like: const
     * MyProcRef: TProc = @MyProc; */
    if (FindSymbol(&proc_var, symtab, proc_id) && proc_var != NULL &&
        (proc_var->hash_type == HASHTYPE_VAR ||
         proc_var->hash_type == HASHTYPE_CONST) &&
      ...
      if (0) /* scope depth check removed — tree scoping has no depth */
      {
        semcheck_error_with_context_at(
            stmt->line_num, stmt->col_num, stmt->source_index,
            "Error on line %d, %s cannot be called in the current context!\n\n",
      ...
      return return_val +
             semcheck_call_with_proc_var(symtab, stmt, proc_var, max_scope_lev);
    }
     ...
     */
    {
      ...
      if (any_arg_unknown) {
        ...
        semcheck_error_with_context_at(
            stmt->line_num, stmt->col_num, stmt->source_index,
            "Error on line %d, call to procedure %s%s does not match any "
            "available overload.\n"
            "Available overloads:\n%s",
        ...
        if (stmt->stmt_data.procedure_call_data.is_method_call_placeholder) {
          ...
          if (FindSymbol(&synth_node, symtab, proc_id) == 0) {
            ...
            if (synth_type != NULL) {
              (void)PushProcedureOntoScope_Typed(symtab, proc_id, proc_id,
                                                 synth_type);
    ...
    semcheck_error_with_context_at(
        stmt->line_num, stmt->col_num, stmt->source_index,
        "Error on line %d, call to procedure %s is ambiguous\n", stmt->line_num,
        proc_id);
    DestroyList(overload_candidates);
    free(mangled_name);
    return ++return_val;
  ...
  if (!scope_return) // Should not happen if match_count > 0
  {
    semcheck_error_with_context_at(
        stmt->line_num, stmt->col_num, stmt->source_index,
        "Error on line %d, unrecognized procedure call %s\n", stmt->line_num,
        proc_id);
    ++return_val;
  } else {
    if (with_context_count > 0 && proc_id != NULL && sym_return != NULL &&
        sym_return->owner_class == NULL &&
        !stmt->stmt_data.procedure_call_data.is_method_call_placeholder) {
      int try_with_override = 0;
      if (sym_return->type != NULL &&
          sym_return->type->kind == TYPE_KIND_PROCEDURE) {
        ...
        if (params != NULL && params->cur != NULL) {
          ...
          if (first_type_id != NULL && strlen(first_type_id) == 1 &&
    ...
    sym_return->referenced +=
        1; /* Moved here: only access if sym_return is valid */

    if (sym_return->type != NULL &&
        sym_return->type->kind == TYPE_KIND_PROCEDURE) {
      ListNode_t *formal_params =
          kgpc_type_get_procedure_params(sym_return->type);
    ...
    if (sym_return->hash_type != HASHTYPE_PROCEDURE &&
      ...
      semcheck_error_with_context_at(stmt->line_num, stmt->col_num,
                                     stmt->source_index,
                                     "Error on line %d, expected %s to be a "
                                     "procedure, function, or builtin!\n\n",
                                     stmt->line_num, proc_id);
    ...
    if (callee_owner_full == NULL && callee_owner_outer == NULL) {
      ...
      if (proc_def != NULL && proc_def->type == TREE_SUBPROGRAM) {
        callee_owner_full =
            proc_def->tree_data.subprogram_data.owner_class_full;
        callee_owner_outer =
    ...
    while (args_given != NULL && true_args != NULL) {
      ...
      while (true_arg_ids != NULL && args_given != NULL) {
        ...
        if (semcheck_prepare_array_literal_argument(
                arg_decl, arg_expr, symtab, INT_MAX, stmt->line_num) != 0) {
          ++return_val;
          args_given = args_given->next;
        ...
        KgpcType *expected_kgpc_type = resolve_param_type_with_owner(
            arg_decl, symtab, callee_owner_full, callee_owner_outer,
            &expected_type_owned);
        if (kgpc_getenv("KGPC_DEBUG_FMTSTR") != NULL && proc_id != NULL &&
          ...
          if (arg_decl->type == TREE_VAR_DECL) {
            fprintf(
                ...
                cur_arg, arg_decl->tree_data.var_decl_data.type,
                arg_decl->tree_data.var_decl_data.type_id
                    ? arg_decl->tree_data.var_decl_data.type_id
                    : "<null>");
          } else if (arg_decl->type == TREE_ARR_DECL) {
            fprintf(stderr,
                    "[KGPC_DEBUG_FMTSTR] param %d ARR_DECL elem_type=%d "
                    "elem_type_id=%s\n",
                    cur_arg, arg_decl->tree_data.arr_decl_data.type,
         ...
         * This is important for tracking whether Result was assigned in a
         * function. */
        ...
        semcheck_stmt_expr_tag(&dummy_type, symtab, arg_expr, INT_MAX,
                               mutate_flag);

        int arg_type_owned = 0;
        KgpcType *arg_kgpc_type = NULL;
        if (arg_expr != NULL && arg_expr->type == EXPR_ARRAY_LITERAL) {
          arg_kgpc_type = semcheck_resolve_expression_kgpc_type(
              symtab, arg_expr, INT_MAX, mutate_flag, &arg_type_owned);
        } else if (arg_expr != NULL && arg_expr->resolved_kgpc_type != NULL) {
          arg_kgpc_type = arg_expr->resolved_kgpc_type;
          arg_type_owned = 0;
        } else {
          arg_type_owned = 0;
          semcheck_expr_main(symtab, arg_expr, INT_MAX, mutate_flag,
                             &arg_kgpc_type);
        }
        ...
        if (kgpc_getenv("KGPC_DEBUG_FMTSTR") != NULL && proc_id != NULL &&
            strcasecmp(proc_id, "FmtStr") == 0) {
          fprintf(stderr, "[KGPC_DEBUG_FMTSTR] param %d expected=%s arg=%s\n",
                  cur_arg,
                  expected_kgpc_type ? kgpc_type_to_string(expected_kgpc_type)
                                     : "<null>",
                  arg_kgpc_type ? kgpc_type_to_string(arg_kgpc_type)
                                : "<null>");
        }
        int param_is_untyped = semcheck_var_decl_is_untyped(arg_decl);
        if (param_is_untyped && proc_id != NULL && cur_arg == 1 &&
            ...
            arg_expr != NULL && arg_expr->type != EXPR_ADDR &&
            arg_kgpc_type != NULL && !kgpc_type_is_pointer(arg_kgpc_type) &&
          ...
          struct Expression *addr_expr =
              mk_addressof(arg_expr->line_num, arg_expr);
          KgpcType *new_arg_kgpc_type = NULL;
          int new_arg_type_owned = 0;
          semcheck_expr_main(symtab, addr_expr, INT_MAX, NO_MUTATE,
                             &new_arg_kgpc_type);
          if (new_arg_kgpc_type == NULL) {
            new_arg_kgpc_type = semcheck_resolve_expression_kgpc_type(
                symtab, addr_expr, INT_MAX, NO_MUTATE, &new_arg_type_owned);
          }
          if (new_arg_kgpc_type != NULL &&
              kgpc_type_is_pointer(new_arg_kgpc_type)) {
        ...
        if ((expected_kgpc_type == NULL || arg_kgpc_type == NULL) &&
            !param_is_untyped) {
          ...
          if (!types_match && expected_kgpc_type != NULL &&
              ...
              expected_kgpc_type->info.points_to->info.primitive_type_tag ==
                  CHAR_TYPE &&
            ...
            if (new_arg_kgpc_type != NULL &&
                are_types_compatible_for_assignment(
                    expected_kgpc_type, new_arg_kgpc_type, symtab)) {
              args_given->cur = addr_expr;
              arg_expr = addr_expr;
           ...
           */
          if (!types_match && cur_arg == 1 && expected_kgpc_type != NULL &&
              arg_kgpc_type != NULL) {
            int expected_is_record =
                (expected_kgpc_type->kind == TYPE_KIND_RECORD);
            int given_is_ptr_record =
                (arg_kgpc_type->kind == TYPE_KIND_POINTER &&
                 arg_kgpc_type->info.points_to != NULL &&
                 arg_kgpc_type->info.points_to->kind == TYPE_KIND_RECORD);
            int expected_is_ptr_record =
                (expected_kgpc_type->kind == TYPE_KIND_POINTER &&
                 expected_kgpc_type->info.points_to != NULL &&
          ...
          if (!types_match && !param_is_var_out && expected_kgpc_type != NULL &&
            ...
            if (semcheck_try_record_conversion_expression(
                    symtab, &arg_expr, NULL, expected_kgpc_type, &arg_kgpc_type,
                    &arg_type_owned)) {
          ...
          if (types_match && expected_kgpc_type->kind == TYPE_KIND_PROCEDURE &&
              arg_kgpc_type->kind == TYPE_KIND_PROCEDURE && arg_expr != NULL &&
              arg_expr->type == EXPR_VAR_ID) {
            ...
            if (FindSymbol(&arg_node, symtab, arg_expr->expr_data.id) != 0 &&
              ...
              arg_expr->expr_data.addr_of_proc_data.proc_mangled_id =
                  arg_node->mangled_id ? strdup(arg_node->mangled_id) : NULL;
              arg_expr->expr_data.addr_of_proc_data.proc_id =
                  arg_node->id ? strdup(arg_node->id) : NULL;
              arg_expr->expr_data.addr_of_proc_data.source_unit_index =
                  arg_node->source_unit_index;
              /* Resolve the type NOW while the symbol is still alive. */
              if (arg_node->type != NULL) {
                kgpc_type_retain(arg_node->type);
                arg_expr->resolved_kgpc_type =
                    create_pointer_type(arg_node->type);
        ...
        if (!types_match && arg_expr != NULL && formal_id_dbg != NULL &&
        ...
        if (expected_kgpc_type != NULL)
          snprintf(expected_type_str, sizeof(expected_type_str), "%s",
                   kgpc_type_to_string(expected_kgpc_type));
        if (arg_kgpc_type != NULL)
          snprintf(given_type_str, sizeof(given_type_str), "%s",
                   kgpc_type_to_string(arg_kgpc_type));

        /* Check for UNKNOWN_TYPE before cleanup */
        int either_unknown =
            (kgpc_type_equals_tag(expected_kgpc_type, UNKNOWN_TYPE) ||
        ...
        if (!types_match && !either_unknown) {
          if (kgpc_getenv("KGPC_DEBUG_SYMCREAT_INSERTSYM") != NULL &&
            ...
            fprintf(stderr,
                    "[KGPC_DEBUG_SYMCREAT_INSERTSYM] arg=%d formal_id=%s "
                    "expected=%s actual=%s expr_type=%d ptr_sub=%d ptr_id=%s\n",
                    cur_arg, formal_id_dbg != NULL ? formal_id_dbg : "<null>",
                    expected_type_str, given_type_str,
          ...
          if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
            fprintf(stderr,
                    "[SemCheck] proccall %s arg %d mismatch: expected=%s "
                    "actual=%s\n",
                    proc_id ? proc_id : "<null>", cur_arg, expected_type_str,
                    given_type_str);
          }
          semcheck_error_with_context_at(
              stmt->line_num, stmt->col_num, stmt->source_index,
              ...
              stmt->line_num, proc_id, cur_arg, expected_type_str,
              given_type_str);
          ++return_val;
        }
    ...
    if (true_args == NULL && args_given != NULL) {
      ...
      if (args_given->next == NULL) {
        ...
        if (only_arg != NULL &&
            ((only_arg->type == EXPR_VAR_ID && only_arg->expr_data.id != NULL &&
    ...
    if (true_args == NULL && args_given != NULL &&
        !(sym_return != NULL && sym_return->is_varargs) &&
        !(sym_return != NULL &&
          sym_return->hash_type == HASHTYPE_BUILTIN_PROCEDURE)) {
      semcheck_error_with_context_at(stmt->line_num, stmt->col_num,
                                     stmt->source_index,
                                     "Error on line %d, on procedure call %s, "
      ...
      if (!all_have_defaults) {
        semcheck_error_with_context_at(stmt->line_num, stmt->col_num,
                                       stmt->source_index,
                                       "Error on line %d, on procedure call "
...
/* Transform TP-style New(p, Constructor(args)) into New(p) +
 * p^.Constructor(args) and Dispose(p, Destructor) into p^.Destructor +
 * Dispose(p). Returns a new statement to insert after (for New) or before (for
 * Dispose) the current statement in the list, or NULL if no transformation
 * needed. */
struct Statement *transform_two_arg_new_dispose(struct Statement *stmt,
  ...
  if (args == NULL || args->next == NULL ||
      (args->next != NULL && args->next->next != NULL))
  ...
  struct Statement *method_call =
      mk_procedurecall(stmt->line_num, placeholder_name, call_args);
  if (method_call != NULL) {
    method_call->stmt_data.procedure_call_data.is_method_call_placeholder = 1;
    method_call->stmt_data.procedure_call_data.placeholder_method_name =
        method_name;
    method_call->stmt_data.procedure_call_data.is_tp_new_dispose_helper_call =
        1;
  } else {
    free(method_name);
  }
