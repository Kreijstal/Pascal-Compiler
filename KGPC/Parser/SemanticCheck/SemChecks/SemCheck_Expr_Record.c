...
static int semcheck_ptrset_contains(const SemcheckPtrSet *set,
                                    const void *ptr) {
...
static int
semcheck_record_declares_method_name_local(const struct RecordType *record_info,
  ...
  for (ListNode_t *cur = record_info->method_templates; cur != NULL;
       cur = cur->next) {
  ...
  while (current != NULL) {
    ...
    if (current->is_type_helper && current->helper_parent_id != NULL) {
      ...
      if (FindSymbol(&parent_node, symtab, current->helper_parent_id) != 0 &&
          parent_node != NULL) {
        struct RecordType *parent_helper =
...
static void semcheck_merge_candidate_lists_dedup_seen(
    ListNode_t **combined_head, ListNode_t **combined_tail,
    ListNode_t *new_candidates, SemcheckPtrSet *seen) {
...
int node_is_record_type(HashNode_t *node) { return hashnode_is_record(node); }

struct RecordType *
semcheck_lookup_parent_record(SymTab_t *symtab,
                              struct RecordType *record_info) {
...
semcheck_find_field_in_members(ListNode_t *members, const char *field_name,
                               int include_hidden) {
  ...
  for (ListNode_t *cur = members; cur != NULL; cur = cur->next) {
    if (cur->type == LIST_RECORD_FIELD && cur->cur != NULL) {
      ...
      for (ListNode_t *b = variant->branches; b != NULL; b = b->next) {
        ...
        struct RecordField *found = semcheck_find_field_in_members(
            branch->members, field_name, include_hidden);
        if (found != NULL)
          return found;
      }
    }
...
static struct ClassProperty *semcheck_find_class_property_in_hierarchy(
  ...
  if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
    fprintf(stderr,
            "[SemCheck] semcheck_find_class_property: searching for '%s' in "
            "record_info=%p\n",
  ...
  while (current != NULL) {
    if (kgpc_getenv("KGPC_DEBUG_SYMCREAT_PROPS") != NULL &&
      ...
      for (ListNode_t *tmp = current->properties; tmp != NULL;
           tmp = tmp->next) {
        if (tmp->type == LIST_CLASS_PROPERTY && tmp->cur != NULL)
          debug_prop_count++;
      }
      fprintf(stderr,
              "[KGPC_DEBUG_SYMCREAT_PROPS] lookup property=%s on class=%s "
              "parent=%s prop_count=%d\n",
              property_name,
              current->type_id != NULL ? current->type_id : "<null>",
              current->parent_class_name != NULL ? current->parent_class_name
                                                 : "<null>",
              debug_prop_count);
    }
    /* Search both class properties and record_properties (plain advanced
     * records) */
    for (int pass = 0; pass < 2; pass++) {
      ListNode_t *node =
          (pass == 0) ? current->properties : current->record_properties;
      int prop_count = 0;
      while (node != NULL) {
        ...
        if (node->type == LIST_CLASS_PROPERTY && node->cur != NULL) {
          ...
          if (kgpc_getenv("KGPC_DEBUG_SYMCREAT_PROPS") != NULL &&
              (pascal_identifier_equals(property_name, "is_registered") ||
               ...
               pascal_identifier_equals(property_name, "realname"))) {
            fprintf(stderr,
                    "[KGPC_DEBUG_SYMCREAT_PROPS] candidate property=%s "
                    "owner=%s read=%s write=%s\n",
          ...
          if (property->name != NULL &&
            ...
            if (kgpc_getenv("KGPC_DEBUG_SYMCREAT_PROPS") != NULL &&
                (pascal_identifier_equals(property_name, "is_registered") ||
                 ...
                 pascal_identifier_equals(property_name, "realname"))) {
              fprintf(stderr,
                      "[KGPC_DEBUG_SYMCREAT_PROPS] found property=%s owner=%s "
                      "read=%s write=%s\n",
                      property_name,
                      current->type_id != NULL ? current->type_id : "<null>",
                      property->read_accessor != NULL ? property->read_accessor
      ...
      if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
        fprintf(stderr,
                "[SemCheck]   Searched %d properties (pass %d) in this record, "
                "no match\n",
                prop_count, pass);
      }
    }
...
static struct RecordField *semcheck_find_class_field_impl(
  ...
  while (current != NULL) {
    if (kgpc_getenv("KGPC_DEBUG_SYMCREAT_FIELDS") != NULL &&
      ...
      fprintf(stderr,
              "[KGPC_DEBUG_SYMCREAT_FIELDS] lookup field=%s on class=%s "
              "parent=%s\n",
              field_name,
              current->type_id != NULL ? current->type_id : "<null>",
              current->parent_class_name != NULL ? current->parent_class_name
    ...
    field = semcheck_find_field_in_members(current->fields, field_name,
                                           include_hidden);
    if (field != NULL) {
      if (kgpc_getenv("KGPC_DEBUG_SYMCREAT_FIELDS") != NULL &&
          field_name != NULL &&
          (pascal_identifier_equals(field_name, "deflist") ||
           ...
           pascal_identifier_equals(field_name, "realname"))) {
        fprintf(
            stderr, "[KGPC_DEBUG_SYMCREAT_FIELDS] found field=%s owner=%s\n",
            field_name, current->type_id != NULL ? current->type_id : "<null>");
      }
      if (owner_out != NULL)
        *owner_out = current;
      return field;
...
HashNode_t *semcheck_find_class_method(SymTab_t *symtab,
  ...
  while (current != NULL && iterations < max_iterations) {
    ...
    if (current->type_id != NULL) {
      ...
      snprintf(mangled_name, sizeof(mangled_name), "%s__%s", class_id,
               method_name);

      if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
        fprintf(stderr,
                "[SemCheck] semcheck_find_class_method: Searching for '%s' "
                "(mangled: '%s') in class '%s'\n",
                method_name, mangled_name, class_id);
      }

      HashNode_t *method_node = NULL;
      if (!FindSymbol(&method_node, symtab, mangled_name) ||
          method_node == NULL) {
        /* If not found, try looking up the class type to get its registered
         * name */
      ...
      if (method_node == NULL) {
        for (int u = 0; u < SYMTAB_MAX_UNITS; u++) {
          if (symtab->unit_scopes[u] == NULL)
            continue;
          HashNode_t *found =
              FindIdentInTable(symtab->unit_scopes[u]->table, mangled_name);
          if (found != NULL && (found->hash_type == HASHTYPE_FUNCTION ||
      ...
      if (method_node != NULL) {
        ...
        if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
          fprintf(stderr,
                  "[SemCheck] semcheck_find_class_method: Found '%s' in class "
                  "'%s'\n",
    ...
    if (current->is_type_helper && current->helper_parent_id != NULL) {
      ...
      if (FindSymbol(&parent_node, symtab, current->helper_parent_id) != 0 &&
        ...
        if (parent_helper != NULL && parent_helper->is_type_helper) {
          if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
            fprintf(stderr,
                    "[SemCheck] semcheck_find_class_method: Walking up helper "
                    "parent chain to '%s'\n",
...
void semcheck_merge_candidate_lists_dedup(ListNode_t **existing,
  ...
  semcheck_merge_candidate_lists_dedup_seen(existing, &tail, new_candidates,
                                            &seen);
...
ListNode_t *
semcheck_collect_hierarchy_method_overloads(SymTab_t *symtab,
                                            struct RecordType *start_record,
                                            const char *method_name) {
  ...
  while (current != NULL) {
    ...
    KGPC_SEMCHECK_HARD_ASSERT(iterations <= SEMCHECK_MAX_HIERARCHY_DEPTH,
                              "record hierarchy traversal exceeded %d while "
                              "collecting overloads for %s",
                              SEMCHECK_MAX_HIERARCHY_DEPTH,
                              method_name != NULL ? method_name : "<unknown>");
    KGPC_SEMCHECK_HARD_ASSERT(
        semcheck_ptrset_insert(&visited_records, current),
        "cycle detected while collecting overloads for %s (record=%s)",
        method_name != NULL ? method_name : "<unknown>",
        current->type_id != NULL ? current->type_id : "<anonymous>");
    if (current->type_id != NULL) {
      char mangled[256];
      snprintf(mangled, sizeof(mangled), "%s__%s", current->type_id,
               method_name);
