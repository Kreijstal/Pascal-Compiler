...
static struct RecordType *semcheck_resolve_record_type_from_ref(
    SymTab_t *symtab, struct TypeRef *type_ref, const char *type_id) {
  if (symtab == NULL || (type_ref == NULL && type_id == NULL))
    return NULL;
  ...
  if (type_node == NULL)
    type_node =
        semcheck_find_type_node_with_kgpc_type_ref(symtab, type_ref, type_id);
  ...
  if (type_node != NULL) {
    ...
    if (alias != NULL) {
      ...
      if (alias->target_type_id != NULL || alias->target_type_ref != NULL) {
        ...
        if (target_node != NULL) {
          ...
          if (target_node->type != NULL &&
            ...
            return kgpc_type_get_record(target_node->type);
        }
      }
    }
  }
  ...
  if (type_id != NULL)
    return semcheck_lookup_record_type(symtab, type_id);
...
int semcheck_typecheck_array_literal(struct Expression *expr, SymTab_t *symtab,
  ...
  if (kgpc_getenv("KGPC_DEBUG_ARRAY_LITERAL") != NULL) {
    fprintf(stderr,
            "[KGPC] typecheck array literal @ line %d: expected_type=%d "
            "expected_id=%s\n",
            line_num, expected_type,
            expected_type_id != NULL ? expected_type_id : "<null>");
  }

  if (expected_type != UNKNOWN_TYPE && expr->array_element_type == UNKNOWN_TYPE)
    expr->array_element_type = expected_type;
  if (expr->array_element_type_id == NULL && expected_type_id != NULL)
    expr->array_element_type_id = strdup(expected_type_id);
  ...
  if (expected_type == ARRAY_OF_CONST_TYPE) {
    ...
    expr->array_lower_bound = 0;
    expr->array_upper_bound =
        expr->expr_data.array_literal_data.element_count - 1;
    expr->is_array_expr = 1;
    expr->array_is_dynamic = 1;
  ...
  const char *resolved_type_id =
      expected_type_id != NULL ? expected_type_id : expr->array_element_type_id;
  struct RecordType *expected_record = semcheck_resolve_record_type_from_ref(
      symtab, expr->array_element_type_ref, resolved_type_id);
  if (expected_record == NULL && expected_type == RECORD_TYPE &&
      expr->array_element_record_type != NULL)
    expected_record = expr->array_element_record_type;
  if (expected_record != NULL) {
    expr->array_element_record_type = expected_record;
  ...
  while (cur != NULL) {
    ...
    if (element_expr->type == EXPR_ARRAY_LITERAL) {
      if (expected_type != UNKNOWN_TYPE) {
        ...
        if (element_expr->array_element_type_id == NULL &&
            expected_type_id != NULL)
          element_expr->array_element_type_id = strdup(expected_type_id);
      }
      int nested_err = semcheck_typecheck_array_literal(
          element_expr, symtab, max_scope_lev, expected_type, expected_type_id,
          line_num);
      if (nested_err != 0)
    ...
    if (expected_record != NULL &&
        element_expr->type == EXPR_RECORD_CONSTRUCTOR) {
      error_count += semcheck_typecheck_record_constructor(
          element_expr, symtab, max_scope_lev, expected_record, line_num);
      ++index;
      cur = cur->next;
      continue;
    }
    ...
    error_count += semcheck_expr_with_type(
        &element_kgpc_type, symtab, element_expr, max_scope_lev, NO_MUTATE);
    ...
    if (error_count == 0 &&
      ...
      if (expected_type_id != NULL && symtab != NULL) {
        HashNode_t *expected_type_node =
            semcheck_find_preferred_type_node(symtab, expected_type_id);
      ...
      if (is_integer_type(expected_type) && is_integer_type(element_type)) {
        if (element_expr->type == EXPR_INUM) {
          if (integer_literal_fits_type(element_expr->expr_data.i_num,
                                        expected_type))
            compatible = 1;
        } else {
          compatible = 1;
        ...
        compatible = 1;
                 ...
                 are_types_compatible_for_assignment(
                     expected_element_kgpc, actual_element_kgpc, symtab)) {
        compatible = 1;
      }
      ...
      if (!compatible) {
        if (kgpc_getenv("KGPC_DEBUG_ARRAY_LITERAL") != NULL) {
          fprintf(stderr,
                  "[KGPC] array literal mismatch @ line %d index %d: "
                  "expected=%d (%s) got=%d\n",
                  line_num, index, expected_type,
                  expected_type_id != NULL ? expected_type_id : "<null>",
                  element_type);
        }
        semcheck_error_with_context(
            "Error on line %d, element %d of array literal "
  ...
  if (expr->array_element_size <= 0) {
    ...
    if (sizeof_from_type_ref(symtab, expected_type, expected_type_id,
                             &element_size, 0, line_num) == 0 &&
        element_size > 0 && element_size <= INT_MAX) {
...
int semcheck_prepare_array_literal_argument(Tree_t *formal_decl,
  ...
  if (formal_decl->type == TREE_VAR_DECL && expected_type_id != NULL &&
      symtab != NULL) {
    HashNode_t *type_node =
        semcheck_find_preferred_type_node(symtab, expected_type_id);
    if (type_node != NULL)
    ...
    if (expected_kgpc != NULL && expected_kgpc->kind == TYPE_KIND_ARRAY) {
      ...
      if (elem != NULL && elem->kind == TYPE_KIND_PRIMITIVE &&
        ...
        if (elem->type_alias != NULL && elem->type_alias->alias_name != NULL) {
          ...
          owned_expected_type_id =
              strdup(elem->info.points_to->type_alias->alias_name);
          expected_type_id = owned_expected_type_id;
        }
  ...
  if (kgpc_getenv("KGPC_DEBUG_ARRAY_LITERAL") != NULL &&
    ...
    fprintf(stderr,
            "[KGPC] array literal @ line %d: formal_decl=%s expected_type=%d "
            "expected_id=%s array_of_const=%d\n",
            line_num,
            formal_decl->type == TREE_ARR_DECL
                ? "ARR_DECL"
  ...
  if (arg_expr->type == EXPR_SET) {
    ...
    if (semcheck_convert_set_literal_to_array_literal(arg_expr) != 0) {
      semcheck_error_with_context(
          "Error on line %d, open array literal cannot contain ranges.\n",
          line_num);
      free(owned_expected_type_id);
      return 1;
    }
  ...
  if (expected_is_array_of_const) {
    ...
    arg_expr->array_upper_bound =
        arg_expr->expr_data.array_literal_data.element_count - 1;
...
struct RecordType *semcheck_record_type_from_decl(Tree_t *decl,
...

  {
    ...
    if (resolved != NULL) {
      ...
      else if (kgpc_type_is_pointer(resolved) &&
               resolved->info.points_to != NULL &&
  ...
  if (decl->type == TREE_VAR_DECL) {
    ...
    if (decl->tree_data.var_decl_data.type_id != NULL)
      return semcheck_lookup_record_type(symtab,
                                         decl->tree_data.var_decl_data.type_id);
  }
...
KgpcType *semcheck_field_expected_kgpc_type(SymTab_t *symtab,
  ...
  if (field->is_array) {
    ...
    if (field->array_element_kgpc_type != NULL) {
    ...
    } else if (field->array_element_type == RECORD_TYPE &&
               field->nested_record != NULL) {
      ...
      type_node = semcheck_find_preferred_type_node_with_ref(
          symtab, field->array_element_type_ref, field->array_element_type_id);
      if (type_node == NULL)
        type_node = semcheck_find_type_node_with_kgpc_type_ref(
            symtab, field->array_element_type_ref,
    ...
    if (element_type == NULL)
      return NULL;
    return create_array_type(element_type, field->array_start,
                             field->array_end);
  }
  ...
  if (field->is_pointer) {
    ...
    if (field->pointer_type_id != NULL) {
      HashNode_t *type_node = semcheck_find_preferred_type_node_with_ref(
          symtab, field->pointer_type_ref, field->pointer_type_id);
      if (type_node == NULL)
        type_node = semcheck_find_type_node_with_kgpc_type_ref(
            symtab, field->pointer_type_ref, field->pointer_type_id);
      if (type_node != NULL) {
  ...
  if (field->type_id != NULL) {
    HashNode_t *type_node = NULL;
    type_node = semcheck_find_preferred_type_node_with_ref(
        symtab, field->type_ref, field->type_id);
    if (type_node == NULL)
      type_node = semcheck_find_type_node_with_kgpc_type_ref(
          symtab, field->type_ref, field->type_id);
    if (type_node != NULL) {
...
int semcheck_typecheck_record_constructor(struct Expression *expr,
  ...
  while (cur != NULL) {
    ...
    for (ListNode_t *prev = expr->expr_data.record_constructor_data.fields;
      ...
      struct RecordConstructorField *prior =
          (struct RecordConstructorField *)prev->cur;
      if (prior != NULL && prior->field_id != NULL &&
          pascal_identifier_equals(prior->field_id, field->field_id)) {
        semcheck_error_with_context(
            "Error on line %d, duplicate record constructor field %s.\n",
    ...
    if (resolve_record_field(symtab, record_type, field->field_id, &field_desc,
                             &field_offset, line_num, 0) != 0 ||
        field_desc == NULL) {
      ++error_count;
      cur = cur->next;
      continue;
    ...
    if (field_desc->nested_record == NULL &&
      ...
      if (type_node == NULL)
        type_node = semcheck_find_type_node_with_kgpc_type_ref(
            symtab, field_desc->type_ref, field_desc->type_id);
      if (type_node != NULL) {
        resolved_record = get_record_type_from_node(type_node);
          ...
          if (alias != NULL && alias->target_type_id != NULL) {
            ...
            if (target_node == NULL)
              target_node = semcheck_find_type_node_with_kgpc_type_ref(
                  symtab, alias->target_type_ref, alias->target_type_id);
            if (target_node != NULL)
              resolved_record = get_record_type_from_node(target_node);
          }
        }
      }
      if (resolved_record == NULL && field_desc->type_id != NULL)
        resolved_record =
    ...
    if (field->field_record_type == NULL) {
      KgpcType *expected_type =
          semcheck_field_expected_kgpc_type(symtab, field_desc);
      if (expected_type != NULL) {
        if (kgpc_type_is_record(expected_type)) {
          field->field_record_type = kgpc_type_get_record(expected_type);
          field->field_type = RECORD_TYPE;
        }
    ...
    if (field_desc->array_element_type == RECORD_TYPE &&
    ...
    if (field->array_element_record_type == NULL &&
      ...
      struct RecordType *resolved_elem = semcheck_lookup_record_type(
    ...
    if (field->array_element_record_type == NULL &&
        (field_desc->array_element_type_ref != NULL ||
         field_desc->array_element_type_id != NULL)) {
      ...
      if (elem_node == NULL)
        elem_node = semcheck_find_type_node_with_kgpc_type_ref(
            ...
            field_desc->array_element_type_id);
      if (elem_node != NULL) {
        struct RecordType *resolved_elem = get_record_type_from_node(elem_node);
        if (resolved_elem == NULL) {
          if (elem_node->type != NULL && kgpc_type_is_record(elem_node->type))
            resolved_elem = kgpc_type_get_record(elem_node->type);
        }
        if (resolved_elem == NULL) {
          struct TypeAlias *alias = get_type_alias_from_node(elem_node);
          if (alias != NULL) {
            if (alias->inline_record_type != NULL)
              resolved_elem = alias->inline_record_type;
          }
          if (alias != NULL && resolved_elem == NULL &&
            ...
            if (target_node == NULL)
              target_node = semcheck_find_type_node_with_kgpc_type_ref(
                  symtab, alias->target_type_ref, alias->target_type_id);
            if (target_node != NULL)
              resolved_elem = get_record_type_from_node(target_node);
            if (resolved_elem == NULL && target_node != NULL &&
                target_node->type != NULL &&
                kgpc_type_is_record(target_node->type)) {
              resolved_elem = kgpc_type_get_record(target_node->type);
            }
          }
        }
        if (resolved_elem != NULL) {
          ...
          if (field->array_element_type_id == NULL &&
              resolved_elem->type_id != NULL)
            field->array_element_type_id = strdup(resolved_elem->type_id);
        }
      }
    }
    ...
    if (field->value->type == EXPR_RECORD_CONSTRUCTOR &&
      ...
      if (field->field_type == RECORD_TYPE) {
        if (field->field_record_type != NULL) {
          ...
          KgpcType *nested_record_type =
              create_record_type(field->field_record_type);
          if (nested_record_type != NULL) {
            semcheck_expr_set_resolved_kgpc_type_shared(field->value,
                                                        nested_record_type);
            destroy_kgpc_type(nested_record_type);
          }
        }
      } else if (field->field_is_array &&
                 field->array_element_type == RECORD_TYPE) {
        if (field->array_element_record_type != NULL) {
          field->value->record_type = field->array_element_record_type;
          KgpcType *nested_record_type =
              create_record_type(field->array_element_record_type);
          if (nested_record_type != NULL) {
            semcheck_expr_set_resolved_kgpc_type_shared(field->value,
                                                        nested_record_type);
            destroy_kgpc_type(nested_record_type);
    ...
    if (field->field_is_array && field->value != NULL) {
      if (field->value->type == EXPR_SET) {
        if (semcheck_convert_set_literal_to_array_literal(field->value) != 0) {
          semcheck_error_with_context(
              "Error on line %d, array field %s cannot use set ranges.\n",
              line_num, field->field_id);
          ++error_count;
        }
      }
      ...
      if (field->value->type == EXPR_ARRAY_LITERAL) {
        ...
        if (field->value->array_element_type_id == NULL &&
          ...
          field->value->array_element_type_id =
              strdup(field->array_element_type_id);
        if (field->value->array_element_record_type == NULL &&
            field->array_element_record_type != NULL) {
          field->value->array_element_record_type =
              field->array_element_record_type;
        }

        int arr_err = semcheck_typecheck_array_literal(
            field->value, symtab, max_scope_lev, field->array_element_type,
            field->array_element_type_id, line_num);
        if (arr_err != 0) {
          error_count += arr_err;
        } else if (!field->array_is_open) {
          int expected_count = field->array_end - field->array_start + 1;
          int actual_count =
              field->value->expr_data.array_literal_data.element_count;
          if (expected_count != actual_count) {
            fprintf(stderr,
                    "Error on line %d, array field %s expects %d elements, got "
                    "%d.\n",
    ...
    error_count += semcheck_expr_with_type(
        &value_kgpc_type, symtab, field->value, max_scope_lev, NO_MUTATE);
    ...
    if (expected_type == NULL) {
      ...
      if (value_type_kgpc == NULL) {
        ...
        ++error_count;
                     ...
                     expected_type, value_type_kgpc, symtab)) {
        fprintf(
            stderr,
            "Error on line %d, incompatible types in record constructor for %s "
            "(expected %s, got %s).\n",
            line_num, field->field_id, kgpc_type_to_string(expected_type),
...
int semcheck_prepare_record_constructor_argument(Tree_t *formal_decl,
                                                 struct Expression *arg_expr,
                                                 SymTab_t *symtab,
                                                 int max_scope_lev,
                                                 int line_num) {
  ...
  struct RecordType *record_type =
      semcheck_record_type_from_decl(formal_decl, symtab);
  if (record_type == NULL) {
    semcheck_error_with_context(
        "Error on line %d, unable to infer record type for constructor.\n",
        line_num);
