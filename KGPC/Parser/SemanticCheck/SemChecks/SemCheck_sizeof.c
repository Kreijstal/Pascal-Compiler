...
static int sizeof_from_record_members(SymTab_t *symtab, ListNode_t *members,
                                      long long *size_out, int is_packed,
                                      int depth, int line_num);
static int sizeof_from_variant_part(SymTab_t *symtab,
                                    struct VariantPart *variant,
                                    long long *size_out, int is_packed,
                                    int depth, int line_num);
static int find_field_in_members(SymTab_t *symtab, ListNode_t *members,
                                 const char *field_name,
                                 struct RecordField **out_field,
                                 long long *offset_out, long long start_offset,
                                 int is_packed, int depth, int line_num,
                                 int *found);
static int find_field_in_variant(SymTab_t *symtab, struct VariantPart *variant,
                                 const char *field_name,
                                 struct RecordField **out_field,
                                 long long *offset_out, int is_packed,
...
static int get_field_alignment(SymTab_t *symtab, struct RecordField *field,
                               int depth, int line_num);
static int get_variant_part_alignment(SymTab_t *symtab,
...
static int get_type_alignment_from_ref(SymTab_t *symtab, int type_tag,
                                       const char *type_id, int *align_out,
                                       int depth, int line_num);
...
static int parse_or_resolve_bound(SymTab_t *symtab, const char *text,
                                  long long *out) {
...
static inline struct RecordType *get_record_type_from_node(HashNode_t *node) {
  ...
  /* If not a direct record, check if it's a pointer to a record (Class types
   * are pointers) */
...
static int sizeof_from_array_type_kgpc(SymTab_t *symtab, KgpcType *type,
                                       long long *size_out) {
  if (symtab == NULL || type == NULL || size_out == NULL ||
      !kgpc_type_is_array(type))
    return 1;
...
static long long fpc_set_storage_size_from_alias(SymTab_t *symtab,
                                                 struct TypeAlias *alias,
                                                 int depth, int line_num) {
  ...
  } else if (alias->set_element_type_id != NULL && symtab != NULL &&
             depth <= SIZEOF_RECURSION_LIMIT) {
    HashNode_t *elem_node =
        semcheck_find_preferred_type_node(symtab, alias->set_element_type_id);
    if (elem_node != NULL) {
      ...
      if (elem_alias != NULL) {
        if (elem_alias->is_enum && elem_alias->enum_literals != NULL) {
          ...
          if (count > 0)
            result =
                fpc_default_set_storage_size_for_high((long long)count - 1);
...
int sizeof_from_type_ref(SymTab_t *symtab, int type_tag, const char *type_id,
  ...
  if (depth > SIZEOF_RECURSION_LIMIT) {
    semcheck_error_with_context("Error on line %d, SizeOf exceeded recursion "
                                "depth while resolving type.\n",
                                line_num);
    return 1;
  }
  ...
  if (type_id != NULL) {
    ...
    if (target_node == NULL) {
      /* For generic type parameters or nested types that haven't been resolved
       * yet, treat as pointer-sized rather than hard error - this allows:
       * - generic templates to be processed without full instantiation
       * - FPC bootstrap where nested types (Public type ...) aren't supported
       * yet Nested procedural types and class references are typically
       * pointer-sized */
      const char *debug_env = kgpc_getenv("KGPC_DEBUG_TFPG");
      if (debug_env != NULL) {
        fprintf(stderr,
                "[KGPC] SizeOf: unknown type %s at line %d (may be unresolved "
                "generic parameter or nested type)\n",
                type_id, line_num);
      }
      *size_out =
          POINTER_SIZE_BYTES; /* Assume pointer-sized for unknown types */
      return 0; /* Return success to allow processing to continue */
    }
    return sizeof_from_hashnode(symtab, target_node, size_out, depth + 1,
                                line_num);
  }

  /* If both type_tag and type_id are unspecified, we can't compute size.
   * This might happen for generic type parameters - don't print error, just
   * fail gracefully */
  const char *debug_env = kgpc_getenv("KGPC_DEBUG_TFPG");
  if (debug_env != NULL) {
    fprintf(stderr,
            "[KGPC] SizeOf: unable to resolve type at line %d (unspecified "
            "type_tag and type_id)\n",
            line_num);
  }
  *size_out = 0;
...
int sizeof_from_record(SymTab_t *symtab, struct RecordType *record,
  ...
  if (depth > SIZEOF_RECURSION_LIMIT) {
    semcheck_error_with_context("Error on line %d, SizeOf exceeded recursion "
                                "depth while resolving record type.\n",
                                line_num);
    return 1;
  }
  ...
  if (record->parent_class_name != NULL && !record->parent_fields_merged &&
    ...
    HashNode_t *parent_node =
        semcheck_find_preferred_type_node(symtab, record->parent_class_name);
    struct RecordType *parent_record = get_record_type_from_node(parent_node);
    if (parent_record != NULL) {
      if (sizeof_from_record(symtab, parent_record, &computed_size, depth + 1,
                             line_num) != 0) {
        if (!was_cached)
  ...
  *size_out = computed_size;
  if (computed_size > 1000000) {
    ...
    if (debug_env != NULL)
      fprintf(stderr,
              "[KGPC_SIZE] Large class/record size: %lld for type_id=%s "
              "is_class=%d parent=%s\n",
              computed_size, record->type_id ? record->type_id : "<null>",
              record->is_class,
              record->parent_class_name ? record->parent_class_name : "<none>");
  }
  return 0;
}
...
static int get_record_alignment(SymTab_t *symtab, struct RecordType *record,
  ...
  if (record->parent_class_name != NULL && !record->parent_fields_merged &&
    ...
    if (parent_record != NULL) {
      int parent_align =
          get_record_alignment(symtab, parent_record, depth + 1, line_num);
...
static int get_field_alignment(SymTab_t *symtab, struct RecordField *field,
  ...
  if (field->is_array) {
    if (field->array_element_type == RECORD_TYPE &&
      ...
      return get_record_alignment(symtab, field->array_element_record,
                                  depth + 1, line_num);

    {
      ...
      if (field->array_element_type != UNKNOWN_TYPE ||
        ...
        status = get_type_alignment_from_ref(
            symtab, field->type, field->type_id, &align, depth + 1, line_num);
      }
      if (status != 0 || align <= 0)
  ...
  if (field->nested_record != NULL)
    return get_record_alignment(symtab, field->nested_record, depth + 1,
                                line_num);
  ...
  {
    int align = 1;
    int status = get_type_alignment_from_ref(
        symtab, field->type, field->type_id, &align, depth + 1, line_num);
    if (status != 0 || align <= 0)
      return 1; /* Unresolvable type — use minimum alignment */
    return align;
...
static int compute_field_size_uncached(SymTab_t *symtab,
                                       struct RecordField *field,
  ...
  if (debug_env != NULL && field->name != NULL) {
    fprintf(
        stderr,
        "[KGPC] compute_field_size: field=%s type=%d type_id=%s is_array=%d\n",
        field->name, field->type, field->type_id ? field->type_id : "<null>",
        field->is_array);
  }

  if (depth > SIZEOF_RECURSION_LIMIT) {
    semcheck_error_with_context("Error on line %d, SizeOf exceeded recursion "
                                "depth while resolving record field.\n",
                                line_num);
    return 1;
  }
  ...
  if (field->is_array) {
    ...
    if (debug_env != NULL && field->name != NULL) {
      fprintf(stderr,
              "[KGPC] compute_field_size array: field=%s is_open=%d start=%d "
              "end=%d\n",
              field->name, field->array_is_open, field->array_start,
              field->array_end);
    }
    ...
    if (count < 0) {
      semcheck_error_with_context(
          "Error on line %d, invalid bounds for array field %s.\n", line_num,
  ...
  if (field->nested_record != NULL)
    return sizeof_from_record(symtab, field->nested_record, size_out, depth + 1,
                              line_num);
  ...
  return sizeof_from_type_ref(symtab, field->type, field->type_id, size_out,
                              depth + 1, line_num);
}
...
static int compute_field_size(SymTab_t *symtab, struct RecordField *field,
  ...
  int result =
      compute_field_size_uncached(symtab, field, size_out, depth, line_num);
  if (result == 0 && field != NULL && !field->has_cached_layout) {
    field->cached_size = (size_out != NULL) ? *size_out : 0;
    field->cached_alignment =
        get_field_alignment(symtab, field, depth, line_num);
...
static int get_record_members_alignment(SymTab_t *symtab, ListNode_t *members,
  ...
  while (cur != NULL) {
    if (cur->type == LIST_RECORD_FIELD && cur->cur != NULL) {
      ...
      if (field != NULL && !field->is_class_var) {
        int field_align =
            get_field_alignment(symtab, field, depth + 1, line_num);
...
static int sizeof_from_record_members(SymTab_t *symtab, ListNode_t *members,
  ...
  while (cur != NULL) {
    if (cur->type == LIST_RECORD_FIELD) {
      ...
      int field_result =
          compute_field_size(symtab, field, &field_size, depth + 1, line_num);
      if (field_result != 0) {
#ifdef DEBUG
        fprintf(stderr,
                "DEBUG: compute_field_size FAILED for field %s: result=%d\n",
      ...
      if (sizeof_from_variant_part(symtab, variant, &variant_size, is_packed,
                                   depth + 1, line_num) != 0)
        return 1;
      total += variant_size;
    }
...
static int sizeof_from_variant_part(SymTab_t *symtab,
  ...
  if (depth > SIZEOF_RECURSION_LIMIT) {
    semcheck_error_with_context("Error on line %d, SizeOf exceeded recursion "
                                "depth while resolving variant part.\n",
                                line_num);
    return 1;
  }
  ...
  while (cur != NULL) {
    if (cur->type == LIST_VARIANT_BRANCH) {
      ...
      if (find_field_in_members(symtab, branch->members, field_name, out_field,
                                &branch_offset, 0, is_packed, depth + 1,
                                line_num, &branch_found) != 0)
        return 1;
      if (branch_found) {
        if (offset_out != NULL)
...
static int find_field_in_members(SymTab_t *symtab, ListNode_t *members,
  ...
  long long classvar_offset = 0; /* separate running offset for class var fields
                                    within CLASSVAR storage */
  ListNode_t *cur = members;
  while (cur != NULL) {
    if (cur->type == LIST_RECORD_FIELD) {
      struct RecordField *field = (struct RecordField *)cur->cur;
      if (field != NULL && !record_field_is_hidden(field)) {
        ...
        if (field->is_class_var) {
          ...
          (void)compute_field_size(symtab, field, &cv_field_size, depth + 1,
                                   line_num);
          if (cv_field_size <= 0)
          ...
          int cv_alignment = (cv_field_size >= 8)   ? 8
                             : (cv_field_size >= 4) ? 4
                                                    : 1;
          classvar_offset =
              (classvar_offset + cv_alignment - 1) & ~(cv_alignment - 1);
      ...
      long long field_size = 0;
      if (compute_field_size(symtab, field, &field_size, depth + 1, line_num) !=
          0)
        return 1;
      offset += field_size;
    } else if (cur->type == LIST_VARIANT_PART) {
      ...
      if (find_field_in_variant(symtab, variant, field_name, out_field,
                                &variant_field_offset, is_packed, depth + 1,
                                line_num, &variant_found) != 0)
        return 1;
      if (variant_found) {
        if (offset_out != NULL)
...
int sizeof_from_alias(SymTab_t *symtab, struct TypeAlias *alias,
  ...
  if (alias == NULL) {
    semcheck_error_with_context(
        "Error on line %d, SizeOf encountered incomplete type alias.\n",
        line_num);
    return 1;
  }
  ...
  if (depth > SIZEOF_RECURSION_LIMIT) {
    semcheck_error_with_context("Error on line %d, SizeOf exceeded recursion "
                                "depth while resolving type alias.\n",
                                line_num);
    return 1;
  }
  ...
  if (alias->is_array) {
    ...
    if (alias->alias_name != NULL) {
      HashNode_t *alias_node =
          semcheck_find_preferred_type_node(symtab, alias->alias_name);
      if (alias_node != NULL && alias_node->type != NULL &&
          kgpc_type_is_array(alias_node->type) &&
          sizeof_from_array_type_kgpc(symtab, alias_node->type, size_out) ==
              0 &&
          *size_out >= 0) {
        return 0;
      }
    ...
    if (count < 0) {
      semcheck_error_with_context(
          "Error on line %d, invalid bounds for array type in SizeOf.\n",
          line_num);
      return 1;
    }
  ...
  if (alias->is_set) {
    *size_out =
        fpc_set_storage_size_from_alias(symtab, alias, depth + 1, line_num);
    return 0;
  }
  ...
  if (alias->base_type != UNKNOWN_TYPE) {
    return sizeof_from_type_ref(symtab, alias->base_type, NULL, size_out,
                                depth + 1, line_num);
  }
  ...
  semcheck_error_with_context(
      "Error on line %d, SizeOf encountered unresolved type alias.\n",
      line_num);
  return 1;
}
...
static int get_type_alignment_from_ref(SymTab_t *symtab, int type_tag,
  ...
  if (type_id != NULL && symtab != NULL) {
    ...
    if (type_node != NULL) {
      ...
      if (alias != NULL) {
        if (alias->is_set) {
          long long set_size = fpc_set_storage_size_from_alias(
              symtab, alias, depth + 1, line_num);
          int align = fpc_size_to_alignment(set_size);
          *align_out =
              (align > POINTER_SIZE_BYTES) ? POINTER_SIZE_BYTES : align;
          return 0;
        }

        if (alias->is_enum) {
          *align_out =
              fpc_size_to_alignment(fpc_enum_storage_size_from_alias(alias));
          return 0;
        }
        ...
        if (alias->is_array) {
          ...
          if (alias->array_element_type_id != NULL) {
            int status = get_type_alignment_from_ref(
                symtab, alias->array_element_type, alias->array_element_type_id,
          ...
          /* Fallback to determining alignment from the type tag and size. */
            ...
            long long elem_size =
                sizeof_from_type_tag(alias->array_element_type);
            if (elem_size > 0)
              elem_align = fpc_type_alignment_from_size(
                  elem_size, alias->array_element_type);
          }
      ...
      if (type_node->type != NULL) {
        ...
        if (size > 0) {
          *align_out = fpc_type_alignment_from_size(
              size, kgpc_type_get_primitive_tag(type_node->type));
...
int sizeof_from_hashnode(SymTab_t *symtab, HashNode_t *node,
  ...
  if (node == NULL) {
    semcheck_error_with_context(
        "Error on line %d, SizeOf encountered null symbol information.\n",
        line_num);
    return 1;
  }
  ...
  if (is_array) {
    ...
    if (is_dynamic) {
      semcheck_error_with_context("Error on line %d, SizeOf cannot determine "
                                  "size of dynamic array %s.\n",
                                  line_num, node->id);
      return 1;
    }
    ...
    if (element_size <= 0) {
      semcheck_error_with_context(
          "Error on line %d, cannot determine element size for array %s.\n",
          line_num, node->id);
      return 1;
    }
    ...
    if (count < 0) {
      semcheck_error_with_context(
          "Error on line %d, invalid bounds for array %s in SizeOf.\n",
          line_num, node->id);
      return 1;
    }
  ...
  semcheck_error_with_context(
      "Error on line %d, SizeOf cannot determine size of %s.\n", line_num,
      node->id != NULL ? node->id : "symbol");
  return 1;
}
...
int resolve_record_field(SymTab_t *symtab, struct RecordType *record,
  ...
  while (current != NULL && depth < 32) /* guard against infinite loops */
  {
    ...
    if (current->parent_class_name != NULL && !current->parent_fields_merged &&
        symtab != NULL) {
      HashNode_t *parent_node =
          semcheck_find_preferred_type_node(symtab, current->parent_class_name);
      struct RecordType *parent_record = get_record_type_from_node(parent_node);
      if (parent_record != NULL &&
          sizeof_from_record(symtab, parent_record, &start_offset, 0,
                             line_num) != 0)
        return 1;
    }
    if (find_field_in_members(symtab, current->fields, field_name, out_field,
                              &offset, start_offset, current->is_packed, 0,
                              line_num, &found) != 0)
      return 1;
