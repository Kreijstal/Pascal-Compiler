...
int semcheck_resolve_overload(HashNode_t **best_match_out, int *num_best_out,
                              ListNode_t *overload_candidates,
                              ListNode_t *args_given, SymTab_t *symtab,
                              struct Expression *call_expr, int max_scope_lev,
...
static const char *
semcheck_contextual_enum_literal_name(struct Expression *expr) {
...
static int semcheck_resolve_enum_literal_from_alias(
    SymTab_t *symtab, const struct TypeAlias *alias, const char *literal,
    long long *out_value) {
  if (symtab == NULL || alias == NULL || literal == NULL || out_value == NULL)
    return 0;

  if (alias->alias_name != NULL &&
      semcheck_resolve_scoped_enum_literal(symtab, alias->alias_name, literal,
                                           out_value)) {
    return 1;
  }
  ...
  if (alias->target_type_ref != NULL && alias->target_type_ref->name != NULL &&
      semcheck_resolve_scoped_enum_literal_ref(
          symtab, alias->target_type_ref->name, literal, out_value)) {
    return 1;
  }
...
static void semcheck_bind_set_literal_to_left_enum(SymTab_t *symtab,
  ...
  for (ListNode_t *cur = set_expr->expr_data.set_data.elements; cur != NULL;
       cur = cur->next) {
...
typedef enum {
  ...
  BINOP_MATCH_COMPATIBLE =
      1, /* are_types_compatible_for_assignment: implicit conversion ok */
...
static HashNode_t *semcheck_find_best_binary_operator_candidate(
  ...
  int best_quality =
      -1; /* sum of BinopOperandMatch values for the best candidate */
  ...
  for (ListNode_t *cur = candidates; cur != NULL; cur = cur->next) {
    ...
    int owns_left = 0;
    int owns_right = 0;
    KgpcType *left_formal =
        resolve_type_from_vardecl(left_decl, symtab, &owns_left);
    KgpcType *right_formal =
        resolve_type_from_vardecl(right_decl, symtab, &owns_right);
    ...
    if (left_formal != NULL && left_arg_type != NULL) {
      ...
      else if (are_types_compatible_for_assignment(left_formal, left_arg_type,
                                                   symtab))
    ...
    if (valid && right_formal != NULL && right_arg_type != NULL) {
      if (kgpc_type_equals(right_formal, right_arg_type))
        quality += BINOP_MATCH_EXACT;
      else if (are_types_compatible_for_assignment(right_formal, right_arg_type,
                                                   symtab))
        quality += BINOP_MATCH_COMPATIBLE;
      else
        valid = 0;
...
static int semcheck_find_ident_by_prefix_visible(HashNode_t **hash_return,
  ...
  for (ScopeNode *cur = symtab->current_scope; cur != NULL; cur = cur->parent) {
    HashNode_t *node =
        FindIdentByPrefixInTableForUnit(cur->table, prefix, caller_unit_index);
    if (node != NULL) {
      *hash_return = node;
      return return_val;
    }
  ...
  /* 2. Unit scopes — own unit first, then dependency units. */
  {
    ...
    if (caller_unit_index > 0 && caller_unit_index < SYMTAB_MAX_UNITS &&
      ...
      HashNode_t *node = FindIdentByPrefixInTableForUnit(
          symtab->unit_scopes[caller_unit_index]->table, prefix,
          caller_unit_index);
      if (node != NULL) {
        *hash_return = node;
        return global_level;
    ...
    /* 2c. When current scope is program-level (unit_index == 0), search all
     * unit scopes. */
  ...
  {
    HashNode_t *node =
        FindIdentByPrefixInTable(symtab->builtin_scope->table, prefix);
    if (node != NULL) {
      *hash_return = node;
      return return_val;
    }
...
static int semcheck_candidate_returns_bool(SymTab_t *symtab,
  ...
  if (candidate->type->info.proc_info.return_type_id != NULL) {
    HashNode_t *ret_node = semcheck_find_preferred_type_node(
        symtab, candidate->type->info.proc_info.return_type_id);
    if (ret_node != NULL && ret_node->type != NULL)
      return semcheck_tag_from_kgpc(ret_node->type) == BOOL;

    return semcheck_map_builtin_type_name(
               symtab, candidate->type->info.proc_info.return_type_id) == BOOL;
  }

  return 0;
}

static int semcheck_try_refine_funccall_to_bool(SymTab_t *symtab,
  ...
  ListNode_t *all_candidates =
      FindAllIdents(symtab, call_expr->expr_data.function_call_data.id);
  ...
  int resolve_status = semcheck_resolve_overload(
      &best_match, &num_best, bool_candidates,
      call_expr->expr_data.function_call_data.args_expr, symtab, call_expr,
      max_scope_lev, 1);
  ...
  call_expr->expr_data.function_call_data.mangled_id =
      best_match->mangled_id != NULL ? strdup(best_match->mangled_id)
                                     : strdup(best_match->id);
...
static void semcheck_set_result_expr_metadata(struct Expression *expr,
                                              SymTab_t *symtab,
                                              KgpcType *type) {
  if (expr == NULL)
    return;
...
static int semcheck_normalize_result_identifier(struct Expression *expr) {
  ...
  if (expr->expr_data.id != NULL &&
      pascal_identifier_equals(expr->expr_data.id, replacement))
    return 0;
...
static HashNode_t *semcheck_find_preferred_value_ident(SymTab_t *symtab,
                                                       const char *id,
                                                       int *scope_out) {
...
static int semcheck_expr_is_char_array_like(const struct Expression *expr) {
  ...
  if (kgpc->info.array_info.element_type->kind != TYPE_KIND_PRIMITIVE)
    return 0;
  return kgpc->info.array_info.element_type->info.primitive_type_tag ==
         CHAR_TYPE;
}

static HashNode_t *semcheck_pick_type_node_with_origin_preference(
    SymTab_t *symtab, const char *type_id, int prefer_unit_defined) {
...
int semcheck_relop(int *type_return, SymTab_t *symtab, struct Expression *expr,
  ...
  return_val += semcheck_expr_with_type(&kgpc_type_first, symtab, expr1,
                                        max_scope_lev, mutating);
  type_first = semcheck_tag_from_kgpc(kgpc_type_first);
  if (expr->expr_data.relop_data.type == IN && expr2 != NULL &&
      expr2->type == EXPR_SET)
    semcheck_bind_set_literal_to_left_enum(symtab, expr2, kgpc_type_first);
  if (expr2 != NULL) {
    return_val += semcheck_expr_with_type(&kgpc_type_second, symtab, expr2,
                                          max_scope_lev, mutating);
  ...
  if (expr2 == NULL) {
    ...
    semcheck_error_with_context_at(
        ...
        "Error on line %d, expected relational type after \"NOT\"!\n\n",
        expr->line_num);
    ++return_val;
    *type_return = UNKNOWN_TYPE;
    return return_val;
    ...
    if (is_and_or(&expr->expr_data.relop_data.type)) {
      if (type_first != BOOL || type_second != BOOL) {
        fprintf(stderr,
                "Error on line %d, expected two relational types between "
                "AND/OR!\n\n",
                expr->line_num);
        ++return_val;
      }
      ...
      if (relop_type == IN) {
        ...
        if (type_second != SET_TYPE && type_second != UNKNOWN_TYPE) {
          semcheck_error_with_context_at(
              expr->line_num, expr->col_num, expr->source_index,
              "Error on line %d, expected set operand on right side of IN "
              "expression!\n\n",
              expr->line_num);
          ++return_val;
        }
        if (!is_integer_type(type_first) && type_first != ENUM_TYPE &&
          ...
          /* Secondary check: KgpcType for char/integer (e.g. PPAnsiChar^[i]
           * indexing) */
          int in_ok = 0;
          ...
          if (!in_ok) {
            semcheck_error_with_context_at(
                expr->line_num, expr->col_num, expr->source_index,
                "Error on line %d, expected integer operand on left side of IN "
                "expression!\n\n",
                expr->line_num);
            ++return_val;
          }
        ...
        if (type_first == RECORD_TYPE || type_second == RECORD_TYPE) {
          ...
          if (record_type_name != NULL) {
            ...
            if (operator_method != NULL) {
              snprintf(operator_method, name_len, "%s__%s", record_type_name,
                       op_suffix);

              HashNode_t *operator_node = NULL;
              ListNode_t *operator_candidates =
                  FindAllIdents(symtab, operator_method);
              if (operator_candidates != NULL) {
                KgpcType *left_arg_type =
                    expr1 != NULL ? expr1->resolved_kgpc_type : NULL;
                KgpcType *right_arg_type =
                    expr2 != NULL ? expr2->resolved_kgpc_type : NULL;
                HashNode_t *best_exact =
                    semcheck_find_best_binary_operator_candidate(
                        operator_candidates, left_arg_type, right_arg_type,
                        symtab);
                if (best_exact != NULL)
                ...
                if (operator_node == NULL) {
                  ...
                  if (args_given != NULL) {
                    ...
                    int resolve_status = semcheck_resolve_overload(
                        &best_match, &num_best, operator_candidates, args_given,
                        symtab, expr, max_scope_lev, 1);
              ...
              if (operator_node == NULL &&
                  semcheck_find_ident_by_prefix_visible(&operator_node, symtab,
                                                        operator_method) >= 0 &&
                  operator_node != NULL) {
                /* prefix search: finds return-type-disambiguated names not
                 * indexed by base id */
              }
              if (operator_node != NULL) {
                if (operator_node->type != NULL &&
                  ...
                  KgpcType *return_type =
                      kgpc_type_get_return_type(operator_node->type);
                  if (return_type != NULL) {
      ...
      relop_implicit_conversion:
        ...
        semcheck_coerce_char_string_operands(&type_first, expr1, &type_second,
                                             expr2);
        ...
        int string_ok =
            (is_string_type(type_first) && is_string_type(type_second));
        ...
        int pointer_ok =
            ((type_first == POINTER_TYPE || type_first == PROCEDURE) &&
        ...
        int enum_ok =
            ((type_first == ENUM_TYPE || is_integer_type(type_first)) &&
         ...
         * Use type tags first (string vs pointer), then refine with KgpcType if
         * available. */
        ...
        if (!string_ok && expr1 != NULL && expr2 != NULL) {
          ...
          /* Also check using KgpcType for more accurate detection when
           * available */
          KgpcType *kgpc1 = expr1->resolved_kgpc_type;
          KgpcType *kgpc2 = expr2->resolved_kgpc_type;
          if (kgpc1 != NULL && kgpc2 != NULL) {
        ...
        /* Also check using KgpcType for enum compatibility (handles scoped
         * enums) */
         ...
         * In Pascal, dynamic arrays can be compared with nil to check if they
         * are empty/uninitialized. A := nil; if A = nil then ... Also, any
         * pointer (including class/interface pointers) can be compared with
         * nil. */
        ...
        if (!pointer_ok && expr1 != NULL && expr2 != NULL) {
          ...
          if (kgpc1 != NULL && kgpc2 != NULL) {
            ...
            int is_nil1 = (kgpc1->kind == TYPE_KIND_POINTER &&
                           kgpc1->info.points_to == NULL);
            int is_nil2 = (kgpc2->kind == TYPE_KIND_POINTER &&
                           kgpc2->info.points_to == NULL);
             ...
             * Also allow class/interface (record) = nil comparisons since
             * classes are pointer types. */
        ...
        if (!set_ok && expr1 != NULL && expr2 != NULL) {
          KgpcType *kgpc1 = expr1->resolved_kgpc_type;
          KgpcType *kgpc2 = expr2->resolved_kgpc_type;
          struct TypeAlias *alias1 =
              kgpc1 != NULL ? kgpc_type_get_type_alias(kgpc1) : NULL;
          struct TypeAlias *alias2 =
              kgpc2 != NULL ? kgpc_type_get_type_alias(kgpc2) : NULL;
          if ((alias1 != NULL && alias1->is_set) ||
            ...
            if ((alias1 != NULL && alias1->is_set) || kgpc1 == NULL ||
                ...
                relop_type == EQ || relop_type == NE || relop_type == LE ||
                relop_type == GE) {
              set_ok = 1;
            }
          }
        ...
        if (!numeric_ok && !boolean_ok && !string_ok && !char_ok &&
            !pointer_ok && !enum_ok && !string_pchar_ok && !dynarray_nil_ok &&
            !pointer_nil_ok && !record_ok && !set_ok && !class_record_ok &&
        ...
        /* Implicit conversion path for record/scalar equality: only implicit
         * casts are available at this point (e.g. TUInt24Rec overload bodies in
         * FPC RTL). Try to cast the record operand to the scalar operand type
         * and re-check. */
        if (!numeric_ok && !boolean_ok && !string_ok && !char_ok &&
            !pointer_ok && !enum_ok && !string_pchar_ok && !dynarray_nil_ok &&
            !pointer_nil_ok && !record_ok && !set_ok && !class_record_ok &&
            !unknown_nil_ok && expr1 != NULL && expr2 != NULL) {
          if (type_first == RECORD_TYPE && type_second != UNKNOWN_TYPE &&
            ...
            if (target_name != NULL) {
              struct Expression *cast_expr = mk_typecast(
                  expr1->line_num, type_second, strdup(target_name), expr1);
              if (cast_expr != NULL) {
            ...
            if (target_name != NULL) {
              struct Expression *cast_expr = mk_typecast(
                  expr2->line_num, type_first, strdup(target_name), expr2);
              if (cast_expr != NULL) {
        ...
        if (!numeric_ok && !boolean_ok && !string_ok && !char_ok &&
            !pointer_ok && !enum_ok && !string_pchar_ok && !dynarray_nil_ok &&
            !pointer_nil_ok && !record_ok && !set_ok && !class_record_ok &&
            !unknown_nil_ok && type_first != VARIANT_TYPE &&
            type_second != VARIANT_TYPE && type_first != UNKNOWN_TYPE &&
            type_second != UNKNOWN_TYPE && type_first != RECORD_TYPE &&
            type_second != RECORD_TYPE &&
            !((is_integer_type(type_first) && type_second == POINTER_TYPE) ||
              (type_first == POINTER_TYPE && is_integer_type(type_second)))) {
          semcheck_error_with_context_at(
              expr->line_num, expr->col_num, expr->source_index,
              "Error on line %d, equality comparison requires matching "
              "numeric, boolean, string, character, or pointer types!\n\n",
              expr->line_num);
          ++return_val;
        }
      } else {
        if (type_first == RECORD_TYPE || type_second == RECORD_TYPE) {
          struct Expression *record_expr =
              (type_first == RECORD_TYPE) ? expr1 : expr2;
          const char *record_type_name =
              get_expr_type_name(record_expr, symtab);
          ...
          if (record_type_name != NULL && op_suffix != NULL) {
            ...
            if (operator_method != NULL) {
              ...
              if (operator_candidates != NULL) {
                ...
                if (args_given != NULL) {
                  args_given->next = CreateListNode(expr2, LIST_EXPR);
                  if (semcheck_resolve_overload(
                          &best_match, &num_best, operator_candidates,
                          args_given, symtab, expr, max_scope_lev, 1) == 0 &&
                      best_match != NULL) {
                    operator_node = best_match;
                  }
                  DestroyList(args_given);
         ...
         * Same pattern as the equality relop implicit conversion above (lines
         * ~925-959). */
        if (!numeric_ok && !string_ok && !char_ok && !pointer_ok && !enum_ok &&
            !string_pchar_ok && !dynarray_nil_ok && !pointer_nil_ok &&
            !record_ok && !set_ok && expr1 != NULL && expr2 != NULL) {
          if (type_first == RECORD_TYPE && type_second != UNKNOWN_TYPE &&
              type_second != RECORD_TYPE) {
            const char *target_name = get_expr_type_name(expr2, symtab);
            if (target_name == NULL || target_name[0] == '\0')
        ...
        if (!numeric_ok && !string_ok && !char_ok && !pointer_ok && !enum_ok &&
            !string_pchar_ok && !dynarray_nil_ok && !pointer_nil_ok &&
            !record_ok && !set_ok && type_first != VARIANT_TYPE &&
            type_second != VARIANT_TYPE && type_first != UNKNOWN_TYPE &&
          ...
          semcheck_error_with_context_at(
              expr->line_num, expr->col_num, expr->source_index,
              "Error on line %d, expected compatible numeric, string, or "
              "character types between relational op!\n\n",
              expr->line_num);
          ++return_val;
        }
      }
...
int semcheck_signterm(int *type_return, SymTab_t *symtab,
  ...
  return_val += semcheck_expr_with_type(&sign_kgpc_type, symtab, sign_expr,
                                        max_scope_lev, mutating);
  *type_return = semcheck_tag_from_kgpc(sign_kgpc_type);
    ...
    if (record_type_name != NULL) {
      ...
      if (operator_method != NULL) {
        ...
        if (operator_candidates != NULL) {
          ...
          if (args_given != NULL) {
            int resolve_status = semcheck_resolve_overload(
                &best_match, &num_best, operator_candidates, args_given, symtab,
                expr, max_scope_lev, 1);
            if (resolve_status == 0 && best_match != NULL && num_best == 1)
              operator_node = best_match;
            DestroyList(args_given);
          }
        ...
        if (operator_node == NULL) {
          size_t exact_len =
              strlen(operator_method) + strlen(record_type_name) + 2;
          char *operator_exact = (char *)malloc(exact_len);
          if (operator_exact != NULL) {
            snprintf(operator_exact, exact_len, "%s_%s", operator_method,
                     record_type_name);
  ...
  if (!is_type_ir(type_return)) {
    semcheck_error_with_context_at(
        expr->line_num, expr->col_num, expr->source_index,
        "Error on line %d, expected int or real after \"-\"!\n\n",
        expr->line_num);
    ++return_val;
  }
...
int semcheck_addop(int *type_return, SymTab_t *symtab, struct Expression *expr,
  ...
  if (op_type == OR) {
    ...
    if (type_first == RECORD_TYPE || type_second == RECORD_TYPE) {
      ...
      semcheck_error_with_context_at(expr->line_num, expr->col_num,
                                     expr->source_index,
                                     "Error on line %d, expected boolean or "
                                     "integer operands for OR expression!\n\n",
                                     expr->line_num);
      if (kgpc_getenv("KGPC_DEBUG_ANDOR") != NULL) {
        fprintf(stderr,
                "[SemCheck] OR mismatch at line %d: lhs=%s(%d) rhs=%s(%d)\n",
                expr->line_num, semcheck_type_tag_name(type_first), type_first,
  ...
  if (type_first == SET_TYPE && type_second == SET_TYPE) {
    if (op_type == PLUS || op_type == MINUS) {
      *type_return = SET_TYPE;
      ...
      semcheck_error_with_context_at(
          expr->line_num, expr->col_num, expr->source_index,
          "Error on line %d, unsupported set additive operator.\n\n",
          expr->line_num);
      ++return_val;
      *type_return = SET_TYPE;
    }
  ...
  if (op_type == PLUS) {
    ...
    int left_is_string_like =
        (is_string_type(type_first) || type_first == CHAR_TYPE ||
         semcheck_expr_is_char_array_like(expr1) ||
         semcheck_expr_is_char_pointer(expr1));
    int right_is_string_like =
        (is_string_type(type_second) || type_second == CHAR_TYPE ||
  ...
  if (op_type == PLUS || op_type == MINUS) {
    ...
    if (left_is_pointer && right_is_int) {
      ...
      *type_return = POINTER_TYPE;
      ...
      semcheck_set_pointer_info(expr, expr1->pointer_subtype,
                                expr1->pointer_subtype_id);
    ...
    if (op_type == PLUS && left_is_int && right_is_pointer) {
      ...
      *type_return = POINTER_TYPE;
      ...
      semcheck_set_pointer_info(expr, expr2->pointer_subtype,
                                expr2->pointer_subtype_id);
      /* Propagate KgpcType information for proper type checking */
      if (expr2->resolved_kgpc_type != NULL) {
        semcheck_expr_set_resolved_kgpc_type_shared(expr,
                                                    expr2->resolved_kgpc_type);
      }
  ...
  if (type_first == RECORD_TYPE || type_second == RECORD_TYPE) {
    ...
    if (kgpc_getenv("KGPC_DEBUG_ADDOP") != NULL) {
      fprintf(stderr,
              "[SemCheck] addop record branch line %d: lhs_type_name=%s "
              "rhs_type_name=%s chosen=%s op=%d\n",
              expr->line_num,
              left_type_name != NULL ? left_type_name : "<null>",
              right_type_name != NULL ? right_type_name : "<null>",
              record_type_name != NULL ? record_type_name : "<null>", op_type);
    }
    ...
    if (record_type_name != NULL) {
      ...
      if (op_suffix != NULL) {
        ...
        if (operator_method != NULL) {
          ...
          if (kgpc_getenv("KGPC_DEBUG_ADDOP") != NULL) {
            fprintf(stderr,
                    "[SemCheck] addop operator key=%s candidates=%d "
                    "scope_unit=%d lookup_unit=%d\n",
                    operator_method, ListLength(operator_candidates),
                    (symtab != NULL && symtab->current_scope != NULL)
          ...
          if (operator_node == NULL && right_type_name != NULL) {
            size_t exact_len = strlen(operator_method) +
                               strlen(right_type_name) +
                               strlen(record_type_name) + 3;
            char *operator_exact = (char *)malloc(exact_len);
            if (operator_exact != NULL) {
              snprintf(operator_exact, exact_len, "%s_%s_%s", operator_method,
                       right_type_name, record_type_name);
              FindSymbol(&operator_node, symtab, operator_exact);
              free(operator_exact);
            }
          }
          if (operator_node == NULL &&
            ...
            if (operator_node->type != NULL) {
              ListNode_t *check_params =
                  kgpc_type_get_procedure_params(operator_node->type);
            ...
            if (operator_node->type != NULL &&
              ...
              if (return_type != NULL) {
                *type_return = semcheck_tag_from_kgpc(return_type);
                ...
                struct Expression *saved_left =
                    expr->expr_data.addop_data.left_expr;
                struct Expression *saved_right =
  ...
  /* Pointer arithmetic: pointer +/- integer yields pointer (FPC {$POINTERMATH
   * ON}) */
  ...
  if (!types_numeric_compatible(type_first, type_second)) {
    semantic_error(expr->line_num, expr->col_num,
                   "type mismatch on addop: lhs is %s, rhs is %s",
                   semcheck_type_tag_name(type_first),
                   semcheck_type_tag_name(type_second));
    if (kgpc_getenv("KGPC_DEBUG_ADDOP") != NULL) {
      fprintf(stderr,
  ...
  if (!is_type_ir(&type_first) || !is_type_ir(&type_second)) {
    semantic_error(expr->line_num, expr->col_num,
                   "expected int/real on both sides of addop");
    if (kgpc_getenv("KGPC_DEBUG_ADDOP") != NULL) {
      fprintf(
          stderr,
...
int semcheck_mulop(int *type_return, SymTab_t *symtab, struct Expression *expr,
  ...
  if (op_type == AND || op_type == XOR) {
    if (type_first == BOOL && type_second != BOOL && expr2 != NULL &&
      ...
      semcheck_try_refine_funccall_to_bool(symtab, expr2, max_scope_lev,
                                           mutating, &type_second);
    }
    if (type_second == BOOL && type_first != BOOL && expr1 != NULL &&
        expr1->type == EXPR_FUNCTION_CALL) {
      semcheck_try_refine_funccall_to_bool(symtab, expr1, max_scope_lev,
                                           mutating, &type_first);
    }
    ...
    /* Integer/enum/pointer bitwise operations (enums are ordinal types in
     * Pascal). Records are NOT included here — they must route through the
    ...
    if (type_first == RECORD_TYPE || type_second == RECORD_TYPE) {
      ...
      semcheck_error_with_context_at(
          expr->line_num, expr->col_num, expr->source_index,
          "Error on line %d, expected boolean, integer, or set operands for %s "
          "expression!\n\n",
          expr->line_num, op_type == AND ? "AND" : "XOR");
      if (kgpc_getenv("KGPC_DEBUG_ANDOR") != NULL) {
        fprintf(stderr,
                "[SemCheck] %s mismatch at line %d: lhs=%s(%d) rhs=%s(%d)\n",
                op_type == AND ? "AND" : "XOR", expr->line_num,
  ...
  if (type_first == SET_TYPE && type_second == SET_TYPE) {
    if (op_type == STAR) {
      *type_return = SET_TYPE;
      ...
      semcheck_error_with_context_at(
          expr->line_num, expr->col_num, expr->source_index,
          "Error on line %d, unsupported set multiplicative operator.\n\n",
          expr->line_num);
      ++return_val;
      *type_return = SET_TYPE;
    }
    ...
    if (record_type_name != NULL) {
      ...
      if (op_suffix != NULL) {
        ...
        if (operator_method != NULL) {
           ...
           * Try exact match first, then prefix match for return-type-suffixed
           * names. */
          HashNode_t *operator_node = NULL;
          if (FindSymbol(&operator_node, symtab, operator_method) != 0 &&
              operator_node != NULL) {
            /* exact match */
          } else if (semcheck_find_ident_by_prefix_visible(
                         &operator_node, symtab, operator_method) >= 0 &&
                     operator_node != NULL) {
  ...
  if (!types_numeric_compatible(type_first, type_second)) {
    semcheck_error_with_context_at(
        expr->line_num, expr->col_num, expr->source_index,
        "Error on line %d, type mismatch on mulop: lhs is %s, rhs is %s!\n\n",
        expr->line_num, semcheck_type_tag_name(type_first),
        semcheck_type_tag_name(type_second));
    ++return_val;
  }
  if (!is_type_ir(&type_first) || !is_type_ir(&type_second)) {
    semcheck_error_with_context_at(
        expr->line_num, expr->col_num, expr->source_index,
        "Error on line %d, expected int/real on both sides of mulop, got lhs: "
        "%s, rhs: %s!\n\n",
        expr->line_num, semcheck_type_tag_name(type_first),
        semcheck_type_tag_name(type_second));
    ++return_val;
  ...
  if (op_type == DIV || op_type == MOD) {
    if (type_first == REAL_TYPE || type_second == REAL_TYPE ||
      ...
      semcheck_error_with_context_at(expr->line_num, expr->col_num,
                                     expr->source_index,
                                     "Error on line %d, DIV and MOD operators "
...
semcheck_resolve_helper_self_record(SymTab_t *symtab, HashNode_t *self_node,
                                    struct RecordType *helper_self_record) {
  ...
  if (self_record == NULL && self_node != NULL) {
    ...
    if (helper_record != NULL) {
      ...
      if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
        fprintf(stderr,
                "[SemCheck] varid type-helper lookup: Using type helper %s for "
                "Self\n",
...
static int semcheck_try_helper_member(int *type_return, SymTab_t *symtab,
  ...
  HashNode_t *method_node =
      semcheck_find_class_method(symtab, self_record, id, NULL);
  if (method_node != NULL) {
    ...
    memset(&expr->expr_data.function_call_data, 0,
           sizeof(expr->expr_data.function_call_data));
    expr->expr_data.function_call_data.id =
        (moved_id != NULL) ? moved_id : strdup(id);
  ...
  struct ClassProperty *property =
      semcheck_find_class_property(symtab, self_record, id, &property_owner);
  if (property != NULL) {
    ...
    if (self_str != NULL) {
      ...
      if (self_expr != NULL) {
        ...
        return semcheck_recordaccess(type_return, symtab, expr, max_scope_lev,
                                     mutating);
      }
    }
  }
  ...
  struct RecordField *field_desc =
      semcheck_find_class_field(symtab, self_record, id, &field_owner);
  if (field_desc != NULL) {
    char *self_str = strdup("Self");
    if (self_str != NULL) {
      struct Expression *self_expr = mk_varid(expr->line_num, self_str);
      if (self_expr != NULL) {
        ...
        if (resolve_record_field(symtab, self_record, id, NULL, &field_offset,
                                 0, 1) == 0) {
...
static int semcheck_try_self_field_access(int *type_return, SymTab_t *symtab,
  ...
  HashNode_t *preferred_node =
      semcheck_find_preferred_value_ident(symtab, id, &preferred_scope);
  if (preferred_node != NULL && preferred_scope == 0) {
    ...
    int is_imported_method =
        (scope_unit != 0 && scope_unit != symtab->current_unit_index);
  ...
  struct RecordField *field = semcheck_find_class_field_including_hidden(
      symtab, self_record, id, &field_owner);
  if (kgpc_getenv("KGPC_DEBUG_MONITOR") != NULL && id != NULL &&
      pascal_identifier_equals(id, "_MonitorData")) {
    fprintf(stderr,
            "[KGPC_DEBUG_MONITOR] try_self_field record=%s field=%s found=%d "
            "owner=%s\n",
            self_record != NULL && self_record->type_id != NULL
                ? self_record->type_id
                : "<null>",
            id, field != NULL,
            field_owner != NULL && field_owner->type_id != NULL
                ? field_owner->type_id
                : "<null>");
    if (field == NULL && self_record != NULL && self_record->fields != NULL) {
      int field_count = 0;
      for (ListNode_t *cur = self_record->fields;
           cur != NULL && field_count < 5; cur = cur->next) {
        if (cur->type == LIST_RECORD_FIELD && cur->cur != NULL) {
          struct RecordField *rf = (struct RecordField *)cur->cur;
          fprintf(stderr, "  field[%d]=%s hidden=%d\n", field_count,
                  rf->name != NULL ? rf->name : "<null>", rf->is_hidden);
          field_count++;
        }
      }
    }
  ...
  if (field == NULL) {
    ...
    if (trace_nonlocal != NULL && id != NULL &&
        pascal_identifier_equals(id, trace_nonlocal)) {
      fprintf(stderr,
              "[KGPC_TRACE_NONLOCAL] self-field miss id=%s record=%s "
              "self_node=%p self_type=%s\n",
              id,
              self_record != NULL && self_record->type_id != NULL
                  ? self_record->type_id
                  : "<null>",
              ...
              self_node != NULL && self_node->type != NULL
                  ? kgpc_type_to_string(self_node->type)
                  : "<null>");
    }
    return -1;
  }
...
int semcheck_varid_ex(int *type_return, SymTab_t *symtab,
  ...
  if (mutating != NO_MUTATE && id != NULL) {
    ...
    if (kgpc_getenv("KGPC_DEBUG_RESULT_NAME") != NULL &&
      ...
      fprintf(stderr,
              "[KGPC_DEBUG_RESULT_NAME] early id=%s mutating=%d cur=%s "
              "result=%s method=%s\n",
              id, mutating, cur_sub_id != NULL ? cur_sub_id : "<null>",
    ...
    if (is_result_name) {
      int owns_ret = 0;
      KgpcType *ret_type =
          semcheck_get_current_subprogram_return_kgpc_type(symtab, &owns_ret);
      if (ret_type != NULL) {
        ...
        const char *norm_result_var =
            semcheck_get_current_subprogram_result_var_name();
        const char *norm_replacement =
            (norm_result_var != NULL && norm_result_var[0] != '\0')
                ? norm_result_var
                : "Result";
        ...
        if (!pascal_identifier_equals(id, norm_replacement)) {
          ...
          int local_scope = 0;
          local_check = semcheck_find_preferred_value_ident(
              symtab, norm_replacement, &local_scope);
  ...
  if (kgpc_getenv("KGPC_DEBUG_WITH") != NULL &&
    ...
    fprintf(stderr,
            "[KGPC_DEBUG_WITH] varid=%s with_context_count=%zu line=%d\n",
  ...
  if (kgpc_getenv("KGPC_DEBUG_WITH") != NULL &&
      (pascal_identifier_equals(id, "BufPtr") ||
       ...
       pascal_identifier_equals(id, "Chars"))) {
    fprintf(stderr,
            "[KGPC_DEBUG_WITH] varid=%s with_status=%d with_expr=%p line=%d\n",
  ...
  if (scope_return && mutating != NO_MUTATE && hash_return != NULL &&
      ...
      !hash_return->is_typed_const && hash_return->type != NULL &&
  ...
  if (id != NULL) {
    ...
    if (trace_nonlocal != NULL &&
        pascal_identifier_equals(id, trace_nonlocal)) {
      fprintf(stderr,
              "[KGPC_TRACE_NONLOCAL] sem_varid id=%s scope=%d hash=%p "
              "hash_type=%d subprogram=%s owner_ctx=%s\n",
              id, scope_return, (void *)hash_return,
              hash_return != NULL ? hash_return->hash_type : -1,
              semcheck_get_current_subprogram_id() != NULL
                  ? semcheck_get_current_subprogram_id()
                  : "<null>",
  ...
  if (scope_return && hash_return != NULL && id != NULL && !suppress_self &&
    ...
    if (!skip_self_check) {
      ...
      if (FindSymbol(&self_node, symtab, "Self") != 0 && self_node != NULL) {
        ...
        if (self_record == NULL)
          self_record = semcheck_resolve_helper_self_record(symtab, self_node,
                                                            self_record);
        if (self_record != NULL) {
          int field_result = semcheck_try_self_field_access(
              type_return, symtab, expr, max_scope_lev, mutating, self_node,
              self_record, id);
          if (field_result >= 0)
            return field_result;
        }
    ...
    if (owner != NULL) {
      ...
      if (!class_const_scope) {
        ...
        if (outer != NULL) {
          ...
          snprintf(outer_mangled, sizeof(outer_mangled), "%s__%s", outer, id);
          class_const = NULL;
    ...
    if (!scope_return) {
      if (expr->id_ref != NULL && expr->id_ref->count >= 2) {
        ...
        if (prefix_scope && prefix_node != NULL &&
          ...
          if (FindSymbol(&field_node, symtab, suffix) != 0 &&
              field_node != NULL) {
          ...
          for (int pp = 0; pp < 2 && !scope_return; pp++) {
            ...
            while (pnode != NULL) {
              if (pnode->type == LIST_CLASS_PROPERTY && pnode->cur != NULL) {
                struct ClassProperty *cprop =
                    (struct ClassProperty *)pnode->cur;
                if (cprop->name != NULL &&
                  ...
                  int acc_scope =
                      FindSymbol(&accessor_node, symtab, cprop->read_accessor);
      ...
      if (FindSymbol(&self_node, symtab, "Self") != 0 && self_node != NULL) {
        if (id != NULL) {
          ...
          if (trace_nonlocal != NULL &&
              pascal_identifier_equals(id, trace_nonlocal)) {
            fprintf(stderr,
                    "[KGPC_TRACE_NONLOCAL] sem_varid self-lookup id=%s "
                    "self_node=%p self_hash=%d\n",
        ...
        if (self_record != NULL) {
          ...
          if (field_result >= 0)
            return field_result;
          int helper_result = semcheck_try_helper_member(
              type_return, symtab, expr, max_scope_lev, mutating, self_node,
              self_record, id);
          if (helper_result >= 0)
  ...
  if (kgpc_getenv("KGPC_DEBUG_RESULT") != NULL && id != NULL &&
    ...
    fprintf(stderr,
            "[KGPC] semcheck_varid Result: scope_return=%d hash_return=%p "
            "hash_type=%d type=%s\n",
            scope_return, (void *)hash_return,
            hash_return != NULL ? hash_return->hash_type : -1,
            hash_return != NULL ? kgpc_type_to_string(hash_return->type)
                                : "<null>");
  }
  if (scope_return && hash_return != NULL &&
      hash_return->hash_type == HASHTYPE_FUNCTION && mutating == NO_MUTATE &&
      with_status != 0) {
    int value_scope = scope_return;
    HashNode_t *value_node =
        semcheck_find_preferred_value_ident(symtab, id, &value_scope);
    if (value_node != NULL) {
  ...
  if (kgpc_getenv("KGPC_DEBUG_TYPE_HELPER") != NULL && id != NULL &&
    ...
    fprintf(stderr,
            "[KGPC] semcheck_varid: FindIdent Self scope_return=%d "
            "hash_return=%p kind=%d\n",
            scope_return, (void *)hash_return,
            hash_return && hash_return->type ? hash_return->type->kind : -1);
  }
  if (scope_return && with_status != 0 && id != NULL && hash_return != NULL &&
    ...
    if (FindSymbol(&self_node, symtab, "Self") != 0 && self_node != NULL) {
      ...
      if (self_record != NULL) {
        ...
        struct ClassProperty *property = semcheck_find_class_property(
            symtab, self_record, id, &property_owner);
        if (property != NULL) {
          char *self_str = strdup("Self");
          if (self_str != NULL) {
            struct Expression *self_expr = mk_varid(expr->line_num, self_str);
  ...
  if (FindSymbol(&helper_self_node, symtab, "Self") != 0 &&
      helper_self_node != NULL) {
    helper_self_record = get_record_type_from_node(helper_self_node);
    if (helper_self_record == NULL) {
      helper_self_record = semcheck_resolve_helper_self_record(
          symtab, helper_self_node, helper_self_record);
    }
  ...
  if (scope_return > 0 && id != NULL && helper_self_node != NULL &&
    ...
    if (!is_func_result_name && !is_value_symbol) {
      struct RecordType *self_record = helper_self_record;
      if (self_record == NULL) {
        self_record = semcheck_resolve_helper_self_record(
            symtab, helper_self_node, helper_self_record);
      }
      if (kgpc_getenv("KGPC_DEBUG_MONITOR") != NULL && id != NULL &&
          pascal_identifier_equals(id, "_MonitorData")) {
        fprintf(stderr,
                "[KGPC_DEBUG_MONITOR] scope>0 self_node=%p self_record=%p "
                "helper_context=%d\n",
                (void *)helper_self_node, (void *)self_record, helper_context);
      }
      int field_result = semcheck_try_self_field_access(
          type_return, symtab, expr, max_scope_lev, mutating, helper_self_node,
          self_record, id);
      if (field_result >= 0)
        return field_result;
      int helper_result = semcheck_try_helper_member(
  ...
  if (!scope_return) {
    {
      ...
      if (with_status != 0 && id != NULL) {
        ...
        if (getter_id != NULL) {
          ...
          if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
            fprintf(stderr,
                    "[SemCheck] varid getter-lookup: id=%s getter=%s found=%d "
                    "hash=%d\n",
      ...
      if ((with_status != 0 || helper_context) && id != NULL) {
        ...
        if (self_node != NULL) {
          struct RecordType *self_record = semcheck_resolve_helper_self_record(
              symtab, self_node, helper_self_record);
          if (kgpc_getenv("KGPC_DEBUG_MONITOR") != NULL && id != NULL &&
              pascal_identifier_equals(id, "_MonitorData")) {
            fprintf(stderr,
                    "[KGPC_DEBUG_MONITOR] scope=-1 self_node=%p self_record=%p "
                    "helper_context=%d\n",
                    (void *)self_node, (void *)self_record, helper_context);
            if (self_record != NULL && self_record->type_id != NULL) {
              char mangled[256];
              snprintf(mangled, sizeof(mangled), "%s__%s", self_record->type_id,
                       id);
              HashNode_t *mnode = NULL;
              int mscope = FindSymbol(&mnode, symtab, mangled);
              fprintf(stderr,
                      "[KGPC_DEBUG_MONITOR] mangled lookup %s scope=%d node=%p "
                      "hash=%d\n",
           ...
           * Check both when Self is a record type helper or when it's a
           * primitive string type. */
          int is_string_type_helper = 0;
          if (self_record != NULL && self_record->is_type_helper &&
              self_record->helper_base_type_id != NULL &&
              (pascal_identifier_equals(self_record->helper_base_type_id,
                                        "AnsiString") ||
               pascal_identifier_equals(self_record->helper_base_type_id,
                                        "String") ||
               pascal_identifier_equals(self_record->helper_base_type_id,
          ...
          if (!is_string_type_helper && self_node->type != NULL) {
            ...
            if (current_owner != NULL) {
              ...
              if (owner_record != NULL && owner_record->is_type_helper &&
                  ...
                  (pascal_identifier_equals(owner_record->helper_base_type_id,
                                            "AnsiString") ||
                   pascal_identifier_equals(owner_record->helper_base_type_id,
                                            "String") ||
                   pascal_identifier_equals(owner_record->helper_base_type_id,
      ...
      if (with_status != 0 && id != NULL) {
        ...
        if (self_node != NULL) {
          ...
          if (self_record == NULL)
            self_record =
                semcheck_resolve_helper_self_record(symtab, self_node, NULL);
          if (self_record == NULL) {
            const char *owner = semcheck_get_current_method_owner();
            if (owner != NULL)
          ...
          if (self_record != NULL) {
            int member_result = semcheck_try_helper_member(
                type_return, symtab, expr, max_scope_lev, mutating, self_node,
                self_record, id);
            if (member_result >= 0)
      ...
      if (with_status == -1) {
        ...
        *type_return = UNKNOWN_TYPE;
        ...
        semantic_error_at(expr->line_num, expr->col_num, expr->source_index,
                          "undeclared identifier \"%s\"", id);
        ++return_val;
      }
    ...
    if (mutating != NO_MUTATE && id != NULL &&
        pascal_identifier_equals(id, "Result") && hash_return != NULL &&
    ...
    if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL && id != NULL &&
        strcmp(id, "DefaultComparer") == 0) {
      fprintf(stderr,
              "[SemCheck] Debug DefaultComparer: hash_type=%d scope=%d "
              "mutating=%d type_kind=%d\n",
              hash_return != NULL ? hash_return->hash_type : -1, scope_return,
              mutating,
              hash_return != NULL && hash_return->type != NULL
                  ? hash_return->type->kind
                  : -1);
    }
    /* If this is a function being used in an expression context (not being
       assigned to), convert it to a function call with no arguments.

       When mutating == NO_MUTATE, we're reading the function's return value.
       When mutating != NO_MUTATE, we're inside the function assigning to its
       return value, which should remain as HASHTYPE_FUNCTION_RETURN access. */
    if (hash_return->hash_type == HASHTYPE_FUNCTION && mutating == NO_MUTATE) {
      if (kgpc_getenv("KGPC_DEBUG_RESULT_NAME") != NULL && id != NULL &&
          pascal_identifier_equals(id, "correct_fpuregister")) {
        fprintf(stderr,
                "[KGPC_DEBUG_RESULT_NAME] late id=%s cur=%s result=%s "
                "method=%s hash_type=%d\n",
                id,
                semcheck_get_current_subprogram_id() != NULL
                    ? semcheck_get_current_subprogram_id()
                    : "<null>",
                semcheck_get_current_subprogram_result_var_name() != NULL
                    ? semcheck_get_current_subprogram_result_var_name()
                    : "<null>",
                semcheck_get_current_subprogram_method_name() != NULL
                    ? semcheck_get_current_subprogram_method_name()
                    : "<null>",
      ...
      if (_is_own_result) {
        ...
        const char *_norm_rv =
            semcheck_get_current_subprogram_result_var_name();
        const char *_norm_repl =
            (_norm_rv != NULL && _norm_rv[0] != '\0') ? _norm_rv : "Result";
        int _has_local = 0;
        if (!pascal_identifier_equals(id, _norm_repl)) {
          HashNode_t *_lc =
              semcheck_find_preferred_value_ident(symtab, _norm_repl, NULL);
      ...
      if (id != NULL) {
        ...
        if (FindSymbol(&self_node, symtab, "Self") != 0 && self_node != NULL) {
          ...
          if (self_record == NULL) {
            ...
            self_record = semcheck_lookup_type_helper(symtab, self_type_tag,
                                                      self_type_name);
          }
            ...
            if (property != NULL) {
              ...
              if (self_str != NULL) {
                struct Expression *self_expr =
                    mk_varid(expr->line_num, self_str);
                if (self_expr != NULL) {
                  if (self_node->type != NULL) {
                    self_expr->resolved_kgpc_type = self_node->type;
                    kgpc_type_retain(self_node->type);
                    ...
                    KgpcType *self_record_type =
                        create_record_type(self_record);
                    if (self_record_type != NULL) {
                      self_expr->resolved_kgpc_type = self_record_type;
                    }
                  }
              ...
              if (getter_name != NULL) {
                ...
                HashNode_t *getter_node = semcheck_find_class_method(
                    symtab, self_record, getter_name, NULL);
                if (getter_node != NULL) {
                  char *self_str = strdup("Self");
                  if (self_str != NULL) {
                    struct Expression *self_expr =
                        mk_varid(expr->line_num, self_str);
                    if (self_expr != NULL) {
                      ListNode_t *args_list =
                          CreateListNode(self_expr, LIST_EXPR);
                      if (args_list != NULL) {
                        expr->type = EXPR_FUNCTION_CALL;
                        expr->expr_data.function_call_data.is_operator_call = 1;
                        memset(&expr->expr_data.function_call_data, 0,
      ...
      /* Set to NULL to transfer ownership to function_call_data.id and avoid
       * double-free */
      expr->expr_data.id = NULL;
          ...
          if (ret != NULL) {
            ...
            *type_return = semcheck_tag_from_kgpc(ret);
            ...
            if (mangled_node->mangled_id != NULL)
              expr->expr_data.function_call_data.mangled_id =
                  strdup(mangled_node->mangled_id);
            else
              expr->expr_data.function_call_data.mangled_id =
    ...
    if (hash_return->hash_type == HASHTYPE_TYPE && mutating == NO_MUTATE) {
      ...
      if (type_record != NULL && type_record->is_interface) {
        HashNode_t *tguid_node =
            semcheck_find_type_node_with_kgpc_type(symtab, "TGUID");
    ...
    if (0) /* scope depth check removed — tree scoping has no depth */
    {
      if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
        fprintf(stderr,
                "[SemCheck] semcheck_varid: scope_return=%d max_scope_lev=%d\n",
                scope_return, max_scope_lev);
      }
      if (mutating != NO_MUTATE && hash_return->hash_type != HASHTYPE_CONST &&
    ...
    if (is_function_result) {
      ...
      if (hash_return->type != NULL &&
          (kgpc_type_is_array(hash_return->type) ||
           kgpc_type_is_array_of_const(hash_return->type))) {
        node_is_array = 1;
      } else {
      ...
      if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL)
        fprintf(stderr,
                "[SemCheck] semcheck_varid: id=%s hash_type=%d node_type=%p "
                "kind=%d node_is_array=%d defined_in_unit=%d source_unit=%d\n",
                id ? id : "<null>", hash_return->hash_type,
                (void *)hash_return->type,
                hash_return->type ? hash_return->type->kind : -1, node_is_array,
                hash_return->defined_in_unit, hash_return->source_unit_index);
    }

    if (hash_return->hash_type != HASHTYPE_VAR &&
        hash_return->hash_type != HASHTYPE_FUNCTION_RETURN &&
        hash_return->hash_type != HASHTYPE_TYPE && !node_is_array &&
        !is_function_result) {
      if (hash_return->hash_type == HASHTYPE_CONST && mutating == 0) {
        /* Constants are readable values. */
      } else if (hash_return->hash_type == HASHTYPE_PROCEDURE &&
                 mutating == 0) {
        /* Procedures can be used as values when not mutating (for procedure
         * variables). */
      } else {
        semcheck_error_with_context_at(expr->line_num, expr->col_num,
                                       expr->source_index,
                                       "Error on line %d, cannot assign "
    ...
    set_type_from_hashtype(type_return, hash_return);
    if (node_is_array)
      semcheck_set_array_info_from_hashnode(expr, symtab, hash_return,
                                            expr->line_num);
    else
      semcheck_clear_array_info(expr);
    ...
    if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
      fprintf(stderr,
              "[SemCheck] semcheck_varid: expr=%p, id=%s, type_return=%d\n",
    ...
    if (*type_return == POINTER_TYPE) {
      ...
      if (alias != NULL && alias->alias_name != NULL) {
        HashNode_t *alias_type_node =
            semcheck_find_type_node_with_kgpc_type(symtab, alias->alias_name);
      ...
      if (!prefer_unit_pointer_targets && hash_return->type != NULL &&
        ...
        HashNode_t *alias_type_node = semcheck_find_type_node_with_kgpc_type(
            symtab, hash_return->type->type_alias->alias_name);
        if (alias_type_node != NULL && alias_type_node->defined_in_unit)
          prefer_unit_pointer_targets = 1;
      }
      ...
      if (subtype == UNKNOWN_TYPE && hash_return->type != NULL &&
        ...
        if (type_id == NULL) {
          ...
          if (pts != NULL && pts->type_alias != NULL &&
              pts->type_alias->alias_name != NULL)
      ...
      if (subtype == UNKNOWN_TYPE && alias != NULL &&
      ...
      if (subtype == UNKNOWN_TYPE && type_id != NULL) {
        HashNode_t *target_node =
            semcheck_pick_type_node_with_origin_preference(
                symtab, type_id, prefer_unit_pointer_targets);
        if (target_node == NULL &&
      ...
      if (subtype == UNKNOWN_TYPE && type_id == NULL && alias != NULL &&
        ...
        if (FindSymbol(&candidate_node, symtab, (char *)candidate_type_id) !=
                0 &&
            candidate_node != NULL) {
      ...
      if (subtype == RECORD_TYPE && type_id == NULL) {
        ...
        if (type_id == NULL) {
          struct RecordType *node_record =
              get_record_type_from_node(hash_return);
      ...
      if (subtype == UNKNOWN_TYPE && type_id == NULL &&
          kgpc_getenv("KGPC_DEBUG_CG_ERR")) {
        fprintf(stderr,
                "[semcheck-debug] varid pointer UNKNOWN: id=%s line=%d "
                "hash_type=%d\n",
        ...
        if (alias)
          fprintf(
              stderr,
              " alias_name=%s is_pointer=%d pointer_type=%d pointer_type_id=%s",
        ...
        fprintf(stderr, "\n");
        fprintf(stderr, "[semcheck-debug]   hash_return->type=%p",
                (void *)hash_return->type);
        if (hash_return->type)
          fprintf(stderr, " kind=%d", hash_return->type->kind);
        fprintf(stderr, "\n");
      }
      ...
      if (expr->resolved_kgpc_type != NULL &&
        ...
        if (expr->pointer_subtype_id != NULL) {
          HashNode_t *target_node =
              semcheck_pick_type_node_with_origin_preference(
                  symtab, expr->pointer_subtype_id,
                  prefer_unit_pointer_targets);
          if (target_node == NULL) {
          ...
          if (target_node != NULL && target_node->type != NULL) {
            ...
            struct RecordType *target_record =
                semcheck_lookup_record_type(symtab, expr->pointer_subtype_id);
      ...
      if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
        fprintf(stderr,
                "[SemCheck] semcheck_varid: id=%s pointer_subtype=%d "
                "subtype_id=%s\n",
      ...
      if (hash_return->type != NULL &&
        ...
        if (points_to != NULL && kgpc_type_is_record(points_to) &&
          ...
          fprintf(stderr,
                  "[SemCheck] semcheck_varid: id=%s, type=POINTER, "
                  "points_to_record=%p\n",
