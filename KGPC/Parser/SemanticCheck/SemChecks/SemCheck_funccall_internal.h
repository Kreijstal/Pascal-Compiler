...
typedef struct {
    ...
    ListNode_t *args_given;     /* alias into expr->expr_data.function_call_data.args_expr */
...
/* FSM state enum.
 * FC_OVERLOAD_RESOLVE corresponds to the original `method_call_resolved:` label.
 * FC_OVERLOAD_POST corresponds to the original `skip_overload_resolution:` label.
 * FC_CLEANUP corresponds to the original `funccall_cleanup:` label. */
typedef enum {
    FC_NORMALIZE,
    FC_INHERITED,
    FC_INTRINSICS,
...
int semcheck_expr_is_explicit_char_typecast_for_call_local(const struct Expression *expr);
int semcheck_kgpc_type_is_char_like_for_call_local(const KgpcType *type);
int semcheck_expr_is_char_typecast_call_for_call_local(const struct Expression *expr);
int semcheck_try_rewrite_internconst_call(int *type_return,
...
int semcheck_candidate_is_direct_for_current_unit(SymTab_t *symtab, HashNode_t *candidate);
int semcheck_candidate_is_local_callable_for_shadowing(SymTab_t *symtab, HashNode_t *candidate);
void semcheck_set_pointer_info_from_kgpc_type(struct Expression *expr, SymTab_t *symtab, KgpcType *type);
int semcheck_candidate_is_owner_method(HashNode_t *candidate, const char *owner_type_id);
