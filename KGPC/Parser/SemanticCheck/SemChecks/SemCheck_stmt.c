...
int semcheck_expr_best_context(const struct Expression *expr,
...
int semcheck_collection_is_enumerator_class(SymTab_t *symtab, KgpcType *collection_kgpc_type,
...
int rewrite_tfpglist_constructor_if_needed(SymTab_t *symtab,
...
void semcheck_stmt_set_receiver_virtual_dispatch(struct Statement *stmt,
    struct RecordType *receiver_record, const char *method_name,
    KgpcType *call_type)
{
    if (stmt == NULL || stmt->type != STMT_PROCEDURE_CALL ||
...
int semcheck_call_with_proc_var(SymTab_t *symtab, struct Statement *stmt, HashNode_t *proc_node,
    int max_scope_lev);
int semcheck_try_property_assignment(SymTab_t *symtab,
    struct Statement *stmt, int max_scope_lev);
...
int try_resolve_builtin_procedure(SymTab_t *symtab,
...
int semcheck_builtin_initialize_finalize(SymTab_t *symtab, struct Statement *stmt,
