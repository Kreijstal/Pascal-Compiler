...
static char *semcheck_normalize_helper_base_type_id(SymTab_t *symtab,
  ...
  HashNode_t *type_node =
      semcheck_find_preferred_type_node(symtab, base_type_id);
  if (type_node != NULL) {
...
static int
semcheck_helper_entry_matches_record(TypeHelperEntry *entry,
                                     const char *record_type_id,
                                     const char *record_canonical_id) {
...
static int
semcheck_helper_record_defines_member(SymTab_t *symtab,
  ...
  if (member_name == NULL)
    return 1;
  if (semcheck_find_class_method(symtab, helper_record, member_name, NULL) !=
          NULL ||
      semcheck_find_class_field_including_hidden(symtab, helper_record,
                                                 member_name, NULL) != NULL ||
      semcheck_find_class_property(symtab, helper_record, member_name, NULL) !=
          NULL)
    return 1;
  return 0;
}
...
static int
semcheck_helper_record_matches_hierarchy(SymTab_t *symtab,
                                         struct RecordType *helper_record,
                                         struct RecordType *record_info) {
  if (symtab == NULL || helper_record == NULL || record_info == NULL ||
      !helper_record->is_type_helper ||
      helper_record->helper_base_type_id == NULL)
    return 0;
  ...
  while (current != NULL) {
    ...
    if (current->type_id != NULL)
      current_node =
          semcheck_find_preferred_type_node(symtab, current->type_id);
    if (current->type_id != NULL &&
        pascal_identifier_equals(helper_record->helper_base_type_id,
                                 current->type_id))
      return 1;
...
struct RecordType *semcheck_lookup_type_helper_for_record_member(
    SymTab_t *symtab, struct RecordType *record_info, const char *member_name) {
  ...
  if (current_owner != NULL) {
    ...
    if (owner_record != NULL &&
        semcheck_helper_record_matches_hierarchy(symtab, owner_record,
                                                 record_info) &&
        semcheck_helper_record_defines_member(symtab, owner_record,
                                              member_name))
      return owner_record;
  }
  ...
  while (current != NULL) {
    ...
    for (ListNode_t *cur = type_helper_entries; cur != NULL; cur = cur->next) {
      ...
      if (entry == NULL || entry->helper_record == NULL)
        continue;
      if (!semcheck_helper_entry_matches_record(entry, current->type_id,
                                                canonical_id))
        continue;
      if (semcheck_helper_record_defines_member(symtab, entry->helper_record,
                                                member_name))
...
void semcheck_register_type_helper(struct RecordType *record_info,
                                   SymTab_t *symtab) {
  ...
  TypeHelperEntry *entry =
      (TypeHelperEntry *)calloc(1, sizeof(TypeHelperEntry));
  if (entry == NULL)
    return;
  entry->base_type_id = semcheck_normalize_helper_base_type_id(
      symtab, record_info->helper_base_type_id);
  entry->base_type_tag =
      semcheck_map_builtin_type_name(symtab, record_info->helper_base_type_id);
...
struct RecordType *semcheck_lookup_type_helper(SymTab_t *symtab,
  ...
  if (current_owner != NULL && symtab != NULL) {
    ...
    if (owner_record != NULL && owner_record->is_type_helper &&
      ...
      int owner_base_tag = semcheck_map_builtin_type_name(
          symtab, owner_record->helper_base_type_id);
      if (type_name != NULL) {
        if (pascal_identifier_equals(owner_record->helper_base_type_id,
                                     type_name))
          return owner_record;
      } else if (base_type_tag != UNKNOWN_TYPE &&
                 owner_base_tag == base_type_tag)
        return owner_record;
    }
  }
  if (kgpc_getenv("KGPC_DEBUG_TYPE_HELPER") != NULL) {
    fprintf(stderr,
            ...
            base_type_tag, type_name != NULL ? type_name : "<null>");
  }
  ListNode_t *cur = type_helper_entries;
  while (cur != NULL) {
    TypeHelperEntry *entry = (TypeHelperEntry *)cur->cur;
    if (entry != NULL) {
      if (kgpc_getenv("KGPC_DEBUG_TYPE_HELPER") != NULL) {
        fprintf(stderr,
                "[KGPC]   helper_entry: base_type_tag=%d base_type_id=%s "
                "helper=%s\n",
                entry->base_type_tag,
                entry->base_type_id != NULL ? entry->base_type_id : "<null>",
                entry->helper_record && entry->helper_record->type_id
                    ? entry->helper_record->type_id
                    : "<null>");
      }
  ...
  if (type_name != NULL) {
    ...
    if (fallback_name != NULL &&
        !pascal_identifier_equals(fallback_name, type_name))
...
struct RecordType *
semcheck_lookup_type_helper_for_member(SymTab_t *symtab, int base_type_tag,
                                       const char *type_name,
                                       const char *member_name) {
  if (member_name == NULL)
    return semcheck_lookup_type_helper(symtab, base_type_tag, type_name);

  struct RecordType *preferred =
      semcheck_lookup_type_helper(symtab, base_type_tag, type_name);
  if (preferred != NULL) {
...
int ensure_with_capacity(void) {
  ...
  size_t new_capacity =
      with_context_capacity == 0 ? 8 : with_context_capacity * 2;
  WithContextEntry *new_stack =
      realloc(with_context_stack, new_capacity * sizeof(*with_context_stack));
  if (new_stack == NULL)
    return 1;
...
int semcheck_with_push(struct Expression *context_expr,
...
struct RecordType *resolve_record_type_for_with(SymTab_t *symtab,
  ...
  if (context_expr->type == EXPR_FUNCTION_CALL) {
    ...
    semcheck_try_reinterpret_as_typecast(&cast_type, symtab, context_expr,
                                         INT_MAX);
  }
  ...
  if (context_expr->type == EXPR_TYPECAST) {
    ...
    const TypeRef *target_ref =
        context_expr->expr_data.typecast_data.target_type_ref;
  ...
  if (expr_type == POINTER_TYPE) {
    ...
    if (record_info == NULL && context_expr->type == EXPR_FUNCTION_CALL) {
      HashNode_t *func_node =
          context_expr->expr_data.function_call_data.resolved_func;
      if (func_node != NULL && func_node->type != NULL) {
        ...
        if (ret_type != NULL && kgpc_type_is_pointer(ret_type) &&
          ...
          if (alias_name != NULL) {
            ...
            if (FindSymbol(&alias_node, symtab, alias_name) != 0 &&
                alias_node != NULL)
...
struct RecordType *
semcheck_with_resolve_record_type(SymTab_t *symtab,
                                  struct Expression *context_expr,
                                  int expr_type, int line_num) {
  return resolve_record_type_for_with(symtab, context_expr, expr_type,
                                      line_num);
}

int semcheck_with_try_resolve(const char *field_id, SymTab_t *symtab,
                              struct Expression **out_record_expr,
                              int line_num) {
  ...
  for (size_t index = with_context_count; index > 0; --index) {
    ...
    long long offset = 0;
    if (resolve_record_field(symtab, entry->record_type, field_id, &field_desc,
                             &offset, line_num, 1) == 0 &&
        field_desc != NULL) {
    ...
    struct ClassProperty *prop = semcheck_find_class_property(
        symtab, entry->record_type, field_id, NULL);
    if (prop != NULL) {
      struct Expression *clone = clone_expression(entry->context_expr);
      if (clone == NULL)
        return -1;
...
int semcheck_with_try_resolve_method(const char *method_id, SymTab_t *symtab,
  ...
  for (size_t index = with_context_count; index > 0; --index) {
    ...
    HashNode_t *method_node =
        semcheck_find_class_method(symtab, entry->record_type, method_id, NULL);
    if (method_node == NULL && entry->record_type->type_id != NULL &&
      ...
      struct RecordType *helper_record =
          semcheck_lookup_type_helper_for_record_member(
              symtab, entry->record_type, method_id);
      if (helper_record != NULL)
        method_node =
            semcheck_find_class_method(symtab, helper_record, method_id, NULL);
    }
    if (method_node != NULL) {
      struct Expression *clone = clone_expression(entry->context_expr);
      if (clone == NULL)
    ...
    /* Also check record fields with procedural type (e.g. CompareFn:
     * TCompareFunc) */
    struct RecordField *field_desc = NULL;
    long long offset = 0;
    ...
    if (rf_result == 0 && field_desc != NULL) {
      /* Check if field has a procedural type, either directly or via type_id
       * lookup */
