...
semcheck_array_element_record_from_kgpc(SymTab_t *symtab,
...
KgpcType *semcheck_expr_effective_kgpc_type(SymTab_t *symtab,
  ...
  if (resolved != NULL && resolved->kind == TYPE_KIND_POINTER &&
      expr->pointer_subtype_id != NULL) {
    KgpcType *inner = resolved->info.points_to;
    if (inner == NULL || (inner->kind == TYPE_KIND_PRIMITIVE &&
  ...
  if (resolved == NULL && expr->pointer_subtype == POINTER_TYPE &&
      expr->pointer_subtype_id != NULL) {
    HashNode_t *type_node = NULL;
    if (FindSymbol(&type_node, symtab, expr->pointer_subtype_id) != 0 &&
        type_node != NULL && type_node->type != NULL) {
...
int semcheck_class_type_ids_compatible(SymTab_t *symtab, const char *formal_id,
  ...
  struct RecordType *formal_record =
      semcheck_lookup_record_type(symtab, formal_id);
  struct RecordType *actual_record =
      semcheck_lookup_record_type(symtab, actual_id);
...
static HashNode_t *semcheck_pick_type_node_with_unit_preference(
...
struct RecordType *semcheck_lookup_record_type(SymTab_t *symtab,
  ...
  if (alias != NULL && alias->target_type_id != NULL) {
    HashNode_t *target_node = semcheck_pick_type_node_with_unit_preference(
        symtab, alias->target_type_id, type_node->defined_in_unit);
    if (target_node == NULL)
      target_node =
...
void semcheck_set_array_info_from_alias(struct Expression *expr,
  ...
  if (alias->array_element_type_id != NULL) {
    ...
    if (expr->array_element_type_id == NULL)
      fprintf(stderr,
              "Error: failed to allocate array element type identifier.\n");
  }
  ...
  if (expr->array_element_type == UNKNOWN_TYPE &&
    ...
    if (resolve_type_identifier(&resolved_type, symtab,
                                expr->array_element_type_id, line_num) == 0)
...
void semcheck_set_array_info_from_hashnode(struct Expression *expr,
  ...
  if (node->type == NULL || !(kgpc_type_is_array(node->type) ||
                              kgpc_type_is_array_of_const(node->type))) {
    if (alias != NULL && alias->is_array)
  ...
  if (node->type != NULL && kgpc_type_is_array(node->type)) {
    KgpcType *elem_type =
        kgpc_type_get_array_element_type_resolved(node->type, symtab);
    if (elem_type != NULL) {
      ...
      expr->array_element_type_id =
          strdup(node->type->info.array_info.element_type_id);
    }
  }
  ...
  if (alias != NULL && alias->is_array) {
    ...
    if (expr->array_element_size <= 0 && node_element_size > 0)
      ...
      if (count > 0 && alias->array_element_type != UNKNOWN_TYPE) {
        ...
        if (sizeof_from_type_ref(symtab, alias->array_element_type,
                                 alias->array_element_type_id, &element_size, 0,
                                 line_num) == 0) {
    ...
    if (element_type != NULL) {
      if (element_type->kind == TYPE_KIND_RECORD) {
        ...
        /* Element is itself an array (nested array) - get its type alias if
         * available */
        ...
        if (element_alias != NULL && element_alias->is_array) {
          ...
          if (element_alias->array_element_type_id != NULL) {
            expr->array_element_type_id =
                strdup(element_alias->array_element_type_id);
            if (expr->array_element_type_id == NULL)
              fprintf(
                  stderr,
                  "Error: failed to allocate array element type identifier.\n");
        ...
        /* If the element type has a type_alias with pointer info, use its
         * type_id */
        struct TypeAlias *element_alias = element_type->type_alias;
        if (element_alias != NULL && element_alias->is_pointer &&
            element_alias->pointer_type_id != NULL) {
  ...
  if (expr->array_element_record_type == NULL &&
      expr->array_element_type == RECORD_TYPE && expr->expr_data.id != NULL &&
      symtab != NULL) {
      ...
      if (self_record != NULL) {
        ...
        struct RecordField *field_desc =
            semcheck_find_class_field_including_hidden(
                symtab, self_record, expr->expr_data.id, &field_owner);
        if (field_desc != NULL) {
          ...
          if (expr->array_element_record_type == NULL &&
            ...
            expr->array_element_record_type =
                semcheck_array_element_record_from_kgpc(
                    symtab, field_desc->array_element_kgpc_type);
          }
          if (expr->array_element_record_type == NULL &&
