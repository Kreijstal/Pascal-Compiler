...
if (ctx->expr->expr_data.function_call_data.is_call_info_valid) {
    if (ctx->expr->expr_data.function_call_data.resolved_func != NULL) {
        ...
        ctx->scope_return = 1;
        ctx->overload_candidates = CreateListNode(ctx->hash_return, LIST_UNSPECIFIED);
        if (ctx->expr->expr_data.function_call_data.mangled_id != NULL)
            ctx->mangled_name = strdup(ctx->expr->expr_data.function_call_data.mangled_id);
        ...
        if (ctx->type_return != NULL) {
            KgpcType *cached_type = ctx->expr->expr_data.function_call_data.call_kgpc_type;
...
if (ctx->expr->expr_data.function_call_data.is_procedural_var_call) {
    ...
    {
        struct Expression *proc_expr = ctx->expr->expr_data.function_call_data.procedural_var_expr;
        ctx->return_val += semcheck_expr_with_type(NULL, ctx->symtab, proc_expr, ctx->max_scope_lev, NO_MUTATE);
        ...
        if (resolved == NULL || resolved->kind != TYPE_KIND_PROCEDURE)
        {
            ...
            if (ctx->id != NULL && FindSymbol(&type_node, ctx->symtab, ctx->id) != 0 &&
    ...
    if (ctx->type_return != NULL)
    {
        KgpcType *call_type = ctx->expr->expr_data.function_call_data.call_kgpc_type;
        KgpcType *ret_type = NULL;
        if (kgpc_getenv("KGPC_DEBUG_PROC_VAR") != NULL &&
            ctx->expr->expr_data.function_call_data.id != NULL &&
            pascal_identifier_equals(ctx->expr->expr_data.function_call_data.id, "Ctr"))
        {
            fprintf(stderr,
            ...
            if (pve2 != NULL && pve2->type == EXPR_RECORD_ACCESS)
            {
                ...
                if (recv2 != NULL && recv2->type == EXPR_VAR_ID && fname2 != NULL)
                {
                        ...
                        rn != NULL)
                    {
                        ...
                        {
                            long long foff = 0;
                            resolve_record_field(ctx->symtab, rrt, fname2,
                                                 &pve_field_desc, &foff,
                                                 ctx->expr->line_num, 1);
                        }
                    }
...
ctx->args_given = ctx->expr->expr_data.function_call_data.args_expr;
if (ctx->expr != NULL)
    ctx->is_operator_dispatch = ctx->expr->expr_data.function_call_data.is_operator_call;
if (ctx->id != NULL)
{
    const char *qualifier = ctx->expr->expr_data.function_call_data.call_qualifier;
    if (qualifier != NULL)
    {
        ...
        if (!prefix_is_unit)
        {
            ...
            if (receiver_expr != NULL)
            {
                ...
                ctx->expr->expr_data.function_call_data.placeholder_method_name = strdup(ctx->id);
            }
        }
        else
        {
...
if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
    fprintf(stderr, "[SemCheck] semcheck_funccall: id='%s' expr=%p resolved_func=%p\n", 
        ctx->id != NULL ? ctx->id : "(null)", (void*)ctx->expr, (void*)ctx->expr->expr_data.function_call_data.resolved_func);
}
    ...
    pascal_identifier_equals(ctx->id, "Format"))
{
    ...
    {
        struct Expression *first = (struct Expression *)ctx->expr->expr_data.function_call_data.args_expr->cur;
    ...
    ctx->args_given != NULL && ctx->args_given->cur != NULL)
{
    ...
    if (normalize_not_receiver)
    {
        ...
        if (call_expr == NULL)
        {
            semcheck_error_with_context_at(ctx->expr->line_num, ctx->expr->col_num, ctx->expr->source_index, "Error on line %d: failed to allocate expression for NOT call fixup.\n",
                ctx->expr->line_num);
        ...
        *call_expr = *ctx->expr;
        ...
        memset(&ctx->expr->expr_data.relop_data, 0, sizeof(ctx->expr->expr_data.relop_data));
        ...
        do { ctx->final_status = semcheck_relop(ctx->type_return, ctx->symtab, ctx->expr, ctx->max_scope_lev, ctx->mutating); return FC_CLEANUP; } while (0);
    }
}
 ...
 * (without "__") exists. */
if (ctx->expr->expr_data.function_call_data.is_method_call_placeholder && ctx->args_given != NULL)
{
    struct Expression *first_arg = (struct Expression *)ctx->args_given->cur;
    ...
    if (first_arg != NULL && first_arg->record_type == NULL)
    {
        ...
        {
            (void)semcheck_expr_with_type(NULL, ctx->symtab, first_arg, ctx->max_scope_lev, NO_MUTATE);
        }
        if (first_arg->resolved_kgpc_type != NULL &&
    ...
    if (first_arg != NULL && first_arg->record_type != NULL &&
        !record_type_is_class(first_arg->record_type))
    {
        const char *method_name = (ctx->expr->expr_data.function_call_data.placeholder_method_name != NULL)
            ? ctx->expr->expr_data.function_call_data.placeholder_method_name : ctx->id;
        if (method_name != NULL)
        {
            struct RecordType *actual_method_owner = NULL;
            HashNode_t *method_node = semcheck_find_class_method(ctx->symtab, first_arg->record_type,
            ...
            if (method_node != NULL)
            {
                ...
                if (mangled_method_name != NULL)
                {
                    ctx->mangled_name = MangleFunctionNameFromCallSite(mangled_method_name,
                        ctx->args_given, ctx->symtab, ctx->max_scope_lev);
                    if (ctx->mangled_name == NULL)
    ...
    if (first_arg != NULL && first_arg->type == EXPR_VAR_ID && first_arg->expr_data.id != NULL)
    {
        if (semcheck_is_unit_name(first_arg->expr_data.id))
        {
            ...
            int receiver_is_local = (FindSymbol(&local_sym, ctx->symtab, first_arg->expr_data.id) != 0 &&
                local_sym != NULL &&
            ...
            if (!receiver_is_local)
            {
            char *real_func_name = (ctx->expr->expr_data.function_call_data.placeholder_method_name != NULL)
                ? strdup(ctx->expr->expr_data.function_call_data.placeholder_method_name)
                : NULL;
            if (real_func_name != NULL)
            {
                ListNode_t *func_candidates = FindAllIdents(ctx->symtab, real_func_name);
    ...
    if (first_arg != NULL && first_arg->type == EXPR_VAR_ID && first_arg->expr_data.id != NULL)
    {
            ...
            ctx->expr->expr_data.function_call_data.placeholder_method_name != NULL)
        {
            ...
            if (base_name != NULL)
            {
                ...
                {
                    const char *method_name = ctx->expr->expr_data.function_call_data.placeholder_method_name;
                    ...
                    if (mangled_method != NULL)
                    {
                        ...
                        ListNode_t *method_candidates = FindAllIdents(ctx->symtab, mangled_method);
                        if (method_candidates != NULL)
                        {
        ...
        if (is_unit_qualifier)
        {
            ...
            if (FindSymbol(&var_shadow, ctx->symtab, first_arg->expr_data.id) != 0 &&
                var_shadow != NULL &&
            ...
            if (is_unit_qualifier)
            {
                ...
                if (FindSymbol(&self_node2, ctx->symtab, "Self") != 0 && self_node2 != NULL)
                {
                    struct RecordType *self_rec2 = get_record_type_from_node(self_node2);
                    if (self_rec2 != NULL &&
                        semcheck_find_class_field_including_hidden(ctx->symtab, self_rec2,
        ...
        if (!is_unit_qualifier)
        {
            ...
            if (FindSymbol(&first_node, ctx->symtab, first_arg->expr_data.id) == 0 ||
                first_node == NULL)
            {
                ...
                if (FindSymbol(&self_node, ctx->symtab, "Self") != 0 && self_node != NULL)
                {
                    ...
                    if (self_record != NULL &&
                        semcheck_find_class_field_including_hidden(ctx->symtab, self_record,
                            first_arg->expr_data.id, NULL) != NULL)
                    {
                    ...
                    from_cparser_is_method_static(first_arg->expr_data.id, ctx->id))
                {
                    ...
                    if (mangled_method != NULL)
                    {
                        ...
                        {
                            free(ctx->expr->expr_data.function_call_data.id);
                            ctx->expr->expr_data.function_call_data.id = strdup(mangled_method);
            ...
            else if (first_node->hash_type == HASHTYPE_TYPE)
            {
                ...
                if (ctx->id != NULL && FindSymbol(&nested_type_node, ctx->symtab, ctx->id) != 0 &&
                ...
                if (!can_strip && ctx->id != NULL && first_arg->expr_data.id != NULL)
                {
                    size_t recv_len = strlen(first_arg->expr_data.id);
                    size_t id_len = strlen(ctx->id);
                    ...
                    if (qualified_id != NULL)
                    {
                        ...
                        {
                            free(ctx->expr->expr_data.function_call_data.id);
                            ctx->expr->expr_data.function_call_data.id = strdup(qualified_id);
                            ctx->id = ctx->expr->expr_data.function_call_data.id;
                            can_strip = 1;
                        }
                        free(qualified_id);
...
if (is_unit_qualifier)
{
            if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL)
            {
                fprintf(stderr, "[SemCheck] funccall unit-qual strip: receiver=%s id=%s\n",
                    first_arg->expr_data.id, ctx->id != NULL ? ctx->id : "(null)");
            }
            if (ctx->unit_qualifier_name == NULL && first_arg->expr_data.id != NULL)
                ctx->unit_qualifier_name = strdup(first_arg->expr_data.id);
            ListNode_t *remaining_args = ctx->args_given->next;
