    ...
    ctx->expr->expr_data.function_call_data.mangled_id != NULL)
{
    ...
    if (ctx->hash_return->type != NULL && !ctx->expr->expr_data.function_call_data.is_call_info_valid) {
        semcheck_expr_set_call_kgpc_type(ctx->expr, ctx->hash_return->type, 0);
        ctx->expr->expr_data.function_call_data.call_hash_type = ctx->hash_return->hash_type;
...
{
    const char *owner_class = ctx->expr->expr_data.function_call_data.cached_owner_class;
    const char *method_name = ctx->expr->expr_data.function_call_data.cached_method_name;
 ...
 * FindAllIdents(ctx->id) above cannot see — only the child-class forward declaration
...
if (ctx->expr->expr_data.function_call_data.resolved_func != NULL)
{
    HashNode_t *early_resolved = ctx->expr->expr_data.function_call_data.resolved_func;
        ...
        early_resolved->hash_type == HASHTYPE_PROCEDURE)
    {
        ...
        {
            ctx->overload_candidates = CreateListNode(early_resolved, LIST_UNSPECIFIED);
        }
    }
}
if (!ctx->was_unit_qualified && ctx->overload_candidates != NULL)
{
    ...
    if (has_local_candidate)
    {
        ...
        for (ListNode_t *cn = ctx->overload_candidates; cn != NULL; cn = cn->next)
        {
            HashNode_t *hn = (HashNode_t *)cn->cur;
            if (hn != NULL &&
                semcheck_candidate_is_local_callable_for_shadowing(ctx->symtab, hn) &&
...
if (ctx->was_unit_qualified && ctx->unit_qualifier_name != NULL && ctx->overload_candidates != NULL)
{
    ListNode_t *filtered = NULL;
    for (ListNode_t *cn = ctx->overload_candidates; cn != NULL; cn = cn->next)
    {
        ...
        if (hn->source_unit_index != 0)
        {
            ...
            if (src_name != NULL && pascal_identifier_equals(src_name, ctx->unit_qualifier_name))
                match = 1;
        }
...
if (ctx->overload_candidates == NULL && ctx->id != NULL && ctx->args_given != NULL)
{
    struct Expression *first_arg = (struct Expression *)ctx->args_given->cur;
    ...
    if (first_arg != NULL)
    {
        arg_type_local = semcheck_resolve_expression_kgpc_type(ctx->symtab,
            first_arg, ctx->max_scope_lev, NO_MUTATE, &arg_type_owned);
        ...
        if (arg_type_local != NULL && arg_type_local->kind == TYPE_KIND_RECORD &&
            arg_type_local->info.record_info != NULL)
        {
            helper_record = semcheck_lookup_type_helper_for_record_member(ctx->symtab,
                arg_type_local->info.record_info, ctx->id);
        }
        if (helper_record == NULL)
        {
            helper_record = semcheck_lookup_type_helper_for_member(ctx->symtab,
                helper_tag, helper_name, ctx->id);
        }
            ...
            first_arg->expr_data.id != NULL)
        {
                ...
                var_node != NULL)
            {
                ...
                if (var_helper_name != NULL)
                    helper_record = semcheck_lookup_type_helper_for_member(ctx->symtab,
                        UNKNOWN_TYPE, var_helper_name, ctx->id);
            }
        }
    }
    ...
    if (helper_record != NULL)
    {
        HashNode_t *method_node = semcheck_find_class_method(ctx->symtab,
            helper_record, ctx->id, &actual_method_owner);
            ...
            owner_for_mangle->type_id != NULL)
        {
            ...
            if (candidate_name != NULL)
            {
                snprintf(candidate_name, class_len + 2 + method_len + 1,
                    "%s__%s", owner_for_mangle->type_id, ctx->id);
...
{
    HashNode_t *cand = (ctx->overload_candidates != NULL) ? (HashNode_t *)ctx->overload_candidates->cur : NULL;
    fprintf(stderr,
...
if (ctx->overload_candidates != NULL && ctx->overload_candidates->cur != NULL)
{
        ...
        first_candidate->type->kind == TYPE_KIND_PROCEDURE)
    {
        ...
        {
            HashNode_t *ret_node = semcheck_find_preferred_type_node(ctx->symtab,
        ...
        if (semcheck_count_total_params(formal_params) != ListLength(ctx->args_given))
        {
        ...
        while (formal != NULL && actual != NULL)
        {
            ...
            semcheck_expr_with_type(&actual_kgpc_type_proc, ctx->symtab, actual_expr, ctx->max_scope_lev, NO_MUTATE);
        ...
        if (return_type != NULL)
        {
            ...
            semcheck_expr_set_resolved_type(ctx->expr, *ctx->type_return);  /* Also set resolved_type explicitly */
        }
        else
        {
...
if (ctx->mangled_name != NULL && ctx->overload_candidates != NULL)
{
    ...
    if (mangled_matches == 1 && first_match != NULL && ListLength(ctx->overload_candidates) > 1)
    {
        for (ListNode_t *check = ctx->overload_candidates; check != NULL; check = check->next)
        {
            ...
            if (other != first_match && other != NULL &&
                ...
                semcheck_candidates_share_signature(ctx->symtab, first_match, other) &&
                !semcheck_candidates_string_subtypes_differ(ctx->symtab, first_match, other))
            {
    ...
    if (best_match != NULL && semcheck_call_has_noarg_identifier_calls(ctx->args_given))
    {
...
if (best_match == NULL)
{
    ...
    {
        int idx = 0;
        for (ListNode_t *cur = ctx->args_given; cur != NULL; cur = cur->next, idx++)
        {
            struct Expression *arg_expr = (struct Expression *)cur->cur;
            KgpcType *arg_type_local = NULL;
            semcheck_expr_with_type(&arg_type_local, ctx->symtab, arg_expr, ctx->max_scope_lev, NO_MUTATE);
    ...
    int resolve_status = semcheck_resolve_overload(&best_match,
        &num_best_matches, ctx->overload_candidates, ctx->args_given, ctx->symtab, ctx->expr,
        ctx->max_scope_lev, ctx->prefer_non_builtin);
    if (FUNCCALL_TIMINGS_ENABLED()) {
        fprintf(stderr, "[timing] funccall resolve_overload id=%s: %.2f ms\n",
            ctx->id != NULL ? ctx->id : "(null)", funccall_now_ms() - resolve_t0);
    }
    if (resolve_status == 3)
    {
        *ctx->type_return = UNKNOWN_TYPE;
...
if (best_match != NULL && ctx->overload_candidates != NULL && ctx->overload_candidates->next != NULL)
{
    HashNode_t *override_match = NULL;
    int override_count = 0;
    int override_status = semcheck_resolve_overload(&override_match,
        &override_count, ctx->overload_candidates, ctx->args_given, ctx->symtab, ctx->expr,
        ctx->max_scope_lev, ctx->prefer_non_builtin);
        ...
        override_match != best_match)
    {
        ...
        if (!semcheck_candidates_string_subtypes_differ(ctx->symtab, best_match, override_match))
        {
    ...
    best_match->type != NULL && best_match->type->kind == TYPE_KIND_PROCEDURE)
{
    ...
    if (best_ret == NULL && best_match->type->info.proc_info.return_type_id == NULL)
    {
        ...
        for (ListNode_t *cur = ctx->overload_candidates; cur != NULL; cur = cur->next)
        {
            HashNode_t *candidate = (HashNode_t *)cur->cur;
            if (candidate == NULL ||
                (candidate->hash_type != HASHTYPE_FUNCTION &&
        ...
        if (value_candidates != NULL)
        {
            ...
            int value_status = semcheck_resolve_overload(&value_match,
                &value_count, value_candidates, ctx->args_given, ctx->symtab, ctx->expr,
                ctx->max_scope_lev, ctx->prefer_non_builtin);
...
if (num_best_matches == 1)
{
        ...
        best_match->type->kind == TYPE_KIND_PROCEDURE)
    {
            ...
            best_match->owner_class != NULL && best_match->method_name != NULL)
        {
            ...
            if (FindSymbol(&class_node, ctx->symtab, best_match->owner_class) != 0 && class_node != NULL)
            {
              ...
              strcmp(best_match->mangled_id, best_match->id) == 0)))
        {
            char *computed = MangleFunctionName(best_match->id,
                best_match->type->info.proc_info.params, ctx->symtab);
            if (computed != NULL)
            {
    ...
    if (best_match->type != NULL && best_match->type->kind == TYPE_KIND_PROCEDURE)
    {
        ...
        if (proc_def != NULL)
        {
            ...
            if (no_body)
            {
                ...
                {
                    free(ctx->expr->expr_data.function_call_data.mangled_id);
                    ctx->expr->expr_data.function_call_data.mangled_id = strdup(target_name);
                }
            }
    ...
    {
        struct RecordType *iface_record = semcheck_lookup_record_type(ctx->symtab,
            best_match->owner_class);
            ...
            iface_record->method_templates != NULL)
        {
            ...
            ListNode_t *call_args = ctx->expr->expr_data.function_call_data.args_expr;
            if (call_args != NULL)
            {
    ...
    if (!ctx->expr->expr_data.function_call_data.is_class_method_call)
    {
        ...
        if (has_self_param)
        {
            ...
            {
                struct RecordType *check_record = semcheck_lookup_record_type(ctx->symtab, cm_class);
    ...
    if (ctx->expr->expr_data.function_call_data.is_virtual_call)
    {
        KGPC_SEMCHECK_HARD_ASSERT(
            ctx->expr->expr_data.function_call_data.cached_owner_class != NULL &&
            ctx->expr->expr_data.function_call_data.cached_owner_class[0] != '\0',
            "virtual expression call '%s' resolved without owner metadata",
            ctx->id != NULL ? ctx->id : "(unknown)");
        KGPC_SEMCHECK_HARD_ASSERT(
            ctx->expr->expr_data.function_call_data.cached_method_name != NULL &&
            ctx->expr->expr_data.function_call_data.cached_method_name[0] != '\0',
...
{
    if (ctx->id != NULL &&
        pascal_identifier_equals(ctx->id, "AllocMem") &&
        !ctx->expr->expr_data.function_call_data.is_method_call_placeholder &&
        ctx->expr->expr_data.function_call_data.call_qualifier == NULL)
        do { ctx->final_status = semcheck_builtin_allocmem(ctx->type_return, ctx->symtab, ctx->expr, ctx->max_scope_lev); return FC_CLEANUP; } while (0);

    if (ctx->id != NULL &&
        pascal_identifier_equals(ctx->id, "New") &&
        !ctx->expr->expr_data.function_call_data.is_method_call_placeholder &&
        ctx->expr->expr_data.function_call_data.call_qualifier == NULL)
        ...
        !ctx->expr->expr_data.function_call_data.is_method_call_placeholder)
    {
        ...
        int wm = semcheck_with_try_resolve_method(ctx->id, ctx->symtab, &with_expr, ctx->expr->line_num);
        if ((wm == 0 || wm == 2) && with_expr != NULL)
        {
            if (wm == 2)
            {
                ...
                if (field_access != NULL)
                {
                    ...
                    field_access->expr_data.record_access_data.field_id = strdup(ctx->id);
                    ...
                    semcheck_expr_with_type(&field_kgpc_type, ctx->symtab, field_access, ctx->max_scope_lev, NO_MUTATE);
                    ...
                    if (proc_kgpc_type != NULL && proc_kgpc_type->kind == TYPE_KIND_PROCEDURE)
                    {
                        ...
                        ctx->expr->expr_data.function_call_data.procedural_var_expr = field_access;
                        ctx->expr->expr_data.function_call_data.is_method_call_placeholder = 0;
                        ctx->expr->expr_data.function_call_data.call_kgpc_type = proc_kgpc_type;
                        kgpc_type_retain(proc_kgpc_type);
                        ctx->expr->expr_data.function_call_data.is_call_info_valid = 1;

                        /* Type-check arguments */
                        for (ListNode_t *arg_cur = ctx->expr->expr_data.function_call_data.args_expr;
                             arg_cur != NULL; arg_cur = arg_cur->next)
                        {
                            struct Expression *arg = (struct Expression *)arg_cur->cur;
                            if (arg != NULL)
                                semcheck_expr_with_type(NULL, ctx->symtab, arg, ctx->max_scope_lev, NO_MUTATE);
                        }
            ...
            else
            {
                ...
                if (self_node != NULL)
                {
                    ...
                    ctx->expr->expr_data.function_call_data.is_method_call_placeholder = 1;
                    if (ctx->expr->expr_data.function_call_data.placeholder_method_name != NULL)
                        free(ctx->expr->expr_data.function_call_data.placeholder_method_name);
                    ...
                    int retry_result = semcheck_funccall(ctx->type_return, ctx->symtab, ctx->expr,
                        ctx->max_scope_lev, ctx->mutating);
                    do { ctx->final_status = retry_result; return FC_CLEANUP; } while (0);
                }
                else
                {
                    destroy_expr(with_expr);
    ...
    /* Build detailed error message with argument types and available overloads */
    {
        if (kgpc_getenv("KGPC_DEBUG_OVERLOAD_FAIL") != NULL)
        {
            ...
            for (ListNode_t *cur = ctx->args_given; cur != NULL; cur = cur->next)
            {
                ...
                semcheck_expr_with_type(&arg_kgpc_type_dbg, ctx->symtab, arg, ctx->max_scope_lev, NO_MUTATE);
        ...
        if (!any_arg_unknown)
        {
            ...
            {
                semcheck_error_with_context_at(ctx->expr->line_num, ctx->expr->col_num, ctx->expr->source_index,
...
else
{
    ...
    semcheck_error_with_context_at(ctx->expr->line_num, ctx->expr->col_num, ctx->expr->source_index, "Error on line %d, call to function %s is ambiguous\n", ctx->expr->line_num, ctx->id);
    *ctx->type_return = UNKNOWN_TYPE;
    ctx->final_status = ++ctx->return_val;
    return FC_CLEANUP;
}
...
{
    semcheck_error_with_context_at(ctx->expr->line_num, ctx->expr->col_num, ctx->expr->source_index, "Error on line %d, undeclared function %s (mangled to %s)!\n\n", ctx->expr->line_num, ctx->id, ctx->mangled_name);
    ++ctx->return_val;
...
else
{
    ...
    if(0) /* scope depth check removed — tree scoping has no depth */
    {
        ...
        semcheck_error_with_context_at(ctx->expr->line_num, ctx->expr->col_num, ctx->expr->source_index, "Error on line %d, cannot change \"%s\", invalid scope!\n\n",
            ctx->expr->line_num, ctx->id);
    ...
    {
        semcheck_error_with_context_at(ctx->expr->line_num, ctx->expr->col_num, ctx->expr->source_index, "Error on line %d, \"%s\" is not a function!\n\n",
            ctx->expr->line_num, ctx->id);
        ++ctx->return_val;
    }
        ...
        ctx->hash_return->owner_class != NULL)
    {
        ...
        struct RecordType *ctor_owner = semcheck_lookup_record_type(ctx->symtab, ctx->hash_return->owner_class);
        if (ctor_owner != NULL)
        {
            ...
            {
                struct MethodTemplate *tmpl =
                    from_cparser_get_method_template(search, ctx->hash_return->method_name);
                if (tmpl != NULL)
                {
         ...
         pascal_identifier_equals(ctx->id, "GetString")))
    {
        ...
        if (ctx->hash_return->type != NULL && ctx->hash_return->type->kind == TYPE_KIND_PROCEDURE)
        {
    ...
    if (*ctx->type_return == PROCEDURE &&
        ctx->hash_return->type != NULL &&
    ...
    {
        const char *ret_id = ctx->hash_return->type->info.proc_info.return_type_id;
        KgpcType *ret_type = NULL;
        HashNode_t *ret_node = semcheck_find_type_node_with_kgpc_type(ctx->symtab, ret_id);
        if (ret_node != NULL && ret_node->type != NULL)
        {
    ...
    if (ctx->hash_return->type != NULL && ctx->hash_return->type->kind == TYPE_KIND_PROCEDURE)
    {
        ...
        if (return_type != NULL)
        {
            ...
            int skip_override_for_ctor = 0;
            {
                ...
                if (ctx->expr->expr_data.function_call_data.placeholder_method_name != NULL)
                    ctor_method = ctx->expr->expr_data.function_call_data.placeholder_method_name;
                else if (ctx->hash_return != NULL && ctx->hash_return->method_name != NULL)
                ...
                if (ctor_method != NULL && strncasecmp(ctor_method, "Create", 6) == 0)
                {
                    ...
                    int is_real_ctor = ctx->expr->expr_data.function_call_data.is_constructor_call;
                    if (is_real_ctor &&
            ...
            if (!skip_override_for_ctor)
                semcheck_expr_set_resolved_kgpc_type_shared(ctx->expr, return_type);
            semcheck_set_pointer_info_from_kgpc_type(ctx->expr, ctx->symtab, return_type);
            if (return_type->kind == TYPE_KIND_ARRAY)
                semcheck_set_array_info_from_kgpctype(ctx->expr, ctx->symtab, return_type, ctx->expr->line_num);
            else
                semcheck_clear_array_info(ctx->expr);
            if (return_type->kind == TYPE_KIND_PRIMITIVE &&
                return_type->info.primitive_type_tag == UNKNOWN_TYPE)
            {
                char *target_return_id = ctx->hash_return->type->info.proc_info.return_type_id;
                if (target_return_id != NULL)
                {
                    HashNode_t *type_node = semcheck_find_type_node_with_kgpc_type(ctx->symtab, target_return_id);
                    if (type_node != NULL && type_node->type != NULL)
                    {
                        destroy_kgpc_type(return_type);
                        kgpc_type_retain(type_node->type);
                        ctx->hash_return->type->info.proc_info.return_type = type_node->type;
                        return_type = type_node->type;
                        semcheck_expr_set_resolved_kgpc_type_shared(ctx->expr, type_node->type);
                        if (return_type->kind == TYPE_KIND_ARRAY)
                            semcheck_set_array_info_from_kgpctype(ctx->expr, ctx->symtab, return_type, ctx->expr->line_num);
                        else
                            semcheck_clear_array_info(ctx->expr);
    ...
    {
        semcheck_expr_set_resolved_kgpc_type_shared(ctx->expr, ctx->hash_return->type);
        semcheck_clear_array_info(ctx->expr);
    }

    if (ctx->hash_return->type != NULL && ctx->hash_return->type->kind == TYPE_KIND_PROCEDURE)
    {
        ListNode_t *formal_params = kgpc_type_get_procedure_params(ctx->hash_return->type);
        if (semcheck_append_default_args(&ctx->args_given, formal_params, ctx->expr->line_num) != 0)
            ++ctx->return_val;
        ctx->expr->expr_data.function_call_data.args_expr = ctx->args_given;
    }
    if (!ctx->is_operator_dispatch &&
        ctx->injected_self && ctx->hash_return != NULL && ctx->hash_return->type != NULL &&
        ctx->hash_return->type->kind == TYPE_KIND_PROCEDURE)
    {
                ...
                first_param->tree_data.var_decl_data.ids != NULL)
            {
                    ...
                    pascal_identifier_equals(ctx->id, "Format"))
                {
                    fprintf(stderr,
                        ...
                        resolved_expects_self);
                }
            }
        }
        if (!resolved_expects_self)
        {
            ListNode_t *first_arg_node = ctx->expr->expr_data.function_call_data.args_expr;
            if (first_arg_node != NULL && first_arg_node->cur != NULL)
            {
                struct Expression *first_arg = (struct Expression *)first_arg_node->cur;
                    ...
                    pascal_identifier_equals(first_arg->expr_data.id, "Self"))
                {
                    ...
                    {
                        fprintf(stderr,
                            "[KGPC_DEBUG_FORMAT] removed injected Self for non-method resolution\n");
                    }
                }
            }
        }
    ...
    if (kgpc_getenv("KGPC_DEBUG_FORMAT") != NULL && ctx->id != NULL &&
        pascal_identifier_equals(ctx->id, "Format"))
    {
        const char *owner = ctx->hash_return != NULL ? ctx->hash_return->owner_class : NULL;
        const char *hid = ctx->hash_return != NULL ? ctx->hash_return->id : NULL;
        fprintf(stderr,
            "[KGPC_DEBUG_FORMAT] resolved hash_return id=%s owner=%s mangled=%s\n",
            hid != NULL ? hid : "(null)",
            owner != NULL ? owner : "(null)",
            (ctx->hash_return != NULL && ctx->hash_return->mangled_id != NULL) ? ctx->hash_return->mangled_id : "(null)");
    }
    ...
    {
        fprintf(stderr,
            "[KGPC_DEBUG_FORMAT] args_given_ptr=%p args_to_validate_ptr=%p true_args_ptr=%p\n",
            (void *)ctx->args_given, (void *)args_to_validate, (void *)ctx->true_args);
        int idx = 0;
        for (ListNode_t *cur = args_to_validate; cur != NULL; cur = cur->next, idx++)
        {
            struct Expression *arg_expr = (struct Expression *)cur->cur;
            const char *arg_id = (arg_expr != NULL && arg_expr->type == EXPR_VAR_ID) ? arg_expr->expr_data.id : NULL;
            fprintf(stderr,
                "[KGPC_DEBUG_FORMAT] args_to_validate[%d]=node=%p expr=%p type=%d id=%s\n",
        ...
        ctx->hash_return->owner_class != NULL)
    {
        ...
        if (method_name == NULL &&
            ...
            method_name = ctx->expr->expr_data.function_call_data.placeholder_method_name;
        struct RecordType *record_info = semcheck_lookup_record_type(ctx->symtab, ctx->hash_return->owner_class);
        if (record_info != NULL && record_info->is_class && !record_info->is_type_helper &&
            ...
            !from_cparser_is_method_static(ctx->hash_return->owner_class, method_name))
        {
             ...
             ListLength(ctx->true_args) == ListLength(ctx->args_given) + 1)
    {
        ...
        if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
            ...
            for (ListNode_t *c = ctx->args_given; c != NULL; c = c->next) args_given_count++;
            ...
            fprintf(stderr, "[SemCheck] Constructor %s: args_given=%d, true_args=%d, args_to_validate=%d\n",
                ctx->id, args_given_count, true_args_count, args_to_validate_count);
        }
    ...
    while(true_args_to_validate != NULL && args_to_validate != NULL)
    {
        ...
        if (ctx->arg_decl->type != TREE_VAR_DECL && ctx->arg_decl->type != TREE_ARR_DECL)
        {
            semcheck_error_with_context_at(ctx->expr->line_num, ctx->expr->col_num, ctx->expr->source_index, "Error on line %d, unsupported parameter declaration in call to %s.\n",
                ctx->expr->line_num, ctx->id);
            ++ctx->return_val;
            true_args_to_validate = true_args_to_validate->next;
            ...
            current_arg_expr->expr_data.relop_data.right != NULL)
        {
            ...
            if (ctx->arg_decl->type == TREE_VAR_DECL &&
                ...
                expected_name = (const char *)ctx->arg_decl->tree_data.var_decl_data.ids->cur;
            else if (ctx->arg_decl->type == TREE_ARR_DECL &&
                ctx->arg_decl->tree_data.arr_decl_data.ids != NULL)
                expected_name = (const char *)ctx->arg_decl->tree_data.arr_decl_data.ids->cur;
            ...
            if (expected_name != NULL && given_name != NULL)
            {
                if (pascal_identifier_equals(expected_name, given_name))
                {
                    ...
                    semcheck_expr_with_type(&rhs_kgpc_type, ctx->symtab, named_value, ctx->max_scope_lev, NO_MUTATE);
                    rhs_type = semcheck_tag_from_kgpc(rhs_kgpc_type);
                    if (semcheck_named_arg_type_compatible(ctx->arg_decl, named_value, rhs_type, ctx->symtab))
                    {
                ...
                else if (semcheck_param_list_contains_name(ctx->true_args, given_name))
                {
                    named_arg_mismatch = 1;
                }
            }
        }
        if (semcheck_prepare_array_literal_argument(ctx->arg_decl, current_arg_expr,
                ctx->symtab, ctx->max_scope_lev, ctx->expr->line_num) != 0)
        {
            ++ctx->return_val;
            true_args_to_validate = true_args_to_validate->next;
            args_to_validate = args_to_validate->next;
        ...
        if (semcheck_prepare_record_constructor_argument(ctx->arg_decl, current_arg_expr,
                ctx->symtab, ctx->max_scope_lev, ctx->expr->line_num) != 0)
        {
            ++ctx->return_val;
            true_args_to_validate = true_args_to_validate->next;
        ...
        if (ctx->arg_decl->type == TREE_VAR_DECL && ctx->arg_decl->tree_data.var_decl_data.is_var_param)
        {
            arg_mutating = MUTATE;
        }
            ...
            current_arg_expr->expr_data.function_call_data.id != NULL)
        {
            ...
            HashNode_t *type_node = semcheck_find_preferred_type_node(ctx->symtab, type_id);
            if (type_node != NULL && type_node->hash_type == HASHTYPE_TYPE)
            {
                ...
                {
                    HashNode_t *tguid_node = semcheck_find_type_node_with_kgpc_type(ctx->symtab, "TGUID");
        ...
        {
            ctx->return_val += semcheck_expr_with_type(&arg_kgpc_type_call,
                ctx->symtab, current_arg_expr, ctx->max_scope_lev, arg_mutating);
            ctx->arg_type = semcheck_tag_from_kgpc(arg_kgpc_type_call);
        }
        if (ctx->arg_type == UNKNOWN_TYPE &&
            current_arg_expr != NULL &&
            current_arg_expr->type == EXPR_VAR_ID &&
            current_arg_expr->expr_data.id != NULL)
        {
            HashNode_t *type_node = NULL;
            if (FindSymbol(&type_node, ctx->symtab, current_arg_expr->expr_data.id) != 0 &&
        ...
        if (kgpc_getenv("KGPC_DEBUG_FORMAT") != NULL &&
            ctx->id != NULL && pascal_identifier_equals(ctx->id, "Format"))
        {
        ...
        if (kgpc_getenv("KGPC_DEBUG_COMPAREMEM") != NULL &&
            ctx->id != NULL && pascal_identifier_equals(ctx->id, "CompareMem"))
        {
            const char *kgpc_str = "<null>";
            if (current_arg_expr != NULL && current_arg_expr->resolved_kgpc_type != NULL)
                kgpc_str = kgpc_type_to_string(current_arg_expr->resolved_kgpc_type);
        ...
        while(ctx->true_arg_ids != NULL && args_to_validate != NULL)
        {
            ...
            {
                struct TypeAlias *inline_alias = ctx->arg_decl->tree_data.var_decl_data.inline_type_alias;
            ...
            if (!formal_is_array_decl)
            {
                ...
                KgpcType *formal_probe = resolve_type_from_vardecl(ctx->arg_decl, ctx->symtab, &owns_formal_probe);
            ...
            {
                fprintf(stderr,
                    "[KGPC_DEBUG_FORMAT] arg=%d expected_type=%d actual_type=%d expr_type=%d\n",
                    ctx->cur_arg,
                    expected_type,
                    ctx->arg_type,
                    current_arg_expr != NULL ? current_arg_expr->type : -1);
            ...
            if (param_is_untyped || expected_type == UNKNOWN_TYPE || ctx->arg_type == UNKNOWN_TYPE || expected_type == BUILTIN_ANY_TYPE)
            {
                ...
                kgpc_type_is_array(current_arg_expr->resolved_kgpc_type))
            {
                ...
                KgpcType *expected_kgpc = resolve_type_from_vardecl(ctx->arg_decl, ctx->symtab, &owns_expected_kgpc);
                ...
                {
                    if (are_types_compatible_for_assignment(expected_kgpc, arg_array, ctx->symtab))
                    {
                        array_compatible = 1;
                    }
                    else
                    {
                        KgpcType *expected_elem = kgpc_type_get_array_element_type_resolved(expected_kgpc, ctx->symtab);
                        KgpcType *arg_elem = kgpc_type_get_array_element_type_resolved(arg_array, ctx->symtab);
                        if (expected_elem != NULL && arg_elem != NULL)
                        {
                            if (kgpc_type_equals(expected_elem, arg_elem))
                                ...
                                array_compatible = 1;
                            else if (are_types_compatible_for_assignment(expected_elem, arg_elem, ctx->symtab))
                                array_compatible = 1;
                        }
                    }
                }
                ...
                if (!array_compatible)
                {
                    ...
                    semcheck_error_with_context_at(ctx->expr->line_num, ctx->expr->col_num, ctx->expr->source_index, "Error on line %d, on function call %s, argument %d: Type mismatch (expected: %s, given: %s)!\n\n",
                        ctx->expr->line_num, ctx->id, ctx->cur_arg, expected_str, given_str);
                    ++ctx->return_val;
                }
            }
            else if(ctx->arg_type != expected_type)
            {
                ...
                if (expected_kgpc != NULL && arg_kgpc != NULL &&
                    are_types_compatible_for_assignment(expected_kgpc, arg_kgpc, ctx->symtab))
                {
                    type_compatible = 1;
                }
                      ...
                      ctx->arg_decl->tree_data.var_decl_data.is_var_param))
                {
                    ...
                    if (semcheck_try_record_conversion_expression(ctx->symtab, &conv_expr, NULL,
                            expected_kgpc, &conv_type, &conv_owned))
                    {
                        args_to_validate->cur = conv_expr;
                        current_arg_expr = conv_expr;
                        if (are_types_compatible_for_assignment(expected_kgpc, conv_type, ctx->symtab))
                        {
                            type_compatible = 1;
                ...
                if (!type_compatible &&
                    is_integer_type(expected_type) && is_integer_type(ctx->arg_type))
                {
                    type_compatible = 1;
                }
                    ...
                    expected_type != PROCEDURE)
                {
                    KGPC_SEMCHECK_HARD_ASSERT(0,
                        ...
                        ctx->id != NULL ? ctx->id : "<unknown>", ctx->cur_arg, expected_type);
                }
                    ...
                    current_arg_expr->pointer_subtype_id != NULL)
                {
                    ...
                    if (ctx->arg_decl->type == TREE_VAR_DECL)
                        ...
                        formal_id = ctx->arg_decl->tree_data.arr_decl_data.type_id;
                    if (formal_id != NULL &&
                        semcheck_class_type_ids_compatible(ctx->symtab, formal_id,
                ...
                if (!type_compatible && current_arg_expr != NULL &&
                    (ctx->arg_type == STRING_TYPE || ctx->arg_type == SHORTSTRING_TYPE ||
                ...
                if (!type_compatible && current_arg_expr != NULL)
                {
                    if (expected_kgpc != NULL && expected_kgpc->kind == TYPE_KIND_ARRAY)
                    {
                        ...
                        {
                            if (are_types_compatible_for_assignment(expected_kgpc, arg_kgpc, ctx->symtab))
                                type_compatible = 1;
                            else
                            {
                                           ...
                                           kgpc_type_equals_tag(expected_elem, POINTER_TYPE))))
                                {
                                    type_compatible = 1;
                                         ...
                                         are_types_compatible_for_assignment(expected_elem, arg_elem, ctx->symtab))
                                {
                                    type_compatible = 1;
                                }
                            }
                    ...
                    current_arg_expr != NULL && current_arg_expr->type == EXPR_VAR_ID)
                {
                    ...
                    if (FindSymbol(&arg_node, ctx->symtab, current_arg_expr->expr_data.id) != 0 &&
                ...
                if (!type_compatible &&
                    ctx->id != NULL && pascal_identifier_equals(ctx->id, "supports") &&
                    ...
                    current_arg_expr->type == EXPR_FUNCTION_CALL)
                {
                    ...
                    if (fc_id != NULL) {
                        ...
                        if (FindSymbol(&fc_node, ctx->symtab, fc_id) &&
                            fc_node != NULL &&
                ...
                if (!type_compatible)
                {
                    ...
                    if (kgpc_getenv("KGPC_DEBUG_CALL_TYPES") != NULL &&
                        ...
                        (pascal_identifier_equals(ctx->id, "FileDateToDateTime") ||
                         ...
                         (ctx->hash_return != NULL && ctx->hash_return->id != NULL &&
                          pascal_identifier_equals(ctx->hash_return->id, "TGUIDHelper__Create"))))
                    {
                        fprintf(stderr,
                         ...
                         current_arg_expr->is_array_expr))
                    {
                        ...
                        semcheck_expr_with_type(&new_arg_kgpc_type, ctx->symtab, addr_expr, ctx->max_scope_lev, NO_MUTATE);
                    ...
                    if (!type_compatible && expected_type == POINTER_TYPE && ctx->arg_type == CHAR_TYPE)
                    {
                        int expected_is_pchar = 0;
                        int owns_expected = 0;
                        KgpcType *expected_kgpc = resolve_type_from_vardecl(ctx->arg_decl, ctx->symtab, &owns_expected);
    ...
    if(true_args_to_validate == NULL && args_to_validate != NULL)
    {
        ...
        if (ctx->expr->expr_data.function_call_data.resolved_func != NULL &&
            ctx->expr->expr_data.function_call_data.resolved_func->owner_class != NULL)
        {
        ...
        if (!allow_forward_params && !(ctx->hash_return != NULL && ctx->hash_return->is_varargs))
        {
            semcheck_error_with_context_at(ctx->expr->line_num, ctx->expr->col_num, ctx->expr->source_index, "Error on line %d, on function call %s, too many arguments given!\n\n",
                ctx->expr->line_num, ctx->id);
            ++ctx->return_val;
        }
        }
    ...
    {
        semcheck_error_with_context_at(ctx->expr->line_num, ctx->expr->col_num, ctx->expr->source_index, "Error on line %d, on function call %s, not enough arguments given!\n\n",
            ctx->expr->line_num, ctx->id);
        ++ctx->return_val;
    }
}
