...
const char *semcheck_base_type_name(const char *id) { return id; }
...
HashNode_t *semcheck_find_type_node_in_owner_chain(SymTab_t *symtab,
  ...
  if (owner_full != NULL) {
    ...
    if (qualified_name != NULL) {
      snprintf(qualified_name, owner_len + 1 + type_len + 1, "%s.%s",
               owner_full, type_id);
...
int resolve_type_identifier_ref(int *out_type, SymTab_t *symtab,
                                const char *type_id, const TypeRef *type_ref,
                                int line_num) {
  ...
  if (type_node == NULL) {
    ...
    type_node = semcheck_find_type_node_in_owner_chain(symtab, type_id,
    ...
    if (type_node == NULL) {
      for (int u = 0; u < SYMTAB_MAX_UNITS; u++) {
        if (symtab->unit_scopes[u] != NULL &&
          ...
          type_node =
              FindTypeBySuffixInTable(symtab->unit_scopes[u]->table, type_id);
          if (type_node != NULL)
            break;
        }
      }
  ...
  if (type_node == NULL) {
    ...
    semcheck_error_with_context(
        "Error on line %d, typecast references unknown type %s!\n\n", line_num,
        type_id);
    return 1;
  }
  ...
  if (type_node->hash_type != HASHTYPE_TYPE) {
    semcheck_error_with_context(
        "Error on line %d, %s is not a type identifier.\n\n", line_num,
        type_id);
    return 1;
  }
  ...
  if (hash_node->type != NULL) {
    ...
    if (hash_node->type->kind == TYPE_KIND_PRIMITIVE) {
      *type = kgpc_type_get_primitive_tag(hash_node->type);
      ...
      /* When this is a variable/const with a procedural type (not a
       * function/procedure definition), we return PROCEDURE to indicate it's a
       * procedure variable. Only for actual function/procedure definitions
       * (HASHTYPE_FUNCTION/HASHTYPE_PROCEDURE) do we return the function's
       * return type. */
      if (hash_node->hash_type == HASHTYPE_FUNCTION ||
          hash_node->hash_type == HASHTYPE_PROCEDURE) {
        /* This is an actual function/procedure definition - return its return
         * type */
      ...
      *type = PROCEDURE;
      ...
      /* Check if this is a ShortString (array[0..255] of Char with
       * is_shortstring flag) */
      if (hash_node->type->type_alias != NULL &&
          hash_node->type->type_alias->is_shortstring) {
        *type = SHORTSTRING_TYPE;
        return 0;
      }
