...
if (!ctx->was_unit_qualified &&
    ctx->expr->expr_data.function_call_data.is_method_call_placeholder && ctx->args_given != NULL) {
        ...
        first_arg->expr_data.id != NULL)
    {
        ...
        int first_scope = FindSymbol(&first_ident, ctx->symtab, first_arg->expr_data.id);
        ...
        if (FindSymbol(&self_node, ctx->symtab, "Self") != 0 && self_node != NULL)
        {
            ...
            if (self_record == NULL && self_node->type != NULL)
            {
                ...
                if (self_type->kind == TYPE_KIND_RECORD)
                    ...
                    self_record = semcheck_lookup_record_type(ctx->symtab,
        ...
        if (self_record != NULL)
        {
            ...
            struct RecordField *field_desc = semcheck_find_class_field(ctx->symtab,
                self_record, first_arg->expr_data.id, &field_owner);
            struct ClassProperty *prop_desc = semcheck_find_class_property(ctx->symtab,
        ...
        first_arg->expr_data.id != NULL && with_context_count > 0)
    {
        ...
        int with_status = semcheck_with_try_resolve(first_arg->expr_data.id,
            ctx->symtab, &with_expr, ctx->expr->line_num);
    ...
    semcheck_expr_with_type(&first_arg_kgpc_type, ctx->symtab, first_arg, ctx->max_scope_lev, NO_MUTATE);
        ...
        (first_arg->resolved_kgpc_type == NULL || first_arg->record_type == NULL))
    {
        ...
        semcheck_expr_with_type(&mutating_type, ctx->symtab, first_arg, ctx->max_scope_lev, MUTATE);
    ...
    if (record_info == NULL && owner_type != NULL) {
        if (owner_type->kind == TYPE_KIND_RECORD) {
            ...
            KgpcType *pointee = kgpc_type_resolve_pointer_pointee(owner_type, ctx->symtab);
        ...
        if (record_info != NULL && record_info->type_id != NULL) {
            ...
            else
            {
                ...
                HashNode_t *method_node = semcheck_find_class_method(ctx->symtab, record_info,
            ...
            if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
                    ...
                    !record_info->is_type_helper)
                {
                    struct RecordType *helper_record =
                        semcheck_lookup_type_helper_for_record_member(ctx->symtab,
                            record_info, method_name);
                    if (helper_record != NULL)
                    {
                        actual_method_owner = NULL;
                        method_node = semcheck_find_class_method(ctx->symtab, helper_record,
                            method_name, &actual_method_owner);
                        if (method_node != NULL)
                        {
                ...
                if (method_node != NULL) {
                        ...
                        first_arg->expr_data.id != NULL)
                    {
                        ...
                        if (FindSymbol(&first_ident_node, ctx->symtab, first_arg->expr_data.id) != 0 &&
                     ...
                     * fix up the return type to ^record instead of procedure. */
                    {
                        ...
                        while (ctor_search != NULL)
                        {
                            ...
                            if (tmpl != NULL)
                            {
                                    ...
                                    !record_info->is_type_helper)
                                {
                                    ...
                                    if (rec_type != NULL)
                                    {
                                        ...
                                        if (ctor_ret != NULL)
                                        {
                                            semcheck_expr_set_resolved_kgpc_type_shared(ctx->expr, ctor_ret);
                            ...
                            ctor_search = semcheck_lookup_parent_record(ctx->symtab, ctor_search);
                        }
                    }
                 ...
                 * of type TThreadSwitchHandler (a procedural type). */
                {
                    ...
                    if (resolve_record_field(ctx->symtab, record_info, method_name,
                            &proc_field, NULL, ctx->expr->line_num, 1 /* silent */) == 0 &&
                        proc_field != NULL)
                    {
        ...
        else
        {
            ...
            struct RecordType *helper_record = semcheck_lookup_type_helper(ctx->symtab,
            ...
            if (helper_record != NULL && helper_record->type_id != NULL)
            {
                ...
                else
                {
                    ...
                    struct RecordType *actual_method_owner = NULL;
                    HashNode_t *method_node = semcheck_find_class_method(ctx->symtab, record_info, method_name, &actual_method_owner);
                    if (method_node != NULL)
                    {
                        set_type_from_hashtype(ctx->type_return, method_node);
                        semcheck_expr_set_resolved_kgpc_type_shared(ctx->expr, method_node->type);
...
if (ctx->id != NULL &&
    (strncasecmp(ctx->id, "Create", 6) == 0 || strcasecmp(ctx->id, "Destroy") == 0 ||
     is_potential_static_method_call || is_potential_class_method_call) &&
    ctx->args_given != NULL) {
    struct Expression *first_arg = (struct Expression *)ctx->args_given->cur;
    int first_arg_type_tag;
    KgpcType *first_arg_kgpc_type_ctor = NULL;
    semcheck_expr_with_type(&first_arg_kgpc_type_ctor, ctx->symtab, first_arg, ctx->max_scope_lev, NO_MUTATE);
    ...
    if (record_info != NULL && record_info->type_id != NULL) {
        ...
        if (method_candidates == NULL)
        {
            while (method_owner != NULL && method_owner->type_id != NULL)
            {
                ...
                free(candidate_name);
                method_owner = semcheck_lookup_parent_record(ctx->symtab, method_owner);
            }
        }
        ...
        if (method_candidates != NULL && mangled_method_name != NULL) {
            ...
            int is_static_method = from_cparser_is_method_static(method_owner->type_id, ctx->id);
            if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
            ...
            int is_nonstatic_class_method =
                (!is_static_method &&
                 from_cparser_is_method_class_method(method_owner->type_id, ctx->id));
            int method_is_declared_constructor =
                semcheck_method_is_declared_constructor(ctx->symtab, method_owner, ctx->id);
            ...
            {
                if (ctx->expr->expr_data.function_call_data.constructor_receiver_expr != NULL)
                {
                    destroy_expr(ctx->expr->expr_data.function_call_data.constructor_receiver_expr);
                    ctx->expr->expr_data.function_call_data.constructor_receiver_expr = NULL;
                }
            ...
            else if (ctx->expr->expr_data.function_call_data.constructor_receiver_expr != NULL)
            {
                destroy_expr(ctx->expr->expr_data.function_call_data.constructor_receiver_expr);
                ctx->expr->expr_data.function_call_data.constructor_receiver_expr = NULL;
            }
            ...
            ctx->expr->expr_data.function_call_data.id = strdup(mangled_method_name);
            if (ctx->expr->expr_data.function_call_data.mangled_id != NULL)
                free(ctx->expr->expr_data.function_call_data.mangled_id);
            ctx->expr->expr_data.function_call_data.mangled_id = strdup(mangled_method_name);
            ...
            if (method_name == NULL && ctx->expr->expr_data.function_call_data.placeholder_method_name != NULL)
            ...
            method_is_declared_constructor =
                semcheck_method_is_declared_constructor(ctx->symtab, record_info, method_name);
            if (method_is_declared_constructor && owner_type != NULL) {
                ...
                semcheck_expr_set_resolved_kgpc_type_shared(ctx->expr, ctor_return_type);
                *ctx->type_return = semcheck_tag_from_kgpc(ctor_return_type);
                if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
                    ...
                    fprintf(stderr,
                        ...
                        ctx->expr->resolved_kgpc_type != NULL ? ctx->expr->resolved_kgpc_type->kind : -1,
