...
if (ctx->id != NULL && pascal_identifier_equals(ctx->id, "CompareMem") && ctx->args_given != NULL)
{
    ...
    pascal_identifier_equals(ctx->id, "Format"))
{
    ...
    for (ListNode_t *cur = ctx->args_given; cur != NULL; cur = cur->next, idx++)
    {
        ...
        fprintf(stderr,
            "[KGPC_DEBUG_FORMAT] pre args[%d]=node=%p expr=%p type=%d line=%d id=%s\n",
    ...
    if (!ctx->is_operator_dispatch &&
        !ctx->was_unit_qualified && ctx->id != NULL && !ctx->expr->expr_data.function_call_data.is_method_call_placeholder)
    {
        HashNode_t *global_node = NULL;
        if (FindSymbol(&global_node, ctx->symtab, ctx->id) != 0 && global_node != NULL &&
            ...
            global_node->owner_class == NULL)
        {
            ...
            if (current_owner != NULL)
            {
                ...
                struct RecordType *owner_rec = semcheck_lookup_record_type(ctx->symtab, current_owner);
                if (owner_rec != NULL && owner_rec->is_type_helper)
                {
                    struct Expression *first_arg_expr = (ctx->args_given != NULL) ?
                        (struct Expression *)ctx->args_given->cur : NULL;
                    int first_is_self = (first_arg_expr != NULL &&
                        first_arg_expr->type == EXPR_VAR_ID &&
                ...
                {
                    fprintf(stderr,
                        "[KGPC_DEBUG_CHECKOBS] global present but in method owner=%s; continuing self lookup\n",
                        current_owner);
                }
            }
            else
            ...
            {
                fprintf(stderr,
                    "[KGPC_DEBUG_CHECKOBS] skip_self_injection global_node=%p owner_class=%s method_name=%s owner_full=%s\n",
                    (void *)global_node,
                    global_node->owner_class != NULL ? global_node->owner_class : "(null)",
        ...
        if (FindSymbol(&self_node, ctx->symtab, "Self") != 0 && self_node != NULL)
        {
            ...
            {
                fprintf(stderr,
                    "[KGPC_DEBUG_TRIM] Self found: node_type_kind=%d record=%s is_helper=%d current_owner=%s\n",
            ...
            if (self_record == NULL)
            {
                ...
                struct RecordType *helper_record = semcheck_lookup_type_helper_for_member(ctx->symtab,
                    self_type_tag, self_type_name, ctx->id);
                ...
                if (self_record == NULL)
                {
                    ...
                    if (current_owner != NULL)
                    {
                        struct RecordType *owner_record = semcheck_lookup_record_type(ctx->symtab, current_owner);
            ...
            if (self_record != NULL)
            {
                ...
                if (self_record->type_id != NULL)
                {
                    ...
                    if (all_methods != NULL)
                    {
                        ...
                        int overload_status = semcheck_resolve_overload(
                            ...
                            all_methods, ctx->args_given, ctx->symtab,
                            &call_stub, ctx->max_scope_lev, 0);
                ...
                if (method_node == NULL)
                {
                    method_node = semcheck_find_class_method(ctx->symtab, self_record, ctx->id, NULL);
                    ...
                    if (method_node == NULL || method_params_len == 0)
                    {
                        ...
                        ListNode_t *class_list = from_cparser_find_classes_with_method((char *)ctx->id, &class_count);
                        if (class_list != NULL)
                        {
                            ...
                            while (cur_class != NULL)
                            {
                                ...
                                if (class_name != NULL)
                                {
                                    if (self_record != NULL && self_record->type_id != NULL)
                                    {
                                        ...
                                        while (check_name != NULL)
                                        {
                                            ...
                                            const char *parent = (check_record != NULL)
                                                ? check_record->parent_class_name : NULL;
                                            if (parent == NULL)
                                                break;
                                    ...
                                    if (FindSymbol(&class_node, ctx->symtab, class_name) && class_node != NULL)
                                    {
                                        ...
                                        if (correct_record != NULL)
                                        {
                                            ...
                                            snprintf(local_mangled_name, sizeof(local_mangled_name), "%s__%s", 
                                                class_name, (char *)ctx->id);
                                            
                                            HashNode_t *correct_method = NULL;
                                            FindSymbol(&correct_method, ctx->symtab, local_mangled_name);
                ...
                if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
                    fprintf(stderr, "[SemCheck] Self injection: Self found, self_record=%s, method=%s, method_node=%p expects_self=%d\n",
                        ...
                        ctx->id ? ctx->id : "(null)", (void*)method_node, expects_self);
                }
                if (kgpc_getenv("KGPC_DEBUG_CHECKOBS") != NULL && ctx->id != NULL &&
                    pascal_identifier_equals(ctx->id, "CheckObserving"))
                {
                    fprintf(stderr,
                        "[KGPC_DEBUG_CHECKOBS] self_found=%d self_record=%s args_count=%d expects_self=%d method_node=%p placeholder=%d\n",
                        self_found,
                        self_record != NULL && self_record->type_id != NULL ? self_record->type_id : "(null)",
                ...
                if (method_node != NULL)
                {
                    ...
                    if (!early_overload_resolved &&
                        self_record != NULL && self_record->type_id != NULL && ctx->id != NULL)
                    {
                        char overload_name[256];
                        snprintf(overload_name, sizeof(overload_name), "%s__%s",
                            self_record->type_id, ctx->id);
                        ListNode_t *method_overloads = FindAllIdents(ctx->symtab, overload_name);
                    ...
                    if (expects_self && !already_has_self)
                    {
                        ...
                        if (!self_found && self_record != NULL &&
                            ...
                            self_record->type_id != NULL)
                        {
                            self_arg_name = self_record->type_id;
                        }
                        struct Expression *self_expr = mk_varid(ctx->expr->line_num, strdup(self_arg_name));
                        if (kgpc_getenv("KGPC_DEBUG_CHECKOBS") != NULL && ctx->id != NULL &&
                            pascal_identifier_equals(ctx->id, "CheckObserving"))
                        {
                            fprintf(stderr,
                                "[KGPC_DEBUG_CHECKOBS] injecting self arg name=%s self_record=%s self_node_type=%d\n",
                                self_arg_name,
                                self_record != NULL && self_record->type_id != NULL ? self_record->type_id : "(null)",
                                self_node != NULL && self_node->type != NULL ? self_node->type->kind : -1);
                        }
                        if (self_expr != NULL && self_record != NULL &&
                            self_record->is_type_helper &&
                            self_record->helper_base_type_id != NULL)
                        {
                            HashNode_t *base_node = semcheck_find_preferred_type_node(ctx->symtab,
                                ...
                                pascal_identifier_equals(ctx->id, "Format"))
                            {
                                fprintf(stderr,
                                    ...
                                    self_record->helper_base_type_id != NULL ? self_record->helper_base_type_id : "(null)",
                                    (void *)base_node,
                                    base_node != NULL ? (void *)base_node->type : NULL);
                            }
                            if (base_node != NULL && base_node->type != NULL)
                                semcheck_expr_set_resolved_kgpc_type_shared(self_expr, base_node->type);
                        }
                        if (self_expr != NULL && self_record != NULL &&
                            pascal_identifier_equals(self_arg_name, "Self"))
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    semcheck_expr_set_resolved_kgpc_type_shared(self_expr, self_record_type);
                                ...
                                pascal_identifier_equals(ctx->id, "CheckObserving"))
                            {
                                fprintf(stderr,
                                    ...
                                    self_expr->resolved_kgpc_type != NULL ? self_expr->resolved_kgpc_type->kind : -1);
                            }
                        }
                        ...
                        ctx->expr->expr_data.function_call_data.mangled_id == NULL)
                    {
                        ...
                        if (resolved_name != NULL)
                            ctx->expr->expr_data.function_call_data.mangled_id = strdup(resolved_name);
                    }
                    ...
                    if (!method_is_overloaded && method_node->type != NULL) {
                        ...
                        ctx->expr->expr_data.function_call_data.call_hash_type = method_node->hash_type;
                    ...
                    if (expects_self)
                    {
                            ...
                            method_node->type->kind == TYPE_KIND_PROCEDURE)
                        {
                            ...
                            if (method_node->owner_class != NULL &&
                                !from_cparser_is_method_static(method_node->owner_class,
                                    method_node->method_name != NULL ? method_node->method_name : ctx->id))
                            {
                         ...
                         * inherited (e.g., TA.ClassName where ClassName is on TObject). */
                        {
                            ...
                            {
                                if (from_cparser_is_method_nonstatic_class_method(check_class, ctx->id))
                                {
        ...
        else if (kgpc_getenv("KGPC_DEBUG_CHECKOBS") != NULL && ctx->id != NULL &&
                 pascal_identifier_equals(ctx->id, "CheckObserving"))
        {
            const char *owner = semcheck_get_current_method_owner();
            fprintf(stderr,
                "[KGPC_DEBUG_CHECKOBS] self_not_found current_owner=%s placeholder=%d\n",
...
if (ctx->id != NULL && strncmp(ctx->id, "__tfpg_ctor$", strlen("__tfpg_ctor$")) == 0)
{
...
int typecast_result = semcheck_try_reinterpret_as_typecast(ctx->type_return, ctx->symtab, ctx->expr, ctx->max_scope_lev);
if (typecast_result != 0 || ctx->expr->type == EXPR_TYPECAST)
    do { ctx->final_status = typecast_result; return FC_CLEANUP; } while (0);
...
{
    struct Expression *receiver_expr = (struct Expression *)ctx->args_given->cur;
    int recv_type = UNKNOWN_TYPE;
    KgpcType *recv_kgpc_type = NULL;
    semcheck_expr_with_type(&recv_kgpc_type, ctx->symtab, receiver_expr, ctx->max_scope_lev, NO_MUTATE);
        ...
        receiver_expr->expr_data.id != NULL)
    {
        ...
        if (FindSymbol(&recv_node, ctx->symtab, receiver_expr->expr_data.id) != 0 && recv_node != NULL)
        {
    ...
    if (recv_record != NULL)
    {
        ...
        if (resolve_record_field(ctx->symtab, recv_record, field_lookup, &field_desc,
                                 &field_offset, ctx->expr->line_num, 1) == 0 &&
            field_desc != NULL)
        {
            ...
            if (is_proc_field)
            {
                if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
                    fprintf(stderr, "[SemCheck] treating %s.%s as procedural field call\n",
                        receiver_expr->type == EXPR_VAR_ID ? receiver_expr->expr_data.id : "<expr>", ctx->id);
                }
                ...
                if (proc_expr == NULL)
                {
                    semcheck_error_with_context_at(ctx->expr->line_num, ctx->expr->col_num, ctx->expr->source_index, "Error on line %d: failed to allocate procedural field expression.\n",
                        ctx->expr->line_num);
                ...
                if (proc_type != NULL)
                {
                    ...
                    {
                        semcheck_error_with_context_at(ctx->expr->line_num, ctx->expr->col_num, ctx->expr->source_index, "Error on line %d, call to procedural field %s: expected %d arguments, got %d\n",
                            ctx->expr->line_num, ctx->id, semcheck_count_total_params(formal_params), ListLength(remaining_args));
                        if (proc_type != NULL)
                    ...
                    while (formal != NULL && actual != NULL)
                    {
                        ...
                        semcheck_expr_with_type(&actual_kgpc_type, ctx->symtab, actual_expr, ctx->max_scope_lev, NO_MUTATE);
                    ...
                    {
                        HashNode_t *ret_node =
                            semcheck_find_preferred_type_node(ctx->symtab, proc_type->info.proc_info.return_type_id);
                    ...
                    if (ret_type != NULL)
                        ctx->expr->expr_data.function_call_data.call_hash_type = HASHTYPE_FUNCTION;
                    ...
                    if (ret_type != NULL) {
                        ...
                        if (alias != NULL && alias->target_type_id != NULL) {
                            HashNode_t *target_node =
                                semcheck_find_preferred_type_node(ctx->symtab, alias->target_type_id);
                    ...
                    else if (ret_type != NULL && ret_type->kind == TYPE_KIND_RECORD)
                    {
                        *ctx->type_return = RECORD_TYPE;
                        ...
                        ctx->expr->expr_data.function_call_data.cached_procvar_sret_size = new_sret;
                        if (field_desc != NULL)
                        {
                    ...
                    else if (ret_type != NULL && ret_type->kind == TYPE_KIND_POINTER)
                    {
                        *ctx->type_return = POINTER_TYPE;
                        ...
                        kgpc_type_retain(ret_type);
                        ctx->expr->resolved_kgpc_type = ret_type;
                        semcheck_set_pointer_info_from_kgpc_type(ctx->expr, ctx->symtab, ret_type);
                    }
                    ...
                    if (ctx->expr->expr_data.function_call_data.cached_procvar_sret_size == 0 &&
                ...
                else
                {
                    ...
                    if (field_desc != NULL && field_desc->cached_proc_return_kgpc_type != NULL)
                    {
                        *ctx->type_return = RECORD_TYPE;
                        ...
                        ctx->expr->resolved_kgpc_type = field_desc->cached_proc_return_kgpc_type;
                        ctx->expr->expr_data.function_call_data.cached_procvar_sret_size =
                            field_desc->cached_proc_return_sret_size;
                    }
                    else
                ...
                ctx->expr->expr_data.function_call_data.procedural_var_expr = proc_expr;
            ...
            else if (ctx->expr->expr_data.function_call_data.is_method_call_placeholder &&
                     ctx->args_given->next == NULL)
            {
                ...
                ctx->expr->expr_data.record_access_data.field_offset = (int)field_offset;

                /* Delegate full type resolution to the record access handler */
                do { ctx->final_status = semcheck_recordaccess(ctx->type_return, ctx->symtab, ctx->expr, ctx->max_scope_lev, ctx->mutating); return FC_CLEANUP; } while (0);
            }
        }
    }
}
...
if (ctx->id != NULL && pascal_identifier_equals(ctx->id, "SizeOf"))
    do { ctx->final_status = semcheck_builtin_sizeof(ctx->type_return, ctx->symtab, ctx->expr, ctx->max_scope_lev); return FC_CLEANUP; } while (0);
if (ctx->id != NULL && pascal_identifier_equals(ctx->id, "BitSizeOf"))
    do { ctx->final_status = semcheck_builtin_bitsizeof(ctx->type_return, ctx->symtab, ctx->expr, ctx->max_scope_lev); return FC_CLEANUP; } while (0);
if (ctx->id != NULL && pascal_identifier_equals(ctx->id, "IsManagedType"))
    do { ctx->final_status = semcheck_builtin_ismanagedtype(ctx->type_return, ctx->symtab, ctx->expr, ctx->max_scope_lev); return FC_CLEANUP; } while (0);
if (ctx->id != NULL && pascal_identifier_equals(ctx->id, "TypeInfo"))
    do { ctx->final_status = semcheck_builtin_typeinfo(ctx->type_return, ctx->symtab, ctx->expr, ctx->max_scope_lev); return FC_CLEANUP; } while (0);
...
if (ctx->id != NULL && pascal_identifier_equals(ctx->id, "GetMem"))
{
    ...
    {
        semcheck_error_with_context_at(ctx->expr->line_num, ctx->expr->col_num, ctx->expr->source_index, "Error on line %d, GetMem expects exactly one argument.\n",
            ctx->expr->line_num);
        *ctx->type_return = UNKNOWN_TYPE;
    ...
    int error_count = semcheck_expr_with_type(&size_kgpc_type, ctx->symtab, size_expr, ctx->max_scope_lev, NO_MUTATE);
    if (error_count != 0)
    {
        *ctx->type_return = UNKNOWN_TYPE;
        do { ctx->final_status = error_count; return FC_CLEANUP; } while (0);
    }
    ...
    if (best_match != NULL && best_match->mangled_id != NULL)
    {
        ...
        ctx->expr->expr_data.function_call_data.mangled_id = strdup(best_match->mangled_id);
        if (ctx->expr->expr_data.function_call_data.mangled_id == NULL)
        {
    ...
    {
        char *mangled_name_local = MangleFunctionNameFromCallSite("GetMem", args, ctx->symtab, ctx->max_scope_lev);
...
{
    fprintf(stderr,
        "[KGPC_DEBUG_EOF] allow_builtins=%d placeholder=%d qualifier=%s args=%d\n",
        allow_builtins,
        ctx->expr->expr_data.function_call_data.is_method_call_placeholder,
        ctx->expr->expr_data.function_call_data.call_qualifier != NULL
            ? ctx->expr->expr_data.function_call_data.call_qualifier : "(null)",
...
if (ctx->id != NULL && pascal_identifier_equals(ctx->id, "Assigned"))
    do { ctx->final_status = semcheck_builtin_assigned(ctx->type_return, ctx->symtab, ctx->expr, ctx->max_scope_lev); return FC_CLEANUP; } while (0);
...
if (ctx->id != NULL && pascal_identifier_equals(ctx->id, "UpperCase"))
{
    ...
    if (args != NULL && args->next == NULL)
    {
        ...
        if (arg_expr != NULL && arg_expr->type == EXPR_FUNCTION_CALL &&
            ...
            semcheck_try_reinterpret_as_typecast(&arg_cast_type, ctx->symtab, arg_expr, ctx->max_scope_lev);
        int error_count = semcheck_expr_with_type(&arg_kgpc_type_uc, ctx->symtab, arg_expr, ctx->max_scope_lev, NO_MUTATE);
             ...
             semcheck_expr_is_explicit_char_typecast_for_call_local(arg_expr)))
        {
            ...
            ctx->expr->expr_data.function_call_data.mangled_id = strdup("kgpc_upcase_char");
            if (ctx->expr->expr_data.function_call_data.mangled_id == NULL)
            {
...
if (ctx->id != NULL && pascal_identifier_equals(ctx->id, "LowerCase"))
{
    ...
    if (args != NULL && args->next == NULL)
    {
        ...
        if (arg_expr != NULL && arg_expr->type == EXPR_FUNCTION_CALL &&
            semcheck_expr_is_char_typecast_call_for_call_local(arg_expr))
            semcheck_try_reinterpret_as_typecast(&arg_cast_type, ctx->symtab, arg_expr, ctx->max_scope_lev);
        int error_count = semcheck_expr_with_type(&arg_kgpc_type_lc, ctx->symtab, arg_expr, ctx->max_scope_lev, NO_MUTATE);
...
if (ctx->id != NULL && pascal_identifier_equals(ctx->id, "UpCase"))
{
    ...
    if (args != NULL && args->next == NULL)
    {
        ...
        if (arg_expr != NULL && arg_expr->type == EXPR_FUNCTION_CALL &&
            semcheck_expr_is_char_typecast_call_for_call_local(arg_expr))
            semcheck_try_reinterpret_as_typecast(&arg_cast_type, ctx->symtab, arg_expr, ctx->max_scope_lev);
        int error_count = semcheck_expr_with_type(&arg_kgpc_type_upcase, ctx->symtab, arg_expr, ctx->max_scope_lev, NO_MUTATE);
    ...
    !ctx->was_unit_qualified && ctx->id != NULL && !ctx->expr->expr_data.function_call_data.is_method_call_placeholder)
{
    ...
    if (current_owner != NULL)
    {
        ...
        if (owner_record != NULL)
        {
            ...
            HashNode_t *method_node = semcheck_find_class_method(ctx->symtab, owner_record, ctx->id, &actual_method_owner);
            ...
            if (record_for_mangling->type_id != NULL && ctx->id != NULL)
                is_static_owner_method = from_cparser_is_method_static(record_for_mangling->type_id, ctx->id);
            ...
            if (method_node != NULL)
            {
                ...
                if (is_static_owner_method && ctx->args_given != NULL && ctx->args_given->cur != NULL)
                {
                    struct Expression *first_arg = (struct Expression *)ctx->args_given->cur;
                    if (first_arg->type == EXPR_VAR_ID &&
                        first_arg->expr_data.id != NULL &&
                ...
                for (ListNode_t *cur = candidate_list; cur != NULL; cur = cur->next)
                {
                    ...
                    if (record_for_mangling->type_id != NULL && ctx->id != NULL)
                        is_static_method = from_cparser_is_method_static(record_for_mangling->type_id, ctx->id);
                ...
                if (!has_direct_match && has_self_match &&
                    record_for_mangling != NULL && record_for_mangling->is_type_helper)
                {
                    ListNode_t *standalone_candidates = FindAllIdents(ctx->symtab, ctx->id);
                    for (ListNode_t *sc = standalone_candidates; sc != NULL; sc = sc->next)
                    {
                            ...
                            sc_node->type != NULL)
                        {
                            ...
                            ListNode_t *sc_params = kgpc_type_get_procedure_params(sc_node->type);
                            ...
                            if (!has_aoc)
                            {
                                for (ListNode_t *p = sc_params; p != NULL; p = p->next)
                                {
                                    ...
                                    if (pdecl != NULL && pdecl->type == TREE_ARR_DECL &&
                                        pdecl->tree_data.arr_decl_data.type == ARRAY_OF_CONST_TYPE)
                                    {
                                        has_aoc = 1;
                                        break;
                                    }
                ...
                {
                    HashNode_t *self_node = NULL;
                    int self_found = (FindSymbol(&self_node, ctx->symtab, "Self") != 0 && self_node != NULL);
                    const char *self_arg_name = "Self";
                    ...
                    if (self_expr != NULL)
                    {
                            ...
                            record_for_mangling->helper_base_type_id != NULL)
                        {
                                ...
                                pascal_identifier_equals(ctx->id, "Format"))
                            {
                                fprintf(stderr,
                                    "[KGPC_DEBUG_FORMAT] helper base=%s node=%p type=%p\n",
                                    record_for_mangling->helper_base_type_id != NULL ? record_for_mangling->helper_base_type_id : "(null)",
                                    (void *)base_node,
                                    base_node != NULL ? (void *)base_node->type : NULL);
                            }
                            if (base_node != NULL && base_node->type != NULL)
                        ...
                        {
                            if (self_found && self_node != NULL && self_node->type != NULL)
                            {
                                semcheck_expr_set_resolved_kgpc_type_shared(self_expr, self_node->type);
                            }
                            else if (record_for_mangling != NULL)
                            {
                                KgpcType *self_record_type = create_record_type(record_for_mangling);
                                if (self_record_type != NULL)
                                {
                                    semcheck_expr_set_resolved_kgpc_type_shared(self_expr, self_record_type);
                                    destroy_kgpc_type(self_record_type);
                ...
                if (!method_is_overloaded)
                {
                    ...
                    semcheck_expr_set_resolved_kgpc_type_shared(ctx->expr, method_node->type);
                    ctx->expr->expr_data.function_call_data.resolved_func = method_node;
                    ...
                    ctx->mangled_name = (resolved_method_name != NULL) ? strdup(resolved_method_name)
                ...
                {
                    ctx->overload_candidates = CreateListNode(method_node, LIST_UNSPECIFIED);
                }
             ...
             * where FCanObserve is a field of type TCanObserveEvent. */
            {
                ...
                if (resolve_record_field(ctx->symtab, owner_record, ctx->id,
                        &proc_field, &pf_offset, ctx->expr->line_num, 1) == 0 &&
                    proc_field != NULL)
                {
                    ...
                    {
                        HashNode_t *type_node = NULL;
                        if (FindSymbol(&type_node, ctx->symtab, proc_field->type_id) != 0 &&
                            type_node != NULL && type_node->type != NULL &&
                            type_node->type->kind == TYPE_KIND_PROCEDURE)
                        {
                        ...
                        proc_kgpc_type->kind == TYPE_KIND_PROCEDURE)
                    {
                        ...
                        semcheck_expr_with_type(&field_kgpc, ctx->symtab, field_access, ctx->max_scope_lev, NO_MUTATE);
