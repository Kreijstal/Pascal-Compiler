...
int semcheck_candidate_is_builtin(SymTab_t *symtab, HashNode_t *node) {
  ...
  ListNode_t *matches =
      FindAllIdentsInTable(symtab->builtin_scope->table, node->id);
  int is_builtin = 0;
...
typedef enum {
  ...
  STRING_KIND_NARROW =
      1, /* Char, AnsiChar, String, AnsiString, RawByteString, ShortString */
...
int semcheck_candidates_share_signature(SymTab_t *symtab, HashNode_t *a,
                                        HashNode_t *b) {
...
int semcheck_candidates_string_subtypes_differ(SymTab_t *symtab, HashNode_t *a,
                                               HashNode_t *b) {
  if (a == NULL || b == NULL || a->type == NULL || b->type == NULL)
    return 0;
  if (a->type->kind != TYPE_KIND_PROCEDURE ||
  ...
  while (args_a != NULL && args_b != NULL) {
    ...
    if (type_a == STRING_TYPE && type_b == STRING_TYPE) {
      ...
      if (id_a != NULL && id_b != NULL &&
        ...
        StringKind kind_b = semcheck_string_kind_from_type_id(id_b);
        if (kind_a != kind_b ||
            (kind_a == STRING_KIND_UNKNOWN && kind_b == STRING_KIND_UNKNOWN))
          return 1;
      }
    }
    args_a = args_a->next;
...
int semcheck_count_required_params(ListNode_t *params) {
  ...
  /* Once we see a parameter with a default, all following must also have
   * defaults */
  while (cur != NULL) {
    Tree_t *param_decl = (Tree_t *)cur->cur;
    ...
    if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
      const char *param_id = "?";
      if (param_decl && param_decl->type == TREE_VAR_DECL &&
          param_decl->tree_data.var_decl_data.ids)
        param_id = (char *)param_decl->tree_data.var_decl_data.ids->cur;
      fprintf(stderr,
              "[SemCheck] count_required_params: param=%s has_default=%d\n",
...
int semcheck_method_accepts_arg_count(ListNode_t *params, int arg_count,
                                      int *expects_self_out, int is_varargs) {
  int expects_self = 0;
...
int semcheck_append_default_args(ListNode_t **args_head,
                                 ListNode_t *formal_params, int line_num) {
  if (args_head == NULL)
    return 0;
...
int semcheck_named_arg_type_compatible(Tree_t *formal_decl,
  ...
  if (expected_type == POINTER_TYPE && rhs_type == CHAR_TYPE) {
    ...
    KgpcType *expected_kgpc =
        resolve_type_from_vardecl(formal_decl, symtab, &owns_expected);
  ...
  if (rhs_expr != NULL && rhs_expr->resolved_kgpc_type != NULL) {
    ...
    if (expected_kgpc != NULL) {
      if (are_types_compatible_for_assignment(
              expected_kgpc, rhs_expr->resolved_kgpc_type, symtab)) {
        if (owns_expected)
          destroy_kgpc_type(expected_kgpc);
        return 1;
      }
...
static int semcheck_infer_literal_elem_type(struct Expression *expr) {
  ...
  if (expr->type == EXPR_SET) {
    ...
    if (expr->expr_data.array_literal_data.elements != NULL)
      elem_expr =
          (struct Expression *)expr->expr_data.array_literal_data.elements->cur;
  }
...
static int semcheck_resolve_arg_kgpc_type(struct Expression *arg_expr,
  ...
  if (arg_expr->type == EXPR_FUNCTION_CALL &&
    ...
    HashNode_t *type_node = semcheck_find_preferred_type_node(
        symtab, arg_expr->expr_data.function_call_data.id);
  ...
  semcheck_expr_with_type(&arg_kgpc_type, symtab, arg_expr, max_scope_lev,
                          NO_MUTATE);
  ...
  if (arg_expr->type == EXPR_TYPECAST &&
    ...
    struct RecordType *rec = semcheck_lookup_record_type(
        symtab, arg_expr->expr_data.typecast_data.target_type_id);
    if (rec != NULL) {
      if (owns_type_out != NULL)
        *owns_type_out = 1;
      if (arg_type_out != NULL)
  ...
  if (arg_type != NULL && arg_tag != UNKNOWN_TYPE) {
    ...
    if (arg_tag == POINTER_TYPE) {
       ...
       * fell through to the generic fallback. */
               ...
               arg_type->type_alias != NULL && arg_type->type_alias->is_pointer)
        keep = 1;
    } else if (arg_type->kind == TYPE_KIND_PRIMITIVE &&
  ...
  if (arg_type == NULL && arg_expr->is_array_expr) {
    ...
    if (arg_expr->array_element_type_id != NULL && symtab != NULL) {
      ...
      if (FindSymbol(&elem_node, symtab, arg_expr->array_element_type_id) !=
              0 &&
    ...
    if (elem_type != NULL) {
      ...
      if (arr_type != NULL) {
        ...
        if (owns_type_out != NULL)
          *owns_type_out = 1;
        /* create_array_type takes ownership of elem_type, so we must NOT
         * destroy it here. The elem_type will be freed when arr_type is
         * destroyed. */
      } else if (elem_owned && elem_type != NULL) {
        /* Only destroy elem_type if array creation failed (ownership not
         * transferred) */
  ...
  if ((arg_tag == INT_TYPE || arg_tag == LONGINT_TYPE ||
       arg_tag == INT64_TYPE) &&
...
static void
semcheck_bind_set_literal_to_formal_type(struct Expression *arg_expr,
                                         KgpcType *formal_kgpc, int *owns_arg,
                                         KgpcType **arg_kgpc, int *arg_tag) {
  if (arg_expr == NULL || arg_expr->type != EXPR_SET || formal_kgpc == NULL ||
      !kgpc_type_is_set(formal_kgpc))
    return;
...
static int semcheck_overload_record_assign_operator_score(
  ...
  if (arg_rank < 0 &&
      are_types_compatible_for_assignment(param_type, source_type, symtab))
    arg_rank = 4;
...
static int semcheck_overload_has_record_assign_conversion(SymTab_t *symtab,
                                                          KgpcType *actual_kgpc,
                                                          KgpcType *formal_kgpc,
                                                          int actual_tag) {
  int formal_is_record =
      (formal_kgpc != NULL && semcheck_overload_type_is_recordish(formal_kgpc));
  int actual_is_record =
      (actual_kgpc != NULL && semcheck_overload_type_is_recordish(actual_kgpc));
  ...
  const char *target_type_id =
      semcheck_overload_record_type_id_from_kgpc(formal_kgpc);
  const char *source_type_id =
      semcheck_overload_record_type_id_from_kgpc(actual_kgpc);
  int found = 0;
  ...
  if (target_type_id != NULL && source_type_id != NULL) {
    snprintf(ids[4], sizeof(ids[4]), "%s__op_assign_%s", target_type_id,
             source_type_id);
    snprintf(ids[5], sizeof(ids[5]), "%s__op_assign_%s", source_type_id,
             target_type_id);
  }
  ...
  for (int i = 0; i < 8 && !found; ++i) {
    ...
    for (ListNode_t *cur = cands; cur != NULL; cur = cur->next) {
      if (semcheck_overload_record_assign_operator_score(
              symtab, (HashNode_t *)cur->cur, formal_kgpc, actual_kgpc)) {
        found = 1;
        break;
      }
    }
  ...
  if (!found) {
    for (ScopeNode *scope = symtab->current_scope;
         scope != NULL && scope != symtab->builtin_scope && !found;
         scope = scope->parent) {
      ...
      for (int i = 0; i < TABLE_SIZE && !found; ++i) {
        for (ListNode_t *cur = table->table[i]; cur != NULL; cur = cur->next) {
          ...
          if (!semcheck_overload_symbol_is_assign_operator(cand))
            continue;
          if (semcheck_overload_record_assign_operator_score(
                  symtab, cand, formal_kgpc, actual_kgpc)) {
            found = 1;
            break;
          }
        }
  ...
  if (!found && symtab->builtin_scope->table != NULL) {
    for (int i = 0; i < TABLE_SIZE && !found; ++i) {
      for (ListNode_t *cur = symtab->builtin_scope->table->table[i];
...
static MatchQuality
semcheck_classify_match(int actual_tag, KgpcType *actual_kgpc, int formal_tag,
                        KgpcType *formal_kgpc, int is_var_param,
                        SymTab_t *symtab, int is_integer_literal) {
  ...
  if (is_var_param) {
    if (actual_kgpc != NULL && formal_kgpc != NULL) {
      ...
      int formal_is_ptr =
          (formal_kgpc->kind == TYPE_KIND_POINTER || formal_is_untyped_ptr);
      int actual_is_ptr =
          (actual_kgpc->kind == TYPE_KIND_POINTER || actual_is_untyped_ptr);
      if (formal_is_ptr && actual_is_ptr) {
        ...
        /* This handles constructor Self parameter matching where both are
         * ^record */
        if (formal_kgpc->kind == TYPE_KIND_POINTER &&
            actual_kgpc->kind == TYPE_KIND_POINTER) {
          /* Check if types are directly compatible */
          if (are_types_compatible_for_assignment(formal_kgpc, actual_kgpc,
                                                  symtab))
            return semcheck_make_quality(MATCH_EXACT);
          /* For class pointers, also check if the pointed-to records are
           * related. This handles cases where ^Record and ^record represent the
           * same class but have different KgpcType instances. */
          ...
          if (formal_inner != NULL && actual_inner != NULL &&
            ...
            if (formal_inner->info.record_info ==
                actual_inner->info.record_info)
              return semcheck_make_quality(MATCH_EXACT);
            /* Also allow if actual's record has the formal's record as an
             * ancestor via the class hierarchy (checked by name since
             * record_info may differ) */
            if (are_types_compatible_for_assignment(formal_inner, actual_inner,
                                                    symtab))
              return semcheck_make_quality(MATCH_EXACT);
          }
        }
      ...
      /* For primitive types with the same size and signedness, treat as
         compatible. This allows Integer/LongInt and Cardinal/LongWord/DWord to
         be interchangeable for VAR/OUT parameters, matching FPC behavior. */
  ...
  if (actual_tag == formal_tag && formal_tag != POINTER_TYPE) {
    ...
    if (is_real_family_type(formal_tag) && actual_kgpc != NULL &&
        formal_kgpc != NULL) {
    ...
    if (formal_tag == STRING_TYPE && formal_kgpc != NULL &&
        actual_kgpc != NULL) {
      const char *formal_alias = (formal_kgpc->type_alias != NULL)
                                     ? formal_kgpc->type_alias->alias_name
                                     : NULL;
      const char *actual_alias = (actual_kgpc->type_alias != NULL)
                                     ? actual_kgpc->type_alias->alias_name
                                     : NULL;
      ...
      if (formal_kind != STRING_KIND_WIDE &&
          formal_kind != STRING_KIND_UNKNOWN && actual_kind == STRING_KIND_WIDE)
        return semcheck_make_quality(MATCH_PROMOTION);
    }
    ...
    if (formal_tag == RECORD_TYPE && formal_kgpc != NULL) {
      if (kgpc_getenv("KGPC_DEBUG_OVERLOAD_QUALITY") != NULL)
        fprintf(
            stderr,
            "[KGPC] RECORD match: actual_kgpc=%p formal_kgpc=%p equals=%d\n",
  ...
  if (is_string_type(formal_tag) && is_string_type(actual_tag)) {
    const char *formal_alias =
        (formal_kgpc != NULL && formal_kgpc->type_alias != NULL)
            ? formal_kgpc->type_alias->alias_name
            : NULL;
    StringKind formal_kind = semcheck_string_kind_from_type_id(formal_alias);
  ...
  if (actual_kgpc != NULL && formal_kgpc != NULL &&
    ...
    if (actual_sub != UNKNOWN_TYPE && formal_sub != UNKNOWN_TYPE) {
      if (actual_sub == formal_sub) {
        ...
        if (actual_sub == RECORD_TYPE) {
          ...
          if (actual_inner != NULL && formal_inner != NULL &&
            ...
            if (actual_kgpc->type_alias != NULL &&
                actual_kgpc->type_alias->pointer_type_id != NULL)
              actual_name = actual_kgpc->type_alias->pointer_type_id;
            else if (actual_kgpc->type_alias != NULL &&
                     actual_kgpc->type_alias->alias_name != NULL)
              actual_name = actual_kgpc->type_alias->alias_name;
            if (formal_kgpc->type_alias != NULL &&
                formal_kgpc->type_alias->pointer_type_id != NULL)
              formal_name = formal_kgpc->type_alias->pointer_type_id;
            else if (formal_kgpc->type_alias != NULL &&
      ...
      if (actual_kgpc->info.points_to != NULL &&
          formal_kgpc->info.points_to != NULL) {
        int actual_is_widechar =
            semcheck_is_widechar_like_type(actual_kgpc->info.points_to);
        int formal_is_widechar =
            semcheck_is_widechar_like_type(formal_kgpc->info.points_to);
      ...
      if (actual_kgpc->info.points_to != NULL &&
          formal_kgpc->info.points_to != NULL) {
        int rank = kgpc_type_conversion_rank(actual_kgpc->info.points_to,
                                             formal_kgpc->info.points_to);
        if (rank >= 0)
          return semcheck_make_quality(MATCH_CONVERSION);
      }
      ...
      if (actual_sub == RECORD_TYPE && formal_sub == POINTER_TYPE &&
          ...
          formal_kgpc->info.points_to->info.points_to->kind ==
              TYPE_KIND_RECORD) {
        ...
        if (actual_inner->kind == TYPE_KIND_RECORD &&
            formal_inner->kind == TYPE_KIND_RECORD) {
          if (actual_inner->info.record_info ==
                  formal_inner->info.record_info ||
              are_types_compatible_for_assignment(formal_inner, actual_inner,
                                                  symtab)) {
            return semcheck_make_quality(MATCH_CONVERSION);
          }
        }
  ...
  }
  {
    ...
    /* Reverse direction: actual is a record with := operator to the formal
     * type. E.g., Tconstexprint has operator := to qword/int64/bestreal, so
     * passing a Tconstexprint where an integer/real is expected should be
     * allowed. */
    const char *actual_type_id =
        semcheck_overload_record_type_id_from_kgpc(actual_kgpc);
    if (actual_type_id != NULL &&
      ...
      static const char *real_suffixes[] = {"bestreal", "real",     "double",
                                            "single",   "extended", NULL};
      const char **suffixes =
          is_integer_type(formal_tag) ? int_suffixes : real_suffixes;
  ...
  if (semcheck_overload_has_record_assign_conversion(symtab, actual_kgpc,
                                                     formal_kgpc, actual_tag))
    return semcheck_make_quality(MATCH_CONVERSION);
  ...
  if (is_string_type(formal_tag) && actual_tag == CHAR_TYPE)
    return semcheck_make_quality(MATCH_PROMOTION);
  if (actual_tag == CHAR_TYPE && formal_kgpc != NULL &&
  ...
  if (is_string_type(actual_tag) && formal_kgpc != NULL &&
      kgpc_type_is_pointer(formal_kgpc)) {
    KgpcType *points_to = formal_kgpc->info.points_to;
    if (points_to != NULL && points_to->kind == TYPE_KIND_PRIMITIVE &&
        points_to->info.primitive_type_tag == CHAR_TYPE)
  ...
  if (actual_tag == RECORD_TYPE && formal_tag == POINTER_TYPE &&
    ...
    if (formal_kgpc->info.points_to != NULL) {
      ...
      if (inner->kind == TYPE_KIND_RECORD) {
        formal_points_to_class =
            (inner->info.record_info != NULL)
                ? record_type_is_class(inner->info.record_info)
                : 1;
...
static const char *semcheck_get_expr_decl_type_id(struct Expression *expr,
                                                  SymTab_t *symtab) {
  ...
  if (node->type->type_alias != NULL &&
      node->type->type_alias->alias_name != NULL)
    return node->type->type_alias->alias_name;
  if (node->type->type_alias != NULL &&
      node->type->type_alias->target_type_id != NULL)
...
static const char *
semcheck_explicit_string_cast_target_id(struct Expression *expr) {
  if (expr == NULL)
    return NULL;
...
static const char *semcheck_numeric_type_canonical_id(const char *type_id) {
  ...
  if (pascal_identifier_equals(id, "Word") ||
    ...
    return "UInt16";
  if (pascal_identifier_equals(id, "SmallInt") ||
...
static int semcheck_integer_promotion_rank(int actual_tag,
                                           KgpcType *actual_kgpc,
                                           int formal_tag,
                                           KgpcType *formal_kgpc,
  ...
  int actual_unsigned =
      (actual_tag == BYTE_TYPE || actual_tag == WORD_TYPE ||
       actual_tag == LONGWORD_TYPE || actual_tag == QWORD_TYPE);
  int formal_unsigned =
      (formal_tag == BYTE_TYPE || formal_tag == WORD_TYPE ||
  ...
  if (actual_size > 0 && formal_size > 0) {
    ...
    if (formal_size == 1)
      ...
      formal_rank = 1;
    else if (formal_size == 4)
      formal_rank = 2;
    else if (formal_size == 8)
      formal_rank = 3;
 ...
 */
static int compare_single_quality(const MatchQuality *a,
                                  const MatchQuality *b) {
  if (a->kind < b->kind)
    return -1;
  if (a->kind > b->kind)
    return 1;
 ...
 */
static int semcheck_compare_match_quality(int arg_count, const MatchQuality *a,
                                          const MatchQuality *b) {
  int a_exact = 0;
  int b_exact = 0;
  int a_promotion = 0;
   ...
   * so exact subtype/id matches can still dominate deterministically. */
  {
    ...
    for (int i = 0; i < arg_count; i++) {
      ...
      if (cmp < 0)
        ...
        b_better_count++;
      if ((int)a[i].kind > a_worst)
        a_worst = (int)a[i].kind;
      if ((int)b[i].kind > b_worst)
        b_worst = (int)b[i].kind;
    }
...
int semcheck_resolve_overload(HashNode_t **best_match_out, int *num_best_out,
  ...
  for (ListNode_t *cur = overload_candidates; cur != NULL; cur = cur->next) {
    ...
    if (guard > guard_limit) {
      fprintf(stderr,
              "ERROR: semcheck_resolve_overload exceeded guard limit (%d); "
              "possible cycle in overload candidates list (node=%p).\n",
    ...
    if (fast != NULL && fast->next != NULL) {
      ...
      if (fast != NULL && slow == fast) {
        fprintf(
            stderr,
            "ERROR: Cycle detected in overload candidates list (node=%p).\n",
            (void *)cur);
        break;
      }
    }
    ...
    if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
      fprintf(stderr,
              "[SemCheck] semcheck_resolve_overload: candidate %s args=%d "
              "required=%d given=%d\n",
              candidate->id, total_params, required_params, given_count);
    }

    if (candidate_args != NULL && args_given != NULL &&
        args_given->cur != NULL) {
      ...
      if (!allow_implicit_leading_self && first_actual != NULL &&
          first_actual->type == EXPR_VAR_ID &&
          first_actual->expr_data.id != NULL &&
          given_count == total_params + 1) {
        int candidate_is_owner_method = 0;
        if (candidate->owner_class != NULL &&
            pascal_identifier_equals(candidate->owner_class,
                                     first_actual->expr_data.id))
          candidate_is_owner_method = 1;
        else if (candidate->mangled_id != NULL) {
          size_t owner_len = strlen(first_actual->expr_data.id);
          if (strncasecmp(candidate->mangled_id, first_actual->expr_data.id,
                          owner_len) == 0 &&
    ...
    int arity_matches =
        ((given_count >= required_params && given_count <= total_params) ||
         (total_params == 0 && given_count > 0 && candidate->type != NULL &&
    ...
    if (!arity_matches && !allow_implicit_leading_self &&
      ...
      if (first_formal_check != NULL &&
          first_formal_check->type == TREE_VAR_DECL &&
          first_formal_check->tree_data.var_decl_data.ids != NULL)
        ff_name =
            (const char *)first_formal_check->tree_data.var_decl_data.ids->cur;
    ...
    if (given_count > 0) {
      qualities =
          (MatchQuality *)calloc((size_t)given_count, sizeof(MatchQuality));
      if (qualities == NULL)
        return 3;
    }
    ...
    while (formal_args != NULL && call_args != NULL) {
      ...
      if (arg_expr != NULL && arg_expr->type == EXPR_RELOP &&
        ...
        if (formal_decl != NULL && formal_decl->type == TREE_VAR_DECL &&
          ...
          formal_name =
              (const char *)formal_decl->tree_data.var_decl_data.ids->cur;
        else if (formal_decl != NULL && formal_decl->type == TREE_ARR_DECL &&
                 formal_decl->tree_data.arr_decl_data.ids != NULL)
          formal_name =
              (const char *)formal_decl->tree_data.arr_decl_data.ids->cur;

        const char *given_name =
            arg_expr->expr_data.relop_data.left->expr_data.id;
        if (formal_name != NULL && given_name != NULL) {
          if (pascal_identifier_equals(formal_name, given_name)) {
            ...
            semcheck_expr_with_type(&rhs_kgpc_type, symtab, rhs_expr,
                                    max_scope_lev, NO_MUTATE);
            rhs_type = semcheck_tag_from_kgpc(rhs_kgpc_type);
            if (semcheck_named_arg_type_compatible(formal_decl, rhs_expr,
                                                   rhs_type, symtab))
              arg_expr = rhs_expr;
          } else if (semcheck_param_list_contains_name(candidate_args,
                                                       given_name)) {
            candidate_valid = 0;
            break;
          }
      ...
      if (formal_decl != NULL) {
        if (formal_decl->type == TREE_ARR_DECL)
          ...
          formal_inline_alias =
              formal_decl->tree_data.var_decl_data.inline_type_alias;
      ...
      if (!formal_is_array_decl && formal_decl != NULL) {
        int owns_probe = 0;
        KgpcType *probe =
            resolve_type_from_vardecl(formal_decl, symtab, &owns_probe);
        if (probe != NULL &&
            (kgpc_type_is_array(probe) || kgpc_type_is_array_of_const(probe)))
          formal_is_array_decl = 1;
      ...
      if (arg_expr != NULL &&
          (arg_expr->type == EXPR_SET || arg_expr->type == EXPR_ARRAY_LITERAL ||
           ...
           (arg_expr->array_element_type_id != NULL &&
            arg_expr->is_array_expr))) {
        arg_is_array = 1;
      } else if (arg_expr != NULL) {
        int owns_probe = 0;
        KgpcType *arg_probe = NULL;
        semcheck_resolve_arg_kgpc_type(arg_expr, symtab, max_scope_lev,
                                       &owns_probe, &arg_probe);
        if (arg_probe != NULL && (kgpc_type_is_array(arg_probe) ||
                                  kgpc_type_is_array_of_const(arg_probe)))
          arg_is_array = 1;
      ...
      if (formal_is_array_decl && arg_is_array) {
        ...
        int owns_formal = 0;
        KgpcType *formal_kgpc =
            resolve_type_from_vardecl(formal_decl, symtab, &owns_formal);
        if (formal_kgpc == NULL && formal_inline_alias != NULL) {
          formal_kgpc =
              create_kgpc_type_from_type_alias(formal_inline_alias, symtab, 0);
        ...
        if (formal_kgpc != NULL && formal_kgpc->kind == TYPE_KIND_ARRAY &&
            kgpc_type_is_dynamic_array(formal_kgpc) && formal_decl != NULL &&
            formal_decl->type == TREE_VAR_DECL &&
            (formal_inline_alias == NULL ||
             !formal_inline_alias->is_open_array)) {
        ...
        if (formal_kgpc != NULL &&
            formal_kgpc->kind == TYPE_KIND_ARRAY_OF_CONST) {
          quality = semcheck_make_quality(MATCH_PROMOTION);
        } else if (formal_kgpc != NULL &&
                   formal_kgpc->kind == TYPE_KIND_ARRAY) {
          KgpcType *formal_elem =
              kgpc_type_get_array_element_type_resolved(formal_kgpc, symtab);
          ...
          if (arg_expr->type == EXPR_SET ||
            ...
            arg_elem_tag = semcheck_map_builtin_type_name(
                symtab, arg_expr->array_element_type_id);
            if (arg_elem_tag == UNKNOWN_TYPE) {
              HashNode_t *type_node = NULL;
              if (FindSymbol(&type_node, symtab,
                             arg_expr->array_element_type_id) != 0 &&
                  type_node != NULL)
          ...
          if (arg_elem_tag == UNKNOWN_TYPE && arg_expr != NULL) {
            ...
            if (arg_kgpc != NULL && arg_kgpc->kind == TYPE_KIND_ARRAY) {
              KgpcType *arg_elem =
                  kgpc_type_get_array_element_type_resolved(arg_kgpc, symtab);
          ...
          if (arg_elem_tag != UNKNOWN_TYPE &&
            ...
            if (formal_elem_tag != UNKNOWN_TYPE) {
              if (arg_elem_tag != UNKNOWN_TYPE &&
                  arg_elem_tag == formal_elem_tag) {
                rank = 0;
              } else {
                KgpcType *arg_elem = create_primitive_type(arg_elem_tag);
                KgpcType *formal_elem_prim =
                    create_primitive_type(formal_elem_tag);
            ...
            if (rank < 0) {
              if (!arg_is_literal && arg_elem_tag == SHORTSTRING_TYPE &&
                ...
                rank = 1;
              } else if (!arg_is_literal && arg_elem_tag == SHORTSTRING_TYPE &&
            ...
            if (rank < 0 && formal_elem != NULL && arg_elem_tag == CHAR_TYPE &&
                ...
                formal_elem->info.array_info.element_type->kind ==
                    TYPE_KIND_PRIMITIVE &&
                formal_elem->info.array_info.element_type->info
                        .primitive_type_tag == CHAR_TYPE) {
              rank = 1;
            }
            if (rank < 0) {
              if (arg_is_literal && arg_elem_tag == STRING_TYPE &&
                  formal_elem_tag == SHORTSTRING_TYPE) {
        ...
        if (arg_expr != NULL) {
          ...
          const char *arg_decl_id =
              semcheck_get_expr_decl_type_id(arg_expr, symtab);
          if (formal_id != NULL && arg_decl_id != NULL &&
              pascal_identifier_equals(formal_id, arg_decl_id))
            quality.exact_type_id = 1;
          if (!quality.exact_type_id && formal_id != NULL &&
              arg_decl_id != NULL) {
            const char *formal_num =
                semcheck_numeric_type_canonical_id(formal_id);
            const char *arg_num =
                semcheck_numeric_type_canonical_id(arg_decl_id);
          ...
          if (formal_id != NULL) {
            ...
            int arg_expr_tag = (arg_expr_kgpc != NULL)
                                   ...
                                   : UNKNOWN_TYPE;
            StringKind formal_kind =
                semcheck_string_kind_from_type_id(formal_id);
            StringKind arg_kind =
                (arg_decl_id != NULL)
            ...
            if (formal_kind != STRING_KIND_UNKNOWN &&
                arg_kind != STRING_KIND_UNKNOWN) {
              if (formal_kind == arg_kind)
                ...
                quality.string_rank = 1;
                       ...
                       (arg_expr_tag == CHAR_TYPE ||
                        arg_expr_tag == BYTE_TYPE)) {
              if (quality.kind == MATCH_EXACT)
                quality.kind = MATCH_PROMOTION;
              quality.string_rank = 1;
            }
        ...
        if (semcheck_prepare_array_literal_argument(formal_decl, arg_expr,
                                                    symtab, max_scope_lev,
                                                    call_expr->line_num) != 0) {
          free(qualities);
          if (best_match_out != NULL)
            *best_match_out = NULL;
          return 3;
        ...
        if (semcheck_prepare_record_constructor_argument(
                formal_decl, arg_expr, symtab, max_scope_lev,
                call_expr->line_num) != 0) {
          free(qualities);
          if (best_match_out != NULL)
            *best_match_out = NULL;
        ...
        KgpcType *formal_kgpc =
            resolve_type_from_vardecl(formal_decl, symtab, &owns_formal);
        KgpcType *arg_kgpc = NULL;
        int arg_tag = semcheck_resolve_arg_kgpc_type(
            arg_expr, symtab, max_scope_lev, &owns_arg, &arg_kgpc);
        ...
        if (formal_kgpc != NULL && formal_kgpc->kind == TYPE_KIND_PRIMITIVE &&
            is_real_family_type(formal_kgpc->info.primitive_type_tag) &&
          ...
          if (ftype_id != NULL) {
            ...
            if (FindSymbol(&ftype_node, symtab, ftype_id) &&
                ftype_node != NULL && ftype_node->hash_type == HASHTYPE_TYPE &&
        ...
        semcheck_bind_set_literal_to_formal_type(
            arg_expr, formal_kgpc, &owns_arg, &arg_kgpc, &arg_tag);
          ...
          if (is_string_type(arg_tag) || arg_tag == CHAR_TYPE) {
            ...
            if (formal_kgpc != NULL && formal_kgpc->kind == TYPE_KIND_ARRAY)
              formal_elem = kgpc_type_get_array_element_type_resolved(
                  formal_kgpc, symtab);
            int elem_tag =
                formal_elem ? semcheck_tag_from_kgpc(formal_elem) : formal_tag;
        ...
        if (formal_kgpc == NULL) {
          if (formal_tag == UNKNOWN_TYPE) {
            ...
            int is_integer_literal =
                (arg_expr != NULL && arg_expr->type == EXPR_INUM);
            quality = semcheck_classify_match(arg_tag, arg_kgpc, formal_tag,
                                              formal_kgpc, is_var_param, symtab,
                                              is_integer_literal);
          } else {
            quality = semcheck_make_quality(MATCH_PROMOTION);
          }
          ...
          if (kgpc_getenv("KGPC_DEBUG_OVERLOAD_QUALITY") != NULL) {
            ...
            int formal_ptr_sub =
                (formal_kgpc != NULL && formal_kgpc->kind == TYPE_KIND_POINTER)
                    ? kgpc_type_get_pointer_subtype_tag(formal_kgpc)
                    : -1;
            int arg_ptr_sub =
                (arg_kgpc != NULL && arg_kgpc->kind == TYPE_KIND_POINTER)
                    ? kgpc_type_get_pointer_subtype_tag(arg_kgpc)
                    : -1;
            fprintf(stderr,
                    "[KGPC]   arg_tag=%d formal_tag=%d formal_kind=%d "
                    "arg_kind=%d var=%d formal_id=%s formal_ptr=%d arg_ptr=%d "
                    "is_int_literal=%d\n",
                    arg_tag, formal_tag, formal_kgpc ? formal_kgpc->kind : -1,
                    arg_kgpc ? arg_kgpc->kind : -1, is_var_param,
                    formal_id_dbg ? formal_id_dbg : "<null>", formal_ptr_sub,
                    arg_ptr_sub, is_integer_literal);
          }
          quality = semcheck_classify_match(arg_tag, arg_kgpc, formal_tag,
                                            formal_kgpc, is_var_param, symtab,
                                            is_integer_literal);
          ...
          if (quality.kind == MATCH_INCOMPATIBLE && !is_var_param &&
            ...
            const char *formal_type_id =
                semcheck_get_param_type_id(formal_decl);
            if (formal_type_id != NULL &&
                pascal_identifier_equals(formal_type_id, "tconstexprint")) {
              quality = semcheck_make_quality(MATCH_CONVERSION);
            }
          }
          /* Penalize var/out parameters that receive non-addressable
           * expressions (literals, computed values). A literal like 1 cannot be
           * passed by reference, so the value-param overload should be
           * preferred. We demote to MATCH_INCOMPATIBLE so that value-param
           * overloads win. Skip Self parameters — they are implicit and always
           * valid. */
          if (is_var_param && arg_expr != NULL && quality.kind == MATCH_EXACT) {
            /* Check if this is the Self parameter (implicit first param of
             * methods) */
            int is_self_param = 0;
            if (formal_decl != NULL && formal_decl->type == TREE_VAR_DECL &&
                formal_decl->tree_data.var_decl_data.ids != NULL) {
          ...
          const char *formal_id = semcheck_get_param_type_id(formal_decl);
          if (arg_expr != NULL && arg_expr->type == EXPR_ADDR_OF_PROC &&
              formal_kgpc != NULL && formal_kgpc->kind == TYPE_KIND_PROCEDURE) {
          ...
          if (arg_expr != NULL && arg_expr->type == EXPR_STRING &&
            ...
            if (formal_id != NULL &&
                pascal_identifier_equals(formal_id, "ShortString")) {
            ...
            if (cast_target_id != NULL &&
              ...
              if (formal_tag == SHORTSTRING_TYPE ||
                ...
                quality.string_rank = 10;
                         ...
                         (pascal_identifier_equals(formal_id,
                                                   "RawByteString") ||
                          pascal_identifier_equals(formal_id, "AnsiString") ||
                          pascal_identifier_equals(formal_id, "String"))) {
                quality.kind = MATCH_EXACT;
          ...
          /* For char types, prefer smaller formal type (AnsiChar over
           * WideChar). kgpc_type_equals(AnsiChar, WideChar) returns true (same
           * CHAR_TYPE tag), so without this fix, AnsiChar→WideChar would get
           * MATCH_EXACT (kind=0). We detect WideChar formals and upgrade kind
           * to MATCH_PROMOTION. */
          if (arg_tag == CHAR_TYPE && formal_tag == CHAR_TYPE) {
            long long formal_size =
                formal_kgpc != NULL ? kgpc_type_sizeof(formal_kgpc) : 1;
            int is_wide = (formal_size > 1);
            StringKind actual_char_kind = STRING_KIND_UNKNOWN;
            StringKind formal_char_kind =
                is_wide ? STRING_KIND_WIDE : STRING_KIND_NARROW;
            /* Also check by name: WideChar type alias may not carry size in
             * formal params */
            const char *fid = semcheck_get_param_type_id(formal_decl);
            if (!is_wide) {
              if (fid != NULL &&
                  (pascal_identifier_equals(fid, "WideChar") ||
            ...
            if (actual_char_kind != STRING_KIND_UNKNOWN &&
                formal_char_kind != STRING_KIND_UNKNOWN) {
          ...
          if (arg_expr != NULL) {
            ...
            if (arg_type_id == NULL && arg_kgpc != NULL &&
                arg_kgpc->type_alias != NULL) {
            ...
            if (!quality.exact_type_id && formal_id != NULL &&
              ...
              StringKind formal_kind =
                  semcheck_string_kind_from_type_id(formal_id);
              StringKind arg_kind =
                  semcheck_string_kind_from_type_id(arg_type_id);
              if (formal_kind != STRING_KIND_UNKNOWN &&
                  arg_kind != STRING_KIND_UNKNOWN) {
                if (formal_kind == arg_kind)
                  quality.exact_type_id = 1;
            ...
            if (!quality.exact_type_id && formal_id != NULL &&
              ...
              const char *formal_num =
                  semcheck_numeric_type_canonical_id(formal_id);
              const char *arg_num =
                  semcheck_numeric_type_canonical_id(arg_type_id);
              if (formal_num != NULL && arg_num != NULL &&
                  pascal_identifier_equals(formal_num, arg_num))
                quality.exact_type_id = 1;
            }
    ...
    if (num_best == 0) {
      ...
      int cmp = semcheck_compare_match_quality(given_count, qualities,
                                               best_qualities);
      int tiebreak_cmp = 0;
      if (cmp == 0) {
      ...
      if (cmp < 0 || (cmp == 0 && missing_args < best_missing) ||
        ...
        if (best_match != NULL && best_match->mangled_id != NULL &&
            ...
            pascal_identifier_equals(best_match->mangled_id,
                                     candidate->mangled_id)) {
          /* Identical mangled target: treat as duplicate candidate, not
           * ambiguity. */
          free(qualities);
          continue;
        }
        ...
        int share_sig = 0;
        if (best_match != NULL)
          share_sig = semcheck_candidates_share_signature(symtab, best_match,
                                                          candidate);
        if (share_sig) {
