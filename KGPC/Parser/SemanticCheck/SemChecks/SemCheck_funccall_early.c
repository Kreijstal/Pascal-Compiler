...
typedef int (*SemcheckBuiltinFnWithArg)(int *, SymTab_t *, struct Expression *,
                                        int, int);
  ...
  for (int i = 0; i < BUILTIN_FUNC_MAP_COUNT; ++i) {
    if (pascal_identifier_equals(id, BUILTIN_FUNC_MAP[i].name)) {
      if (BUILTIN_FUNC_MAP[i].fn != NULL)
        *status_out =
            BUILTIN_FUNC_MAP[i].fn(type_return, symtab, expr, max_scope_lev);
      else
    ...
    for (ListNode_t *arg_cur = ctx->args_given; arg_cur != NULL;
      ...
      if (kgpc_getenv("KGPC_DEBUG_RESULT_NAME") != NULL &&
          ...
          pascal_identifier_equals(arg_expr->expr_data.id,
                                   "correct_fpuregister")) {
        fprintf(stderr,
                "[KGPC_DEBUG_RESULT_NAME] funccall arg id=%s cur=%s result=%s "
                "method=%s\n",
      ...
      if (!pascal_identifier_equals(arg_id, replacement)) {
        ...
        if (ctx->symtab != NULL && ctx->symtab->current_scope != NULL &&
          ...
          orig_check =
              FindIdentInTable(ctx->symtab->current_scope->table, arg_id);
        ...
        if (ctx->symtab != NULL && ctx->symtab->current_scope != NULL &&
            ctx->symtab->current_scope->table != NULL) {
          local_check =
              FindIdentInTable(ctx->symtab->current_scope->table, replacement);
          if (local_check != NULL &&
...
FunccallState funccall_state_inherited(FunccallCtx *ctx) {
  /* Handle inherited calls in expression context: resolve to parent class
   * method. E.g., T(inherited Get(Index)^) should call TFPSList.Get, not
   * TFPGList.Get. */
  if (ctx->expr->expr_data.function_call_data.is_inherited_call &&
      ctx->id != NULL) {
    if (ctx->args_given == NULL &&
        ctx->expr->expr_data.function_call_data.is_bare_inherited) {
    ...
    if (FindSymbol(&self_node, ctx->symtab, "Self") != 0 && self_node != NULL) {
      ...
      if (current_class != NULL && current_class->parent_class_name != NULL) {
        ...
        while (search_parent != NULL) {
          ...
          if (FindSymbol(&parent_node, ctx->symtab, (char *)search_parent) !=
                  0 &&
              parent_node != NULL) {
...
FunccallState funccall_state_intrinsics(FunccallCtx *ctx) {
  ...
  if (ctx->id != NULL && pascal_identifier_equals(ctx->id, "supports") &&
      ctx->args_given != NULL && ctx->args_given->next != NULL &&
      ctx->args_given->next->next == NULL) {
    struct Expression *arg2 = (struct Expression *)ctx->args_given->next->cur;
    if (arg2 != NULL && arg2->type == EXPR_FUNCTION_CALL &&
      ...
      HashNode_t *type_node = semcheck_find_preferred_type_node(
          ctx->symtab, arg2->expr_data.function_call_data.id);
      if (type_node != NULL && type_node->hash_type == HASHTYPE_TYPE) {
        ListNode_t *captured_arg_node =
            arg2->expr_data.function_call_data.args_expr;
        struct Expression *captured_arg =
            (struct Expression *)captured_arg_node->cur;
  ...
  if (kgpc_getenv("KGPC_DEBUG_EOF") != NULL && ctx->id != NULL &&
    ...
    fprintf(stderr,
            "[KGPC_DEBUG_EOF] semcheck_funccall enter id=%s placeholder=%d "
            "args=%d procvar=%d call_valid=%d\n",
            ctx->id,
  ...
  /* FPC compiler intrinsics: get_frame, get_pc_addr, get_caller_addr,
   * get_caller_frame These return Pointer and are used for exception handling /
   * stack trace support. */
  if (ctx->id != NULL &&
    ...
    while (args != NULL) {
      ...
      if (arg_expr != NULL) {
        ...
        ctx->return_val +=
            semcheck_expr_legacy_tag(&arg_type_local, ctx->symtab, arg_expr,
  ...
  if (semcheck_try_rewrite_internconst_call(ctx->type_return, ctx->symtab,
                                            ctx->expr))
    do {
      ctx->final_status = ctx->return_val;
      return FC_CLEANUP;
  ...
  if (ctx->id != NULL && ctx->args_given != NULL) {
    ...
    if (first_arg != NULL && first_arg->type == EXPR_VAR_ID &&
      ...
      HashNode_t *builtin_node =
          FindIdentInTable(ctx->symtab->builtin_scope->table, ctx->id);
  ...
  if (ctx->id != NULL &&
    ...
    if (builtin_node == NULL) {
      ...
      if (sys_idx > 0 && sys_idx < SYMTAB_MAX_UNITS &&
        ...
        HashNode_t *sys_node =
            FindIdentInTable(ctx->symtab->unit_scopes[sys_idx]->table, ctx->id);
        if (sys_node != NULL &&
    ...
    if (builtin_node != NULL) {
      ...
      if (with_context_count > 0) {
        ...
        int wm = semcheck_with_try_resolve_method(
            ctx->id, ctx->symtab, &with_check, ctx->expr->line_num);
    ...
    if (pascal_identifier_equals(ctx->id, "UpCase") ||
      ...
      if (uc_args != NULL && uc_args->next == NULL) {
        ...
        if (uc_arg != NULL && uc_arg->type == EXPR_FUNCTION_CALL &&
          ...
          semcheck_try_reinterpret_as_typecast(&uc_cast_type, ctx->symtab,
                                               uc_arg, ctx->max_scope_lev);
        int uc_err = semcheck_expr_with_type(&uc_kgpc_type, ctx->symtab, uc_arg,
                                             ctx->max_scope_lev, NO_MUTATE);
        int uc_tag = semcheck_tag_from_kgpc(uc_kgpc_type);
        if (uc_err == 0 &&
            (uc_tag == CHAR_TYPE || semcheck_expr_is_char_like(uc_arg) ||
        ...
        if (uc_err == 0 && uc_tag == STRING_TYPE && uc_arg != NULL &&
            uc_arg->type == EXPR_STRING && uc_arg->expr_data.string != NULL &&
  ...
  /* Trunc with Currency argument needs special handling — Currency is Int64
     internally, not a double, so it must call kgpc_trunc_currency instead of
     kgpc_trunc. Must intercept before normal overload resolution (which would
     coerce Currency to Real). */
  if (ctx->id != NULL && pascal_identifier_equals(ctx->id, "Trunc") &&
    ...
    if (trunc_args != NULL && trunc_args->next == NULL) {
      ...
      int trunc_err =
          semcheck_expr_with_type(&trunc_arg_type, ctx->symtab, trunc_arg,
                                  ctx->max_scope_lev, NO_MUTATE);
      if (trunc_err == 0 && trunc_arg_type != NULL &&
          semcheck_is_currency_kgpc_type(trunc_arg_type))
        do {
          ctx->final_status = semcheck_builtin_trunc(
              ctx->type_return, ctx->symtab, ctx->expr, ctx->max_scope_lev);
          return FC_CLEANUP;
        } while (0);
    }
