/*
    SemCheck_Expr_Types_recordaccess.c - Record/class field access semantic
   checking
...
int semcheck_recordaccess(int *type_return, SymTab_t *symtab,
  ...
  if (record_expr == NULL || field_id == NULL) {
    semcheck_error_with_context_at(
        ...
        "Error on line %d, malformed record field access.\n\n", expr->line_num);
  ...
  if (record_expr->type == EXPR_FUNCTION_CALL) {
    ...
    int cast_result = semcheck_try_reinterpret_as_typecast(
        &rec_cast_type, symtab, record_expr, max_scope_lev);
    if (cast_result != 0) {
      *type_return = UNKNOWN_TYPE;
      return cast_result;
    }
  ...
  if (record_expr->type == EXPR_VAR_ID ||
    ...
    if (record_expr->type == EXPR_VAR_ID) {
      size_t qualified_len =
          strlen(record_expr->expr_data.id) + 1 + strlen(field_id) + 1;
      qualified_id = (char *)malloc(qualified_len);
      if (qualified_id != NULL)
        snprintf(qualified_id, qualified_len, "%s.%s",
  ...
  /* Scoped enum support for identifiers that resolve as types (e.g.,
   * TEndian.Little). If record_expr is a type name, resolve field_id as enum
   * literal. Skip if the identifier is a known variable — e.g., a local
   * variable named "hmodule" should not be confused with the HMODULE type
   * alias.
   */
  if (record_expr->type == EXPR_VAR_ID && record_expr->expr_data.id != NULL) {
    ...
    HashNode_t *enum_type_node =
        semcheck_find_visible_enum_type_candidate_with_literal(
    ...
    if (type_node == NULL || type_node->hash_type != HASHTYPE_TYPE) {
      ...
      HashNode_t *chain_node = semcheck_find_type_node_in_owner_chain(
          symtab, record_expr->expr_data.id, owner_full, owner_outer);
    ...
    if (type_node != NULL && type_node->hash_type == HASHTYPE_TYPE) {
      ...
      if (type_alias != NULL) {
        ...
        if ((type_alias->is_enum && type_alias->enum_literals != NULL) ||
            ...
            (type_alias->target_type_ref != NULL &&
          ...
          if (type_alias->is_enum) {
            resolved = semcheck_resolve_scoped_enum_literal(
                symtab, resolve_name, field_id, &enum_value);
          } else if (type_alias->target_type_ref != NULL &&
                     type_alias->target_type_ref->name != NULL) {
            resolved = semcheck_resolve_scoped_enum_literal_ref(
                symtab, type_alias->target_type_ref->name, field_id,
                &enum_value);
          } else if (type_alias->target_type_id != NULL) {
            resolved = semcheck_resolve_scoped_enum_literal(
                symtab, type_alias->target_type_id, field_id, &enum_value);
          }
        ...
        if (qualified != NULL) {
          ...
          if (FindSymbol(&nested_node, symtab, qualified) != 0 &&
            ...
            if (nested_node->type != NULL)
              semcheck_expr_set_resolved_kgpc_type_shared(expr,
                                                          nested_node->type);
    ...
    if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
      ...
      if (type_node != NULL)
        fprintf(stderr, " id=%s hash_type=%d unit=%d",
                type_node->id ? type_node->id : "NULL", type_node->hash_type,
...
skip_scoped_enum_resolution:
  ...
  /* AST TRANSFORMATION FIX: Parser incorrectly parses `-r.x` as `(-r).x`
   * instead of `-(r.x)`. When we detect this pattern (record access on a sign
   * term), we restructure the AST to have the correct operator precedence: the
   * sign term should wrap the record access.
   *
   * We only handle EXPR_SIGN_TERM (unary operators) as binary operators (ADDOP,
   * MULOP) would require more complex transformation logic. */
  if (record_expr->type == EXPR_SIGN_TERM) {
    ...
    if (inner_expr != NULL) {
      ...
      struct Expression *new_record_access =
          (struct Expression *)calloc(1, sizeof(struct Expression));
      if (new_record_access == NULL) {
        semcheck_error_with_context_at(
            expr->line_num, expr->col_num, expr->source_index,
            "Error on line %d: failed to allocate expression for AST "
            "transformation in semcheck_recordaccess.\n",
            expr->line_num);
        *type_return = UNKNOWN_TYPE;
        return 1;
      }
      ...
      new_record_access->expr_data.record_access_data.field_id =
          strdup(field_id);
      if (new_record_access->expr_data.record_access_data.field_id == NULL) {
        semcheck_error_with_context_at(expr->line_num, expr->col_num,
                                       expr->source_index,
                                       "Error on line %d: failed to duplicate "
                                       "field name in AST transformation.\n",
                                       expr->line_num);
      ...
      /* Restructure: make the current expr be the SIGN_TERM wrapping the new
       * RECORD_ACCESS */
      record_expr->expr_data.sign_term = new_record_access;

      /* Now swap the types so expr becomes SIGN_TERM and the original SIGN_TERM
      ...
      /* Now expr is SIGN_TERM wrapping new_record_access, and we can process it
       * as a sign term */
      /* Redirect to semcheck_signterm for the transformed expression */
      return semcheck_signterm(type_return, symtab, expr, max_scope_lev,
                               mutating);
    }
  }
    ...
    while (walk != NULL && walk->type == EXPR_RECORD_ACCESS) {
      ...
      if (inner != NULL && inner->type == EXPR_RELOP &&
        ...
        *not_node = *expr;
        ...
        return semcheck_relop(type_return, symtab, expr, max_scope_lev,
                              mutating);
      }
      walk = inner;
    }
  ...
  /* Similar AST transformation for NOT operator: parser produces (NOT
   * record).field instead of NOT (record.field). Detect EXPR_RELOP with NOT and
   * restructure so the NOT wraps the record access rather than the record
   * expression itself. */
  if (record_expr->type == EXPR_RELOP &&
    ...
    /* Now expr is the NOT expression wrapping record access; re-run semantic
     * check as relop */
    return semcheck_relop(type_return, symtab, expr, max_scope_lev, mutating);
  }

  /* FPC Bootstrap Feature: Handle unit-qualified identifiers in runtime
   * expressions. When we see UnitName.ConstName and UnitName is an unresolvable
   * identifier, check if ConstName is a known constant/var in the current scope
   * (since unit exports are merged). If so, transform the expression to just
   * the identifier. */
  if (record_expr->type == EXPR_VAR_ID && record_expr->expr_data.id != NULL) {
    ...
    if (!unit_is_qualifier &&
        !(find_result && (unit_check->hash_type == HASHTYPE_VAR ||
    ...
    if (!unit_is_qualifier && !find_result && unit_registry_contains(unit_id)) {
      ...
      if (kgpc_getenv("KGPC_DEBUG_RECORD_ACCESS") != NULL) {
        fprintf(stderr,
                "[KGPC_DEBUG_RECORD_ACCESS] unit-qualifier registry fallback: "
                "unit=%s field=%s\n",
      ...
      if (kgpc_getenv("KGPC_ASSERT_UNIT_QUALIFIER") != NULL)
        assert(!find_result &&
               "unit-qualifier registry fallback requires unresolved name");
    }
    if (unit_is_qualifier) {
      if (kgpc_getenv("KGPC_DEBUG_RECORD_ACCESS") != NULL) {
        fprintf(stderr,
                "[KGPC_DEBUG_RECORD_ACCESS] unit-qualifier resolve: unit=%s "
                "field=%s\n",
                unit_id != NULL ? unit_id : "(null)",
                field_id != NULL ? field_id : "(null)");
      }
      /* Unit-qualified access: look up field_id directly in the unit's own
       * table. This ensures e.g. System.MaxInt returns System's MaxInt (32767),
      ...
      if (unit_idx > 0 && unit_idx < SYMTAB_MAX_UNITS &&
        ...
        field_node =
            FindIdentInTable(symtab->unit_scopes[unit_idx]->table, field_id);
      }
      if (field_node == NULL) {
        ...
        if (field_node->hash_type == HASHTYPE_CONST) {
          ...
          *type_return = LONGINT_TYPE;
          ...
          if (field_copy == NULL) {
            semcheck_error_with_context_at(
                expr->line_num, expr->col_num, expr->source_index,
                "Error on line %d: failed to allocate memory for "
                "unit-qualified variable.\n",
                expr->line_num);
            *type_return = UNKNOWN_TYPE;
            return 1;
          }
          ...
          if (qualified_name != NULL) {
            if (FindSymbol(&qualified_type, symtab, qualified_name) == 0 ||
                qualified_type == NULL ||
                qualified_type->hash_type != HASHTYPE_TYPE) {
              QualifiedIdent *qid = qualified_ident_from_dotted(qualified_name);
              if (qid != NULL) {
                qualified_type =
                    semcheck_find_exact_qualified_type_node(symtab, qid);
          ...
          semcheck_expr_set_resolved_type(
              expr, semcheck_tag_from_kgpc(qualified_type->type));
          ...
          *type_return = semcheck_tag_from_kgpc(qualified_type->type);
          ...
          return semcheck_funccall(type_return, symtab, expr, max_scope_lev,
                                   mutating);
        }
      }
    }
    /* Scoped enum support: TEnumType.EnumValue
     * When unit_check is found and it's a type with an enum, look up the
     * field_id as an enum literal and transform to its ordinal constant. */
    else if (preferred_type != NULL &&
             preferred_type->hash_type == HASHTYPE_TYPE) {
      /* Check if the type has an enum type alias - look up field_id as enum
       * literal */
      ...
      if (type_alias != NULL && type_alias->is_enum &&
        ...
        while (literal_node != NULL) {
          if (literal_node->cur != NULL) {
            ...
            if (strcasecmp(literal_name, field_id) == 0) {
              ...
              semcheck_expr_set_resolved_type(expr, ENUM_TYPE);
              if (type_alias->kgpc_type != NULL) {
                /* Use the shared type setter to properly manage reference
                 * counting */
                semcheck_expr_set_resolved_kgpc_type_shared(
                    expr, type_alias->kgpc_type);
              } else {
        ...
        if (type_alias->target_type_ref != NULL &&
            type_alias->target_type_ref->name != NULL) {
          resolved_via_target = semcheck_resolve_scoped_enum_literal_ref(
              symtab, type_alias->target_type_ref->name, field_id,
              &alias_target_value);
          HashNode_t *target_type_node = semcheck_find_exact_type_node_for_ref(
              symtab, type_alias->target_type_ref, type_alias->target_type_id,
              field_id);
          struct TypeAlias *target_alias =
              (target_type_node != NULL)
                  ? hashnode_get_type_alias(target_type_node)
                  : NULL;
          if (!resolved_via_target &&
              semcheck_type_alias_has_enum_literal(target_alias, field_id)) {
            int ordinal = 0;
          ...
          if (!resolved_via_target && qualified_target != NULL) {
            resolved_via_target = semcheck_resolve_scoped_enum_literal(
                symtab, qualified_target, field_id, &alias_target_value);
          ...
          resolved_via_target = semcheck_resolve_scoped_enum_literal(
              symtab, type_alias->target_type_id, field_id,
              &alias_target_value);
        }
        if (resolved_via_target) {
          record_access_clear_payload(expr, 1);
          ...
          if (type_alias->kgpc_type != NULL)
            semcheck_expr_set_resolved_kgpc_type_shared(expr,
                                                        type_alias->kgpc_type);
          *type_return = ENUM_TYPE;
          return 0;
        }
      }

      /* If this type is an alias to another type, try resolving scoped enum
       * literal against the alias target (e.g., TEndian = ObjPas.TEndian).
       */
      if (type_alias != NULL) {
        long long enum_value = 0;
        int resolved = 0;
        if (type_alias->target_type_ref != NULL &&
            type_alias->target_type_ref->name != NULL) {
          resolved = semcheck_resolve_scoped_enum_literal_ref(
              symtab, type_alias->target_type_ref->name, field_id, &enum_value);
          HashNode_t *target_type_node = semcheck_find_exact_type_node_for_ref(
              symtab, type_alias->target_type_ref, type_alias->target_type_id,
              field_id);
          struct TypeAlias *target_alias =
          ...
          if (!resolved && qualified_target != NULL) {
            resolved = semcheck_resolve_scoped_enum_literal(
                symtab, qualified_target, field_id, &enum_value);
            free(qualified_target);
          }
          if (!resolved) {
            resolved = semcheck_resolve_scoped_enum_literal_ref(
        ...
        if (FindSymbol(&literal_node, symtab, field_id) != 0 &&
            ...
            (literal_node->hash_type == HASHTYPE_CONST ||
             literal_node->is_constant || literal_node->is_typed_const)) {
          record_access_clear_payload(expr, 1);
          expr->type = EXPR_INUM;
  ...
  if (!mutating && record_expr->type == EXPR_VAR_ID &&
  ...
  if (record_expr->type == EXPR_VAR_ID && record_expr->expr_data.id != NULL &&
    ...
    int trace_unit_case =
        (trace_nonlocal != NULL && record_expr->expr_data.id != NULL &&
         (strcmp(trace_nonlocal, "1") == 0 ||
          pascal_identifier_equals(record_expr->expr_data.id, trace_nonlocal)));
    if (trace_unit_case) {
      fprintf(stderr,
              "[KGPC_TRACE_NONLOCAL] semcheck unit-branch unit=%s field=%s "
              "line=%d\n",
              record_expr->expr_data.id, field_id != NULL ? field_id : "<null>",
              expr->line_num);
    }
    if (kgpc_getenv("KGPC_DEBUG_RECORD_ACCESS") != NULL) {
      fprintf(stderr,
    ...
    if (trace_unit_case) {
      fprintf(stderr,
              "[KGPC_TRACE_NONLOCAL] semcheck unit-branch lookup unit=%s "
              "idx=%d hit=%s hash=%d\n",
              record_expr->expr_data.id, unit_idx,
              unit_sym != NULL && unit_sym->id != NULL ? unit_sym->id
                                                       : "<null>",
              unit_sym != NULL ? unit_sym->hash_type : -1);
    }
    if (unit_sym == NULL && FindSymbol(&unit_sym, symtab, field_id) != 0 &&
        unit_sym != NULL) {
      if (trace_unit_case) {
        fprintf(stderr,
                "[KGPC_TRACE_NONLOCAL] semcheck unit-branch fallback hit=%s "
                "hash=%d\n",
                unit_sym->id != NULL ? unit_sym->id : "<null>",
                unit_sym->hash_type);
      }
      /* Transform EXPR_RECORD_ACCESS into EXPR_VAR_ID for the resolved
       * identifier */
      ...
      int result = semcheck_expr_with_type(&resolved_kgpc, symtab, expr,
  ...
  error_count += semcheck_expr_with_type(&record_kgpc_type, symtab, record_expr,
  ...
  if (kgpc_getenv("KGPC_DEBUG_RECORD_ACCESS") != NULL) {
    ...
    fprintf(stderr,
            "[KGPC_DEBUG_RECORD_ACCESS] record_expr_type=%d record_type=%d "
            "kgpc=%s\n",
            record_expr->type, record_type,
  ...
  if (record_type == PROCEDURE && record_kgpc_type != NULL &&
    ...
    if (ret_type == NULL &&
        record_kgpc_type->info.proc_info.return_type_id != NULL) {
      ...
      if (FindSymbol(&ret_node, symtab,
                     record_kgpc_type->info.proc_info.return_type_id) != 0 &&
  ...
  if (record_expr->type == EXPR_RECORD_ACCESS) {
    struct Expression *inner_rec =
        record_expr->expr_data.record_access_data.record_expr;
    const char *inner_field =
        record_expr->expr_data.record_access_data.field_id;
    if (inner_rec != NULL && inner_rec->type == EXPR_VAR_ID &&
        ...
        inner_field != NULL && semcheck_has_value_ident(symtab, inner_field)) {
      struct Expression *value_expr =
          mk_varid(expr->line_num, strdup(inner_field));
      if (value_expr != NULL) {
        destroy_expr(record_expr);
        ...
        error_count += semcheck_expr_with_type(
            &record_kgpc_type, symtab, record_expr, max_scope_lev, NO_MUTATE);
        record_type = semcheck_tag_from_kgpc(record_kgpc_type);
      }
    }
  }
    ...
    if (FindSymbol(&self_node, symtab, "Self") != 0 && self_node != NULL) {
      ...
      if (self_record != NULL) {
        ...
        if (expr_record == self_record) {
          ...
          if (resolve_record_field(symtab, self_record,
                                   record_expr->expr_data.id, &self_field,
                                   &field_offset, expr->line_num, 1) == 0 &&
              self_field != NULL) {
            struct Expression *self_expr =
                mk_varid(expr->line_num, strdup("Self"));
            if (self_expr != NULL) {
              ...
              record_expr->expr_data.record_access_data.field_offset =
                  field_offset;
...
SKIP_SELF_FIELD_REWRITE:
  ...
  if (record_type == ENUM_TYPE) {
    ...
    if (enum_type_name != NULL &&
        semcheck_resolve_scoped_enum_literal(symtab, enum_type_name, field_id,
                                             &enum_value)) {
    ...
    if (record_kgpc_type != NULL && record_kgpc_type->type_alias != NULL &&
      ...
      if (semcheck_resolve_scoped_enum_literal(symtab, enum_type_name, field_id,
                                               &enum_value)) {
  ...
  if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
    fprintf(stderr,
            "[SemCheck] semcheck_recordaccess: field_id=%s, record_type=%d\n",
  ...
  if (record_type == RECORD_TYPE) {
    ...
    if (kgpc_getenv("KGPC_DEBUG_RECORD_ACCESS") != NULL) {
      fprintf(
          stderr, "[KGPC_DEBUG_RECORD_ACCESS] record_info=%p from_kgpc=%d\n",
          (void *)record_info,
    ...
    if (record_info == NULL && record_expr != NULL &&
      ...
      if (record_info == NULL && record_expr->array_element_type_id != NULL)
        record_info = semcheck_lookup_record_type(
            symtab, record_expr->array_element_type_id);

      struct Expression *base_array_expr =
          record_expr->expr_data.array_access_data.array_expr;
      if (record_info == NULL && base_array_expr != NULL) {
        ...
        if (record_info == NULL &&
            base_array_expr->array_element_type_id != NULL)
          record_info = semcheck_lookup_record_type(
              symtab, base_array_expr->array_element_type_id);
        if (record_info == NULL &&
            base_array_expr->resolved_kgpc_type != NULL &&
            kgpc_type_is_array(base_array_expr->resolved_kgpc_type)) {
          KgpcType *elem_type = kgpc_type_get_array_element_type(
              base_array_expr->resolved_kgpc_type);
    ...
    if (record_info == NULL && record_expr->type == EXPR_TYPECAST) {
      const char *target_id =
          record_expr->expr_data.typecast_data.target_type_id;
      if (target_id != NULL) {
        HashNode_t *type_node =
            semcheck_find_type_node_with_kgpc_type(symtab, target_id);
        if (type_node == NULL)
    ...
    if (record_info == NULL && record_expr->type == EXPR_POINTER_DEREF) {
      struct Expression *ptr_expr =
          record_expr->expr_data.pointer_deref_data.pointer_expr;
      ...
      if (ptr_expr != NULL) {
        ...
        if (subtype_id == NULL && ptr_expr->type == EXPR_TYPECAST) {
          const char *target_id =
              ptr_expr->expr_data.typecast_data.target_type_id;
          const TypeRef *target_ref =
              ptr_expr->expr_data.typecast_data.target_type_ref;
          ...
          if (target_node == NULL && target_id != NULL) {
            ...
            target_node = semcheck_find_type_node_in_owner_chain(
                symtab, target_id, owner_full, owner_outer);
          }
          if (target_node != NULL) {
    ...
    if (record_info == NULL && record_expr->pointer_subtype_id != NULL) {
      HashNode_t *target_node = semcheck_find_type_node_with_kgpc_type(
          symtab, record_expr->pointer_subtype_id);
      if (target_node == NULL)
    ...
    if (record_info == NULL) {
      ...
      if (record_type != UNKNOWN_TYPE) {
        ...
        struct RecordType *helper_record =
            semcheck_lookup_type_helper_for_member(symtab, record_type,
                                                   expr_type_name, field_id);
      ...
      if (record_info == NULL) {
        semcheck_error_with_context_at(
            expr->line_num, expr->col_num, expr->source_index,
            "Error on line %d, pointer does not reference a record type.\n\n",
            expr->line_num);
        *type_return = UNKNOWN_TYPE;
    ...
    if (record_expr->type == EXPR_POINTER_DEREF) {
      ...
      if (record_info == NULL && record_expr->pointer_subtype_id != NULL) {
        struct RecordType *ptr_rec = semcheck_lookup_record_type(
            symtab, record_expr->pointer_subtype_id);
        if (ptr_rec != NULL) {
          record_type = RECORD_TYPE;
          record_info = ptr_rec;
    ...
    if (record_type == RECORD_TYPE && record_info != NULL) {
      ...
      semcheck_error_with_context_at(
          expr->line_num, expr->col_num, expr->source_index,
          "Error on line %d, pointer dereference does not reference a record "
          "type "
      ...
      *type_return = UNKNOWN_TYPE;
      ...
      if (is_self_expr) {
        ...
        if (current_owner != NULL) {
          struct RecordType *owner_rec =
              semcheck_lookup_record_type(symtab, current_owner);
      ...
      if (helper_record != NULL) {
        ...
        /* FPC allows Float.IsNan via type helpers. If helpers weren't
         * registered, fall back to a simple NaN check (x <> x). */
        struct Expression *left_expr = record_expr;
        struct Expression *right_expr = clone_expression(record_expr);
        if (right_expr == NULL) {
          semcheck_error_with_context_at(
              expr->line_num, expr->col_num, expr->source_index,
              "Error on line %d, failed to allocate IsNan expression.\n\n",
              expr->line_num);
          *type_return = UNKNOWN_TYPE;
          return error_count + 1;
        }
         ...
         * The inner access resolved to an enum type; look up field_id as a
         * literal. */
        const char *enum_type_name = expr_type_name;
        struct TypeAlias *resolved_alias =
            (record_expr->resolved_kgpc_type != NULL)
                ? record_expr->resolved_kgpc_type->type_alias
                : NULL;
        ...
        if (resolved_alias != NULL) {
          resolved =
              resolved || semcheck_try_resolve_enum_literal_from_type_alias(
                              symtab, resolved_alias, field_id, &enum_value);
        }
        if (enum_type_name != NULL)
          resolved =
              resolved || semcheck_resolve_scoped_enum_literal(
                              symtab, enum_type_name, field_id, &enum_value);
        if (!resolved) {
          /* Try looking up the field_id directly as a global enum constant */
          HashNode_t *enum_node = NULL;
          if (FindSymbol(&enum_node, symtab, field_id) != 0 &&
              enum_node != NULL && enum_node->hash_type == HASHTYPE_CONST) {
            resolved = 1;
            if (enum_node->type != NULL &&
                enum_node->type->kind == TYPE_KIND_PRIMITIVE)
        ...
        if (resolved) {
          ...
          if (record_expr->resolved_kgpc_type != NULL)
            semcheck_expr_set_resolved_kgpc_type_shared(
                expr, record_expr->resolved_kgpc_type);
          *type_return = ENUM_TYPE;
          return 0;
        }
        semcheck_error_with_context_at(
            expr->line_num, expr->col_num, expr->source_index,
            "Error on line %d, field access requires a record value.\n\n",
            expr->line_num);
        *type_return = UNKNOWN_TYPE;
        return error_count + 1;
      } else {
        if (record_expr->type == EXPR_VAR_ID &&
          ...
          HashNode_t *type_node = semcheck_find_preferred_type_node(
              symtab, record_expr->expr_data.id);
          if (type_node == NULL) {
            const char *owner_full =
                semcheck_get_current_subprogram_owner_class_full();
            const char *owner_outer =
            ...
            if (owner_full == NULL)
              owner_full = semcheck_get_current_method_owner();
            type_node = semcheck_find_type_node_in_owner_chain(
                symtab, record_expr->expr_data.id, owner_full, owner_outer);
          }
          if ((type_node == NULL || type_node->hash_type != HASHTYPE_TYPE)) {
            HashNode_t *fallback_node = NULL;
            if (FindSymbol(&fallback_node, symtab, record_expr->expr_data.id) !=
                    0 &&
                fallback_node != NULL &&
                fallback_node->hash_type == HASHTYPE_TYPE)
              type_node = fallback_node;
          }
          if (type_node != NULL && type_node->hash_type == HASHTYPE_TYPE) {
            struct RecordType *type_record =
        ...
        if (record_info != NULL) {
          ...
          if (kgpc_getenv("KGPC_DEBUG_RECORD_ACCESS") != NULL) {
            ...
            fprintf(
                stderr,
                "[KGPC_DEBUG_RECORD_ACCESS] not-record: line=%d expr=%p "
                "record_expr=%p type=%d rec_id=%s record_type=%d field=%s\n",
                expr->line_num, (void *)expr, (void *)record_expr,
                record_expr != NULL ? record_expr->type : -1,
                rec_id != NULL ? rec_id : "(null)", record_type,
                field_id != NULL ? field_id : "(null)");
          }
          if (record_type == UNKNOWN_TYPE) {
            *type_return = UNKNOWN_TYPE;
  ...
  if (record_info == NULL) {
    semcheck_error_with_context_at(
        expr->line_num, expr->col_num, expr->source_index,
        "Error on line %d, unable to resolve record type for field %s.\n\n",
      ...
      if (self_record != NULL) {
        ...
        if (expr_record == NULL)
          self_match = 1;
                 ...
                 pascal_identifier_equals(expr_record->type_id,
                                          self_record->type_id))
          self_match = 1;
        ...
        if (resolve_record_field(symtab, self_record, record_expr->expr_data.id,
                                 &self_field, &field_offset, expr->line_num,
                                 1) == 0 &&
            self_field != NULL) {
          struct Expression *self_expr =
              mk_varid(expr->line_num, strdup("Self"));
          if (self_expr != NULL) {
            ...
            if (self_field->nested_record != NULL)
              ...
              record_info =
                  semcheck_lookup_record_type(symtab, self_field->type_id);
  ...
  /* For classes and records with potential methods, use silent mode when
   * looking for fields, since we'll check properties and methods next */
  int silent_mode = 1; /* Always use silent mode - we'll print a better error
                          later if needed */
  if (resolve_record_field(symtab, record_info, field_id, &field_desc,
                           &field_offset, expr->line_num, silent_mode) != 0 ||
      field_desc == NULL) {
      ...
      if (alias_target != NULL || alias_name != NULL) {
        ...
        if (alias_record != NULL) {
          ...
          if (resolve_record_field(symtab, alias_record, field_id, &alias_field,
                                   &alias_offset, expr->line_num, 1) == 0 &&
              alias_field != NULL) {
  ...
  if (field_desc == NULL) {
    ...
    if (property != NULL) {
      ...
      if (mutating == NO_MUTATE) {
        if (property->read_accessor == NULL) {
          semcheck_error_with_context_at(
              expr->line_num, expr->col_num, expr->source_index,
              "Error on line %d, property %s is write-only.\n\n",
              expr->line_num,
        ...
        if (read_field != NULL &&
            resolve_record_field(symtab, record_info, property->read_accessor,
                                 &field_desc, &field_offset, expr->line_num,
                                 0) == 0 &&
            field_desc != NULL) {
          if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
            fprintf(stderr,
                    "[SemCheck]   Transforming property '%s' to field '%s'\n",
          ...
          if (!pascal_identifier_equals(field_id, property->read_accessor)) {
            ...
            if (expr->expr_data.record_access_data.field_id == NULL) {
              semcheck_error_with_context_at(
                  expr->line_num, expr->col_num, expr->source_index,
                  "Error on line %d, failed to allocate property field "
                  "name.\n\n",
                  expr->line_num);
              *type_return = UNKNOWN_TYPE;
              return error_count + 1;
        ...
        HashNode_t *getter_node = semcheck_find_class_method(
            symtab, property_owner, property->read_accessor, NULL);
        if (getter_node == NULL) {
          semcheck_error_with_context_at(
              expr->line_num, expr->col_num, expr->source_index,
              "Error on line %d, getter %s for property %s not found.\n\n",
              expr->line_num,
              property->read_accessor != NULL ? property->read_accessor
                                              : "<unknown>",
              property->name != NULL ? property->name : field_id);
          *type_return = UNKNOWN_TYPE;
          return error_count + 1;
        ...
        if (!getter_is_function) {
          semcheck_error_with_context_at(
              expr->line_num, expr->col_num, expr->source_index,
              "Error on line %d, property getter %s must be a function.\n\n",
              expr->line_num, property->read_accessor);
          *type_return = UNKNOWN_TYPE;
          return error_count + 1;
        }

        return semcheck_transform_property_getter_call(
            type_return, symtab, expr, max_scope_lev, mutating, getter_node,
            property_owner);
      } else {
        if (property->write_accessor == NULL) {
          semcheck_error_with_context_at(
              expr->line_num, expr->col_num, expr->source_index,
              "Error on line %d, property %s is read-only.\n\n", expr->line_num,
              property->name != NULL ? property->name : field_id);
          *type_return = UNKNOWN_TYPE;
          return error_count + 1;
        }
        ...
        struct RecordField *write_field =
            semcheck_find_class_field_including_hidden(
                symtab, record_info, property->write_accessor, NULL);
        if (write_field != NULL &&
        ...
        if (mutating == BOTH_MUTATE_REFERENCE) {
          semcheck_error_with_context_at(
              expr->line_num, expr->col_num, expr->source_index,
              "Error on line %d, property %s cannot be passed as a var "
              "parameter.\n\n",
              expr->line_num,
              property->name != NULL ? property->name : field_id);
          *type_return = UNKNOWN_TYPE;
    ...
    HashNode_t *method_node = semcheck_find_class_method(
        symtab, record_info, field_id, &method_owner_record);
    if (method_node != NULL) {
      ...
      if (method_node->hash_type == HASHTYPE_FUNCTION ||
        ...
        if (record_info->type_id != NULL && field_id != NULL) {
          is_static_method =
              from_cparser_is_method_static(record_info->type_id, field_id);
        }
        ...
        if (!is_static_method && method_owner_record != NULL &&
            method_owner_record->type_id != NULL && field_id != NULL) {
          is_static_method = from_cparser_is_method_static(
              method_owner_record->type_id, field_id);
        }

        /* For overloaded methods, we need to find the correct overload based on
         * argument count. Since this is a bare record access (s.Foo without
         * parens), there are no explicit arguments - only the implicit Self
         * parameter. Find the overload that accepts just Self (1 param). */
        int args_for_call =
            is_static_method ? 0 : 1; /* Just Self for non-static */
        ...
        if (record_info->type_id != NULL) {
          ...
          if (all_methods != NULL) {
            /* Find the best overload: one that requires exactly args_for_call
             * params */
            ...
            while (cur != NULL) {
              ...
              if (candidate != NULL &&
                ...
                if (args_for_call >= required_params &&
                    args_for_call <= total_params) {
                  /* Prefer exact match over range match */
                  if (best_match == NULL) {
                    best_match = candidate;
                  } else {
                    /* Pick the one with fewer total params (more specific) */
                    ListNode_t *best_params =
                        kgpc_type_get_procedure_params(best_match->type);
        ...
        /* For static methods:
         * - If receiver is a type identifier (TypeName.Method), pass it as
         * first arg so semcheck_funccall can detect and handle the
         * type-qualified call
         * - If receiver is an instance variable, no receiver needed for static
         * method
         */
        ...
        if (is_static_method) {
          if (record_expr->type == EXPR_VAR_ID &&
            ...
            HashNode_t *type_node = NULL;
            if (FindSymbol(&type_node, symtab, record_expr->expr_data.id) !=
                    0 &&
                type_node != NULL && type_node->hash_type == HASHTYPE_TYPE) {
              /* It's a type-qualified static method call - pass type as first
               * arg */
        ...
        int is_constructor_call =
            (!is_static_method && record_info != NULL &&
             record_type_is_class(record_info) &&
             !record_info->is_type_helper && method_name != NULL &&
             semcheck_method_is_declared_constructor(symtab, record_info,
                                                     method_name));
        char *method_name_copy =
        ...
        int funccall_result = semcheck_funccall(type_return, symtab, expr,
                                                max_scope_lev, mutating);
    ...
    /* If the field is "Create" and this is a class type, treat it as a default
     * constructor */
    if (record_type_is_class(record_info) && field_id != NULL &&
       ...
       * 2. A class reference variable like ClassRef.Create - use variable's
       * value as VMT
       */
      if (record_expr->type == EXPR_VAR_ID &&
          record_expr->expr_data.id != NULL) {
        ...
        int is_type = 0;
        if (FindSymbol(&ident_node, symtab, (char *)expr_name) != 0 &&
            ident_node != NULL) {
        ...
        if (record_kgpc == NULL) {
          semcheck_error_with_context_at(
              expr->line_num, expr->col_num, expr->source_index,
              "Error on line %d: Unable to create KgpcType for class %s\n",
        ...
        if (class_size <= 0) {
          semcheck_error_with_context_at(
              expr->line_num, expr->col_num, expr->source_index,
              "Error on line %d: Unable to determine size for class %s\n",
              expr->line_num, expr_name);
          destroy_expr(record_expr);
          return error_count + 1;
        }
        ...
        /* Transform the expression into a function call to
         * __kgpc_default_create */
        expr->type = EXPR_FUNCTION_CALL;
        ...
        *type_return = POINTER_TYPE;
        ...
        if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
          fprintf(stderr,
                  "[SemCheck] semcheck_recordaccess: Transformed '%s.%s' to "
                  "__kgpc_default_create(%lld, %s) call\n",
    ...
    if (record_info != NULL && !record_type_is_class(record_info)) {
      HashNode_t *method_node =
          semcheck_find_class_method(symtab, record_info, field_id, NULL);
      if (method_node != NULL) {
        /* Found a method on an advanced record */
        if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
          fprintf(stderr,
                  "[SemCheck] semcheck_recordaccess: Found advanced record "
                  "method %s\n",
                  field_id);
        }
        ...
        if (method_node->hash_type == HASHTYPE_FUNCTION ||
          ...
          if (type_name != NULL && field_id != NULL) {
            is_static_method =
                from_cparser_is_method_static(type_name, field_id);
          }

          if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
            fprintf(stderr,
                    "[SemCheck] semcheck_recordaccess: type=%s method=%s "
                    "is_static=%d\n",
                    type_name ? type_name : "<null>", field_id,
                    is_static_method);
          }
    ...
    if (record_info != NULL && record_info->type_id != NULL &&
      ...
      if (helper_record != NULL) {
        HashNode_t *method_node =
            semcheck_find_class_method(symtab, helper_record, field_id, NULL);
        if (method_node != NULL) {
          if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
            fprintf(stderr,
                    "[SemCheck] semcheck_recordaccess: Found type helper "
                    "method %s on %s\n",
    ...
    if (record_info != NULL && record_info->is_type_helper &&
      ...
      char *mangled_const =
          semcheck_mangle_helper_const_id(record_info->type_id, field_id);
    ...
    if (record_info != NULL && record_info->is_type_helper &&
        record_info->helper_base_type_id != NULL &&
        (pascal_identifier_equals(record_info->helper_base_type_id,
                                  "AnsiString") ||
    ...
    if (record_info != NULL && record_info->record_properties != NULL) {
      ...
      if (property != NULL) {
        const char *accessor = (mutating != NO_MUTATE)
                                   ? property->write_accessor
                                   : property->read_accessor;
        if (accessor != NULL) {
          /* Resolve property to its backing field */
          if (resolve_record_field(symtab, record_info, accessor, &field_desc,
                                   &field_offset, expr->line_num, 1) == 0 &&
              field_desc != NULL) {
          ...
          if (mutating != NO_MUTATE) {
            /* For setter method calls on record properties, leave the
             * expression as EXPR_RECORD_ACCESS so the assignment handler in
          ...
          HashNode_t *method_node =
              semcheck_find_class_method(symtab, record_info, accessor, NULL);
          if (method_node != NULL) {
            struct Expression *receiver = record_expr;
    ...
    /* For classes, check if the field is a nested type (e.g.,
     * TClass.TNestedEnum). Nested types are registered as
     * "ClassName.NestedTypeName" in the symbol table. */
    if (record_info != NULL && record_info->type_id != NULL &&
        field_id != NULL) {
    ...
    if (record_info != NULL && record_info->type_id != NULL) {
      semcheck_error_with_context_at(
          expr->line_num, expr->col_num, expr->source_index,
          "Error on line %d, record field %s not found on type '%s'.\n",
          expr->line_num, field_id, record_info->type_id);
    } else
      semcheck_error_with_context_at(
          expr->line_num, expr->col_num, expr->source_index,
          "Error on line %d, record field %s not found.\n", expr->line_num,
          field_id);
    *type_return = UNKNOWN_TYPE;
    return error_count + 1;
  }
...
FIELD_RESOLVED:
  ...
  if (kgpc_getenv("KGPC_DEBUG_PROC_FIELD") != NULL) {
    fprintf(stderr,
            "[KGPC_DEBUG_PROC_FIELD] field=%s type=%d raw_type=%d type_id=%s "
            "proc_type=%p\n",
            field_id != NULL ? field_id : "<null>", field_type,
            field_desc->type,
  ...
  if (kgpc_getenv("KGPC_DEBUG_POINTER_FIELD") != NULL && field_id != NULL) {
    fprintf(stderr,
            "[KGPC_DEBUG_POINTER_FIELD] field=%s type=%d is_pointer=%d "
            "pointer_type=%d pointer_type_id=%s\n",
            field_id, field_desc->type, field_desc->is_pointer,
            field_desc->pointer_type,
            field_desc->pointer_type_id ? field_desc->pointer_type_id
                                        : "<null>");
  }
  if (kgpc_getenv("KGPC_DEBUG_RECORD_FIELD") != NULL && field_id != NULL &&
  ...
  if (kgpc_getenv("KGPC_DEBUG_RECORD_ACCESS") != NULL && field_id != NULL) {
    fprintf(
        ...
        "resolved=%s is_array=%d elem_type=%d elem_type_id=%s elem_record=%p "
        "elem_kgpc=%p is_pointer=%d\n",
        field_id, field_desc->type,
        field_desc->type_id ? field_desc->type_id : "<null>",
        (void *)field_desc->nested_record,
        expr->resolved_kgpc_type ? kgpc_type_to_string(expr->resolved_kgpc_type)
                                 : "<null>",
        field_desc->is_array, field_desc->array_element_type,
        field_desc->array_element_type_id ? field_desc->array_element_type_id
                                          : "<null>",
        (void *)field_desc->array_element_record,
        (void *)field_desc->array_element_kgpc_type, field_desc->is_pointer);
  }
  ...
  if (field_desc->is_array) {
    ...
    if (array_element_id != NULL || array_element_ref != NULL) {
      ...
      if (expr->array_element_type == UNKNOWN_TYPE) {
        HashNode_t *elem_type_node = semcheck_find_preferred_type_node_with_ref(
            symtab, array_element_ref, array_element_id);
        if (elem_type_node == NULL)
          elem_type_node = semcheck_find_type_node_with_kgpc_type_ref(
              symtab, array_element_ref, array_element_id);
        if (elem_type_node != NULL)
      ...
      if (expr->array_element_record_type == NULL && elem_node != NULL) {
        ...
        if (elem_alias != NULL) {
          if (elem_alias->inline_record_type != NULL)
          ...
          else if (elem_alias->target_type_id != NULL ||
                   elem_alias->target_type_ref != NULL) {
            HashNode_t *target_node =
                semcheck_find_preferred_type_node_with_ref(
                    symtab, elem_alias->target_type_ref,
                    elem_alias->target_type_id);
            if (target_node == NULL)
            ...
            if (target_node != NULL) {
              expr->array_element_record_type =
                  get_record_type_from_node(target_node);
      ...
      if (expr->array_element_record_type == NULL && array_element_id != NULL)
        expr->array_element_record_type =
            semcheck_lookup_record_type(symtab, array_element_id);
      if (expr->array_element_record_type != NULL &&
    ...
    if (expr->array_element_record_type != NULL)
      ...
      size_status =
          sizeof_from_type_ref(symtab, expr->array_element_type, sizeof_id,
                               &computed_size, 0, expr->line_num);
    ...
    if (expr->resolved_kgpc_type == NULL &&
      ...
      if (field_desc->array_element_kgpc_type != NULL) {
        ...
        elem_owned = 0;
      } else if (expr->array_element_type_ref != NULL ||
                 expr->array_element_type_id != NULL) {
        HashNode_t *elem_node = semcheck_find_preferred_type_node_with_ref(
            symtab, expr->array_element_type_ref, expr->array_element_type_id);
        if (elem_node == NULL)
          elem_node = semcheck_find_type_node_with_kgpc_type_ref(
              symtab, expr->array_element_type_ref,
              expr->array_element_type_id);
        if (elem_node != NULL && elem_node->type != NULL) {
          elem_type =
              semcheck_create_value_kgpc_type_from_node_local(elem_node);
          elem_owned = 1;
        }
      } else if (elem_type_tag_hint == RECORD_TYPE &&
        ...
        elem_owned = 1;
                 ...
                 expr->array_element_type_id != NULL) {
        struct RecordType *elem_record =
            semcheck_lookup_record_type(symtab, expr->array_element_type_id);
        if (elem_record != NULL) {
          elem_type =
              semcheck_create_value_kgpc_type_for_record_local(elem_record);
          elem_owned = 1;
        }
      }
      ...
      if (elem_type != NULL) {
        if (kgpc_getenv("KGPC_DEBUG_RECORD_ACCESS") != NULL &&
            field_id != NULL && pascal_identifier_equals(field_id, "FArr")) {
          fprintf(stderr,
                  "[KGPC_DEBUG_RECORD_ACCESS] build-array field=%s "
                  "elem_type=%s kind=%d elem_owned=%d elem_record=%p "
                  "array_elem_record=%p array_elem_type_id=%s\n",
                  field_id, kgpc_type_to_string(elem_type), elem_type->kind,
                  elem_owned,
                  elem_type->kind == TYPE_KIND_RECORD
                      ? (void *)elem_type->info.record_info
                      : NULL,
                  (void *)expr->array_element_record_type,
                  expr->array_element_type_id ? expr->array_element_type_id
                                              : "<null>");
        }
        KgpcType *arr_type = create_array_type(
            elem_type, expr->array_lower_bound, expr->array_upper_bound);
        if (arr_type != NULL) {
  ...
  if (!field_desc->is_pointer &&
      (field_desc->type_id != NULL || field_desc->type_ref != NULL)) {
    ...
    HashNode_t *type_node = semcheck_find_preferred_type_node_with_ref(
        symtab, field_ref, type_id_to_use);
    if (type_node == NULL)
      type_node = semcheck_find_type_node_with_kgpc_type_ref(symtab, field_ref,
                                                             type_id_to_use);

    if (type_node == NULL && record_info != NULL &&
        record_info->type_id != NULL) {
      ...
      if (type_node == NULL)
        type_node =
            semcheck_find_type_node_with_kgpc_type(symtab, qualified_name_buf);
      if (type_node != NULL) {
        type_id_to_use = qualified_name_buf;
        qualified_name_allocated = 1;
      }
    }
    /* Fallback: suffix search for nested types (e.g., TPtrWrapper →
     * TMarshal.TPtrWrapper) */
    if (type_node == NULL && field_desc->type_id != NULL) {
      for (ScopeNode *cur = symtab->current_scope;
           cur != NULL && type_node == NULL; cur = cur->parent)
      ...
      if (type_node == NULL) {
        for (int u = 0; u < SYMTAB_MAX_UNITS; u++) {
          if (symtab->unit_scopes[u] != NULL &&
            ...
            type_node = FindTypeBySuffixInTable(symtab->unit_scopes[u]->table,
                                                field_desc->type_id);
            if (type_node != NULL)
              break;
          }
    ...
    if (type_node != NULL) {
      ...
      if (type_node->type != NULL &&
          (kgpc_type_is_array(type_node->type) ||
           kgpc_type_is_array_of_const(type_node->type)) &&
          !kgpc_type_is_shortstring(type_node->type) && !expr->is_array_expr) {
        semcheck_set_array_info_from_hashnode(expr, symtab, type_node,
                                              expr->line_num);
      }
      if (type_node->type != NULL &&
          (kgpc_type_is_array(type_node->type) ||
  ...
  if (kgpc_getenv("KGPC_DEBUG_RECORD_ACCESS") != NULL && field_id != NULL) {
    fprintf(stderr,
            "[KGPC_DEBUG_RECORD_ACCESS] resolved field=%s type=%d type_id=%s "
            "record=%p\n",
            field_id, field_type,
            field_desc->type_id ? field_desc->type_id : "<null>",
            (void *)field_record);
  ...
  if (field_record != NULL && field_type == RECORD_TYPE) {
    ...
    if (expr->resolved_kgpc_type == NULL) {
      KgpcType *record_kgpc =
          semcheck_create_value_kgpc_type_for_record_local(field_record);
      if (record_kgpc != NULL) {
  ...
  if (field_type == UNKNOWN_TYPE && field_record == NULL &&
      array_alias == NULL && !field_is_array_type) {
    semcheck_error_with_context_at(
        expr->line_num, expr->col_num, expr->source_index,
        "Error on line %d, unable to resolve type for field %s.\n\n",
        expr->line_num, field_id);
    *type_return = UNKNOWN_TYPE;
    return error_count + 1;
  }
  ...
  if (field_type == RECORD_TYPE && field_record == NULL &&
    ...
    semcheck_error_with_context_at(
        expr->line_num, expr->col_num, expr->source_index,
        "Error on line %d, missing record definition for field %s.\n\n",
        expr->line_num, field_id);
    *type_return = UNKNOWN_TYPE;
    return error_count + 1;
  }
  ...
  if (array_alias != NULL)
    semcheck_set_array_info_from_alias(expr, symtab, array_alias,
                                       expr->line_num);
  if (array_alias != NULL && expr->resolved_kgpc_type == NULL) {
    KgpcType *arr_type =
        create_kgpc_type_from_type_alias(array_alias, symtab, 0);
    if (arr_type != NULL) {
      semcheck_expr_set_resolved_kgpc_type_shared(expr, arr_type);
      destroy_kgpc_type(arr_type);
    }
  ...
  if (expr->resolved_kgpc_type == NULL && field_record == NULL &&
      !field_desc->is_array && !field_desc->is_pointer &&
  ...
  if (expr->resolved_kgpc_type == NULL && field_desc->is_array) {
    ...
    if (field_desc->array_element_kgpc_type != NULL) {
      ...
      HashNode_t *elem_node = semcheck_find_preferred_type_node_with_ref(
          symtab, field_desc->array_element_type_ref,
      ...
      if (elem_node != NULL && elem_node->type != NULL) {
        ...
        elem_owned = 1;
               ...
               field_desc->array_element_record != NULL) {
      elem_type = semcheck_create_value_kgpc_type_for_record_local(
    ...
    if (elem_type != NULL) {
      ...
      KgpcType *arr_type = create_array_type(elem_type, field_desc->array_start,
                                             field_desc->array_end);
      if (arr_type != NULL)
        semcheck_expr_set_resolved_kgpc_type_shared(expr, arr_type);
      if (arr_type != NULL)
        destroy_kgpc_type(arr_type);
      /* Note: elem_type ownership was transferred to create_array_type, do NOT
       * destroy it here */
    }
  }

  if (field_type == POINTER_TYPE && !field_desc->is_array &&
      !(expr->resolved_kgpc_type != NULL &&
        (kgpc_type_is_array(expr->resolved_kgpc_type) ||
         kgpc_type_is_array_of_const(expr->resolved_kgpc_type))) &&
      !field_desc->is_pointer && field_desc->pointer_type_id == NULL &&
      field_desc->pointer_type == UNKNOWN_TYPE) {
    HashNode_t *ptr_node = NULL;
    if (field_desc->type_id != NULL || field_desc->type_ref != NULL) {
      ptr_node = semcheck_find_preferred_type_node_with_ref(
          symtab, field_desc->type_ref, field_desc->type_id);
      if (ptr_node == NULL)
        ptr_node = semcheck_find_type_node_with_kgpc_type_ref(
            symtab, field_desc->type_ref, field_desc->type_id);
    }
  ...
  if (field_type == POINTER_TYPE && !field_desc->is_array &&
    ...
    if (!typed_pointer &&
        (field_desc->type_id != NULL || field_desc->type_ref != NULL)) {
      type_node = semcheck_find_preferred_type_node_with_ref(
          symtab, field_desc->type_ref, field_desc->type_id);
      if (type_node == NULL)
    ...
    if (!typed_pointer) {
      ...
      if (expr->resolved_kgpc_type == NULL ||
          !(expr->resolved_kgpc_type->kind == TYPE_KIND_PRIMITIVE &&
            expr->resolved_kgpc_type->info.primitive_type_tag ==
                POINTER_TYPE)) {
        if (expr->resolved_kgpc_type != NULL) {
          destroy_kgpc_type(expr->resolved_kgpc_type);
          expr->resolved_kgpc_type = NULL;
      ...
      if (field_desc->pointer_type_id != NULL ||
        ...
        if (field_desc->pointer_type_id != NULL)
          ...
          pointer_subtype_rendered =
              type_ref_render_mangled(field_desc->pointer_type_ref);
        ...
        if (type_node != NULL) {
          ...
          if (alias_local != NULL && alias_local->is_pointer) {
            ...
            if (alias_local->pointer_type_ref != NULL) {
              pointer_subtype_rendered =
                  type_ref_render_mangled(alias_local->pointer_type_ref);
              pointer_subtype_id = pointer_subtype_rendered;
            } else
          ...
          if (pointer_subtype == UNKNOWN_TYPE && type_node->type != NULL)
            pointer_subtype =
                kgpc_type_get_pointer_subtype_tag(type_node->type);
        }
      }
      ...
      if (!force_untyped && pointer_subtype_id != NULL) {
        HashNode_t *sub_node = semcheck_find_type_node_with_kgpc_type_ref(
            symtab, field_desc->pointer_type_ref, pointer_subtype_id);
      ...
      if (pointer_subtype == POINTER_TYPE && field_desc->is_pointer == 0 &&
      ...
      if (force_untyped) {
        ...
        if (expr->resolved_kgpc_type == NULL) {
          ...
          if (pointer_subtype_id != NULL) {
            HashNode_t *target_node =
                semcheck_find_type_node_with_kgpc_type_ref(
                    symtab, field_desc->pointer_type_ref, pointer_subtype_id);
        ...
        if (pointer_record == NULL && pointer_subtype_id != NULL)
          pointer_record =
              semcheck_lookup_record_type(symtab, pointer_subtype_id);
        (void)pointer_record;
      }
    }
  }
  if (kgpc_getenv("KGPC_DEBUG_BUFPTR") != NULL && field_id != NULL &&
      pascal_identifier_equals(field_id, "bufptr")) {
    fprintf(
        stderr, "[KGPC_DEBUG_BUFPTR] field=%s type=%d resolved=%s kind=%d\n",
        field_id, field_type,
        expr->resolved_kgpc_type ? kgpc_type_to_string(expr->resolved_kgpc_type)
                                 : "<null>",
  ...
  /* For procedural type fields (function/procedure pointers), set the full
   * KgpcType so type compatibility can check return types and parameters. */
  if (field_type == PROCEDURE &&
      (field_desc->type_id != NULL || field_desc->type_ref != NULL)) {
    HashNode_t *proc_type_node = semcheck_find_type_node_with_kgpc_type_ref(
        symtab, field_desc->type_ref, field_desc->type_id);
  ...
  if (field_type == REAL_TYPE) {
    ...
    if (field_desc->type_id != NULL || field_desc->type_ref != NULL) {
      real_node = semcheck_find_preferred_type_node_with_ref(
          symtab, field_desc->type_ref, field_desc->type_id);
      if (real_node == NULL)
        real_node = semcheck_find_type_node_with_kgpc_type_ref(
            symtab, field_desc->type_ref, field_desc->type_id);
    ...
    if (expr->resolved_kgpc_type == NULL ||
      ...
      if (real_node != NULL && real_node->type != NULL &&
        ...
        KgpcType *real_type =
            create_primitive_type_with_size(REAL_TYPE, desired_real_size);
        if (real_type != NULL) {
  ...
  } else if (expr->resolved_kgpc_type == NULL && field_type != UNKNOWN_TYPE &&
             field_type != RECORD_TYPE && field_type != PROCEDURE) {
  ...
  if (field_type == RECORD_TYPE && expr->resolved_kgpc_type == NULL &&
      field_record != NULL) {
    KgpcType *record_kgpc =
        semcheck_create_value_kgpc_type_for_record_local(field_record);
    if (record_kgpc != NULL) {
  ...
  if (expr->resolved_kgpc_type != NULL) {
    if (kgpc_type_is_array(expr->resolved_kgpc_type) ||
      ...
      /* Preserve array/procedure KgpcType for overload resolution and
       * var/open-array behavior. */
      preserve_resolved_kgpc = 1;
    } else if (kgpc_type_equals_tag(expr->resolved_kgpc_type, field_type)) {
      /* Keep richer alias metadata (e.g. Single vs generic Real) when the type
       * tag already matches. */
      preserve_resolved_kgpc = 1;
    }
  }
  ...
  *type_return = field_type;
  ...
  if (kgpc_getenv("KGPC_DEBUG_RECORD_FIELD") != NULL && field_id != NULL &&
      (pascal_identifier_equals(field_id, "st_ctime") ||
       pascal_identifier_equals(field_id, "st_mtime") ||
       pascal_identifier_equals(field_id, "st_atime"))) {
    fprintf(stderr, "[KGPC_DEBUG_RECORD_FIELD] field=%s resolved_type=%d\n",
            field_id, field_type);
  }
  return error_count;
}
