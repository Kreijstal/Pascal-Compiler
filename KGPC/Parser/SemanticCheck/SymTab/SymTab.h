...
typedef struct ScopeNode {
  HashTable_t *table; /* Symbols declared in this scope (owned by this node,
                         freed in DestroyScope) */
  struct ScopeNode *parent; /* Walk this chain for FindIdent */
  int unit_index;           /* Which unit this scope belongs to (0 = program) */
  int codegen_unit_scope_registered; /* Codegen-only lazy population of
                                        persistent unit scopes */
...
typedef struct SymTab {
  ...
  ScopeNode *unit_scopes[SYMTAB_MAX_UNITS]; /* O(1) lookup by unit index; each
                                               scope OWNS its table */
  int current_unit_index; /* Unit being compiled (0 = main program); set by
                             SemCheck */
  int skip_unit_filter;   /* When 1, FindSymbol skips the unit_is_public filter
                             (used by codegen) */
} SymTab_t;
...
/* Searches for a symbol and sets the hash_return that contains the id and type
 * information */
...
/* Find an identifier in the current (top) scope only, bypassing unit-aware
 * resolution. Use after pushing an entry to get the just-added node reliably.
 */
 ...
 * (one level deep, which is sufficient since units are wired in topological
 * order). */
