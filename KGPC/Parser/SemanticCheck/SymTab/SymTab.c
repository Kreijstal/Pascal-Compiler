/*
    Damon Gwinn
    Creates a symbol table which is simply a stack of hash tables for
   identifiers Used to perform semantic checking on a ParseTree

    WARNING: Symbol table will NOT free given identifier strings or args when
...
ScopeNode *GetOrCreateUnitScope(SymTab_t *symtab,
                                int unit_index); /* defined later */

/* Forward declarations for tree-walking lookups (defined at end of file) */
static int FindIdent_Tree(HashNode_t **hash_return, SymTab_t *symtab,
                          const char *id);
static int FindIdentByPrefix_Tree(HashNode_t **hash_return, SymTab_t *symtab,
                                  const char *prefix);
static ListNode_t *FindAllIdents_Tree(SymTab_t *symtab, const char *id);
static ListNode_t *FindAllIdentsInNearestScope_Tree(SymTab_t *symtab,
                                                    const char *id);
static HashNode_t *FindIdentInCurrentScope_Tree(SymTab_t *symtab,
                                                const char *id);
...
int PushConstOntoScope_Typed(SymTab_t *symtab, char *id, long long value,
                             KgpcType *type) {
  assert(symtab != NULL);
  assert(id != NULL);
...
int PushVarOntoScope_Typed(SymTab_t *symtab, char *id, KgpcType *type) {
  assert(symtab != NULL);
  assert(id != NULL);
  return AddIdentToTable(SymTab_GetTargetTable(symtab), id, NULL, HASHTYPE_VAR,
                         type);
}
...
int PushArrayOntoScope_Typed(SymTab_t *symtab, char *id, KgpcType *type) {
  assert(symtab != NULL);
  assert(id != NULL);
  return AddIdentToTable(SymTab_GetTargetTable(symtab), id, NULL,
                         HASHTYPE_ARRAY, type);
}

int PushProcedureOntoScope_Typed(SymTab_t *symtab, char *id, char *mangled_id,
                                 KgpcType *type) {
  assert(symtab != NULL);
  assert(id != NULL);
  return AddIdentToTable(SymTab_GetTargetTable(symtab), id, mangled_id,
                         HASHTYPE_PROCEDURE, type);
}

int PushFunctionOntoScope_Typed(SymTab_t *symtab, char *id, char *mangled_id,
                                KgpcType *type) {
  assert(symtab != NULL);
  assert(id != NULL);
  return AddIdentToTable(SymTab_GetTargetTable(symtab), id, mangled_id,
                         HASHTYPE_FUNCTION, type);
}
...
int PushFuncRetOntoScope_Typed(SymTab_t *symtab, char *id, KgpcType *type) {
  ...
  assert(id != NULL);
  return AddIdentToTable(SymTab_GetTargetTable(symtab), id, NULL,
                         HASHTYPE_FUNCTION_RETURN, type);
}

int PushTypeOntoScope_Typed(SymTab_t *symtab, char *id, KgpcType *type) {
  assert(symtab != NULL);
  assert(id != NULL);
  return AddIdentToTable(SymTab_GetTargetTable(symtab), id, NULL, HASHTYPE_TYPE,
                         type);
}
...
static HashTable_t *get_or_create_system_unit_table(SymTab_t *symtab) {
  ...
  if (sys_idx <= 0 || sys_idx >= SYMTAB_MAX_UNITS)
    return symtab->builtin_scope
        ->table; /* last resort if System not registered */
...
int AddBuiltinType_Typed(SymTab_t *symtab, char *id, KgpcType *type) {
  ...
  assert(id != NULL);
  assert(type != NULL);
  return AddIdentToTable(get_or_create_system_unit_table(symtab), id, NULL,
                         HASHTYPE_TYPE, type);
}
...
int AddBuiltinProc_Typed(SymTab_t *symtab, char *id, KgpcType *type) {
  ...
  assert(type->kind == TYPE_KIND_PROCEDURE &&
         "Builtin proc must have procedure type");
  assert(type->info.proc_info.return_type == NULL &&
         "Procedure must not have return type");
  type->info.proc_info.owns_params = 1;
  return AddIdentToTable(get_or_create_system_unit_table(symtab), id, NULL,
                         HASHTYPE_BUILTIN_PROCEDURE, type);
}
...
int AddBuiltinFunction_Typed(SymTab_t *symtab, char *id, KgpcType *type) {
  ...
  assert(type != NULL);
  assert(type->kind == TYPE_KIND_PROCEDURE &&
         "Builtin function must have procedure type");
  assert(type->info.proc_info.return_type != NULL &&
...
int FindIdentByPrefix(HashNode_t **hash_return, SymTab_t *symtab,
                      const char *prefix) {
  assert(symtab != NULL);
  assert(prefix != NULL);
...
void PrintSymTab(SymTab_t *symtab, FILE *f, int num_indent) {
  ...
  for (i = 1; i < SYMTAB_MAX_UNITS; i++) {
    if (symtab->unit_scopes[i] != NULL &&
        symtab->unit_scopes[i]->table != NULL) {
      int j;
...
ScopeNode *GetOrCreateUnitScope(SymTab_t *symtab, int unit_index) {
  ...
  ScopeNode *scope =
      CreateScope(symtab->builtin_scope, unit_index, InitHashTable());
...
void ScopeAddDependency(ScopeNode *scope, ScopeNode *dep_scope,
  ...
  if (scope->num_deps >= scope->cap_deps) {
    ...
    ScopeNode **new_arr = (ScopeNode **)realloc(
        scope->dep_scopes, (size_t)new_cap * sizeof(ScopeNode *));
    assert(new_arr != NULL);
    scope->dep_scopes = new_arr;
    int *new_iface =
        (int *)realloc(scope->dep_is_iface, (size_t)new_cap * sizeof(int));
...
void ScopeAddDependencyTransitive(ScopeNode *scope, ScopeNode *dep_scope,
                                  int is_interface) {
...
void EnterScope(SymTab_t *symtab, int unit_index) {
  ...
  ScopeNode *new_scope =
      CreateScope(symtab->current_scope, unit_index, InitHashTable());
...
static int FindIdent_Tree(HashNode_t **hash_return, SymTab_t *symtab,
                          const char *id) {
  ScopeNode *scope = symtab->current_scope;
  int trace_flush = (id != NULL && strcasecmp(id, "Flush") == 0 &&
                     getenv("KGPC_DEBUG_FLUSH") != NULL);
  ...
  while (scope != NULL) {
    HashNode_t *node =
        FindIdentInTableCanonical(scope->table, canonical_id, hash);

    if (trace_flush)
      fprintf(stderr,
              "[FLUSH] scope=%p unit_idx=%d table=%p node=%p num_deps=%d "
              "parent=%p\n",
              (void *)scope, scope->unit_index, (void *)scope->table,
              (void *)node, scope->num_deps, (void *)scope->parent);
     ...
     * Search in reverse order: last `uses` clause entry wins (Pascal
     * semantics).
     *
      ...
      if (cur_unit > 0) {
        for (int i = scope->num_deps - 1; i >= 0; i--) {
          if (scope->dep_scopes[i]->unit_index != cur_unit)
            continue;
          node = FindIdentInTableCanonical(scope->dep_scopes[i]->table,
                                           canonical_id, hash);
          if (node != NULL) {
            *hash_return = node;
            pascal_identifier_lower_buf_free(canonical_id, stack_buf);
      ...
      for (int i = scope->num_deps - 1; i >= 0; i--) {
        ...
        if (trace_flush)
          fprintf(stderr, "[FLUSH]   dep[%d] unit_idx=%d table=%p node=%p\n", i,
                  scope->dep_scopes[i]->unit_index,
                  (void *)scope->dep_scopes[i]->table, (void *)node);
        if (node != NULL) {
          ...
          if (!symtab->skip_unit_filter && node->defined_in_unit &&
              !node->unit_is_public &&
...
static ListNode_t *FindAllIdents_Tree(SymTab_t *symtab, const char *id) {
  ...
  while (scope != NULL) {
    all = append_list(
        all, FindAllIdentsInTableCanonical(scope->table, canonical_id, hash));

    if (scope->num_deps > 0) {
      for (int i = 0; i < scope->num_deps; i++)
        all = append_list(
            all, FindAllIdentsInTableCanonical(scope->dep_scopes[i]->table,
                                               canonical_id, hash));
    }
...
static ListNode_t *FindAllIdentsInNearestScope_Tree(SymTab_t *symtab,
  ...
  while (scope != NULL) {
    ListNode_t *matches =
        FindAllIdentsInTableCanonical(scope->table, canonical_id, hash);
    if (matches != NULL) {
