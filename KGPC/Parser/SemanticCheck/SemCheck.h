...
#define KGPC_SEMCHECK_HARD_ASSERT(cond, fmt, ...)                              \
  do {                                                                         \
    if (!(cond)) {                                                             \
      fprintf(stderr, "FATAL: semcheck invariant failed at %s:%d: " fmt "\n",  \
...
/* start_semcheck variant that accepts a pre-existing symtab (from per-unit
 * semcheck). If existing_symtab is NULL, creates a new one. */
...
/* Lightweight per-unit semantic check: processes declarations only (types,
 * consts, vars, subprogram signatures) without checking subprogram bodies.
...
int semcheck_resolve_scoped_enum_literal(SymTab_t *symtab,
                                         ...
                                         long long *out_value);
int semcheck_resolve_scoped_enum_literal_ref(
    SymTab_t *symtab, const struct QualifiedIdent *type_ref,
...
KgpcType *
semcheck_get_current_subprogram_return_kgpc_type(struct SymTab *symtab,
                                                 int *owns_type);
...
static inline const char *semcheck_label_scope_name(const char *subprogram_id) {
  return (subprogram_id != NULL && subprogram_id[0] != '\0') ? subprogram_id
                                                             : "$program$";
}

static inline char *semcheck_build_label_symbol_id(const char *scope_name,
                                                   const char *label_name) {
  if (scope_name == NULL || label_name == NULL)
    return NULL;

  size_t needed =
      snprintf(NULL, 0, "__kgpc_label__%s__%s", scope_name, label_name) + 1;
