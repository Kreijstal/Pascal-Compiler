...
int semcheck_subprogram(SymTab_t *symtab, Tree_t *subprogram,
   ...
   * to the implementation. This is needed because Pascal allows defaults only
   * in the declaration, not in the implementation. */
  ...
  /* Record lexical nesting depth so codegen can reason about static links
   * accurately. Store depth as parent depth + 1 so the top-level program has
   * depth 1 and nested subprograms continue to increase. */
  ...
  if (subprogram->tree_data.subprogram_data.owner_class == NULL &&
      max_scope_lev > 0 && prev_current_subprogram != NULL &&
      prev_owner != NULL) {
    const char *inherit_owner =
        prev_current_subprogram->tree_data.subprogram_data.owner_class;
    const char *inherit_owner_full =
        prev_current_subprogram->tree_data.subprogram_data.owner_class_full;
    const char *inherit_owner_outer =
        prev_current_subprogram->tree_data.subprogram_data.owner_class_outer;
    ...
    if (inherit_owner != NULL)
      subprogram->tree_data.subprogram_data.owner_class =
          (char *)string_intern(inherit_owner);
    if (inherit_owner_full != NULL)
      subprogram->tree_data.subprogram_data.owner_class_full =
          (char *)string_intern(inherit_owner_full);
    if (inherit_owner_outer != NULL)
      subprogram->tree_data.subprogram_data.owner_class_outer =
          (char *)string_intern(inherit_owner_outer);
  }
  ...
  // --- Name Mangling Logic ---
  // Only set mangled_id if not already set by predeclare_subprogram (which
  // handles nested functions with unique parent$child naming)
  if (subprogram->tree_data.subprogram_data.mangled_id == NULL) {
    ...
    const char *explicit_name =
        subprogram->tree_data.subprogram_data.cname_override;
    if (explicit_name != NULL) {
      if (subprogram->tree_data.subprogram_data.statement_list != NULL) {
        subprogram->tree_data.subprogram_data.mangled_id =
            strdup(explicit_name);
      } else {
        char *overload_mangled = MangleFunctionName(
            subprogram->tree_data.subprogram_data.id,
            subprogram->tree_data.subprogram_data.args_var, symtab);
  ...
  /* Check if already declared (e.g., by predeclare_subprogram in two-pass
   * approach) */
  ...
  if (subprogram->tree_data.subprogram_data.cached_predecl_node != NULL) {
    existing_decl =
        (HashNode_t *)subprogram->tree_data.subprogram_data.cached_predecl_node;
    already_declared = 1;
  }

  if (!already_declared) {
    /* For overloaded functions, find the correct overload by matching mangled
     * name */
    if (subprogram->tree_data.subprogram_data.mangled_id != NULL) {
      ...
      if (kgpc_getenv("KGPC_DEBUG_OVERLOAD_MATCH") != NULL) {
        ...
        fprintf(stderr, "[KGPC] Matching impl %s mangled=%s\n",
                ...
                subprogram->tree_data.subprogram_data.mangled_id);
        for (ListNode_t *debug_cur = all_matches; debug_cur != NULL;
          ...
          fprintf(stderr, "  candidate: id=%s mangled=%s\n",
                  ...
                  debug_cand && debug_cand->mangled_id ? debug_cand->mangled_id
                                                       : "<null>");
        }
        if (all_matches != NULL)
      ...
      existing_decl = lookup.exact_match != NULL ? lookup.exact_match
                                                 : lookup.first_mangled_match;
    ...
    if (!already_declared &&
        subprogram->tree_data.subprogram_data.mangled_id == NULL)
      already_declared =
          (FindSymbol(&existing_decl, symtab, id_to_use_for_lookup) != 0);
  }
  ...
  if (sub_type == TREE_SUBPROGRAM_PROC) {
    ...
    if (already_declared && existing_decl != NULL &&
      ...
      /* Copy default parameter values from forward declaration to
       * implementation */
      ...
      proc_type =
          create_procedure_type(subprogram->tree_data.subprogram_data.args_var,
    ...
    /* ARCHITECTURAL FIX: Resolve array bounds in parameter types now that
      ...
      while (param != NULL) {
        if (param->type == LIST_TREE && param->cur != NULL) {
          ...
          if (param_tree->type == TREE_ARR_DECL) {
            /* If element type is a named type, look it up to get proper
             * KgpcType */
            if (param_tree->tree_data.arr_decl_data.type_id != NULL) {
              HashNode_t *elem_type_node = NULL;
              if (FindSymbol(&elem_type_node, symtab,
                             param_tree->tree_data.arr_decl_data.type_id) !=
                      0 &&
    ...
    if (!already_declared) {
      func_return = PushProcedureOntoScope_Typed(
          symtab, id_to_use_for_lookup,
          subprogram->tree_data.subprogram_data.mangled_id, proc_type);
      semcheck_update_symbol_alias(
          symtab, id_to_use_for_lookup,
          subprogram->tree_data.subprogram_data.mangled_id);
      if (subprogram->tree_data.subprogram_data.defined_in_unit) {
        ...
        HashTable_t *target_table = SymTab_GetTargetTable(symtab);
        HashNode_t *pushed =
            FindIdentInTable(target_table, id_to_use_for_lookup);
        if (pushed != NULL) {
      ...
      register_nested_type_short_alias(
          symtab, subprogram->tree_data.subprogram_data.owner_class_outer,
          subprogram->tree_data.subprogram_data.owner_class,
      ...
      /* If we created a new type but it was already declared (e.g. existing had
       * no type), update it */
      if (created_new_type && existing_decl != NULL &&
          existing_decl->type == NULL) {
        existing_decl->type = proc_type;
      } else if (created_new_type) {
        /* We created a type but didn't use it (shouldn't happen if logic is
         * correct, but for safety) */
    ...
    if (existing_decl != NULL &&
      ...
      existing_decl->internproc_id =
          strdup(subprogram->tree_data.subprogram_data.internproc_id);
    }
    if (existing_decl != NULL &&
        subprogram->tree_data.subprogram_data.internconst_id != NULL) {
      if (existing_decl->internconst_id != NULL)
        free(existing_decl->internconst_id);
      existing_decl->internconst_id =
          strdup(subprogram->tree_data.subprogram_data.internconst_id);
    }

    EnterScope(symtab, subprogram->tree_data.subprogram_data.source_unit_index);
  ...
  } else // Function
  {
    ...
    if (return_type_needs_rebuild) {
      ...
      KgpcType *rebuilt =
          build_function_return_type(subprogram, symtab, &return_val, 0);
      ...
      semcheck_debug_error_step("build_return_type", subprogram, before_ret,
                                return_val);
#ifdef DEBUG
      if (return_val > 0)
        fprintf(stderr,
                "DEBUG: semcheck_subprogram %s error after "
                "build_function_return_type: %d\n",
    ...
    if (!already_declared) {
#ifdef DEBUG
      fprintf(stderr,
              "DEBUG: semcheck_subprogram %s NOT already declared in Pass 2!\n",
              subprogram->tree_data.subprogram_data.id);
#endif
      func_type = create_procedure_type(
          subprogram->tree_data.subprogram_data.args_var, return_kgpc_type);
      if (func_type != NULL) {
      ...
      func_return = PushFunctionOntoScope_Typed(
          symtab, id_to_use_for_lookup,
          subprogram->tree_data.subprogram_data.mangled_id, func_type);
      semcheck_update_symbol_alias(
          symtab, id_to_use_for_lookup,
          subprogram->tree_data.subprogram_data.mangled_id);
      if (subprogram->tree_data.subprogram_data.defined_in_unit) {
      ...
      register_nested_type_short_alias(
          symtab, subprogram->tree_data.subprogram_data.owner_class_full,
          subprogram->tree_data.subprogram_data.owner_class,
          subprogram->tree_data.subprogram_data.method_name,
    ...
    /* Also add "Result" as an alias for the return variable for Pascal
     * compatibility */
    ...
    result_check =
        (cur_hash != NULL) ? FindIdentInTable(cur_hash, "Result") : NULL;
    if (result_check == NULL) {
      ...
      if (kgpc_getenv("KGPC_DEBUG_RESULT") != NULL) {
        fprintf(stderr, "[KGPC] add Result alias for %s type=%s\n",
                subprogram->tree_data.subprogram_data.id
                    ? subprogram->tree_data.subprogram_data.id
                    : "<anon>",
    ...
    /* For operator declarations with named result variables (e.g., "operator
     * :=(src) dest: variant"), push the named result variable as an additional
     * alias for the return variable. */
    if (subprogram->tree_data.subprogram_data.result_var_name != NULL) {
      PushFuncRetOntoScope_Typed(
          symtab, subprogram->tree_data.subprogram_data.result_var_name,
          return_kgpc_type);
    }
    ...
    /* Note: We don't check for "result" anymore since it conflicts with
     * built-in Result alias */
  ...
  if (func_return > 0) {
    ...
    if (FindSymbol(&existing_decl, symtab, id_to_use_for_lookup) != 0 &&
        existing_decl != NULL && existing_decl->mangled_id != NULL &&
        subprogram->tree_data.subprogram_data.mangled_id != NULL &&
        strcmp(existing_decl->mangled_id,
        ...
        if (arg_tree->type == TREE_VAR_DECL) {
          ...
          if (ids != NULL && ids->cur != NULL)
            fprintf(stderr, "  - %s (type_id=%s)\n", (char *)ids->cur,
                    arg_tree->tree_data.var_decl_data.type_id
                        ? arg_tree->tree_data.var_decl_data.type_id
                        : "<null>");
        }
      }
  ...
  /* Register generic type parameters (e.g., T, U) as opaque types in the
   * function scope. This allows parameters of type T and expressions using T to
   * pass semantic checking. */
  if (subprogram->tree_data.subprogram_data.num_generic_type_params > 0) {
    for (int i = 0;
         ...
         i++) {
      const char *tparam =
          subprogram->tree_data.subprogram_data.generic_type_params[i];
      assert(tparam != NULL);
    ...
    /* Some parsed method implementations (notably generic class methods in FPC
     * RTL) may omit explicit generic_type_params metadata even though their
     * signatures contain placeholders like T/U. Infer only single-letter
     * uppercase params. */
  ...
  }
  {
      ...
      if (kgpc_getenv("KGPC_DEBUG_TSIZE") != NULL) {
        ...
        while (a != NULL) {
          if (a->type == LIST_TREE && a->cur != NULL) {
            ...
            if (ad->type == TREE_VAR_DECL &&
                ad->tree_data.var_decl_data.type_id != NULL &&
                pascal_identifier_equals(ad->tree_data.var_decl_data.type_id,
                                         "TSize")) {
              fprintf(stderr,
                      "[SUBPROGRAM_TSIZE] func=%s source_unit_idx=%d "
                      "param_type_id=%s\n",
                      subprogram->tree_data.subprogram_data.id
                          ? subprogram->tree_data.subprogram_data.id
                          : "<null>",
    ...
    return_val +=
        semcheck_decls(symtab, subprogram->tree_data.subprogram_data.args_var);
    g_semcheck_imported_decl_unit_index = saved_imported_unit;
    semcheck_debug_error_step("args_decls", subprogram, before_args,
                              return_val);
  }
#ifdef DEBUG
  if (return_val > 0)
    fprintf(stderr, "DEBUG: semcheck_subprogram %s error after args: %d\n",
            subprogram->tree_data.subprogram_data.id, return_val);
#endif
        ...
        while (ids != NULL) {
          ...
          if (FindSymbol(&existing, symtab, ids->cur) == 0) {
            ...
            if (param_tree->type == TREE_VAR_DECL) {
              ...
              if (param_type == NULL &&
                  param_tree->tree_data.var_decl_data.type_id != NULL) {
                ...
                if (builtin_tag == STRING_TYPE &&
                    param_tree->tree_data.var_decl_data.type ==
                        SHORTSTRING_TYPE) {
              ...
              if (param_type != NULL &&
                  param_type ==
                      param_tree->tree_data.var_decl_data.cached_kgpc_type)
            ...
            if (FindSymbol(&existing, symtab, ids->cur) != 0 &&
              ...
              if (param_tree->type == TREE_VAR_DECL) {
                ...
                is_untyped =
                    param_tree->tree_data.var_decl_data.is_untyped_param;
              }
  ...
  if (!subprogram->tree_data.subprogram_data.is_static_method) {
    ...
    if (trace_nonlocal != NULL &&
      ...
      fprintf(stderr,
              "[KGPC_TRACE_NONLOCAL] self-inject subprogram=%s owner_ctx=%s "
              "owner_class=%s nesting=%d\n",
              subprogram->tree_data.subprogram_data.id,
    ...
    if (owner_record != NULL) {
      ...
      if (owner_record->is_type_helper &&
          owner_record->helper_base_type_id != NULL) {
        self_is_var_param = semcheck_helper_self_is_var(
            symtab, owner_record->helper_base_type_id);
      ...
      if (self_type != NULL) {
        if (FindSymbol(&self_node, symtab, "Self") == 0) {
          ...
          if (trace_nonlocal != NULL) {
            fprintf(stderr,
                    "[KGPC_TRACE_NONLOCAL] self-inject pushed Self for "
                    "subprogram=%s type=%s is_var=%d\n",
                    subprogram->tree_data.subprogram_data.id != NULL
                        ? subprogram->tree_data.subprogram_data.id
                        : "<null>",
                    kgpc_type_to_string(self_type), self_is_var_param);
          }
        } else if (self_node->type == NULL ||
                   !kgpc_type_equals(self_node->type, self_type)) {
        ...
        if (trace_nonlocal != NULL) {
          fprintf(stderr,
                  "[KGPC_TRACE_NONLOCAL] self-inject final Self node=%p "
                  "type=%s is_var=%d\n",
                  (void *)self_node,
...

  {
    ...
    return_val += semcheck_register_declared_labels(
        symtab, subprogram->tree_data.subprogram_data.label_declarations,
        subprogram->tree_data.subprogram_data.id);
    return_val += predeclare_enum_literals(
        symtab, subprogram->tree_data.subprogram_data.type_declarations);
    /* Pre-declare types so they're available for const expressions like
     * High(MyType) */
    return_val += predeclare_types(
        symtab, subprogram->tree_data.subprogram_data.type_declarations);
    return_val += semcheck_const_decls(
        symtab, subprogram->tree_data.subprogram_data.const_declarations);
    return_val += semcheck_type_decls(
        symtab, subprogram->tree_data.subprogram_data.type_declarations);
    return_val += semcheck_decls(
        symtab, subprogram->tree_data.subprogram_data.declarations);
    semcheck_debug_error_step("local_decls", subprogram, before_local,
                              return_val);
  }
#ifdef DEBUG
  if (return_val > 0)
    fprintf(stderr, "DEBUG: semcheck_subprogram %s error after decls: %d\n",
            subprogram->tree_data.subprogram_data.id, return_val);
#endif

  {
    ...
    return_val += semcheck_subprograms(
        symtab, subprogram->tree_data.subprogram_data.subprograms,
        new_max_scope, subprogram);
    semcheck_debug_error_step("nested_subprograms", subprogram, before_nested,
                              return_val);
  }
  ...
  if (subprogram->tree_data.subprogram_data.defined_in_unit) {
    ...
    if (subprogram->tree_data.subprogram_data.source_unit_index > 0) {
      const char *uname = unit_registry_get(
          subprogram->tree_data.subprogram_data.source_unit_index);
      g_semcheck_error_unit_name = uname ? uname : "<unit>";
    }
  }

  /* Functions cannot have side effects, so need to call a special function in
   * that case */
  if (sub_type == TREE_SUBPROGRAM_PROC) {
    {
      int before_body = return_val;
      return_val += semcheck_stmt(symtab, body, new_max_scope);
      semcheck_debug_error_step("proc_body", subprogram, before_body,
                                return_val);
    }
  } else {
    assert(FindSymbol(&hash_return, symtab,
                      subprogram->tree_data.subprogram_data.id) != 0);
    ...
    int func_stmt_ret = 0;
    {
      ...
      semcheck_debug_error_step("func_body", subprogram, before_body,
                                return_val);
    }
#ifdef DEBUG
    if (func_stmt_ret > 0)
      fprintf(
          stderr,
          "DEBUG: semcheck_subprogram %s error after semcheck_func_stmt: %d\n",
    ...
    int return_was_assigned =
        is_constructor ? 1 : (hash_return->mutated != NO_MUTATE);
    ...
    /* Methods use mangled identifiers like Class__Func; allow assignments to
     * the unmangled method name to satisfy the return check. */
    if (!return_was_assigned &&
  ...
  if (subprogram->tree_data.subprogram_data.id != NULL) {
    ListNode_t *defs =
        FindAllIdents(symtab, subprogram->tree_data.subprogram_data.id);
    ListNode_t *iter = defs;
    while (iter != NULL) {
      if (iter->cur != NULL) {
        HashNode_t *node = (HashNode_t *)iter->cur;
        if (node != NULL && node->mangled_id != NULL &&
            subprogram->tree_data.subprogram_data.mangled_id != NULL &&
            strcmp(node->mangled_id,
                   subprogram->tree_data.subprogram_data.mangled_id) == 0) {
          node->requires_static_link =
              subprogram->tree_data.subprogram_data.requires_static_link ? 1
                                                                         : 0;
          node->has_nested_requiring_link =
              subprogram->tree_data.subprogram_data.has_nested_requiring_link
                  ? 1
                  : 0;
...
#ifdef DEBUG
  if (return_val > 0)
    fprintf(stderr, "DEBUG: semcheck_subprogram %s returning at end: %d\n",
            subprogram->tree_data.subprogram_data.id, return_val);
#endif
  if (owner_copy != NULL)
    free(owner_copy);
 ...
 */
int predeclare_subprogram(SymTab_t *symtab, Tree_t *subprogram,
  ...
  if (kgpc_getenv("KGPC_DEBUG_PREDECLARE_PROC") != NULL) {
    fprintf(stderr, "[PREDECLARE_PROC] %s (line %d) parent=%s\n",
            subprogram->tree_data.subprogram_data.id, subprogram->line_num,
            parent_subprogram != NULL
                ? parent_subprogram->tree_data.subprogram_data.id
                : "(null)");
  }
  ...
  if (keep_existing_mangled) {
    ...
    subprogram->tree_data.subprogram_data.mangled_id =
        strdup(subprogram->tree_data.subprogram_data.id);
  } else {
    // Pass the symbol table to the mangler
    ...
    // For nested functions, prepend the parent's mangled_id to make the name
    // unique
    if (parent_subprogram != NULL &&
        parent_subprogram->tree_data.subprogram_data.mangled_id != NULL) {
      const char *parent_mangled =
          parent_subprogram->tree_data.subprogram_data.mangled_id;
      size_t len = strlen(parent_mangled) + 2 + strlen(base_mangled) +
                   1; // parent$nested\0
  ...
  if (lookup.tree_match != NULL) {
    ...
    subprogram->tree_data.subprogram_data.cached_predecl_node =
        (struct HashNode *)lookup.tree_match;
  ...
  if (lookup.exact_match != NULL) {
    ...
    subprogram->tree_data.subprogram_data.cached_predecl_node =
        (struct HashNode *)lookup.exact_match;
    semcheck_restore_scope(symtab, saved_scope_for_unit);
    return 0; /* Already declared - skip to avoid duplicates */
  }
  ...
  if (lookup.body_pair_match != NULL) {
    ...
    subprogram->tree_data.subprogram_data.cached_predecl_node =
        (struct HashNode *)lookup.body_pair_match;
    semcheck_restore_scope(symtab, saved_scope_for_unit);
  ...
  if (lookup.signature_match_diff_mangle != NULL &&
    ...
    Tree_t *def = NULL;
    if (existing->type != NULL && existing->type->kind == TYPE_KIND_PROCEDURE)
      def = existing->type->info.proc_info.definition;
    int def_is_forward =
        (def != NULL && def->tree_data.subprogram_data.statement_list == NULL);
    if (def_is_forward) {
  ...
  if (sub_type == TREE_SUBPROGRAM_PROC) {
    ...
    /* For nested type methods, also register the short alias during
     * predeclaration. */
    if (func_return == 0)
      register_nested_type_short_alias(
          symtab, subprogram->tree_data.subprogram_data.owner_class_outer,
    ...
    if (func_return == 0) {
      ...
      if (node != NULL) {
        ...
        if (subprogram->tree_data.subprogram_data.internproc_id != NULL) {
          if (node->internproc_id != NULL)
            free(node->internproc_id);
          node->internproc_id =
              strdup(subprogram->tree_data.subprogram_data.internproc_id);
        }
        if (subprogram->tree_data.subprogram_data.internconst_id != NULL) {
          if (node->internconst_id != NULL)
            free(node->internconst_id);
          node->internconst_id =
              strdup(subprogram->tree_data.subprogram_data.internconst_id);
        }
    ...
    if (func_return == 0) {
      SubprogramPredeclLookup pushed_lookup =
          semcheck_lookup_subprogram_predecl(
              symtab, subprogram, id_to_use_for_lookup,
              subprogram->tree_data.subprogram_data.mangled_id);
      HashNode_t *cached = pushed_lookup.exact_match != NULL
                               ? pushed_lookup.exact_match
                               : pushed_lookup.first_mangled_match;
      subprogram->tree_data.subprogram_data.cached_predecl_node =
          (struct HashNode *)cached;
    }
    /* Release creator's reference - hash table retained its own */
    destroy_kgpc_type(proc_type);
  } else // Function
  {
    KgpcType *return_kgpc_type =
        build_function_return_type(subprogram, symtab, &return_val, 1);
#ifdef DEBUG
    if (return_val > 0)
      fprintf(stderr,
              "DEBUG: predeclare_subprogram %s error after "
              "build_function_return_type: %d\n",
              subprogram->tree_data.subprogram_data.id, return_val);
#endif
...
#ifdef DEBUG
  if (return_val > 0)
    fprintf(stderr, "DEBUG: predeclare_subprogram %s returning error: %d\n",
            subprogram->tree_data.subprogram_data.id, return_val);
#endif
 ...
 * Safe to call multiple times thanks to duplicate checks in
 * predeclare_subprogram. */
int predeclare_subprograms(SymTab_t *symtab, ListNode_t *subprograms,
                           int max_scope_lev, Tree_t *parent_subprogram) {
  int return_val = 0;
  ...
  while (cur != NULL) {
    ...
    return_val +=
        predeclare_subprogram(symtab, child, max_scope_lev, parent_subprogram);
    /* Note: We intentionally do NOT predeclare nested subprograms here
     * globally. Nested subprograms are predeclared within their parent's scope
     * when semcheck_subprogram calls semcheck_subprograms (which has its own
     * Pass 1). This ensures nested functions with the same name in different
     * parents don't shadow each other. */
    cur = cur->next;
  }
  return return_val;
}
...
int semcheck_subprograms(SymTab_t *symtab, ListNode_t *subprograms,
...
#ifdef DEBUG
  if (return_val > 0)
    fprintf(stderr, "DEBUG: semcheck_subprograms error after Pass 1: %d\n",
            return_val);
#endif

  /* Pass 1.5: Resolve parameter types for all predeclared subprograms.
   * This must run AFTER type declarations are fully processed
   * (semcheck_type_decls) so that types like PAnsiChar, TypedFile, etc. are
   * available. predeclare_subprogram can't do this because it runs before
   * semcheck_type_decls. */
  cur = subprograms;
  while (cur != NULL) {
    assert(cur->cur != NULL);
  ...
  while (cur != NULL) {
    ...
    /* If child needs a static link, mark parent as having nested children that
     * need links. This is used by codegen to know when to PASS a static link
     * when calling nested functions. We do NOT propagate requires_static_link
     * to parent - the parent only needs to RECEIVE a static link if it's nested
     * itself or accesses outer scope variables. */
    if (parent_subprogram != NULL && child != NULL &&
        child->tree_data.subprogram_data.requires_static_link) {
      parent_subprogram->tree_data.subprogram_data.has_nested_requiring_link =
          1;
    }
    cur = cur->next;
  }
