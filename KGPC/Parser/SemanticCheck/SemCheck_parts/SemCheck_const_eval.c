...
int semcheck_subprograms(SymTab_t *symtab, ListNode_t *subprograms,
                         int max_scope_lev, Tree_t *parent_subprogram);

/* Resolve the return type for a function declaration once so callers share the
 * same KgpcType. */
HashNode_t *semcheck_find_type_node_with_kgpc_type_ref(SymTab_t *symtab,
  ...
  HashNode_t *preferred =
      semcheck_find_preferred_type_node_with_ref(symtab, type_ref, type_id);
...
HashNode_t *semcheck_find_type_node_with_kgpc_type(SymTab_t *symtab,
                                                   const char *type_id) {
...
static int
statement_list_uses_shortstring_result_slot(const ListNode_t *list,
                                            const Tree_t *subprogram);
...
static int statement_uses_shortstring_result_slot(const struct Statement *stmt,
  ...
  case STMT_VAR_ASSIGN:
    return is_result_length_byte_access(stmt->stmt_data.var_assign_data.var,
                                        subprogram);
...
KgpcType *build_function_return_type(Tree_t *subprogram, SymTab_t *symtab,
  ...
  if (effective_return_type == STRING_TYPE &&
      ...
      pascal_identifier_equals(
          subprogram->tree_data.subprogram_data.return_type_id, "string") &&
      statement_uses_shortstring_result_slot(
  ...
  if (subprogram->tree_data.subprogram_data.return_type_id != NULL) {
    ...
    type_node = semcheck_find_type_node_with_kgpc_type_ref(
        symtab, subprogram->tree_data.subprogram_data.return_type_ref,
        subprogram->tree_data.subprogram_data.return_type_id);
    if (type_node == NULL) {
      const char *owner_full =
          subprogram->tree_data.subprogram_data.owner_class_full;
      ...
      type_node = semcheck_find_type_node_in_owner_chain(
          symtab, subprogram->tree_data.subprogram_data.return_type_id,
    ...
    if (type_node == NULL) {
      ...
      const char *type_base =
          type_ref != NULL ? type_ref_base_name(type_ref) : type_id;
      ...
      if (builtin_type == UNKNOWN_TYPE) {
        ...
        for (int i = 0;
             i < subprogram->tree_data.subprogram_data.num_generic_type_params;
             i++) {
          if (type_base != NULL &&
              strcasecmp(type_base, subprogram->tree_data.subprogram_data
                                        .generic_type_params[i]) == 0) {
            is_generic_param = 1;
            break;
          }
        }
  ...
  KgpcType *result = kgpc_type_build_function_return(
      subprogram->tree_data.subprogram_data.inline_return_type, type_node,
      effective_return_type, symtab);
  ...
  if (effective_return_type == SHORTSTRING_TYPE && result != NULL &&
...
static HashNode_t *
find_type_node_with_range_metadata(SymTab_t *symtab, const char *base_name,
                                   const struct TypeAlias *exclude_alias);
...
/* Helper to check if an expression contains a real number literal or real
 * constant */
int expression_contains_real_literal_impl(SymTab_t *symtab,
  ...
  if (expr->type == EXPR_VAR_ID && symtab != NULL) {
    ...
    if (FindSymbol(&node, symtab, expr->expr_data.id) != 0 && node != NULL &&
        node->hash_type == HASHTYPE_CONST && node->type != NULL &&
        kgpc_type_is_real(node->type)) {
      return 1;
    }
    return 0;
  }
  ...
  if (expr->type == EXPR_SIGN_TERM) {
    return expression_contains_real_literal_impl(symtab,
                                                 expr->expr_data.sign_term);
  }
  ...
  if (expr->type == EXPR_TYPECAST) {
    ...
    if (is_real_family_type(target_type) ||
        is_real_type_name(symtab, target_id))
      return 1;
    return 0;
  }
  ...
  if (expr->type == EXPR_ADDOP) {
    return expression_contains_real_literal_impl(
               symtab, expr->expr_data.addop_data.left_expr) ||
           expression_contains_real_literal_impl(
               symtab, expr->expr_data.addop_data.right_term);
  }

  if (expr->type == EXPR_MULOP) {
    /* Pascal '/' (SLASH) always produces a real result, even with integer
     * operands */
    if (expr->expr_data.mulop_data.mulop_type == SLASH)
      return 1;
    return expression_contains_real_literal_impl(
               symtab, expr->expr_data.mulop_data.left_term) ||
           expression_contains_real_literal_impl(
               symtab, expr->expr_data.mulop_data.right_factor);
  }
...
int expression_is_string(SymTab_t *symtab, struct Expression *expr) {
  ...
  if (expr->type == EXPR_RECORD_ACCESS && symtab != NULL &&
    ...
    if (FindSymbol(&node, symtab,
                   expr->expr_data.record_access_data.field_id) != 0 &&
        node != NULL) {
      if ((node->hash_type == HASHTYPE_CONST || node->is_typed_const) &&
          node->const_string_value != NULL)
        return 1;
...
int expression_is_set_const_expr(SymTab_t *symtab, struct Expression *expr) {
  ...
  if (expr->type == EXPR_ADDOP) {
    ...
    if ((op == PLUS || op == MINUS) &&
        expression_is_set_const_expr(symtab,
...
int evaluate_set_const_bytes(SymTab_t *symtab, struct Expression *expr,
                             ...
                             size_t *out_size, long long *out_mask,
                             int *is_char_set) {
  ...
  if (expr->type == EXPR_ADDOP || expr->type == EXPR_MULOP) {
    ...
    int left_char = 0;
    int right_char = 0;
    int op = (expr->type == EXPR_ADDOP) ? expr->expr_data.addop_data.addop_type
    ...
    if (evaluate_set_const_bytes(symtab, left_expr, left_bytes,
                                 sizeof(left_bytes), &left_size, NULL,
                                 &left_char) != 0)
      return 1;
    if (evaluate_set_const_bytes(symtab, right_expr, right_bytes,
                                 sizeof(right_bytes), &right_size, NULL,
                                 &right_char) != 0)
      return 1;
    ...
    if (element->cur != NULL) {
      ...
      if (set_element->upper != NULL) {
        if (evaluate_const_expr(symtab, set_element->upper, &upper) != 0) {
          if (!g_semcheck_decl_only_mode)
            fprintf(stderr, "Error: set element upper bound is not a constant "
                            "expression.\n");
          return 1;
        }
      } else {
      ...
      if (upper < 0 || upper > 255 || lower < 0) {
        if (!g_semcheck_decl_only_mode)
          fprintf(
              stderr,
              "Error: set literal value %lld out of supported range 0..255.\n",
              upper);
        return 1;
      }
...
int evaluate_string_const_expr(SymTab_t *symtab, struct Expression *expr,
                               char **out_value) {
  ...
  case EXPR_RECORD_ACCESS: {
    ...
    HashNode_t *node = NULL;
    if (fid != NULL && FindSymbol(&node, symtab, fid) != 0 && node != NULL &&
        (node->hash_type == HASHTYPE_CONST || node->is_typed_const)) {
      if (node->const_string_value != NULL) {
        *out_value = strdup(node->const_string_value);
        return (*out_value == NULL) ? 1 : 0;
  ...
  case EXPR_ADDOP: {
    if (expr->expr_data.addop_data.addop_type != PLUS) {
      fprintf(
          stderr,
          "Error: only + operator is supported for string concatenation.\n");
      return 1;
    }

    char *left = NULL, *right = NULL;
    if (evaluate_string_const_expr(symtab, expr->expr_data.addop_data.left_expr,
                                   &left) != 0)
      return 1;
    if (evaluate_string_const_expr(
            symtab, expr->expr_data.addop_data.right_term, &right) != 0) {
      free(left);
      return 1;
    }
...
int semcheck_const_expr_char_cast_value(struct Expression *expr,
                                        long long *out_value,
                                        int *out_char_size) {
...
static int semcheck_subprogram_node_source_unit_index(HashNode_t *candidate) {
  ...
  if (candidate->type != NULL && candidate->type->kind == TYPE_KIND_PROCEDURE &&
    ...
    int def_unit_idx = candidate->type->info.proc_info.definition->tree_data
...
semcheck_lookup_subprogram_predecl(SymTab_t *symtab, Tree_t *subprogram,
  ...
  while (cur != NULL) {
    ...
    candidate_source_unit =
        semcheck_subprogram_node_source_unit_index(candidate);
    ...
    if (candidate->mangled_id != NULL && mangled_id != NULL &&
...
void semcheck_refresh_predecl_match(HashNode_t *node, Tree_t *subprogram) {
  ...
  if (subprogram->tree_data.subprogram_data.defined_in_unit) {
    ...
    if (node_unit == sub_unit ||
        (node_unit == 0 && !node->defined_in_unit && sub_unit == 0))
 ...
 * printed to stderr). */
static struct Expression *extract_single_const_arg(ListNode_t *args,
                                                   const char *func_name) {
  assert(func_name != NULL);
  if (args == NULL || args->next != NULL) {
    fprintf(stderr,
            "Error: %s in const expression requires exactly one argument.\n",
            func_name);
    return NULL;
  }
...
static int get_builtin_type_bounds(const char *base_name, long long *low_out,
                                   long long *high_out) {
...
static int const_fold_real_expr_mode(SymTab_t *symtab, struct Expression *expr,
  ...
  case EXPR_TYPECAST: {
    ...
    if (is_real_family_type(target_type) ||
        is_real_type_name(symtab, target_id)) {
      return const_fold_real_expr_mode(symtab,
  ...
  case EXPR_ADDOP: {
    double left, right;
    if (const_fold_real_expr_mode(symtab, expr->expr_data.addop_data.left_expr,
                                  &left, emit_diagnostics) != 0)
      return 1;
    if (const_fold_real_expr_mode(symtab, expr->expr_data.addop_data.right_term,
                                  &right, emit_diagnostics) != 0)
      return 1;
  ...
  case EXPR_MULOP: {
    double left, right;
    if (const_fold_real_expr_mode(symtab, expr->expr_data.mulop_data.left_term,
                                  &left, emit_diagnostics) != 0)
      return 1;
    if (const_fold_real_expr_mode(symtab,
                                  expr->expr_data.mulop_data.right_factor,
                                  &right, emit_diagnostics) != 0)
      return 1;
  ...
  case EXPR_FUNCTION_CALL: {
    ...
    if (id != NULL && is_real_type_name(symtab, id)) {
      ...
      if (arg == NULL)
        return 1;
      return const_fold_real_expr_mode(symtab, arg, out_value,
                                       emit_diagnostics);
    }
    if (id != NULL && pascal_identifier_equals(id, "Ln")) {
      ...
      if (value <= 0.0) {
        if (emit_diagnostics)
          fprintf(stderr,
                  "Error: Ln argument must be > 0 in const expression.\n");
        return 1;
      }
      *out_value = log(value);
      return 0;
 ...
 */
/* Track recursion depth to prevent stack overflow from circular type
 * definitions. Note: This compiler is single-threaded, so static variable is
 * safe. */
...
static const char *resolve_type_to_base_name(SymTab_t *symtab,
  ...
  if (resolve_type_depth >= MAX_RESOLVE_TYPE_DEPTH) {
    KGPC_SEMCHECK_HARD_ASSERT(0,
                              "type resolution depth limit reached for '%s' "
                              "(possible circular definition)",
                              input_name);
    return NULL;
  }
  ...
  if (symtab != NULL) {
    ...
    if (type_id != NULL && type_id->count > 1) {
      const char *unit_name =
          type_id->segments != NULL ? type_id->segments[0] : NULL;
      const char *base_name = qualified_ident_last(type_id);
      if (unit_name != NULL && base_name != NULL &&
          semcheck_is_unit_name(unit_name)) {
        type_node =
            semcheck_find_type_node_with_unit_flag(symtab, base_name, 1);
    ...
    /* Resolve with preferred-type lookup (unit/scope-aware + nested suffix
     * fallback). */
    if (type_node == NULL)
    ...
    if (kgpc_getenv("KGPC_DEBUG_RESOLVE_TYPE") != NULL) {
      fprintf(stderr, "[resolve_type] '%s' initial lookup node=%p (%s)\n",
              input_name, (void *)type_node,
              lookup_name != NULL ? lookup_name : "<null>");
    }
    ...
    if (type_node == NULL) {
      const char *last =
          (type_id != NULL) ? qualified_ident_last(type_id) : NULL;
      if (last != NULL) {
        lookup_name = last;
        type_node = semcheck_find_preferred_type_node(symtab, lookup_name);
        if (kgpc_getenv("KGPC_DEBUG_RESOLVE_TYPE") != NULL) {
    ...
    if (type_node != NULL) {
      ...
      if (type_node->hash_type == HASHTYPE_TYPE && type_node->type != NULL) {
        ...
        /* CRITICAL: Check TypeAlias FIRST for target_type_id before checking
         * primitive tag. This preserves the original type name for small
         * integer types (Byte, Word, etc.) which are all mapped to LONGINT_TYPE
         * by the predeclare_types function but need to retain their original
         * type semantics for SizeOf/High/Low operations. */
        ...
        if (alias != NULL && alias->target_type_id != NULL && type_id != NULL) {
          ...
          if (qualified != NULL &&
            ...
            HashNode_t *alt =
                semcheck_find_type_excluding_alias(symtab, qualified, alias);
        ...
        if (kgpc_getenv("KGPC_DEBUG_RESOLVE_TYPE") != NULL) {
          ...
          if (alias != NULL) {
            fprintf(
                stderr,
                "[resolve_type] '%s' alias target_type_id='%s' base_type=%d\n",
                input_name,
        ...
        if (alias != NULL) {
          ...
          if (alias->target_type_ref != NULL &&
              alias->target_type_ref->name != NULL && type_id != NULL &&
              qualified_ident_equals_ci(alias->target_type_ref->name,
                                        type_id)) {
            allow_recursion = 0;
          }
          ...
          if (alias->target_type_id != NULL && type_name != NULL &&
          ...
          if (!allow_recursion && type_id != NULL) {
            const char *base_name = qualified_ident_last(type_id);
            HashNode_t *alt =
                find_type_node_with_range_metadata(symtab, base_name, alias);
            if (alt != NULL && alt->type != NULL) {
              kgpc_type = alt->type;
              alias = kgpc_type_get_type_alias(kgpc_type);
            }
           ...
           */
          if (allow_recursion && (alias->target_type_ref != NULL ||
                                  alias->target_type_id != NULL)) {
            resolve_type_depth++;
            const char *result = resolve_type_to_base_name(
                symtab,
                alias->target_type_ref != NULL ? alias->target_type_ref->name
                                               : NULL,
...
static int resolve_range_bounds_for_type_ref(SymTab_t *symtab,
                                             const QualifiedIdent *type_ref,
                                             long long *out_low,
                                             long long *out_high);

static int resolve_range_bounds_for_type(SymTab_t *symtab,
                                         const char *type_name,
                                         long long *out_low,
                                         long long *out_high) {
  if (symtab == NULL || type_name == NULL || out_low == NULL ||
      out_high == NULL)
    return 0;
  ...
  if (alias != NULL) {
    ...
    if (alias->target_type_id != NULL &&
        !pascal_identifier_equals(alias->target_type_id, type_name) &&
        resolve_range_bounds_for_type(symtab, alias->target_type_id, out_low,
                                      out_high)) {
      return 1;
    }
  }
...
int semcheck_resolve_range_bounds_for_type(SymTab_t *symtab,
                                           const char *type_name,
                                           long long *out_low,
                                           long long *out_high) {
...
HashNode_t *
semcheck_find_exact_type_node_for_qid(SymTab_t *symtab,
                                      const QualifiedIdent *type_ref) {
...
static int resolve_range_bounds_for_type_ref(SymTab_t *symtab,
  ...
  HashNode_t *type_node =
      semcheck_find_exact_type_node_for_qid(symtab, type_ref);
  if (type_node == NULL)
    return 0;
...
static int evaluate_const_expr_ordinal_bounds(SymTab_t *symtab,
                                              struct Expression *expr,
                                              long long *out_low,
                                              long long *out_high) {
  ...
  if (expr->resolved_kgpc_type != NULL) {
    ...
    if (alias != NULL) {
      ...
      if (alias->target_type_ref != NULL &&
          alias->target_type_ref->name != NULL &&
          resolve_range_bounds_for_type_ref(
              symtab, alias->target_type_ref->name, out_low, out_high)) {
        return 1;
      }
      if (alias->target_type_id != NULL &&
          resolve_range_bounds_for_type(symtab, alias->target_type_id, out_low,
    ...
    if (expr->resolved_kgpc_type->kind == TYPE_KIND_PRIMITIVE) {
      ...
      case LONGWORD_TYPE:
        *out_low = 0;
        *out_high = 4294967295LL;
        return 1;
      case QWORD_TYPE:
        *out_low = 0;
        *out_high = 9223372036854775807LL;
        return 1;
      case INT_TYPE:
      case LONGINT_TYPE:
        *out_low = -2147483648LL;
        *out_high = 2147483647LL;
        return 1;
      case INT64_TYPE:
        *out_low = (-9223372036854775807LL - 1);
        *out_high = 9223372036854775807LL;
        return 1;
      case BOOL:
        *out_low = 0;
  ...
  if (expr->type == EXPR_VAR_ID && expr->expr_data.id != NULL &&
      resolve_range_bounds_for_type(symtab, expr->expr_data.id, out_low,
                                    out_high)) {
    return 1;
  }
  ...
  if (expr->type == EXPR_RECORD_ACCESS) {
    ...
    if (qid != NULL) {
      int resolved =
          resolve_range_bounds_for_type_ref(symtab, qid, out_low, out_high);
...
static int const_typecast_target_is_ordinal(SymTab_t *symtab,
                                            struct Expression *expr,
                                            long long *out_low,
                                            long long *out_high) {
  ...
  if (target_ref != NULL && target_ref->name != NULL &&
      resolve_range_bounds_for_type_ref(symtab, target_ref->name, out_low,
                                        out_high)) {
    return 1;
  }
  if (target_id != NULL &&
  ...
  if (target_id != NULL &&
      get_builtin_type_bounds(semcheck_base_type_name(target_id), out_low,
                              out_high)) {
    return 1;
  }
  return 0;
...
static int const_fold_uint32_to_signed_ordinal(long long value,
                                               long long ordinal_low,
                                               long long ordinal_high,
                                               long long *out_value) {
  if (out_value == NULL)
    return 0;
...
static int const_fold_int_expr_mode(SymTab_t *symtab, struct Expression *expr,
  ...
  case EXPR_STRING:
    ...
    if (emit_diagnostics)
      fprintf(stderr, "Error: string literal in const expression must be a "
                      "single character.\n");
    return 1;
  case EXPR_CHAR_CODE:
  ...
  case EXPR_TYPECAST: {
    ...
    if (const_fold_int_expr_mode(symtab, expr->expr_data.typecast_data.expr,
                                 &inner_value, emit_diagnostics) != 0) {
      double real_value = 0.0;
      if (const_fold_real_expr_mode(symtab, expr->expr_data.typecast_data.expr,
                                    &real_value, emit_diagnostics) != 0)
        return 1;
    ...
    if (expr->expr_data.typecast_data.target_type_id != NULL)
      id =
          semcheck_base_type_name(expr->expr_data.typecast_data.target_type_id);
    ...
    if (target_type == UNKNOWN_TYPE &&
      ...
      else if (strcasecmp(id, "Cardinal") == 0 ||
               strcasecmp(id, "LongWord") == 0 || strcasecmp(id, "DWord") == 0)
        target_type = LONGWORD_TYPE;
      else if (strcasecmp(id, "Int64") == 0 || strcasecmp(id, "UInt64") == 0 ||
               strcasecmp(id, "QWord") == 0 || strcasecmp(id, "SizeInt") == 0 ||
               strcasecmp(id, "SizeUInt") == 0 ||
               strcasecmp(id, "NativeInt") == 0 ||
               strcasecmp(id, "NativeUInt") == 0) {
    ...
    case CHAR_TYPE: {
      int is_wide_char = 0;
      if (id != NULL && (strcasecmp(id, "WideChar") == 0 ||
    ...
    case BOOL:
      *out_value = (inner_value != 0);
      return 0;
    case ENUM_TYPE:
      if (const_typecast_target_is_ordinal(symtab, expr, &ordinal_low,
                                           &ordinal_high)) {
        if (const_fold_uint32_to_signed_ordinal(inner_value, ordinal_low,
                                                ordinal_high, out_value))
          return 0;
  ...
  case EXPR_ADDOP: {
    ...
    if (const_fold_int_expr_mode(symtab, expr->expr_data.addop_data.left_expr,
                                 &left, emit_diagnostics) != 0)
      return 1;
    if (const_fold_int_expr_mode(symtab, expr->expr_data.addop_data.right_term,
                                 &right, emit_diagnostics) != 0)
      return 1;
    switch (expr->expr_data.addop_data.addop_type) {
    case PLUS:
    ...
    case MINUS:
      *out_value = left - right;
      return 0;
    case OR:
      /* Bitwise OR for const expressions (FPC bootstrap: S_IRWXU = S_IRUSR or
       * S_IWUSR) */
      *out_value = left | right;
      return 0;
    default:
  ...
  case EXPR_MULOP: {
    long long left, right;
    if (const_fold_int_expr_mode(symtab, expr->expr_data.mulop_data.left_term,
                                 &left, emit_diagnostics) != 0)
      return 1;
    if (const_fold_int_expr_mode(symtab,
                                 expr->expr_data.mulop_data.right_factor,
                                 &right, emit_diagnostics) != 0)
      return 1;
    switch (expr->expr_data.mulop_data.mulop_type) {
  ...
  case EXPR_SET: {
    ...
    size_t used = 0;
    long long mask = 0;
    if (evaluate_set_const_bytes(symtab, expr, bytes, sizeof(bytes), &used,
                                 &mask, NULL) != 0)
      return 1;
    if (used > 8) {
      if (emit_diagnostics)
        fprintf(stderr, "Error: set literal requires 32-byte storage and "
                        "cannot be reduced to integer const.\n");
      return 1;
    }
    *out_value = mask;
    return 0;
  ...
  case EXPR_RECORD_ACCESS: {
    ...
    /* The field_id is the constant name, look it up directly in the symbol
     * table */
    ...
    if (field_id != NULL) {
      ...
      if (record_expr != NULL) {
        ...
        if (owner_name != NULL) {
          ...
          if (semcheck_resolve_scoped_enum_literal(symtab, owner_name, field_id,
                                                   &enum_value)) {
        ...
        if (record_expr->type == EXPR_VAR_ID &&
            ...
            semcheck_resolve_scoped_enum_literal(
    ...
    if (emit_diagnostics)
      fprintf(stderr,
              "Error: qualified constant '%s' is undefined or not a const.\n",
  ...
  case EXPR_FUNCTION_CALL: {
    ...
    if (id != NULL && (pascal_identifier_equals(id, "Succ") ||
                       pascal_identifier_equals(id, "Pred"))) {
      struct Expression *arg = extract_single_const_arg(args, id);
      ...
      if (arg == NULL)
        return 1;
      if (const_fold_int_expr_mode(symtab, arg, &arg_value, emit_diagnostics) !=
          0)
        return 1;
    ...
    if (id != NULL && args != NULL && args->next == NULL) {
      HashNode_t *type_node = NULL;
      int found_type =
          (FindSymbol(&type_node, symtab, id) != 0 && type_node != NULL &&
           type_node->hash_type == HASHTYPE_TYPE);
      const char *base_id = semcheck_base_type_name(id);
      if (!found_type && base_id != NULL && base_id != id) {
        found_type =
            (FindSymbol(&type_node, symtab, base_id) != 0 &&
             type_node != NULL && type_node->hash_type == HASHTYPE_TYPE);
      }

      if (found_type ||
          semcheck_map_builtin_type_name_local(id) != UNKNOWN_TYPE) {
        struct Expression *arg = (struct Expression *)args->cur;
        long long inner_value = 0;
        if (const_fold_int_expr_mode(symtab, arg, &inner_value,
                                     emit_diagnostics) != 0) {
          double real_value = 0.0;
    ...
    if (id != NULL && pascal_identifier_equals(id, "Ord")) {
      ...
      if (arg->type == EXPR_STRING) {
        ...
        if (literal == NULL || literal[0] == '\0') {
          if (emit_diagnostics)
            fprintf(stderr,
                    "Error: Ord expects a non-empty character literal.\n");
          return 1;
        }
        if (literal[1] != '\0') {
          if (emit_diagnostics)
      ...
      /* Handle const variable reference */
        ...
        HashNode_t *node = NULL;
        if (FindSymbol(&node, symtab, arg->expr_data.id) && node != NULL &&
            (node->hash_type == HASHTYPE_CONST || node->is_typed_const)) {
          if (node->const_string_value != NULL &&
      ...
      /* Handle nested const expressions */
        ...
        long long arg_value;
        if (const_fold_int_expr_mode(symtab, arg, &arg_value,
                                     emit_diagnostics) == 0) {
          *out_value = arg_value;
          return 0;
        }
        if (emit_diagnostics)
          fprintf(stderr,
                  "Error: Ord argument is not a valid const expression.\n");
        return 1;
      }
    }
    ...
    if (id != NULL && pascal_identifier_equals(id, "High")) {
      ...
      if (arg->type == EXPR_VAR_ID || arg->type == EXPR_RECORD_ACCESS) {
        ...
        if ((type_id_ref != NULL &&
             resolve_range_bounds_for_type_ref(symtab, type_id_ref, &range_low,
                                               &range_high)) ||
            (lookup_name != NULL &&
             resolve_range_bounds_for_type(symtab, lookup_name, &range_low,
                                           &range_high))) {
        ...
        const char *resolved =
            resolve_type_to_base_name(symtab, type_id_ref, lookup_name);
        if (resolved != NULL)
        ...
        if (emit_diagnostics)
          fprintf(stderr,
                  "Error: High(%s) - unsupported type in const expression.\n",
    ...
    if (id != NULL && pascal_identifier_equals(id, "Low")) {
      ...
      if (arg->type == EXPR_VAR_ID || arg->type == EXPR_RECORD_ACCESS) {
        ...
        if (emit_diagnostics)
          fprintf(stderr,
                  "Error: Low(%s) - unsupported type in const expression.\n",
                  qualified_name != NULL ? qualified_name : arg->expr_data.id);
        status = 1;
      low_cleanup:
        if (qualified_name != NULL)
    ...
    if (id != NULL && pascal_identifier_equals(id, "SizeOf")) {
      ...
      if (arg->type == EXPR_VAR_ID) {
        ...
        /* Resolve type aliases to base type */
        const char *resolved =
            resolve_type_to_base_name(symtab, type_id_ref, type_name);
        if (resolved != NULL)
          base_name = resolved;

        /* Prefer computing from an actual type node (handles nested/local
         * types). */
        ...
        if (symtab != NULL) {
          if (type_id_ref != NULL && type_id_ref->count > 1) {
            ...
            if (qualified_name != NULL) {
              size_type_node =
                  semcheck_find_preferred_type_node(symtab, qualified_name);
              free(qualified_name);
            }
          }
        ...
        if (size_type_node != NULL &&
            size_type_node->hash_type == HASHTYPE_TYPE) {
          long long computed_size = 0;
          if (sizeof_from_hashnode(symtab, size_type_node, &computed_size, 0,
                                   expr->line_num) == 0) {
        ...
        if (emit_diagnostics)
          fprintf(stderr,
                  "Error: SizeOf(%s) - unsupported type in const expression.\n",
                  arg->expr_data.id);
        qualified_ident_free(type_id_ref);
        return 1;
      }
      if (emit_diagnostics)
        fprintf(stderr,
                "Error: SizeOf expects a type identifier as argument.\n");
      return 1;
    }

    /* Handle BitSizeOf() function for constant expressions - returns SizeOf(T)
     * * 8 */
    if (id != NULL && pascal_identifier_equals(id, "BitSizeOf")) {
      ...
      if (arg->type == EXPR_VAR_ID) {
        ...
        if (emit_diagnostics)
          fprintf(
              stderr,
              "Error: BitSizeOf(%s) - unsupported type in const expression.\n",
              arg->expr_data.id);
        qualified_ident_free(type_id_ref);
        return 1;
      }
      if (emit_diagnostics)
        fprintf(stderr,
                "Error: BitSizeOf expects a type identifier as argument.\n");
      return 1;
    }
    ...
    if (id != NULL && pascal_identifier_equals(id, "Chr")) {
      ...
      long long char_code;
      if (const_fold_int_expr_mode(symtab, arg, &char_code, emit_diagnostics) !=
          0) {
        if (emit_diagnostics)
      ...
      if (char_code < 0 || char_code > 255) {
        if (emit_diagnostics)
          fprintf(stderr,
                  "Error: Chr argument %lld is out of valid range (0..255).\n",
                  char_code);
        return 1;
      }
    ...
    if (id != NULL && pascal_identifier_equals(id, "Pointer")) {
      ...
      /* Evaluate the argument as a const expression */
      long long ptr_value;
      if (const_fold_int_expr_mode(symtab, arg, &ptr_value, emit_diagnostics) !=
          0) {
        if (emit_diagnostics)
          fprintf(stderr,
                  "Error: Pointer argument must be a const expression.\n");
        return 1;
      }
    ...
    if (id != NULL && (pascal_identifier_equals(id, "PtrInt") ||
      ...
      /* Evaluate the argument as a const expression */
      long long int_value;
      if (const_fold_int_expr_mode(symtab, arg, &int_value, emit_diagnostics) !=
          0) {
        if (emit_diagnostics)
    ...
    /* Handle Cardinal, LongWord, DWord, QWord, Int64, UInt64, HRESULT and other
     * integer typecasts for constant expressions (FPC bootstrap: Cardinal(not
     * Cardinal(0)), HRESULT($80020004)) */
    if (id != NULL && (pascal_identifier_equals(id, "Cardinal") ||
      ...
      if (pascal_identifier_equals(id, "Cardinal") ||
        ...
        *out_value = (int)(int_value & 0xFFFFFFFFULL);
        ...
        /* QWord, Int64, UInt64, NativeInt, NativeUInt, SizeInt, SizeUInt -
         * 64-bit */
        *out_value = int_value;
      }
      return 0;
    ...
    if (id != NULL && args != NULL && args->next == NULL) {
      ...
      if (FindSymbol(&type_node, symtab, id) != 0 && type_node != NULL &&
        ...
        if (legacy_tag != UNKNOWN_TYPE) {
          ...
          if (const_fold_int_expr_mode(symtab, arg, &int_value,
                                       emit_diagnostics) != 0) {
            if (emit_diagnostics)
              fprintf(stderr,
                      "Error: %s argument must be a const expression.\n", id);
            return 1;
          }
    ...
    if (emit_diagnostics) {
      if (id != NULL)
        fprintf(stderr,
                "Error: const expression uses unsupported function %s on line "
                "%d.\n",
                id, expr->line_num);
      fprintf(stderr, "Error: only Ord(), High(), Low(), SizeOf(), "
                      "BitSizeOf(), Chr(), Trunc(), and integer typecasts are "
                      "supported in const expressions.\n");
    }
    return 1;
  }
  case EXPR_RELOP: {
    ...
    if (expr->expr_data.relop_data.type == NOT) {
      ...
      if (const_fold_int_expr_mode(symtab, operand, &operand_value,
                                   emit_diagnostics) != 0) {
        if (emit_diagnostics)
    ...
    if (expr->expr_data.relop_data.left != NULL &&
      ...
      if (const_fold_int_expr_mode(symtab, expr->expr_data.relop_data.left,
                                   &left_val, emit_diagnostics) != 0)
        return 1;
      if (const_fold_int_expr_mode(symtab, expr->expr_data.relop_data.right,
                                   &right_val, emit_diagnostics) != 0)
        return 1;

      switch (expr->expr_data.relop_data.type) {
      case EQ:
        *out_value = (left_val == right_val) ? 1 : 0;
        return 0;
      case NE:
        *out_value = (left_val != right_val) ? 1 : 0;
        return 0;
    ...
    if (emit_diagnostics)
      fprintf(stderr,
              "Error: unsupported relational operator in const expression.\n");
    return 1;
  }
  case EXPR_ADDR: {
    ...
    if (deref_check->type != EXPR_POINTER_DEREF ||
        ...
        deref_check->expr_data.pointer_deref_data.pointer_expr->type !=
            EXPR_NIL)
      break;
...
int const_fold_int_expr(SymTab_t *symtab, struct Expression *expr,
                        long long *out_value) {
  return const_fold_int_expr_mode(symtab, expr, out_value, 0);
}

int const_fold_real_expr(SymTab_t *symtab, struct Expression *expr,
                         double *out_value) {
  return const_fold_real_expr_mode(symtab, expr, out_value, 0);
}

int evaluate_const_expr(SymTab_t *symtab, struct Expression *expr,
                        long long *out_value) {
