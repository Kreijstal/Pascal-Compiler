...
TypeRef **semcheck_param_types_from_params(ListNode_t *params,
                                           int skip_first_param,
                                           int *out_count);
static void
semcheck_ensure_implicit_tobject_parent(struct RecordType *record_info,
...
static int build_class_vmt(SymTab_t *symtab, struct RecordType *record_info,
   ...
   * For nested types (e.g., TMarshaller.TDeferBase), methods may have been
   * registered under the unqualified name before renaming. Fall back to
   * unqualified name if needed. */
  ...
  if (record_info->parent_class_name != NULL) {
    ...
    HashNode_t *parent_node = semcheck_find_preferred_type_node(
    ...
    if (parent_node != NULL) {
      ...
      if (parent_record != NULL && parent_record->methods != NULL) {
        ...
        while (parent_vmt != NULL) {
          struct MethodInfo *parent_method =
              (struct MethodInfo *)parent_vmt->cur;
          if (parent_method != NULL) {
            struct MethodInfo *cloned =
                (struct MethodInfo *)calloc(1, sizeof(struct MethodInfo));
            if (cloned != NULL) {
              cloned->name =
                  parent_method->name ? strdup(parent_method->name) : NULL;
              cloned->mangled_name = parent_method->mangled_name
                                         ? strdup(parent_method->mangled_name)
                                         : NULL;
              cloned->is_virtual = parent_method->is_virtual;
              cloned->is_override =
                  0; /* Parent's methods aren't overrides in child */
              ...
              cloned->param_types = param_types_clone(
                  parent_method->param_types, parent_method->param_types_count);
              cloned->param_types_count = (cloned->param_types != NULL)
                                              ? parent_method->param_types_count
                                              : -1;
              cloned->resolved_mangled_id =
                  parent_method->resolved_mangled_id
                      ? strdup(parent_method->resolved_mangled_id)
                      : NULL;
              ...
              if (node != NULL) {
                ...
                *tail = node;
                ...
                param_types_free(cloned->param_types,
                                 cloned->param_types_count);
                free(cloned);
              }
            }
          }
  ...
  while (cur_method != NULL) {
    ...
    if (binding != NULL && binding->method_name != NULL) {
       ...
       */
      char *base_name =
          make_method_lookup_key(class_name, binding->method_name);
      char *mangled = NULL;
      if (base_name != NULL && symtab != NULL) {
        ...
        if (binding->param_types != NULL && binding->param_types_count >= 0) {
          for (ListNode_t *m = matches; m != NULL; m = m->next) {
            ...
            int matched = type_ref_array_equal_ci(binding->param_types,
                                                  binding->param_types_count,
        ...
        if (best != NULL) {
          ...
          if (best->type != NULL &&
              best->type->info.proc_info.definition != NULL) {
      ...
      if (binding->is_virtual || binding->is_override) {
        ...
        if (override_target == NULL && binding->is_override) {
          for (ListNode_t *vmt_entry = vmt; vmt_entry != NULL;
               vmt_entry = vmt_entry->next) {
            struct MethodInfo *info = (struct MethodInfo *)vmt_entry->cur;
            if (info == NULL || info->name == NULL || info->param_types != NULL)
              continue;
            if (strcasecmp(info->name, binding->method_name) != 0)
              continue;
        ...
        if (override_target != NULL) {
          ...
          if (info->param_types == NULL && binding->param_types != NULL) {
            info->param_types = param_types_clone(binding->param_types,
                                                  binding->param_types_count);
            info->param_types_count =
                (info->param_types != NULL) ? binding->param_types_count : -1;
          }
        }
      }
      ...
      if (is_actual_override) {
        ...
        struct MethodInfo *new_method =
            (struct MethodInfo *)calloc(1, sizeof(struct MethodInfo));
        if (new_method != NULL) {
          new_method->name =
              binding->method_name ? strdup(binding->method_name) : NULL;
          ...
          new_method->param_types = param_types_clone(
              binding->param_types, binding->param_types_count);
          new_method->param_types_count = (new_method->param_types != NULL)
                                              ? binding->param_types_count
                                              : -1;
          ...
          if (node != NULL) {
            ...
            param_types_free(new_method->param_types,
                             new_method->param_types_count);
            free(new_method);
          }
        }
      }
  ...
  for (ListNode_t *vmt_node = vmt; vmt_node != NULL;
       vmt_node = vmt_node->next) {
    ...
    if (FindSymbol(&func_sym, symtab, mi->mangled_name) != 0 &&
        ...
        func_sym->type != NULL && func_sym->type->kind == TYPE_KIND_PROCEDURE &&
    ...
    /* Priority 0: exact mangled_id match — only for defined (non-abstract)
     * functions to avoid pointing VMT slots to labels that are never emitted */
    for (ListNode_t *m = matches; m != NULL && mi->resolved_mangled_id == NULL;
         m = m->next) {
      HashNode_t *cand = (HashNode_t *)m->cur;
    ...
    if (mi->resolved_mangled_id == NULL) {
      ...
      int exact_name_match =
          0; /* mi->mangled_name exactly matched a passing candidate */
      for (ListNode_t *m = matches; m != NULL; m = m->next) {
        HashNode_t *cand = (HashNode_t *)m->cur;
        if (cand == NULL || cand->type == NULL ||
            cand->type->kind != TYPE_KIND_PROCEDURE)
        ...
        if (cand->type->info.proc_info.definition != NULL) {
          ...
          if (def != NULL && def->tree_data.subprogram_data.mangled_id != NULL)
...
static int
semcheck_template_matches_binding(const struct MethodTemplate *tmpl,
                                  const ClassMethodBinding *binding) {
  if (tmpl == NULL || binding == NULL || tmpl->name == NULL ||
...
static int
semcheck_template_matches_vmt_method(const struct MethodTemplate *tmpl,
...
void semcheck_refresh_generic_specialization_vmts(SymTab_t *symtab,
  ...
  for (ListNode_t *cur = type_decls; cur != NULL; cur = cur->next) {
    ...
    for (ListNode_t *tmpl_cur = record_info->method_templates; tmpl_cur != NULL;
      ...
      if (!already_registered) {
        from_cparser_register_method_template(
            class_name, tmpl->name, tmpl->is_virtual, tmpl->is_override,
            tmpl->is_static, from_cparser_count_params_ast(tmpl->params_ast));
      }
    }
    ...
    for (ListNode_t *tmpl_cur = record_info->method_templates; tmpl_cur != NULL;
      ...
      for (ListNode_t *m = matches; m != NULL; m = m->next) {
        ...
        if (cand->type->info.proc_info.definition != NULL &&
            cand->type->info.proc_info.definition->tree_data.subprogram_data
                    .mangled_id != NULL)
          resolved_id = cand->type->info.proc_info.definition->tree_data
                            .subprogram_data.mangled_id;
        break;
      }
      if (matches != NULL)
        DestroyList(matches);
      ...
      for (ListNode_t *vmt_node = record_info->methods; vmt_node != NULL;
           vmt_node = vmt_node->next) {
        struct MethodInfo *mi = (struct MethodInfo *)vmt_node->cur;
...
int resolve_const_identifier(SymTab_t *symtab, const char *id,
                             long long *out_value) {
...
static int semcheck_try_resolve_enum_literal_by_base(SymTab_t *symtab,
                                                     const char *base_name,
                                                     const char *literal_name,
                                                     long long *out_value) {
  if (symtab == NULL || base_name == NULL || literal_name == NULL ||
      out_value == NULL)
    return 0;
  ...
  while (cur != NULL) {
    ...
    for (int i = 0; i < name_count; ++i) {
      if (count == capacity) {
        ...
        TypeRef **grown =
            (TypeRef **)realloc(arr, (size_t)new_cap * sizeof(TypeRef *));
        if (grown == NULL) {
...
int semcheck_resolve_scoped_enum_literal(SymTab_t *symtab,
                                         ...
                                         const char *literal_name,
                                         long long *out_value) {
  if (symtab == NULL || type_name == NULL || literal_name == NULL ||
      out_value == NULL)
    return 0;

  QualifiedIdent *qualified_ref = qualified_ident_from_dotted(type_name);
  if (qualified_ref != NULL && qualified_ref->count > 1) {
    int resolved = semcheck_resolve_scoped_enum_literal_ref(
        symtab, qualified_ref, literal_name, out_value);
  ...
  for (int depth = 0; depth < 8; ++depth) {
    ...
    for (ListNode_t *cur = matches; cur != NULL; cur = cur->next) {
      ...
      struct TypeAlias *candidate_alias =
          kgpc_type_get_type_alias(candidate->type);
      int has_enum_literals =
          (candidate_alias != NULL && candidate_alias->is_enum &&
      ...
      int unit_match =
          (cur_unit_idx > 0 && candidate->source_unit_index == cur_unit_idx);
      ...
      struct TypeAlias *best_alias =
          kgpc_type_get_type_alias(best_type_node->type);
      int best_has_enum_literals = (best_alias != NULL && best_alias->is_enum &&
                                    best_alias->enum_literals != NULL);
      int best_has_alias_target =
          (best_alias != NULL && (best_alias->target_type_ref != NULL ||
    ...
    if (type_node == NULL || type_node->hash_type != HASHTYPE_TYPE) {
      ...
      if (qid != NULL && qid->count > 1) {
        ...
        if (base_name != NULL &&
            semcheck_try_resolve_enum_literal_by_base(
                symtab, base_name, literal_name, out_value)) {
    ...
    if (alias != NULL && alias->target_type_ref != NULL &&
      ...
      if (qualified != NULL &&
          !pascal_identifier_equals(qualified, current_type)) {
        if (owned_type != NULL)
          free(owned_type);
...
int semcheck_resolve_scoped_enum_literal_ref(SymTab_t *symtab,
                                             ...
                                             const char *literal_name,
                                             long long *out_value) {
  if (symtab == NULL || type_ref == NULL || literal_name == NULL ||
      out_value == NULL)
    return 0;
  ...
  for (int depth = 0; depth < 8; ++depth) {
    HashNode_t *type_node =
        semcheck_find_exact_type_node_for_qid(symtab, current_ref);
    if (type_node == NULL || type_node->hash_type != HASHTYPE_TYPE)
      break;
    ...
    if (alias != NULL && alias->target_type_ref != NULL &&
      ...
      if (base != NULL && semcheck_try_resolve_enum_literal_by_base(
                              symtab, base, literal_name, out_value)) {
        if (owned_ref != NULL)
          qualified_ident_free(owned_ref);
        return 1;
      }
    ...
    if (alias->target_type_ref != NULL &&
      ...
      if (current_name != NULL) {
        int same_target =
            pascal_identifier_equals(alias->target_type_id, current_name);
        free(current_name);
        if (same_target)
          break;
      }
    ...
    int resolved = semcheck_resolve_scoped_enum_literal(
        symtab, alias->target_type_id, literal_name, out_value);
    return resolved;
  }
...
static void sync_alias_array_bounds(KgpcType *kgpc_type,
...
static void sync_alias_range_bounds(KgpcType *kgpc_type,
                                    struct TypeAlias *alias) {
  if (kgpc_type == NULL || alias == NULL)
    return;
  if (kgpc_type->type_alias != NULL && kgpc_type->type_alias != alias) {
...
static void resolve_alias_range_bounds_in_kgpctype(SymTab_t *symtab,
                                                   KgpcType *kgpc_type,
                                                   struct TypeAlias *alias) {
...
void resolve_array_bounds_in_kgpctype(SymTab_t *symtab, KgpcType *kgpc_type,
                                      struct TypeAlias *alias) {
  ...
  if (alias->array_dimensions != NULL) {
    ...
    if (start_str_ref != NULL && end_str_ref != NULL) {
      ...
      int tmp_val = 0;
      int tmp_resolved =
          (resolve_array_bound_expr(symtab, start_str, &tmp_val) == 0);
      if (resolve_const_identifier(symtab, start_str, &start_val) == 0 ||
          tmp_resolved) {
        if (tmp_resolved)
          start_val = tmp_val;
      ...
      int tmp_end_val = 0;
      int tmp_end_resolved =
          (resolve_array_bound_expr(symtab, end_str, &tmp_end_val) == 0);
      if (resolve_const_identifier(symtab, end_str, &end_val) == 0 ||
          tmp_end_resolved) {
        if (tmp_end_resolved)
          end_val = tmp_end_val;
      ...
      const char *dim_str =
          (first_dim != NULL && first_dim->type == LIST_STRING)
              ...
              : NULL;
      HashNode_t *type_node = NULL;
      if (dim_str != NULL && FindSymbol(&type_node, symtab, dim_str) != 0 &&
...
int semcheck_type_decls(SymTab_t *symtab, ListNode_t *type_decls) {
  ...
  while (cur != NULL) {
    ...
    if (tree->tree_data.type_decl_data.defined_in_unit &&
      ...
      const char *uname =
          unit_registry_get(tree->tree_data.type_decl_data.source_unit_index);
      if (uname != NULL) {
    ...
    if (debug_pss != NULL && tree->tree_data.type_decl_data.id != NULL &&
        pascal_identifier_equals(tree->tree_data.type_decl_data.id,
                                 "PShortString")) {
      struct TypeAlias *a = &tree->tree_data.type_decl_data.info.alias;
      fprintf(stderr,
              "[PShortString] decl kind=%d is_pointer=%d pointer_type_id=%s "
              "target=%s\n",
              tree->tree_data.type_decl_data.kind, a->is_pointer,
    ...
    if (kgpc_getenv("KGPC_DEBUG_TFLOAT") != NULL &&
        ...
        pascal_identifier_equals(tree->tree_data.type_decl_data.id,
                                 "TFloatFormatProfile")) {
      fprintf(stderr,
              "[TFLOAT] semcheck_type_decls kind=%d defined_in_unit=%d\n",
    ...
    if (debug_env_check != NULL) {
      fprintf(stderr,
              "[KGPC] semcheck_type_decls processing: id=%s kind=%d "
              "return_val=%d\n",
              tree->tree_data.type_decl_data.id
                  ? tree->tree_data.type_decl_data.id
                  : "<null>",
    ...
    case TYPE_DECL_RECORD:
      ...
      semcheck_ensure_implicit_tobject_parent(
          record_info, tree->tree_data.type_decl_data.id);

      /* Handle class inheritance - merge parent fields */
      if (record_info != NULL && record_info->parent_class_name != NULL) {
        int merge_result = merge_parent_class_fields(
            symtab, record_info, tree->tree_data.type_decl_data.id,
            tree->line_num);
        if (merge_result > 0) {
        ...
        /* Classes without parents (like TObject) still need default indexed
         * property detection */
      ...
      if (record_info != NULL && record_info->is_interface &&
        ...
        HashNode_t *parent_iface_node = semcheck_find_preferred_type_node(
            symtab, record_info->parent_class_name);
        struct RecordType *parent_iface =
            parent_iface_node != NULL
                ? get_record_type_from_node(parent_iface_node)
                : NULL;
        if (parent_iface != NULL && parent_iface->is_interface &&
          ...
          for (ListNode_t *pm = parent_iface->method_templates; pm != NULL;
               pm = pm->next) {
            ...
            for (ListNode_t *own = record_info->method_templates; own != NULL;
                 own = own->next) {
              struct MethodTemplate *own_mt = (struct MethodTemplate *)own->cur;
              if (own_mt != NULL && own_mt->name != NULL &&
                  orig->name != NULL &&
            ...
            if (already_present)
              continue;
            struct MethodTemplate *cloned =
                clone_method_template_detached(orig);
            if (cloned == NULL)
              continue;
        ...
        while (template_cur != NULL) {
          if (template_cur->type == LIST_METHOD_TEMPLATE) {
            struct MethodTemplate *tmpl =
                (struct MethodTemplate *)template_cur->cur;
            if (tmpl != NULL && tmpl->name != NULL &&
              ...
              if (mangled != NULL) {
                ...
                ListNode_t *bindings = NULL;
                int method_count = 0;
                get_class_methods(tree->tree_data.type_decl_data.id, &bindings,
                                  &method_count);
                ...
                if (bindings != NULL) {
                  ...
                  if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
                    fprintf(stderr,
                            "[SemCheck] No existing methods found for %s\n",
                            tree->tree_data.type_decl_data.id);
                  }
                }
                DestroyList(bindings);
                ...
                if (!already_registered) {
                  from_cparser_register_method_template(
                      tree->tree_data.type_decl_data.id, tmpl->name,
                      tmpl->is_virtual, tmpl->is_override, tmpl->is_static,
                      from_cparser_count_params_ast(tmpl->params_ast));

                  if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
                    fprintf(stderr,
                            "[SemCheck] Registered method template: %s.%s "
                            "(virtual=%d, override=%d, static=%d)\n",
                            tree->tree_data.type_decl_data.id, tmpl->name,
                            tmpl->is_virtual, tmpl->is_override,
                            tmpl->is_static);
                  }
                }
                ...
                if (tmpl->params_ast != NULL) {
                  ...
                  pascal_frontend_set_default_shortstring(
                      tmpl->default_shortstring != 0);
                ...
                if (!tmpl->is_static &&
                  ...
                  int self_is_var = 1;
                  if (record_info != NULL && record_info->is_class) {
                    /* Class instances are already pointers; Self should not be
                     * var. */
                    self_is_var = 0;
                  }
                  if (record_info != NULL && record_info->is_type_helper) {
                    ...
                    self_is_var =
                        semcheck_helper_self_is_var(symtab, self_type_id);
                  }
                  ListNode_t *self_ids =
                      CreateListNode(strdup("Self"), LIST_STRING);
                  if (self_ids != NULL && self_type_id != NULL) {
                    Tree_t *self_param = mk_vardecl(
                        0, self_ids, UNKNOWN_TYPE, strdup(self_type_id),
                        self_is_var, 0, NULL, NULL, NULL, NULL);
                    ListNode_t *self_node =
                        CreateListNode(self_param, LIST_TREE);
                    if (self_node != NULL) {
                ...
                if (!has_matching_signature) {
                  ...
                  if (return_type_node == NULL && tmpl->method_ast != NULL) {
                    ...
                    for (ast_t *child = method_ast->child; child != NULL;
                         child = child->next) {
                  ...
                  int is_function_template =
                      (tmpl->kind == METHOD_TEMPLATE_FUNCTION ||
                       tmpl->kind == METHOD_TEMPLATE_CONSTRUCTOR ||
                       tmpl->has_return_type || return_type_node != NULL);

                  if (return_type_node != NULL) {
                    if (return_type_node->child != NULL)
                      return_type = convert_type_spec_to_kgpctype(
                          return_type_node->child, symtab);
                    if (return_type_id == NULL)
                      return_type_id =
                          semcheck_dup_type_id_from_ast(return_type_node);
                  }
                  if (return_type_id != NULL) {
                    const char *owner_full =
                        (record_info != NULL && record_info->type_id != NULL)
                            ? record_info->type_id
                            : tree->tree_data.type_decl_data.id;
                    const char *owner_outer = (record_info != NULL)
                                                  ? record_info->outer_type_id
                                                  : NULL;
                    if (owner_full != NULL)
                      semcheck_maybe_qualify_nested_type(symtab, owner_full,
                                                         owner_outer,
                                                         &return_type_id, NULL);
                  }
                  if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL &&
                    ...
                    fprintf(stderr,
                            "[SemCheck] tmpl %s return_type_node=%p "
                            "return_type_id=%s return_type=%p\n",
                            tmpl->name, (void *)return_type_node,
                  ...
                  if (return_type == NULL && return_type_id != NULL) {
                    ...
                    if (FindSymbol(&type_node, symtab, return_type_id) != 0 &&
                      ...
                      int tag =
                          semcheck_map_builtin_type_name_local(return_type_id);
                      if (tag != UNKNOWN_TYPE)
                  ...
                  KgpcType *proc_type =
                      create_procedure_type(params, return_type);
                  ...
                  char *overload_mangled =
                      MangleFunctionName(mangled, params, symtab);
                  if (overload_mangled == NULL)
                    overload_mangled = strdup(mangled);
                  /* Free original param list nodes (data owned by proc_type
                   * now) */
                  DestroyList(params);
                  params = NULL;

                  if (is_function_template)
                    PushFunctionOntoScope_Typed(symtab, mangled,
                                                overload_mangled, proc_type);
                  else
                    PushProcedureOntoScope_Typed(symtab, mangled,
                                                 overload_mangled, proc_type);

                  /* Set method identity on the newly-pushed symbol */
                  {
                    HashNode_t *pushed_node = NULL;
                    if (FindSymbol(&pushed_node, symtab, mangled) != 0 &&
                        pushed_node != NULL) {
                      if (pushed_node->method_name == NULL &&
                          tmpl->name != NULL)
                        pushed_node->method_name = strdup(tmpl->name);
                      if (pushed_node->owner_class == NULL)
                        pushed_node->owner_class =
                            strdup(tree->tree_data.type_decl_data.id);
                      if (tree->tree_data.type_decl_data.defined_in_unit) {
                        pushed_node->defined_in_unit = 1;
                        pushed_node->unit_is_public =
                            tree->tree_data.type_decl_data.unit_is_public ? 1
                                                                          : 0;
                  ...
                  if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
                    fprintf(stderr,
                            "[SemCheck] Added method forward declaration: %s "
                            "-> %s\n",
                            tmpl->name, mangled);
                  }
                } else {
                  int is_function_template =
                      (tmpl->kind == METHOD_TEMPLATE_FUNCTION ||
                       tmpl->kind == METHOD_TEMPLATE_CONSTRUCTOR ||
                       tmpl->has_return_type || tmpl->return_type_ast != NULL);
                    ...
                    if (return_type_id != NULL) {
                      ...
                      if (owner_full != NULL)
                        semcheck_maybe_qualify_nested_type(
                            symtab, owner_full, owner_outer, &return_type_id,
                            NULL);
                    }
                    if (return_type == NULL && return_type_id != NULL) {
                      HashNode_t *type_node = NULL;
                      if (FindSymbol(&type_node, symtab, return_type_id) != 0 &&
                        ...
                        int tag = semcheck_map_builtin_type_name_local(
                            return_type_id);
                        if (tag != UNKNOWN_TYPE)
                          return_type = create_primitive_type(tag);
                      }
                  ...
                  if (existing != NULL && existing->type != NULL &&
                    ...
                    copy_default_values_to_impl_params(
                        params, existing->type->info.proc_info.params);
            ...
            if (tmpl != NULL && tmpl->name != NULL) {
              ...
              if (mangled != NULL) {
                snprintf(mangled, class_len + 2 + method_len + 1, "%s__%s",
                         record_info->type_id, tmpl->name);
                HashNode_t *existing = NULL;
                if (FindSymbol(&existing, symtab, mangled) == 0) {
                  KgpcType *proc_type =
                      from_cparser_method_template_to_proctype(
                          tmpl, record_info, symtab);
                  if (proc_type != NULL) {
                    char *mangled_dup = strdup(mangled);
                    if (proc_type->info.proc_info.return_type != NULL ||
                        tmpl->has_return_type) {
                      PushFunctionOntoScope_Typed(symtab, mangled, mangled_dup,
                                                  proc_type);
                    } else {
                      PushProcedureOntoScope_Typed(symtab, mangled, mangled_dup,
                                                   proc_type);
                    }
                    /* Set method identity on the newly-pushed symbol */
                    {
      ...
      /* Now also check if this class has method implementations that need to be
       * added. These are in the subprogram list (nested_subs of the class type
       * decl) */
      ...
      if (record_info != NULL) {
        int vmt_result =
            build_class_vmt(symtab, record_info,
        ...
        if (!is_unspecialized_generic) {
          ...
          int size_result = semcheck_compute_record_size(
              symtab, record_info, &record_size, tree->line_num);
          if (size_result != 0) {
#ifdef DEBUG
            fprintf(stderr,
                    "DEBUG: semcheck_compute_record_size FAILED for %s: "
                    "result=%d\n",
                    tree->tree_data.type_decl_data.id, size_result);
#endif
            if (kgpc_getenv("KGPC_DEBUG_SEMSTEPS") != NULL) {
              fprintf(stderr,
                      "[SemCheck] record size failed for %s (line %d)\n",
                      tree->tree_data.type_decl_data.id
                          ? tree->tree_data.type_decl_data.id
                          : "<null>",
                      tree->line_num);
    ...
    case TYPE_DECL_ALIAS: {
      ...
      if (alias_info->alias_name != NULL &&
        ...
        alias_info->is_wide_string = 1;
                 ...
                 (pascal_identifier_equals(alias_info->target_type_id,
                                           "UnicodeString") ||
                  pascal_identifier_equals(alias_info->target_type_id,
                                           "WideString"))) {
        alias_info->is_wide_string = 1;
      ...
      alias_info->is_char_alias =
          semcheck_alias_should_be_char_like(tree->tree_data.type_decl_data.id,
                                             alias_info->target_type_id) &&
          !alias_info->is_pointer && !alias_info->is_array &&
          !alias_info->is_set && !alias_info->is_enum && !alias_info->is_file;
      ...
      if (alias_info->inline_record_type != NULL) {
        ...
        if (tree->tree_data.type_decl_data.kgpc_type == NULL ||
          ...
          KgpcType *inline_kgpc =
              create_record_type(alias_info->inline_record_type);
      ...
      if (kgpc_getenv("KGPC_DEBUG_PROC_TYPE") != NULL &&
        ...
        fprintf(
            stderr,
            "[KGPC_DEBUG_PROC_TYPE] alias=%s base=%d kgpc=%s kind=%d "
            "return_id=%s\n",
            tree->tree_data.type_decl_data.id, alias_info->base_type,
            tree->tree_data.type_decl_data.kgpc_type
                ? kgpc_type_to_string(tree->tree_data.type_decl_data.kgpc_type)
                : "<null>",
            tree->tree_data.type_decl_data.kgpc_type
                ? tree->tree_data.type_decl_data.kgpc_type->kind
                : -1,
            (tree->tree_data.type_decl_data.kgpc_type &&
             tree->tree_data.type_decl_data.kgpc_type->kind ==
                 TYPE_KIND_PROCEDURE &&
             tree->tree_data.type_decl_data.kgpc_type->info.proc_info
                 .return_type_id)
                ? tree->tree_data.type_decl_data.kgpc_type->info.proc_info
                      .return_type_id
                : "<null>");
      ...
      if (alias_info->is_array) {
        ...
        if (var_type == HASHVAR_UNTYPED &&
            (alias_info->target_type_ref != NULL ||
          ...
          if (alias_info->target_type_ref != NULL)
            target_node = semcheck_find_preferred_type_node_with_ref(
                symtab, alias_info->target_type_ref,
                alias_info->target_type_id);
          else
            target_node = semcheck_find_preferred_type_node(
                symtab, alias_info->target_type_id);

          if (target_node != NULL && target_node->type != NULL) {
            struct TypeAlias *target_alias =
                kgpc_type_get_type_alias(target_node->type);
            if (target_alias == alias_info) {
              char *lookup = NULL;
              if (alias_info->target_type_ref != NULL)
                lookup = type_ref_render_mangled(alias_info->target_type_ref);
              const char *name =
                  (lookup != NULL) ? lookup : alias_info->target_type_id;
              HashNode_t *alt =
                  semcheck_find_type_excluding_alias(symtab, name, alias_info);
              if (alt != NULL)
                target_node = alt;
              free(lookup);
            }
          ...
          if (target_node != NULL) {
            if (alias_info->target_type_ref != NULL &&
                semcheck_is_explicit_unit_qualified_type_ref(
                    alias_info->target_type_ref) &&
              ...
              const char *unit_name =
                  (alias_info->target_type_ref->name != NULL &&
                   ...
                   alias_info->target_type_ref->name->count > 0)
                      ...
                      : NULL;
              const char *base_name =
                  type_ref_base_name(alias_info->target_type_ref);
              if (base_name != NULL) {
                ListNode_t *matches = FindAllIdents(symtab, base_name);
                HashNode_t *unit_match = NULL;
                for (ListNode_t *mcur = matches; mcur != NULL;
                     mcur = mcur->next) {
            ...
            if (target_node->type != NULL && !alias_info->is_array &&
                !alias_info->is_pointer && !alias_info->is_set &&
      ...
      if (alias_info->base_type == RECORD_TYPE) {
        ...
        if (alias_record != NULL) {
          ...
          if (alias_record->parent_class_name != NULL &&
            ...
            int merge_result = merge_parent_class_fields(
                symtab, alias_record, tree->tree_data.type_decl_data.id,
                tree->line_num);
            if (merge_result > 0)
              return_val += merge_result;
          ...
          if (!is_unspecialized_generic) {
            ...
            if (debug_env3 != NULL) {
              fprintf(stderr,
                      "[KGPC] Computing size for alias %s, generic_decl=%p "
                      "num_args=%d\n",
                      tree->tree_data.type_decl_data.id,
            ...
            if (semcheck_compute_record_size(symtab, alias_record, &record_size,
              ...
              if (kgpc_getenv("KGPC_DEBUG_SEMSTEPS") != NULL) {
                fprintf(
                    stderr,
                    "[SemCheck] alias record size failed for %s (line %d)\n",
                    tree->tree_data.type_decl_data.id
                        ? tree->tree_data.type_decl_data.id
                        : "<null>",
                    tree->line_num);
              ...
              fprintf(stderr,
                      "[KGPC] Size computation succeeded for %s: %lld bytes\n",
      ...
      if (debug_alias != NULL && tree->tree_data.type_decl_data.id != NULL) {
        fprintf(stderr,
                "[KGPC] semcheck alias %s: base_type=%d target=%s kgpc_type=%p "
                "kind=%d\n",
                tree->tree_data.type_decl_data.id, alias_info->base_type,
                alias_info->target_type_id != NULL ? alias_info->target_type_id
                                                   : "<null>",
    ...
    case TYPE_DECL_GENERIC: {
      ...
      struct GenericDecl *generic_info =
          &tree->tree_data.type_decl_data.info.generic;

      const char *debug_env = kgpc_getenv("KGPC_DEBUG_TFPG");
      if (debug_env != NULL) {
        fprintf(stderr,
                "[KGPC] semcheck TYPE_DECL_GENERIC: id=%s num_params=%d\n",
                tree->tree_data.type_decl_data.id
                    ? tree->tree_data.type_decl_data.id
                    : "<null>",
      ...
      if (generic_info != NULL && generic_info->num_type_params > 0 &&
        ...
        GenericTypeDecl *generic_decl = generic_registry_add_decl(
            tree->tree_data.type_decl_data.id, generic_info->type_parameters,
            generic_info->num_type_params, tree);
        ...
        if (generic_decl != NULL) {
          ...
          if (debug_env != NULL) {
            fprintf(stderr,
                    "[KGPC] Registered generic type %s with %d parameters\n",
                    tree->tree_data.type_decl_data.id,
      ...
      /* Generics don't go in symbol table directly, only specialized instances
       * do */
    ...
    if (debug_env_check != NULL && return_val > before_switch_errors) {
      fprintf(stderr,
              "[KGPC] Error in switch for type %s (kind=%d), was %d now %d\n",
              tree->tree_data.type_decl_data.id
                  ? tree->tree_data.type_decl_data.id
                  : "<null>",
              tree->tree_data.type_decl_data.kind, before_switch_errors,
              return_val);
    }
    ...
    if (record_info != NULL && record_type_is_class(record_info) &&
      ...
      /* Classes are reference types - represent them as pointers to the class
       * record */
      KgpcType *wrapped = create_pointer_type(kgpc_type);
      kgpc_type_release(
          tree->tree_data.type_decl_data.kgpc_type); /* release old tree ref */
      kgpc_type = wrapped;
    ...
    if (tree->tree_data.type_decl_data.kind == TYPE_DECL_ALIAS &&
      ...
      if (!need_rebuild && kgpc_type->kind == TYPE_KIND_PRIMITIVE &&
        ...
        need_rebuild = 1;
      if (need_rebuild) {
        KgpcType *rebuilt = create_kgpc_type_from_type_alias(
            alias_info, symtab, tree->tree_data.type_decl_data.defined_in_unit);
        if (rebuilt != NULL) {
          if (kgpc_type != NULL)
      ...
      if (trace_sym != NULL && tree->tree_data.type_decl_data.id != NULL &&
          pascal_identifier_equals(tree->tree_data.type_decl_data.id,
                                   trace_sym)) {
        fprintf(stderr,
                "[KGPC_TRACE_NONLOCAL] sem_type_decl id=%s alias target=%s "
                "is_array=%d [%d,%d] short=%d kgpc=%p kind=%d\n",
                tree->tree_data.type_decl_data.id,
                alias_info->target_type_id != NULL ? alias_info->target_type_id
                                                   : "<null>",
                alias_info->is_array, alias_info->array_start,
                alias_info->array_end, alias_info->is_shortstring,
                (void *)kgpc_type, kgpc_type != NULL ? kgpc_type->kind : -1);
      }
    }

    /* Check if this type was already pre-declared by predeclare_types().
     * If so, skip the push to avoid "redeclaration" errors.
     * IMPORTANT: Only skip if the type exists in the CURRENT scope (scope level
     * 0), not if it exists as a builtin or in a parent scope. Also don't treat
     * as predeclared if existing is from a different source unit. */
    HashNode_t *existing_type =
        FindIdentInCurrentScope(symtab, tree->tree_data.type_decl_data.id);
    int already_predeclared =
        (existing_type != NULL && existing_type->hash_type == HASHTYPE_TYPE);
    if (debug_pss != NULL && tree->tree_data.type_decl_data.id != NULL &&
        pascal_identifier_equals(tree->tree_data.type_decl_data.id,
                                 "PShortString")) {
      fprintf(stderr, "[PShortString] predeclared=%d existing_type=%p\n",
              already_predeclared, (void *)existing_type);
      if (existing_type != NULL) {
        fprintf(stderr,
                "[PShortString] existing defined_in_unit=%d unit_is_public=%d "
                "source_unit_index=%d\n",
     ...
     * coexistence, multiple entries with the same name may exist in the same
     * scope. This handles:
    ...
    if (already_predeclared &&
      ...
      ListNode_t *all_matches =
          FindAllIdents(symtab, tree->tree_data.type_decl_data.id);
      ...
      if (same_unit_entry != NULL)
        ...
        if (tree->tree_data.type_decl_data.kind == TYPE_DECL_ALIAS &&
            (existing_is_record || current_is_enum_alias ||
             existing_is_enum_alias) &&
            existing_type->defined_in_unit &&
            existing_type->source_unit_index != 0 &&
            tree->tree_data.type_decl_data.source_unit_index != 0)
    ...
    /* Skip symbol table registration for generic declarations - they're only in
     * the registry */
    if (tree->tree_data.type_decl_data.kind == TYPE_DECL_GENERIC) {
      ...
      if (debug_pss != NULL && tree->tree_data.type_decl_data.id != NULL &&
          pascal_identifier_equals(tree->tree_data.type_decl_data.id,
                                   "PShortString")) {
        fprintf(stderr,
                "[PShortString] already_predeclared kgpc_type=%p "
                "existing_type->type=%p\n",
                (void *)kgpc_type,
                existing_type ? (void *)existing_type->type : NULL);
      }
      mark_hashnode_source_unit(
          existing_type, tree->tree_data.type_decl_data.source_unit_index);

      /* Type was already registered by predeclare_types().
       * We still need to update any additional metadata like array bounds. */
      if (tree->tree_data.type_decl_data.kind == TYPE_DECL_ALIAS &&
          alias_info != NULL && alias_info->is_pointer && kgpc_type != NULL &&
          kgpc_type_is_pointer(kgpc_type) &&
          (existing_type->type == NULL ||
           !kgpc_type_is_pointer(existing_type->type))) {
        /* Replace predeclared non-pointer placeholder with proper pointer alias
         * type. Properly manage ref counts: retain new, release old. */
        ...
        if (debug_pss != NULL && tree->tree_data.type_decl_data.id != NULL &&
          ...
          fprintf(stderr, "[PShortString] updated existing pointer type=%p\n",
                 ...
                 kgpc_type != NULL && existing_type->type != NULL &&
         ...
         * real target type becomes available (e.g. SysUtils.TEndian ->
         * ObjPas.TEndian). */
        kgpc_type_retain(kgpc_type);
        kgpc_type_release(existing_type->type);
        existing_type->type = kgpc_type;
      } else if (tree->tree_data.type_decl_data.kind == TYPE_DECL_ALIAS &&
                 kgpc_type != NULL && kgpc_type->kind == TYPE_KIND_PROCEDURE &&
                 existing_type->type != NULL &&
                 existing_type->type->kind == TYPE_KIND_PRIMITIVE &&
      ...
      if (tree->tree_data.type_decl_data.kind == TYPE_DECL_ALIAS &&
          alias_info != NULL && existing_type->type != NULL) {
        struct TypeAlias *existing_alias =
        ...
        if (existing_alias != NULL && existing_alias != alias_info &&
          ...
          if (!existing_is_unknown_placeholder &&
            ...
            /* Avoid overwriting unrelated alias metadata when reusing another
             * type. */
        ...
        if (existing_alias != NULL) {
          ...
          if (!alias_info->is_range && existing_alias->is_range &&
              existing_alias->range_known) {
        ...
        if (can_override_alias) {
          ...
          if (existing_type->type->type_alias != NULL &&
            ...
            existing_type->type->type_alias->storage_size =
                alias_info->storage_size;
        } else {
          inherit_alias_metadata(symtab, alias_info);
        }

        /* Resolve array bounds from constant identifiers now that constants are
         * in scope */
        if (alias_info->is_array) {
          resolve_array_bounds_in_kgpctype(symtab, existing_type->type,
                                           alias_info);
        }
        if (alias_info->is_range) {
          resolve_alias_range_bounds_in_kgpctype(symtab, existing_type->type,
                                                 alias_info);
        }
        if (alias_info->is_pointer &&
            kgpc_type_is_pointer(existing_type->type) &&
      ...
      if (tree->tree_data.type_decl_data.kind == TYPE_DECL_ALIAS &&
         ...
         * This is critical for types like WideChar (2 bytes) that are
         * represented as INT_TYPE (4 bytes) in the primitive type system but
         * have a custom storage_size defined. Without this, SizeOf(TMyChar)
         * where TMyChar = WideChar would return 4 instead of the correct 2
         * bytes. */
        inherit_alias_metadata(symtab, alias_info);
        if (can_override_alias) {
          kgpc_type_set_type_alias(kgpc_type, alias_info);
        }
        if (can_override_alias && kgpc_type_get_type_alias(kgpc_type) != NULL &&
            alias_info->storage_size > 0) {
          kgpc_type_get_type_alias(kgpc_type)->storage_size =
              alias_info->storage_size;
        }
      ...
      if (tree->tree_data.type_decl_data.kind == TYPE_DECL_RECORD &&
          record_info != NULL && kgpc_type->kind == TYPE_KIND_RECORD)
        kgpc_type->info.record_info = record_info;

      if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
        fprintf(
            stderr,
            "[SemCheck] Pushing type '%s' onto scope, kgpc_type=%p kind=%d\n",
            tree->tree_data.type_decl_data.id, (void *)kgpc_type,
            kgpc_type ? kgpc_type->kind : -1);
      }
      func_return = PushTypeOntoScope_Typed(
          symtab, tree->tree_data.type_decl_data.id, kgpc_type);
      if (debug_pss != NULL && tree->tree_data.type_decl_data.id != NULL &&
          pascal_identifier_equals(tree->tree_data.type_decl_data.id,
                                   "PShortString")) {
        fprintf(
            stderr,
            "[PShortString] PushTypeOntoScope_Typed result=%d kgpc_type=%p\n",
            func_return, (void *)kgpc_type);
      }
      if (func_return == 0) {
        /* Hash table retained its own reference. Release the tree/creator
         * reference. */
        if (tree->tree_data.type_decl_data.kgpc_type != NULL)
          destroy_kgpc_type(tree->tree_data.type_decl_data.kgpc_type);
        tree->tree_data.type_decl_data.kgpc_type = NULL;
        /* Note: var_type is automatically set from KgpcType in HashTable.c via
         * set_var_type_from_kgpctype() */
      }
    } else {
      /* Fall back to legacy API for types we can't convert yet */
      func_return = PushTypeOntoScope(symtab, tree->tree_data.type_decl_data.id,
                                      var_type, record_info, alias_info);
      if (debug_pss != NULL && tree->tree_data.type_decl_data.id != NULL &&
          pascal_identifier_equals(tree->tree_data.type_decl_data.id,
                                   "PShortString")) {
    ...
    /* Note: Enum literals are declared in predeclare_enum_literals() during
     * first pass. We don't redeclare them here to avoid "redeclaration" errors.
     */
    ...
    if (func_return > 0) {
      ...
      if (already_predeclared) {
        ...
        if (type_node == NULL || type_node->hash_type != HASHTYPE_TYPE)
          type_node = semcheck_find_type_node_with_unit_flag(
      ...
      if (type_node != NULL) {
        ...
        mark_hashnode_source_unit(
            type_node, tree->tree_data.type_decl_data.source_unit_index);
      }

      /* Register nested types under their short name too.
       * E.g., "TCgExceptionStateHandler.TExceptionTemps" also as
       * "TExceptionTemps". */
      {
        ...
        if (type_id != NULL) {
          ...
          if (last_dot != NULL && last_dot[1] != '\0') {
            ...
            if (FindSymbol(&existing_short, symtab, short_name) == 0) {
              KgpcType *short_type =
                  type_node != NULL ? type_node->type : kgpc_type;
              if (short_type != NULL)
      ...
      if (tree->tree_data.type_decl_data.kind == TYPE_DECL_ALIAS &&
          alias_info != NULL && alias_info->inline_record_type != NULL &&
        ...
        if (FindSymbol(&existing, symtab, mangled_name) == 0) {
          ...
          KgpcType *inline_kgpc_type =
              create_record_type(alias_info->inline_record_type);
          if (inline_kgpc_type != NULL) {
            int push_result = PushTypeOntoScope_Typed(
                symtab, (char *)mangled_name, inline_kgpc_type);
            if (push_result == 0) {
              if (debug_env2 != NULL) {
                fprintf(
                    stderr,
                    "[KGPC] Registered inline record type %s for alias %s\n",
    ...
    if (debug_env2 != NULL && return_val > before_symtab_errors) {
      fprintf(stderr,
              "[KGPC] Error increased in type %s (kind=%d), was %d now %d\n",
              tree->tree_data.type_decl_data.id
                  ? tree->tree_data.type_decl_data.id
                  : "<null>",
              tree->tree_data.type_decl_data.kind, before_symtab_errors,
              return_val);
    }
...
#ifdef DEBUG
      fprintf(stderr,
              "DEBUG: semcheck_type_decls ERROR in type %s: was %d now %d\n",
              tree->tree_data.type_decl_data.id
                  ? tree->tree_data.type_decl_data.id
                  : "<null>",
  ...
  while (cur != NULL) {
    ...
    HashNode_t *alias_node = semcheck_find_type_node_with_unit_flag(
        symtab, tree->tree_data.type_decl_data.id,
        tree->tree_data.type_decl_data.defined_in_unit);
    ...
    if (post_alias->pointer_type_ref != NULL) {
      pointee_node = semcheck_find_preferred_type_node_with_ref(
          symtab, post_alias->pointer_type_ref, post_alias->pointer_type_id);
    } else {
      pointee_node = semcheck_find_preferred_type_node(
