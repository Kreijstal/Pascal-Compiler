 ...
 */
extern int resolve_const_int_from_ast(const char *identifier,
                                      void *const_section, int fallback_value);
 ...
 * O(1) pointer-hash cache: all callers pass string literals with stable
 * addresses. */
...
int semcheck_register_source_buffer(const char *path, const char *buffer,
                                    size_t length) {
  ...
  if (g_source_buffer_registry[g_source_buffer_count].path &&
      g_source_buffer_registry[g_source_buffer_count].buffer) {
    memcpy(g_source_buffer_registry[g_source_buffer_count].buffer, buffer,
           length);
    ...
    g_source_buffer_next_global =
        global_start + (int)length + 1; /* +1 to avoid overlap */
    ...
    if (kgpc_getenv("KGPC_DEBUG_MEMBUF") != NULL) {
      ...
      fprintf(
          stderr,
          "[MEMBUF] source_buffer_registry: %d entries, %zu bytes (%.1f MB)\n",
...
static int semcheck_print_context_from_file(const char *file_path, int line_num,
                                            int col_num, int context_lines) {
  ...
  int printed = print_source_context_from_buffer(buffer, read_len, line_num,
                                                 col_num, context_lines);
  free(buffer);
  return printed;
}
...
void semcheck_mark_type_decl_units(ListNode_t *type_decls, int unit_index) {
  ...
  for (ListNode_t *cur = type_decls; cur != NULL; cur = cur->next) {
    ...
    if (tree->tree_data.type_decl_data.kind == TYPE_DECL_RECORD &&
      ...
      tree->tree_data.type_decl_data.info.record->source_unit_index =
          unit_index;
    }
  }
}
...
void semcheck_mark_resolved_forward_stub(
    ListNode_t *type_decls, ListNode_t *limit, const char *type_id,
    int source_unit_index, const struct RecordType *canonical_record) {
  if (type_decls == NULL || type_id == NULL || canonical_record == NULL)
    return;

  for (ListNode_t *cur = type_decls; cur != NULL && cur != limit;
       cur = cur->next) {
    if (cur->type != LIST_TREE || cur->cur == NULL)
      continue;
...
struct RecordType *semcheck_record_from_type_decl(Tree_t *tree,
                                                  SymTab_t *symtab) {
  ...
  if (tree->tree_data.type_decl_data.kind == TYPE_DECL_RECORD) {
    ...
    if (record != NULL && record->is_class && record->fields == NULL &&
        record->parent_fields_merged && symtab != NULL &&
        tree->tree_data.type_decl_data.id != NULL) {
...
static int
semcheck_record_candidate_is_forward_stub(struct RecordType *record) {
  if (record == NULL)
    return 0;
...
/* type_ref_render_mangled_unqualified is defined as static inline in
 * SemCheck_internal.h */
...
void semcheck_maybe_qualify_nested_type(SymTab_t *symtab,
                                        ...
                                        const char *owner_outer, char **type_id,
                                        TypeRef **type_ref) {
  if (symtab == NULL || owner_full == NULL || type_id == NULL ||
      *type_id == NULL)
    return;
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    if (qualified_name == NULL)
      continue;
    snprintf(qualified_name, owner_len + 1 + type_len + 1, "%s.%s", owner,
             *type_id);
...
void semcheck_qualify_nested_types_for_record(SymTab_t *symtab,
...
void semcheck_assert_pointer_type_resolved(const char *id) {
  ...
  static const char *expected_pointer_types[] = {
      "PInt64",    "PByte",  "PWord",    "PLongInt", "PLongWord", "PInteger",
      "PCardinal", "PQWord", "PPointer", "PBoolean", "PComp",     NULL};
  for (int i = 0; expected_pointer_types[i] != NULL; i++) {
    if (pascal_identifier_equals(id, expected_pointer_types[i])) {
      fprintf(stderr,
              "[KGPC] WARNING: pointer type '%s' not resolved through "
...
int semcheck_scope_level_for_type_candidate(SymTab_t *symtab,
  ...
  for (ScopeNode *scope = symtab->current_scope;
       scope != NULL && scope != symtab->builtin_scope;
...
static int semcheck_is_char_alias_name(const char *id) {
  return (id != NULL && (pascal_identifier_equals(id, "Char") ||
                         pascal_identifier_equals(id, "AnsiChar") ||
                         pascal_identifier_equals(id, "WideChar") ||
                         pascal_identifier_equals(id, "UnicodeChar")));
}

int semcheck_alias_should_be_char_like(const char *alias_id,
...
int semcheck_alias_targets_match(const struct TypeAlias *lhs,
                                 const struct TypeAlias *rhs) {
  if (lhs == NULL || rhs == NULL)
    return 0;

  if (lhs->target_type_ref != NULL && rhs->target_type_ref != NULL &&
      lhs->target_type_ref->name != NULL &&
      rhs->target_type_ref->name != NULL &&
      qualified_ident_equals_ci(lhs->target_type_ref->name,
                                rhs->target_type_ref->name)) {
    return 1;
  }
...
HashNode_t *
semcheck_find_type_excluding_alias(SymTab_t *symtab, const char *type_id,
...
HashNode_t *semcheck_find_owner_record_type_node(SymTab_t *symtab,
                                                 const char *owner_id) {
    ...
    if (candidate != NULL && candidate->hash_type == HASHTYPE_TYPE) {
      ...
      if (get_record_type_from_node(candidate) != NULL) {
        ...
        if (alias != NULL && alias->target_type_id != NULL) {
          ...
          if (target_node != NULL &&
              get_record_type_from_node(target_node) != NULL)
      ...
      if (record_node != NULL) {
        if (best_record == NULL) {
          ...
          int best_is_class =
              (best_rec != NULL && record_type_is_class(best_rec));
          int cand_is_class =
              (cand_rec != NULL && record_type_is_class(cand_rec));
          if (!best_is_class && cand_is_class) {
            ...
            if (prefer_unit_defined) {
              if (!best_record->defined_in_unit && record_node->defined_in_unit)
                best_record = record_node;
            } else if (best_record->defined_in_unit &&
                       !record_node->defined_in_unit) {
              best_record = record_node;
            }
          }
        }
...
int semcheck_param_decl_equivalent(const Tree_t *lhs, const Tree_t *rhs) {
  ...
  if (lhs->type == TREE_VAR_DECL) {
    ...
    if (lhs_id != NULL || rhs_id != NULL)
      return (lhs_id != NULL && rhs_id != NULL &&
              pascal_identifier_equals(lhs_id, rhs_id));
...
int semcheck_subprogram_signatures_equivalent(const Tree_t *lhs,
                                              const Tree_t *rhs) {
  if (lhs == NULL || rhs == NULL)
    return 0;
...
ListNode_t *semcheck_create_builtin_param(const char *name, int type_tag) {
  ...
  Tree_t *decl =
      mk_vardecl(0, ids, type_tag, NULL, 0, 0, NULL, NULL, NULL, NULL);
  if (decl == NULL) {
    destroy_list(ids);
    return NULL;
  }
...
ListNode_t *semcheck_create_builtin_param_var(const char *name, int type_tag) {
  ...
  Tree_t *decl =
      mk_vardecl(0, ids, type_tag, NULL, 1, 0, NULL, NULL, NULL, NULL);
  if (decl == NULL) {
    destroy_list(ids);
    return NULL;
  }
...
__attribute__((unused)) static ListNode_t *
semcheck_create_builtin_param_with_id(const char *name, int type_tag,
  ...
  Tree_t *decl = mk_vardecl(0, ids, type_tag, type_id_copy, is_var_param, 0,
                            NULL, NULL, NULL, NULL);
  if (decl == NULL) {
    destroy_list(ids);
...
static void print_error_context(int line_num, int col_num, int source_index,
                                const char *directive_file) {
  const char *file_path =
      (directive_file != NULL && directive_file[0] != '\0')
          ? directive_file
          : ((file_to_parse != NULL && *file_to_parse != '\0')
                 ? file_to_parse
                 : ((preprocessed_path != NULL && *preprocessed_path != '\0')
                        ? preprocessed_path
  ...
  if (line_num > 0) {
    ...
    if (kgpc_getenv("KGPC_DEBUG_SEM_CONTEXT") != NULL) {
      fprintf(stderr,
              "[SemCheck] context file=%s pre_len=%zu buf_len=%zu line=%d "
              "col=%d offset=%d\n",
              file_path != NULL ? file_path : "<null>", context_len,
              context_buf_len, line_num, col_num, source_index);
    }
    ...
    if (directive_file[0] != '\0')
      printed = semcheck_print_context_from_file(directive_file, line_num,
                                                 col_num, 2);
    if (!printed)
      printed = print_source_context_at_offset(
          context_buf, context_buf_len, source_index, line_num, col_num, 2);
    if (!printed)
      printed =
          semcheck_print_context_from_file(file_path, line_num, col_num, 2);
...
static int semcheck_parse_line_directive(const char *line, size_t len,
  ...
  if (pos < len && line[pos] == '"' && filename_out != NULL &&
      filename_size > 0) {
...
static int semcheck_line_from_source_offset(const char *buffer, size_t length,
                                            int source_offset, char *file_out,
                                            size_t file_out_size) {
  if (file_out != NULL && file_out_size > 0)
    file_out[0] = '\0';

  if (buffer == NULL || length == 0 || source_offset < 0 ||
      (size_t)source_offset >= length)
    return -1;
  ...
  while (scan_pos > 0) {
    ...
    int directive_line = semcheck_parse_line_directive(
        buffer + line_start, line_len, file_out, file_out_size);
    if (directive_line >= 0) {
...
/* Find the file from a source byte offset by scanning backwards for line
 * directives */
__attribute__((unused)) static void
semcheck_file_from_source_index(int source_index, char *file_out,
                                size_t file_out_size) {
  if (file_out == NULL || file_out_size == 0)
    return;
  ...
  while (scan_pos > 0) {
    ...
    int directive_line = semcheck_parse_line_directive(
        buffer + line_start, line_len, directive_file, sizeof(directive_file));
    if (directive_line >= 0) {
      if (directive_file[0] != '\0') {
 ...
 */
static void semcheck_file_from_buffer_line_number(int line_num, char *file_out,
                                                  size_t file_out_size) {
...
static int eval_const_expr_parse_additive(SymTab_t *symtab, const char **p,
                                          long long *out_value);

static int eval_const_expr_parse_primary(SymTab_t *symtab, const char **p,
                                         long long *out_value) {
  ...
  if (match_const_expr_keyword(p, "sizeof")) {
    ...
    int ok = (sizeof_from_type_ref(symtab, UNKNOWN_TYPE, type_id, &size_val, 0,
                                   0) == 0);
    free(type_id);
    if (!ok)
      return -1;
...
static int eval_const_expr_parse_unary(SymTab_t *symtab, const char **p,
                                       long long *out_value) {
  if (p == NULL || *p == NULL || out_value == NULL)
    return -1;
...
static int eval_const_expr_parse_multiplicative(SymTab_t *symtab,
                                                const char **p,
                                                long long *out_value) {
...
int resolve_array_bound_expr(SymTab_t *symtab, const char *expr,
                             int *out_value) {
...
int semcheck_tag_from_kgpc(const KgpcType *type) {
  ...
  /* Check for ShortString: array[0..255] of Char or explicit is_shortstring
   * flag */
...
void semcheck_debug_error_step(const char *step, Tree_t *subprogram, int before,
                               int after) {
  ...
  fprintf(stderr,
          "[KGPC_DEBUG_ERRORS] sub=%s sub_line=%d step=%s delta=%d err_line=%d "
          "err_col=%d err_src=%d\n",
          sub_id != NULL ? sub_id : "<null>", sub_line,
          step != NULL ? step : "<null>", after - before, g_semcheck_error_line,
          g_semcheck_error_col, g_semcheck_error_source_index);
}
...
int resolve_error_source_context(int source_index, char *directive_file_out,
                                 size_t dir_size, int search_registry) {
    ...
    for (int i = 0; i < g_source_buffer_count; i++) {
      ...
      if (source_index >= gs && source_index < gs + len) {
        ...
        int computed_line = semcheck_line_from_source_offset(
            ...
            g_source_buffer_registry[i].length, local_offset, temp_file,
            sizeof(temp_file));
        if (computed_line > 0) {
          if (temp_file[0] != '\0') {
            ...
            strncpy(directive_file_out, g_source_buffer_registry[i].path,
                    dir_size - 1);
            directive_file_out[dir_size - 1] = '\0';
          }
  ...
  int computed_line = semcheck_line_from_source_offset(
      context_buf, context_buf_len, source_index, directive_file_out, dir_size);
...
int resolve_unit_error_location(int source_index, int line_num,
  ...
  if (unit_chain_out != NULL && chain_size > 0 &&
      g_semcheck_error_unit_name != NULL) {
...
void semantic_error(int line_num, int col_num, const char *format, ...) {
  ...
  if (g_semcheck_error_unit_name != NULL) {
    int resolved_line = resolve_unit_error_location(
        effective_source_index, effective_line, directive_file,
        sizeof(directive_file), unit_chain, sizeof(unit_chain));
    if (resolved_line > 0)
    ...
    semcheck_file_from_buffer_line_number(effective_line, directive_file,
                                          sizeof(directive_file));
  }

  const char *file_path =
      (directive_file[0] != '\0') ? directive_file : semcheck_get_error_path();
...
/* Helper function to print semantic error with accurate source context using
 * byte offset */
void semantic_error_at(int line_num, int col_num, int source_index,
                       const char *format, ...) {
  ...
  if (g_semcheck_error_unit_name != NULL) {
    int resolved_line = resolve_unit_error_location(
        source_index, line_num, directive_file, sizeof(directive_file),
    ...
    /* Look up file from line number when source_index is not available */
    semcheck_file_from_buffer_line_number(line_num, directive_file,
                                          sizeof(directive_file));
  }
...
static void
v_semcheck_format_error_with_context(const char *file_path, int effective_line,
  ...
  if (effective_line > 0) {
    ...
    if (kgpc_getenv("KGPC_DEBUG_SEM_CONTEXT") != NULL) {
      fprintf(stderr,
              ...
              "local_offset=%d\n",
              file_path != NULL ? file_path : "<null>", context_buf_len,
              effective_line, effective_col, source_index, local_source_offset);
    }
    int printed = 0;
    ...
    if (local_source_offset >= 0 &&
      ...
      printed = print_source_context_at_offset(
          context_buf, context_buf_len, local_source_offset, effective_line,
          effective_col, 2);
    }
    /* Fall back to reading the original source file from disk — this
     * avoids the #line-directive ambiguity in preprocessed buffers. */
    if (!printed)
      printed = semcheck_print_context_from_file(file_path, effective_line,
                                                 effective_col, 2);
    if (!printed && file_path != NULL)
...
void semcheck_error_with_context(const char *format, ...) {
  ...
  if (g_semcheck_error_unit_name != NULL) {
    int resolved_line = resolve_unit_error_location(
        effective_source_index, line_num, directive_file,
        sizeof(directive_file), unit_chain, sizeof(unit_chain));
    if (resolved_line > 0)
      effective_line = resolved_line;
  } else if (effective_source_index >= 0) {
...
int semcheck_id_not_main(char *id) {
  if (pascal_identifier_equals(id, "main")) {
    fprintf(stderr, "ERROR: main is special keyword, it cannot be a program, "
                    "or subprogram\n");
    return 1;
  }
  return 0;
