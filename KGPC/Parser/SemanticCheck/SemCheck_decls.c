...
int resolve_scoped_enum_literal(SymTab_t *symtab, const char *type_name,
...
int resolve_range_bounds_for_type(SymTab_t *symtab, const char *type_name,
...
ListNode_t *clone_property_list_unique(struct RecordType *record_info,
...
int check_circular_inheritance(SymTab_t *symtab, const char *class_name, const char *parent_name, int max_depth)
{
    ...
    struct RecordType *parent_record = hashnode_get_record_type_extended(parent_node);
...
int build_class_vmt(SymTab_t *symtab, struct RecordType *record_info,
        ...
        while(ids != NULL)
        {
            ...
            if(tree->type == TREE_VAR_DECL)
            {
                ...
                if (tree->tree_data.var_decl_data.type_id != NULL)
                {
                    ...
                    else
                    {
                        ...
                        if (needs_inline_pointer)
                        {
                            ...
                            {
                                struct RecordType *target_record = hashnode_get_record_type_extended(type_node);
                ...
                else
                {
                    ...
                    {
                        struct TypeAlias *type_alias = hashnode_get_type_alias(resolved_type);
                        if (type_alias != NULL)
                        {
                            kgpc_type_set_type_alias(var_kgpc_type, type_alias);
                        }
                        struct RecordType *record_type = hashnode_get_record_type_extended(resolved_type);
                ...
                if (!is_array_of_const && tree->tree_data.arr_decl_data.type_id != NULL)
                {
                    ...
                    if (element_type_node != NULL)
                    {
                        ...
                        if (element_type == NULL)
                        {
                            ...
                            struct RecordType *record_type = hashnode_get_record_type_extended(element_type_node);
                            if (record_type != NULL)
                            {
                                /* Use the canonical RecordType, not a clone */
