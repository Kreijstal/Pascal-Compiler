...
struct TypeAlias {
  ...
  int array_element_storage_size; /* For shortstring elements: N+1 bytes for
                                     string[N] */
  int is_shortstring;
  int is_wide_string;
  int is_open_array;
  ListNode_t *array_dimensions;
  char *array_dim_start_str; /* Symbolic lower bound (e.g. "Low(T)"), NULL if
                                numeric */
  char *array_dim_end_str;   /* Symbolic upper bound (e.g. "High(T)"), NULL if
                                numeric */
  int array_dims_parsed;     /* 1 if start_str/end_str were extracted from
                                dimensions */
  int is_pointer;
...
struct GenericDecl {
  ...
  struct ast_t
      *original_ast; /* Pointer to original AST node for substitution */
  struct RecordType
      *record_template; /* Optional record/class template for generics */
};
...
struct RecordField {
  ...
  struct TypeRef *array_element_type_ref;
  struct RecordType
      *array_element_record; /* Anonymous record as array element type */
  struct KgpcType *array_element_kgpc_type; /* Pre-built element type for nested
                                               arrays (array of array of ...) */
  int array_is_open;
  int is_hidden;
  int is_class_var;
  int is_pointer;
  ...
  char *pointer_type_id;
  struct TypeRef *pointer_type_ref;
  ListNode_t *enum_literals; /* Anonymous enum values for fields like `kind: (a,
                                b, c)` */
  char *set_element_type_id; /* For set fields: element type name (e.g.
                                "tsystemflags") */
  long long
      cached_size;      /* Cached field size (valid when has_cached_layout=1) */
  int cached_alignment; /* Cached field alignment (valid when
                           has_cached_layout=1) */
  int has_cached_layout; /* 1 if cached_size and cached_alignment are valid */
  long long cached_proc_return_sret_size; /* Cached sret return size for
                                             PROCEDURE fields; set on first
                                             successful proc_type resolution */
  struct KgpcType
      *cached_proc_return_kgpc_type; /* Retained return KgpcType; survives
                                        proc_type being freed (MSYS2 UAF) */
};
...
struct MethodInfo {
  ...
  char *resolved_mangled_id;    /* Fully resolved mangled ID for codegen (set by
                                   semcheck) */
};
...
struct RecordType {
  ...
  char *helper_parent_id; /* Parent helper type name (for inheritance like "type
                             helper(Parent) for X") */
  char *type_id;          /* Canonical type name if available */
  char *outer_type_id;    /* Outer class for nested types (e.g. "TOuter" for
                             "TOuter.TInner"), NULL if not nested */
  int parent_fields_merged; /* 1 if parent class fields have been merged into
                               fields */
  ...
  char *default_indexed_property;   /* Field name for default indexed access
                                       (e.g., "FItems" for array-like classes) */
  int default_indexed_element_type; /* Type tag for elements of the default
                                       indexed property */
  char *default_indexed_element_type_id; /* Type identifier for elements of the
                                            default indexed property */
  ListNode_t
      *record_properties; /* Properties on plain records (Delphi advanced
                             records), not checked by record_type_is_class */
  char
      *guid_string; /* GUID string for interfaces, e.g. "{12345678-1234-...}" */
  ...
  int source_unit_index;  /* Unit registry index for the unit that defined this
                             record (0 = none) */
};
...
struct CaseBranch {
  ListNode_t *labels; /* List of expressions or range elements representing case
                         labels */
  struct Statement *stmt;
};

enum AsmSyntaxMode { ASM_SYNTAX_ATT = 0, ASM_SYNTAX_INTEL = 1 };

/* A statement subtree */
struct Statement {
  int line_num;
  int col_num;
  int source_index; /* Byte offset in preprocessed buffer for accurate error
                       context (-1 if unknown) */
  int source_unit_index; /* Unit registry index for error context (0 =
                            local/program) */
  enum StmtType type;
  union stmt_data {
    /* Variable assignment */
    ...
    struct ProcedureCall {
      ...
      struct HashNode *resolved_proc; /* DEPRECATED: May point to freed memory!
                                         Use fields below instead. */

      /* Cached information from HashNode to avoid use-after-free.
       * These are populated during semantic checking and remain valid for
       * codegen. */
      int call_hash_type; /* HashType enum value (HASHTYPE_VAR,
                             HASHTYPE_PROCEDURE, etc.) */
      struct KgpcType
          *call_kgpc_type;    /* KgpcType for getting formal parameters */
      int is_call_info_valid; /* 1 if the above fields are valid, 0 otherwise */
      char *cached_owner_class;   /* Cached owner class from resolved proc (NULL
                                     if not a method) */
      char *cached_method_name;   /* Cached bare method name from resolved proc
                                     (NULL if not a method) */
      int is_procedural_var_call; /* 1 if calling through a procedural
                                     variable/expression */
      struct HashNode
          *procedural_var_symbol; /* Symbol for procedural var (if any) */
      struct Expression
          *procedural_var_expr; /* Expression yielding procedure pointer */
      int is_method_call_placeholder;    /* 1 if created from member access and
                                            needs method resolution */
      char *placeholder_method_name;     /* Bare method name when
                                            is_method_call_placeholder=1 (e.g.
                                            "Create") */
      int is_tp_new_dispose_helper_call; /* 1 if synthesized from TP-style
                                            New/Dispose helper lowering */
      int arg0_is_dynarray_descriptor; /* 1 if arg0 should be passed as dynarray
                                          descriptor */
      int is_virtual_call;   /* 1 if this is a virtual method call (needs VMT
                                dispatch) */
      int is_interface_call; /* 1 if this is an interface method call (needs
      ...
      int is_class_method_call; /* 1 if calling a class method (Self = VMT, not
                                   instance) */
      int is_constructor_call;  /* 1 if this is a class constructor call (needs
                                   object allocation) */
      char *constructor_class_name; /* Class to allocate for constructor calls
                                       (NULL if not constructor) */
      char *call_qualifier; /* Unit prefix if call was qualified, e.g. "System"
                               (NULL if unqualified) */
    } procedure_call_data;
    ...
    struct ForIn {
      ...
      struct Expression
          *collection; /* Collection to iterate over (array, set, etc.) */
    ...
    struct TryExcept {
      ...
      char *exception_var_name;      /* Variable name from 'on E: Exception do'
                                        (optional) */
      char *exception_type_name;     /* Type name from 'on E: Exception do'
                                        (optional) */
...
struct Expression {
  ...
  struct QualifiedIdent
      *id_ref; /* Structured identifier for EXPR_VAR_ID (optional) */
  enum ExprType type;
  union expr_data {
    ...
    struct ArrayAccess {
      ...
      ListNode_t *extra_indices; /* For multi-dimensional arrays: additional
                                    indices beyond the first */
      int linear_index_count; /* Total number of indices (first + extra_indices)
                               */
      long long *linear_strides; /* Byte stride for each index (length
                                    linear_index_count) */
      long long *linear_lowers;  /* Lower bounds for each index (length
                                    linear_index_count) */
    ...
    struct FunctionCall {
      ...
      struct HashNode *resolved_func; /* DEPRECATED: may be invalid after
                                         leaving semantic scope. */
      ...
      char *cached_owner_class; /* Cached owner class from resolved func (NULL
                                   if not a method) */
      char *cached_method_name; /* Cached bare method name from resolved func
                                   (NULL if not a method) */
      ...
      int is_procedural_var_call; /* 1 if calling through a procedural variable
                                   */
      struct HashNode
          *procedural_var_symbol; /* Symbol for the procedural variable */
      struct Expression
          *procedural_var_expr; /* Expression yielding a function pointer (for
                                   record fields, etc.) */
      int is_method_call_placeholder; /* 1 if created from member access and
                                         needs method resolution */
      char *placeholder_method_name;  /* Bare method name when
                                         is_method_call_placeholder=1 (e.g.
                                         "Create") */
      int is_constructor_call;        /* 1 if this call was resolved as a class
                                         constructor */
      int is_virtual_call;   /* 1 if this is a virtual method call (needs VMT
                                dispatch) */
      int is_interface_call; /* 1 if this is an interface method call (needs
      ...
      int is_class_method_call; /* 1 if calling a class method (Self = VMT, not
                                   instance) */
      struct Expression
          *constructor_receiver_expr;  /* Original explicit constructor receiver
                                          for codegen */
      int arg0_is_dynarray_descriptor; /* 1 if arg0 should be passed as dynarray
                                          descriptor */
      char *call_qualifier;  /* Unit/object prefix if call was qualified, e.g.
                                "SysUtils" (NULL if unqualified) */
      int is_inherited_call; /* 1 if this is an "inherited MethodName(args)"
                                call */
      int is_bare_inherited; /* 1 if bare "inherited" (no explicit method name)
                                — forward enclosing args */
      int is_operator_call;  /* 1 if this call targets an operator (set by
                                parser) */
      enum BuiltinCallLowering
          builtin_call_lowering; /* Special lowering rule set by semcheck
                                    builtins */
      /* Cached sret size for proc-var calls returning records, set during
       * semcheck. Avoids relying on call_kgpc_type pointer validity at codegen
       * time. */
    ...
    struct RecordConstructor {
      ListNode_t *fields; /* LIST_UNSPECIFIED nodes pointing to
    ...
    struct TypeCast {
      int target_type;
      char *target_type_id;
      char *type_qualifier; /* Unit prefix for qualified types, e.g. "SysUtils"
                               (NULL if unqualified) */
      struct TypeRef *target_type_ref;
      struct Expression *expr;
    } typecast_data;
