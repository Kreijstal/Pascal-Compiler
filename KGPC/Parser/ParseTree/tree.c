...
static void print_record_field(struct RecordField *field, FILE *f,
                               int num_indent);
static void print_class_property(struct ClassProperty *property, FILE *f,
                                 int num_indent);
static void destroy_record_field(struct RecordField *field);
static void print_variant_part(struct VariantPart *variant, FILE *f,
                               int num_indent);
static void print_variant_branch(struct VariantBranch *branch, FILE *f,
                                 int num_indent);
...
static struct MethodTemplate *
clone_method_template(const struct MethodTemplate *method);
...
static struct VariantBranch *
clone_variant_branch_internal(const struct VariantBranch *branch);
static struct VariantPart *
clone_variant_part_internal(const struct VariantPart *variant);
  ...
  while (cur != NULL) {
    ...
    case LIST_METHOD_TEMPLATE: {
      ...
      fprintf(f, "[METHOD_TEMPLATE:%s]\n",
              method != NULL && method->name != NULL ? method->name
                                                     : "<unnamed>");
      break;
    }
...
clone_method_template(const struct MethodTemplate *method) {
  ...
  struct MethodTemplate *clone =
      (struct MethodTemplate *)calloc(1, sizeof(struct MethodTemplate));
  if (clone == NULL)
    return NULL;

  clone->name = method->name != NULL ? strdup(method->name) : NULL;
  clone->delegated_interface_name =
      method->delegated_interface_name != NULL
          ? strdup(method->delegated_interface_name)
          : NULL;
  clone->delegated_target_name = method->delegated_target_name != NULL
                                     ? strdup(method->delegated_target_name)
                                     : NULL;
  clone->method_ast =
      method->method_ast != NULL ? copy_ast_detached(method->method_ast) : NULL;
  ...
  clone->method_impl_ast = method->method_impl_ast != NULL
                               ? copy_ast_detached(method->method_impl_ast)
                               : NULL;
...
static ListNode_t *clone_method_template_list(const ListNode_t *methods) {
  ...
  while (cur != NULL) {
    if (cur->type == LIST_METHOD_TEMPLATE) {
      struct MethodTemplate *method_clone =
          clone_method_template((struct MethodTemplate *)cur->cur);
...
static ListNode_t *clone_property_list(const ListNode_t *properties) {
  ...
  while (cur != NULL) {
    if (cur->type == LIST_CLASS_PROPERTY) {
      struct ClassProperty *property_clone =
          clone_class_property((struct ClassProperty *)cur->cur);
...
void tree_print(Tree_t *tree, FILE *f, int num_indent) {
  ...
  case TREE_PROGRAM_TYPE:
    ...
    print_indent(f, num_indent);
    fprintf(f, "[LABELS]:\n");
    list_print(tree->tree_data.program_data.label_declaration, f,
               num_indent + 1);

    print_indent(f, num_indent);
    fprintf(f, "[CONST_DECLS]:\n");
    list_print(tree->tree_data.program_data.const_declaration, f,
               num_indent + 1);

    print_indent(f, num_indent);
    fprintf(f, "[TYPE_DECLS]:\n");
    list_print(tree->tree_data.program_data.type_declaration, f,
               num_indent + 1);
    ...
    list_print(tree->tree_data.program_data.finalization_statements, f,
               num_indent + 1);
    break;
  ...
  case TREE_UNIT:
    ...
    list_print(tree->tree_data.unit_data.interface_const_decls, f,
               num_indent + 1);

    print_indent(f, num_indent);
    fprintf(f, "[INTERFACE_TYPE_DECLS]:\n");
    list_print(tree->tree_data.unit_data.interface_type_decls, f,
               num_indent + 1);

    print_indent(f, num_indent);
    fprintf(f, "[INTERFACE_VAR_DECLS]:\n");
    list_print(tree->tree_data.unit_data.interface_var_decls, f,
               num_indent + 1);

    print_indent(f, num_indent);
    fprintf(f, "[IMPLEMENTATION_USES]:\n");
    list_print(tree->tree_data.unit_data.implementation_uses, f,
               num_indent + 1);

    print_indent(f, num_indent);
    fprintf(f, "[IMPLEMENTATION_CONST_DECLS]:\n");
    list_print(tree->tree_data.unit_data.implementation_const_decls, f,
               num_indent + 1);

    print_indent(f, num_indent);
    fprintf(f, "[IMPLEMENTATION_TYPE_DECLS]:\n");
    list_print(tree->tree_data.unit_data.implementation_type_decls, f,
               num_indent + 1);

    print_indent(f, num_indent);
    fprintf(f, "[IMPLEMENTATION_VAR_DECLS]:\n");
    list_print(tree->tree_data.unit_data.implementation_var_decls, f,
               num_indent + 1);
  ...
  case TREE_SUBPROGRAM:
    ...
    print_indent(f, num_indent);
    fprintf(f, "[CONST_DECLS]:\n");
    list_print(tree->tree_data.subprogram_data.const_declarations, f,
               num_indent + 1);

    print_indent(f, num_indent);
    fprintf(f, "[LABELS]:\n");
    list_print(tree->tree_data.subprogram_data.label_declarations, f,
               num_indent + 1);

    print_indent(f, num_indent);
    fprintf(f, "[TYPE_DECLS]:\n");
    list_print(tree->tree_data.subprogram_data.type_declarations, f,
               num_indent + 1);
    ...
    print_indent(f, num_indent);
    fprintf(f, "[BODY]:\n");
    stmt_print(tree->tree_data.subprogram_data.statement_list, f,
               num_indent + 1);
    break;
  ...
  case TREE_TYPE_DECL:
    if (tree->tree_data.type_decl_data.kind == TYPE_DECL_RECORD) {
      ...
      if (tree->tree_data.type_decl_data.info.record != NULL) {
        print_indent(f, num_indent + 1);
        fprintf(f, "[FIELDS]:\n");
        list_print(tree->tree_data.type_decl_data.info.record->fields, f,
                   num_indent + 2);
        if (tree->tree_data.type_decl_data.info.record->properties != NULL) {
          print_indent(f, num_indent + 1);
          fprintf(f, "[PROPERTIES]:\n");
          list_print(tree->tree_data.type_decl_data.info.record->properties, f,
                     num_indent + 2);
        }
      }
      ...
      const struct GenericDecl *generic =
      ...
      fprintf(f, "[TYPEDECL:%s = %d..%d]\n", tree->tree_data.type_decl_data.id,
              tree->tree_data.type_decl_data.info.range.start,
              tree->tree_data.type_decl_data.info.range.end);
    }
    break;
...
void stmt_print(struct Statement *stmt, FILE *f, int num_indent) {
  ...
  case STMT_PROCEDURE_CALL:
    ...
    print_indent(f, num_indent);
    fprintf(f, "[ARGS]:\n");
    list_print(stmt->stmt_data.procedure_call_data.expr_args, f,
               num_indent + 1);
    break;
  ...
  case STMT_FOR:
    ...
    case STMT_FOR_VAR:
      expr_print(stmt->stmt_data.for_data.for_assign_data.var, f,
                 num_indent + 1);
      break;
    case STMT_FOR_ASSIGN_VAR:
      stmt_print(stmt->stmt_data.for_data.for_assign_data.var_assign, f,
                 num_indent + 1);
      break;
    default:
  ...
  case STMT_TRY_FINALLY:
    ...
    print_indent(f, num_indent);
    fprintf(f, "[TRY]:\n");
    list_print(stmt->stmt_data.try_finally_data.try_statements, f,
               num_indent + 1);
    print_indent(f, num_indent);
    fprintf(f, "[FINALLY]:\n");
    list_print(stmt->stmt_data.try_finally_data.finally_statements, f,
               num_indent + 1);
    break;
  ...
  case STMT_TRY_EXCEPT:
    ...
    print_indent(f, num_indent);
    fprintf(f, "[TRY]:\n");
    list_print(stmt->stmt_data.try_except_data.try_statements, f,
               num_indent + 1);
    print_indent(f, num_indent);
    fprintf(f, "[EXCEPT]:\n");
    list_print(stmt->stmt_data.try_except_data.except_statements, f,
               num_indent + 1);
    break;
  ...
  case STMT_ON_EXCEPTION:
    fprintf(f, "[ON_EXCEPTION:%s:%s]:\n",
            stmt->stmt_data.on_exception_data.exception_var_name != NULL
                ? stmt->stmt_data.on_exception_data.exception_var_name
                : "<none>",
            stmt->stmt_data.on_exception_data.exception_type_name != NULL
                ? stmt->stmt_data.on_exception_data.exception_type_name
                : "<none>");
    if (stmt->stmt_data.on_exception_data.handler_stmt != NULL)
      stmt_print(stmt->stmt_data.on_exception_data.handler_stmt, f,
                 num_indent + 1);
    break;
...
void expr_print(struct Expression *expr, FILE *f, int num_indent) {
  ...
  case EXPR_RECORD_ACCESS:
    ...
    print_indent(f, num_indent);
    fprintf(f, "[RECORD]:\n");
    expr_print(expr->expr_data.record_access_data.record_expr, f,
               num_indent + 1);
    break;
  ...
  case EXPR_RECORD_CONSTRUCTOR: {
    ...
    while (cur != NULL) {
      ...
      print_indent(f, num_indent + 1);
      fprintf(f, "[FIELD:%s]:\n",
              field != NULL && field->field_id != NULL ? field->field_id
                                                       : "<unknown>");
  ...
  case EXPR_POINTER_DEREF:
    ...
    print_indent(f, num_indent);
    fprintf(f, "[POINTER]:\n");
    expr_print(expr->expr_data.pointer_deref_data.pointer_expr, f,
               num_indent + 1);
    --num_indent;
    break;
  ...
  if (list != NULL) {
    ...
    while (cur != NULL) {
      ...
      case LIST_METHOD_TEMPLATE:
        ...
        break;
      case LIST_UNSPECIFIED:
        /* LIST_UNSPECIFIED nodes have unknown content type - cannot safely
         * free. These are used for temporary storage where the content
         * ownership is managed elsewhere. Do not free cur->cur here. */
        break;
      default:
...
void destroy_tree(Tree_t *tree) {
  ...
  case TREE_SUBPROGRAM:
    ...
    if (tree->tree_data.subprogram_data.inline_return_type != NULL) {
      clear_type_alias_fields(
          tree->tree_data.subprogram_data.inline_return_type);
    ...
    if (tree->tree_data.subprogram_data.generic_type_params != NULL) {
      for (int i = 0;
           i < tree->tree_data.subprogram_data.num_generic_type_params; i++)
  ...
  case TREE_VAR_DECL:
    ...
    if (tree->tree_data.var_decl_data.static_label != NULL)
      free(tree->tree_data.var_decl_data.static_label);
    break;
...
void destroy_expr(struct Expression *expr) {
  assert(expr != NULL);
  if (getenv("KGPC_DEBUG_EXPR_FREE") != NULL &&
    ...
    fprintf(stderr,
            "[destroy_expr_pre] type=%p kind=%d ref=%d expr_type=%d line=%d "
            "src=%d id=%s\n",
            (void *)t, t->kind, t->ref_count, expr->type, expr->line_num,
            expr->source_index, id ? id : "");
  }
...
struct RecordType *clone_record_type(const struct RecordType *record_type) {
  ...
  struct RecordType *clone =
      (struct RecordType *)malloc(sizeof(struct RecordType));
  assert(clone != NULL);
  clone->fields = NULL;
  clone->properties = NULL;
  clone->parent_class_name = record_type->parent_class_name
                                 ? strdup(record_type->parent_class_name)
                                 : NULL;
  clone->methods =
      NULL; /* Methods list copied during semantic checking if needed */
  clone->method_templates =
  ...
  clone->helper_base_type_id = record_type->helper_base_type_id
                                   ? strdup(record_type->helper_base_type_id)
                                   : NULL;
  clone->helper_parent_id = record_type->helper_parent_id
                                ? strdup(record_type->helper_parent_id)
                                : NULL;
  clone->type_id = record_type->type_id ? strdup(record_type->type_id) : NULL;
  clone->outer_type_id =
      record_type->outer_type_id ? strdup(record_type->outer_type_id) : NULL;
  ...
  if (record_type->generic_args != NULL && record_type->num_generic_args > 0) {
    clone->generic_args =
        (char **)calloc((size_t)record_type->num_generic_args, sizeof(char *));
    if (clone->generic_args != NULL) {
      for (int i = 0; i < record_type->num_generic_args; ++i)
        clone->generic_args[i] = record_type->generic_args[i] != NULL
                                     ? strdup(record_type->generic_args[i])
                                     : NULL;
    }
  }

  clone->default_indexed_property =
      record_type->default_indexed_property
          ? strdup(record_type->default_indexed_property)
          : NULL;
  clone->default_indexed_element_type =
      record_type->default_indexed_element_type;
  clone->default_indexed_element_type_id =
      record_type->default_indexed_element_type_id
          ? strdup(record_type->default_indexed_element_type_id)
          : NULL;

  clone->fields = clone_member_list(record_type->fields);
  clone->properties = clone_property_list(record_type->properties);
  clone->record_properties =
      clone_property_list(record_type->record_properties);

  clone->guid_string =
      record_type->guid_string ? strdup(record_type->guid_string) : NULL;
  ...
  if (record_type->interface_names != NULL && record_type->num_interfaces > 0) {
    clone->interface_names =
        (char **)calloc((size_t)record_type->num_interfaces, sizeof(char *));
    if (clone->interface_names != NULL) {
      for (int i = 0; i < record_type->num_interfaces; ++i)
        clone->interface_names[i] =
            record_type->interface_names[i] != NULL
                ? strdup(record_type->interface_names[i])
                : NULL;
    }
  }
...
static struct RecordField *clone_record_field(const struct RecordField *field) {
  ...
  clone->type_ref =
      field->type_ref != NULL ? type_ref_clone(field->type_ref) : NULL;
  ...
  clone->array_dim_start_str = field->array_dim_start_str != NULL
                                   ? strdup(field->array_dim_start_str)
                                   : NULL;
  clone->array_dim_end_str = field->array_dim_end_str != NULL
                                 ? strdup(field->array_dim_end_str)
                                 : NULL;
  ...
  clone->array_element_type_id = field->array_element_type_id != NULL
                                     ...
                                     : NULL;
  clone->array_element_type_ref =
      field->array_element_type_ref != NULL
          ? type_ref_clone(field->array_element_type_ref)
          : NULL;
  ...
  clone->pointer_type_id =
      field->pointer_type_id != NULL ? strdup(field->pointer_type_id) : NULL;
  clone->pointer_type_ref = field->pointer_type_ref != NULL
                                ? type_ref_clone(field->pointer_type_ref)
                                : NULL;
  clone->enum_literals = NULL; /* enum_literals are not cloned - they're
...
clone_variant_part_internal(const struct VariantPart *variant) {
  ...
  clone->tag_type_id =
      variant->tag_type_id != NULL ? strdup(variant->tag_type_id) : NULL;
  ...
  while (cur != NULL) {
    if (cur->type == LIST_VARIANT_BRANCH) {
      struct VariantBranch *branch_clone =
          clone_variant_branch_internal((struct VariantBranch *)cur->cur);
...
mk_unit(int line_num, char *id, ListNode_t *interface_uses,
  ...
  new_tree->tree_data.unit_data.implementation_const_decls =
      implementation_const_decls;
  new_tree->tree_data.unit_data.implementation_type_decls =
      implementation_type_decls;
  new_tree->tree_data.unit_data.implementation_var_decls =
      implementation_var_decls;
...
Tree_t *mk_procedure(int line_num, char *id, ListNode_t *args,
  ...
  new_tree->tree_data.subprogram_data.is_used =
...
/*enum TreeType{TREE_PROGRAM_TYPE, TREE_SUBPROGRAM, TREE_VAR_DECL,
 * TREE_STATEMENT_TYPE};*/
...
struct Statement *mk_procedurecall(int line_num, char *id,
  ...
  new_stmt->stmt_data.procedure_call_data.call_hash_type =
      HASHTYPE_VAR; /* Default, will be overwritten */
...
struct Statement *mk_tryexcept(int line_num, ListNode_t *try_stmts,
  ...
  new_stmt->stmt_data.try_except_data.has_on_clause =
      (exception_var_name != NULL || exception_type_name != NULL) ? 1 : 0;
...
struct Statement *mk_on_exception(int line_num, char *exception_var_name,
  ...
  new_stmt->stmt_data.on_exception_data.exception_type_name =
      exception_type_name;
...
/*********** Expression routines ***************/
static void init_expression(struct Expression *expr, int line_num,
                            enum ExprType type) {
  assert(expr != NULL);
...
struct Expression *mk_functioncall(int line_num, char *id, ListNode_t *args) {
  ...
  new_expr->expr_data.function_call_data.builtin_call_lowering =
      BUILTIN_CALL_NONE;
...
struct SetElement *mk_set_element(struct Expression *lower,
  ...
  struct SetElement *element =
      (struct SetElement *)malloc(sizeof(struct SetElement));
  assert(element != NULL);
...
struct Expression *mk_anonymous_procedure(int line_num, char *generated_name,
  ...
  new_expr->expr_data.anonymous_method_data.generated_name = generated_name;
  new_expr->expr_data.anonymous_method_data.parameters = parameters;
  new_expr->expr_data.anonymous_method_data.return_type =
      -1; // No return type for procedures
...
static void clear_type_alias_fields(struct TypeAlias *alias) {
   ...
   * the tree (e.g. codegen's LeaveScope runs before
   * compilation_context_destroy). */
  alias->kgpc_type = NULL;
