...
void operator_registry_add(const char *type_name, const char *operator_symbol,
                           ...
                           const char *return_type, int param_count) {
  if (type_name == NULL || operator_symbol == NULL ||
      mangled_method_name == NULL)
    return;

  OperatorOverload *overload =
      (OperatorOverload *)calloc(1, sizeof(OperatorOverload));
  if (overload == NULL)
    return;
  ...
  overload->right_param_type =
      right_param_type ? strdup(right_param_type) : NULL;
  ...
  overload->is_comparison =
      (strcmp(operator_symbol, "=") == 0 ||
       strcmp(operator_symbol, "<>") == 0 ||
       strcmp(operator_symbol, "<") == 0 || strcmp(operator_symbol, ">") == 0 ||
       strcmp(operator_symbol, "<=") == 0 ||
       strcmp(operator_symbol, ">=") == 0);
...
int operator_registry_has_overload(const char *type_name,
