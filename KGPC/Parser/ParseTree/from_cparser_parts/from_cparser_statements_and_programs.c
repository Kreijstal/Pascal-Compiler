...
static int directive_keyword_matches(const char *start, const char *end,
                                     const char *keyword) {
...
static enum AsmSyntaxMode
asm_syntax_mode_before_source_index(ast_t *stmt_node) {
...
struct Statement *convert_statement(ast_t *stmt_node) {
  ...
  if (stmt_node == NULL) {
    if (kgpc_getenv("KGPC_DEBUG_BODY") != NULL)
      fprintf(stderr,
              "[KGPC] convert_statement: stmt_node NULL after unwrap\n");
    return NULL;
  }
  ...
  case PASCAL_T_LABEL_STMT: {
    ...
    struct Statement *inner_stmt =
        convert_statement(unwrap_pascal_node(stmt_part));
  ...
  case PASCAL_T_STATEMENT: {
    ...
    if (inner->typ == PASCAL_T_MEMBER_ACCESS) {
      struct Statement *method_stmt =
          convert_method_call_statement(inner, NULL);
      if (method_stmt != NULL)
        return method_stmt;
    }
    {
  ...
  case PASCAL_T_MEMBER_ACCESS: {
    struct Statement *method_stmt =
        convert_method_call_statement(stmt_node, NULL);
    if (method_stmt != NULL)
      return method_stmt;
    return NULL;
  }
  case PASCAL_T_ASM_BLOCK: {
    ...
    return mk_asmblock(stmt_node->line, code,
                       asm_syntax_mode_before_source_index(stmt_node));
  }
  ...
  case PASCAL_T_IF_STMT: {
    ...
    struct Statement *then_stmt =
        convert_statement(unwrap_pascal_node(then_wrapper));
    struct Statement *else_stmt =
        convert_statement(unwrap_pascal_node(else_wrapper));
  ...
  case PASCAL_T_REPEAT_STMT: {
    ...
    if (body_wrapper != NULL) {
      ast_t *body_nodes =
          body_wrapper->child != NULL ? body_wrapper->child : body_wrapper;
  ...
  case PASCAL_T_FOR_STMT: {
    ...
    ast_t *end_wrapper =
        direction_wrapper != NULL ? direction_wrapper->next : NULL;
    ast_t *end_node = unwrap_pascal_node(end_wrapper);
    ast_t *body_node =
        unwrap_pascal_node(end_wrapper != NULL ? end_wrapper->next : NULL);
  ...
  case PASCAL_T_FOR_IN_STMT: {
    ...
    fprintf(stderr,
            "DEBUG FOR_IN: id=%p typ=%d, expr=%p typ=%d, body=%p typ=%d\n",
            (void *)id_node, id_node ? id_node->typ : -1, (void *)expr_node,
            expr_node ? expr_node->typ : -1, (void *)body_node,
    ...
    if (collection_expr == NULL) {
      fprintf(stderr,
              "ERROR: FOR_IN collection expression conversion failed\n");
  ...
  case PASCAL_T_EXIT_STMT: {
    ...
    while (child != NULL && child->typ == PASCAL_T_NONE &&
           child->child == NULL) {
      child = child->next;
    }

    /* Check for a parenthesized expression - the NONE node with a child
     * expression */
    if (child != NULL) {
      ast_t *unwrapped = unwrap_pascal_node(child);
      /* Look inside the optional NONE wrapper that contains: '(' expr ')' */
      if (unwrapped != NULL && unwrapped->typ == PASCAL_T_NONE &&
        ...
        while (expr_node != NULL &&
               (expr_node->typ == PASCAL_T_NONE ||
                expr_node->typ == PASCAL_T_CHAR) &&
  ...
  case PASCAL_T_WITH_STMT: {
    ...
    ast_t *body_node = unwrap_pascal_node(
        contexts_wrapper != NULL ? contexts_wrapper->next : NULL);
    ...
    return build_nested_with_statements(stmt_node->line, contexts_start,
                                        body_stmt);
  }
      ...
      if (unwrapped->typ == PASCAL_T_FINALLY_BLOCK ||
        ...
        while (inner != NULL) {
          ...
          if (inner_unwrapped != NULL &&
              inner_unwrapped->typ == PASCAL_T_ON_CLAUSE) {
            ...
            /* Next might be a NONE node containing the type, or directly the
             * type */
            if (on_child != NULL) {
              /* Unwrap if it's a NONE node */
              ast_t *type_node =
                  (on_child->typ == PASCAL_T_NONE && on_child->child != NULL)
                      ? on_child->child
                      : on_child;
            ...
            while (on_child != NULL && on_child->typ != PASCAL_T_STATEMENT &&
                   on_child->typ != PASCAL_T_BEGIN_BLOCK &&
                   on_child->typ != PASCAL_T_ASSIGNMENT &&
                   on_child->typ != PASCAL_T_FUNC_CALL &&
                   on_child->typ != PASCAL_T_TRY_BLOCK) {
              on_child = on_child->next;
            }
  ...
  case PASCAL_T_RAISE_STMT: {
    struct Expression *exc_expr =
        convert_expression(unwrap_pascal_node(stmt_node->child));
  ...
  case PASCAL_T_INHERITED_STMT: {
    ...
    if (call_expr == NULL && g_current_method_name != NULL) {
      call_expr =
          mk_functioncall(stmt_node->line, strdup(g_current_method_name), NULL);
      if (call_expr != NULL) {
  ...
  case PASCAL_T_CASE_STMT: {
    ...
    while (cur != NULL) {
      if (cur->typ == PASCAL_T_CASE_BRANCH) {
        ...
        /* Create the branch */
        struct CaseBranch *branch =
            (struct CaseBranch *)malloc(sizeof(struct CaseBranch));
        if (branch != NULL) {
    ...
    return mk_case(stmt_node->line, selector_expr,
                   list_builder_finish(&branches_builder), else_stmt);
  }
  default: {
    /* Try to convert unknown tags as expressions (e.g., NOT, AND, OR in const
     * contexts) */
    ...
    if (stmt_node->child != NULL)
      fprintf(stderr, " (child: %s/%d)", tag_name(stmt_node->child->typ),
              stmt_node->child->line);
    fprintf(stderr, "\n");
    break;
  }
  }
...
ListNode_t *convert_statement_list(ast_t *stmt_list_node) {
  ...
  if (visited == NULL) {
    fprintf(
        stderr,
        "ERROR: Failed to allocate visited set for statement list traversal\n");
    return NULL;
  }

  if (kgpc_getenv("KGPC_DEBUG_BODY") != NULL) {
    fprintf(stderr,
            "[KGPC] convert_statement_list: starting, stmt_list_node=%p\n",
            (void *)stmt_list_node);
  }
  ...
  while (cur != NULL && cur != ast_nil) {
    ...
    if (!is_safe_to_continue(visited, cur)) {
      fprintf(stderr, "ERROR: Circular reference detected in statement list, "
                      "stopping traversal\n");
      break;
    }
...
struct Statement *convert_block(ast_t *block_node) {
  ...
  if (stmts == ast_nil) {
    if (kgpc_getenv("KGPC_DEBUG_BODY") != NULL) {
      fprintf(stderr,
              "[KGPC] convert_block: stmts is ast_nil, treating as NULL\n");
    }
    stmts = NULL;
  }
 ...
 */
static int ast_has_keyword_in_list(ast_t *node, const char *keyword,
                                   int max_depth) {
...
static int ast_list_has_directive_keyword(ast_t *node, const char *keyword,
                                          int max_depth) {
  ...
  for (ast_t *cur = node; cur != NULL; cur = cur->next) {
    if (cur->typ == PASCAL_T_FUNCTION_BODY ||
        cur->typ == PASCAL_T_BEGIN_BLOCK || cur->typ == PASCAL_T_ASM_BLOCK ||
        cur->typ == PASCAL_T_CONST_SECTION ||
        cur->typ == PASCAL_T_VAR_SECTION || cur->typ == PASCAL_T_TYPE_SECTION ||
        cur->typ == PASCAL_T_LABEL_SECTION ||
        cur->typ == PASCAL_T_PROCEDURE_DECL ||
        cur->typ == PASCAL_T_FUNCTION_DECL || cur->typ == PASCAL_T_METHOD_IMPL)
      break;
...
static int ast_has_routine_directive(ast_t *node, const char *keyword,
                                     int max_depth) {
  if (node == NULL || keyword == NULL || max_depth <= 0)
    return 0;
 ...
 */
static struct TypeAlias *build_inline_return_alias(TypeInfo *type_info,
                                                   int return_type,
                                                   char *return_type_id) {
  ...
  if (type_info->is_array || type_info->is_pointer || type_info->is_set ||
    ...
    if (alias != NULL) {
      ...
      alias->target_type_id =
          (return_type_id != NULL) ? strdup(return_type_id) : NULL;
...
Tree_t *convert_method_impl(ast_t *method_node) {
  ...
  while (cur != NULL) {
    ...
    if (skip_node == NULL)
      skip_node = cur;
    if (skip_node->typ == PASCAL_T_IDENTIFIER && skip_node->sym != NULL &&
        skip_node->sym->name != NULL &&
  ...
  /* Check for standalone operator: the first child is directly the operator
   * symbol like "+" or "-" This is detected by checking if the first identifier
   * is a known operator symbol */
  if (qualified != NULL && qualified->typ == PASCAL_T_IDENTIFIER) {
    char *potential_op = dup_symbol(qualified);
    int is_operator_symbol =
        (potential_op != NULL && is_operator_token_name(potential_op));
    int should_parse_standalone_operator =
        method_decl_uses_operator_keyword || is_operator_symbol;
    /* Check if this is an operator symbol (encoded to op_xxx means it's an
     * operator) */
    char *encoded_op =
        (potential_op != NULL && should_parse_standalone_operator)
            ? encode_operator_name(potential_op)
            : NULL;
    free(potential_op);

    if (should_parse_standalone_operator) {
      /* This is a standalone operator - qualified is the operator symbol
       * directly */
      if (kgpc_getenv("KGPC_DEBUG_OPERATOR") != NULL) {
      ...
      if (first_param != NULL) {
        ...
        if (params != NULL) {
          ...
          if (first_param_tree != NULL &&
            ...
            const char *param_type_id =
                first_param_tree->tree_data.var_decl_data.type_id;
            ...
            if (params->next != NULL) {
              ...
              if (second_param_tree != NULL &&
                  second_param_tree->type == TREE_VAR_DECL &&
                  second_param_tree->tree_data.var_decl_data.type_id != NULL)
                second_param_type_id =
                    second_param_tree->tree_data.var_decl_data.type_id;
            }

            /* Build mangled name: TypeName__op_suffix[_SecondParamType] */
            size_t name_len =
                strlen(param_type_id) + strlen(encoded_op) + 3 +
                (second_param_type_id != NULL ? strlen(second_param_type_id) + 1
                                              : 0);
            char *mangled_name = (char *)malloc(name_len);
            if (mangled_name != NULL) {
              if (second_param_type_id != NULL)
                snprintf(mangled_name, name_len, "%s__%s_%s", param_type_id,
                         encoded_op, second_param_type_id);
              else
                snprintf(mangled_name, name_len, "%s__%s", param_type_id,
                         encoded_op);
              ...
              while (scan != NULL) {
                ...
                /* Named result identifier before RETURN_TYPE (e.g., dest :
                 * variant) */
              ...
              if (return_type_node != NULL) {
                ...
                return_type = convert_type_spec(
                    return_type_node->child, &return_type_id, NULL, &type_info);
                if (return_type_ref == NULL)
                  return_type_ref =
                      type_ref_from_info_or_id(&type_info, return_type_id);
              ...
              if (base_id != NULL)
                snprintf(base_id, base_id_len, "%s__%s", param_type_id,
                         encoded_op);
              const char *ret_suffix = return_type_id;
              if (ret_suffix == NULL)
                ret_suffix = result_var_name_method;
              if (ret_suffix != NULL) {
                size_t new_len = strlen(mangled_name) + strlen(ret_suffix) + 2;
                char *new_name = (char *)malloc(new_len);
                if (new_name != NULL) {
                  snprintf(new_name, new_len, "%s_%s", mangled_name,
                           ret_suffix);
                  free(mangled_name);
              ...
              ListBuilder op_var_builder;
              list_builder_init(&op_var_builder);
              ListBuilder op_label_builder;
              ...
              ListNode_t *op_label_decls =
                  list_builder_finish(&op_label_builder);
              Tree_t *tree = mk_function(
                  method_node->line, base_id != NULL ? base_id : mangled_name,
                  params, op_const_decls, op_label_decls, op_type_decls,
                  op_var_decls, op_nested_subs, body, return_type,
                  return_type_id, inline_return_type, 0, 0);
              if (tree != NULL) {
                if (base_id != NULL)
                  tree->tree_data.subprogram_data.mangled_id = mangled_name;
                else
                  tree->tree_data.subprogram_data.mangled_id =
                      strdup(tree->tree_data.subprogram_data.id);
              ...
              if (tree != NULL && result_var_name_method != NULL) {
                tree->tree_data.subprogram_data.return_type_ref =
                    return_type_ref != NULL
                        ? type_ref_clone(return_type_ref)
                        : type_ref_from_single_name(return_type_id);
                tree->tree_data.subprogram_data.result_var_name =
                    result_var_name_method;
  ...
  // For "Class.Method" we get 2 identifiers, for "Class.Inner.Method" we get 3,
  // etc. The LAST identifier is always the method name; all preceding ones form
  // the class path.
  ast_t *ident_nodes[32]; // more than enough for any reasonable nesting depth
  int ident_count = 0;

  ast_t *method_type_param_list = NULL; /* TYPE_PARAM_LIST after the last
                                           IDENTIFIER (method-level generic) */
  ...
  for (ast_t *cursor = class_node; cursor != NULL; cursor = cursor->next) {
    if (cursor->typ == PASCAL_T_IDENTIFIER && ident_count < 32) {
      ...
      pending_type_param_list =
          NULL; /* TYPE_PARAM_LIST before an IDENTIFIER is class-level, reset */
    }
  ...
  method_type_param_list = pending_type_param_list; /* Last TYPE_PARAM_LIST with
                                                       no IDENTIFIER after it */

  if (ident_count < 2)
    return NULL; // Need at least ClassName and MethodName

  ast_t *method_id_node =
      ident_nodes[ident_count - 1]; // last identifier = method name
  ...
  const char *effective_class = (cleaned_class_name != NULL)
                                    ? cleaned_class_name
  ...
  if (effective_class != NULL) {
    ...
    if (last_dot != NULL && last_dot[1] != '\0') {
      ...
      effective_class_outer =
          strndup(effective_class, (size_t)(last_dot - effective_class));
  ...
  if (kgpc_getenv("KGPC_DEBUG_SEMCHECK") != NULL) {
    fprintf(stderr,
            "[FromCParser] convert_method_impl: class_name=%s method_name=%s "
            "effective_class=%s effective_class_full=%s\n",
  ...
  /* Don't re-register the method here - it was already registered during class
   * declaration */
  ...
  int is_class_method_impl =
      from_cparser_is_method_class_method(effective_class, method_name);
  int method_param_count = count_params_in_method_impl(method_node);
  int method_param_types_count = 0;
  TypeRef **method_param_types =
      param_types_from_method_impl(method_node, &method_param_types_count);
  ...
  if (method_node != NULL && method_name != NULL) {
    ...
    /* Only truly static methods skip Self; class methods have Self = VMT
     * pointer */
  ...
  if (!is_static_method) {
    is_static_method = is_method_static_with_types(
        effective_class, method_name, method_param_count, method_param_types,
        method_param_types_count);
  }
  ...
  if (kgpc_getenv("KGPC_DEBUG_GENERIC_METHODS") != NULL) {
    fprintf(stderr,
            "[KGPC] convert_method_impl: class=%s method=%s is_static=%d\n",
            effective_class ? effective_class : "<null>",
            method_name ? method_name : "<null>", is_static_method);
  }
  ...
  /* Extract method-level generic type params found inside the qualified
   * identifier */
  if (method_type_param_list != NULL) {
    num_generic_type_params = extract_generic_type_params(
        method_type_param_list, &generic_type_params);
  }
  ...
  /* Return type information for methods that are functions (like class
   * operators) */
  char *return_type_id = NULL;
  int return_type = UNKNOWN_TYPE;
  struct TypeAlias *inline_return_type = NULL;
  ...
  const char *helper_base = (effective_class != NULL)
                                ? lookup_type_helper_base(effective_class)
                                : NULL;
  int is_helper_method = (helper_base != NULL);
  if (kgpc_getenv("KGPC_DEBUG_TYPE_HELPER") != NULL && effective_class != NULL)
    fprintf(stderr,
            "[KGPC] convert_method_impl: looking up helper base for %s -> %s\n",
            effective_class, helper_base ? helper_base : "(not found)");

  /* Add Self parameter only for instance methods, not for class operators or
   * static methods */
  ...
  if (!is_class_operator && !is_static_method) {
    ...
    if (effective_class != NULL) {
      if (is_helper_method) {
         ...
         * This is important for correct calling convention (e.g., Double should
         * use xmm0). */
        ...
        self_type_id = strdup(effective_class_full ? effective_class_full
                                                   : effective_class);
    ...
    Tree_t *self_param =
        mk_vardecl(method_node->line, self_ids, self_type_tag, self_type_id,
                   self_is_var, 0, NULL, NULL, self_type_alias, NULL);
    if (self_param != NULL)
      self_param->tree_data.var_decl_data.type_ref =
          type_ref_from_single_name(self_type_id);
  ...
  while (cur != NULL) {
    ...
    case PASCAL_T_TYPE_PARAM_LIST:
      if (num_generic_type_params == 0) {
        num_generic_type_params =
            extract_generic_type_params(node, &generic_type_params);
      }
      break;
    ...
    case PASCAL_T_RETURN_TYPE: {
      ...
      return_type =
          convert_type_spec(node->child, &return_type_id, NULL, &type_info);
      ...
      if (return_type_id == NULL && node->sym != NULL &&
          node->sym->name != NULL)
        return_type_id = strdup(node->sym->name);
      inline_return_type =
          build_inline_return_alias(&type_info, return_type, return_type_id);
      destroy_type_info_contents(&type_info);
      break;
    }
    ...
    case PASCAL_T_ASM_BLOCK: {
      ...
      body =
          mk_compoundstatement(node->line, list_builder_finish(&stmts_builder));
      break;
    }
    case PASCAL_T_IDENTIFIER: {
  ...
  /* Wrap method body in WITH Self for instance methods, but not for helpers,
   * class operators, or static methods */
  ...
  if (is_class_operator && method_name != NULL && mangle_owner != NULL) {
    ...
    if (encoded_method != NULL &&
        (param_suffix != NULL || ret_suffix != NULL)) {
      ...
      if (disambiguated != NULL) {
        int written = snprintf(disambiguated, name_len, "%s__%s", mangle_owner,
                               encoded_method);
        if (written > 0 && (size_t)written < name_len) {
          size_t used = (size_t)written;
          if (param_suffix != NULL) {
            snprintf(disambiguated + used, name_len - used, "_%s",
                     param_suffix);
  ...
  if (has_return_type) {
    tree = mk_function(method_node->line, proc_name, params, const_decls,
                       label_decls, type_decls,
                       list_builder_finish(&var_builder), nested_subs, body,
                       return_type, return_type_id, inline_return_type, 0, 0);
  } else {
    tree = mk_procedure(
        method_node->line, proc_name, params, const_decls, label_decls,
        type_decls, list_builder_finish(&var_builder), nested_subs, body, 0, 0);
  }
  if (tree != NULL && method_node->index >= 0)
    tree->source_index = method_node->index + g_source_offset;
  if (!is_nostackframe)
  ...
  if (tree != NULL) {
    if (has_return_type)
      tree->tree_data.subprogram_data.return_type_ref =
          return_type_ref != NULL ? return_type_ref
                                  : type_ref_from_single_name(return_type_id);
    ...
    if (effective_class_full != NULL &&
        effective_class_full != effective_class) {
      char *full_mangled =
          mangle_method_name_raw(effective_class_full, method_name);
      tree->tree_data.subprogram_data.mangled_id =
          full_mangled != NULL ? full_mangled : strdup(proc_name);
    } else {
      tree->tree_data.subprogram_data.mangled_id = strdup(proc_name);
    }
    tree->tree_data.subprogram_data.method_name =
        (char *)string_intern(method_name);
    tree->tree_data.subprogram_data.owner_class =
        (char *)string_intern(effective_class);
    ...
    if (effective_class_full != NULL && effective_class_full != effective_class)
      tree->tree_data.subprogram_data.owner_class_full =
          (char *)string_intern(effective_class_full);
    ...
    if (effective_class_outer != NULL)
      tree->tree_data.subprogram_data.owner_class_outer =
          (char *)string_intern(effective_class_outer);
    ...
    if (num_generic_type_params > 0) {
      ...
      tree->tree_data.subprogram_data.num_generic_type_params =
          num_generic_type_params;
      tree->tree_data.subprogram_data.is_generic_template = 1;
      tree->tree_data.subprogram_data.generic_template_ast =
          copy_ast_detached(method_node);
      tree->tree_data.subprogram_data.generic_template_source_offset =
          g_source_offset;
  ...
  /* Check if this method belongs to a generic type. If so, we've recorded the
   * AST template above and should NOT generate a concrete implementation.
   * Return NULL to prevent adding this to the subprograms list. */
  ...
  if (generic_decl != NULL && generic_decl->record_template != NULL) {
    if (kgpc_getenv("KGPC_DEBUG_GENERIC_METHODS") != NULL &&
        effective_class != NULL && method_name != NULL) {
      fprintf(stderr,
              "[KGPC] convert_method_impl: recorded template for %s.%s, not "
              "generating concrete impl\n",
...
/* Extract generic type parameter names from a PASCAL_T_TYPE_PARAM_LIST AST
 * node. Returns the number of type parameters found, and fills *out_params with
...
static void process_subprogram_declarations(ast_t **cursor,
                                            SubprogramDeclState *s) {
  ast_t *cur = *cursor;
  while (cur != NULL) {
    switch (cur->typ) {
    case PASCAL_T_TYPE_SECTION:
      append_type_decls_from_section(cur, &s->type_decls, &s->nested_subs,
                                     &s->const_decls, &s->var_decls_builder,
                                     NULL);
      s->type_section_ast = cur;
      break;
    case PASCAL_T_CONST_SECTION:
      append_const_decls_from_section(
          cur, &s->const_decls, &s->var_decls_builder, s->type_section_ast);
      break;
    case PASCAL_T_VAR_SECTION:
    ...
    case PASCAL_T_FUNCTION_BODY:
      convert_routine_body(cur, &s->const_decls, &s->var_decls_builder,
                           &s->label_decls_builder, &s->nested_subs, &s->body,
                           &s->type_decls);
      break;
    case PASCAL_T_BEGIN_BLOCK:
    ...
    case PASCAL_T_ASM_BLOCK: {
      ...
      if (stmt != NULL)
        list_builder_append(&stmts_builder, stmt, LIST_STMT);
      s->body =
          mk_compoundstatement(cur->line, list_builder_finish(&stmts_builder));
      break;
    }
    case PASCAL_T_IDENTIFIER: {
    ...
    case PASCAL_T_NONE: {
      for (ast_t *child = cur->child; child != NULL; child = child->next) {
        ...
        } else if (child->typ == PASCAL_T_EXTERNAL_NAME ||
                   child->typ == PASCAL_T_EXTERNAL_NAME_EXPR) {
...
static void finalize_subprogram_tree(Tree_t *tree, ast_t *node,
                                     SubprogramDeclState *s,
                                     char **generic_type_params,
                                     int num_generic_type_params) {
...
Tree_t *convert_procedure(ast_t *proc_node) {
  ...
  while (cur != NULL && cur->typ == PASCAL_T_IDENTIFIER && cur->sym != NULL &&
         cur->sym->name != NULL &&
  ...
  if (cur != NULL && cur->typ == PASCAL_T_TYPE_PARAM_LIST) {
    num_generic_type_params =
        extract_generic_type_params(cur, &generic_type_params);
    cur = cur->next;
  }
  ...
  Tree_t *tree =
      mk_procedure(proc_node->line, id, params, ds.const_decls, label_decls,
                   ds.type_decls, list_builder_finish(&ds.var_decls_builder),
                   ds.nested_subs, ds.body, ds.is_external, 0);
  finalize_subprogram_tree(tree, proc_node, &ds, generic_type_params,
                           num_generic_type_params);
  return tree;
}
...
Tree_t *convert_function(ast_t *func_node) {
  ...
  if (is_standalone_operator && operator_symbol != NULL && params != NULL) {
    /* Get the type of the first parameter (params is a list of Tree_t* with
     * TREE_VAR_DECL) */
    Tree_t *first_param = (Tree_t *)params->cur;
    if (kgpc_getenv("KGPC_DEBUG_OPERATOR") != NULL) {
      fprintf(stderr,
              "[Operator] is_standalone_operator=%d operator_symbol=%s\n",
      ...
      if (params->next != NULL) {
        ...
        if (second_param != NULL && second_param->type == TREE_VAR_DECL &&
          ...
          deferred_second_param_type_id =
              second_param->tree_data.var_decl_data.type_id;
      }
  ...
  /* For operator declarations with named results (e.g., "operator :=(source:
   * byte) dest: variant"), the named result identifier appears as a
   * PASCAL_T_IDENTIFIER node before PASCAL_T_RETURN_TYPE. */
  if (cur != NULL && cur->typ == PASCAL_T_IDENTIFIER &&
      is_standalone_operator) {
  ...
  if (cur != NULL && cur->typ == PASCAL_T_RETURN_TYPE) {
    TypeInfo type_info = {0};
    return_type =
        convert_type_spec(cur->child, &return_type_id, NULL, &type_info);
  ...
  if (deferred_encoded_op != NULL && deferred_param_type_id != NULL) {
    const char *ret_suffix = return_type_id;
    if (ret_suffix == NULL)
      ret_suffix = result_var_name;

    /* Set id to the base name: TypeName__op_suffix */
    size_t base_len =
        strlen(deferred_param_type_id) + strlen(deferred_encoded_op) + 3;
    char *base_name = (char *)malloc(base_len);
    if (base_name != NULL) {
      snprintf(base_name, base_len, "%s__%s", deferred_param_type_id,
               deferred_encoded_op);
      free(id);
      id = base_name;
    }
    ...
    size_t name_len = strlen(deferred_param_type_id) +
                      strlen(deferred_encoded_op) + 3 +
                      (ret_suffix != NULL ? strlen(ret_suffix) + 1 : 0) +
                      (deferred_second_param_type_id != NULL
                           ? strlen(deferred_second_param_type_id) + 1
                           : 0);
    char *mangled_name = (char *)malloc(name_len);
    if (mangled_name != NULL) {
      if (ret_suffix != NULL && deferred_second_param_type_id != NULL) {
        snprintf(mangled_name, name_len, "%s__%s_%s_%s", deferred_param_type_id,
                 deferred_encoded_op, deferred_second_param_type_id,
                 ret_suffix);
      } else if (ret_suffix != NULL) {
        snprintf(mangled_name, name_len, "%s__%s_%s", deferred_param_type_id,
                 deferred_encoded_op, ret_suffix);
      } else if (deferred_second_param_type_id != NULL) {
        snprintf(mangled_name, name_len, "%s__%s_%s", deferred_param_type_id,
                 deferred_encoded_op, deferred_second_param_type_id);
      } else {
        snprintf(mangled_name, name_len, "%s__%s", deferred_param_type_id,
                 deferred_encoded_op);
      }
  ...
  ListNode_t *label_decls = list_builder_finish(&ds.label_decls_builder);
  Tree_t *tree = mk_function(
      func_node->line, id, params, ds.const_decls, label_decls, ds.type_decls,
      list_builder_finish(&ds.var_decls_builder), ds.nested_subs, ds.body,
  ...
  if (tree != NULL)
    ...
    type_ref_free(return_type_ref);
  finalize_subprogram_tree(tree, func_node, &ds, generic_type_params,
                           num_generic_type_params);
...
static void resolve_deferred_arrays_in_list(ListNode_t *decl_list) {
  for (ListNode_t *cur = decl_list; cur != NULL; cur = cur->next) {
    ...
    if (decl == NULL || decl->type != TREE_ARR_DECL)
      continue;
    if (decl->tree_data.arr_decl_data.unresolved_index_type == NULL)
      continue;
...
void from_cparser_resolve_deferred_arrays(Tree_t *program) {
  ...
  if (program->type == TREE_PROGRAM_TYPE) {
    resolve_deferred_arrays_in_list(
...
void from_cparser_cleanup(void) {
  ...
  while (type_helper_mappings != NULL) {
    ...
    struct TypeHelperMapping *entry =
        (struct TypeHelperMapping *)type_helper_mappings->cur;
    if (entry != NULL) {
  ...
  /* Free class method bindings (interned strings - do NOT free, just free
   * structs + list nodes) */
  cmb_index_reset();
  ...
  while (class_method_bindings != NULL) {
    ...
    ClassMethodBinding *binding =
        (ClassMethodBinding *)class_method_bindings->cur;
    if (binding != NULL) {
...
Tree_t *tree_from_pascal_ast(ast_t *program_ast) {
  ...
  if (cur->typ == PASCAL_T_PROGRAM_DECL || cur->typ == 88) {
     ...
     *   - optional(program_header)       [PASCAL_T_PROGRAM_HEADER or
     * NULL/skipped]
     *   - optional(uses_section)          [PASCAL_T_USES_SECTION or
     * NULL/skipped]
    ...
    if (kgpc_getenv("KGPC_DEBUG_BODY") != NULL) {
      fprintf(stderr,
              "[KGPC] tree_from_pascal_ast: program block entered. "
              "first_child=%p\n",
              first_child);
      if (first_child) {
        ...
        if (sibling != NULL) {
          fprintf(stderr,
                  "[KGPC] tree_from_pascal_ast: ... more siblings ...\n");
        }
          ...
          if (last->child != NULL) {
            ...
            while (child != NULL && child_count < 20) {
              ...
              if (child->next != NULL) {
                fprintf(
                    stderr,
                    "[KGPC] tree_from_pascal_ast:     Siblings of child[%d]:\n",
                    child_count);
    ...
    if (visited == NULL) {
      fprintf(stderr,
              "ERROR: Failed to allocate visited set for program sections\n");
      free(program_id);
      return NULL;
    }

    /* Start iterating from the first child, or the node after the program
     * header */
    ast_t *section =
        program_header_node != NULL ? program_header_node->next : first_child;
    while (section != NULL) {
      ...
      if (!is_safe_to_continue(visited, section)) {
        fprintf(stderr, "ERROR: Circular reference detected in program "
                        "sections, stopping traversal\n");
        break;
      }
      ...
      case PASCAL_T_CONST_SECTION:
        append_const_decls_from_section(section, &const_decls,
                                        &var_decls_builder, type_section_ast);
        break;
      case PASCAL_T_VAR_SECTION:
      ...
      case PASCAL_T_MAIN_BLOCK: {
        ...
        if (kgpc_getenv("KGPC_DEBUG_BODY") != NULL) {
          fprintf(stderr,
                  "[KGPC] tree_from_pascal_ast: body assigned, body=%p\n",
                  body);
        }
        break;
      }
      ...
      case PASCAL_T_CONST_DECL:
         ...
         * They should be children of VAR_SECTION, TYPE_SECTION, or
         * CONST_SECTION. If we encounter them here, it means the AST structure
         * is malformed, but we can safely skip them to continue parsing. */
        if (kgpc_getenv("KGPC_DEBUG_BODY") != NULL) {
          fprintf(stderr,
    ...
    if (body == NULL) {
      if (kgpc_getenv("KGPC_DEBUG_BODY") != NULL) {
        fprintf(stderr, "[KGPC] tree_from_pascal_ast: Main block not found in "
                        "sibling chain, searching recursively\n");
      }
      ...
      if (kgpc_getenv("KGPC_DEBUG_BODY") != NULL) {
        fprintf(
            stderr,
            "[KGPC] tree_from_pascal_ast: Searching for VAR_SECTION (typ=%d)\n",
            PASCAL_T_VAR_SECTION);
      }
      ast_t *var_section_node =
          find_node_by_type(cur->child, PASCAL_T_VAR_SECTION);
      if (var_section_node != NULL) {
        if (kgpc_getenv("KGPC_DEBUG_BODY") != NULL) {
          fprintf(stderr, "[KGPC] tree_from_pascal_ast: Found VAR_SECTION via "
                          "recursive search\n");
        }
        list_builder_extend(&var_decls_builder,
                            convert_var_section(var_section_node));
      }
      ...
      if (kgpc_getenv("KGPC_DEBUG_BODY") != NULL) {
        fprintf(
            stderr,
            "[KGPC] tree_from_pascal_ast: Searching for MAIN_BLOCK (typ=%d)\n",
            PASCAL_T_MAIN_BLOCK);
      }
      ast_t *main_block_node =
          find_last_node_by_type(cur->child, PASCAL_T_MAIN_BLOCK);
      if (main_block_node == NULL) {
        if (kgpc_getenv("KGPC_DEBUG_BODY") != NULL) {
          fprintf(stderr,
                  "[KGPC] tree_from_pascal_ast: MAIN_BLOCK not found, "
                  "searching for BEGIN_BLOCK (typ=%d)\n",
                  PASCAL_T_BEGIN_BLOCK);
        }
        main_block_node =
            find_last_node_by_type(cur->child, PASCAL_T_BEGIN_BLOCK);
      }
      /* Also try typ=112 which appears in the AST */
      if (main_block_node == NULL) {
        if (kgpc_getenv("KGPC_DEBUG_BODY") != NULL) {
          fprintf(stderr, "[KGPC] tree_from_pascal_ast: BEGIN_BLOCK not found, "
                          "trying typ=112\n");
        }
      ...
      if (main_block_node != NULL) {
        ...
        /* MAIN_BLOCK not found directly. Check if there's a typ=100 node that
         * contains it */
        if (kgpc_getenv("KGPC_DEBUG_BODY") != NULL) {
          fprintf(stderr, "[KGPC] tree_from_pascal_ast: MAIN_BLOCK not found "
                          "directly, searching for typ=100 wrapper\n");
        }
        ast_t *wrapper_node = find_last_node_by_type(cur->child, 100);
        if (wrapper_node != NULL && wrapper_node->child != NULL) {
          if (kgpc_getenv("KGPC_DEBUG_BODY") != NULL) {
            fprintf(stderr, "[KGPC] tree_from_pascal_ast: Found typ=100 "
                            "wrapper, checking its children\n");
          }
        ...
        if (body == NULL && kgpc_getenv("KGPC_DEBUG_BODY") != NULL) {
          ...
          fprintf(stderr,
                  "[KGPC] tree_from_pascal_ast: PASCAL_T_MAIN_BLOCK=%d, "
                  "PASCAL_T_BEGIN_BLOCK=%d\n",
    ...
    if (body == NULL) {
      if (kgpc_getenv("KGPC_DEBUG_BODY") != NULL) {
        fprintf(stderr, "[KGPC] tree_from_pascal_ast: No main block found, "
                        "creating empty body\\n");
      }
    ...
    int program_line =
        (program_header_node != NULL) ? program_header_node->line : cur->line;
    Tree_t *tree = mk_program(
        program_line, program_id, args, uses, label_decls, const_decls,
  ...
  if (cur->typ == PASCAL_T_UNIT_DECL) {
    ...
    char *unit_id =
        unit_name_node != NULL ? dup_symbol(unit_name_node) : strdup("unit");
    ...
    ast_t *interface_type_section_ast =
        NULL; /* Track interface types for enum resolution */
    ...
    ast_t *implementation_type_section_ast =
        NULL; /* Track implementation types for enum resolution */
    ListNode_t *subprograms = NULL;
    ...
    if (visited_unit == NULL) {
      fprintf(stderr,
              "ERROR: Failed to allocate visited set for unit sections\n");
      free(unit_id);
    ...
    while (section != NULL) {
      ...
      if (!is_safe_to_continue(visited_unit, section)) {
        fprintf(stderr, "ERROR: Circular reference detected in unit sections, "
                        "stopping traversal\n");
        break;
      }
    ...
    if (interface_node != NULL &&
        interface_node->typ == PASCAL_T_INTERFACE_SECTION) {
      /* Create visited set for interface sections */
      VisitedSet *visited_if = visited_set_create();
      if (visited_if == NULL) {
        fprintf(
            stderr,
            "ERROR: Failed to allocate visited set for interface sections\n");
      } else {
        ...
        while (section != NULL) {
          ...
          if (node_cursor != NULL) {
            ...
            case PASCAL_T_TYPE_SECTION:
              ...
              interface_type_section_ast =
                  node_cursor; /* Save for const array enum resolution */
              ...
              append_type_decls_from_section(node_cursor, &interface_type_decls,
                                             NULL, &interface_const_decls,
                                             &interface_var_builder, NULL);
              break;
            case PASCAL_T_CONST_SECTION:
              append_const_decls_from_section(
                  node_cursor, &interface_const_decls, &interface_var_builder,
                  interface_type_section_ast);
              break;
            case PASCAL_T_VAR_SECTION:
              list_builder_extend(&interface_var_builder,
                                  convert_var_section(node_cursor));
              break;
            case PASCAL_T_PROCEDURE_DECL: {
              Tree_t *proc = convert_procedure(node_cursor);
              if (kgpc_debug_subprog_enabled()) {
                char *proc_id = (node_cursor->child != NULL)
                                    ? dup_symbol(node_cursor->child)
                                    : strdup("?");
              ...
              if (proc != NULL) {
                proc->tree_data.subprogram_data.unit_is_public =
            ...
            case PASCAL_T_FUNCTION_DECL: {
              ...
              if (kgpc_debug_subprog_enabled()) {
                char *func_id = (node_cursor->child != NULL)
                                    ? dup_symbol(node_cursor->child)
                                    : strdup("?");
              ...
              if (func != NULL) {
                func->tree_data.subprogram_data.unit_is_public =
    ...
    if (interface_node != NULL) {
      ...
      if (visited_subs != NULL) {
        append_subprograms_from_ast_recursive(interface_node->child,
                                              &subprograms, visited_subs);
        visited_set_destroy(visited_subs);
      }
    }

    if (implementation_node != NULL &&
        implementation_node->typ == PASCAL_T_IMPLEMENTATION_SECTION) {
      ...
      if (kgpc_getenv("KGPC_DEBUG_IMPL_SECTION") != NULL) {
        ...
        while (dbg != NULL) {
          count++;
          if (dbg->line > max_line)
            max_line = dbg->line;
          if (dbg->typ == PASCAL_T_PROCEDURE_DECL)
            proc_count++;
          else if (dbg->typ == PASCAL_T_FUNCTION_DECL)
            func_count++;
          else if (dbg->typ == PASCAL_T_METHOD_IMPL)
            method_count++;
          else if (dbg->typ == PASCAL_T_TYPE_SECTION)
            type_count++;
          dbg = dbg->next;
        }
        fprintf(stderr,
                "[IMPL_SECTION] child count=%d max_line=%d procs=%d funcs=%d "
                "methods=%d types=%d\n",
                count, max_line, proc_count, func_count, method_count,
                type_count);
      }
      ...
      if (visited_impl == NULL) {
        fprintf(stderr, "ERROR: Failed to allocate visited set for "
        ...
        while (definition != NULL && definition != ast_nil) {
          ...
          if (kgpc_getenv("KGPC_DEBUG_IMPL_NONE") != NULL &&
              definition->typ == PASCAL_T_NONE && definition->child == NULL &&
              definition->sym != NULL && definition->sym->name != NULL) {
          ...
          if (node_cursor != NULL) {
            ...
            case PASCAL_T_USES_SECTION:
              ...
              break;
            case PASCAL_T_TYPE_SECTION:
              implementation_type_section_ast =
                  node_cursor; /* Save for const array enum resolution */
              g_implementation_type_section_ast = node_cursor;
              enum_registry_scan_type_section(node_cursor);
              append_type_decls_from_section(
                  node_cursor, &implementation_type_decls, &subprograms,
                  &implementation_const_decls, &implementation_var_builder,
                  NULL);
              break;
            case PASCAL_T_CONST_SECTION:
              append_const_decls_from_section(
                  node_cursor, &implementation_const_decls,
                  &implementation_var_builder, implementation_type_section_ast);
              break;
            case PASCAL_T_VAR_SECTION:
              list_builder_extend(&implementation_var_builder,
                                  convert_var_section(node_cursor));
              break;
            ...
            case PASCAL_T_PROCEDURE_DECL: {
              Tree_t *proc = convert_procedure(node_cursor);
              if (kgpc_debug_subprog_enabled() ||
                  kgpc_getenv("KGPC_DEBUG_IMPL_PROCS") != NULL) {
                char *proc_id = (node_cursor->child != NULL)
                                    ? dup_symbol(node_cursor->child)
                                    : strdup("?");
                fprintf(stderr,
                        "[KGPC] impl convert_procedure(%s) line=%d => %p\n",
            ...
            case PASCAL_T_FUNCTION_DECL: {
              ...
              if (kgpc_debug_subprog_enabled() ||
                ...
                char *func_id = (node_cursor->child != NULL)
                                    ...
                                    : strdup("?");
                fprintf(stderr,
                        "[KGPC] impl convert_function(%s) line=%d => %p\n",
    ...
    if (implementation_node != NULL) {
      VisitedSet *visited_subs = visited_set_create();
      if (visited_subs != NULL) {
        append_subprograms_from_ast_recursive(implementation_node->child,
                                              &subprograms, visited_subs);
        visited_set_destroy(visited_subs);
      }
    }

    {
      ...
      if (visited_subs != NULL) {
        ...
        append_top_level_subprograms_from_ast(scan_root, &subprograms,
                                              visited_subs, 0);
        visited_set_destroy(visited_subs);
      }
    }
    ...
    if (initialization_node != NULL &&
        initialization_node->typ == PASCAL_T_INITIALIZATION_SECTION) {
      /* The initialization section's child can be:
       * 1. A PASCAL_T_NONE wrapper from make_stmt_list_parser, with stmt_list
       * as child
    ...
    if (finalization_node != NULL &&
        finalization_node->typ == PASCAL_T_FINALIZATION_SECTION) {
    ...
    Tree_t *tree = mk_unit(cur->line, unit_id, interface_uses,
                           ...
                           list_builder_finish(&interface_var_builder),
                           implementation_uses, implementation_const_decls,
                           implementation_type_decls,
                           list_builder_finish(&implementation_var_builder),
 ...
 * implementations. For overloaded methods, we need to match by parameter count.
 */
      ...
      if (template->params_ast != NULL) {
        ...
        while (param != NULL) {
          if (param->typ == PASCAL_T_PARAM) {
            for (ast_t *child = param->child; child != NULL;
                 child = child->next) {
