  ...
  if (kgpc_getenv("KGPC_DEBUG_TFPG") != NULL)
    fprintf(stderr, "[KGPC] inspect node typ=%d (%s) sym=%s\n", node->typ,
            ...
            (node->sym != NULL && node->sym->name != NULL) ? node->sym->name
                                                           : "<null>");
  if ((node->typ == PASCAL_T_IDENTIFIER || node->typ == PASCAL_T_TYPE_ARG) &&
...
ListNode_t *collect_constructed_type_args(ast_t *args_node) {
  ...
  while (cursor != NULL) {
    ...
    if (node != NULL &&
        (node->typ == PASCAL_T_TYPE_ARG || node->typ == PASCAL_T_IDENTIFIER ||
...
int extract_constructed_type_info(ast_t *spec_node, char **base_name_out,
...
static char *mangle_specialized_type_name(const char *base_name,
                                          char **type_ids, int num_types) {
  if (base_name == NULL)
    return NULL;
...
char *mangle_specialized_name_from_list(const char *base_name,
  ...
  if (arg_ids == NULL)
    return NULL;
  int idx = 0;
  for (ListNode_t *cur = type_args; cur != NULL && idx < count;
       cur = cur->next, ++idx) {
...
static void substitute_identifier(char **type_id, GenericTypeDecl *generic_decl,
                                  char **arg_types) {
  if (type_id == NULL || *type_id == NULL || generic_decl == NULL ||
      arg_types == NULL)
    return;
...
static void substitute_type_ref(TypeRef **type_ref,
                                GenericTypeDecl *generic_decl,
                                char **arg_types) {
  if (type_ref == NULL || *type_ref == NULL || generic_decl == NULL ||
      arg_types == NULL)
    return;
...
static void substitute_record_type_parameters(struct RecordType *record,
                                              GenericTypeDecl *generic_decl,
                                              char **arg_types);

static void substitute_record_field(struct RecordField *field,
                                    GenericTypeDecl *generic_decl,
                                    char **arg_types) {
  if (field == NULL)
    return;
  ...
  if (debug_env != NULL && field->name != NULL) {
    fprintf(stderr,
            "[KGPC] substitute_record_field BEFORE: name=%s is_array=%d "
            "type_id=%s array_element_type_id=%s\n",
            field->name, field->is_array,
            field->type_id ? field->type_id : "<null>",
            field->array_element_type_id ? field->array_element_type_id
                                         : "<null>");
  }
  ...
  if (field->array_element_type_id != NULL)
    substitute_identifier(&field->array_element_type_id, generic_decl,
                          arg_types);
  ...
  if (field->nested_record != NULL)
    substitute_record_type_parameters(field->nested_record, generic_decl,
                                      arg_types);

  if (debug_env != NULL && field->name != NULL) {
    fprintf(stderr,
            "[KGPC] substitute_record_field AFTER: name=%s is_array=%d "
            "type_id=%s array_element_type_id=%s\n",
            field->name, field->is_array,
            field->type_id ? field->type_id : "<null>",
            field->array_element_type_id ? field->array_element_type_id
...
static void substitute_record_type_parameters(struct RecordType *record,
  ...
  while (field_node != NULL) {
    if (field_node->type == LIST_RECORD_FIELD)
      substitute_record_field((struct RecordField *)field_node->cur,
                              generic_decl, arg_types);
...
struct RecordType *instantiate_generic_record(const char *base_name,
                                              ListNode_t *type_args,
                                              char **specialized_name_out) {
  ...
  if (arg_count != generic->num_type_params || arg_count <= 0) {
    if (debug_env != NULL)
      fprintf(stderr,
              "[KGPC] instantiate_generic_record wrong arg count for %s: got "
              "%d expected %d\n",
  ...
  if (idx != arg_count) {
    if (debug_env != NULL)
      fprintf(stderr,
              "[KGPC] instantiate_generic_record failed to copy args for %s\n",
              base_name);
  ...
  char *specialized_name =
      mangle_specialized_type_name(base_name, arg_types, arg_count);
  ...
  if (record == NULL) {
    if (debug_env != NULL)
      fprintf(
          stderr,
          "[KGPC] instantiate_generic_record failed to clone template for %s\n",
          base_name);
        ...
        if (field->type_id != NULL) {
          ...
          while (nt != NULL) {
            if (nt->type == LIST_TREE && nt->cur != NULL) {
              ...
              if (nt_tree->type == TREE_TYPE_DECL &&
                  nt_tree->tree_data.type_decl_data.id != NULL) {
                ...
                if (strncmp(nt_id, base_name, gen_prefix_len) == 0 &&
                  ...
                  if (strcasecmp(field->type_id, short_name) == 0) {
                     ...
                     */
                    size_t new_len =
                        strlen(specialized_name) + 1 + strlen(short_name) + 1;
                    char *new_id = (char *)malloc(new_len);
                    if (new_id != NULL) {
                      snprintf(new_id, new_len, "%s.%s", specialized_name,
                               short_name);
                      free(field->type_id);
  ...
  generic_registry_add_specialization(base_name, record->generic_args,
                                      arg_count);
...
static int build_generic_arg_array(ListNode_t *type_args, char ***arg_types_out,
                                   int *arg_count_out) {
...
int resolve_generic_alias_type(const char *base_name, ListNode_t *type_args,
                               char **type_id_out, TypeInfo *type_info,
                               int *result_out) {
  ...
  ast_t *generic_ast = generic_decl_tree != NULL
                           ? generic_decl_tree->tree_data.type_decl_data.info
                                 .generic.original_ast
                           : NULL;
  if (generic == NULL || generic->record_template != NULL ||
      generic_ast == NULL) {
    if (debug_env != NULL)
      fprintf(stderr,
              "[KGPC] resolve_generic_alias_type skip base=%s generic=%p "
              "record_template=%p original_ast=%p\n",
  ...
  if (arg_count != generic->num_type_params) {
    if (debug_env != NULL)
      fprintf(stderr,
              "[KGPC] resolve_generic_alias_type arg mismatch base=%s got=%d "
              "expected=%d\n",
              base_name, arg_count, generic->num_type_params);
    for (int i = 0; i < arg_count; ++i)
      free(arg_types[i]);
  ...
  substitute_generic_identifiers(ast_copy, generic->type_parameters, arg_types,
                                 arg_count);
  int result = convert_type_spec(ast_copy, type_id_out, NULL, type_info);
  if (debug_env != NULL)
    fprintf(stderr,
            "[KGPC] resolve_generic_alias_type resolved base=%s result=%d "
            "is_array=%d\n",
...
void record_generic_method_impl(const char *class_name, const char *method_name,
                                ast_t *method_ast) {
  ...
  if (generic == NULL || generic->record_template == NULL) {
    if (kgpc_getenv("KGPC_DEBUG_GENERIC_METHODS") != NULL && class_name != NULL)
      fprintf(stderr,
              "[KGPC] record_generic_method_impl: no generic decl for %s\n",
              class_name);
    return;
  }
...
static void substitute_generic_identifier_nodes(ast_t *node,
  ...
  while (cursor != NULL) {
    if (cursor->sym != NULL && cursor->sym->name != NULL) {
      ...
      for (int i = 0; i < record->generic_decl->num_type_params &&
                      i < record->num_generic_args;
           ++i) {
       ...
       * E.g. TFPGListEnumeratorSpec ->
       * TFPGList$TMyRecord.TFPGListEnumeratorSpec inside method bodies of the
       * specialized class. */
      if (record->generic_decl->nested_type_decls != NULL &&
          record->type_id != NULL) {
            ...
            if (nt_tree->type == TREE_TYPE_DECL &&
              ...
              if (gen_prefix_len > 0 &&
                ...
                if (strcasecmp(cursor->sym->name, short_name) == 0) {
                  ...
                  size_t new_len =
                      strlen(record->type_id) + 1 + strlen(short_name) + 1;
                  char *new_name = (char *)malloc(new_len);
                  if (new_name != NULL) {
                    snprintf(new_name, new_len, "%s.%s", record->type_id,
                             short_name);
                    free(cursor->sym->name);
...
static void rewrite_method_impl_ast(ast_t *method_ast,
                                    struct RecordType *record) {
  ...
  if (qualified != NULL && qualified->typ == PASCAL_T_QUALIFIED_IDENTIFIER) {
    ...
    if (class_node != NULL && class_node->sym != NULL &&
        record->type_id != NULL) {
...
static Tree_t *
instantiate_method_template(struct MethodTemplate *method_template,
                            struct RecordType *record) {
  if (method_template == NULL || method_template->method_impl_ast == NULL ||
      record == NULL)
    return NULL;
  ...
  if (record->type_id != NULL && method_template->name != NULL) {
    int param_count =
        from_cparser_count_params_ast(method_template->params_ast);
    int param_types_count = 0;
    TypeRef **param_types = param_types_from_params_ast(
        method_template->params_ast, &param_types_count);
    register_class_method_ex(
        record->type_id, method_template->name, method_template->is_virtual,
        method_template->is_override, method_template->is_static,
        method_template->is_class_method, param_count, param_types,
        param_types_count);
  }
...
static int subprogram_list_has_id(const ListNode_t *subprograms,
                                  const char *id) {
...
static int parse_generic_mangled_id(const char *mangled, char **base_out,
  ...
  if (mangled == NULL || base_out == NULL || args_out == NULL ||
      argc_out == NULL)
    return 0;
...
void substitute_generic_identifiers(ast_t *node, char **params, char **args,
                                    int count) {
...
static void collect_local_type_names(ast_t *node, char ***names_out,
                                     int *count_out) {
  *names_out = NULL;
  *count_out = 0;
...
static void rewrite_generic_subprogram_ast(ast_t *subprogram_ast,
  ...
  if (local_count > 0 && local_names != NULL) {
    ...
    if (renamed != NULL) {
      ...
      substitute_generic_identifiers(subprogram_ast, local_names, renamed,
                                     local_count);
      for (int i = 0; i < local_count; i++)
        free(renamed[i]);
      free(renamed);
...
static Tree_t *instantiate_generic_subprogram(Tree_t *template,
  ...
  ast_t *ast_copy =
      copy_ast(template->tree_data.subprogram_data.generic_template_ast);
  if (ast_copy == NULL)
    return NULL;

  rewrite_generic_subprogram_ast(
      ast_copy, specialized_name,
      template->tree_data.subprogram_data.generic_type_params, arg_types,
      arg_count);
  ...
  int saved_offset = g_source_offset;
  g_source_offset =
      template->tree_data.subprogram_data.generic_template_source_offset;
...
static void collect_specialize_from_expr(struct Expression *expr,
                                         Tree_t *program_tree) {
  ...
  case EXPR_TYPECAST: {
    ...
    if (target_id != NULL &&
      ...
      for (int i = 0; i < argc; ++i) {
        if (args[i] != NULL && strlen(args[i]) == 1 &&
            isupper((unsigned char)args[i][0])) {
        ...
        while (cur != NULL) {
          if (cur->type == LIST_TREE && cur->cur != NULL) {
            ...
            if (sub->type == TREE_SUBPROGRAM &&
                ...
                sub->tree_data.subprogram_data.num_generic_type_params ==
                    argc &&
              ...
              if (!subprogram_list_has_id(
                      program_tree->tree_data.program_data.subprograms,
                      spec_name)) {
                Tree_t *specialized =
                    instantiate_generic_subprogram(sub, args, argc, spec_name);
                if (specialized != NULL)
                  append_subprogram_node(
                      &program_tree->tree_data.program_data.subprograms,
                      specialized);
              }
    ...
    if (base != NULL)
      free(base);
    collect_specialize_from_expr(expr->expr_data.typecast_data.expr,
                                 program_tree);
    break;
  }
  case EXPR_FUNCTION_CALL: {
    ...
    while (arg != NULL) {
      if (arg->type == LIST_EXPR)
        collect_specialize_from_expr((struct Expression *)arg->cur,
                                     program_tree);
      arg = arg->next;
    }
    break;
  ...
  case EXPR_RECORD_ACCESS:
    collect_specialize_from_expr(expr->expr_data.record_access_data.record_expr,
                                 program_tree);
    break;
  case EXPR_ARRAY_ACCESS:
    collect_specialize_from_expr(expr->expr_data.array_access_data.array_expr,
                                 program_tree);
    collect_specialize_from_expr(expr->expr_data.array_access_data.index_expr,
                                 program_tree);
    break;
  case EXPR_RELOP:
    collect_specialize_from_expr(expr->expr_data.relop_data.left, program_tree);
    collect_specialize_from_expr(expr->expr_data.relop_data.right,
                                 program_tree);
    break;
  case EXPR_SIGN_TERM:
    ...
    break;
  case EXPR_ADDOP:
    collect_specialize_from_expr(expr->expr_data.addop_data.left_expr,
                                 program_tree);
    collect_specialize_from_expr(expr->expr_data.addop_data.right_term,
                                 program_tree);
    break;
  case EXPR_MULOP:
    collect_specialize_from_expr(expr->expr_data.mulop_data.left_term,
                                 program_tree);
    collect_specialize_from_expr(expr->expr_data.mulop_data.right_factor,
                                 program_tree);
    break;
  case EXPR_ADDR:
  ...
  case EXPR_POINTER_DEREF:
    collect_specialize_from_expr(
        expr->expr_data.pointer_deref_data.pointer_expr, program_tree);
    break;
  default:
    break;
  }
...
static void collect_specialize_from_stmt(struct Statement *stmt,
                                         Tree_t *program_tree) {
  ...
  case STMT_VAR_ASSIGN:
    collect_specialize_from_expr(stmt->stmt_data.var_assign_data.var,
                                 program_tree);
    collect_specialize_from_expr(stmt->stmt_data.var_assign_data.expr,
                                 program_tree);
    break;
  ...
  case STMT_EXPR:
    collect_specialize_from_expr(stmt->stmt_data.expr_stmt_data.expr,
                                 program_tree);
    break;
  case STMT_COMPOUND_STATEMENT: {
    ...
    while (cur != NULL) {
      if (cur->type == LIST_STMT)
        collect_specialize_from_stmt((struct Statement *)cur->cur,
                                     program_tree);
      cur = cur->next;
    }
    break;
  ...
  case STMT_LABEL:
    ...
    break;
  case STMT_IF_THEN:
    collect_specialize_from_expr(stmt->stmt_data.if_then_data.relop_expr,
                                 program_tree);
    collect_specialize_from_stmt(stmt->stmt_data.if_then_data.if_stmt,
                                 program_tree);
    collect_specialize_from_stmt(stmt->stmt_data.if_then_data.else_stmt,
                                 program_tree);
    break;
  case STMT_WHILE:
    collect_specialize_from_expr(stmt->stmt_data.while_data.relop_expr,
                                 program_tree);
    collect_specialize_from_stmt(stmt->stmt_data.while_data.while_stmt,
                                 program_tree);
    break;
  case STMT_REPEAT: {
    ...
    collect_specialize_from_expr(stmt->stmt_data.repeat_data.until_expr,
                                 program_tree);
    break;
  }
  case STMT_FOR:
    ...
    if (stmt->stmt_data.for_data.for_assign_type == STMT_VAR_ASSIGN)
      collect_specialize_from_stmt(
          stmt->stmt_data.for_data.for_assign_data.var_assign, program_tree);
    else
      collect_specialize_from_expr(stmt->stmt_data.for_data.for_assign_data.var,
                                   program_tree);
    break;
  case STMT_FOR_IN:
    collect_specialize_from_expr(stmt->stmt_data.for_in_data.loop_var,
                                 program_tree);
    collect_specialize_from_expr(stmt->stmt_data.for_in_data.collection,
                                 program_tree);
    collect_specialize_from_stmt(stmt->stmt_data.for_in_data.do_stmt,
                                 program_tree);
    break;
  case STMT_CASE: {
    collect_specialize_from_expr(stmt->stmt_data.case_data.selector_expr,
                                 program_tree);
      ...
      if (branch->type == LIST_CASE_BRANCH && branch->cur != NULL) {
        ...
        while (label != NULL) {
          if (label->type == LIST_EXPR)
            collect_specialize_from_expr((struct Expression *)label->cur,
                                         program_tree);
          label = label->next;
        }
    ...
    collect_specialize_from_stmt(stmt->stmt_data.case_data.else_stmt,
                                 program_tree);
    break;
  }
  case STMT_WITH:
    collect_specialize_from_expr(stmt->stmt_data.with_data.context_expr,
                                 program_tree);
    collect_specialize_from_stmt(stmt->stmt_data.with_data.body_stmt,
                                 program_tree);
    break;
  ...
  case STMT_RAISE:
    collect_specialize_from_expr(stmt->stmt_data.raise_data.exception_expr,
                                 program_tree);
    break;
  case STMT_INHERITED:
    collect_specialize_from_expr(stmt->stmt_data.inherited_data.call_expr,
                                 program_tree);
    break;
  case STMT_EXIT:
    collect_specialize_from_expr(stmt->stmt_data.exit_data.return_expr,
                                 program_tree);
    break;
  default:
    break;
...
void resolve_pending_generic_subprograms(Tree_t *program_tree) {
    ...
    if (sub->type == LIST_TREE && sub->cur != NULL) {
      ...
      if (sub_tree->type == TREE_SUBPROGRAM) {
        ...
        if (sub_tree->tree_data.subprogram_data.generic_type_params == NULL &&
          ...
          collect_specialize_from_stmt(
              sub_tree->tree_data.subprogram_data.statement_list, program_tree);
      }
    }
    sub = sub->next;
  }
  ...
  collect_specialize_from_stmt(
      program_tree->tree_data.program_data.body_statement, program_tree);
    ...
    if (comp_ctx != NULL) {
      for (int ui = 0; ui < comp_ctx->loaded_unit_count; ++ui) {
        ...
        if (unit_tree->tree_data.unit_data.initialization != NULL)
          collect_specialize_from_stmt(
              unit_tree->tree_data.unit_data.initialization, program_tree);
        if (unit_tree->tree_data.unit_data.finalization != NULL)
          collect_specialize_from_stmt(
              unit_tree->tree_data.unit_data.finalization, program_tree);
      }
    }
  }
}

static int subprogram_list_has_decl(const ListNode_t *subprograms,
                                    const Tree_t *tree) {
...
void append_subprograms_from_ast_recursive(ast_t *node,
...
void append_top_level_subprograms_from_ast(ast_t *node,
                                           ListNode_t **subprograms,
                                           VisitedSet *visited,
                                           int in_subprogram) {
  /* Iterate over siblings to avoid deep recursion on node->next chains */
  for (ast_t *cur = node; cur != NULL && cur != ast_nil; cur = cur->next) {
    ...
    /* Recurse into children (tree depth is bounded) */
    append_top_level_subprograms_from_ast(cur->child, subprograms, visited,
                                          child_in_subprogram);
  }
}
...
static int subprogram_list_has_mangled(const ListNode_t *subprograms,
  ...
  while (cur != NULL) {
    if (cur->type == LIST_TREE && cur->cur != NULL) {
      const Tree_t *tree = (const Tree_t *)cur->cur;
      if ((tree->type == TREE_SUBPROGRAM_FUNC ||
           tree->type == TREE_SUBPROGRAM_PROC) &&
...
void append_specialized_method_clones(Tree_t *decl, ListNode_t **subprograms) {
  ...
  if (record == NULL || record->method_templates == NULL ||
    ...
    if (kgpc_getenv("KGPC_DEBUG_GENERIC_CLONES") != NULL && record != NULL &&
        record->type_id != NULL)
  ...
  if (record->generic_decl->type_parameters != NULL) {
    ...
    for (int i = 0; i < record->num_generic_args &&
                    i < record->generic_decl->num_type_params;
         ++i) {
    ...
    if (all_unresolved) {
      if (kgpc_getenv("KGPC_DEBUG_GENERIC_CLONES") != NULL &&
          record->type_id != NULL)
        fprintf(stderr,
                "[KGPC] skipping clone for %s (unresolved type params)\n",
                record->type_id);
      return;
    }
  }
        ...
        if (method_tree != NULL) {
          ...
          if (debug_env != NULL && record->type_id != NULL &&
            ...
            fprintf(stderr, "[KGPC] cloned method %s.%s\n", record->type_id,
                    template->name);
        } else if (debug_env != NULL && record->type_id != NULL &&
                   template->name != NULL) {
          fprintf(
              stderr,
              "[KGPC] failed to clone method %s.%s (missing implementation)\n",
...
void append_generic_method_clones(Tree_t *program_tree) {
  ...
  while (type_cursor != NULL) {
    if (type_cursor->type == LIST_TREE && type_cursor->cur != NULL) {
      ...
      append_specialized_method_clones(
          decl, &program_tree->tree_data.program_data.subprograms);
    }
...
static void clone_nested_types_for_specialization(
    ...
    ListNode_t *
        *subprograms_out) /* where to append method clones (may be NULL) */
{
  ...
  while (cur != NULL) {
    if (cur->type == LIST_TREE && cur->cur != NULL) {
      ...
      if (orig->type == TREE_TYPE_DECL &&
          orig->tree_data.type_decl_data.id != NULL) {
        ...
        if (prefix_len > 0 && strncmp(orig_id, generic_name, prefix_len) == 0 &&
          ...
          clone->tree_data.type_decl_data.kind =
              orig->tree_data.type_decl_data.kind;
          clone->tree_data.type_decl_data.defined_in_unit =
              orig->tree_data.type_decl_data.defined_in_unit;

          if (orig->tree_data.type_decl_data.kind == TYPE_DECL_ALIAS) {
            struct TypeAlias *src_alias =
                &orig->tree_data.type_decl_data.info.alias;
            struct TypeAlias *dst_alias =
                &clone->tree_data.type_decl_data.info.alias;
            *dst_alias = *src_alias; /* Shallow copy */
            dst_alias->target_type_id = src_alias->target_type_id
                                            ? strdup(src_alias->target_type_id)
                                            : NULL;
            dst_alias->pointer_type_id =
                src_alias->pointer_type_id ? strdup(src_alias->pointer_type_id)
                                           : NULL;
            dst_alias->array_element_type_id =
                src_alias->array_element_type_id
                    ? strdup(src_alias->array_element_type_id)
                    : NULL;
            dst_alias->inline_record_type = NULL;
            dst_alias->target_type_ref =
                src_alias->target_type_ref
                    ? type_ref_clone(src_alias->target_type_ref)
                    : NULL;
            dst_alias->array_element_type_ref =
                src_alias->array_element_type_ref
                    ? type_ref_clone(src_alias->array_element_type_ref)
                    : NULL;
            ...
            for (int pi = 0;
                 pi < generic_decl->num_type_params && pi < arg_count; pi++) {
              ...
              for (int pi = 0;
                ...
                while ((pos = strchr(pos, '$')) != NULL) {
                  ...
                  if (strncasecmp(pos, param, param_len) == 0 &&
                    ...
                    char *new_target =
                        malloc(prefix_len2 + concrete_len + suffix_len + 1);
                    if (new_target != NULL) {
                      memcpy(new_target, dst_alias->target_type_id,
                             prefix_len2);
            ...
            if (dst_alias->target_type_id != NULL &&
                strncmp(dst_alias->target_type_id, generic_name, prefix_len) ==
                    0 &&
                dst_alias->target_type_id[prefix_len] == '.') {
              const char *inner_suffix = dst_alias->target_type_id + prefix_len;
              size_t new_target_len =
                  strlen(specialized_name) + strlen(inner_suffix) + 1;
              char *new_target = (char *)malloc(new_target_len);
              if (new_target != NULL) {
                snprintf(new_target, new_target_len, "%s%s", specialized_name,
                         inner_suffix);
                free(dst_alias->target_type_id);
                dst_alias->target_type_id = new_target;
              }
          ...
          if (clone->tree_data.type_decl_data.kind == TYPE_DECL_ALIAS) {
            ...
            if (debug_env != NULL && dst_alias->target_type_id != NULL)
              fprintf(stderr, "[KGPC] clone alias %s target_type_id=%s\n",
                      clone->tree_data.type_decl_data.id,
                      dst_alias->target_type_id);
            if (dst_alias->target_type_id != NULL) {
              ...
              if (dollar != NULL) {
                ...
                GenericTypeDecl *inner_generic =
                    generic_registry_find_decl(inner_base);
                if (inner_generic != NULL) {
                  /* Build type args from the suffix */
                  const char *arg_str = dollar + 1;
                  ListNode_t *inner_args =
                      CreateListNode(strdup(arg_str), LIST_STRING);
                  char *inner_spec_name = NULL;
                  struct RecordType *inner_record = instantiate_generic_record(
                      inner_base, inner_args, &inner_spec_name);
                  if (inner_record != NULL) {
                    ...
                    if (debug_env != NULL)
                      fprintf(stderr,
                              "[KGPC] triggered nested specialization %s\n",
                              inner_spec_name);
                  }
                  if (inner_spec_name != NULL)
                    free(inner_spec_name);
          ...
          if (!alias_type_set &&
              orig->tree_data.type_decl_data.kgpc_type != NULL) {
            clone->tree_data.type_decl_data.kgpc_type =
                orig->tree_data.type_decl_data.kgpc_type;
...
void resolve_pending_generic_aliases(Tree_t *program_tree) {
  ...
  while (cur != NULL) {
    ...
    struct RecordType *record = instantiate_generic_record(
        cur->base_name, cur->type_args, &specialized_name);
    if (record != NULL && cur->decl != NULL &&
      ...
      if (type_list != NULL && specialized_name != NULL &&
          cur->base_name != NULL) {
        GenericTypeDecl *generic = generic_registry_find_decl(cur->base_name);
        if (generic != NULL && generic->nested_type_decls != NULL) {
          clone_nested_types_for_specialization(
              generic, specialized_name, cur->type_args, type_list, clone_dest);
        }
      }
      ...
      fprintf(stderr,
              "ERROR: Failed to instantiate generic record %s for deferred "
              "alias.\n",
