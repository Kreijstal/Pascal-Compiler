...
static Tree_t *convert_var_decl(ast_t *decl_node) {
  ...
  while (first_non_identifier != NULL &&
         first_non_identifier->typ == PASCAL_T_IDENTIFIER)
  ...
  if (type_node != NULL && type_node->typ == PASCAL_T_TYPE_SPEC) {
    ...
    cur = type_node->next;
  } else if (type_node != NULL && type_node->typ == PASCAL_T_IDENTIFIER &&
             type_node->next == NULL) {
    ...
    while (wrapped_type != NULL && wrapped_type->typ != PASCAL_T_TYPE_SPEC &&
  ...
  if (var_type == UNKNOWN_TYPE &&
      (type_node == NULL || var_type == UNKNOWN_TYPE)) {
  ...
  if (var_type == PROCEDURE) {
    ...
    if (search != NULL) {
      ...
      if (spec_node != NULL && (spec_node->typ == PASCAL_T_PROCEDURE_TYPE ||
  ...
  if (type_info.is_array) {
    ...
    if (cur != NULL) {
      /* Handle optional initializer wrapper: optional(seq(...)) creates
       * PASCAL_T_NONE node */
      ast_t *init_node = cur;
      if (init_node->typ == PASCAL_T_NONE && init_node->child != NULL) {
        if (absolute_clause_target(init_node) != NULL ||
            is_var_hint_clause(init_node)) {
          init_node = NULL;
        } else {
      ...
      if (init_node != NULL && ids != NULL && ids->next == NULL) {
        ...
        if (init_node->typ == PASCAL_T_TUPLE) {
          ...
          for (ast_t *elem = init_node->child; elem != NULL;
               elem = elem->next) {
            ...
            struct Expression *base_expr =
                mk_varid(decl_node->line, strdup(var_name));
            struct Expression *array_access =
                mk_arrayaccess(decl_node->line, base_expr, index_expr);
            struct Statement *assign_stmt = mk_varassign(
                decl_node->line, decl_node->col, array_access, elem_expr);
          ...
          if (init_expr != NULL) {
            struct Expression *lhs =
                mk_varid(decl_node->line, strdup(var_name));
            initializer_stmt =
                mk_varassign(decl_node->line, decl_node->col, lhs, init_expr);
          }
        }
      }
    }
    ...
    Tree_t *decl = mk_arraydecl(decl_node->line, ids, element_type,
                                element_type_id, type_info.start, type_info.end,
                                range_str, initializer_stmt, inline_record);
    if (decl != NULL)
    ...
    if (decl != NULL && type_info.element_kgpc_type != NULL) {
      decl->tree_data.arr_decl_data.element_kgpc_type =
    ...
    if (decl != NULL && type_info.unresolved_index_type != NULL) {
      decl->tree_data.arr_decl_data.unresolved_index_type =
          type_info.unresolved_index_type;
    ...
    if (decl != NULL && type_info.array_dimensions != NULL &&
      ...
      decl->tree_data.arr_decl_data.array_dimensions =
          type_info.array_dimensions;
     ...
     * Transfer the dimension name as unresolved_index_type so semcheck can
     * resolve it. */
    if (decl != NULL &&
  ...
  if (cur != NULL) {
    ...
    /* Skip nodes that should not be treated as initializers */
    if (is_node_to_skip_as_initializer(init_node) &&
        !(init_node->typ == PASCAL_T_IDENTIFIER && init_node != type_node &&
          (type_id != NULL || var_type != UNKNOWN_TYPE))) {
      init_node = NULL;
    }

    /* Skip EXTERNAL_NAME and PUBLIC_NAME modifiers - they're handled later */
    if (init_node != NULL && init_node->typ != PASCAL_T_EXTERNAL_NAME &&
      ...
      if (init_node->typ == PASCAL_T_TUPLE &&
          !tuple_is_record_constructor(init_node) && ids != NULL &&
          ids->next == NULL) {
        char *var_name = (char *)ids->cur;
        ListBuilder stmt_builder;
        list_builder_init(&stmt_builder);
  ...
  if (var_type == UNKNOWN_TYPE && ids != NULL && ids->next != NULL &&
      type_node == NULL) {
  ...
  if (inline_alias == NULL && type_info.is_set) {
    ...
    if (inline_alias != NULL) {
      ...
      if (type_info.set_element_type_id != NULL)
        inline_alias->set_element_type_id =
            strdup(type_info.set_element_type_id);
  ...
  if (kgpc_debug_decl_scan_enabled() && absolute_target != NULL &&
      ids != NULL && ids->type == LIST_STRING) {
    fprintf(stderr, "[KGPC] var absolute: %s -> %s\n", (char *)ids->cur,
            absolute_target);
  }

  Tree_t *decl = mk_vardecl(decl_node->line, ids, var_type, type_id, 0,
                            inferred, initializer_stmt, inline_record,
                            inline_alias, absolute_target);
...
ListNode_t *convert_var_section(ast_t *section_node) {
  ...
  if (visited == NULL) {
    fprintf(
        stderr,
        "ERROR: Failed to allocate visited set for var section traversal\n");
    return NULL;
  }
  ...
  while (cur != NULL && cur->typ == PASCAL_T_VAR_DECL) {
    ...
    if (!is_safe_to_continue(visited, cur)) {
      fprintf(stderr, "ERROR: Circular reference detected in var section, "
                      "stopping traversal\n");
      break;
    }
...
static int lower_const_array(ast_t *const_decl_node, char **id_ptr,
                             TypeInfo *type_info, ast_t *value_node,
  ...
  if (var_builder == NULL) {
    fprintf(stderr,
            "ERROR: Cannot lower const array %s without a variable declaration "
            "list.\n",
            *id_ptr);
    return -1;
  }
  ...
  int is_multidim = (type_info->array_dimensions != NULL &&
                     type_info->array_dimensions->next != NULL);
  int is_3d = 0;
  int is_4d = 0;
  int multidim_inner_start = 0, multidim_inner_end = -1;
  int multidim_infer_bounds =
      0; /* Set to 1 if we need to infer bounds from initializer */
  ...
  if (is_multidim) {
    ...
    if (type_info->array_dimensions->next->next != NULL) {
      ...
      const char *dim2_range =
          (const char *)type_info->array_dimensions->next->cur;
      if (dim2_range != NULL) {
      ...
      const char *dim3_range =
          (const char *)type_info->array_dimensions->next->next->cur;
      if (dim3_range != NULL) {
      ...
      if (is_4d) {
        const char *dim4_range =
            (const char *)type_info->array_dimensions->next->next->next->cur;
        if (dim4_range != NULL) {
            ...
            if (dotdot != NULL) {
              *dotdot = '\0';
              ...
              if (dim4_start == 0 && dim4_end == 0) {
                ...
                if (has_alpha) {
                  int s =
                      resolve_enum_ordinal_from_ast(range_copy, type_section);
                  int e =
                      resolve_enum_ordinal_from_ast(dotdot + 2, type_section);
                  /* Cross-unit fallback (see comment on 2nd dim) */
                  if (s < 0) {
                    int cached = 0;
                    if (lookup_const_int(range_copy, &cached) == 0)
  ...
  if (type_info->is_open_array) {
    ...
    if (open_tuple != NULL) {
      ...
      if (uw != NULL &&
          (uw->typ == PASCAL_T_TYPE_SPEC || uw->typ == PASCAL_T_ARRAY_TYPE ||
  ...
  if (tuple_node == NULL) {
    fprintf(
        stderr,
        "ERROR: Const array %s must use tuple syntax for its initializer.\n",
        *id_ptr);
    return -1;
  }
  int single_record_element = 0;
  int is_widechar_array_target = 0;
  int is_char_array_target =
      type_info_targets_char_array(type_info, &is_widechar_array_target);
  ...
  if (tuple_node->typ == PASCAL_T_STRING) {
    ...
    if (tuple_node->sym != NULL && tuple_node->sym->name != NULL) {
      if (resolve_const_string_from_ast_internal(tuple_node->sym->name,
        ...
        fprintf(stderr,
                "ERROR: Const array %s must use tuple syntax for its "
                "initializer.\n",
                *id_ptr);
        return -1;
      }
    ...
    if (evaluate_const_string_ast(tuple_node, const_section,
                                  &owned_string_initializer, 0) == 0) {
      is_string_initializer = 1;
  ...
  while (!single_record_element && expected_count != 1 && tuple_node != NULL &&
         tuple_node->typ == PASCAL_T_TUPLE && tuple_node->child != NULL &&
         tuple_node->child->next == NULL) {
    ast_t *nested_tuple = unwrap_pascal_node(tuple_node->child);
    if (nested_tuple == NULL || nested_tuple->typ != PASCAL_T_TUPLE ||
  ...
  if (is_string_initializer) {
    if (owned_string_initializer.data != NULL ||
        owned_string_initializer.len > 0)
  ...
  if (is_string_initializer && is_multidim && !is_shortstring_target) {
    int inner_count = (multidim_inner_end >= multidim_inner_start)
                          ? (multidim_inner_end - multidim_inner_start + 1)
                          : -1;
    ...
    if (outer_count > 0 && inner_count > 0) {
      ...
      if (is_4d && total > 0) {
        int fourth =
            (dim4_end >= dim4_start) ? (dim4_end - dim4_start + 1) : -1;
        if (fourth > 0)
          total *= fourth;
        else
          total = -1;
  ...
  if (is_shortstring_target && expected_count > 0) {
    ...
    if (actual_count > visible_capacity) {
      ast_string_value_reset(&owned_string_initializer);
      fprintf(stderr,
              "ERROR: Const shortstring %s initializer length %d exceeds "
              "declared capacity %d.\n",
    ...
    if (actual_count != flat_string_multidim_total) {
      ast_string_value_reset(&owned_string_initializer);
      fprintf(stderr,
              "ERROR: Const array %s flat-string initializer length %d does "
              "not match total element count %d.\n",
    ...
    ast_string_value_reset(&owned_string_initializer);
    fprintf(stderr,
            "ERROR: Const array %s initializer count %d does not match "
            "declared range %d..%d.\n",
  ...
  if (type_info->element_type == UNKNOWN_TYPE &&
      type_info->element_type_id != NULL) {
    if (resolve_array_type_info_from_ast(type_info->element_type_id,
                                         type_section, &element_array_info,
                                         0) == 0 &&
      ...
      if (element_array_info.array_dimensions != NULL &&
        ...
        fprintf(stderr,
                "ERROR: Unsupported multi-dimensional array element for const "
                "%s.\n",
                *id_ptr);
        destroy_type_info_contents(&element_array_info);
        return -1;
      }
      resolve_array_bounds(&element_array_info, type_section, const_section,
                           type_info->element_type_id);
    }
  }
  ...
  if (is_multidim && !element_is_array) {
    ...
    element_array_info.element_type_id =
        type_info->element_type_id != NULL ? strdup(type_info->element_type_id)
                                           : NULL;
    ...
    if (is_3d) {
      ...
      element_2d_inner_info.element_type_id =
          type_info->element_type_id != NULL
              ? strdup(type_info->element_type_id)
              : NULL;
    }
    ...
    if (is_4d) {
      ...
      element_3d_inner_info.element_type_id =
          type_info->element_type_id != NULL
              ? strdup(type_info->element_type_id)
              : NULL;
    }
    ...
    /* For enum-indexed arrays, infer inner dimension from first row of
     * initializer */
    if (multidim_infer_bounds && tuple_node != NULL &&
        tuple_node->typ == PASCAL_T_TUPLE) {
      ast_t *first_row = tuple_node->child;
      if (first_row != NULL) {
        ast_t *first_row_unwrapped = unwrap_pascal_node(first_row);
        if (first_row_unwrapped != NULL &&
            first_row_unwrapped->typ == PASCAL_T_TUPLE) {
          int inner_count = 0;
          for (ast_t *inner = first_row_unwrapped->child; inner != NULL;
               inner = inner->next)
            ++inner_count;
     ...
     */
    if (is_3d && dim3_infer_bounds && tuple_node != NULL &&
        tuple_node->typ == PASCAL_T_TUPLE) {
      ast_t *first_row = tuple_node->child;
      if (first_row != NULL) {
        ast_t *first_row_unwrapped = unwrap_pascal_node(first_row);
        if (first_row_unwrapped != NULL &&
          ...
          if (first_inner != NULL) {
            ...
            if (first_inner_unwrapped != NULL &&
                first_inner_unwrapped->typ == PASCAL_T_TUPLE) {
              int innermost_count = 0;
              for (ast_t *elem = first_inner_unwrapped->child; elem != NULL;
                   elem = elem->next)
                ++innermost_count;
     ...
     */
    if (is_4d && dim4_infer_bounds && tuple_node != NULL &&
        tuple_node->typ == PASCAL_T_TUPLE) {
      ast_t *first_row = tuple_node->child;
      if (first_row != NULL) {
        ast_t *first_row_unwrapped = unwrap_pascal_node(first_row);
          ...
          if (first_inner != NULL) {
            ...
            if (first_inner_unwrapped != NULL &&
              ...
              if (first_inner2 != NULL) {
                ast_t *first_inner2_unwrapped =
                    unwrap_pascal_node(first_inner2);
                if (first_inner2_unwrapped != NULL &&
                    first_inner2_unwrapped->typ == PASCAL_T_TUPLE) {
                  int innermost_count = 0;
                  for (ast_t *elem = first_inner2_unwrapped->child;
  ...
  if (is_string_initializer) {
    ...
    if (is_shortstring_target) {
      struct Expression *len_rhs =
          mk_charcode(const_decl_node->line, (unsigned int)actual_count);
      struct Expression *len_index_expr = mk_inum(const_decl_node->line, index);
      struct Expression *len_base_expr =
          mk_varid(const_decl_node->line, strdup(*id_ptr));
      struct Expression *len_lhs =
          mk_arrayaccess(const_decl_node->line, len_base_expr, len_index_expr);
      struct Statement *len_assign = mk_varassign(
          const_decl_node->line, const_decl_node->col, len_lhs, len_rhs);
    ...
    if (flat_string_multidim_total >= 0 && !is_3d && !is_4d) {
      ...
      for (int i = 0; i < actual_count; ++i) {
        ...
        struct Expression *rhs =
            mk_charcode(const_decl_node->line, (unsigned int)byte);
        struct Expression *lhs = mk_const_array_element_lhs(
            const_decl_node->line, *id_ptr, start + outer_off,
            multidim_inner_start + inner_off, is_multidim);
        ...
        struct Statement *assign =
            mk_varassign(const_decl_node->line, const_decl_node->col, lhs, rhs);
      ...
      for (int i = 0; i < actual_count; ++i) {
        ...
        struct Expression *lhs =
            mk_arrayaccess(const_decl_node->line, base_expr, index_expr);
        if (is_widechar_array_target) {
          lhs->array_element_type = CHAR_TYPE;
          lhs->array_element_size = 2;
          lhs->array_element_type_id = strdup("WideChar");
    ...
    while (element != NULL) {
      ...
      if (element_is_array) {
        ...
        int row_is_char_array_target =
            (!element_is_2d_array && !element_is_3d_array &&
             type_info_targets_char_array(&element_array_info,
                                          &row_is_widechar_target));
        ...
        if (!row_is_string_initializer &&
          ...
          fprintf(stderr,
                  "ERROR: Const array %s expects tuple initializer for element "
                  "%d.\n",
                  *id_ptr, index);
        ...
        if (row_is_string_initializer) {
          ...
          for (ast_t *inner = unwrapped->child; inner != NULL;
               inner = inner->next)
            ++inner_actual;
        }

        if (!row_is_string_initializer && inner_expected >= 0 &&
            inner_actual != inner_expected) {
          if (inner_start == 0 && inner_end == 0 &&
              element_array_info.array_dimensions != NULL &&
              element_array_info.array_dimensions->cur != NULL) {
            const char *range_str =
                (const char *)element_array_info.array_dimensions->cur;
            int has_alpha = 0;
            for (const char *p = range_str; p != NULL && *p != '\0'; ++p) {
              if (isalpha((unsigned char)*p)) {
                has_alpha = 1;
        ...
        if (row_is_string_initializer && inner_expected >= 0 &&
            inner_actual > inner_expected) {
          fprintf(stderr,
                  "ERROR: Const array %s element %d string initializer length "
                  "%d exceeds declared range %d..%d.\n",
        ...
        if (!row_is_string_initializer && inner_expected >= 0 &&
            inner_actual != inner_expected) {
          fprintf(stderr,
                  "ERROR: Const array %s element %d initializer count %d does "
                  "not match declared range %d..%d.\n",
                  *id_ptr, index, inner_actual, inner_start, inner_end);
          destroy_list(stmt_builder.head);
          destroy_type_info_contents(&element_array_info);
          ast_string_value_reset(&row_string_initializer);
        ...
        for (ast_t *inner = unwrapped->child; inner != NULL;
          ...
          if (element_is_2d_array) {
            if (inner_unwrapped == NULL ||
                inner_unwrapped->typ != PASCAL_T_TUPLE) {
              fprintf(stderr,
                      "ERROR: Const 3D array %s expects tuple for element "
                      "[%d][%d].\n",
            ...
            for (ast_t *innermost = inner_unwrapped->child; innermost != NULL;
              ...
              if (element_is_3d_array) {
                if (innermost_unwrapped == NULL ||
                    innermost_unwrapped->typ != PASCAL_T_TUPLE) {
                  fprintf(stderr,
                          "ERROR: Const 4D array %s expects tuple for element "
                          "[%d][%d][%d].\n",
                ...
                for (ast_t *dim4_elem = innermost_unwrapped->child;
                     dim4_elem != NULL; dim4_elem = dim4_elem->next) {
                  ast_t *dim4_unwrapped = unwrap_pascal_node(dim4_elem);
                  struct Expression *rhs = convert_expression(dim4_unwrapped);
                  if (rhs == NULL) {
                    fprintf(stderr,
                            "ERROR: Unsupported const array element in "
                            "%s[%d][%d][%d][%d].\n",
                            *id_ptr, index, inner_index, innermost_index,
                            dim4_index);
                    destroy_list(stmt_builder.head);
                  ...
                  struct Expression *outer_index_expr =
                      mk_inum(element->line, index);
                  struct Expression *base_expr =
                      mk_varid(element->line, strdup(*id_ptr));
                  struct Expression *inner_index_expr =
                      mk_inum(element->line, inner_index);
                  struct Expression *innermost_index_expr =
                      mk_inum(element->line, innermost_index);
                  struct Expression *dim4_index_expr =
                      mk_inum(element->line, dim4_index);
                  struct Expression *lhs;
                  if (is_multidim) {
                    /* True multi-dim: arr[d1, d2, d3, d4] */
                    lhs = mk_arrayaccess(element->line, base_expr,
                                         outer_index_expr);
                    ListNode_t *extra3 =
                        CreateListNode(dim4_index_expr, LIST_EXPR);
                    ListNode_t *extra2 =
                        CreateListNode(innermost_index_expr, LIST_EXPR);
                    ListNode_t *extra1 =
                    ...
                    struct Expression *a1 = mk_arrayaccess(
                        element->line, base_expr, outer_index_expr);
                    struct Expression *a2 =
                        mk_arrayaccess(element->line, a1, inner_index_expr);
                    struct Expression *a3 =
                        mk_arrayaccess(element->line, a2, innermost_index_expr);
                ...
                struct Expression *rhs =
                    convert_expression(innermost_unwrapped);
                if (rhs == NULL) {
                  fprintf(stderr,
                          "ERROR: Unsupported const array element in "
                          "%s[%d][%d][%d].\n",
                ...
                if (is_multidim) {
                  ...
                  struct Expression *outer_access = mk_arrayaccess(
                      element->line, base_expr, outer_index_expr);
                  struct Expression *middle_access = mk_arrayaccess(
                      element->line, outer_access, inner_index_expr);
                  lhs = mk_arrayaccess(element->line, middle_access,
                                       innermost_index_expr);
                }
                struct Statement *assign =
                    mk_varassign(element->line, element->col, lhs, rhs);
                list_builder_append(&stmt_builder, assign, LIST_STMT);
            ...
            if (rhs == NULL) {
              fprintf(stderr,
                      "ERROR: Unsupported const array element in %s[%d].\n",
                      *id_ptr, index);
              destroy_list(stmt_builder.head);
              destroy_type_info_contents(&element_array_info);
              ast_string_value_reset(&row_string_initializer);
      ...
      /* A single-field record constructor (Ch: [...]) gets parsed as
         FIELD_WIDTH. Convert it to a single-child RECORD_CONSTRUCTOR for
         uniform handling. Skip when the field value is a TUPLE —
         convert_expression's FIELD_WIDTH handler already recognizes TUPLE
         values as record constructors. */
      if (unwrapped != NULL && unwrapped->typ == PASCAL_T_FIELD_WIDTH &&
          unwrapped->child != NULL &&
          unwrapped->child->typ == PASCAL_T_IDENTIFIER) {
      ...
      if (unwrapped != NULL && (unwrapped->typ == PASCAL_T_RECORD_CONSTRUCTOR ||
        ...
        while (field_assignment != NULL) {
          ast_t *assignment_node =
              unwrap_record_constructor_elem(field_assignment);
          if (assignment_node != NULL &&
            ...
            ast_t *field_value_node =
                (field_name_node != NULL) ? field_name_node->next : NULL;

            if (field_name_node != NULL && field_value_node != NULL &&
                field_name_node->sym != NULL) {
              char *field_name = field_name_node->sym->name;
              struct Expression *field_value = NULL;
              if (field_value_node != NULL &&
                  field_value_node->typ == PASCAL_T_TUPLE &&
              ...
              if (field_value == NULL) {
                fprintf(stderr,
                        "ERROR: Failed to convert field value for %s[%d].%s.\n",
              ...
              if (field_value != NULL) {
                ...
                struct Expression *base_expr =
                    mk_varid(element->line, strdup(*id_ptr));
                struct Expression *array_elem =
                    mk_arrayaccess(element->line, base_expr, index_expr);
                struct Expression *lhs = mk_recordaccess(
                    element->line, array_elem, strdup(field_name));

                /* Generate assignment statement */
                struct Statement *field_assign =
                    mk_varassign(element->line, element->col, lhs, field_value);
                list_builder_append(&stmt_builder, field_assign, LIST_STMT);
              } else {
                fprintf(stderr,
                        "ERROR: Unsupported field value in record constructor "
                        "for %s[%d].%s.\n",
                        *id_ptr, index, field_name);
                destroy_list(stmt_builder.head);
                return -1;
  ...
  if (inline_record != NULL)
    type_info->record_type = NULL;
  Tree_t *array_decl = mk_arraydecl(
      const_decl_node->line, ids, decl_element_type, decl_element_type_id,
      start, end, range_str, initializer, inline_record);
  ...
  if (type_info->element_kgpc_type != NULL) {
    array_decl->tree_data.arr_decl_data.element_kgpc_type =
        type_info->element_kgpc_type;
  ...
  if (type_info->array_dimensions != NULL &&
      type_info->array_dimensions->next != NULL) {
    array_decl->tree_data.arr_decl_data.array_dimensions =
        type_info->array_dimensions;
    type_info->array_dimensions = NULL;
  }

  if (type_info->unresolved_index_type != NULL) {
    array_decl->tree_data.arr_decl_data.unresolved_index_type =
        type_info->unresolved_index_type;
  ...
  if (array_decl->tree_data.arr_decl_data.unresolved_index_type == NULL &&
      start == 0 && end == 0 && type_info->array_dimensions != NULL &&
      type_info->array_dimensions->type == LIST_STRING &&
      type_info->array_dimensions->cur != NULL) {
    const char *dim_str = (const char *)type_info->array_dimensions->cur;
    if (strstr(dim_str, "..") == NULL) {
  ...
  snprintf(label_buffer, sizeof(label_buffer), "__kgpc_tconst_array_%s_%d",
           g_typed_const_unit_tag, typed_const_counter);
  array_decl->tree_data.arr_decl_data.static_label = strdup(label_buffer);
  snprintf(label_buffer, sizeof(label_buffer), "__kgpc_tconst_guard_%s_%d",
           g_typed_const_unit_tag, typed_const_counter);
...
static Tree_t *convert_const_decl(ast_t *const_decl_node,
  ...
  if (cur != NULL &&
      (cur->typ == PASCAL_T_TYPE_SPEC || cur->typ == PASCAL_T_ARRAY_TYPE ||
       cur->typ == PASCAL_T_RECORD_TYPE || cur->typ == PASCAL_T_POINTER_TYPE ||
    ...
    if (unwrapped_cur->typ == PASCAL_T_TYPE_SPEC &&
        unwrapped_cur->child != NULL)
    ...
    if (spec_type == PROCEDURE && type_id == NULL && unwrapped_cur != NULL &&
        (unwrapped_cur->typ == PASCAL_T_PROCEDURE_TYPE ||
         unwrapped_cur->typ == PASCAL_T_FUNCTION_TYPE)) {
      ...
      struct Statement *initializer_stmt = NULL;
      if (init_expr != NULL)
        initializer_stmt = mk_varassign(const_decl_node->line,
                                        const_decl_node->col, lhs, init_expr);
      else
        destroy_expr(lhs);
      Tree_t *decl = mk_vardecl(const_decl_node->line, ids, PROCEDURE, NULL, 0,
                                0, initializer_stmt, NULL, NULL, NULL);
    ...
    cur = cur->next;
             ...
             cur->sym != NULL && cur->sym->name != NULL && cur->next != NULL) {
    /* Bare identifier as type annotation (common in record/class const
     * sections). AST: IDENTIFIER("Single") -> REAL("1.5") The identifier is the
     * type name, the next sibling is the value. Only consume when map_type_name
     * recognizes it as a known type to avoid misinterpreting keywords like
     * "set" in "set of TEnum". */
  ...
  if (type_id != NULL && !type_info.is_array) {
    ...
    if (resolve_array_type_info_from_ast(type_id, type_section, &resolved_info,
                                         0) == 0) {
  ...
  /* A single-field record constructor like (Ch: [Ch_Mop2]) gets parsed as a
     parenthesized FIELD_WIDTH expression (the : is the WriteLn format
     operator). When we have a typed const, convert FIELD_WIDTH back to a
     single-field RECORD_CONSTRUCTOR so the lowering code handles it correctly.
     FIELD_WIDTH structure:  child=IDENTIFIER(Ch) -> next=SET(...)
     Needed structure:       RECORD_CONSTRUCTOR -> child=ASSIGNMENT ->
     child=IDENTIFIER(Ch) -> next=SET(...) We repurpose the FIELD_WIDTH node as
     the ASSIGNMENT and create a wrapper. */
  if (value_node->typ == PASCAL_T_FIELD_WIDTH && type_id != NULL &&
      value_node->child != NULL &&
      value_node->child->typ == PASCAL_T_IDENTIFIER) {
    /* Check the field value — if it's a TUPLE, the existing convert_expression
       FIELD_WIDTH handler already handles it as a single-field record
       constructor. Only convert for non-TUPLE values (e.g. set constructors)
       that need help. */
    ast_t *field_val = value_node->child->next;
    ast_t *field_val_unwrapped = unwrap_pascal_node(field_val);
    if (field_val_unwrapped == NULL ||
        field_val_unwrapped->typ != PASCAL_T_TUPLE) {
      ...
      /* Wrap: create a RECORD_CONSTRUCTOR that replaces value_node in the AST.
         We do this by swapping: copy value_node's ASSIGNMENT data into a new
         node, then make value_node the RECORD_CONSTRUCTOR pointing to it. */
  ...
  if (type_info.is_array) {
    if (lower_const_array(const_decl_node, &id, &type_info, value_node,
                          var_builder, type_section, const_section) != 0)
      free(id);
  ...
  if (type_id != NULL) {
    ...
    if (value_node != NULL && (value_node->typ == PASCAL_T_TUPLE ||
                               value_node->typ == PASCAL_T_NONE)) {
      ast_t *child = value_node->child;
      if (child == NULL || (child->typ == PASCAL_T_NONE &&
                            child->child == NULL && child->next == NULL)) {
    ...
    if (typed_const_tag == STRING_TYPE) {
      ...
      struct Statement *initializer_stmt = mk_varassign(
          const_decl_node->line, const_decl_node->col, lhs, init_expr);
      Tree_t *decl =
          mk_vardecl(const_decl_node->line, ids, typed_const_tag, type_id, 0, 0,
                     initializer_stmt, NULL, NULL, NULL);
      decl->tree_data.var_decl_data.is_typed_const = 1;
      mark_var_decl_static_storage(decl);
      decl->tree_data.var_decl_data.type_ref =
    ...
    /* If the value is a tuple that looks like a record constructor (field:
       value; ...), treat it as a record constructor instead of a plain
       expression. */
    if (value_node != NULL && value_node->typ == PASCAL_T_TUPLE &&
    ...
    if (empty_tuple_record_const) {
      ...
      Tree_t *var_decl =
          mk_vardecl(const_decl_node->line, var_ids, var_type, type_id, 0, 0,
                     NULL, inline_record, NULL, NULL);
      var_decl->tree_data.var_decl_data.is_typed_const = 1;
      mark_var_decl_static_storage(var_decl);
      var_decl->tree_data.var_decl_data.type_ref =
          type_ref_from_info_or_id(&type_info, type_id);
  ...
  /* Handle record constructor constants by lowering to variable with field
   * initializers */
  if (value_node != NULL && value_node->typ == PASCAL_T_RECORD_CONSTRUCTOR) {
    /* Lower record const to variable declaration with field-by-field
     * initialization */
    ...
    while (field_assignment != NULL) {
      if (field_assignment->typ == PASCAL_T_ASSIGNMENT ||
        ...
        if (field_name_node != NULL && field_value_node != NULL &&
          ...
          if (field_value != NULL) {
            if (record_info != NULL &&
                field_value->type == EXPR_ARRAY_LITERAL) {
            ...
            struct Expression *base_expr =
                mk_varid(const_decl_node->line, strdup(id));
            struct Expression *lhs = mk_recordaccess(
                const_decl_node->line, base_expr, strdup(field_name));

            /* Generate assignment statement */
            struct Statement *field_assign = mk_varassign(
                const_decl_node->line, const_decl_node->col, lhs, field_value);
            list_builder_append(&field_stmts, field_assign, LIST_STMT);
          } else {
            fprintf(stderr,
                    "ERROR: Unsupported field value in record constructor for "
                    "%s.%s.\n",
                    id, field_name);
    ...
    if (field_assignments != NULL)
      initializer =
          mk_compoundstatement(const_decl_node->line, field_assignments);
    ...
    Tree_t *var_decl =
        mk_vardecl(const_decl_node->line, var_ids, var_type, type_id, 0, 0,
                   initializer, inline_record, NULL, NULL);
    var_decl->tree_data.var_decl_data.is_typed_const = 1;
    mark_var_decl_static_storage(var_decl);
    var_decl->tree_data.var_decl_data.type_ref =
        type_ref_from_info_or_id(&type_info, type_id);
  ...
  if (evaluate_const_int_expr(value_node, &const_value, 0) == 0) {
    ...
    if (unwrapped_value != NULL &&
        unwrapped_value->typ == PASCAL_T_MEMBER_ACCESS) {
    ...
    } else if (unwrapped_value != NULL &&
               unwrapped_value->typ == PASCAL_T_FUNC_CALL) {
  ...
  if (decl != NULL)
    decl->tree_data.const_decl_data.type_ref =
        type_ref_from_info_or_id(&type_info, type_id);
...
void append_const_decls_from_section(ast_t *const_section, ListNode_t **dest,
  ...
  while (const_decl != NULL) {
    ...
    if (node != NULL && node->typ == PASCAL_T_CONST_DECL) {
      ...
      Tree_t *decl =
          convert_const_decl(node, var_builder, type_section, const_section);
      if (decl != NULL) {
...
static Tree_t *convert_type_decl_ex(ast_t *type_decl_node,
                                    ListNode_t **method_clones,
                                    ListNode_t **nested_type_decls_out) {
  ...
  if (spec_node != NULL) {
    ...
    if (trace_sym != NULL && id != NULL &&
        pascal_identifier_equals(id, trace_sym)) {
      ast_t *dbg = spec_node;
      fprintf(stderr,
    ...
    } else if (spec_node->typ == PASCAL_T_TYPE_SPEC &&
               spec_node->child != NULL &&
    ...
    /* Handle "class of T" - check if TYPE_SPEC wraps CLASS_OF_TYPE */
      ...
      if (target_name != NULL && kgpc_getenv("KGPC_DEBUG_TFPG") != NULL) {
        fprintf(stderr,
                "[KGPC] convert_type_decl: class of target=%s for id=%s\n",
                (target_name->sym && target_name->sym->name)
    ...
    if (interface_spec != NULL) {
      record_type =
          convert_interface_type_ex(id, interface_spec, &nested_type_sections);
      if (nested_type_sections != NULL && nested_type_decls_out != NULL) {
        ...
        while (section_cursor != NULL) {
          ...
          if (type_section_ast != NULL) {
            append_type_decls_from_section(
                type_section_ast, nested_type_decls_out, NULL, NULL, NULL, id);
          }
      ...
      record_type =
          convert_class_type_ex(id, class_spec, &nested_type_sections);
      /* Process nested type sections - extract and convert nested type
       * declarations */
      if (nested_type_sections != NULL && nested_type_decls_out != NULL) {
        ListNode_t *section_cursor = nested_type_sections;
        while (section_cursor != NULL) {
          ...
          if (type_section_ast != NULL) {
            ...
            if (kgpc_getenv("KGPC_DEBUG_TYPE_SECTION") != NULL)
              fprintf(stderr,
                      "[KGPC] convert_type_decl: processed nested TYPE_SECTION "
                      "for class %s\n",
                      id);
          }
          section_cursor = section_cursor->next;
      ...
      if (spec_node->typ == PASCAL_T_RECORD_TYPE ||
        ...
        record_spec = spec_node;
                 ...
                 (spec_node->child->typ == PASCAL_T_RECORD_TYPE ||
                  spec_node->child->typ == PASCAL_T_OBJECT_TYPE)) {
      ...
      if (record_spec != NULL) {
        ...
        record_type =
            convert_record_type_ex(record_spec, &nested_type_sections);
        ...
        if (nested_type_sections != NULL && nested_type_decls_out != NULL) {
          ...
          while (section_cursor != NULL) {
            ast_t *type_section_ast = (ast_t *)section_cursor->cur;
            if (type_section_ast != NULL) {
              append_type_decls_from_section(type_section_ast,
                                             nested_type_decls_out, NULL, NULL,
                                             NULL, id);
              if (kgpc_getenv("KGPC_DEBUG_TYPE_SECTION") != NULL)
                fprintf(stderr,
                        "[KGPC] convert_type_decl: processed nested "
                        "TYPE_SECTION for record %s\n",
                        id);
            }
            section_cursor = section_cursor->next;
          }
        ...
        mapped_type =
            convert_type_spec(spec_node, &type_id, &record_type, &type_info);
        /* Preserve qualified type names for alias targets like ObjPas.TEndian.
         */
        {
          ast_t *spec_child = spec_node;
          if (spec_child != NULL && spec_child->typ == PASCAL_T_TYPE_SPEC &&
              spec_child->child != NULL)
          ...
          if (spec_child != NULL &&
            ...
            if (qid != NULL) {
              if (type_info.type_ref == NULL)
                type_info.type_ref =
                    type_ref_create(qualified_ident_clone(qid), NULL, 0);
              if (qid->count > 1) {
      ...
      if (kgpc_getenv("KGPC_DEBUG_TFPG") != NULL)
        fprintf(stderr,
                "[KGPC] convert_type_decl after convert_type_spec id=%s "
                "mapped=%d type_id=%s record_type=%p type_info.record=%p\n",
                id, mapped_type, type_id != NULL ? type_id : "<null>",
  ...
  if (record_type != NULL) {
    ...
    if (record_type->is_type_helper &&
        record_type->helper_base_type_id != NULL &&
      ...
      register_type_helper_mapping(record_type->type_id,
                                   record_type->helper_base_type_id);
    }
    ...
    while (field_cur != NULL) {
      if (field_cur->type == LIST_UNSPECIFIED) {
        ...
        if (method_ast != NULL &&
            (method_ast->typ == PASCAL_T_METHOD_DECL ||
          ...
          if (template != NULL) {
            if (!template->is_interface_delegation) {
              int param_count =
                  from_cparser_count_params_ast(template->params_ast);
              int param_types_count = 0;
              TypeRef **param_types = param_types_from_params_ast(
                  template->params_ast, &param_types_count);
              register_class_method_ex(id, template->name, template->is_virtual,
                                       template->is_override,
                                       template->is_static,
                                       template->is_class_method, param_count,
                                       param_types, param_types_count);
            }
            if (kgpc_getenv("KGPC_DEBUG_CLASS_METHODS") != NULL)
              fprintf(stderr,
                      "[KGPC] Registered record method %s.%s (static=%d)\n", id,
                      template->name, template->is_static);
    ...
    if (record_type->is_type_helper && record_type->method_templates != NULL &&
        id != NULL) {
      ListNode_t *tmpl_cur = record_type->method_templates;
      while (tmpl_cur != NULL) {
        if (tmpl_cur->type == LIST_METHOD_TEMPLATE) {
          struct MethodTemplate *template =
              (struct MethodTemplate *)tmpl_cur->cur;
          if (template != NULL) {
            ...
            if (kgpc_getenv("KGPC_DEBUG_CLASS_METHODS") != NULL)
              fprintf(stderr,
                      "[KGPC] Registered helper method %s.%s (static=%d)\n", id,
                      template->name, template->is_static);
          }
        }
    ...
    decl = mk_typealiasdecl(type_decl_node->line, id, 1, type_info.element_type,
                            type_info.element_type_id, type_info.start,
                            type_info.end);
    if (decl != NULL) {
      ...
      if (type_info.element_kgpc_type != NULL) {
        ...
        if (esize > 0 && esize <= INT_MAX)
          decl->tree_data.type_decl_data.info.alias.array_element_storage_size =
              (int)esize;
      }
    }
    type_info.element_type_id = NULL;
    ...
    decl = mk_typealiasdecl(type_decl_node->line, id, 0, RECORD_TYPE, type_id,
                            0, 0);
    type_id = NULL;
  } else if (mapped_type != UNKNOWN_TYPE || type_id != NULL) {
    decl = mk_typealiasdecl(type_decl_node->line, id, 0, mapped_type, type_id,
                            0, 0);
    type_id = NULL;
  ...
  if (decl != NULL) {
    ...
    if (trace_sym != NULL && id != NULL &&
      ...
      fprintf(stderr,
              "[KGPC_TRACE_NONLOCAL] parse_type_decl id=%s base=%d target=%s "
              "is_array=%d [%d,%d] short=%d kgpc=%p\n",
              id, alias->base_type,
              alias->target_type_id != NULL ? alias->target_type_id : "<null>",
              alias->is_array, alias->array_start, alias->array_end,
    ...
    if (kgpc_getenv("KGPC_DEBUG_TFPG") != NULL &&
      ...
      fprintf(stderr,
              "[KGPC] convert_type_decl alias %s: base=%d target=%s kgpc=%p "
              "kind=%d type_info.is_record=%d record=%p generic=%d\n",
              ...
              decl->tree_data.type_decl_data.kgpc_type
                  ? decl->tree_data.type_decl_data.kgpc_type->kind
                  : -1,
              type_info.is_record, (void *)type_info.record_type,
  ...
  if (decl != NULL && decl->type == TREE_TYPE_DECL &&
    ...
    struct TypeAlias *alias = &decl->tree_data.type_decl_data.info.alias;
    /* Set the alias name */
    if (decl->tree_data.type_decl_data.id != NULL &&
...
static Tree_t *convert_generic_type_decl(ast_t *type_decl_node) {
  ...
  if (param_list != NULL && param_list->typ == PASCAL_T_TYPE_PARAM_LIST) {
    ...
    if (param_count > 0) {
      ...
      int index = 0;
      for (ast_t *param = param_list->child;
           param != NULL && index < param_count; param = param->next) {
        ast_t *name_node = param;
        if (name_node->typ == PASCAL_T_TYPE_PARAM && name_node->child != NULL)
          name_node = name_node->child;
        while (name_node != NULL && name_node->typ != PASCAL_T_IDENTIFIER)
  ...
  if (type_spec_node != NULL) {
    ...
    if (spec_body != NULL) {
      if (spec_body->typ == PASCAL_T_CLASS_TYPE) {
        ...
        record_template =
            convert_class_type_ex(id, spec_body, &nested_type_sections);
        /* Convert nested type sections into type declarations for later
         * specialization */
        if (nested_type_sections != NULL) {
          ...
          while (section_cursor != NULL) {
            ...
            if (type_section_ast != NULL) {
              append_type_decls_from_section(type_section_ast,
                                             &generic_nested_types, NULL, NULL,
                                             NULL, id);
            }
            section_cursor = section_cursor->next;
          }
          ...
          while (nested_type_sections != NULL) {
            ...
            nested_type_sections = next;
                 ...
                 spec_body->typ == PASCAL_T_OBJECT_TYPE)
  ...
  if (type_spec_node != NULL)
    decl->tree_data.type_decl_data.info.generic.original_ast =
        copy_ast_detached(type_spec_node);
...
void convert_routine_body(ast_t *body_node, ListNode_t **const_decls,
                          ListBuilder *var_builder, ListBuilder *label_builder,
                          ListNode_t **nested_subs, struct Statement **body_out,
                          ListNode_t **type_decl_list) {
  if (body_node == NULL)
    return;

  ast_t *type_section_ast =
      NULL; /* Track local type section for enum resolution */
  ...
  while (cursor != NULL) {
    ...
    if (node != NULL) {
      ...
      case PASCAL_T_TYPE_SECTION:
        if (kgpc_getenv("KGPC_DEBUG_TYPE_SECTION") != NULL) {
          fprintf(stderr,
                  "[KGPC] convert_routine_body TYPE_SECTION at line=%d\n",
                  node->line);
        }
      ...
      case PASCAL_T_ASM_BLOCK: {
        if (body_out != NULL) {
          ...
          *body_out = mk_compoundstatement(node->line,
                                           list_builder_finish(&stmts_builder));
        }
        break;
      }
      default:
...
void append_uses_from_section(ast_t *uses_node, ListNode_t **dest) {
  ...
  if (visited == NULL) {
    fprintf(stderr,
            "ERROR: Failed to allocate visited set for uses traversal\n");
    return;
  }
  ...
  while (unit != NULL) {
    ...
    if (!is_safe_to_continue(visited, unit)) {
      fprintf(stderr, "ERROR: Circular reference detected in uses section, "
                      "stopping traversal\n");
      break;
    }
...
void append_labels_from_section(ast_t *label_node, ListBuilder *builder) {
  ...
  if (visited == NULL) {
    fprintf(stderr,
            "ERROR: Failed to allocate visited set for labels traversal\n");
    return;
  }
  ...
  while (cur != NULL) {
    /* Check for circular reference before processing */
    if (!is_safe_to_continue(visited, cur)) {
      fprintf(stderr, "ERROR: Circular reference detected in label section, "
                      "stopping traversal\n");
      break;
    }
    ...
    if (node != NULL &&
        (node->typ == PASCAL_T_INTEGER || node->typ == PASCAL_T_IDENTIFIER)) {
...
static char *mangle_helper_const_name(const char *helper_id,
                                      const char *const_id) {
...
static int type_name_exists_in_sections(const char *name,
                                        ListNode_t *sections) {
...
static void qualify_expr_type_id(struct Expression *expr,
                                 const char *owner_id) {
...
static int type_id_is_qualified(const char *type_id,
                                const struct TypeRef *type_ref) {
  (void)type_id;
...
static struct TypeRef *ensure_type_ref_from_id(char **type_id,
...
static void qualify_param_type_id(char **type_id, struct TypeRef **type_ref,
  ...
  for (size_t i = 0; i < 2; ++i) {
    ...
    if (FindSymbol(&node, symtab, qualified) != 0 && node != NULL &&
...
void qualify_param_decl_types(ListNode_t *params, const char *owner_full,
                              const char *owner_outer, SymTab_t *symtab) {
  ...
  for (ListNode_t *cur = params; cur != NULL; cur = cur->next) {
    ...
    if (decl->type == TREE_VAR_DECL) {
      qualify_param_type_id(&decl->tree_data.var_decl_data.type_id,
                            &decl->tree_data.var_decl_data.type_ref, owner_full,
                            owner_outer, symtab);
    } else if (decl->type == TREE_ARR_DECL) {
      qualify_param_type_id(&decl->tree_data.arr_decl_data.type_id,
                            &decl->tree_data.arr_decl_data.type_ref, owner_full,
                            owner_outer, symtab);
    }
  }
}
...
static void
rewrite_class_const_expr_nested_types(struct Expression *expr,
                                      const char *owner_id,
                                      ListNode_t *nested_type_sections) {
  ...
  case EXPR_FUNCTION_CALL: {
    ...
    for (ListNode_t *cur = args; cur != NULL; cur = cur->next) {
      ...
      if (is_type_intrinsic && arg != NULL && arg->type == EXPR_VAR_ID &&
          ...
          type_name_exists_in_sections(arg->expr_data.id,
                                       nested_type_sections)) {
        qualify_expr_type_id(arg, owner_id);
      }
      rewrite_class_const_expr_nested_types(arg, owner_id,
                                            nested_type_sections);
    }
    break;
  }
  case EXPR_TYPEINFO:
    if (expr->expr_data.typeinfo_data.type_id != NULL &&
      ...
      qualify_type_ref_id(&expr->expr_data.typeinfo_data.type_id,
                          &expr->expr_data.typeinfo_data.type_ref, owner_id,
  ...
  case EXPR_IS:
    ...
    if (expr->expr_data.is_data.target_type_id != NULL &&
      ...
      qualify_type_ref_id(&expr->expr_data.is_data.target_type_id,
                          &expr->expr_data.is_data.target_type_ref, owner_id,
  ...
  case EXPR_AS:
    ...
    if (expr->expr_data.as_data.target_type_id != NULL &&
      ...
      qualify_type_ref_id(&expr->expr_data.as_data.target_type_id,
                          &expr->expr_data.as_data.target_type_ref, owner_id,
  ...
  case EXPR_MULOP:
    rewrite_class_const_expr_nested_types(expr->expr_data.mulop_data.left_term,
                                          owner_id, nested_type_sections);
    rewrite_class_const_expr_nested_types(
        expr->expr_data.mulop_data.right_factor, owner_id,
        nested_type_sections);
    break;
  case EXPR_RELOP:
    ...
    break;
  case EXPR_SIGN_TERM:
    rewrite_class_const_expr_nested_types(expr->expr_data.sign_term, owner_id,
                                          nested_type_sections);
    break;
  case EXPR_ARRAY_ACCESS:
    rewrite_class_const_expr_nested_types(
        expr->expr_data.array_access_data.array_expr, owner_id,
        nested_type_sections);
    rewrite_class_const_expr_nested_types(
        expr->expr_data.array_access_data.index_expr, owner_id,
        nested_type_sections);
  ...
  case EXPR_RECORD_ACCESS:
    rewrite_class_const_expr_nested_types(
        expr->expr_data.record_access_data.record_expr, owner_id,
        nested_type_sections);
    break;
  case EXPR_SET:
    for (ListNode_t *cur = expr->expr_data.set_data.elements; cur != NULL;
      ...
      if (cur->type == LIST_SET_ELEMENT && cur->cur != NULL) {
        ...
        rewrite_class_const_expr_nested_types(elem->lower, owner_id,
                                              nested_type_sections);
        rewrite_class_const_expr_nested_types(elem->upper, owner_id,
                                              nested_type_sections);
      }
    }
    break;
  case EXPR_ARRAY_LITERAL:
    for (ListNode_t *cur = expr->expr_data.array_literal_data.elements;
         cur != NULL; cur = cur->next) {
      if (cur->type == LIST_EXPR)
        rewrite_class_const_expr_nested_types((struct Expression *)cur->cur,
                                              owner_id, nested_type_sections);
  ...
  case EXPR_RECORD_CONSTRUCTOR:
    for (ListNode_t *cur = expr->expr_data.record_constructor_data.fields;
         cur != NULL; cur = cur->next) {
      if (cur->type == LIST_UNSPECIFIED && cur->cur != NULL) {
        struct RecordConstructorField *field =
            (struct RecordConstructorField *)cur->cur;
        rewrite_class_const_expr_nested_types(field->value, owner_id,
                                              nested_type_sections);
      }
    }
    break;
  ...
  case EXPR_POINTER_DEREF:
    rewrite_class_const_expr_nested_types(
        expr->expr_data.pointer_deref_data.pointer_expr, owner_id,
        nested_type_sections);
    break;
  default:
    break;
...
static void
rewrite_class_const_statement_nested_types(struct Statement *stmt,
                                           const char *owner_id,
                                           ListNode_t *nested_type_sections) {
  ...
  case STMT_COMPOUND_STATEMENT:
    for (ListNode_t *cur = stmt->stmt_data.compound_statement; cur != NULL;
         cur = cur->next) {
      if (cur->type == LIST_STMT)
        rewrite_class_const_statement_nested_types(
            (struct Statement *)cur->cur, owner_id, nested_type_sections);
    }
    break;
  case STMT_IF_THEN:
    rewrite_class_const_expr_nested_types(
        stmt->stmt_data.if_then_data.relop_expr, owner_id,
        nested_type_sections);
    rewrite_class_const_statement_nested_types(
        stmt->stmt_data.if_then_data.if_stmt, owner_id, nested_type_sections);
    rewrite_class_const_statement_nested_types(
        stmt->stmt_data.if_then_data.else_stmt, owner_id, nested_type_sections);
    break;
  case STMT_WHILE:
    rewrite_class_const_expr_nested_types(stmt->stmt_data.while_data.relop_expr,
                                          owner_id, nested_type_sections);
    rewrite_class_const_statement_nested_types(
        stmt->stmt_data.while_data.while_stmt, owner_id, nested_type_sections);
    break;
  case STMT_REPEAT:
    for (ListNode_t *cur = stmt->stmt_data.repeat_data.body_list; cur != NULL;
         cur = cur->next) {
      if (cur->type == LIST_STMT)
        rewrite_class_const_statement_nested_types(
            (struct Statement *)cur->cur, owner_id, nested_type_sections);
    ...
    rewrite_class_const_expr_nested_types(
        stmt->stmt_data.repeat_data.until_expr, owner_id, nested_type_sections);
    break;
  case STMT_FOR:
    if (stmt->stmt_data.for_data.for_assign_type == STMT_FOR_ASSIGN_VAR)
      rewrite_class_const_statement_nested_types(
          stmt->stmt_data.for_data.for_assign_data.var_assign, owner_id,
          nested_type_sections);
    else if (stmt->stmt_data.for_data.for_assign_type == STMT_FOR_VAR)
      rewrite_class_const_expr_nested_types(
          stmt->stmt_data.for_data.for_assign_data.var, owner_id,
          nested_type_sections);
    rewrite_class_const_expr_nested_types(stmt->stmt_data.for_data.to, owner_id,
                                          nested_type_sections);
  ...
  case STMT_FOR_IN:
    rewrite_class_const_expr_nested_types(stmt->stmt_data.for_in_data.loop_var,
                                          owner_id, nested_type_sections);
    rewrite_class_const_expr_nested_types(
        stmt->stmt_data.for_in_data.collection, owner_id, nested_type_sections);
    rewrite_class_const_statement_nested_types(
        stmt->stmt_data.for_in_data.do_stmt, owner_id, nested_type_sections);
    break;
  case STMT_EXPR:
...
static void rewrite_class_const_expr(struct Expression *expr, ListNode_t *map) {
  ...
  case EXPR_RECORD_ACCESS:
    rewrite_class_const_expr(expr->expr_data.record_access_data.record_expr,
                             map);
    break;
  case EXPR_TYPECAST:
  ...
  case EXPR_ADDR:
    ...
    break;
  case EXPR_POINTER_DEREF:
    rewrite_class_const_expr(expr->expr_data.pointer_deref_data.pointer_expr,
                             map);
    break;
  case EXPR_IS:
...
static void rewrite_class_const_statement(struct Statement *stmt,
                                          ListNode_t *map) {
  ...
  case STMT_FOR:
    if (stmt->stmt_data.for_data.for_assign_type == STMT_FOR_ASSIGN_VAR)
      rewrite_class_const_statement(
          stmt->stmt_data.for_data.for_assign_data.var_assign, map);
    else if (stmt->stmt_data.for_data.for_assign_type == STMT_FOR_VAR)
      rewrite_class_const_expr(stmt->stmt_data.for_data.for_assign_data.var,
                               map);
...
static void add_class_const_map_entry(ListBuilder *builder,
                                      const char *original,
                                      const char *mangled) {
...
static void append_class_const_decls_from_type_decl(ast_t *type_decl_node,
  ...
  for (ast_t *cur = class_spec->child; cur != NULL; cur = cur->next) {
    ...
    if (node != NULL && node->typ == PASCAL_T_CONST_SECTION)
      is_const_section = 1;
    else if (node != NULL && node->typ == PASCAL_T_NONE && node->sym != NULL &&
             node->sym->name != NULL &&
    ...
    if (kgpc_getenv("KGPC_DEBUG_CLASS_CONST") != NULL) {
      fprintf(stderr, "[KGPC] class const section in %s at line %d\n", class_id,
              node != NULL ? node->line : -1);
    }

    ListNode_t *class_consts = NULL;
    ListNode_t **var_tail_before = var_builder->tail_next;
    append_const_decls_from_section(node, &class_consts, var_builder,
                                    type_section);
    ListNode_t *new_var_nodes =
        (var_tail_before != NULL) ? *var_tail_before : NULL;
    ...
    while (class_iter != NULL) {
      ...
      if (decl != NULL && decl->type == TREE_CONST_DECL &&
        ...
        char *mangled = mangle_helper_const_name(
            class_id, decl->tree_data.const_decl_data.id);
        if (mangled != NULL) {
          add_class_const_map_entry(
              &class_map_builder, decl->tree_data.const_decl_data.id, mangled);
          free(mangled);
        }
      }
    ...
    while (iter != NULL) {
      ...
      if (decl != NULL && decl->type == TREE_CONST_DECL &&
          decl->tree_data.const_decl_data.id != NULL) {
        const char *mangled = lookup_class_const_map(
            map_list, decl->tree_data.const_decl_data.id);
        if (mangled != NULL) {
      ...
      if (decl != NULL && decl->type == TREE_CONST_DECL) {
        rewrite_class_const_expr(decl->tree_data.const_decl_data.value,
                                 map_list);
        rewrite_class_const_expr_nested_types(
            decl->tree_data.const_decl_data.value, class_id,
            nested_type_sections);
      }
    ...
    for (ListNode_t *iter = new_var_nodes; iter != NULL; iter = iter->next) {
      ...
      if (decl->type == TREE_VAR_DECL) {
        for (ListNode_t *id_node = decl->tree_data.var_decl_data.ids;
          ...
          if (id_node->type == LIST_STRING && id_node->cur != NULL) {
            const char *mangled =
                lookup_class_const_map(map_list, (char *)id_node->cur);
            if (mangled != NULL) {
              free(id_node->cur);
        ...
        rewrite_class_const_statement(decl->tree_data.var_decl_data.initializer,
                                      map_list);
        rewrite_class_const_statement_nested_types(
            decl->tree_data.var_decl_data.initializer, class_id,
            nested_type_sections);
      } else if (decl->type == TREE_ARR_DECL) {
        for (ListNode_t *id_node = decl->tree_data.arr_decl_data.ids;
             id_node != NULL; id_node = id_node->next) {
        ...
        rewrite_class_const_statement(decl->tree_data.arr_decl_data.initializer,
                                      map_list);
        rewrite_class_const_statement_nested_types(
            decl->tree_data.arr_decl_data.initializer, class_id,
            nested_type_sections);
      }
    }
  }
  ...
  ListNode_t *map_list = list_builder_finish(&class_map_builder);
  for (ListNode_t *cur_map = map_list; cur_map != NULL;
...
static void append_helper_const_decls_from_type_decl(ast_t *type_decl_node,
  ...
  for (ast_t *cur = record_spec->child; cur != NULL; cur = cur->next) {
    ...
    ListNode_t **var_tail_before = var_builder->tail_next;
    append_const_decls_from_section(node, &helper_consts, var_builder,
                                    type_section);
    ListNode_t *new_var_nodes =
        (var_tail_before != NULL) ? *var_tail_before : NULL;
    ...
    while (const_cur != NULL) {
      ...
      if (decl != NULL && decl->type == TREE_CONST_DECL &&
          decl->tree_data.const_decl_data.id != NULL) {
        char *mangled = mangle_helper_const_name(
            helper_id, decl->tree_data.const_decl_data.id);
        if (mangled != NULL) {
          free(decl->tree_data.const_decl_data.id);
    ...
    if (helper_map != NULL) {
      for (ListNode_t *iter = new_var_nodes; iter != NULL; iter = iter->next) {
        ...
        if (decl->type == TREE_VAR_DECL)
          rewrite_class_const_statement(
              decl->tree_data.var_decl_data.initializer, helper_map);
        else if (decl->type == TREE_ARR_DECL)
          rewrite_class_const_statement(
              decl->tree_data.arr_decl_data.initializer, helper_map);
      }
      for (ListNode_t *cur_map = helper_map; cur_map != NULL;
           cur_map = cur_map->next) {
        ClassConstMap *entry = (ClassConstMap *)cur_map->cur;
        if (entry != NULL) {
          free(entry->original);
...
static void qualify_record_field_nested_types(struct RecordType *record,
  ...
  for (ListNode_t *cur = record->fields; cur != NULL; cur = cur->next) {
    ...
    struct TypeRef *type_ref =
        ensure_type_ref_from_id(&field->type_id, &field->type_ref);
    if (type_ref != NULL && type_ref->name != NULL &&
      ...
      if (base != NULL && nested_type_section_has_id(type_section, base)) {
        qualify_type_ref_id(&field->type_id, &field->type_ref, parent_type_name,
                            base);
      }
    }

    struct TypeRef *elem_ref = ensure_type_ref_from_id(
        &field->array_element_type_id, &field->array_element_type_ref);
    if (elem_ref != NULL && elem_ref->name != NULL &&
        elem_ref->name->count == 1) {
      const char *base = qualified_ident_last(elem_ref->name);
      if (base != NULL && nested_type_section_has_id(type_section, base)) {
        qualify_type_ref_id(&field->array_element_type_id,
                            &field->array_element_type_ref, parent_type_name,
                            base);
      }
    }
  ...
  while (type_decl != NULL) {
    ...
    if (treat_as_generic && unwrapped != NULL) {
        ...
        if (parent_type_name != NULL &&
          ...
          if (qualified_id != NULL) {
            ...
            if (decl->tree_data.type_decl_data.kind == TYPE_DECL_RECORD &&
                ...
                decl->tree_data.type_decl_data.info.record->outer_type_id ==
                    NULL)
              decl->tree_data.type_decl_data.info.record->outer_type_id =
                  strdup(parent_type_name);
          }
          ...
          if (decl->tree_data.type_decl_data.kind == TYPE_DECL_ALIAS) {
            struct TypeAlias *alias =
                &decl->tree_data.type_decl_data.info.alias;
            if (alias->is_pointer) {
              struct TypeRef *ptr_ref = ensure_type_ref_from_id(
                  &alias->pointer_type_id, &alias->pointer_type_ref);
              if (ptr_ref != NULL && ptr_ref->name != NULL &&
                  ptr_ref->name->count == 1) {
                const char *base = qualified_ident_last(ptr_ref->name);
                if (base != NULL &&
                    nested_type_section_has_id(type_section, base)) {
                  qualify_type_ref_id(&alias->pointer_type_id,
                                      &alias->pointer_type_ref,
                                      parent_type_name, base);
                }
              }
            }
      ...
      if (const_decls != NULL && var_builder != NULL) {
        ...
        if (helper_id != NULL) {
          append_helper_const_decls_from_type_decl(
              unwrapped, helper_id, const_decls, var_builder, type_section);
          append_class_const_decls_from_type_decl(
              unwrapped, helper_id, const_decls, var_builder, type_section);
          if (parent_type_name != NULL) {
            size_t len = strlen(parent_type_name) + 1 + strlen(helper_id) + 1;
            nested_parent_name = (char *)malloc(len);
            if (nested_parent_name != NULL) {
              snprintf(nested_parent_name, len, "%s.%s", parent_type_name,
                       helper_id);
            }
          } else {
      ...
      Tree_t *decl =
          convert_type_decl_ex(unwrapped, subprograms, &nested_type_decls);
      if (decl != NULL && parent_type_name != NULL &&
          decl->tree_data.type_decl_data.id != NULL) {
        char *orig_id = decl->tree_data.type_decl_data.id;
        size_t len = strlen(parent_type_name) + 1 + strlen(orig_id) + 1;
        char *qualified_id = (char *)malloc(len);
        if (qualified_id != NULL) {
          ...
          if (decl->tree_data.type_decl_data.kind == TYPE_DECL_RECORD &&
            ...
            decl->tree_data.type_decl_data.info.record->type_id =
                strdup(qualified_id);
            if (decl->tree_data.type_decl_data.info.record->outer_type_id ==
                NULL)
              decl->tree_data.type_decl_data.info.record->outer_type_id =
                  strdup(parent_type_name);
            qualify_record_field_nested_types(
                decl->tree_data.type_decl_data.info.record, parent_type_name,
                type_section);
            if (decl->tree_data.type_decl_data.info.record->parent_class_name !=
                    NULL &&
                nested_type_section_has_id(type_section,
                                           decl->tree_data.type_decl_data.info
                                               .record->parent_class_name)) {
              char *orig_parent =
                  decl->tree_data.type_decl_data.info.record->parent_class_name;
              size_t plen =
                  strlen(parent_type_name) + 1 + strlen(orig_parent) + 1;
              char *qualified_parent = (char *)malloc(plen);
              if (qualified_parent != NULL) {
                snprintf(qualified_parent, plen, "%s.%s", parent_type_name,
                         orig_parent);
                free(orig_parent);
                decl->tree_data.type_decl_data.info.record->parent_class_name =
                    qualified_parent;
          ...
          if (decl->tree_data.type_decl_data.kind == TYPE_DECL_ALIAS) {
            ...
            if (alias->kgpc_type != NULL &&
              ...
              struct RecordType *points_to_record =
                  alias->kgpc_type->info.points_to->info.record_info;
