 ...
 * yet available.
 */
static int extract_builtin_type_bounds(const char *name, long long *low_out,
                                       long long *high_out) {
  if (name == NULL)
    return 0;
...
int extract_constant_int(struct Expression *expr, long long *out_value) {
  ...
  case EXPR_ADDOP: {
    ...
    if (extract_constant_int(expr->expr_data.addop_data.left_expr,
                             &left_value) != 0)
      return 1;
    if (extract_constant_int(expr->expr_data.addop_data.right_term,
                             &right_value) != 0)
      return 1;
  ...
  case EXPR_MULOP: {
    long long left_value = 0;
    long long right_value = 0;
    if (extract_constant_int(expr->expr_data.mulop_data.left_term,
                             &left_value) != 0)
      return 1;
    if (extract_constant_int(expr->expr_data.mulop_data.right_factor,
                             &right_value) != 0)
      return 1;
  ...
  case EXPR_FUNCTION_CALL: {
    ...
    if ((is_high || is_low) && args != NULL && args->cur != NULL) {
      ...
      if (arg != NULL && arg->type == EXPR_VAR_ID &&
        ...
        if (extract_builtin_type_bounds(arg->expr_data.id, &low_val,
                                        &high_val)) {
...
static ListNode_t *append_set_element(ListNode_t *elements,
...
struct Expression *convert_set_literal(ast_t *set_node) {
  ...
  for (ast_t *element = set_node->child; element != NULL && element != ast_nil;
    ...
    if (unwrapped->typ == PASCAL_T_RANGE) {
      ...
      elements =
          append_set_element(elements, mk_set_element(start_expr, end_expr));

      long long start_value = 0;
      long long end_value = 0;
      int have_start = (start_expr != NULL) &&
                       extract_constant_int(start_expr, &start_value) == 0;
      int have_end =
          (end_expr != NULL) && extract_constant_int(end_expr, &end_value) == 0;
...
static struct Expression *convert_inherited_operator_expr(int line_num,
                                                          const char *op_id,
                                                          ast_t *arg_node);
...
static struct Expression *convert_factor(ast_t *expr_node) {
  ...
  case PASCAL_T_CHAR: {
    ...
    const char *char_str =
        (expr_node->sym != NULL) ? expr_node->sym->name : NULL;
  ...
  case PASCAL_T_CHAR_CODE: {
    ...
    if (literal != NULL) {
      ...
      fprintf(stderr, "ERROR: invalid character code literal at line %d.\n",
              expr_node->line);
    }
      ...
      if (base_name != NULL && type_args != NULL) {
        ...
        if (args_copy != NULL) {
          ...
          struct RecordType *record =
              instantiate_generic_record(base_name, args_copy, &inst_name);
          if (record != NULL) {
            char *rec_id = inst_name != NULL ? strdup(inst_name)
                                             : strdup(specialized_name);
            Tree_t *type_decl = mk_record_type(expr_node->line, rec_id, record);
            if (type_decl != NULL) {
              DeferredInlineSpec *entry =
                  (DeferredInlineSpec *)calloc(1, sizeof(DeferredInlineSpec));
              if (entry != NULL) {
  ...
  case PASCAL_T_FUNC_CALL: {
    ...
    if (child != NULL && child->typ == PASCAL_T_MEMBER_ACCESS) {
        ...
        if (lhs_name != NULL) {
          ...
          if (func_ident != NULL && func_ident->typ == PASCAL_T_IDENTIFIER) {
            ...
            struct Expression *call_expr =
                mk_functioncall(expr_node->line, func_name, args);
            if (call_expr != NULL) {
      ...
      if (callee != NULL && callee->type == EXPR_RECORD_ACCESS &&
        ...
        struct Expression *call_expr =
            mk_functioncall(expr_node->line, method_id, args);
        tag_operator_call(call_expr, is_operator_token_name(method_id));
        if (call_expr != NULL) {
          call_expr->expr_data.function_call_data.is_method_call_placeholder =
              1;
    ...
    if (child != NULL) {
      if (child->typ == PASCAL_T_IDENTIFIER) {
        ...
        struct Expression *call_expr =
            mk_functioncall(expr_node->line, type_id, outer_args);
        if (call_expr != NULL) {
          call_expr->expr_data.function_call_data.is_procedural_var_call = 1;
          call_expr->expr_data.function_call_data.procedural_var_expr =
              typecast_expr;
        }
        return call_expr;
    ...
    if (saw_inherited && id != NULL) {
      struct Expression *inherited_op =
          convert_inherited_operator_expr(expr_node->line, id, child);
    ...
    if (saw_inherited && result != NULL) {
      ...
      result->expr_data.function_call_data.is_bare_inherited =
          is_bare_inherited;
    }
    return result;
  }
  case PASCAL_T_ARRAY_ACCESS: {
    ...
    struct Expression *result =
        mk_arrayaccess(expr_node->line, base, first_index);
...
static struct Expression *convert_inherited_operator_expr(int line_num,
                                                          const char *op_id,
                                                          ast_t *arg_node) {
  if (g_current_method_name == NULL || op_id == NULL || arg_node == NULL ||
      arg_node->next != NULL)
    return NULL;

  struct Expression *lhs =
      mk_functioncall(line_num, strdup(g_current_method_name), NULL);
  if (lhs == NULL)
    return NULL;
...
static struct Expression *convert_unary_expr(ast_t *node) {
  ...
  // Only negate for PASCAL_T_NEG, unary plus (PASCAL_T_POS) returns the inner
  // expression as-is
...
struct Expression *convert_expression(ast_t *expr_node) {
  ...
  case PASCAL_T_IS: {
    ...
    if (type_node != NULL) {
      target_type = convert_type_spec(type_node, &target_type_id,
  ...
  case PASCAL_T_FIELD_WIDTH: {
     ...
     * can be parsed as FIELD_WIDTH. Detect tuple RHS and reinterpret as record
     * ctor. */
  ...
  case PASCAL_T_TYPECAST: {
    ...
    if (unwrapped_type != NULL) {
      ...
      if (kgpc_getenv("KGPC_DEBUG_BODY") != NULL)
        fprintf(stderr,
                "[KGPC] TYPECAST handler: after convert_type_spec "
                "target_type=%d target_type_id=%s\n",
                target_type, target_type_id ? target_type_id : "(null)");
      type_ref_local = type_ref_from_info_or_id(&type_info, target_type_id);
      destroy_type_info_contents(&type_info);
      if (record_type != NULL) {
      ...
      if (target_type_id == NULL && type_ref_local != NULL &&
          type_ref_local->name != NULL) {
        ...
        if (qid->count > 0 && qid->segments != NULL) {
          ...
          if (target_type_id != NULL && target_type_qualifier == NULL &&
              qid->count > 1) {
            target_type_qualifier =
                qualified_ident_join_prefix(qid, qid->count - 1);
          }
        }
      }
      ...
      /* Handle qualified identifiers like
       * widestringmanager.UpperUnicodeStringProc parsed as member access in
       * typecast position */
      if (target_type == UNKNOWN_TYPE && target_type_id == NULL &&
      ...
      /* Handle generic specialization typecasts like specialize
       * TArray<T>(Result). Extract the base type name and build a TypeRef with
       * args. */
      if (target_type == UNKNOWN_TYPE && target_type_id == NULL) {
        char *gen_base = NULL;
    ...
    if (arg_count > 1) {
      ...
      target_type_id = NULL;
      struct Expression *call_expr =
          mk_functioncall(expr_node->line, call_id, args);
      if (call_expr != NULL)
        call_expr->expr_data.function_call_data.call_qualifier =
            target_type_qualifier;
    ...
    struct Expression *tc_expr =
        mk_typecast(expr_node->line, target_type, target_type_id, inner_expr);
    if (tc_expr != NULL) {
      ...
      if (type_ref_local != NULL) {
        ...
        tc_expr->expr_data.typecast_data.target_type_ref =
            type_ref_from_qualifier_and_id(target_type_qualifier,
                                           target_type_id);
      }
    } else if (target_type_qualifier != NULL)
      free(target_type_qualifier);
  ...
  case PASCAL_T_DEREF: {
    struct Expression *result =
        mk_pointer_deref(expr_node->line, convert_expression(expr_node->child));
  ...
  case PASCAL_T_ADDR: {
    ...
    if (child != NULL && child->sym != NULL && child->sym->name != NULL &&
        ...
        expr_node->index >= 0 &&
        (size_t)expr_node->index < preprocessed_length) {
      ...
      if (i < line_end && preprocessed_source[i] == '@')
        i++;
      while (i < line_end &&
             (preprocessed_source[i] == ' ' || preprocessed_source[i] == '\t'))
        i++;
      /* Skip optional 'specialize' keyword */
      const char *kw = "specialize";
      size_t kw_len = 10;
      if (i + kw_len <= line_end &&
          strncasecmp(preprocessed_source + i, kw, kw_len) == 0) {
        i += kw_len;
        while (i < line_end && (preprocessed_source[i] == ' ' ||
                                preprocessed_source[i] == '\t'))
          i++;
      }
      ...
      if (i < line_end && preprocessed_source[i] == '.' && i > type_start) {
        ...
        if (type_raw != NULL) {
          ...
          while (i < line_end &&
                 (isalnum((unsigned char)preprocessed_source[i]) ||
                  preprocessed_source[i] == '_'))
            i++;
          if (i > method_start) {
            ...
            if (method_id != NULL) {
              ...
              if (lt != NULL) {
                ...
                if (gt != NULL && gt > lt) {
                  ...
                  if (mangled != NULL)
                    snprintf(mangled, out_len + 1, "%.*s$%s", (int)base_len,
                             type_raw, args_buf);
                }
              }
              if (mangled == NULL)
              ...
              if (mangled != NULL) {
                ...
                struct Expression *base_expr =
                    mk_varid(expr_node->line, mangled);
                struct Expression *access =
                    mk_recordaccess(expr_node->line, base_expr, method_id);
                if (access != NULL) {
                  access->is_specialize_addr_target = 1;
                  struct Expression *result =
                      mk_addressof(expr_node->line, access);
                  free(type_raw);
                  return set_expr_source_index(result, expr_node);
                }
    ...
    struct Expression *result =
        mk_addressof(expr_node->line, convert_expression(expr_node->child));
    return set_expr_source_index(result, expr_node);
  }
  case PASCAL_T_ANONYMOUS_FUNCTION: {
    ...
    if (generated_name == NULL) {
      fprintf(
          stderr,
          "ERROR: Failed to generate name for anonymous function at line %d\n",
          expr_node->line);
      return NULL;
    }
    ...
    /* Check if first child is a PARAM (parameters exist) or RETURN_TYPE (no
     * params) */
    if (current != NULL) {
    ...
    if (return_type_node != NULL && return_type_node->child != NULL) {
      if (return_type_node->child->sym != NULL &&
  ...
  case PASCAL_T_ANONYMOUS_PROCEDURE: {
     ...
     * Structure from parser (same as anonymous function but without return
     * type):
     * - If params exist: child is first PARAM, params are linked via ->next
    ...
    if (generated_name == NULL) {
      fprintf(
          stderr,
          "ERROR: Failed to generate name for anonymous procedure at line %d\n",
          expr_node->line);
      return NULL;
    }
    ...
    /* Check if first child is a PARAM (parameters exist) or a statement (no
     * params) */
    if (current != NULL) {
      /* Skip NONE nodes */
    ...
    if (body_node != NULL) {
      ...
      if (body == NULL) {
        fprintf(
            stderr,
            "ERROR: Failed to convert anonymous procedure body at line %d\n",
            expr_node->line);
        if (generated_name)
          free(generated_name);
        if (parameters)
          destroy_list(parameters);
        return NULL;
      }
    } else {
    ...
    return mk_anonymous_procedure(expr_node->line, generated_name, parameters,
                                  body);
  }
  default: {
...
ListNode_t *convert_expression_list(ast_t *arg_node) {
  ...
  /* Pre-check: verify the next chain doesn't have cycles using Floyd's
   * algorithm. If a cycle is found, break it by setting the back-edge to NULL.
   ...
   * (e.g., typecast-then-call patterns like TProc(x)(args)). */
  {
    ...
    int has_cycle = 0;
    while (fast != NULL && fast != ast_nil && fast->next != NULL &&
           fast->next != ast_nil) {
      slow = slow->next;
      fast = fast->next->next;
      if (slow == fast) {
  ...
  if (visited == NULL) {
    fprintf(stderr, "ERROR: Failed to allocate visited set for expression list "
                    "traversal\n");
    return NULL;
  }
   ...
   * algorithm */
  {
    ...
    if (has_cycle) {
      fprintf(stderr,
              "PRE-CHECK: Cycle detected in expression list next-chain "
              "starting at %p\n",
              (void *)arg_node);
      /* Find cycle start */
      slow = arg_node;
  ...
  /*
   * cparser often wraps Pascal arguments in synthetic tuple/statement nodes.
   * Unwrap each layer so we only convert real expressions and preserve
   * field-width wrappers explicitly.
   */
  while (cur != NULL && cur != ast_nil) {
    /* Check for circular reference before processing */
    if (!is_safe_to_continue(visited, cur)) {
      fprintf(stderr, "ERROR: Circular reference detected in expression list, "
                      "stopping traversal\n");
      break;
    }
...
convert_member_access_chain(int line, struct Expression *base_expr,
                            ast_t *field_node);
...
struct Expression *convert_member_access(ast_t *node) {
  ...
  if (kgpc_getenv("KGPC_DEBUG_SPECIALIZE_CALLS") != NULL && node != NULL &&
      (node->line == 255 || node->line == 256)) {
    fprintf(stderr,
            ...
            (args_node != NULL && args_node->sym != NULL &&
             args_node->sym->name != NULL)
                ? args_node->sym->name
                : "<null>");
    for (ast_t *it = field_node != NULL ? field_node->child : NULL; it != NULL;
         it = it->next) {
      fprintf(
          stderr,
  ...
  if (kgpc_getenv("KGPC_DEBUG_SPECIALIZE_CALLS") != NULL) {
    ...
    if (field_node != NULL && field_node->typ == PASCAL_T_FUNC_CALL &&
      ...
      const char *fc_sym =
          (fc->sym != NULL && fc->sym->name != NULL) ? fc->sym->name : "<null>";
      fprintf(stderr,
  ...
  if (args_node != NULL && args_node->typ == PASCAL_T_ARG_LIST) {
    if (kgpc_getenv("KGPC_DEBUG_BODY") != NULL) {
      fprintf(stderr, "[KGPC] convert_member_access: detected ARG_LIST child, "
    ...
    args_list =
        PushListNodeFront(args_list, CreateListNode(base_expr, LIST_EXPR));

    struct Expression *call_expr =
        mk_functioncall(node->line, method_id, args_list);
    if (call_expr != NULL) {
...
convert_member_access_chain(int line, struct Expression *base_expr,
  ...
  if (unwrapped != NULL && unwrapped->typ == PASCAL_T_FUNC_CALL) {
    if (kgpc_getenv("KGPC_DEBUG_BODY") != NULL) {
      fprintf(stderr, "[KGPC] convert_member_access_chain: detected FUNC_CALL, "
                      "converting to method call\n");
    }
    ...
    if (method_id_node != NULL) {
      ...
      if (kgpc_getenv("KGPC_DEBUG_SPECIALIZE_CALLS") != NULL &&
        ...
        for (ast_t *dbg = args_node, *it = dbg; it != NULL; it = it->next) {
          const char *sym = (it->sym != NULL && it->sym->name != NULL)
                                ? it->sym->name
                                : "<null>";
          fprintf(stderr,
                  "[KGPC_DEBUG_SPECIALIZE_CALLS]   method-arg node typ=%d(%s) "
                  "sym=%s child=%d(%s)\n",
                  it->typ, pascal_tag_to_string(it->typ), sym,
                  it->child != NULL ? it->child->typ : -1,
                  it->child != NULL ? pascal_tag_to_string(it->child->typ)
                                    : "<null>");
        }
      }
      if (method_unwrapped->typ == PASCAL_T_IDENTIFIER) {
        ...
        char *base_name = NULL;
        ListNode_t *type_args = NULL;
        if (extract_constructed_type_info(method_unwrapped, &base_name,
                                          &type_args)) {
          if (base_name != NULL) {
    ...
    if (method_id != NULL) {
      ...
      struct Expression *call_expr =
          mk_functioncall(line, method_id, args_list);
      if (call_expr != NULL) {
        call_expr->expr_data.function_call_data.is_method_call_placeholder = 1;
        call_expr->expr_data.function_call_data.placeholder_method_name =
  ...
  if (unwrapped != NULL && unwrapped->next != NULL &&
      unwrapped->next->typ == PASCAL_T_ARG_LIST) {
    if (kgpc_getenv("KGPC_DEBUG_BODY") != NULL) {
      fprintf(stderr, "[KGPC] convert_member_access_chain: detected ARG_LIST "
                      "sibling, converting to function call\n");
    }
  ...
  case PASCAL_T_TYPECAST: {
     ...
     * => MEMBER_ACCESS(base=TMarshal,
     * field=TYPECAST(CONSTRUCTED_TYPE(FixArray<T>), Arr))
     */
    ast_t *cast_type_node = unwrapped->child;
    ast_t *cast_value_node =
        (cast_type_node != NULL) ? cast_type_node->next : NULL;
    ...
    if (cast_type != NULL && cast_type->typ == PASCAL_T_IDENTIFIER) {
      ...
      if (method_id != NULL)
        placeholder_name = strdup(method_id);
               ...
               cast_type->typ == PASCAL_T_CONSTRUCTED_TYPE) {
      char *base_name = NULL;
      ListNode_t *type_args = NULL;
  ...
  case PASCAL_T_BOOLEAN: {
     ...
     * The parser treats 'False' and 'True' as boolean keywords, but in the
     * context of a member access (after a dot), they should be treated as
     * identifiers that reference enum literals. */
    char *field_id = dup_symbol(unwrapped);
    if (field_id == NULL) {
      destroy_expr(base_expr);
      return NULL;
  ...
  case PASCAL_T_ARRAY_ACCESS: {
    ...
    struct Expression *field_expr =
        convert_member_access_chain(node_line, base_expr, array_base);
    if (field_expr == NULL)
      return NULL;
    ...
    struct Expression *result =
        mk_arrayaccess(node_line, field_expr, index_expr);
    if (result == NULL)
      return NULL;
  ...
  case PASCAL_T_MEMBER_ACCESS: {
    ...
    struct Expression *next_base =
        convert_member_access_chain(node_line, base_expr, inner_base);
    if (next_base == NULL)
      return NULL;
  ...
  case PASCAL_T_DEREF: {
    ...
    struct Expression *target =
        convert_member_access_chain(node_line, base_expr, inner);
    if (target == NULL)
      return NULL;
...
struct Statement *convert_assignment(ast_t *assign_node) {
  ...
  if (kgpc_getenv("KGPC_DEBUG_SPECIALIZE_CALLS") != NULL &&
    ...
    fprintf(
        ...
        (dbg_rhs != NULL && dbg_rhs->sym != NULL && dbg_rhs->sym->name != NULL)
            ? dbg_rhs->sym->name
            : "<null>");
    for (ast_t *it = rhs != NULL ? rhs->next : NULL; it != NULL;
         it = it->next) {
      fprintf(
          stderr,
  ...
  struct Statement *stmt =
      mk_varassign(assign_node->line, assign_node->col, left, right);
...
struct Statement *convert_proc_call(ast_t *call_node,
  ...
  if (child != NULL && child->typ == PASCAL_T_MEMBER_ACCESS) {
    ...
    if (lhs != NULL && lhs->typ == PASCAL_T_IDENTIFIER && rhs != NULL) {
      ...
      if (lhs_name != NULL && unit_registry_contains(lhs_name)) {
        ...
        if (method_ident != NULL && method_ident->typ == PASCAL_T_IDENTIFIER) {
          ...
          struct Statement *call =
              mk_procedurecall(call_node->line, proc_name, args);
          if (call != NULL) {
    ...
    struct Statement *method_stmt =
        convert_method_call_statement(child, args_node);
    if (method_stmt != NULL) {
      if (kgpc_getenv("KGPC_DEBUG_BODY") != NULL) {
        fprintf(stderr,
                "[KGPC]   convert_method_call_statement returned statement\n");
      }
      return method_stmt;
    }
    if (kgpc_getenv("KGPC_DEBUG_BODY") != NULL) {
  ...
  if (call_node->typ == PASCAL_T_IDENTIFIER) {
    ...
    if (implicit_identifier && child->typ == PASCAL_T_IDENTIFIER) {
      ...
      if (child->typ == PASCAL_T_IDENTIFIER) {
         ...
         * child is TYPECAST with: child->child = IDENTIFIER(TypeName), rest =
         * source args call_node has: child->next = outer call args We need to
         * preserve both: create a procedure call where the first arg is the
         * typecast source, followed by the outer call args. */
        id = dup_symbol(child->child);
  ...
  if (call != NULL) {
    if (call_node->index >= 0)
      call->source_index = call_node->index + g_source_offset;
    /* If id starts with __, it's a method call placeholder from
     * convert_method_call_statement. Extract the bare method name and set the
     * structured flag. */
    if (id != NULL && strncmp(id, "__", 2) == 0) {
      call->stmt_data.procedure_call_data.is_method_call_placeholder = 1;
      call->stmt_data.procedure_call_data.placeholder_method_name =
          strdup(id + 2);
    }
  }
  return call;
...
struct Statement *convert_method_call_statement(ast_t *member_node,
                                                ast_t *args_start) {
  if (member_node == NULL)
    return NULL;
  ...
  if (identifier_node != NULL && identifier_node->typ == PASCAL_T_FUNC_CALL) {
    ...
    /* args_start should be the second child (the arguments) if not already
     * provided */
   ...
   *
   * Solution: Don't mangle the name during parsing. Instead, store the
   * unmangled method name and let the semantic checker resolve it based on the
   * object's type.
   *
  ...
  if (class_name != NULL) {
    ...
    /* No class found at parse time. Create a placeholder name that semantic
     * checker will resolve based on the object's type. Use format
     * "__methodname" to indicate this is a method call that needs type-based
     * resolution. */
...
struct Statement *build_nested_with_statements(int line, ast_t *context_node,
