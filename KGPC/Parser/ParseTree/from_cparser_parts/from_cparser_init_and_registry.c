...
/* Method context for expression conversion (e.g., bare "inherited"
 * expressions). */
...
static void enum_ht_init(void) {
  if (g_enum_ht.buckets != NULL)
    return;
  g_enum_ht.capacity = ENUM_HT_INITIAL_CAPACITY;
  g_enum_ht.buckets =
      (EnumTypeEntry **)calloc(g_enum_ht.capacity, sizeof(EnumTypeEntry *));
  g_enum_ht.size = 0;
}
...
int enum_registry_lookup(const char *name, int *out_start, int *out_end) {
  if (name == NULL || g_enum_ht.buckets == NULL)
    return -1;
  size_t idx = enum_ht_hash(name, g_enum_ht.capacity);
  for (EnumTypeEntry *e = g_enum_ht.buckets[idx]; e != NULL; e = e->next) {
    if (strcasecmp(e->name, name) == 0) {
...
/* Forward declaration for const expression evaluator (defined later in the
 * file). */

/* Scan a PASCAL_T_TYPE_SECTION and register all enumerated and subrange types
...
void enum_registry_scan_type_section(ast_t *type_section) {
  ...
  for (ast_t *decl = type_section->child; decl != NULL; decl = decl->next) {
    if (decl->typ != PASCAL_T_TYPE_DECL)
      continue;
      ...
      for (ast_t *lit = spec->child; lit != NULL; lit = lit->next) {
        ...
        if (lit_id != NULL && lit_id->typ == PASCAL_T_IDENTIFIER) {
          ...
          if (!have_bounds) {
            ...
            if (literal_value < min_ord)
              min_ord = literal_value;
            if (literal_value > max_ord)
              max_ord = literal_value;
          }
          count++;
...
void register_type_helper_mapping(const char *helper_id,
  ...
  struct TypeHelperMapping *entry =
      (struct TypeHelperMapping *)calloc(1, sizeof(struct TypeHelperMapping));
  if (entry == NULL)
    return;
...
VisitedSet *visited_set_create(void) {
  ...
  set->size = 0;
  set->buckets =
      (VisitedSetEntry **)calloc(set->capacity, sizeof(VisitedSetEntry *));
  if (set->buckets == NULL) {
    free(set);
    return NULL;
  }
...
static void visited_set_rehash(VisitedSet *set) {
  ...
  VisitedSetEntry **new_buckets =
      (VisitedSetEntry **)calloc(new_capacity, sizeof(VisitedSetEntry *));
  if (new_buckets == NULL)
    return; /* rehash failed, continue with old table */
...
bool visited_set_add(VisitedSet *set, ast_t *node) {
  ...
  /* Add new entry */
  size_t index = visited_set_hash(node, set->capacity);
  VisitedSetEntry *new_entry =
      (VisitedSetEntry *)malloc(sizeof(VisitedSetEntry));
  if (new_entry == NULL)
    return false;
...
int split_absolute_target(const char *absolute_target, char **out_base,
                          char **out_field) {
  if (out_base != NULL)
    *out_base = NULL;
  if (out_field != NULL)
...
TypeRef *type_ref_from_name_and_args(const char *base_name,
  ...
  if (arg_count > 0) {
    ...
    int idx = 0;
    for (ListNode_t *cur = type_args; cur != NULL && idx < arg_count;
         cur = cur->next) {
...
static int append_qualified_segment(char ***segments, int *count, int *cap,
                                    const char *segment) {
  if (segment == NULL)
    return 0;
  if (*count + 1 > *cap) {
...
static int append_qualified_segments_from_dotted(char ***segments, int *count,
...
QualifiedIdent *qualified_ident_from_ast(ast_t *node) {
  ...
  if (unwrapped->typ == PASCAL_T_IDENTIFIER ||
      unwrapped->typ == PASCAL_T_BOOLEAN) {
  ...
  if (unwrapped->typ == PASCAL_T_MEMBER_ACCESS) {
    ...
    for (int i = 0; i < base_id->count; ++i) {
      if (!append_qualified_segment(&segments, &count, &cap,
                                    base_id->segments[i])) {
    ...
    if (field != NULL) {
      ...
      if (field_node != NULL && (field_node->typ == PASCAL_T_IDENTIFIER ||
                                 field_node->typ == PASCAL_T_BOOLEAN)) {
...
int parse_guid_literal(const char *guid, uint32_t *d1, uint16_t *d2,
...
int from_cparser_scopedenums_enabled_at_line(int target_line) {
  ...
  while (pos < length && current_line <= target_line) {
    ...
    if (line_len >= 4 && buffer[line_start] == '{' &&
        buffer[line_start + 1] == '$') {
      ...
      size_t inner_len =
          (inner_end > inner_start) ? (inner_end - inner_start) : 0;
...
int from_cparser_get_error_count(void) { return g_frontend_error_count; }
...
static ListNode_t *g_const_sections_tail =
    NULL; /* tail pointer for O(1) append */
...
#define CONST_INT_HT_LOAD_FACTOR_DEN                                           \
  4 /* denominator: grow when size > 3/4 capacity */
...
static void const_int_ht_init(void) {
  ...
  g_const_int_ht.buckets = (ConstIntEntry **)calloc(g_const_int_ht.capacity,
                                                    sizeof(ConstIntEntry *));
  g_const_int_ht.size = 0;
}

static void const_int_ht_grow(void) {
  size_t new_cap = g_const_int_ht.capacity * 2;
  ConstIntEntry **new_buckets =
      (ConstIntEntry **)calloc(new_cap, sizeof(ConstIntEntry *));
  if (new_buckets == NULL)
    return;
...
static void const_decl_index_init(void) {
  if (g_const_decl_index.buckets != NULL)
    return;
...
/* Insert a const declaration into the index.  Does NOT overwrite existing
 * entries (first definition wins, matching Pascal semantics). */
static void const_decl_index_insert(const char *name, ast_t *value_node,
                                    ast_t *section) {
  if (name == NULL || g_const_decl_index.buckets == NULL)
    return;
  size_t idx = const_int_ht_hash(name, g_const_decl_index.capacity);
  /* Check if already present */
  for (ConstDeclIndexEntry *e = g_const_decl_index.buckets[idx]; e != NULL;
       e = e->next) {
    if (strcasecmp(e->name, name) == 0)
      return; /* first definition wins */
  }
  ConstDeclIndexEntry *e =
      (ConstDeclIndexEntry *)malloc(sizeof(ConstDeclIndexEntry));
  if (e == NULL)
    return;
  e->name = name;
...
int lookup_const_int(const char *name, int *out_value) {
  ...
  for (ConstIntEntry *cur = g_const_int_ht.buckets[idx]; cur != NULL;
       cur = cur->next) {
...
void register_pending_generic_alias(Tree_t *decl, TypeInfo *type_info) {
  if (!g_allow_pending_specializations || decl == NULL || type_info == NULL)
    return;
  if (type_info->generic_base_name == NULL ||
      type_info->generic_type_args == NULL)
    return;

  PendingGenericAlias *entry =
      (PendingGenericAlias *)calloc(1, sizeof(PendingGenericAlias));
  if (entry == NULL)
    return;
  ...
  if (kgpc_getenv("KGPC_DEBUG_TFPG") != NULL &&
      decl->tree_data.type_decl_data.id != NULL && entry->base_name != NULL) {
...
void from_cparser_set_source_offset(int offset) { g_source_offset = offset; }

/* Helper to copy source index from AST node to Expression for accurate error
 * context. node->index == -1 means the parser position is unknown; preserve
 * that sentinel rather than producing a bogus global offset (-1 +
 * g_source_offset). */
...
static CMBIndexEntry *cmb_index_get_or_create(const char *interned_class) {
  ...
  e->count = 0;
  e->capacity = 4;
  e->bindings = (ClassMethodBinding **)malloc(sizeof(ClassMethodBinding *) *
                                              (size_t)e->capacity);
...
static void cmb_index_add(const char *interned_class,
                          ClassMethodBinding *binding) {
  CMBIndexEntry *e = cmb_index_get_or_create(interned_class);
  if (e->count == e->capacity) {
    e->capacity *= 2;
    e->bindings = (ClassMethodBinding **)realloc(
        e->bindings, sizeof(ClassMethodBinding *) * (size_t)e->capacity);
    assert(e->bindings != NULL);
  }
...
void cmb_index_alias_as_qualified(const char *unqualified_name,
...
static CMBMethodEntry *
cmb_method_index_get_or_create(const char *interned_method) {
  unsigned int h = cmb_ptr_hash(interned_method);
  h = h % CMB_METHOD_INDEX_SIZE;
  for (CMBMethodEntry *e = cmb_method_index[h]; e != NULL; e = e->next)
    if (e->method_name == interned_method)
...
static void cmb_method_index_add(const char *interned_method,
                                 ClassMethodBinding *binding) {
...
char *generate_anonymous_method_name(int is_function) {
  ...
  if (name == NULL)
    return NULL;
  snprintf(name, ANON_METHOD_NAME_SIZE, "_anon_%s_%d",
           is_function ? "func" : "proc", ++anonymous_method_counter);
  return name;
}
...
static int is_param_modifier(const char *name) {
  if (name == NULL)
    return 0;
  return (strcasecmp(name, "const") == 0 || strcasecmp(name, "var") == 0 ||
...
/* Returns a heap-allocated array of N TypeRef* pointers (caller owns each
 * TypeRef* and the outer array) and writes the element count to *out_count.
 * Returns NULL with *out_count=0 when params_ast is empty.  Each TypeRef is
 * produced from convert_type_spec on the param's type node, so generic args
 * and qualified names are preserved structurally. */
...
TypeRef **param_types_from_method_impl(ast_t *method_node, int *out_count) {
  ...
  for (ast_t *cur = method_node->child; cur != NULL; cur = cur->next) {
    ...
    TypeRef **grown = (TypeRef **)realloc(out, (size_t)(total + chunk_count) *
                                                   sizeof(TypeRef *));
    if (grown == NULL) {
...
void register_class_method_ex(const char *class_name, const char *method_name,
                              ...
                              int is_class_method, int param_count,
  ...
  ClassMethodBinding *binding =
      (ClassMethodBinding *)calloc(1, sizeof(ClassMethodBinding));
  if (binding == NULL) {
    param_types_free(param_types, param_types_count);
    return;
  }
  ...
  if (kgpc_getenv("KGPC_DEBUG_CLASS_METHODS") != NULL) {
    fprintf(stderr,
            "[KGPC] Registered method %s.%s (virtual=%d, override=%d, "
            "static=%d, class_method=%d, params=%d)\n",
            class_name, method_name, is_virtual, is_override, is_static,
            is_class_method, param_count);
  }
}
...
void from_cparser_register_method_template(const char *class_name,
  ...
  register_class_method_ex(class_name, method_name, is_virtual, is_override,
                           is_static, 0, param_count, NULL, -1);
}
...
int is_method_static_with_types(const char *class_name, const char *method_name,
                                int param_count, TypeRef *const *param_types,
  ...
  for (int i = 0; i < entry->count; i++) {
    ...
    if (b->method_name != mn)
      continue;
    int matches = 0;
    if (param_types != NULL && b->param_types != NULL &&
        b->param_types_count >= 0) {
...
/* Check if a method is declared with 'class' keyword (Self = VMT pointer, not
 * instance). Returns 1 if class method, 0 otherwise. */
 ...
 * called with 4 args).
 */
int from_cparser_is_method_virtual_with_types(const char *class_name,
                                              const char *method_name,
                                              int param_count,
                                              TypeRef *const *param_types,
                                              int param_types_count) {
  if (class_name == NULL || method_name == NULL)
    return 0;
...
const char *g_typed_const_unit_tag =
    "p"; /* set per unit in tree_from_pascal_ast */
...
void mark_var_decl_static_storage(Tree_t *decl) {
  ...
  snprintf(label_buffer, sizeof(label_buffer), "__kgpc_tconst_var_%s_%d",
...
int is_operator_token_name(const char *name) {
  ...
  return (strcmp(name, "+") == 0 || strcmp(name, "-") == 0 ||
          ...
          strcasecmp(name, "as") == 0 || strcasecmp(name, "Implicit") == 0 ||
          strcasecmp(name, "Explicit") == 0 || strcasecmp(name, "Equal") == 0 ||
          strcasecmp(name, "NotEqual") == 0 ||
...
char *encode_operator_name(const char *op_name) {
  ...
  if (strcmp(op_name, "+") == 0)
    return strdup("op_add");
  if (strcmp(op_name, "-") == 0)
    return strdup("op_sub");
  if (strcmp(op_name, "*") == 0)
    return strdup("op_mul");
  if (strcmp(op_name, "/") == 0)
    return strdup("op_div");
  if (strcmp(op_name, "=") == 0)
    return strdup("op_eq");
  if (strcmp(op_name, "<>") == 0)
    return strdup("op_ne");
  ...
  if (strcmp(op_name, ">=") == 0)
    return strdup("op_ge");
  if (strcmp(op_name, "**") == 0)
    return strdup("op_pow");
  if (strcmp(op_name, "div") == 0 || strcmp(op_name, "DIV") == 0)
    return strdup("op_intdiv");
  if (strcmp(op_name, "mod") == 0 || strcmp(op_name, "MOD") == 0)
    return strdup("op_mod");
  if (strcmp(op_name, "and") == 0 || strcmp(op_name, "AND") == 0)
    return strdup("op_and");
  if (strcmp(op_name, "or") == 0 || strcmp(op_name, "OR") == 0)
    return strdup("op_or");
  if (strcmp(op_name, "not") == 0 || strcmp(op_name, "NOT") == 0)
    return strdup("op_not");
  if (strcmp(op_name, "xor") == 0 || strcmp(op_name, "XOR") == 0)
    return strdup("op_xor");
  if (strcmp(op_name, "shl") == 0 || strcmp(op_name, "SHL") == 0)
    return strdup("op_shl");
  if (strcmp(op_name, "shr") == 0 || strcmp(op_name, "SHR") == 0)
    return strdup("op_shr");
  if (strcmp(op_name, "in") == 0 || strcmp(op_name, "IN") == 0)
    return strdup("op_in");
  if (strcmp(op_name, "is") == 0 || strcmp(op_name, "IS") == 0)
    return strdup("op_is");
  if (strcmp(op_name, "as") == 0 || strcmp(op_name, "AS") == 0)
    return strdup("op_as");
  if (strcasecmp(op_name, "Equal") == 0)
    return strdup("op_eq");
  if (strcasecmp(op_name, "NotEqual") == 0)
    return strdup("op_ne");
  if (strcasecmp(op_name, "GreaterThan") == 0)
    return strdup("op_gt");
  if (strcasecmp(op_name, "GreaterThanOrEqual") == 0)
    return strdup("op_ge");
  if (strcasecmp(op_name, "LessThan") == 0)
    return strdup("op_lt");
  if (strcasecmp(op_name, "LessThanOrEqual") == 0)
    return strdup("op_le");
  if (strcasecmp(op_name, "Add") == 0)
    return strdup("op_add");
  if (strcasecmp(op_name, "Subtract") == 0)
    return strdup("op_sub");
  if (strcasecmp(op_name, "Multiply") == 0)
    return strdup("op_mul");
  if (strcasecmp(op_name, "Divide") == 0)
    return strdup("op_div");
  if (strcmp(op_name, ":=") == 0)
    return strdup("op_assign");
  /* FPC class operator Implicit is assignment-style conversion. */
  if (strcasecmp(op_name, "Implicit") == 0)
    return strdup("op_assign");
...
char *method_param_type_suffix(Tree_t *param_decl) {
  ...
  if (param_decl->type == TREE_VAR_DECL) {
    if (param_decl->tree_data.var_decl_data.type_ref != NULL)
      return type_ref_render_mangled(
