...
static struct VariantPart *convert_variant_part(ast_t *variant_node,
                                                ListNode_t **out_tag_fields);
...
static ListNode_t *convert_class_field_decl(ast_t *field_decl_node) {
  ...
  if (cursor != NULL && cursor->typ == PASCAL_T_IDENTIFIER &&
    ...
    if (next != NULL && next->typ == PASCAL_T_IDENTIFIER && next->sym != NULL &&
        next->sym->name != NULL && strcasecmp(next->sym->name, "var") == 0) {
      cursor = next->next;
    }
  }
  ...
  while (cursor != NULL && cursor->typ != PASCAL_T_TYPE_SPEC &&
         cursor->typ != PASCAL_T_RECORD_TYPE &&
         cursor->typ != PASCAL_T_OBJECT_TYPE &&
         cursor->typ != PASCAL_T_IDENTIFIER &&
         cursor->typ != PASCAL_T_ARRAY_TYPE && cursor->typ != PASCAL_T_SET &&
  ...
  if (cursor != NULL) {
     ...
     */
    field_type =
        convert_type_spec(cursor, &field_type_id, &nested_record, &field_info);
    if (field_info.is_array) {
      ast_t *type_section = g_implementation_type_section_ast != NULL
                                ? g_implementation_type_section_ast
  ...
  while (name_node != NULL) {
    ...
    if (field_desc != NULL) {
      ...
      field_desc->array_dim_start_str =
          field_info.array_dim_start_str
              ? strdup(field_info.array_dim_start_str)
              : NULL;
      field_desc->array_dim_end_str = field_info.array_dim_end_str
                                          ? strdup(field_info.array_dim_end_str)
                                          : NULL;
      ...
      if (name_node->next == NULL) {
        ...
        field_desc->array_element_type_id =
            field_info.element_type_id ? strdup(field_info.element_type_id)
                                       : NULL;
        field_desc->array_element_record =
            field_info.record_type ? clone_record_type(field_info.record_type)
                                   : NULL;
      }
      ...
      if (field_info.is_set && field_info.set_element_type_id != NULL)
        field_desc->set_element_type_id =
            strdup(field_info.set_element_type_id);
...
static struct ClassProperty *convert_property_decl(ast_t *property_node) {
  ...
  if (type_node != NULL) {
    property_type = convert_type_spec(type_node, &property_type_id,
                                      &inline_record, &type_info);
...
void append_module_property_wrappers(ListNode_t **subprograms,
                                     ast_t *property_node) {
  ...
  if (kgpc_getenv("KGPC_DEBUG_PROPERTY") != NULL) {
    fprintf(
        ...
        prop->name ? prop->name : "<null>", prop->type,
  ...
  if (prop->name != NULL && !prop->is_indexed) {
    if (prop->write_accessor != NULL) {
      ...
      if (proc_name != NULL) {
        ...
        if (param_ids != NULL)
          param_decl = mk_vardecl(0, param_ids, prop->type, param_type_id, 0, 0,
                                  NULL, NULL, NULL, NULL);
        ...
        if (call_args != NULL)
          call_stmt =
              mk_procedurecall(0, strdup(prop->write_accessor), call_args);
        ...
        if (body_stmt != NULL)
          proc_tree = mk_procedure(0, proc_name, params, NULL, NULL, NULL, NULL,
                                   NULL, body_stmt, 0, 0);
  ...
  if (prop->read_accessor)
    free(prop->read_accessor);
  if (prop->write_accessor)
    free(prop->write_accessor);
  free(prop);
}

void annotate_method_template(struct MethodTemplate *method_template,
                              ast_t *method_ast) {
  ...
  while (cursor != NULL) {
     ...
     * Note: "class function" has Self = VMT pointer; "class function ...
     * static" has no Self. Only set is_class_method here; is_static is set by
  ...
  while (cursor != NULL) {
    ...
    case PASCAL_T_IDENTIFIER:
      if (method_template->return_type_ast == NULL && sym_name != NULL &&
    ...
    case PASCAL_T_METHOD_DIRECTIVE:
      ...
      /* Check for directive keywords (virtual, override, static) in symbol name
       * and children */
      if (sym_name != NULL) {
        if (strcasecmp(sym_name, "virtual") == 0)
          method_template->is_virtual = 1;
        else if (strcasecmp(sym_name, "abstract") == 0)
          method_template->is_virtual =
              1; /* Abstract methods are implicitly virtual */
      ...
      if (node->child != NULL) {
        ...
        if (dir_child != NULL && dir_child->sym != NULL &&
            dir_child->sym->name != NULL) {
      ...
      while (dir_cur != NULL) {
        ...
        if (unwrapped_dir != NULL &&
            unwrapped_dir->typ == PASCAL_T_IDENTIFIER &&
  ...
  if (method_template->return_type_ast == NULL) {
    method_template->return_type_ast =
        find_ast_node_type(method_ast, PASCAL_T_RETURN_TYPE);
    if (method_template->return_type_ast != NULL)
      method_template->has_return_type = 1;
  }
}
...
static int extract_interface_delegation_info(ast_t *method_decl_node,
  ...
  for (ast_t *cur = method_decl_node->child; cur != NULL; cur = cur->next) {
    ...
    if (node->typ == PASCAL_T_PARAM_LIST || node->typ == PASCAL_T_PARAM ||
        node->typ == PASCAL_T_RETURN_TYPE ||
        node->typ == PASCAL_T_METHOD_DIRECTIVE)
      return 0;
  ...
  char *iface_name = iface_qid != NULL ? qualified_ident_join(iface_qid, ".")
                                       : dup_symbol(significant[0]);
...
struct MethodTemplate *create_method_template(ast_t *method_decl_node) {
  ...
  struct MethodTemplate *template =
      (struct MethodTemplate *)calloc(1, sizeof(struct MethodTemplate));
  if (template == NULL)
    return NULL;

  if (!extract_interface_delegation_info(
          method_decl_node, &template->delegated_interface_name,
          &template->name, &template->delegated_target_name)) {
    ast_t *name_node = method_decl_node->child;
    while (name_node != NULL) {
      if (name_node->typ == PASCAL_T_IDENTIFIER) {
        const char *sym_name =
            name_node->sym != NULL ? name_node->sym->name : NULL;
        if (sym_name != NULL && (is_method_decl_keyword(sym_name) ||
                                 strcasecmp(sym_name, "generic") == 0)) {
    ...
    if (unwrapped != NULL) {
      ...
      case PASCAL_T_CLASS_MEMBER: {
        ...
        if (saw_class && saw_var) {
          ...
          /* "class function/procedure" member: propagate is_class_method to
           * templates */
          for (ast_t *child = unwrapped->child; child != NULL;
               child = child->next) {
            ...
            if (cn->typ == PASCAL_T_METHOD_DECL ||
              ...
              if (template != NULL) {
                ...
                if (!template->is_interface_delegation) {
                  ...
                  register_class_method_ex(
                      class_name, template->name, template->is_virtual,
                      template->is_override, template->is_static,
                      template->is_class_method, param_count, param_types,
                      param_types_count);
                }
                if (method_builder != NULL)
                  list_builder_append(method_builder, template,
              ...
              if (property != NULL && property_builder != NULL)
                list_builder_append(property_builder, property,
                                    LIST_CLASS_PROPERTY);
            }
          }
        } else {
          collect_class_members(unwrapped->child, class_name, field_builder,
                                property_builder, method_builder,
                                nested_type_builder);
        }
        break;
      }
      ...
      case PASCAL_T_VAR_SECTION: {
        ...
        for (ast_t *child = unwrapped->child; child != NULL;
          ...
          if (node != NULL && node->sym != NULL && node->sym->name != NULL) {
            ...
            if (strcasecmp(node->sym->name, "var") == 0) {
              class_var_active = pending_class;
              pending_class = 0;
              continue;
            }
          }
          if (node != NULL && node->typ == PASCAL_T_FIELD_DECL) {
            ListNode_t *fields = convert_class_field_decl(node);
            if (class_var_active && fields != NULL) {
              for (ListNode_t *fnode = fields; fnode != NULL;
                   fnode = fnode->next) {
                if (fnode->type == LIST_RECORD_FIELD && fnode->cur != NULL) {
      ...
      case PASCAL_T_NESTED_TYPE_SECTION: {
        /* Nested type declarations inside record/class (Delphi syntax: public
         * type ...) */
        if (nested_type_builder != NULL) {
          ...
          if (kgpc_getenv("KGPC_DEBUG_TYPE_SECTION") != NULL)
            fprintf(stderr,
                    "[KGPC] collect_class_members: found NESTED_TYPE_SECTION "
                    "in %s at line %d\n",
...
struct RecordType *convert_class_type_ex(const char *class_name,
                                         ast_t *class_node,
                                         ListNode_t **nested_types_out) {
  if (class_node == NULL)
    return NULL;
  ...
  if (kgpc_getenv("KGPC_DEBUG_CLASS_METHODS") != NULL) {
    ...
    if (class_name != NULL && (strcasecmp(class_name, "TList") == 0 ||
  ...
  collect_class_members(body_start, class_name, &field_builder,
                        &property_builder, &method_template_builder,
                        &nested_type_builder);
  ...
  struct RecordType *record =
      (struct RecordType *)malloc(sizeof(struct RecordType));
  if (record == NULL) {
  ...
  if (parent_class_name == NULL) {
    struct RecordField *typeinfo_field =
        (struct RecordField *)calloc(1, sizeof(struct RecordField));
...
struct RecordType *convert_interface_type_ex(const char *interface_name,
                                             ast_t *interface_node,
                                             ListNode_t **nested_types_out) {
  ...
  if (body_start != NULL && body_start->typ == PASCAL_T_INTERFACE_GUID) {
    ...
    while (guid_child != NULL) {
      if ((guid_child->typ == PASCAL_T_STRING ||
        ...
        has_guid = parse_guid_literal(guid_child->sym->name, &guid_d1, &guid_d2,
                                      &guid_d3, guid_d4);
        break;
      }
  ...
  /* Optional GUID string follows parent identifier (when GUID is a string
   * literal) */
  if (body_start != NULL && body_start->typ == PASCAL_T_STRING) {
    if (body_start->sym != NULL && body_start->sym->name != NULL) {
      free(guid_string);
      guid_string = strdup(body_start->sym->name);
      has_guid = parse_guid_literal(body_start->sym->name, &guid_d1, &guid_d2,
                                    &guid_d3, guid_d4);
    }
    body_start = body_start->next;
  }
  ...
  collect_class_members(body_start, interface_name, &field_builder,
                        &property_builder, &method_template_builder,
                        &nested_type_builder);
  ...
  struct RecordType *record =
      (struct RecordType *)calloc(1, sizeof(struct RecordType));
  if (record == NULL) {
    destroy_list(field_builder.head);
    destroy_list(property_builder.head);
    destroy_list(method_template_builder.head);
...
static ListNode_t *convert_field_decl(ast_t *field_decl_node) {
  ...
  while (cursor != NULL) {
    ...
    if (node->typ == PASCAL_T_IDENTIFIER && node->sym != NULL &&
        node->sym->name != NULL &&
        (strcasecmp(node->sym->name, "class") == 0 ||
  ...
  if (names == NULL) {
    fprintf(stderr,
            "ERROR: record field declaration missing identifier list.\n");
    return NULL;
  }
  ...
  while (name_node != NULL) {
    ...
    if (field_desc != NULL) {
      ...
      if (field_info.enum_literals != NULL && name_node->next == NULL) {
        ...
        for (ListNode_t *en = field_info.enum_literals; en != NULL;
             en = en->next)
          if (en->cur != NULL)
            list_builder_append(&clone_builder, strdup((char *)en->cur),
                                LIST_STRING);
...
static struct VariantBranch *convert_variant_branch(ast_t *branch_node) {
  ...
  struct VariantBranch *branch =
      (struct VariantBranch *)calloc(1, sizeof(struct VariantBranch));
  if (branch == NULL)
    return NULL;
...
static struct VariantPart *convert_variant_part(ast_t *variant_node,
  ...
  struct VariantPart *variant =
      (struct VariantPart *)calloc(1, sizeof(struct VariantPart));
  if (variant == NULL)
    return NULL;
  ...
  if (cursor != NULL && cursor->typ == PASCAL_T_VARIANT_TAG) {
    ...
    if (tag_cursor != NULL && tag_cursor->typ == PASCAL_T_IDENTIFIER &&
        tag_cursor->next != NULL) {
    ...
    if (type_cursor != NULL)
      tag_type =
          convert_type_spec(type_cursor, &tag_type_id, &tag_record, NULL);

    if (tag_name != NULL) {
      struct RecordField *tag_field =
          (struct RecordField *)calloc(1, sizeof(struct RecordField));
      if (tag_field != NULL) {
...
void convert_record_members(ast_t *node, ListBuilder *builder,
  ...
  for (ast_t *cur = node; cur != NULL; cur = cur->next) {
    if (cur->typ == PASCAL_T_FIELD_DECL) {
      ...
      /* Store method declaration as a special marker node for operator
       * overloading */
      /* We'll handle this during semantic check when we know the record type
       * name */
      ...
      if (method_template_builder != NULL) {
        ...
        if (template != NULL)
          list_builder_append(method_template_builder, template,
                              LIST_METHOD_TEMPLATE);
      }
      ...
      for (ast_t *child = cur->child; child != NULL; child = child->next) {
        ...
        if (node != NULL && node->typ == PASCAL_T_FIELD_DECL) {
          ListNode_t *fields = convert_field_decl(node);
          if (class_var_active && fields != NULL) {
            for (ListNode_t *fnode = fields; fnode != NULL;
                 fnode = fnode->next) {
              if (fnode->type == LIST_RECORD_FIELD && fnode->cur != NULL) {
       ...
       * 3. plain "var" fields from object types — keep instance fields */
      int has_field_decls = 0;
      int saw_class = 0;
      int saw_var = 0;
      for (ast_t *chk = cur->child; chk != NULL; chk = chk->next) {
        ast_t *node = unwrap_pascal_node(chk);
        if (node != NULL && node->typ == PASCAL_T_FIELD_DECL)
          has_field_decls = 1;
        if (node != NULL && node->sym != NULL && node->sym->name != NULL) {
          if (strcasecmp(node->sym->name, "class") == 0)
            saw_class = 1;
                   ...
                   strcasecmp(node->sym->name, "threadvar") == 0)
            saw_var = 1;
        }
      }
      if (has_field_decls) {
        /* Convert fields and mark class storage only when the parser
         * carried explicit class-var markers. */
        for (ast_t *child = cur->child; child != NULL; child = child->next) {
          ast_t *node = unwrap_pascal_node(child);
          if (node != NULL && node->typ == PASCAL_T_FIELD_DECL) {
            ListNode_t *fields = convert_field_decl(node);
...
void collect_record_nested_types(ast_t *node,
                                 ListBuilder *nested_type_builder) {
  for (ast_t *cur = node; cur != NULL; cur = cur->next) {
    ...
    } else if (unwrapped->typ == PASCAL_T_NESTED_TYPE_SECTION ||
               unwrapped->typ == PASCAL_T_TYPE_SECTION) {
      /* Found a nested type section */
      if (nested_type_builder != NULL) {
        list_builder_append(nested_type_builder, unwrapped, LIST_UNSPECIFIED);
        if (kgpc_getenv("KGPC_DEBUG_TYPE_SECTION") != NULL)
          fprintf(stderr,
                  "[KGPC] collect_record_nested_types: found "
                  "NESTED_TYPE_SECTION at line %d\n",
                  unwrapped->line);
      }
    }
  }
...
struct RecordType *convert_record_type_ex(ast_t *record_node,
                                          ListNode_t **nested_types_out) {
  if (record_node == NULL)
    return NULL;
  ...
  if (record_node->sym != NULL && record_node->sym->name != NULL &&
    ...
    while (base_node != NULL) {
      ...
      if (type_ident != NULL &&
          (type_ident->typ == PASCAL_T_IDENTIFIER ||
           type_ident->typ == PASCAL_T_QUALIFIED_IDENTIFIER) &&
        ...
        while (member != NULL) {
          ...
          if (member_unwrapped != NULL) {
            if (member_unwrapped->typ == PASCAL_T_METHOD_DECL ||
              ...
              list_builder_append(&helper_fields_builder, member_unwrapped,
                                  LIST_UNSPECIFIED);
              struct MethodTemplate *template =
                  create_method_template(member_unwrapped);
              if (template != NULL) {
                list_builder_append(&method_template_builder, template,
                                    LIST_METHOD_TEMPLATE);
              }
              ...
              struct ClassProperty *property =
                  convert_property_decl(member_unwrapped);
              if (property != NULL)
                list_builder_append(&helper_properties_builder, property,
                                    LIST_CLASS_PROPERTY);
            }
          }
          member = member->next;
      ...
      if (unwrapped != NULL && (unwrapped->typ == PASCAL_T_METHOD_DECL ||
                                unwrapped->typ == PASCAL_T_CONSTRUCTOR_DECL ||
                                unwrapped->typ == PASCAL_T_DESTRUCTOR_DECL)) {
        list_builder_append(&helper_fields_builder, unwrapped,
                            LIST_UNSPECIFIED);
  ...
  if (record_node->typ == PASCAL_T_OBJECT_TYPE && members_start != NULL &&
      members_start->typ == PASCAL_T_IDENTIFIER && members_start->sym != NULL &&
...
ListNode_t *convert_param(ast_t *param_node) {
  ...
  if (modifier_node != NULL && modifier_node->sym != NULL &&
      modifier_node->sym->name != NULL) {
    const char *modifier_name = modifier_node->sym->name;
    if (strcasecmp(modifier_name, "var") == 0 ||
        strcasecmp(modifier_name, "out") == 0)
      is_var_param = 1;
    else if (strcasecmp(modifier_name, "const") == 0)
      is_const_param = 1;
    else if (strcasecmp(modifier_name, "constref") == 0) {
      /* constref is like var (pass by reference) but also const (not
       * modifiable). For codegen purposes, we treat it as is_var_param=1 to
       * pass by reference. The const aspect is enforced by semantic checking.
       */
      is_var_param = 1;
      is_const_param = 1;
    }
  ...
  if (type_node == NULL || type_node->typ != PASCAL_T_TYPE_SPEC) {
    ...
    if (type_node->next != NULL &&
        type_node->next->typ == PASCAL_T_DEFAULT_VALUE) {
      default_value_node = type_node->next;
    } else {
      default_value_node =
          find_ast_node_type(param_node, PASCAL_T_DEFAULT_VALUE);
    }
  ...
  if (default_value_node != NULL) {
    /* PASCAL_T_DEFAULT_VALUE's child IS the expression (no "=" token since
     * match returns ast_nil) */
    ...
    if (expr_node != NULL) {
      ...
      if (default_expr != NULL) {
         ...
         */
        default_init =
            mk_varassign(default_value_node->line, default_value_node->col,
                         NULL, default_expr);
        if (kgpc_getenv("KGPC_DEBUG_DEFAULT_PARAMS") != NULL) {
  ...
  while (id_node != NULL) {
    ...
    if (type_info.is_array) {
      ...
      char *element_type_id = type_info.element_type_id != NULL
      ...
      param_decl = mk_arraydecl(param_node->line, id_node, element_type,
                                element_type_id, type_info.start, type_info.end,
                                range_str, NULL, inline_record);
      if (param_decl != NULL)
        param_decl->tree_data.arr_decl_data.is_shortstring =
      ...
      if (is_var_param && param_decl != NULL)
        param_decl->tree_data.arr_decl_data.type =
            var_type; // Store this for compatibility
      if (kgpc_getenv("KGPC_DEBUG_ARRAY_PARAM") != NULL && param_decl != NULL) {
        fprintf(
            stderr,
            "[KGPC] array param %s: element_type=%d element_id=%s "
            "is_array_of_const=%d\n",
            id_node != NULL && id_node->cur != NULL ? (const char *)id_node->cur
                                                    : "<unnamed>",
            element_type, element_type_id != NULL ? element_type_id : "<null>",
            type_info.is_array_of_const);
      }
      /* Note: array parameters with default values are rare but could be
       * supported */
    } else {
      param_decl =
          mk_vardecl(param_node->line, id_node, var_type, type_id_copy,
                     is_var_param, 0, default_init, NULL, inline_alias, NULL);
      if (param_decl != NULL)
        param_decl->tree_data.var_decl_data.is_const_param = is_const_param;
      if (param_decl != NULL &&
          (type_node == NULL || type_node->typ != PASCAL_T_TYPE_SPEC))
...
from_cparser_method_template_to_proctype(struct MethodTemplate *method_template,
  ...
  /* Add implicit Self parameter for instance methods and non-static class
   * methods. For instance methods, Self = instance pointer. For class methods
   * (non-static), Self = VMT pointer (class reference). */
  if (!method_template->is_static) {
    ...
    if (record != NULL) {
      if (record->is_type_helper && record->helper_base_type_id != NULL) {
        ...
        self_is_var =
            helper_self_param_is_var(record->helper_base_type_id, symtab);
    ...
    Tree_t *self_param =
        mk_vardecl(0, self_ids, self_type_tag, self_type_id, self_is_var, 0,
                   NULL, NULL, self_type_alias, NULL);
    if (self_param != NULL)
  ...
  if (method_template->params_ast != NULL) {
    ...
    pascal_frontend_set_default_shortstring(
        method_template->default_shortstring != 0);
    ListNode_t *extra_params =
        from_cparser_convert_params_ast(method_template->params_ast);
  ...
  if (method_template->has_return_type &&
      method_template->return_type_ast != NULL) {
    ...
    if (return_type == NULL && ret_node != NULL &&
        ret_node->typ == PASCAL_T_IDENTIFIER) {
...
bool is_var_hint_clause(ast_t *node) {
  ...
  return strcasecmp(name, "deprecated") == 0 ||
         strcasecmp(name, "platform") == 0 || strcasecmp(name, "library") == 0;
}

ast_t *absolute_clause_target(ast_t *node) {
  if (node == NULL || node->typ != PASCAL_T_ABSOLUTE_CLAUSE)
    return NULL;
  /* The PASCAL_T_ABSOLUTE_CLAUSE node contains the target identifier as its
   * child (the "absolute" keyword itself is consumed by keyword_ci and not in
   * the AST) */
  ast_t *child = node->child;
 ...
 * This handles:
 * - PASCAL_T_ABSOLUTE_CLAUSE: absolute variable aliasing (e.g., "X: Integer
 * absolute Y")
 * - PASCAL_T_IDENTIFIER: trailing identifiers that aren't initializers (legacy
 * case)
 */
int is_node_to_skip_as_initializer(ast_t *node) {
  if (node == NULL)
    return 0;
  return (node->typ == PASCAL_T_IDENTIFIER ||
          node->typ == PASCAL_T_ABSOLUTE_CLAUSE);
}
