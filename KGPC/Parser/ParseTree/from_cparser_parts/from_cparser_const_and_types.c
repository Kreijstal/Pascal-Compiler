 ...
 * The argument subtree is simpler than CONSTRUCTED_TYPE: it consists of
 * identifiers separated by punctuation inside a PASCAL_T_NONE wrapper created
 * by specialize_args. */
...
int extract_specialize_type_info(ast_t *spec_node, char **base_name_out,
                                 ListNode_t **type_args_out) {
...
static struct Expression *convert_case_label_expression(ast_t *node) {
  ...
  if (unwrapped != NULL && (unwrapped->typ == PASCAL_T_QUALIFIED_IDENTIFIER ||
...
int map_type_name(const char *name, char **type_id_out) {
  ...
  if (strcasecmp(name, "nativeint") == 0 || strcasecmp(name, "sizeint") == 0 ||
  ...
  if (strcasecmp(name, "nativeuint") == 0 ||
      strcasecmp(name, "sizeuint") == 0 || strcasecmp(name, "ptruint") == 0) {
    if (type_id_out != NULL)
      *type_id_out = strdup(name);
    return INT64_TYPE;
  }
  ...
  /* WideChar is a 2-byte character type; resolve via symbol table for correct
   * size. */
  ...
  /* Procedure and Function as bare type names (no parameters) - procedure
   * pointers */
...
int helper_self_param_is_var(const char *base_type_id, struct SymTab *symtab) {
  ...
  if (symtab != NULL) {
    ...
    if (FindSymbol(&type_node, symtab, base_type_id) != 0 &&
        type_node != NULL) {
...
int tuple_is_record_constructor(ast_t *tuple_node) {
  ...
  int debug_this =
      debug_tuple && tuple_node->line >= 2165 && tuple_node->line <= 2175;
  if (debug_this) {
...
struct Expression *convert_record_constructor_expr(ast_t *expr_node) {
  ...
  for (ast_t *field_assignment = expr_node->child; field_assignment != NULL;
    ...
    if (field_value == NULL) {
      fprintf(stderr,
              "ERROR: Failed to convert record constructor field value at line "
              "%d.\n",
              expr_node->line);
      goto record_ctor_cleanup;
    }
    ...
    struct RecordConstructorField *field =
        (struct RecordConstructorField *)calloc(
            1, sizeof(struct RecordConstructorField));
    if (field == NULL)
      goto record_ctor_cleanup;
 ...
 */
static int resolve_enum_ordinal_in_section_chain(const char *identifier,
                                                 ast_t *section_root);
static int resolve_enum_ordinal_in_type_section(const char *identifier,
                                                ast_t *type_section);
...
int resolve_enum_ordinal_from_ast(const char *identifier, ast_t *type_section) {
  ...
  if (g_interface_type_section_ast != NULL) {
    int v = resolve_enum_ordinal_in_type_section(identifier,
                                                 g_interface_type_section_ast);
    if (v >= 0)
      return v;
  }
  if (g_implementation_type_section_ast != NULL) {
    int v = resolve_enum_ordinal_in_type_section(
        identifier, g_implementation_type_section_ast);
    if (v >= 0)
      return v;
  }
  int from_iface = resolve_enum_ordinal_in_section_chain(
      identifier, g_interface_section_ast);
  if (from_iface >= 0)
    return from_iface;
  return resolve_enum_ordinal_in_section_chain(identifier,
                                               g_implementation_section_ast);
}
...
static int resolve_enum_ordinal_in_type_section(const char *identifier,
  ...
  while (type_decl != NULL) {
    if (type_decl->typ == PASCAL_T_TYPE_DECL) {
      ...
      if (spec != NULL && spec->typ == PASCAL_T_TYPE_SPEC &&
          spec->child != NULL)
        spec = spec->child;
          ...
          if (type_decl->typ == PASCAL_T_TYPE_DECL) {
            ...
            if (spec != NULL && spec->typ == PASCAL_T_ENUMERATED_TYPE) {
              ...
              while (literal != NULL) {
                ast_t *literal_id = literal;
                if (literal_id != NULL &&
                    literal_id->typ == PASCAL_T_ASSIGNMENT) {
                  ast_t *name_node = literal_id->child;
                  ast_t *value_node =
                      (name_node != NULL) ? name_node->next : NULL;
                  int explicit_val = 0;
                  if (value_node != NULL &&
                      evaluate_const_int_expr(value_node, &explicit_val, 0) ==
                          0) {
                    ordinal = explicit_val;
                  }
                  literal_id = name_node;
                ...
                if (literal_id != NULL &&
                    literal_id->typ == PASCAL_T_IDENTIFIER &&
                    literal_id->sym != NULL &&
...
int resolve_enum_literal_in_type(const char *type_name, const char *literal,
                                 ast_t *type_section) {
        ...
        if (spec != NULL && spec->typ == PASCAL_T_ENUMERATED_TYPE) {
          ...
          for (ast_t *lit = spec->child; lit != NULL; lit = lit->next) {
            ...
            if (lit_id != NULL && lit_id->typ == PASCAL_T_IDENTIFIER &&
                lit_id->sym != NULL && lit_id->sym->name != NULL &&
 ...
 * For example, if color = (red, blue, yellow), then
 * resolve_enum_type_range_from_ast("color", ...) will set out_start=0 and
 * out_end=2 (for 3 values: red, blue, yellow). Returns 0 on success, -1 if the
 * type is not found or is not an enum type.
 */
int resolve_enum_type_range_from_ast(const char *type_name, ast_t *type_section,
                                     int *out_start, int *out_end) {
  if (type_name == NULL || type_section == NULL || out_start == NULL ||
      out_end == NULL)
    return -1;
  ...
  while (type_decl != NULL) {
    if (type_decl->typ == PASCAL_T_TYPE_DECL) {
      ...
      if (type_id != NULL && type_id->typ == PASCAL_T_IDENTIFIER &&
          type_id->sym != NULL) {
...
/* Helper function to resolve a const integer identifier from the same const
 * section. This is needed for array ranges like [C_Low .. C_High]. Returns the
 * integer value if found, otherwise returns fallback_value. Note: Only resolves
 * simple integer consts; does not handle forward references or complex const
 * expressions.
 */
static int resolve_const_int_from_ast_internal(const char *identifier,
                                               ast_t *const_section,
                                               int fallback_value, int depth);
static int resolve_const_int_in_node(const char *identifier, ast_t *node,
                                     ast_t *const_section, int *out_value,
                                     int depth) {
  if (node == NULL)
    return -1;
  if (depth > 32)
  ...
  if (resolve_const_int_in_node(identifier, node->child, const_section,
                                out_value, depth) == 0)
    return 0;
  if (resolve_const_int_in_node(identifier, node->next, const_section,
                                out_value, depth) == 0)
    return 0;
  return -1;
}
...
static int ast_string_value_assign_bytes(AstStringValue *value,
...
static int ast_string_value_append(AstStringValue *dst,
                                   const AstStringValue *src) {
  char *combined = NULL;
...
int evaluate_const_string_ast(ast_t *node, ast_t *const_section,
  ...
  case PASCAL_T_IDENTIFIER: {
    ...
    if (unwrapped->sym != NULL && unwrapped->sym->name != NULL &&
        resolve_const_string_from_ast_internal(
            unwrapped->sym->name, const_section, &resolved, depth + 1) == 0 &&
        resolved != NULL) {
      return ast_string_value_assign_bytes(out_value, resolved,
                                           strlen(resolved));
    }
    return -1;
  }
...
int type_info_targets_char_array(const TypeInfo *type_info,
                                 int *is_widechar_out) {
...
struct Expression *mk_const_array_element_lhs(int line_num,
                                              const char *array_name,
                                              int outer_index, int inner_index,
                                              int is_multidim) {
  ...
  if (is_multidim) {
    struct Expression *lhs =
        mk_arrayaccess(line_num, base_expr, outer_index_expr);
  ...
  {
    struct Expression *outer_access =
        mk_arrayaccess(line_num, base_expr, outer_index_expr);
    return mk_arrayaccess(line_num, outer_access, inner_index_expr);
  }
}

static int resolve_const_string_in_section(const char *identifier,
                                           ast_t *const_section,
                                           const char **out_value, int depth) {
  if (identifier == NULL || const_section == NULL || out_value == NULL)
    return -1;
  return resolve_const_string_in_node(identifier, const_section->child,
                                      const_section, out_value, depth);
}

int resolve_const_string_from_ast_internal(const char *identifier,
                                           ast_t *const_section,
                                           const char **out_value, int depth) {
  if (identifier == NULL || const_section == NULL || out_value == NULL)
    return -1;
  ...
  if (g_const_sections != NULL) {
    for (ListNode_t *cur = g_const_sections; cur != NULL; cur = cur->next) {
      if (resolve_const_string_in_section(identifier, (ast_t *)cur->cur,
                                          out_value, depth) == 0) {
        found = 1;
        break;
      }
    ...
    if (resolve_const_string_in_section(identifier, const_section, out_value,
                                        depth) == 0)
      found = 1;
  }
...
static int resolve_const_string_in_node(const char *identifier, ast_t *node,
    ...
    if (id_node != NULL && id_node->sym != NULL) {
      if (strcasecmp(id_node->sym->name, identifier) == 0) {
        ...
        if (value_node != NULL && value_node->typ == PASCAL_T_IDENTIFIER &&
            value_node->sym != NULL &&
            strcasecmp(value_node->sym->name, identifier) != 0) {
          if (resolve_const_string_from_ast_internal(value_node->sym->name,
                                                     const_section, out_value,
                                                     depth + 1) == 0)
            return 0;
        }
      }
  ...
  if (resolve_const_string_in_node(identifier, node->child, const_section,
                                   out_value, depth) == 0)
    return 0;
  if (resolve_const_string_in_node(identifier, node->next, const_section,
                                   out_value, depth) == 0)
    return 0;
  return -1;
}

static int resolve_enum_type_range_in_section_chain(const char *type_name,
                                                    ast_t *section_root,
                                                    int *out_start,
                                                    int *out_end) {
  if (type_name == NULL || section_root == NULL || out_start == NULL ||
      out_end == NULL)
    return -1;
  ...
  while (section != NULL) {
    ...
    for (ast_t *cursor = node; cursor != NULL;
         cursor = (section->typ == PASCAL_T_NONE) ? cursor->next : NULL) {
      if (cursor->typ == PASCAL_T_TYPE_SECTION) {
        if (resolve_enum_type_range_from_ast(type_name, cursor, out_start,
                                             out_end) == 0)
          return 0;
      }
    }
...
int resolve_enum_type_range_with_fallback(const char *type_name,
                                          ast_t *type_section, int *out_start,
                                          int *out_end) {
  if (resolve_enum_type_range_from_ast(type_name, type_section, out_start,
                                       out_end) == 0)
    return 0;
  if (resolve_enum_type_range_in_section_chain(
          type_name, g_interface_section_ast, out_start, out_end) == 0)
    return 0;
  if (resolve_enum_type_range_in_section_chain(
          type_name, g_implementation_section_ast, out_start, out_end) == 0)
    return 0;
...
ast_t *find_type_decl_in_section(ast_t *type_section, const char *type_name) {
  ...
  for (ast_t *type_decl = type_section->child; type_decl != NULL;
       type_decl = type_decl->next) {
    if (type_decl->typ != PASCAL_T_TYPE_DECL)
      continue;
    ast_t *id_node = type_decl->child;
...
int type_name_is_class_like(const char *type_name) {
  ...
  ast_t *type_decl =
      find_type_decl_in_section(g_interface_type_section_ast, type_name);
  if (type_decl == NULL)
    type_decl =
        find_type_decl_in_section(g_implementation_type_section_ast, type_name);
  if (type_decl == NULL)
    return 0;
...
int resolve_array_type_info_from_ast(const char *type_name, ast_t *type_section,
                                     TypeInfo *out_info, int depth) {
  ...
  if (spec_child != NULL && spec_child->typ == PASCAL_T_IDENTIFIER &&
      spec_child->sym != NULL && spec_child->sym->name != NULL) {
    return resolve_array_type_info_from_ast(spec_child->sym->name, type_section,
                                            out_info, depth + 1);
  }
...
void resolve_array_bounds(TypeInfo *info, ast_t *type_section,
                          ast_t *const_section, const char *id_for_error) {
  if (info == NULL)
    return;
  ...
  if (sep != NULL) {
    *sep = '\0';
    ...
    while (*start_id == ' ' || *start_id == '\t')
      start_id++;
    while (*end_id == ' ' || *end_id == '\t')
      end_id++;
    char *p = start_id + strlen(start_id) - 1;
    while (p > start_id && (*p == ' ' || *p == '\t'))
      *p-- = '\0';
    ...
    } else if (strlen(start_id) == 3 && start_id[0] == '\'' &&
               start_id[2] == '\'' && strlen(end_id) == 3 &&
               end_id[0] == '\'' && end_id[2] == '\'') {
      info->start = (unsigned char)start_id[1];
      info->end = (unsigned char)end_id[1];
    } else {
      int start_val;
      if (evaluate_simple_const_expr(start_id, const_section, &start_val) ==
          0) {
        info->start = start_val;
      } else {
        int resolved_start =
            resolve_const_int_from_ast(start_id, const_section, 0);
    ...
    if (resolve_enum_type_range_with_fallback(range_str, type_section,
...
static int resolve_const_int_in_section(const char *identifier,
                                        ast_t *const_section, int *out_value,
                                        int depth) {
  if (identifier == NULL || const_section == NULL || out_value == NULL)
    return -1;
  return resolve_const_int_in_node(identifier, const_section->child,
                                   const_section, out_value, depth);
}
...
int evaluate_const_int_expr(ast_t *expr, int *out_value, int depth) {
  ...
  case PASCAL_T_IDENTIFIER:
    return lookup_const_int(expr->sym != NULL ? expr->sym->name : NULL,
                            out_value);
  case PASCAL_T_NEG: {
    int inner = 0;
  ...
  case PASCAL_T_FUNC_CALL: {
    if (expr->child != NULL && expr->child->typ == PASCAL_T_IDENTIFIER &&
      ...
      const char *func_name =
          expr->child->sym != NULL ? expr->child->sym->name : NULL;
      ast_t *arg = unwrap_pascal_node(expr->child->next);
      const char *type_name =
          (arg != NULL && arg->typ == PASCAL_T_IDENTIFIER && arg->sym != NULL)
              ? arg->sym->name
              : NULL;
      ...
      } else if (pascal_identifier_equals(func_name, "Length") && arg != NULL &&
                 arg->typ == PASCAL_T_STRING && arg->sym != NULL &&
                 arg->sym->name != NULL) {
...
static int resolve_const_int_from_ast_internal(const char *identifier,
  ...
  if (g_const_sections != NULL) {
    for (ListNode_t *cur = g_const_sections; cur != NULL; cur = cur->next) {
      if (resolve_const_int_in_section(identifier, (ast_t *)cur->cur, &resolved,
                                       depth) == 0) {
        found = 1;
        break;
      }
    ...
    if (resolve_const_int_in_section(identifier, const_section, &resolved,
                                     depth) == 0)
      found = 1;
  }
...
int resolve_const_int_from_ast(const char *identifier, ast_t *const_section,
                               int fallback_value) {
  return resolve_const_int_from_ast_internal(identifier, const_section,
                                             fallback_value, 0);
}
...
int parse_integer_literal(const char *num_str, int base, long long *out_value,
                          char **out_endptr) {
...
static int const_expr_parse_number(ConstExprScanner *scanner,
                                   long long *out_value) {
  const_expr_skip_ws(scanner);
...
static int resolve_sizeof_type(const char *type_name) {
  ...
  if (strcasecmp(type_name, "Double") == 0 ||
    ...
    return 8;
  /* 10-byte extended (padded to 16 on x86_64 in some ABIs, but SizeOf returns
   * 10) */
...
static int const_expr_parse_identifier(ConstExprScanner *scanner,
                                       long long *out_value) {
  const_expr_skip_ws(scanner);
  ...
  if (strcasecmp(ident, "sizeof") == 0) {
    ...
    while (scanner->input[scanner->pos] != '\0' &&
           ...
           (isalnum((unsigned char)scanner->input[scanner->pos]) ||
            scanner->input[scanner->pos] == '_'))
      scanner->pos++;
...
static int const_expr_parse_expression(ConstExprScanner *scanner,
                                       long long *out_value);

static int const_expr_parse_primary(ConstExprScanner *scanner,
                                    long long *out_value) {
  const_expr_skip_ws(scanner);
...
static int const_expr_parse_unary(ConstExprScanner *scanner,
                                  long long *out_value) {
  if (const_expr_match_char(scanner, '-')) {
    long long inner = 0;
...
static int const_expr_parse_mul(ConstExprScanner *scanner,
                                long long *out_value) {
  long long lhs = 0;
...
static int const_expr_parse_add(ConstExprScanner *scanner,
                                long long *out_value) {
  long long lhs = 0;
...
int evaluate_simple_const_expr(const char *expr, ast_t *const_section,
                               int *result) {
...
static char *serialize_expr_to_string_internal(ast_t *expr, int parent_prec) {
  ...
  if (expr->typ == PASCAL_T_CHAR && expr->sym != NULL &&
      expr->sym->name != NULL) {
  ...
  if ((expr->typ == PASCAL_T_ADD || expr->typ == PASCAL_T_SUB ||
       ...
       expr->typ == PASCAL_T_INTDIV || expr->typ == PASCAL_T_MOD ||
       expr->typ == PASCAL_T_SHL || expr->typ == PASCAL_T_SHR) &&
  ...
  if (expr->child != NULL && expr->child->sym != NULL &&
      expr->child->sym->name != NULL) {
...
int convert_type_spec(ast_t *type_spec, char **type_id_out,
  ...
  if (spec_node->typ == PASCAL_T_IDENTIFIER) {
    ...
    if (dup != NULL && spec_node->child != NULL &&
      ...
      if (type_info != NULL) {
        ...
        /* Numeric bounds are in type_info->start/end; no symbolic strings
         * needed */
   ...
   * using the same mangling and RecordType template. */
  {
    ...
    if (spec_node->typ == PASCAL_T_CONSTRUCTED_TYPE) {
      is_generic =
          extract_constructed_type_info(type_spec, &base_name, &type_args);
    } else {
      is_generic =
          extract_specialize_type_info(type_spec, &base_name, &type_args);
    }

    if (is_generic) {
      char *specialized_name = NULL;
      struct RecordType *record =
          instantiate_generic_record(base_name, type_args, &specialized_name);
      ...
      if (record != NULL) {
        if (kgpc_getenv("KGPC_DEBUG_TFPG") != NULL &&
          ...
          fprintf(stderr,
                  "[KGPC] convert_type_spec generic %s -> record=%p "
                  "type_info_ptr=%p record_out_ptr=%p\n",
                  specialized_name, (void *)record, (void *)type_info,
                  (void *)record_out);
        }
        if (type_info != NULL) {
      ...
      if (type_info != NULL) {
        ...
        if (resolve_generic_alias_type(base_name, type_args, type_id_out,
                                       type_info, &alias_result)) {
      ...
      if (can_defer) {
        ...
        if (type_id_out != NULL) {
          ...
          if (specialized_name == NULL)
            specialized_name =
                mangle_specialized_name_from_list(base_name, type_args);
          *type_id_out = specialized_name;
          specialized_name = NULL;
        }
        if (specialized_name != NULL)
  ...
  if (spec_node->typ == PASCAL_T_RANGE_TYPE) {
    ...
    if (type_info != NULL) {
      ...
      char *upper_str = serialize_expr_to_string(
          spec_node->child != NULL ? spec_node->child->next : NULL);
  ...
  if (spec_node->typ == PASCAL_T_ARRAY_TYPE) {
    if (type_info != NULL) {
      ...
      for (ast_t *dim = child; dim != NULL && dim != element_node;
           dim = dim->next) {
        has_dimensions = 1;
        break;
      }

      // If no dimensions specified, it's an open array (e.g., "array of
      // string")
      if (!has_dimensions) {
      ...
      for (ast_t *dim = child; dim != NULL && dim != element_node;
        ...
        if (dim->typ == PASCAL_T_RANGE_TYPE) {
          ...
          /* Extract lower and upper bound strings, handling various AST
           * structures:
          ...
          if (lower_str != NULL && upper_str != NULL) {
            if (dims_builder.head == NULL) {
              ...
              if (lower_ok)
                start_val = lower_val;
              else if (resolve_const_expr_from_sections(lower_str,
                                                        &start_val) != 0)
                start_val = atoi(lower_str);
              if (upper_ok)
                end_val = upper_val;
              else if (resolve_const_expr_from_sections(upper_str, &end_val) !=
                       0)
          ...
          // array[boolean]) This is NOT an open array - it's an array indexed
          // by an enum or subrange type
          ...
          if (dim_name != NULL) {
            ...
            if (is_first_dim) {
              ...
              if (strcasecmp(dim_name, "boolean") == 0) {
                ...
                // For other types (enums, etc.), we'll need to resolve them
                // later For now, mark as needing resolution but set safe bounds
      ...
      if (element_node != NULL) {
        if (element_node->typ == PASCAL_T_IDENTIFIER) {
          ...
          int mapped = convert_type_spec(element_node, &nested_id,
                                         &nested_record, &nested_info);
          ...
          if (nested_info.is_shortstring) {
            ...
            if (nested_info.end > 0 && type_info->element_kgpc_type == NULL) {
              type_info->element_kgpc_type = create_primitive_type_with_size(
                  SHORTSTRING_TYPE, nested_info.end + 1);
            }
            if (nested_id != NULL)
              free(nested_id);
          ...
          if (nested_info.is_array) {
            ...
            if (inner_elem != NULL) {
              ...
              /* Named element type: create array with deferred element
               * resolution */
              type_info->element_kgpc_type =
                  create_array_type(NULL, nested_info.start, nested_info.end);
              if (type_info->element_kgpc_type != NULL)
          ...
          TypeInfo nested_info = {0};
          int mapped =
              convert_type_spec(element_node, NULL, NULL, &nested_info);
    ...
    if (type_info != NULL) {
      ...
      if (elem != NULL) {
        ...
        /* Handle subrange as set element type: set of 0..31 */
          ...
          ast_t *upper =
              (lower != NULL) ? unwrap_pascal_node(lower->next) : NULL;
          long long start_value = 0;
          long long end_value = 0;
          int have_start = 0;
          int have_end = 0;
         ...
         */
          ...
          while (value != NULL) {
            if (value->typ == PASCAL_T_IDENTIFIER)
              list_builder_append(&enum_builder, dup_symbol(value),
                                  LIST_STRING);
            value = value->next;
          }
  ...
  if (spec_node->typ == PASCAL_T_FILE_TYPE) {
    if (type_info != NULL) {
      ...
      ast_t *elem = spec_node->child;
      while (elem != NULL &&
             (elem->typ == PASCAL_T_NONE || elem->typ == PASCAL_T_TYPE_SPEC))
        elem = elem->child;
  ...
  if (spec_node->typ == PASCAL_T_ENUMERATED_TYPE) {
    if (type_info != NULL) {
      ...
      while (value != NULL) {
        ...
        else if (value->typ == PASCAL_T_ASSIGNMENT && value->child != NULL &&
                 value->child->typ == PASCAL_T_IDENTIFIER) {
          ast_t *rhs = value->child->next;
          if (rhs != NULL &&
              evaluate_const_int_expr(rhs, &literal_value, 0) == 0)
            ordinal = literal_value;
          list_builder_append(&enum_builder, dup_symbol(value->child),
                              LIST_STRING);
          type_info->enum_has_explicit_values = 1;
        }
        char ordinal_buf[64];
        snprintf(ordinal_buf, sizeof(ordinal_buf), "%d", literal_value);
        list_builder_append(&enum_value_builder, strdup(ordinal_buf),
                            LIST_STRING);
  ...
  if (spec_node->typ == PASCAL_T_PROCEDURE_TYPE ||
      spec_node->typ == PASCAL_T_FUNCTION_TYPE) {
    // Note: For now, we return PROCEDURE as the type tag for both procedures
    // and functions The actual KgpcType object will be created later in the
    // semantic checker This is a temporary bridge solution until full migration
    // to KgpcType
    return PROCEDURE;
  }
  ...
  if (spec_node->typ == PASCAL_T_DISTINCT_TYPE) {
    ...
    if (target != NULL) {
      ...
      if (target != NULL) {
        ...
        int result = convert_type_spec(target, &nested_type_id, &nested_record,
                                       &nested_info);
  ...
  if (spec_node->typ == PASCAL_T_CLASS_OF_TYPE) {
     ...
     * In Pascal, when T is a class type, T is already a pointer to the class
     * record. "class of T" is semantically a reference to the class itself
     * (metaclass), but structurally it's the same as T - a pointer to the class
     * record.
     *
    ...
    if (type_info != NULL) {
      ...
      if (target != NULL) {
        ...
        if (mapped != UNKNOWN_TYPE) {
          ...
          frontend_error("Error: 'class of' requires a class type, but got %s",
                         dup);
          free(dup);
          return UNKNOWN_TYPE;
        }
 ...
 * This is the Phase 2 bridge function that creates KgpcType objects from AST
 * nodes. Returns NULL if the type cannot be converted or if symtab is needed
 * but not provided.
 */
KgpcType *convert_type_spec_to_kgpctype(ast_t *type_spec,
  ...
  if (spec_node->typ == PASCAL_T_IDENTIFIER) {
    ...
    if (type_tag != UNKNOWN_TYPE) {
      ...
      /* Preserve distinct string-family aliases needed for helper lookup and
       * overloads. */
  ...
  if (spec_node->typ == PASCAL_T_ARRAY_TYPE) {
    ...
    if (child != NULL && child->typ == PASCAL_T_RANGE_TYPE) {
      ...
      ast_t *upper = (lower != NULL) ? lower->next : NULL;
      if (lower != NULL && upper != NULL && lower->sym != NULL &&
          upper->sym != NULL) {
    ...
    KgpcType *element_type =
        convert_type_spec_to_kgpctype(element_node, symtab);
  ...
  if (spec_node->typ == PASCAL_T_FILE_TYPE) {
     ...
     * The detailed element type is tracked in the TypeAlias/TypeInfo
     * structures. */
  ...
  if (spec_node->typ == PASCAL_T_PROCEDURE_TYPE ||
    ...
    if (cursor != NULL && cursor->typ == PASCAL_T_PARAM_LIST) {
      ...
      /* Skip to parameter list if present, but stop at return type (for
...
#ifdef DEBUG_KGPC_TYPE_CREATION
      fprintf(stderr,
              "DEBUG: Looking for return type, cursor=%p, cursor->typ=%d, "
              "cursor->sym=%s, cursor->child=%p\n",
              (void *)cursor, cursor ? cursor->typ : -1,
              (cursor && cursor->sym && cursor->sym->name) ? cursor->sym->name
                                                           : "NULL",
              cursor ? (void *)cursor->child : NULL);
      if (cursor && cursor->child) {
        fprintf(stderr, "DEBUG: cursor->child->typ=%d, cursor->child->sym=%s\n",
                cursor->child->typ,
                (cursor->child->sym && cursor->child->sym->name)
                    ? cursor->child->sym->name
                    : "NULL");
      }
#endif
      ...
      while (cursor != NULL && cursor->typ != PASCAL_T_TYPE_SPEC &&
        ...
        if (cursor->typ == PASCAL_T_RETURN_TYPE) {
          if (cursor->child != NULL &&
              (cursor->child->typ == PASCAL_T_TYPE_SPEC ||
               cursor->child->typ == PASCAL_T_IDENTIFIER)) {
            cursor = cursor->child;
            break;
          }
        }
...
#ifdef DEBUG_KGPC_TYPE_CREATION
        fprintf(stderr,
                "DEBUG: Skipping node type %d (PASCAL_T_TYPE_SPEC=%d, "
                "PASCAL_T_IDENTIFIER=%d)\n",
      ...
      if (cursor == NULL) {
        ...
        while (scan != NULL) {
          ...
          if (node != NULL) {
            ...
            } else if (node->typ == PASCAL_T_TYPE_SPEC ||
        ...
        if (cursor->typ == PASCAL_T_TYPE_SPEC) {
          ...
          if (ret_type_name != NULL) {
            ...
            if (ret_tag != UNKNOWN_TYPE) {
              ...
              HashNode_t *type_node = NULL;
              if (symtab != NULL &&
                  FindSymbol(&type_node, symtab, ret_type_name) != 0 &&
  ...
  if (spec_node->typ == PASCAL_T_INTERFACE_TYPE) {
    struct RecordType *record =
        convert_interface_type_ex(NULL, spec_node, NULL);
    if (record != NULL) {
  ...
  if (spec_node->typ == PASCAL_T_CLASS_TYPE) {
    /* Note: We can't get the class name here because we're inside the type
     * spec, not the type decl. For now, we return NULL to let the class be
     * handled by the legacy path. The class's RecordType will be properly
     * populated during semantic checking. */
    return NULL;
  }
  ...
  if (spec_node->typ == PASCAL_T_CLASS_OF_TYPE) {
    /* Class reference type (class of TMyClass).
     * This is structurally the same as the target class type - a pointer to the
     * class record. Return NULL here to let it be handled by the
     * convert_type_spec path which creates the appropriate pointer type. */
    return NULL;
  }
