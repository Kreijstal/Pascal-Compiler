...
GenericTypeDecl *generic_registry_add_decl(const char *name, char **type_params,
  ...
  if (decl != NULL && decl->type == TREE_TYPE_DECL &&
    ...
    generic->record_template =
        decl->tree_data.type_decl_data.info.generic.record_template;
  }
...
GenericTypeDecl *generic_registry_find_decl(const char *name) {
  for (GenericTypeDecl *decl = g_generic_registry.generic_decls; decl != NULL;
       decl = decl->next) {
...
char *generic_mangle_name(const char *generic_name, char **concrete_types,
                          int num_types) {
...
generic_registry_add_specialization(const char *generic_name,
                                    char **concrete_types, int num_types) {
  // Check if this specialization already exists
  GenericSpecialization *existing = generic_registry_find_specialization(
      generic_name, concrete_types, num_types);
  if (existing != NULL) {
    return existing;
  }
...
GenericSpecialization *
generic_registry_find_specialization(const char *generic_name,
                                     char **concrete_types, int num_types) {
  for (GenericSpecialization *spec = g_generic_registry.specializations;
...
KgpcType *generic_substitute_type_parameter(KgpcType *type,
                                            const char *param_name,
                                            KgpcType *concrete_type) {
  ...
  if (alias != NULL) {
    if ((alias->alias_name != NULL &&
         strcasecmp(alias->alias_name, param_name) == 0) ||
        (alias->target_type_id != NULL &&
         strcasecmp(alias->target_type_id, param_name) == 0) ||
        (alias->pointer_type_id != NULL &&
         strcasecmp(alias->pointer_type_id, param_name) == 0) ||
        (alias->array_element_type_id != NULL &&
         strcasecmp(alias->array_element_type_id, param_name) == 0) ||
        (alias->set_element_type_id != NULL &&
         strcasecmp(alias->set_element_type_id, param_name) == 0) ||
        (alias->file_type_id != NULL &&
         strcasecmp(alias->file_type_id, param_name) == 0)) {
  ...
  case TYPE_KIND_ARRAY:
    if (type->info.array_info.element_type != NULL) {
      ...
      result->info.array_info.element_type = substituted;
               ...
               strcasecmp(type->info.array_info.element_type_id, param_name) ==
                   0) {
  ...
  case TYPE_KIND_PROCEDURE:
    if (type->info.proc_info.return_type != NULL) {
      ...
      result->info.proc_info.return_type = substituted;
               ...
               strcasecmp(type->info.proc_info.return_type_id, param_name) ==
                   0) {
      KgpcType *substituted = kgpc_type_clone_shallow_owned(concrete_type);
      if (substituted == NULL) {
        destroy_kgpc_type(result);
