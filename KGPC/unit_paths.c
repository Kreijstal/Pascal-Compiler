...
static char *duplicate_path(const char *path) {
  ...
  if (dup == NULL)
    fprintf(stderr,
            "Error: Memory allocation failed while duplicating path '%s'\n",
            path);
...
static char *build_candidate_unit_path(const char *dir, const char *unit_name) {
  ...
  int written =
      snprintf(candidate, sizeof(candidate), "%s/%s.p", dir, unit_name);
  if (written > 0 && written < (int)sizeof(candidate) &&
      file_is_readable(candidate))
...
bool unit_search_paths_set_user(UnitSearchPaths *paths,
...
bool unit_search_paths_add_unit_path(UnitSearchPaths *paths, const char *path) {
  ...
  if (paths->unit_path_count >= MAX_UNIT_SEARCH_PATHS) {
    fprintf(stderr,
            "Warning: Maximum unit search paths (%d) exceeded. "
...
char *unit_search_paths_resolve(const UnitSearchPaths *paths,
  ...
  const char *const *include_paths =
      pascal_frontend_get_include_paths(&include_count);
  ...
  if (!paths->disable_vendor_units) {
    ...
    if (source_root != NULL) {
      char buffer[PATH_MAX];
      int written =
          snprintf(buffer, sizeof(buffer), "%s/KGPC/Units", source_root);
      if (written > 0 && written < (int)sizeof(buffer)) {
