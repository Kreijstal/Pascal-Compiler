...
static void test_deserialize_long_identifier_tokens(void) {
    ...
    serialized = format_text("%d %d %s\n%d 7\n",
        EXPR_TYPECAST, UNKNOWN_TYPE, identifier, EXPR_INUM);
    if (serialized == NULL) {
        free(identifier);
        return;
    }
    ...
    if (expr != NULL) {
        ...
        TEST_ASSERT(strcmp(expr->expr_data.typecast_data.target_type_id, identifier) == 0);
...
TEST_LIST = {
    { "test_deserialize_long_identifier_tokens", test_deserialize_long_identifier_tokens },
    { "test_deserialize_long_quoted_string", test_deserialize_long_quoted_string },
    { NULL, NULL }
};
