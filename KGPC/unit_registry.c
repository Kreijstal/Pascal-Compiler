...
static const char
    *registry[MAX_UNIT_NAMES]; /* index 0 unused (means "no unit") */
static int count = 0;
...
void unit_registry_add_dep(int unit_idx, int dep_idx) {
  if (unit_idx <= 0 || unit_idx >= MAX_UNIT_NAMES)
    return;
  if (dep_idx <= 0 || dep_idx >= MAX_UNIT_NAMES)
    return;
...
int unit_registry_count(void) { return count; }
