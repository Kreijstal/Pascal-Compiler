...
static void map_add_secondary(SubprogramEntry **buckets, const char *key,
                              Tree_t *subprogram) {
...
static void map_init(SubprogramMap *map) { memset(map, 0, sizeof(*map)); }
...
static void map_add(SubprogramMap *map, const char *mangled_id,
                    Tree_t *subprogram) {
  if (mangled_id == NULL || subprogram == NULL)
    return;
...
static Tree_t *map_find(SubprogramMap *map, const char *mangled_id) {
  ...
  char *lookup_id =
      pascal_identifier_lower_buf(mangled_id, stack_buf, sizeof(stack_buf));
  if (lookup_id == NULL)
    return NULL;
...
static void mark_class_constructors_from_types(ListNode_t *type_list,
                                               SubprogramMap *map);
static void mark_class_constructors_from_subprograms(ListNode_t *sub_list,
                                                     SubprogramMap *map);
static void mark_subprograms_by_id(SubprogramMap *map, const char *id);
static void mark_class_methods_by_owner(ListNode_t *sub_list,
                                        const char *owner_class,
                                        const char *method_name,
                                        SubprogramMap *map);
static void mark_var_initializer_calls(ListNode_t *var_list,
                                       SubprogramMap *map);
...
static int lookup_id_parse_owner_method(const char *lookup_id, char *owner_buf,
                                        size_t owner_buf_sz, char *method_buf,
                                        size_t method_buf_sz);
static void scan_used_subprogram_bodies(ListNode_t *sub_list,
                                        SubprogramMap *map);

static Tree_t *find_record_type_decl(ListNode_t *type_list,
                                     const char *type_id) {
...
static const char *
find_interface_delegate_target_name(const struct RecordType *record,
  ...
  for (ListNode_t *node = record->method_templates; node != NULL;
       node = node->next) {
...
static void mark_interface_dispatch_target(ListNode_t *type_list,
  ...
  while (cur_class != NULL && cur_record != NULL) {
    ...
    if (cur_record->parent_class_name == NULL)
      break;
    Tree_t *parent_tree =
        find_record_type_decl(type_list, cur_record->parent_class_name);
    if (parent_tree == NULL || parent_tree->type != TREE_TYPE_DECL ||
...
static void mark_subprogram_recursive(Tree_t *sub, SubprogramMap *map) {
  ...
  if (body == NULL) {
    ...
    /* If this forward decl has a cname_override (e.g., [external name
     * 'FPC_FINALIZE']), also mark the implementation that shares the same
     * alias. */
...
/* Helper to check if a pointer looks valid (not garbage from uninitialized
 * union members) */
...
static void mark_expr_calls(struct Expression *expr, SubprogramMap *map) {
  ...
  /* We need to be careful accessing expr->type if expr is garbage but passes
   * is_valid_pointer */
  /* But we can't easily check validity without OS-specific calls (IsBadReadPtr
   * etc) */
  ...
  case EXPR_FUNCTION_CALL: {
    ...
    if (trace_tfplistenum_flag() &&
      ...
      struct Expression *recv =
          expr->expr_data.function_call_data.constructor_receiver_expr;
      fprintf(stderr,
              "[mark_used] ctor call lookup=%s id=%s owner=%s method=%s "
              "recv_type=%d recv_id=%s procdef=%p\n",
              ...
              (recv != NULL && recv->type == EXPR_VAR_ID &&
               recv->expr_data.id != NULL)
                  ? recv->expr_data.id
                  : "(n/a)",
              (void *)(expr->expr_data.function_call_data.call_kgpc_type != NULL
    ...
    if (lookup_id != NULL) {
      ...
      if (trace_missing_calls) {
        fprintf(
            stderr,
            "[mark_used] funccall lookup=%s id=%s owner=%s method=%s "
            "procdef=%p\n",
            lookup_id,
            expr->expr_data.function_call_data.id != NULL
                ? expr->expr_data.function_call_data.id
      ...
      if (called_sub != NULL) {
        ...
        char owner_buf[256];
        char method_buf[256];
        if (lookup_id_parse_owner_method(lookup_id, owner_buf,
                                         sizeof(owner_buf), method_buf,
                                         sizeof(method_buf)))
    ...
    if (called_sub == NULL &&
        ...
        expr->expr_data.function_call_data.call_kgpc_type->kind ==
            TYPE_KIND_PROCEDURE) {
      called_sub = expr->expr_data.function_call_data.call_kgpc_type->info
    ...
    if (is_valid_pointer(
        ...
        expr->expr_data.function_call_data.constructor_receiver_expr != NULL)
      mark_expr_calls(
          expr->expr_data.function_call_data.constructor_receiver_expr, map);
    if (is_valid_pointer(
            expr->expr_data.function_call_data.procedural_var_expr) &&
        expr->expr_data.function_call_data.procedural_var_expr != NULL)
      mark_expr_calls(expr->expr_data.function_call_data.procedural_var_expr,
                      map);
  ...
  case EXPR_SIGN_TERM:
    if (is_valid_pointer(expr->expr_data.sign_term) &&
        expr->expr_data.sign_term != NULL)
  ...
  case EXPR_MULOP:
    if (is_valid_pointer(expr->expr_data.mulop_data.left_term) &&
        expr->expr_data.mulop_data.left_term != NULL)
      mark_expr_calls(expr->expr_data.mulop_data.left_term, map);
    if (is_valid_pointer(expr->expr_data.mulop_data.right_factor) &&
        expr->expr_data.mulop_data.right_factor != NULL)
      mark_expr_calls(expr->expr_data.mulop_data.right_factor, map);
    break;

  case EXPR_ARRAY_ACCESS:
    if (is_valid_pointer(expr->expr_data.array_access_data.array_expr) &&
        expr->expr_data.array_access_data.array_expr != NULL)
      mark_expr_calls(expr->expr_data.array_access_data.array_expr, map);
    if (is_valid_pointer(expr->expr_data.array_access_data.index_expr) &&
  ...
  case EXPR_ADDR_OF_PROC: {
    ...
    if (called_sub == NULL) {
      ...
      if (plain_id != NULL &&
          (mangled_id == NULL || strcmp(plain_id, mangled_id) != 0))
...
static void mark_stmt_calls(struct Statement *stmt, SubprogramMap *map) {
  ...
  case STMT_PROCEDURE_CALL: {
    ...
    if (called_sub == NULL &&
        ...
        stmt->stmt_data.procedure_call_data.call_kgpc_type->kind ==
            TYPE_KIND_PROCEDURE) {
      called_sub = stmt->stmt_data.procedure_call_data.call_kgpc_type->info
                       .proc_info.definition;
      if (called_sub != NULL) {
        mark_subprogram_recursive(called_sub, map);
        mark_related_subprogram_overloads(called_sub, map);
  ...
  case STMT_FOR_ASSIGN_VAR:
    ...
    if (stmt->stmt_data.for_data.for_assign_type == STMT_FOR_ASSIGN_VAR) {
      if (stmt->stmt_data.for_data.for_assign_data.var_assign != NULL) {
        mark_stmt_calls(stmt->stmt_data.for_data.for_assign_data.var_assign,
                        map);
      }
      ...
      if (coll != NULL && coll->resolved_kgpc_type != NULL) {
        ...
        if (kgpc_type_is_record(ctype)) {
          ...
          if (rec != NULL && rec->type_id != NULL) {
            ...
            mark_class_methods_by_owner(NULL, rec->type_id, "GetEnumerator",
                                        map);
            ...
            for (unsigned b = 0; b < SUBPROG_MAP_BUCKETS; b++) {
              ...
              while (e != NULL) {
                ...
                if (sub != NULL && sub->type == TREE_SUBPROGRAM) {
                  ...
                  const char *mangled =
                      sub->tree_data.subprogram_data.mangled_id;
                  int is_enum_method = 0;
                  /* Check by owner_class prefix (nested enumerator owner starts
                  ...
                  if (owner != NULL && method != NULL &&
                    ...
                    is_enum_method = 1;
                  /* Also check by mangled_id prefix (e.g.
                   * "tai_aggregatetypedconst.tadeenumerator__movenext_p") */
  ...
  case STMT_TRY_EXCEPT: {
    ...
    ListNode_t *except_stmts =
        stmt->stmt_data.try_except_data.except_statements;
  ...
  while (sub_list != NULL) {
    if (sub_list->type == LIST_TREE && sub_list->cur != NULL) {
      ...
      if (sub->type == TREE_SUBPROGRAM) {
        /* Don't reset is_used if already marked (e.g., by
         * mark_program_subs_used) */

        const char *mangled_id = sub->tree_data.subprogram_data.mangled_id;
        const char *map_id = (mangled_id != NULL)
                                 ? mangled_id
        ...
        if (map_id != NULL) {
          ...
          if (existing != NULL) {
            ...
            /* Strategy: mark the forward declaration NOW if the implementation
             * gets marked */
            ...
            if (sub->tree_data.subprogram_data.statement_list != NULL) {
              ...
              /* But first, mark the forward declaration to match this one's
               * status */
              existing->tree_data.subprogram_data.is_used =
        ...
        const char *plain_id = sub->tree_data.subprogram_data.id;
        if (plain_id != NULL &&
            (mangled_id == NULL || strcasecmp(plain_id, mangled_id) != 0)) {
          map_add(map, plain_id, sub);
        }
        /* Also register cname_override (alias) so DCE can match alias
         * references. */
        const char *cname = sub->tree_data.subprogram_data.cname_override;
...
static void build_loaded_unit_subprogram_map(CompilationContext *comp_ctx,
                                             SubprogramMap *map) {
  ...
  while (sub_list != NULL) {
    if (sub_list->type == LIST_TREE && sub_list->cur != NULL) {
      ...
      if (sub->type == TREE_SUBPROGRAM &&
        ...
        mark_var_initializer_calls(sub->tree_data.subprogram_data.declarations,
                                   map);
        struct Statement *body = sub->tree_data.subprogram_data.statement_list;
        if (body != NULL)
          mark_stmt_calls(body, map);
  ...
  while (type_list != NULL) {
    if (type_list->type == LIST_TREE && type_list->cur != NULL) {
      ...
      if (type_tree->type == TREE_TYPE_DECL &&
          type_tree->tree_data.type_decl_data.id != NULL) {
        ...
        if (type_tree->tree_data.type_decl_data.kind == TYPE_DECL_RECORD)
          ...
          record_info =
              type_tree->tree_data.type_decl_data.info.alias.inline_record_type;

        if (record_info != NULL && record_type_is_class(record_info)) {
          if (trace_tfplistenum_flag() &&
              strcasecmp(type_tree->tree_data.type_decl_data.id,
                         "TFPListEnumerator") == 0) {
            fprintf(stderr, "[mark_used] type-scan hit class %s\n",
                    type_tree->tree_data.type_decl_data.id);
          }
          mark_class_methods_by_owner(
              NULL, type_tree->tree_data.type_decl_data.id, "Create", map);
        }
      }
    }
    ...
    if (sub_list->type == LIST_TREE && sub_list->cur != NULL) {
      ...
      if (sub->type == TREE_SUBPROGRAM) {
        ...
        if (owner != NULL && method != NULL &&
          ...
          mark_subprogram_recursive(sub, map);
        if (sub->tree_data.subprogram_data.subprograms != NULL)
          mark_class_constructors_from_subprograms(
              sub->tree_data.subprogram_data.subprograms, map);
      }
    }
    sub_list = sub_list->next;
...
void mark_used_functions(Tree_t *program, SymTab_t *symtab) {
  if (program == NULL || symtab == NULL || program->type != TREE_PROGRAM_TYPE)
    return;
  ...
  /* First, traverse bodies of already-used subprograms (e.g., from previous
   * call). This ensures that functions called by specialized methods are
   * discovered. */
  scan_used_subprogram_bodies(program->tree_data.program_data.subprograms,
                              &map);
  {
    CompilationContext *comp_ctx = compilation_context_get_active();
    if (comp_ctx != NULL) {
      for (int ui = 0; ui < comp_ctx->loaded_unit_count; ++ui) {
        ...
        if (unit_tree == NULL || unit_tree->type != TREE_UNIT)
          continue;
        scan_used_subprogram_bodies(unit_tree->tree_data.unit_data.subprograms,
                                    &map);
      }
    }
  }
  ...
  /* Scan typed constant and variable initializers for function references.
     These contain EXPR_ADDR_OF_PROC (e.g. @NoBeginThread) that DCE must
     preserve. Both TREE_VAR_DECL and TREE_ARR_DECL can have initializers with
     function pointer references (e.g. typed constant arrays of records with
     @handler fields). */
  mark_var_initializer_calls(program->tree_data.program_data.var_declaration,
                             &map);
  ...
  mark_class_constructors_from_types(
      program->tree_data.program_data.type_declaration, &map);
  mark_class_constructors_from_subprograms(
    ...
    if (comp_ctx != NULL) {
      for (int ui = 0; ui < comp_ctx->loaded_unit_count; ++ui) {
        ...
        if (unit_tree == NULL || unit_tree->type != TREE_UNIT)
          continue;
        mark_class_constructors_from_types(
            unit_tree->tree_data.unit_data.interface_type_decls, &map);
        mark_class_constructors_from_types(
            unit_tree->tree_data.unit_data.implementation_type_decls, &map);
        mark_class_constructors_from_subprograms(
            unit_tree->tree_data.unit_data.subprograms, &map);
      }
    }
  }
      ...
      if (sub->type == TREE_SUBPROGRAM) {
        ...
        if (mangled_id != NULL) {
          ...
          if (canonical != NULL && canonical != sub) {
            ...
            sub->tree_data.subprogram_data.is_used =
                canonical->tree_data.subprogram_data.is_used;
          }
        }
      }
    }
  ...
  fprintf(stderr,
          "DEBUG mark_used_functions: Reachability analysis complete\n");
#endif
  ...
  while (type_node != NULL) {
    if (type_node->type == LIST_TREE && type_node->cur != NULL) {
      ...
      if (type_tree->type == TREE_TYPE_DECL) {
        ...
        if (type_tree->tree_data.type_decl_data.kind == TYPE_DECL_RECORD) {
          record_info = type_tree->tree_data.type_decl_data.info.record;
        } else if (type_tree->tree_data.type_decl_data.kind ==
                   TYPE_DECL_ALIAS) {
          record_info =
              type_tree->tree_data.type_decl_data.info.alias.inline_record_type;
        }
        ...
        if (record_info != NULL && record_type_is_class(record_info)) {
          ...
          while (method_node != NULL) {
            ...
            if (method != NULL)
              lookup_id =
                  method->resolved_mangled_id != NULL
                      ? method->resolved_mangled_id
                      : (method->mangled_name != NULL ? method->mangled_name
                                                      : method->name);
            if (lookup_id != NULL) {
              ...
              if (sub == NULL && method->name != NULL) {
                mark_class_methods_by_owner(
                    ...
                    type_tree->tree_data.type_decl_data.id, method->name, map);
          ...
          if (record_info->num_interfaces > 0 &&
            ...
            for (int iidx = 0; iidx < record_info->num_interfaces; iidx++) {
              ...
              if (iface_name == NULL || class_id == NULL)
                continue;
              ...
              while (search != NULL) {
                if (search->type == LIST_TREE && search->cur != NULL) {
                  ...
                  if (st->type == TREE_TYPE_DECL &&
                      st->tree_data.type_decl_data.id != NULL &&
                      strcasecmp(st->tree_data.type_decl_data.id, iface_name) ==
                          0) {
                    if (st->tree_data.type_decl_data.kind == TYPE_DECL_RECORD)
                      iface_record = st->tree_data.type_decl_data.info.record;
                    else if (st->tree_data.type_decl_data.kind ==
                             TYPE_DECL_ALIAS)
                      iface_record = st->tree_data.type_decl_data.info.alias
                                         .inline_record_type;
                    break;
                  }
                }
              ...
              /* Use method_templates for interfaces (methods is NULL for
               * interfaces) */
              ...
              if (imethod_list != NULL) {
                ...
                while (imethod != NULL) {
                  ...
                  if (use_method_templates) {
                    struct MethodTemplate *mt =
                        (struct MethodTemplate *)imethod->cur;
                    if (mt != NULL)
                      method_name = mt->name;
                  } else {
...
static void mark_subprograms_by_id(SubprogramMap *map, const char *id) {
  ...
  char stack_buf[PASCAL_ID_STACK_MAX];
  char *lower_id =
      pascal_identifier_lower_buf(id, stack_buf, sizeof(stack_buf));
  if (lower_id == NULL)
    return;
      ...
      while (e != NULL) {
        ...
        if (sub != NULL && sub->type == TREE_SUBPROGRAM) {
          ...
          if (!match && sub_mangled != NULL &&
              ...
              strncasecmp(sub_mangled + owner_len + 2, method_name,
                          method_len) == 0)
            match = 1;
          if (match)
            mark_subprogram_recursive(sub, map);
  ...
  for (ListNode_t *node = sub_list; node != NULL; node = node->next) {
    ...
    if (sub_mangled != NULL) {
      if (strncasecmp(sub_mangled, owner_class, owner_len) == 0 &&
          sub_mangled[owner_len] == '_' && sub_mangled[owner_len + 1] == '_' &&
          strncasecmp(sub_mangled + owner_len + 2, method_name, method_len) ==
              0) {
    ...
    if (trace_tfplistenum) {
      fprintf(stderr,
              "[mark_used] scan owner=%s method=%s sub_owner=%s sub_method=%s "
              "mangled=%s body=%d used=%d\n",
              owner_class, method_name,
...
static int lookup_id_parse_owner_method(const char *lookup_id, char *owner_buf,
  ...
  while (method_start[method_len] != '\0' && method_start[method_len] != '_')
    method_len++;
  if (owner_len + 1 > owner_buf_sz || method_len + 1 > method_buf_sz ||
      method_len == 0)
    return 0;
