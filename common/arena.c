...
void arena_set_global(arena_t *arena) { g_global_arena = arena; }

arena_t *arena_get_global(void) { return g_global_arena; }
...
void *arena_alloc(arena_t *arena, size_t size) {
  if (!arena)
    return malloc(size); // Fallback if no arena provided
...
void arena_reset(arena_t *arena) {
  ...
  /* Free all blocks except the first one (or keep them and reset 'used'?)
     ...
     But for now, let's just free everything to be safe and allocate a fresh
     one.
  */
