...
def _add_pp_pas_bootstrap_test():
    ...
    def test_pp_pas_bootstrap(self):
        ...
        # Generate msgtxt.inc and msgidx.inc from errore.msg if they don't exist.
        # These are normally generated by the FPC build system's msg2inc tool;
        # we use a Python reimplementation instead.
        ...
        if not os.path.isfile(msgtxt_inc) or not os.path.isfile(msgidx_inc):
            msg2inc_script = os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "..", "scripts", "msg2inc.py"
            )
            msg_file = os.path.join(compiler_dir, "msg", "errore.msg")
            msg_out_base = os.path.join(compiler_dir, "msg")
            subprocess.run(
                [sys.executable, msg2inc_script, msg_file, msg_out_base, "msg"],
                check=True,
            )
