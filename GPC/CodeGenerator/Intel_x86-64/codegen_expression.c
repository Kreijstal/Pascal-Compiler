...
static void serialize_expression(FILE *fp, struct Expression *expr) {
...
ListNode_t *codegen_pass_arguments(ListNode_t *args, ListNode_t *inst_list, CodeGenContext *ctx, HashNode_t *proc_node)
{
    ...
    FILE *fp = fopen("serializer_log.txt", "a");
    if (fp != NULL) {
        fprintf(fp, "--- New Call ---\n");
        ListNode_t *cur_arg = args;
        while(cur_arg != NULL)
        {
            serialize_expression(fp, (struct Expression *)cur_arg->cur);
